...
def test_run_xai_oauth_login_from_setup_does_not_hijack_active_provider(
    ...
    `active_provider`), so `renco setup tts` OAuth login hijacked the main
    chat provider.
    """
    ...
    monkeypatch.setattr("renco_cli.auth._is_remote_session", lambda: True)

    from renco_cli.auth import is_source_suppressed, suppress_credential_source
    from renco_cli.setup import _run_xai_oauth_login_from_setup
