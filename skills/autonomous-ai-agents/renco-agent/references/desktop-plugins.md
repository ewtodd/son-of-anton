...
Write plugins for the Renco desktop app: statusbar items, layout panes,
...
system (`~/.renco/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$RENCO_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$RENCO_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Renco desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$RENCO_HOME/desktop-plugins/` (usually
  `~/.renco/desktop-plugins/`).

## How to Run

1. Create `$RENCO_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.renco/...` by default, or `~/.renco/profiles/<profile>/...` under a
...
The ONLY import surface is `@renco/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Renco
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.renco/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@renco/plugin-sdk`, `react`, and
