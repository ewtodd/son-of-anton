...
Covers:
 - Normal path (tokens already in Son of Anton auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Son of Anton
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `son-of-anton auth openai-codex`. The old
...
@pytest.fixture()
def son_of_anton_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Son of Anton auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(son_of_anton_auth_only_env):
    """openai-codex appears when tokens are already in Son of Anton auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Son of Anton
    auth store."""
    son_of_anton_home = tmp_path / ".son-of-anton"
    son_of_anton_home.mkdir()
