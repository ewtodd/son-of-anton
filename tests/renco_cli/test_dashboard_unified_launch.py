...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "renco_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # RENCO_HOME.  For a standard install (RENCO_HOME unset) that root is
        # the platform-native default (~/.renco), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from renco_constants import get_default_renco_root
        assert env.get("RENCO_HOME") == str(get_default_renco_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (RENCO_DESKTOP=1) must NOT
