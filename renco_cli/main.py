...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    pipes run ``renco … chat -q`` with stdio on a pipe. This
...
def _workspace_root(dir: Path) -> Path:
    ...
    consistent — a single helper prevents the checks from diverging if
    someone accidentally creates a sub-package lockfile (e.g. running
    ``npm install`` in the wrong directory).
    """
    if (
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    # to exist first made every prebuilt connection hard-exit before it ever
    # got a chance to try the bundled entry.js it already has. See #56665.
    if not tui_dev:
        if ext_dir:
            p = Path(ext_dir)
    ...
    #    Existing behaviour runs npm from the workspace root.  Termux scopes
    #    the install to ui-tui so launch does not pull web dependencies into
    #    the hot path.
    did_install = False
    ...
    if (
        ...
        # --workspace ui-tui scopes the install to the TUI workspace.
        # See #38772.
...
def _apply_tui_python_env(env: dict) -> None:
    """Seed/repair Python-related env vars shared by CLI and TUI launches."""
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with Renco — chat and gateway. The skills_sync module is
...
def _resolve_use_tui(args) -> bool:
    ...
    jobs and pipelines run ``renco … chat -q`` with stdout on a pipe;
    booting the Ink TUI there hits its no-TTY bail-out, which prints a
    resume hint and exits 0 — the job silently never runs. A user who
...
# argparse wiring and test monkeypatches on renco_cli.main.<name> keep
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        # Read or write a profile's description.
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    command modules for model, fallback, migrate, bundles, plugins, etc.
    even though the TUI immediately execs Node. On Termux only, parse the
