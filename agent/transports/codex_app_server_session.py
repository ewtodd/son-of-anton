...
Owns one Codex thread per Son of Anton session. Drives `turn/start`, consumes
...
# Son of Anton' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SON_OF_ANTON_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Son of Anton/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Son of Anton session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SON_OF_ANTON_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SON_OF_ANTON_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="son-of-anton",
            client_title="Son of Anton Agent",
            client_version=_get_son_of_anton_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        into Son of Anton' messages shape.
        ...
        # supports rich content but Son of Anton' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Son of Anton' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own son-of-anton-tools callback we
            # auto-accept — the user already approved Son of Anton' tools
            ...
            if server_name == "son-of-anton-tools":
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Son of Anton
        approval-mode/timeout logic. The Son of Anton-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Son of Anton approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Son of Anton approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Son of Anton returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Son of Anton approval mode/timeout resolution
    ...
    # "prompt expired"; the Son of Anton-side messaging already distinguishes them.
    return "decline"
...
def _get_son_of_anton_version() -> str:
    """Best-effort Son of Anton version string for codex's userAgent line."""
    try:
