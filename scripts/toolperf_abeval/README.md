...
batch (tracker: [#77056](https://github.com/ewtodd/renco/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Renco home, same model, same tasks, same reps.
...
1. Create a dedicated Renco home with credentials for the models under test:
   ...
   integration at it; no Renco observability plugin needs to be enabled.
