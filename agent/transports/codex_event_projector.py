"""Projects codex app-server events into Son of Anton' messages list.

The translator that lets Son of Anton' memory/skill review keep working under the
...
Son of Anton' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Son of Anton
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Son of Anton' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Son of Anton stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Son of Anton already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Son of Anton' messages store text only.
        text_parts: list[str] = []
