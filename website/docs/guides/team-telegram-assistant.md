...
This tutorial walks you through setting up a Telegram bot powered by Renco Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Renco Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.renco/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Renco itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Renco Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_renco_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Renco Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Renco Gateway...
...
# View live logs
journalctl --user -u renco-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `renco gateway install` to update the plist.
:::
...
```bash
renco gateway stop && renco gateway start
```
...
renco pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.renco/.env`:
...
Control how much detail the bot shows when using tools. In `~/.renco/config.yaml`:
...
Customize how the bot communicates by editing `~/.renco/SOUL.md`:

For a full guide, see [Use SOUL.md with Renco](/guides/use-soul-with-renco).
...
```bash
# From the CLI
renco cron list          # View all scheduled jobs
renco cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u renco-gateway` (Linux) or `~/.renco/logs/gateway.log` (macOS) |
| Cron job output | `~/.renco/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.renco/cron/jobs.json` |
| Pairing data | `~/.renco/pairing/` |
| Session history | `~/.renco/sessions/` |
