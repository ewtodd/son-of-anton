...
Renco Agent includes a full browser automation toolset with multiple backend options:
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `renco setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the browser provider via `renco model` or `renco tools`.
:::
...
**This is the default browser mode**: when `browser.backend` is unset and the `browser-use` CLI is runnable (installed, or available through `uvx`), the agent gets the single `browser_exec` tool. If the CLI can't run, Renco falls back to the built-in browser tools automatically.

The mode is a **driver** that composes with your configured browser backend: it drives your local Chrome, a Nous-subscription cloud browser, Browserbase, Firecrawl, or Browser Use cloud browsers — whichever browser source is selected in `renco tools` → Browser Automation. The one exception is Camofox, which has no CDP endpoint for the harness to attach to; Camofox setups automatically keep the built-in browser tools.
...
When a cloud provider is configured, Renco auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`renco setup tools → Browser Automation`
...
Or configure via `renco tools` → Browser Automation → Camofox.

Camofox is selected like any other browser backend: pick **Camofox** in `renco tools` → Browser Automation, which writes `browser.cloud_provider: camofox` to `config.yaml`. `CAMOFOX_URL` is only the server address — setting it no longer selects the backend by itself once a browser selection exists (never-configured setups still autodetect it).

#### Persistent browser sessions

By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.renco/config.yaml`:
...
Then fully restart Renco so the new config is picked up.

:::warning Nested path matters
Renco reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Renco ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Renco silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Renco profile, so different Renco profiles get different browser profiles (profile isolation).

##### What Renco does not do
- It does not force persistence on the Camofox server. Renco only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Renco cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Renco and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Renco after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Renco derives the stable `userId` from the profile-scoped directory `~/.renco/browser_auth/camofox/` (or the equivalent under `$RENCO_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Renco profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Renco to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Renco uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Renco calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Renco skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Renco does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Renco issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Renco adopts the most recently created one in that group.
3. Otherwise, Renco adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Renco falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — Renco does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Renco to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Renco generates a per-task `session_key` (`task_<id>`) — Renco will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Renco can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Renco runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Renco automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.
...
Renco drives Lightpanda through `agent-browser` over CDP, the same way it drives local Chrome.

**Automatic Chrome fallback.** Lightpanda doesn't yet cover everything Chrome does, so the integration is non-disruptive: Lightpanda handles the actions it supports, and Renco transparently retries on Chrome for anything it doesn't. The supported set covers the core agent workflow — navigate, snapshot, click, type, scroll, back, press, and eval. Screenshots also fall back to Chrome because Lightpanda has no graphical renderer; `browser_vision` is pre-routed straight to Chrome for the same reason.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Renco browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Renco from the terminal (`renco` or `renco chat`) and issue `/browser connect` there.
:::
...
brave-browser \
  ...
  --user-data-dir=$HOME/.renco/chrome-debug \
  --no-first-run \
...
Then launch the Renco CLI and run `/browser connect`.
...
If Renco runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Renco itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Renco talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Renco MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Renco can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Extra Chromium launch flags (comma- or newline-separated). Renco auto-injects
...
# Renco doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `renco config set toolsets '["renco-cli", "browser"]'`.
:::
...
```
Type "renco agent" into the search field @e3
```
...
Screenshots are stored in `~/.renco/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.renco/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Renco automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
...
- **Snapshot size** — large pages may be truncated or LLM-summarized at 15,000 characters (matching `web_extract`); the complete snapshot is saved to `~/.renco/cache/web/` and the output points at it for `read_file` paging
