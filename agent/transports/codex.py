...
# Wire-name used when Renco keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Renco's configured provider
# (Firecrawl / Tavily / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "renco_web_search"
...
# rename on the wire (renco_<name>), map back in normalize_response so
# Renco dispatch is unaffected.
_OPENCODE_RESERVED_TOOL_NAMES = ("web_search", "search_files")
_RESERVED_TOOL_ALIAS_PREFIX = "renco_"
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Renco web providers.
        #
        ...
        #    keep Renco dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``renco_web_search`` so Grok cannot hijack the name. The alias
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Renco dispatches
                ...
                # (renco_web_search / renco_search_files, #85589).
