...
1. Unit contract on :func:`son_of_anton_cli._subprocess_compat.noninteractive_git_env`.
...
class TestNoninteractiveGitEnv:
    ...
    def test_defaults_to_process_environ_copy(self, monkeypatch):
        monkeypatch.setenv("SON_OF_ANTON_TEST_SENTINEL", "xyz")
        env = noninteractive_git_env()
        assert env["SON_OF_ANTON_TEST_SENTINEL"] == "xyz"
        assert env["GIT_TERMINAL_PROMPT"] == "0"
...
class _BasicAuthChallenge(http.server.BaseHTTPRequestHandler):
    def _challenge(self):
        ...
        self.send_header("WWW-Authenticate", 'Basic realm="son-of-anton-test"')
