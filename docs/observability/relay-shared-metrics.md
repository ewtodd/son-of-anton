...
Renco includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Renco and does not require a Renco observability plugin. Renco remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Renco execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `renco-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Renco `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `RENCO_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `RENCO_NEMO_RELAY_ATOF_*` and `RENCO_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Renco does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Renco requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Renco installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Renco process so configured interceptors can
...
Collection remains off unless Renco policy enables it:
...
opt-in. Set `RENCO_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Renco does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Renco
...
retain the same `session_id` and add `renco.session.segment` plus
`renco.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Renco
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Renco process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Renco logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Renco process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Renco
process and share the same policy boundary.

Run profiles in separate Renco processes when they require different trust
...
Renco core owns one Relay host and one isolated Relay session scope per Renco
session. Core lifecycle producers use
`renco_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Renco plugin registration. Shared-metrics marks must
...
```text
Renco turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Renco shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Renco sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Renco used for the logical call, such as
...
New calls use `renco.model_route.count`. The previous
`renco.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `renco.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Renco
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Renco processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `renco.task_run`, parented to
the owning Renco session. The start counter contains only bounded execution
...
provider attempts for the same Renco API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Renco tool-call
...
cancellations. Active task ownership follows the task ID if Renco rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`renco.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Renco
...
tool or plugin names. Renco does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `renco.tool_approval` marks and
...
Successful skill mutations emit `renco.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `renco.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Renco derives reuse and patch-generation
...
```text
$RENCO_HOME/telemetry/shared_metrics/metrics.sqlite3
$RENCO_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Renco version,
...
schema field name, its current scope is one `RENCO_HOME`, so it is more
...
Deleting `$RENCO_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `RENCO_HOME`. To reset it, stop Renco
processes and remove `$RENCO_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Renco CLI turn against the deterministic local model server:
