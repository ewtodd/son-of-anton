...
import son_of_anton_cli.auth as auth
...
def son_of_anton_home(tmp_path, monkeypatch):
    """Point SON_OF_ANTON_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.son-of-anton/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, son_of_anton_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, son_of_anton_home, windows_default_encoding):
        ...
        assert not (son_of_anton_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, son_of_anton_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, son_of_anton_home):
        auth_path = son_of_anton_home / "auth.json"
...
# --- sibling readers of the same ~/.son-of-anton/auth.json in other modules -------
#
# _load_auth_store lives in son_of_anton_cli/auth.py, but several other modules read
# the same ~/.son-of-anton/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, son_of_anton_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(son_of_anton_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nous_provider_reads_non_ascii_store(self, son_of_anton_home, windows_default_encoding, monkeypatch):
        ...
        # SON_OF_ANTON_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", son_of_anton_home / "auth.json")
    ...
    def test_read_shared_nous_state_reads_non_ascii_store(
        ...
        """son_of_anton_cli.auth._read_shared_nous_state must read a non-ASCII store.
        ...
        monkeypatch.setenv("SON_OF_ANTON_SHARED_AUTH_DIR", str(shared_dir))
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, son_of_anton_home, monkeypatch, windows_default_encoding
    ):
        """son_of_anton_cli.main._has_any_provider_configured reads auth.json.
        ...
        # son_of_anton_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_nous_state(
        self, son_of_anton_home, windows_default_encoding
    ):
