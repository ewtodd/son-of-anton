...
Edit with `son-of-anton config edit` or `son-of-anton config set section.key value`.
Full reference: https://son-of-anton.nousresearch.com/docs/user-guide/configuration
...
`son-of-anton config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `son-of-anton tools` (interactive) or `son-of-anton tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_SON_OF_ANTON_CORE_TOOLS` is the default bundle most platforms inherit).
