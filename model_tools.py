...
def _is_dispatcher_owned_worker() -> bool:
    """False when RENCO_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_renco_home() under the active profile.
...
#   - cli.py, renco_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("RENCO_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by RENCO_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like renco-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("renco-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (renco-*) include _RENCO_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("renco-")
    ...
    # window). Core Renco tools (toolsets._RENCO_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from renco_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to renco-agent's coercion layer.
