"""Tests for the stale-dashboard handling run at the end of ``renco update``.

``renco update`` detects ``renco dashboard`` processes left over from the
...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``renco_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``renco_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from renco_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"renco_cli.main.X\")``
    ...
    live = sys.modules.get("renco_cli.main")
    if live is None:
        live = importlib.import_module("renco_cli.main")
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m renco_cli.main dashboard"),
                    _ps_line(12345, "renco dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("renco_cli.main._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "renco-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="renco-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "renco-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "renco-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "renco-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("renco_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "renco-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "renco-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "renco-dashboard.service"],
            ["systemctl", "--user", "restart", "renco-dashboard.service"],
        ]
        assert all(call[:1] != ["sudo"] and call[:2] != ["systemctl"] for call in calls)
        find_pids.assert_not_called()
        kill.assert_not_called()
        assert "✓ restarted renco-dashboard.service" in capsys.readouterr().out
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "renco_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    `renco update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("renco_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m renco_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("renco_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["renco_cli.main"]
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(live, "_get_pid_cgroup_path",
                          return_value="/system.slice/renco-serve.service"), \
             patch.object(live, "_get_systemd_service_for_pid",
                          return_value="renco-serve.service"), \
        ...
        restart.assert_called_once_with(
            "renco-serve.service", "/system.slice/renco-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service renco-serve.service" in out
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: renco update's systemd fleet-restart loop may
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"renco-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        live = self._live()
        argv = ["renco", "dashboard", "--port", "8300"]
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("renco_cli.dashboard_procs._renco_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["renco", "dashboard", "--port", "8300"],
                ["renco", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["renco", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["renco", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["renco", "serve"]])

        assert failed == [["renco", "serve"]]
        out = capsys.readouterr().out
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from renco_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.renco/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["renco", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from renco_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "renco_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "renco_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["renco", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["renco", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["renco", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_renco_home(self):
        from renco_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/renco-home-a"
        a = ["renco", "dashboard", "--port", "8300"]
        b = ["renco", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, home),
            (2, b, home),
        ])
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.renco/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["renco", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["renco", "dashboard", "--port", "8301"]
        home = "/home/u/.renco"
        out = _filter_dashboard_respawn_candidates([
            (1, a, home),
    ...
    def test_distinct_dot_renco_homes_do_not_share_cap(self):
        from renco_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["renco", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00renco_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "renco_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``renco update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``renco_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from renco_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_cleanup_reloads_before_scanning(self):
        ...
        from renco_cli import update_cmd
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("renco_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import renco_cli._subprocess_compat as compat
        from renco_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["renco_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["renco_cli._subprocess_compat"])
            importlib.reload(sys.modules["renco_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "renco_cli._subprocess_compat" in reloaded
        assert "renco_cli.dashboard_procs" in reloaded
