...
import { saveRencoConfig } from '@/renco'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { RencoConfigRecord } from '@/types/renco'

import { setRencoConfigCache, useRencoConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: RencoConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useRencoConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<RencoConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setRencoConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="renco-terminal-font-families"
          ...
          <datalist id="renco-terminal-font-families">
