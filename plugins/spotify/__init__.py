...
``_check_spotify_available()`` — when the user has not run ``renco auth
spotify``, the tools remain registered (so they appear in ``renco
...
The Spotify auth flow (``renco auth spotify``), CLI plumbing, and docs
