...
function sessionsBotsTree() {
  ...
  registerPane('renco-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'renco-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('renco-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('renco-bots:pane')
    expect($hiddenStripTabs.get()).toContain('renco-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('renco-bots:pane')

    expect(setStripTabHidden('renco-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('renco-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('renco-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('renco-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('renco.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('renco-bots:pane')
    ...
    revealTreePane('renco-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('renco-bots:pane')
    expect(window.localStorage.getItem('renco.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'renco-bots:pane', title: 'renco-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'renco-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
