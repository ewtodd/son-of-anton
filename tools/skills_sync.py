...
Copies bundled skills from the repo's skills/ directory into ~/.renco/skills/
...
The manifest lives at ~/.renco/skills/.bundled_manifest.
"""
...
# cron, kanban workers) import this module once under the launch RENCO_HOME
# and later scope requests to a different profile via
# set_renco_home_override(). Frozen module constants would then resolve —
...
# re-resolve from the live profile-scoped RENCO_HOME on every call.
_RENCO_HOME_AT_IMPORT = RENCO_HOME
_SKILLS_DIR_AT_IMPORT = SKILLS_DIR
_MANIFEST_FILE_AT_IMPORT = MANIFEST_FILE


def _renco_home() -> Path:
    """Return the active profile's RENCO_HOME at call time."""
    configured = Path(RENCO_HOME)
    if configured != _RENCO_HOME_AT_IMPORT:
        return configured
...
# Marker file written by `renco profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.renco profile).
# When present in RENCO_HOME, sync_skills() is a no-op so neither the
# installer, `renco update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# renco_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks RENCO_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.renco/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.renco/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``renco skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `renco skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.renco/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.renco) that wrote the
    ...
    if (_renco_home() / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.renco/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `renco update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `renco skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``RENCO_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.renco/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``RENCO_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `renco skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `renco update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``renco update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``renco update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``renco skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `renco skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes RENCO_HOME/.no-bundled-skills so the
    installer, ``renco update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``renco skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = _renco_home() / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            _renco_home().mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding "
                "(`renco skills opt-out`).\n"
            ...
            changed = existed
            message = (
                "Opted back in. The next `renco update` (or `renco skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_renco_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.renco/skills/ ...")
