#!/usr/bin/env python3
"""Bridge desktop-only tools to Renco-desktop renderer events.
...
``RENCO_UI_SESSION_ID`` so the event lands on the window that owns the turn
...
def emit(event: str, payload: dict) -> bool:
    ...
    if fn is None:
        return False
    fn(get_session_env("RENCO_UI_SESSION_ID", ""), event, payload)
    return True
