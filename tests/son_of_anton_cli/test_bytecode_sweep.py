...
Bug class: the checkout's ``.py`` files change (``son-of-anton update``, manual
...
def _make_pycache(repo: Path, subdir: str = "son_of_anton_cli") -> Path:
...
def test_sweep_clears_pycache_when_checkout_changed(monkeypatch, tmp_path):
    ...
    (repo / son_of_anton_main._BYTECODE_FINGERPRINT_FILE).write_text(
        "git:refs/heads/main:" + "a" * 40, encoding="utf-8"
    )

    son_of_anton_main._sweep_stale_bytecode_if_checkout_changed()

    assert not cache.exists()
    # Stamp updated to the current fingerprint.
    recorded = (repo / son_of_anton_main._BYTECODE_FINGERPRINT_FILE).read_text(encoding="utf-8")
...
# ---------------------------------------------------------------------------
# Plugin-update sibling site: __pycache__ under ~/.son-of-anton/plugins/<name>
# ---------------------------------------------------------------------------

def test_clear_plugin_bytecode_removes_nested_caches(tmp_path):
    from son_of_anton_cli import plugins_cmd
