...
#
# When a user invokes a /skill (or /bundle), Renco expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_renco_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``RENCO_PLATFORM`` env var and
    ``RENCO_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("RENCO_PLATFORM")
            or get_session_env("RENCO_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("RENCO_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Return the effective Renco home the skill scan should be scoped to.

    A gateway session can switch between profiles that each carry their own
    ``skills.external_dirs`` (via ``set_renco_home_override``), but the
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.renco.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_renco_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.renco/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core Renco slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Renco command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's Renco home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.renco/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``renco -s <skill>`` or
    a deployment's ``RENCO_TUI_SKILLS`` env var could force-load a skill an
