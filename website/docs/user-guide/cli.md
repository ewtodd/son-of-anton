...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Renco Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Renco Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `renco setup --portal` — and you're ready to `renco chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Renco also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `renco --tui` — see the [TUI](tui.md) guide.
:::
...
renco chat --query-file prompt.txt
renco chat --query-file - < prompt.txt

# With a specific model
renco chat --model "anthropic/claude-sonnet-4"

# With a specific provider
renco chat --provider nous        # Use Nous Portal
renco chat --provider openrouter  # Force OpenRouter
...
renco chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
renco --continue             # Resume the most recent CLI session (-c)
renco --resume <session_id>  # Resume a specific session by ID (-r)
renco --resume latest        # Resume the most recent session (same as -c)
renco --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
renco chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
renco -w                         # Interactive mode in worktree
renco -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`renco -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
renco worktree list              # audit: age, size, verdict, reason per tree
renco worktree prune             # remove safe trees + delete merged branches
renco worktree prune --dry-run   # show the plan without changing anything
renco worktree prune --trees-only     # leave local branches alone
renco worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running renco session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.renco/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
The `renco plugins` commands manage native Renco plugins and portable Agent
...
renco plugins disable <plugin-name>
renco plugins update <plugin-name>
renco plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Renco currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Renco CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Renco CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `RENCO_YOLO_MODE` is on (either `renco --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Renco to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.renco/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Renco supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Renco replies |
...
```bash
renco -s renco-agent-dev,github-auth
renco chat -s github-pr-workflow -s github-auth
```

Renco loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.renco/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.renco/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Renco..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Renco also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Renco recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Renco — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Renco is working, Renco prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Renco to the background — just like any terminal process. The shell prints a confirmation:

```
Renco Agent has been suspended. Run `fg` to bring Renco Agent back.
```
...
Resume this session with:
  renco --resume 20260225_143052_a1b2c3
...
```bash
renco --continue                          # Resume the most recent CLI session
renco -c                                  # Short form
renco -c "my project"                     # Resume a named session (latest in lineage)
renco --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
renco --resume "refactoring auth"         # Resume by title
renco --resume latest                     # Resume the most recent session (same as -c)
renco --resume latest --in ./my-project   # Latest session for ./my-project's workspace
renco -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `renco sessions rename <id> <title>` from the command line. Use `renco sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Renco's SQLite state database under `~/.renco/state.db`. The database keeps:
...
Renco immediately confirms the task and gives you back the prompt:
