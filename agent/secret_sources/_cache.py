...
cache problem must never block Son of Anton startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Son of Anton home used for cache paths.

    ``home_path`` is whatever ``load_son_of_anton_dotenv()`` already resolved;
    falling back to ``$SON_OF_ANTON_HOME`` / ``~/.son-of-anton`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from son_of_anton_constants import get_son_of_anton_home

        home_path = get_son_of_anton_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<son_of_anton_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if SON_OF_ANTON_HOME is ever made traversable.
            try:
