...
``venv\\Scripts\\son-of-anton.exe`` is a launcher that runs the interpreter with the
...
The hand-off sits at the sync boundary, not at the top of ``son-of-anton update``:
...
SHIM_NAMES = ["son-of-anton.exe", "son-of-anton.exe", "son-of-anton-acp.exe", "son-of-anton-gateway.exe"]
...
def venv(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["son-of-anton", "update"])
...
def test_detects_shim_from_zipapp_main_py(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "son-of-anton.exe" / "__main__.py")])
    assert cli_main._windows_shim_in_process_chain() == venv / "son-of-anton.exe"


def test_detects_shim_from_main_module_spec_origin(venv, monkeypatch):
    fake_main = types.SimpleNamespace(
        __file__=None,
        __spec__=types.SimpleNamespace(origin=str(venv / "son-of-anton.exe")),
    )
...
def test_detects_shim_in_ancestor_chain(venv, monkeypatch):
    ...
    _fake_psutil(monkeypatch, [str(venv / "son-of-anton.exe")])
    assert cli_main._windows_shim_in_process_chain() == venv / "son-of-anton.exe"


def test_ignores_son_of_anton_exe_outside_the_project_venv(venv, monkeypatch, tmp_path):
    """A shim from some other install must never trigger a re-exec."""
    other = tmp_path / "other" / "Scripts"
    other.mkdir(parents=True)
    monkeypatch.setattr(sys, "argv", [str(other / "son-of-anton.exe"), "update"])
    _fake_psutil(monkeypatch, [str(other / "son-of-anton.exe")])
...
def test_no_shim_off_windows(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "son-of-anton.exe"), "update"])
    assert cli_main._windows_shim_in_process_chain() is None
...
def test_reexec_runs_same_args_under_venv_python(venv, monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", [str(venv / "son-of-anton.exe"), "update", "--yes"])
    calls = _capture_popen(monkeypatch)

    assert cli_main._reexec_dependency_sync_off_windows_shim() is True
    cmd, env, kwargs = calls[0]
    assert cmd == [
        str(venv / "python.exe"), "-m", "son_of_anton_cli.main", "update", "--yes",
    ]
...
def test_reexec_falls_through_when_venv_python_is_missing(venv, monkeypatch, capsys):
    ...
    assert cli_main._reexec_dependency_sync_off_windows_shim() is False
    assert "-m son_of_anton_cli.main update" not in capsys.readouterr().out
...
def test_reexec_falls_through_when_spawn_fails(venv, monkeypatch, capsys):
    ...
    assert cli_main._reexec_dependency_sync_off_windows_shim() is False
    assert "-m son_of_anton_cli.main update" in capsys.readouterr().out
...
def test_up_to_date_run_never_hands_off(venv, monkeypatch, capsys):
    ...
    The hand-off used to run before the fetch, so every ``son-of-anton update`` —
    ...
    """
    monkeypatch.setattr(sys, "argv", [str(venv / "son-of-anton.exe"), "update", "--check"])
    calls = _capture_popen(monkeypatch)
...
def test_pending_rename_filter_drops_only_our_shim_pairs():
    shims = [Path(r"C:\son-of-anton\venv\Scripts\son-of-anton.exe")]
    entries = [
        r"\??\C:\other\thing.dll", r"!\??\C:\other\thing.dll.bak",
        r"\??\C:\son-of-anton\venv\Scripts\son-of-anton.exe",
        r"!\??\C:\son-of-anton\venv\Scripts\son-of-anton.exe.old.1755624735000",
    ]
...
def test_pending_rename_filter_keeps_a_shim_pair_with_a_foreign_target():
    shims = [Path(r"C:\son-of-anton\venv\Scripts\son-of-anton.exe")]
    entries = [
        r"\??\C:\son-of-anton\venv\Scripts\son-of-anton.exe", r"!\??\C:\somewhere\else.exe",
    ]
    kept, removed = cli_main._filter_pending_shim_renames(entries, shims)
    assert removed == 0
    assert kept == entries
