// Renco Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Renco): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Renco -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Renco-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `renco photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Renco can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "renco-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Renco'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Renco allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Renco, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-renco-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Renco supplied them so a known-good
