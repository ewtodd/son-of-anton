"""Shared fixtures for the renco-agent test suite.
...
2. **Isolated RENCO_HOME.** RENCO_HOME points to a per-test tempdir so
   code reading ``~/.renco/*`` via ``get_renco_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".renco"`` instead of the canonical
   ``get_renco_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No RENCO_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox RENCO_HOME before ANY test module is imported ──────────────────
# `renco_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
...
# real `~/.renco/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes RENCO_HOME, but fixtures run
...
# the REAL Renco root — capture it BEFORE the sandbox rewires RENCO_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.renco (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("RENCO_KANBAN_HOME", "").strip()
_PRE_SANDBOX_RENCO_HOME = os.environ.get("RENCO_HOME", "")


def _renco_home_points_at_production(value: str) -> bool:
    """True when a pre-set RENCO_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    RENCO_HOME=~/.renco``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``renco_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.renco`` — the escape vector that landed pytest
    ...
    RENCO_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".renco").resolve()
    except Exception:
        return True
...
if _renco_home_points_at_production(os.environ.get("RENCO_HOME", "")):
    _SESSION_RENCO_HOME = tempfile.mkdtemp(prefix="renco-test-home-")
    os.environ["RENCO_HOME"] = _SESSION_RENCO_HOME
    atexit.register(shutil.rmtree, _SESSION_RENCO_HOME, True)
...
# like a real CLI") — which used to disarm renco_state's live-DB guard in
# the child at the same moment the child lost the RENCO_HOME redirect.
# RENCO_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# renco_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export RENCO_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["RENCO_TEST_ISOLATION"] = os.environ.get("RENCO_HOME", "") or "1"

#: RENCO_HOME as it stood when conftest was imported - i.e. before any test
...
RENCO_HOME_AT_CONFTEST_IMPORT = os.environ.get("RENCO_HOME", "")
...
# RENCO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_RENCO_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``renco_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Renco agent session inherits them and
    # renco_constants home-resolution helpers prefer them over monkeypatched
    ...
    "RENCO_SESSION_CHAT_TYPE",
    "RENCO_SESSION_THREAD_ID",
    "RENCO_SESSION_SOURCE",
    "RENCO_SESSION_KEY",
    "RENCO_GATEWAY_SESSION",
    "RENCO_CRON_SESSION",
    "_RENCO_GATEWAY",
    "RENCO_PLATFORM",
    "RENCO_MODEL",
    "RENCO_INFERENCE_MODEL",
    "RENCO_INFERENCE_PROVIDER",
    "RENCO_TUI_PROVIDER",
    "RENCO_MANAGED",
    "RENCO_MANAGED_DIR",
    "RENCO_DEV",
    ...
    "RENCO_TIMEZONE",
    "RENCO_REDACT_SECRETS",
    "RENCO_BACKGROUND_NOTIFICATIONS",
    "RENCO_EXEC_ASK",
    "RENCO_HOME_MODE",
    "RENCO_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.renco/kanban.db instead of the per-test RENCO_HOME.
    "RENCO_KANBAN_DB",
    "RENCO_KANBAN_BOARD",
    "RENCO_KANBAN_HOME",
    "RENCO_KANBAN_WORKSPACES_ROOT",
    "RENCO_KANBAN_LOGS_ROOT",
    "RENCO_KANBAN_TASK",
    "RENCO_KANBAN_WORKSPACE",
    "RENCO_KANBAN_RUN_ID",
    "RENCO_KANBAN_CLAIM_LOCK",
    "RENCO_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "renco" host.
    "RENCO_HONCHO_HOST",
    ...
    "RENCO_DASHBOARD_OAUTH_CLIENT_ID",
    "RENCO_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and RENCO_HOME to per-test tempdirs so code that
    reads ``~/.renco/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral RENCO_* vars that could change test semantics.
    for name in _RENCO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("RENCO_HONCHO_HOST", "renco")

    # 3. Redirect RENCO_HOME to a per-test tempdir. Code that reads
    #    ``~/.renco/*`` via ``get_renco_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.renco/*`` via
    #    ``Path.home() / ".renco"`` instead of ``get_renco_home()``
    #    is a bug to fix at the callsite.
    fake_renco_home = tmp_path / "renco_test"
    fake_renco_home.mkdir()
    (fake_renco_home / "sessions").mkdir()
    (fake_renco_home / "cron").mkdir()
    (fake_renco_home / "memories").mkdir()
    (fake_renco_home / "skills").mkdir()
    monkeypatch.setenv("RENCO_HOME", str(fake_renco_home))
    ...
    # renco_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("RENCO_TEST_ISOLATION", str(fake_renco_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("RENCO_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. renco_state computes ``DEFAULT_DB_PATH = get_renco_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from renco_state import ...``) that
    ...
    renco_state_mod = sys.modules.get("renco_state")
    if renco_state_mod is not None and hasattr(renco_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            renco_state_mod, "DEFAULT_DB_PATH", fake_renco_home / "state.db"
        )
    ...
    monkeypatch.setenv("RENCO_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.renco/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import renco_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path RENCO_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_renco_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from renco_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.renco. This autouse
...
# allow-list because test-level fixtures legitimately move RENCO_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session RENCO_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``RENCO_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Renco root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_RENCO_HOME and not _renco_home_points_at_production(
        _PRE_SANDBOX_RENCO_HOME
    ):
        # RENCO_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_renco_root().resolve()
    # No pre-existing RENCO_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".renco").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.renco`` captured at import time. Hermetic tests that legitimately
    move RENCO_HOME to sibling tempdirs are unaffected.

    Only patches when ``renco_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("renco_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import renco_cli.kanban_db`
...
# ``renco_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Renco root. This fixture wires the
# test-side knobs:
...
#     installs where RENCO_HOME is not ~/.renco) into the guard's
...
# so subprocess children that import renco_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("renco_state")
    ...
    if _PRE_SANDBOX_RENCO_HOME and not _renco_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_RENCO_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Renco home override redirects every later
    # ``get_renco_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from renco_constants import get_renco_home_override, set_renco_home_override

        if get_renco_home_override() is not None:
            set_renco_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal renco config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``renco-gateway`` process
...
# correlated with a ``tests/renco_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> renco-gateway``
...
def _wal_is_usable() -> bool:
    """True when Renco will actually put a database into WAL mode here.

    Renco refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Renco can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Renco managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``renco_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_renco_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_renco_home`` fixture
    redirects ``RENCO_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.renco/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``renco_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["RENCO_VOICE_TTS"] = "1"``. The
...
#      ``renco_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``renco_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``renco_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``renco_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Renco runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.renco``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Renco falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Renco uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart renco-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting renco/python patterns are also blocked.
    """
    ...
    def _matches_renco_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _RENCO_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_renco_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch renco-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m renco_cli.main".
                if (
                    "renco" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live renco-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting renco/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `renco update` (or the
        # equivalent `python -m renco_cli.main update`).  These commands
        ...
        if "update" in low and (
            # renco update / renco update --gateway / setsid bash -c ... renco update
            ("renco" in low and "update" in low.split())
            or
            # python -m renco_cli.main update --gateway
            ("renco_cli" in low and "update" in low.split())
            or
            # venv/bin/renco update  (absolute path variant used in tests)
            (".venv/bin/renco" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`renco update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_RENCO_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import renco_cli.voice as _voice
    except Exception:
