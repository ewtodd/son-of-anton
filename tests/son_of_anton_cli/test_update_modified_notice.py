"""Guard: every `son-of-anton update` path that reports user-modified skills must
also tell the user how to find them.

`son-of-anton update` keeps (does not overwrite) bundled skills the user edited and
prints a ``~ N user-modified (kept)`` count. There are two independent update
code paths in ``son_of_anton_cli/main.py`` that print this notice (the git-pull path
in ``_cmd_update_impl`` and the unpack/install path). Both must point the user
at ``son-of-anton skills list-modified`` so the count is actionable — otherwise,
...
import son_of_anton_cli.update_cmd as update_mod


_COUNT_RE = re.compile(r"user-modified \(kept\)")
_HINT_RE = re.compile(r"son-of-anton skills list-modified")


def _source_lines() -> list[str]:
    # The update pipeline was extracted to son_of_anton_cli/update_cmd.py
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`son-of-anton skills list-modified` within the following lines — the "
