...
``gateway run --replace`` only kills same-RENCO_HOME PID-file holders.
...
def test_lock_conflict_names_owning_profile(adapter):
    ...
    existing = {
        ...
        "renco_home": "/opt/data/profiles/lead-gen-outreach",
    }
    ...
    assert adapter._fatal_error_message == (
        ...
        "(renco --profile lead-gen-outreach gateway stop)."
    )
    assert adapter._fatal_error_retryable is True
    assert adapter._fatal_error_code == "telegram-bot-token_lock"


def test_lock_conflict_infers_profile_from_legacy_renco_home(adapter):
    """Locks written before the profile field existed still attribute via renco_home."""
    existing = {
        "pid": 559,
        "start_time": 123,
        "renco_home": "/opt/data/profiles/lead-gen-outreach",
