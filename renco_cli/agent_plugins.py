...
supported components into records consumed by Renco' existing skill and MCP
...
def _translate_remote(config: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    The returned record targets Renco' existing URL-based MCP runtime.
...
def has_enabled_agent_plugin_mcp(raw_config: Mapping[str, Any]) -> bool:
    ...
    Directory scanning belongs to :mod:`renco_cli.plugins` so startup gating
    ...
    from renco_cli.plugins import has_enabled_agent_plugin_mcp as _probe
