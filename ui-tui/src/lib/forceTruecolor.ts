...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.RENCO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Renco on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via RENCO_TUI_TRUECOLOR=1.
