"""Runtime-backed validation behind ``renco plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary RENCO_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="renco-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            ...
            {
                "RENCO_HOME": str(home),
                "RENCO_BUNDLED_PLUGINS": str(bundled),
                "RENCO_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    modules_before = {
        ...
        if name == "renco_plugins" or name.startswith("renco_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Renco discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "renco_plugins" or name.startswith("renco_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_renco_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".renco" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from renco_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Renco supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Renco never auto-installs plugin dependencies; "
    ...
    if schema:
        from renco_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Renco' real scanner and registration path."""
    try:
