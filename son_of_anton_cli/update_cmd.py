"""Son of Anton update pipeline — extracted from ``son_of_anton_cli/main.py``.
...
Imports are one-way: ``son_of_anton_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``son_of_anton_cli.main`` reference.

    Lets callers keep patching ``son_of_anton_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``son_of_anton_cli.main`` -> ``son_of_anton_cli.update_cmd`` stays one-way
    at import time.
    """
    from son_of_anton_cli import main

    return main


_UPDATE_RUNTIME_RELOAD_MODULES = (
    "son_of_anton_constants",
...
#: ``_purge_stale_son_of_anton_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        "son_of_anton_cli",
        "son_of_anton_cli.main",
        "son_of_anton_cli.update_cmd",
        "son_of_anton_cli.son_of_anton_logging",
    }
)


def _purge_stale_son_of_anton_modules() -> None:
    """Evict every cached Son of Anton module after the checkout changed in-place.

    ``son-of-anton update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from son_of_anton_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``son_of_anton_cli.gateway`` does
    ``from son_of_anton_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Son of Anton
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Son of Anton module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Son of Anton modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``son-of-anton update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``son-of-anton update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``son_of_anton_cli.config_defaults``,
    ``son_of_anton_cli.config``, and ``son_of_anton_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``son_of_anton_cli._subprocess_compat`` so that
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "son_of_anton_cli.config_defaults",
        "son_of_anton_cli.config",
        "son_of_anton_cli.config_migrations",
        "son_of_anton_cli._subprocess_compat",
    ):
...
def _run_config_check_fresh() -> tuple:
    ...
    from son_of_anton_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from son_of_anton_cli.config import migrate_config

    return migrate_config(interactive=interactive, quiet=quiet)


# Critical files that Son of Anton must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "son_of_anton_cli/main.py",
    "son_of_anton_cli/config.py",
    "son_of_anton_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``son-of-anton`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="son-of-anton-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    """
    from son_of_anton_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from son_of_anton_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('son_of_anton_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``son-of-anton update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    ...
    print("  Preview now:  son-of-anton curator run --dry-run")
    print("  Pause it:     son-of-anton curator pause")
    print(
        "  Docs:         https://son-of-anton.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `son-of-anton update`.
    ...
    try:
        ...
        return
    db_path = get_son_of_anton_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `son-of-anton sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    son-of-anton sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  son-of-anton sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``son-of-anton update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``son-of-anton update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: son-of-anton curator status)"
    )
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.son-of-anton-update-staging"
    backup = f"{dst}.son-of-anton-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `son-of-anton update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``son_of_anton_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``son-of-anton update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && son-of-anton update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("SON_OF_ANTON_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== son-of-anton-update completed {action_id} ===")
...
def _write_gateway_update_exit_code(ok: bool) -> None:
    path = get_son_of_anton_home() / ".update_exit_code"
    try:
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "son-of-anton-update-autostash-%Y%m%d-%H%M%S"
    )
    ...
    if push.returncode != 0:
        if stash_created:
            ...
            print(
                ...
                "then re-run `son-of-anton update`."
            )
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``son-of-anton update --keep-stash``: the stash made the update
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Son of Anton behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes son-of-anton completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Son of Anton couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Son of Anton couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Son of Anton behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Son of Anton couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Son of Anton couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/ewtodd/son-of-anton.git",
    "git@github.com:ewtodd/son-of-anton.git",
    "https://github.com/ewtodd/son-of-anton",
    "git@github.com:ewtodd/son-of-anton",
}

OFFICIAL_REPO_URL = "https://github.com/ewtodd/son-of-anton.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_son_of_anton_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_son_of_anton_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Son of Anton repository.")
        print("  This means you may miss updates from ewtodd/son-of-anton.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/ewtodd/son-of-anton.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/ewtodd/son-of-anton.git' to add later."
            )
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``son-of-anton tools``."""
    try:
        from son_of_anton_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Son of Anton Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``son-of-anton tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Son of Anton Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Son of Anton Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``son-of-anton update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `son-of-anton update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `son-of-anton update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Son of Anton' editable-install extras or
    ...
    try:
        from son_of_anton_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `son-of-anton update` is exactly the step
...
def _npm_lockfile_changed(son_of_anton_root: Path) -> bool:
    ...
    try:
        ...
        cache_file = son_of_anton_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(son_of_anton_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    son-of-anton update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `son-of-anton update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    # Son of Anton profile using this checkout. Keep one per-checkout cache under the
    # shared Son of Anton root rather than rerunning npm once per named profile.
    shared_son_of_anton_root = get_default_son_of_anton_root()

    if not _m()._npm_lockfile_changed(shared_son_of_anton_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `son-of-anton update`.")
        return list(labels)
    ...
    nixos_env = with_son_of_anton_node_path(_m()._nixos_build_env())
    ...
    # `son-of-anton update` comes from the *desktop* build, not this step; that
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.son-of-anton/logs/update.log`` only, never the terminal.

    During ``son-of-anton update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``son-of-anton update --check``: fetch and report without installing.
    ...
    from son_of_anton_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from son_of_anton_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``son-of-anton update`` without requiring a reinstall.  The
    ...
    ``son-of-anton`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/son-of-anton.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v son-of-anton'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/son-of-anton, code at /usr/local/lib/son-of-anton).
    fhs_link = Path("/usr/local/bin/son-of-anton")
    ...
    path_comment = (
        "# Son of Anton Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of SON_OF_ANTON_HOME under
      ``backups/`` (restorable via ``son-of-anton import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from son_of_anton_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from son_of_anton_constants import
        # get_son_of_anton_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from son_of_anton_cli.config import get_son_of_anton_home as _get_home
    ...
    try:
        from son_of_anton_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_son_of_anton_home so the user sees ~/.son-of-anton/...
    try:
        from son_of_anton_constants import get_son_of_anton_home, display_son_of_anton_home

        home = get_son_of_anton_home()
        try:
            display_path = f"{display_son_of_anton_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  son-of-anton import {out_path}")
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``son-of-anton-gateway*.service`` unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not (
            unit == "son-of-anton-gateway.service"
            or unit.startswith("son-of-anton-gateway-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    ``son-of-anton-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler.
    ...
    (``son-of-anton-gateway-helper`` is fine — profile units are
    ``son-of-anton-gateway-<profile>`` — but ``son-of-anton-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "son-of-anton-gateway" or svc_name.startswith("son-of-anton-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    if any(not name.startswith("ai.son-of-anton.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.son-of-anton.") for name in ordered):
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.son-of-anton.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``son_of_anton_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``son_of_anton_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    try:
        ...
        cache_dir = Path(_m().get_son_of_anton_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``son-of-anton update`` sees a clean tree instead of autostashing every run.
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚛ Updating Son of Anton Agent...")
    print()
    ...
    try:
        from son_of_anton_cli.update_receipt import begin_update_receipt
    ...
    # running Son of Anton runtime, its supervisor, and its running code version —
    ...
    try:
        from son_of_anton_cli.update_receipt import record_step
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Son of Anton gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in son_of_anton_cli/config.py bricked
        # every user who ran ``son-of-anton update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``son-of-anton
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``son-of-anton update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``son-of-anton update`` printed "✓ Code updated!" and
        # reinstalled deps against the stale tree —
        # no error, no warning, ``son-of-anton doctor`` healthy. Compare pre-pull
        ...
        # the old bytecode (e.g. get_son_of_anton_home added to son_of_anton_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # symbols in son_of_anton_constants or lazy_deps. The dependency install
        ...
        if not import_ok:
            ...
            print("    Run `son-of-anton update` again — if it persists, reinstall:")
            print("    https://son-of-anton.nousresearch.com")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_son_of_anton_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.son-of-anton/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit SON_OF_ANTON_HOME so
        # it is not affected by sync_skills()'s module-level SON_OF_ANTON_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SON_OF_ANTON_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from son_of_anton_cli.profiles import backfill_profile_envs
        ...
        # ``son-of-anton update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``son_of_anton_cli.config`` and
        # ``son_of_anton_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from son_of_anton_cli.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'son-of-anton config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'son-of-anton config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: son-of-anton config migrate")
            else:
                print()
                print("Skipped. Run 'son-of-anton config migrate' later to configure.")
        else:
        ...
        try:
            from son_of_anton_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `son-of-anton curator status`. Self-stamps after printing so it
        ...
        # When running as ``son-of-anton update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached Son of Anton modules FIRST: the import below pulls
        ...
        _m()._purge_stale_son_of_anton_modules()
        try:
            from son_of_anton_cli.gateway import (
                is_macos,
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... son-of-anton-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... son-of-anton-gateway*)
            ...
            try:
                from son_of_anton_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all son-of-anton-gateway* units (default + profiles).
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: son-of-anton gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `son-of-anton update` restart it automatically, allow\n"
                        ...
                        # path in `son-of-anton gateway restart`
            ...
            # Restart EVERY ai.son-of-anton.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: son-of-anton gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: son-of-anton -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_son_of_anton_home() / ".update_exit_code"
                    try:
            ...
            try:
                from son_of_anton_cli.update_receipt import record_gateway_restart
        ...
        # current son-of-anton-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `son-of-anton update` surfaces the issue until the user migrates.
        try:
            from son_of_anton_cli.gateway import (
                has_legacy_son_of_anton_units,
                _find_legacy_son_of_anton_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_son_of_anton_units():
                print()
                print("⚠ Legacy Son of Anton gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_son_of_anton_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (son-of-anton.service) fight the current")
                print("  son-of-anton-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    son-of-anton gateway migrate-legacy")
                print()
        ...
        print("  son-of-anton model              # Select provider and model")
        ...
        try:
            from son_of_anton_cli.update_receipt import (
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``son_of_anton_cli.gateway`` no longer imports,
