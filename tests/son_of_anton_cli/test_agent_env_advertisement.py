"""Tests for the AI_AGENT / SON_OF_ANTON_AGENT harness-attribution env vars.
...
Son of Anton-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Son of Anton' id in the public agent-harness
registry (``son-of-anton`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Son of Anton process env, and survives the
cross-session leak guard that strips ``SON_OF_ANTON_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__SON_OF_ANTON_CWD_testsession0__"
        env._snapshot_path = "/tmp/son-of-anton-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'SON_OF_ANTON_AGENT="${SON_OF_ANTON_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT SON_OF_ANTON=$SON_OF_ANTON_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "SON_OF_ANTON_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} SON_OF_ANTON=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", SON_OF_ANTON_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi SON_OF_ANTON=false" in out.stdout
