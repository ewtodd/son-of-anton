...
which broke ``son-of-anton --tui`` because ``ui-tui/`` had vanished.
...
from son_of_anton_cli.main import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.son-of-anton-update-staging").exists()
    assert not (dst.parent / "ui-tui.son-of-anton-update-old").exists()
