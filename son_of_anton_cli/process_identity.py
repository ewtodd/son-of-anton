...
1. **Spawn tag** (``SON_OF_ANTON_SPAWN`` env var): every spawner stamps its children
...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine Son of Anton root): every
...
SPAWN_ENV_VAR = "SON_OF_ANTON_SPAWN"
_TAG_VERSION = "v1"
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different Son of Anton install on the
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``SON_OF_ANTON_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_son_of_anton_root()) / LEDGER_FILENAME
    except Exception:
        from son_of_anton_cli.config import get_son_of_anton_home

        return Path(get_son_of_anton_home()) / LEDGER_FILENAME
...
def register_self(purpose: str, *, project_root: Optional[Path] = None) -> bool:
    ...
    if spawner_pid is None:
        ...
        # SON_OF_ANTON_PARENT_PID (+ optional `winms:<ms>` start marker) for its
        # parent-death watchdog. Reuse it as spawner identity so ledger
        # lineage works with every Desktop version, no TS change needed.
        try:
            raw = int(os.environ.get("SON_OF_ANTON_PARENT_PID", ""))
            if raw > 0:
                spawner_pid = raw
        except (TypeError, ValueError):
            pass
        marker = os.environ.get("SON_OF_ANTON_PARENT_START_MARKER", "")
