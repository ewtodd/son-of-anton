"""Profile distributions — shareable, packaged Renco profiles via git.

A distribution is a Renco profile published as a git repository (or
...
* ``renco profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``renco skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``renco profile``, not a parallel tree):

    renco profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "renco-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            renco_requires=str(data.get("renco_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.renco_requires:
            out["renco_requires"] = self.renco_requires
        if self.author:
...
def check_renco_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Renco {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``renco profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Renco profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Renco distribution."
        )

    # Version check up-front so we fail fast
    check_renco_requires(manifest.renco_requires, renco_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.renco).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="renco_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `renco profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`renco profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="renco_dist_update_") as tmp:
        plan = plan_install(
