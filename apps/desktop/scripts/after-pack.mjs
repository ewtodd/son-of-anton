 ...
 *
 * Stamps the Renco icon + identity onto the packed Windows Renco.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `renco desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Renco')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Renco'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Renco.exe keeps the stock Electron icon`)
  }
}
