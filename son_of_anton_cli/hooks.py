"""son-of-anton hooks — inspect and manage shell-script hooks.

Usage::

    son-of-anton hooks list
    son-of-anton hooks test <event> [--for-tool X] [--payload-file F]
    son-of-anton hooks revoke <command>
    son-of-anton hooks doctor

Consent records live under ``~/.son-of-anton/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.son-of-anton/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``son-of-anton hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: son-of-anton hooks {list|test|revoke|doctor}")
        print("Run 'son-of-anton hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.son-of-anton/config.yaml.")
        print("See `son-of-anton hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.son-of-anton/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `son-of-anton hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `son-of-anton hooks test` and `son-of-anton hooks doctor`
...
def _cmd_test(args) -> None:
    from son_of_anton_cli.config import load_config
    from son_of_anton_cli.plugins import VALID_HOOKS
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Son of Anton wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `son-of-anton hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `son-of-anton hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `son-of-anton hooks doctor`.")
