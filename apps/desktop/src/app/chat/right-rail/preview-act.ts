...
const preamble = () => `  var w = window;
  ...
  w.__rencoActHolder = w.__rencoActHolder || {};
  w.__rencoAct = ${actEngineSource()};
  w.__rencoWatch_fn = (${watchInPage.toString()});
  w.__rencoWatchTag = ${WATCH_TAG};
  var holder = w.__rencoActHolder;
  var act = function (a) { return w.__rencoAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__rencoWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__rencoHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'RENCO-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
