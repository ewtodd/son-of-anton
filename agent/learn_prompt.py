...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a skill via ``skill_manage`` that follows the Renco
     skill-authoring standards (description <=60 chars, the modern section
     order, Renco-tool framing, no invented commands). Small sources get one
...
# (HARDLINE)" and the renco-agent-dev new-skill salvage reference. Embedded in
# the prompt so the agent authors skills the way a maintainer would by hand.
_AUTHORING_STANDARDS = """\
Follow the Renco skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Renco`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Renco.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.renco.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Renco tools.
...
Renco-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Renco tools by name in backticks: `terminal`, `read_file`,
