...
(/opt/renco/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable RENCO_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_renco_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable RENCO_HOME."""
    ...
    monkeypatch.setattr(
        "renco_constants.get_renco_home", lambda: renco_home
    )
    ...
    expected = renco_home / "scripts" / "whatsapp-bridge"
