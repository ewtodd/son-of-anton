...
const RENCO_REF_TYPES = WIRE_REFERENCE_KINDS
type RencoRefType = (typeof RENCO_REF_TYPES)[number]
...
const RENCO_DIRECTIVE_RE = referenceRe()
...
export const rencoDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(RENCO_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...rencoDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Renco directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.rencoDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.rencoDesktop?.readFileDataUrl(id)
