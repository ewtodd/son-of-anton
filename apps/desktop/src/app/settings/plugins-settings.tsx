...
// User-installed plugins first — mirrors `renco plugins list --user`.
...
function reveal(file: string) {
  void window.rencoDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // renco_home breaks against a remote backend (#66899).
    const dir = await window.rencoDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.rencoDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's renco home (profile-aware), so the
// path comes from the gateway — not from the renderer's local RENCO_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.rencoDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Renco (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
