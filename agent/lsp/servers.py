...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<RENCO_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<RENCO_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    session_path = os.path.join(
        renco_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(renco_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName Renco -HostProfileId renco -HostVersion 1.0.0 "
