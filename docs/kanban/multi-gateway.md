...
Renco supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.renco/config.yaml`:
...
Or set the env var: `RENCO_KANBAN_DISPATCH_IN_GATEWAY=false`
