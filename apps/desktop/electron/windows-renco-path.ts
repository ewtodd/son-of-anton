/**
 * windows-renco-path.ts
 *
 * Pure, dependency-injected pieces of Windows `renco` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvRencoCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `renco` shim shadowed the real
 *      renco.cmd/renco.exe; the shim then failed the --version probe and
 ...
 *      and Renco launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvRencoCommand() — unwrapWindowsVenvRencoCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `renco`) must
 * not shadow `renco.cmd`/`renco.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Renco launcher that drives `renco update`.
...
export interface ResolveVenvRencoCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportRencoCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `renco`/`renco.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/renco(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m renco_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportRencoCli(). A
 * venv whose update died mid-`pip install` still has python.exe + renco.exe
 ...
 *
 * Returns null when `command` is not a venv renco shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvRencoCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvRencoCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^renco(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Renco at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Renco Python at ${python}`,
    command: python,
