...
def _default_registry_path() -> Path:
    return get_renco_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_renco_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_renco_home = expected_renco_home if expected_renco_home is not None else str(get_renco_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["RENCO_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("renco_home") or "")
        if got_home and got_home != self.expected_renco_home:
            raise RuntimeError(f"compute host RENCO_HOME mismatch: {got_home} != {self.expected_renco_home}")
