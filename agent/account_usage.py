...
from renco_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_nous_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    """
    try:
        from renco_cli.nous_account import nous_portal_topup_url
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Dev override: when RENCO_DEV_CREDITS_FIXTURE selects a fixture state, /usage
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so RENCO_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — RENCO_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="nous",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Renco's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `renco auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `renco auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
    ...
    if code == "reset":
        ...
        # persisted pool cooldowns so Renco doesn't keep the credential
        # frozen behind the now-stale ``last_error_reset_at`` (issue #43747).
        try:
            from renco_cli.auth import clear_codex_pool_quota_cooldowns
