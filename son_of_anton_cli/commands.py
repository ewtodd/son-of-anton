...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Son of Anton config/state", "Session",
    ...
    CommandDef("update", "Update Son of Anton Agent to the latest version", "Info",
               busy_policy="dispatch"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    Telegram command menu, in Slack's ``/son-of-anton`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Son of Anton ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Son of Anton
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``son-of-anton skills
...
_SLACK_VIA_SON_OF_ANTON_ONLY = frozenset({"topup", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "whoami", "platform"})
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from son_of_anton_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via son_of_anton_cli.personality."""
        try:
