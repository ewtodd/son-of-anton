...
Set via `son-of-anton model` (picker) or `son-of-anton setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://son-of-anton.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `son-of-anton auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `son-of-anton auth add openai-codex` |
| qwen-oauth | OAuth | `son-of-anton auth add qwen-oauth` |
| minimax-oauth | OAuth | `son-of-anton auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`son-of-anton auth`).
Fallback chain when the primary fails: `son-of-anton fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`son_of_anton_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   son-of-anton config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
