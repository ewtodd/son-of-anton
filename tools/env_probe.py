...
machine as Renco itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Renco ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike Renco's own uv call sites: this
    ...
    # local.py puts the Renco-managed $RENCO_HOME/bin there, so a managed-only
