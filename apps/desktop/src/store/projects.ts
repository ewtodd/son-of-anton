...
import type { RencoGitBaseBranch, RencoGitBranch } from '@/global'
import { getRencoConfig, rencoApi, type RencoGateway } from '@/renco'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/renco'
...
const PROJECT_SCOPE_KEY = 'renco.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Renco gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Renco profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await rencoApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<RencoGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<RencoGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Renco
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getRencoConfig(context.profile))
...
// repo, returning where Renco should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<RencoGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<RencoGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.rencoDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.rencoDesktop?.writeClipboard?.(path)
  }
}
