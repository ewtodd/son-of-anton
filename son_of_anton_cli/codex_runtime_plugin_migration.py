"""Migrate Son of Anton' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Son of Anton' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Son of Anton mcp_servers.<n>.command/args/env  → codex stdio transport
  Son of Anton mcp_servers.<n>.url/headers       → codex streamable_http transport
  Son of Anton mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Son of Anton mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Son of Anton-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Son of Anton' own
...
MIGRATION_MARKER = (
    "# managed by son-of-anton — `son-of-anton codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end son-of-anton managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Son of Anton config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Son of Anton' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, son_of_anton_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Son of Anton MCP server config to the codex inline-table dict
    ...
    if not isinstance(son_of_anton_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(son_of_anton_cfg.get("command"))
    has_url = bool(son_of_anton_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(son_of_anton_cfg["command"])
        args = son_of_anton_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = son_of_anton_cfg.get("env") or {}
        if env:
            # Codex expects string values
            out["env"] = {str(k): str(v) for k, v in env.items()}
        cwd = son_of_anton_cfg.get("cwd")
        if cwd:
            out["cwd"] = str(cwd)
    elif has_url:
        # streamable_http transport (codex covers both http and SSE here)
        out["url"] = str(son_of_anton_cfg["url"])
        headers = son_of_anton_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Son of Anton' transport: sse hint is informational; codex auto-negotiates
        if son_of_anton_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in son_of_anton_cfg:
        try:
            out["tool_timeout_sec"] = float(son_of_anton_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in son_of_anton_cfg:
        try:
            out["startup_timeout_sec"] = float(son_of_anton_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if son_of_anton_cfg.get("enabled") is False:
        out["enabled"] = False

    # Detect keys we explicitly drop with warning
    for key in son_of_anton_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_SON_OF_ANTON_KEYS:
            skipped.append(f"{key} (unknown Son of Anton key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (SON_OF_ANTON_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Son of Anton)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Son of Anton' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Son of Anton' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Son of Anton
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="son-of-anton-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a SON_OF_ANTON_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed son-of-anton-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.son-of-anton`` symlink that happens to live under ``/private/var/folders``
...
def _build_son_of_anton_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Son of Anton' own
    ...
    so a son-of-anton installed under /opt/, /usr/local/, or a venv all work.
    SON_OF_ANTON_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # SON_OF_ANTON_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_son_of_anton_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever SON_OF_ANTON_HOME its launcher sets
    ...
    # — that would override the launcher's SON_OF_ANTON_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("SON_OF_ANTON_HOME", tmp_path) would
    ...
    son_of_anton_home = os.environ.get("SON_OF_ANTON_HOME") or ""
    if son_of_anton_home and _looks_like_test_tempdir(son_of_anton_home):
        son_of_anton_home = ""
    if son_of_anton_home:
        env["SON_OF_ANTON_HOME"] = son_of_anton_home
    # PYTHONPATH passes through so a worktree-launched son-of-anton finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["SON_OF_ANTON_QUIET"] = "1"
    env["SON_OF_ANTON_REDACT_SECRETS"] = env.get("SON_OF_ANTON_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.son_of_anton_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    son_of_anton_config: dict,
    *,
    codex_home: Optional[Path] = None,
    dry_run: bool = False,
    discover_plugins: bool = True,
    default_permission_profile: Optional[str] = ":workspace",
    expose_son_of_anton_tools: bool = True,
) -> MigrationReport:
    """Translate Son of Anton mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        son_of_anton_config: full ~/.son-of-anton/config.yaml dict
        ...
        expose_son_of_anton_tools: when True (default), register Son of Anton' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Son of Anton for tools
    ...
    son_of_anton_servers = (son_of_anton_config or {}).get("mcp_servers") or {}
    if not isinstance(son_of_anton_servers, dict):
        report.errors.append(
            "mcp_servers in Son of Anton config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in son_of_anton_servers.items():
    ...
    # Inject Son of Anton' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Son of Anton for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/son_of_anton_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_son_of_anton_tools:
        translated["son-of-anton-tools"] = _build_son_of_anton_tools_mcp_entry()
        if "son-of-anton-tools" not in report.migrated:
            report.migrated.append("son-of-anton-tools")
