...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Renco Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Renco instantly when PRs are opened or updated.
:::
...
- **Renco Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  renco gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.renco/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Renco can access GitHub. Start a chat:
...
```
Run: gh pr list --repo ewtodd/renco --state open --limit 3
```
...
Still in the chat, ask Renco to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo ewtodd/renco
```
...
A skill gives Renco consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.renco/skills/code-review
```

Create `~/.renco/skills/code-review/SKILL.md`:
...
Verify it loaded — start `renco` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Renco your team's standards:
...
2. Teach Renco your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
renco gateway status    # Is the gateway running?
renco cron list         # Is the job enabled?
```
