...
One ``CDPSupervisor`` runs per Renco ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Renco logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "renco-dialog-bridge.invalid"
...
(() => {
  if (window.__rencoDialogBridgeInstalled) return;
  window.__rencoDialogBridgeInstalled = true;
  const ENDPOINT = "http://renco-dialog-bridge.invalid/";
