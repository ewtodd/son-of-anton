...
def son_of_anton_home(tmp_path, monkeypatch):
    ...
    # /etc/son-of-anton on the dev/CI box can't influence the test.
    monkeypatch.setenv("SON_OF_ANTON_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(son_of_anton_home, monkeypatch):
...
def test_env_expansion_in_user_config(son_of_anton_home, monkeypatch):
    from son_of_anton_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_son_of_anton_dotenv(son_of_anton_home=str(home))
