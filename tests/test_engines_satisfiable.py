...
`npm ci` / `npm install` — the installer's workspace step, `son-of-anton update`'s
...
`son-of-anton update` left installs in a mixed state. These tests encode the
...
class TestEnginesAreSatisfiable:
    ...
    def test_managed_node_bundles_an_npm_the_engines_accept(self):
        ...
        11.10–11.16 band — fresh Son of Anton-managed installs then die at
        ...
        assert _satisfies_range(stock_npm, npm_range), (
            ...
            "Son of Anton-managed install cannot run npm ci."
        )
