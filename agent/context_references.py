...
from renco_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
_SENSITIVE_RENCO_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    renco_home = get_renco_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(renco_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(renco_home / rel for rel in _SENSITIVE_RENCO_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Renco path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.renco/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Renco path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
