...
import type { RencoReadDirEntry, RencoReadDirResult } from '@/global'
...
export type ProjectTreeEntry = RencoReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: RencoReadDirEntry) {
...
async function filterIgnored(entries: RencoReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<RencoReadDirResult> {
  if (!window.rencoDesktop) {
