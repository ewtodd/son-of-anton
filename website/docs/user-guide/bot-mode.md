---
title: "Bot Mode"
description: "Turn your Renco profiles into a roster of named Bots — each with its own chat, role, model, memory, skills, and avatar. Bots run routines, share group chats, and message each other."
---

# Bot Mode

**Bot Mode** turns your [Renco profiles](./profiles.md) into a roster of named **Bots**. Each Bot has its own role, model, memory, skills, and avatar; Bots run recurring routines, deliberate together in group chats, and message each other directly. Build a specialist Bot once and it is there forever, one click away.
...
There is no new primitive to learn: a Bot **is** a Renco profile — isolated config, memory, skills, credentials, and chat history under `~/.renco/profiles/<name>/`. Bot Mode is a UI over that primitive, so everything you do in it is visible from the CLI too: `renco -p <bot> chat` opens the same agent, and Bot routines appear in `renco cron list`. No core patches, no background daemons, no extra storage.
:::
...
- **Model & provider pin** — give the Bot its own model. Any provider/model pair Renco knows about works, and different Bots can run on different models side by side. Leave it unset to inherit from the launch profile.
...
- **A pixel pet** — a companion from the [petdex gallery](./features/pets.md) that bounces beside the avatar while the Bot is busy. Run `renco pets` in a terminal to explore the gallery.
...
### Bot-initiated DMs across machines (`renco peer`)
...
```bash
renco peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>
renco peer list
renco peer dm spark < /tmp/dm.txt        # message body from a file (nothing shell-interpreted)
renco peer dm spark/researcher < /tmp/dm.txt   # named profile on a multiplexed peer
```

`renco peer dm` delivers into the remote agent's canonical Bot Chat over the peer's existing API server, runs one agent turn there, and prints the reply on stdout — the exact cross-machine twin of the local `renco -p <bot> chat` command.

Once a peer is registered, the messaging protocol taught to every Bot Chat (`agent.bot_mode_protocol`) automatically includes the peer roster and the `renco peer dm` pattern — so **your bots learn on their own** that teammates exist on other machines and how to reach them. Registering or removing a peer refreshes each Bot Chat's protocol on its next message (capability epoch).

Requirements: the peer machine runs the `api_server` gateway platform with a strong `API_SERVER_KEY`; reachability is your network's business (LAN, Tailscale, VPN). The key is a credential and lives in `~/.renco/.env` as `RENCO_PEER_<NAME>_KEY`; peer names/URLs live in `config.yaml` under `bot_peers`.

## Bots across machines

When you register several backends in **Settings → Connections** — the local runtime, remote gateways, SSH hosts, Renco Cloud instances — the roster shows the Bots from **every** connected source, persistently: SSH sources are inventoried without spawning anything on the remote box, and machines that are momentarily unreachable keep their last-known rows instead of vanishing. When the same profile name exists on several sources, handles disambiguate as `@name-device` (for example `@research-homelab`). A Bot's chats, sessions, memory, and routines live on the machine that owns the profile.

Clicking a Connections Bot does **not** hop your window onto that machine — stay in your chat and `@mention` it, seat it in a group chat, or create new agents on it directly with the **Create on** picker. Cloud and local agents share one roster this way: register your Renco Cloud instance and your desktop (say, over Tailscale or SSH) and their Bots can message each other and sit in the same rooms, with each agent's work running on its own machine.

See [Connecting Desktop to Many Renco Instances](./multi-connection-desktop.md) for the full multi-connection guide.
...
| --- | --- |
| Chat with a Bot | `renco -p <bot> chat` |
| A Bot's files, skills, memory | `~/.renco/profiles/<bot>/` |
| Routines | `renco cron list` (jobs named `[bot:<name>] …`) |
| Create / inspect profiles | `renco profile create`, `renco profile list` |
