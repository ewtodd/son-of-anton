...
row taxonomy is inferred from the atlas shape so Son of Anton can render both legacy
...
# rows (1536x1872), while older Son of Anton/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Son of Anton' activity state names. They are not always identical to the
...
# Legacy Son of Anton/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Son of Anton activity names -> accepted row-name aliases in descending
