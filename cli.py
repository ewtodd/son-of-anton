...
def _cprint(text: str, strip: bool = True):
    ...
    The single funnel for the CLI's own output. Under the Textual front-end it
    is replaced (see ``son_of_anton_tui.backend``) so lines land in the
    transcript instead; this plain implementation is what the non-interactive
    paths (``-q``, one-shot, cron) use, where stdout is a pipe or a plain tty
    and the escapes can go straight out.

    Decorative emoji glyphs are stripped here. Streamed MODEL CONTENT (the
    agent's own words) passes through untouched via ``strip=False`` — glyph
    stripping is for UI chrome, never for message text.
    """
    if strip:
...
# Set by the interactive front-end while it owns output (see
# ``son_of_anton_tui.backend``): bare ``print`` is captured there, so chrome
# that must land in the transcript goes through ``_cprint`` instead.
_CPRINT_ROUTED = False


def _cprint_is_routed() -> bool:
    """True when a front-end has taken over ``_cprint``."""
    return _CPRINT_ROUTED


def _cli_visible_print(text: str = "") -> None:
    """Print so the user actually sees it, whichever front-end is up.

    Under the Textual app bare ``print`` is captured and re-rendered as a
    transcript row, so ``/sessions`` and ``/history`` route through ``_cprint``
    to keep their formatting; everywhere else a plain print is right.
    """
    if _cprint_is_routed():
        _cprint(text)
    else:
        print(text)
...
class SonOfAntonCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _force_full_redraw(self) -> None:
        """Repaint the interactive front-end. No-op without one (``-q``)."""
        return None

    def _apply_tui_skin_style(self) -> bool:
        """Re-apply the active skin to a live front-end. False without one."""
        return False
    ...
    def _trim_status_bar_text(cls, text: str, max_width: int) -> str:
        ...
        if max_width <= 0:
            return ""
        from rich.cells import cell_len as get_cwidth
    ...
    def _on_reaction(self, kind: str) -> None:
        """Affection callback (ily / <3 / good bot) from the reaction core.

        The base CLI has no reaction UI — the TUI emits hearts events for the
        same signal — so this hook only keeps the interactive-host wiring intact.
        """
        return None
...
def main(
    ...
    # Create CLI instance. Textual is the one interactive front-end
    # (son_of_anton_tui.backend.TextualBackend subclasses SonOfAntonCLI), so
    # every option below applies to it unchanged.
    ...
    # Run the interactive front-end
    from son_of_anton_tui.tui import run_app as _run_textual_app

    _run_textual_app(cli)
