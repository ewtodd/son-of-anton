...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SON_OF_ANTON_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Son of Anton on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SON_OF_ANTON_TUI_TRUECOLOR=1.
