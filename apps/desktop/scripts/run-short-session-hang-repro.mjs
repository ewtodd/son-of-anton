...
const UPSTREAM_URL = 'https://github.com/ewtodd/renco.git'
...
const LEGACY_STORAGE_KEY = 'renco.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'renco.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.rencoDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `renco-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const rencoHome = join(sandbox, 'renco-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(rencoHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(rencoHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        RENCO_DESKTOP_APP_NAME: `RencoShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        RENCO_DESKTOP_RENCO_ROOT: target.targetRoot,
        RENCO_DESKTOP_IGNORE_EXISTING: '1',
        RENCO_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'renco-short-session-ab-'))
  const prepared = []
