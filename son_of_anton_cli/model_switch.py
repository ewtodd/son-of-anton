...
- ``son_of_anton_cli.providers``        -- canonical provider identity + overlays
- ``son_of_anton_cli.model_normalize``  -- per-provider name formatting
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Son of Anton wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Son of Anton.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Son of Anton versions wrote an in-mapping
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Son of Anton itself discovered (``models_discovered: true``
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``son-of-anton config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from son_of_anton_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``son_of_anton_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'son-of-anton doctor' — config issues detected:"
        ...
        from son_of_anton_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from son_of_anton_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from son_of_anton_cli.providers import resolve_user_provider as _ruser
    ...
    # kimi) the same way.
    from son_of_anton_cli.models import opencode_provider_family as _oc_family_fn
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from son_of_anton_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from son_of_anton_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS
    ...
    for son_of_anton_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(son_of_anton_id)
        if (
            _alias_target
            and _alias_target != son_of_anton_id
    ...
    for pid, overlay in SON_OF_ANTON_OVERLAYS.items():
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or son_of_anton_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                ...
                pass
        if has_creds:
            slugs.append(son_of_anton_slug)
            seen.add(pid.lower())
            seen.add(son_of_anton_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Compared against son_of_anton_id / mdev_id (section 1), pid / son_of_anton_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from son_of_anton_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for pid, overlay in SON_OF_ANTON_OVERLAYS.items():
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or son_of_anton_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(son_of_anton_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(son_of_anton_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", son_of_anton_slug, exc)
        ...
        results.append({
            ...
            "name": get_label(son_of_anton_slug),
            "is_current": son_of_anton_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "son-of-anton",
        ...
        seen_slugs.add(son_of_anton_slug.lower())
        _record_builtin_endpoint(son_of_anton_slug)
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Son of Anton writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Son of Anton's own writer uses (mirrors section-4 grouping).
            ...
            # ``son-of-anton model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``son-of-anton model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Son of Anton's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, son_of_anton_cli/config.py) already consume that dict.
