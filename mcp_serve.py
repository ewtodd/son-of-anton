"""
Renco MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Renco-specific extra)

Usage:
    renco mcp serve
    renco mcp serve --verbose
...
logger = logging.getLogger("renco.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using RENCO_HOME."""
    try:
        from renco_constants import get_renco_home
        return get_renco_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("RENCO_HOME", Path.home() / ".renco")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from renco_constants import get_renco_home
        directory_file = get_renco_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("RENCO_HOME", Path.home() / ".renco")
...
class EventBridge:
    ...
    This is the Renco equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from renco_constants import get_renco_home
            db_file = get_renco_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("RENCO_HOME", Path.home() / ".renco")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Renco MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "renco",
        instructions=(
            "Renco Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Renco MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
