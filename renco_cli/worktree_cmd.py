"""``renco worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``renco_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    renco worktree list             # audit: verdict + reason per tree
    renco worktree prune            # reap safe trees + merged branches
    renco worktree prune --dry-run  # show the plan, change nothing
    renco worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `renco worktree prune`."
        )
