...
lines to ~/.son-of-anton/logs/tool_calls.log via write_tool_log's rotating handler.
...
async def test_write_tool_log_writes_and_rotates_handler(tmp_path, monkeypatch):
    ...
    tool_logger = logging.getLogger(f"son-of-anton.tool_calls.test.{id(log_queue)}")
