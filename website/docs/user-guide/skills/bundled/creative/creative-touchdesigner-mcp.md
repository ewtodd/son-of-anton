...
```
Renco Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${RENCO_HOME:-$HOME/.renco}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Renco config (if missing)
...
3. **Restart Renco session** to pick up the new MCP server
