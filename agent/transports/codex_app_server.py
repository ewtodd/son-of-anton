...
projection into Son of Anton' display, approval bridging, transcript projection into
...
"codex_app_server"`. Son of Anton' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Son of Anton secret — gateway
        ...
        if spawn_env.get("SON_OF_ANTON_KANBAN_TASK"):
            kanban_db = spawn_env.get("SON_OF_ANTON_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "SON_OF_ANTON_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SON_OF_ANTON_HOME", os.path.expanduser("~/.son-of-anton")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "son-of-anton",
        client_title: str = "Son of Anton Agent",
