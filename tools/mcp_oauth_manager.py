...
`mcp_tool.py`, and `son_of_anton_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# SonOfAntonMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_son_of_anton_provider_class() -> Optional[type]:
    ...
    class SonOfAntonMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._son_of_anton_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`SonOfAntonTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, SonOfAntonTokenStorage)
                ...
                if meta is not None:
                    ...
                    logger.debug(
                        ...
                        "(token_endpoint=%s)",
                        self._son_of_anton_server_name,
                        meta.token_endpoint,
                    )
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._son_of_anton_server_name, exc,
                    )
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not Son of Anton' — mcp 2.0 builds on httpx2,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._son_of_anton_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import SonOfAntonTokenStorage
                        if isinstance(storage, SonOfAntonTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._son_of_anton_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import SonOfAntonTokenStorage
            if not isinstance(storage, SonOfAntonTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``son-of-anton mcp reauth``.
            ...
            back to ``son-of-anton mcp reauth``.
            """
            try:
                if self._son_of_anton_preregistered:
                    return
                ...
                # walking back into the same refusal. `son-of-anton mcp login`
                ...
                if cimd_url and rejected_id == cimd_url:
                    logger.warning(
                        ...
                        self._son_of_anton_server_name, cimd_url,
                    )
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._son_of_anton_server_name,
                    son_of_anton_home=self._son_of_anton_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
    ...
    return SonOfAntonMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_SON_OF_ANTON_PROVIDER_CLS: Optional[type] = _make_son_of_anton_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._son_of_anton_home = key[0]
    ...
    def _key(
        ...
        home = Path(son_of_anton_home) if son_of_anton_home is not None else get_son_of_anton_home()
    ...
    def _build_provider(
        ...
        Constructs :class:`SonOfAntonMCPOAuthProvider` directly using the helpers
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `son-of-anton mcp login "
        ...
        return _SON_OF_ANTON_PROVIDER_CLS(
    ...
    def remove(
        ...
        *,
        ...
        Called by ``son-of-anton mcp remove <name>`` and (indirectly) by
        ``son-of-anton mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, son_of_anton_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, son_of_anton_home=son_of_anton_home)
        logger.info(
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, son_of_anton_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(son_of_anton_home) / f"{_safe_filename(server_name)}.json"
            try:
