...
Ledger location: `$SON_OF_ANTON_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `SON_OF_ANTON_CITATION_LEDGER`.

## How to Run

```bash
S=~/.son-of-anton/skills/research/grounded-citations/scripts/sources.py
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `SON_OF_ANTON_CITATION_LEDGER`) if their
