...
request omits ``thinking``.  Renco' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Renco reasoning
...
(per Z.AI / BigModel docs).  Renco' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Renco reasoning effort onto GLM-5.2's native ``high``/``max``.
