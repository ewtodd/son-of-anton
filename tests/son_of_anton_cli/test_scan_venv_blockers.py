"""Tests for son_of_anton_cli/_scan_venv_blockers.py.
...
from son_of_anton_cli._scan_venv_blockers import (
...
def test_redact_long_flag_value_space_separated() -> None:
    ...
    raw = "python.exe -m son_of_anton_cli.main serve --token ghp_abc123 --host 10.0.0.1"
    result = _redact_sensitive_cmdline(raw)
    assert result == "python.exe -m son_of_anton_cli.main serve --token <redacted>"
...
def test_redact_normal_host_port_profile_remain() -> None:
    raw = "python.exe -m son_of_anton_cli.main serve --host 10.0.0.1 --port 9119 --profile work"
    result = _redact_sensitive_cmdline(raw)
...
def test_redact_no_sensitive_flags_is_noop() -> None:
    raw = "python.exe -m son_of_anton_cli.main serve --host 127.0.0.1"
...
def test_redact_short_flags_not_redacted() -> None:
    ...
    raw = "python.exe -m son_of_anton_cli.main serve -t web -p default -k somearg"
    result = _redact_sensitive_cmdline(raw)
...
def test_classify_local_preview_args_preserves_full_directory_label_and_port() -> None:
    args = [
        r"C:\Son of Anton\venv\Scripts\python.exe",
        "-m",
        "http.server",
        "8766",
        "--bind",
...
#
# `son-of-anton-setup` always invokes `son-of-anton update --yes --gateway`, whose
...
@pytest.mark.parametrize(
    ...
    [
        ...
        r"C:\Users\u\AppData\Local\son-of-anton\son-of-anton\venv\Scripts\python.exe"
        " -m son_of_anton_cli.main gateway run --replace",
        ...
        "python.exe -m son_of_anton_cli.main --profile work gateway run",
        ...
        "python.exe -m son_of_anton_cli.main --profile gateway gateway run",
        "python.exe -m son_of_anton_cli.main -p gateway gateway run",
        # bare `gateway` defaults to `run` (mirrors the canonical matcher)
        "python.exe -m son_of_anton_cli.main gateway",
        # case variations survive
        "PYTHON.EXE -m son_of_anton_cli.main GATEWAY RUN",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "python.exe -m son_of_anton_cli.main serve --host 127.0.0.1 --port 8756",
        # other gateway subcommands are not running gateways
        "python.exe -m son_of_anton_cli.main gateway stop",
        "python.exe -m son_of_anton_cli.main gateway status",
        "python.exe -m son_of_anton_cli.main gateway install",
        # operator REPL / stray script
        "python.exe",
        "python.exe myscript.py gateway run",  # not a son_of_anton_cli.main invocation
        "",
    ],
)
...
def test_main_exempts_gateway_chain_but_keeps_other_holders(monkeypatch, capsys):
    ...
    gateway_launcher = (
        12,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m son_of_anton_cli.main gateway run --replace",
    )
    gateway_worker = (
        34,
        "python.exe",
        r'"C:\u\uv\python\python.exe"  -m son_of_anton_cli.main gateway run --replace',
    )
...
def test_main_desktop_serve_backend_still_blocks(monkeypatch, capsys):
    ...
    serve = (
        78,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m son_of_anton_cli.main serve --host 127.0.0.1",
    )
...
def test_main_gateway_with_long_managed_runtime_path_is_exempt(monkeypatch, capsys):
    ...
    exe path (`.son-of-anton-runtime\python\generation-...\cpython-3.11-...`).
    The old `cmdline_raw[:120]` truncation in the detector cut the cmdline
    before `-m son_of_anton_cli.main gateway run`, so the exemption never matched
    ...
    """
    long_exe = (
        r'"C:\Users\u\AppData\Local\son-of-anton\son-of-anton\.son-of-anton-runtime\python'
    ...
    gateway = (91, "python.exe", long_exe + "  -m son_of_anton_cli.main gateway run --replace")
