"""Tests for the per-profile Projects store (son_of_anton_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Son of Anton Agent", folders=["/tmp/son-of-anton"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "son-of-anton"
    assert proj.name == "Son of Anton Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/son-of-anton"
    assert [f.path for f in proj.folders] == ["/tmp/son-of-anton"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "son-of-anton").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SON_OF_ANTON_HOME.
