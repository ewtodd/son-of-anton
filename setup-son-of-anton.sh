#!/bin/bash
# ============================================================================
# Son of Anton Agent Setup Script
# ============================================================================
...
# 5. Symlinks the 'son-of-anton' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Son of Anton Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/son-of-anton-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/son-of-anton-uv-installer.$$.sh")"
...
# ============================================================================
# PATH setup — symlink son-of-anton into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up son-of-anton command..."

SON_OF_ANTON_BIN="$SCRIPT_DIR/venv/bin/son-of-anton"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SON_OF_ANTON_BIN" "$COMMAND_LINK_DIR/son-of-anton"
echo -e "${GREEN}✓${NC} Symlinked son-of-anton → $COMMAND_LINK_DISPLAY_DIR/son-of-anton"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Son of Anton Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.son-of-anton/skills/
# ============================================================================

SON_OF_ANTON_SKILLS_DIR="${SON_OF_ANTON_HOME:-$HOME/.son-of-anton}/skills"
mkdir -p "$SON_OF_ANTON_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.son-of-anton/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SON_OF_ANTON_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  son-of-anton status        # Check configuration"
if is_termux; then
    echo "  son-of-anton gateway       # Run gateway in foreground"
else
    echo "  son-of-anton gateway install # Install gateway service (messaging + cron)"
fi
echo "  son-of-anton cron list     # View scheduled jobs"
echo "  son-of-anton doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m son_of_anton_cli.main setup
fi
