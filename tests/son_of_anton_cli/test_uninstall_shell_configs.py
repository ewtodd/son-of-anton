...
This rewrites files Son of Anton does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
from son_of_anton_cli import uninstall
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``SON_OF_ANTON_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    monkeypatch.setenv("SON_OF_ANTON_HOME", str(home / ".son-of-anton"))
    return home


class TestHappyPath:
    def test_son_of_anton_path_block_is_removed(self, fake_home: Path):
        rc = fake_home / ".zshrc"
        ...
        assert "# Son of Anton Agent" not in text
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/SON_OF_ANTON_HOME patches the fake_home fixture installed.
    ...
    def test_symlinked_shell_config_stays_a_symlink(self, fake_home: Path):
        ...
        assert "# Son of Anton Agent" not in real.read_text(encoding="utf-8")
