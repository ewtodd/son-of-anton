"""``renco peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Renco gateway (any machine: homelab, Spark, Renco
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    renco peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    renco peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``renco -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.renco/.env`` as
  ``RENCO_PEER_<NAME>_KEY``.
...
def _peer_key_env(name: str) -> str:
    return f"RENCO_PEER_{name.upper().replace('-', '_')}_KEY"
...
def _ensure_bot_chat(base: str, key: str) -> str:
    ...
    # Real api_server wraps the row: {"object": "renco.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (renco peer dm <peer>[/<agent>] ...)")
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.renco/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  renco peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.renco/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: renco peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    if action == "dm":
        ...
        if not isinstance(peer, dict) or not peer.get("url"):
            print(f"No peer named '{peer_name}'. Run: renco peer list", file=sys.stderr)
            return 1
        key = _peer_secret(peer_name)
        if not key:
            print(
                f"No API key for peer '{peer_name}'. Set it: renco peer add {peer_name} "
                f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.renco/.env)",
                file=sys.stderr,
            )
            return 1
    ...
    print("Unknown peer action. See: renco peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Renco gateways)",
        description=(
            "Register other Renco gateways as peers and message their agents. "
            "'renco peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'renco -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.renco/.env."
        ),
        epilog=(
            "Examples:\n"
            "  renco peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  renco peer list\n"
            '  renco peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  renco peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  renco peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.renco/.env)")
