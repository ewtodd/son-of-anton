...
``renco*.exe`` shims aside (``renco.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``renco`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("renco", "renco-agent", "renco-acp", "renco-gateway"):
...
# ---------------------------------------------------------------------------
# renco_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "renco.exe" in names, "renco.exe must be restored after a no-op install"
    assert "renco-acp.exe" in names
    assert "renco-gateway.exe" in names
    assert (scripts / "renco.exe").read_bytes() == b"MZ-old-renco"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "renco.exe").read_bytes() == b"MZ-new-renco"
...
# ---------------------------------------------------------------------------
# renco_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "renco.exe" in names, "renco.exe must be restored after a no-op recovery install"
    assert (scripts / "renco.exe").read_bytes() == b"MZ-old-renco"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "renco.exe").write_bytes(b"MZ-new-renco")
