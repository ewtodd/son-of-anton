...
Uses a throwaway RENCO_HOME so it never touches ~/.renco. No real names.
"""
...
def _fresh_home():
    """Point RENCO_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_renco_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["RENCO_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER RENCO_HOME is set.
...
# a different get_renco_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"RENCO_HOME (throwaway): {HOME}")
