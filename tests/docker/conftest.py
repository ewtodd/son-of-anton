...
Override the image with ``RENCO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("RENCO_TEST_IMAGE", "renco-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``RENCO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("RENCO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged renco user
# ---------------------------------------------------------------------------
#
# Background: every Renco runtime path inside the container drops to UID
# 10000 (the ``renco`` user) via ``s6-setuidgid renco``. ``docker exec``
...
# fails (PermissionError swallowed) for renco, so a test that ran as root
...
# helpers below run every probe under ``-u renco`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: renco).
