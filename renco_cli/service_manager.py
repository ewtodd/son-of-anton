...
the existing module-level functions in renco_cli.gateway and
renco_cli.gateway_windows directly. This protocol is a thin facade
...
``renco gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``renco gateway start/stop/restart``).
    """
    ...
    # `renco gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged renco
    ...
    detection always fails. Since every Renco runtime call inside the
    container drops to renco via ``s6-setuidgid``, that silent failure
...
# in ``renco_cli.gateway`` (systemd/launchd) and ``renco_cli.gateway_windows``
...
# active (set via the ``renco -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in renco_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from renco_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from renco_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from renco_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from renco_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in renco_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from renco_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from renco_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from renco_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from renco_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``renco_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `renco profile create`
# runs inside the container (Phase 4). Static services (main-renco, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's RENCO_HOME blindly;
    derive the shared profile root from the current RENCO_HOME and map the
    ...
    renco_home = Path(os.environ.get("RENCO_HOME", "/opt/data"))
    if renco_home.parent.name == "profiles":
        root = renco_home.parent.parent
    else:
        root = renco_home
...
# UID/GID of the in-image ``renco`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the renco user.
    ...
    (the ``renco`` user — UID 10000 — running every Renco runtime
    ...
    is dispatched as the renco user.
    ...
    with renco ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           renco:renco, 0755 (parent must already exist)
    ``svc_dir/event/``                     renco:renco, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 renco:renco, 0755
    ``svc_dir/supervise/event/``           renco:renco, 03730
    ``svc_dir/supervise/control``          renco:renco, 0660    (FIFO)
    ...
    slot's supervise/ was renco-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _RENCO_UID, _RENCO_GID)
        except PermissionError:
            # Running as the renco user already — directory is renco-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _RENCO_UID, _RENCO_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is renco-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _RENCO_UID, _RENCO_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``renco -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`renco profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-renco, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources RENCO_HOME (and any extra env) via with-contenv —
             so e.g. ``-e RENCO_HOME=/data/renco`` is honored at run
          ...
          4. Drops to the renco user and exec's
             ``renco -p <profile> gateway run`` (or just ``renco
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``renco gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root RENCO_HOME profile" (the implicit profile that exists at
        the top of $RENCO_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``renco_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$RENCO_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``RENCO_HOME``,
        ...
        allocator in ``renco_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export RENCO_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's RENCO_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``renco gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-RENCO_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The RENCO_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own RENCO_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "renco gateway run --replace"
        else:
            gateway_cmd = f"renco -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid renco {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${RENCO_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the RENCO_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e RENCO_HOME=/data/renco``
        gets its logs under /data/renco/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``renco logs`` reads and what
        ...
        return (
            ...
            f': "${{RENCO_HOME:=/opt/data}}"\n'
            f'log_dir="$RENCO_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as renco when
            # this script starts as root. Never chown or unlink renco-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is renco-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded renco-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid renco mkdir -p "$log_dir"\n'
            f'  s6-setuidgid renco rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid renco s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``renco gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``renco -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with renco ownership
            ...
            # the renco user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is renco-owned (see ``_seed_supervise_skeleton``), and on
