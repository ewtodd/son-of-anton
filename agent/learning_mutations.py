...
- **skills** → the skill name (e.g. ``"debugging-son-of-anton-desktop"``)
...
shared by the CLI (``son-of-anton journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``son-of-anton curator restore``); deleting a memory rewrites its
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (son-of-anton curator unpin {name})"}

    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()

    return {"ok": ok, "message": f"archived '{name}' — restore with: son-of-anton curator restore {name}" if ok else message}
