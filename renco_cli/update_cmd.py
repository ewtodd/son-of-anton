"""Renco update pipeline — extracted from ``renco_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``renco_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``renco_cli.main`` reference — so existing call sites and test monkeypatches
that target ``renco_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``renco_cli.main``.
...
Imports are one-way: ``renco_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``renco_cli.main`` reference.

    Lets callers keep patching ``renco_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``renco_cli.main`` -> ``renco_cli.update_cmd`` stays one-way
    at import time.
    """
    from renco_cli import main
...
#: ``_purge_stale_renco_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "renco_cli.renco_logging",
    }
)


def _purge_stale_renco_modules() -> None:
    """Evict every cached Renco module after the checkout changed in-place.

    ``renco update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from renco_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``renco_cli.gateway`` does
    ``from renco_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Renco
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Renco module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Renco modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``renco update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``renco update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``renco_cli.config_defaults``,
...
def _run_config_check_fresh() -> tuple:
    ...
    from renco_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from renco_cli.config import migrate_config
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``renco.exe`` has
    ...
    Skipping is safe because Renco pins its editable finder to a *static*
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``renco`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="renco-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``renco update`` can be driven by a
    ...
    """
    from renco_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from renco_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('renco_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``renco update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    ...
    print("  Preview now:  renco curator run --dry-run")
    print("  Pause it:     renco curator pause")
    print(
        "  Docs:         https://renco-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `renco update`.
    ...
    try:
        ...
        return
    db_path = get_renco_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `renco sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    renco sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  renco sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``renco update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``renco update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: renco curator status)"
    )
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``renco_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.renco-update-staging"
    backup = f"{dst}.renco-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `renco update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``renco_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``renco update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && renco update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("RENCO_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== renco-update completed {action_id} ===")
...
def _write_gateway_update_exit_code(ok: bool) -> None:
    path = get_renco_home() / ".update_exit_code"
    try:
...
def _update_via_zip(args) -> bool:
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `renco update "
            f"--branch {branch}`, or update against main with `renco update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/ewtodd/renco/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="renco-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"renco-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for renco-agent itself
        ...
        # GitHub ZIPs extract to renco-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"renco-agent-{branch}")
        ...
        print(
            "  Re-run `renco update` to retry; if the agent won't start, "
            "reinstall from https://renco-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from renco_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `renco update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: renco skills list-modified  "
    ...
    try:
        from renco_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from renco_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_renco_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_renco_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "renco-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Renco behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes renco completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Renco couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Renco couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Renco behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Renco couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Renco couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/ewtodd/renco.git",
    "git@github.com:ewtodd/renco.git",
    "https://github.com/ewtodd/renco",
    "git@github.com:ewtodd/renco",
}

OFFICIAL_REPO_URL = "https://github.com/ewtodd/renco.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_renco_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_renco_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Renco repository.")
        print("  This means you may miss updates from ewtodd/renco.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/ewtodd/renco.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/ewtodd/renco.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``renco update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from renco_constants import get_default_renco_root

    default_home = get_default_renco_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "renco.exe"
    lines = ["✗ Another renco.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Renco Desktop, exit any open `renco` REPLs, and")
    lines.append("  stop the gateway (`renco gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `renco update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``renco tools``."""
    try:
        from renco_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Renco Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``renco tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Renco Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Renco Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``renco update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `renco update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `renco update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Renco' editable-install extras or
    ...
    try:
        from renco_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$RENCO_HOME/bin/uv``, but on Termux the official
    ...
    """
    from renco_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `renco update` is exactly the step
...
def _npm_lockfile_changed(renco_root: Path) -> bool:
    ...
    try:
        ...
        cache_file = renco_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(renco_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    renco update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `renco update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from renco_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `renco update`.")
            failed = []
            if any(
    ...
    # Renco profile using this checkout. Keep one per-checkout cache under the
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `renco update`.")
        return list(labels)
    ...
    nixos_env = with_renco_node_path(_m()._nixos_build_env())
    ...
    # `renco update` comes from the *desktop* build, not this step; that
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.renco/logs/update.log`` only, never the terminal.

    During ``renco update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``renco update --check``: fetch and report without installing.
    ...
    from renco_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from renco_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``renco update`` without requiring a reinstall.  The
    ...
    ``renco`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/renco.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v renco'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/renco, code at /usr/local/lib/renco-agent).
    fhs_link = Path("/usr/local/bin/renco")
    ...
    path_comment = (
        "# Renco Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of RENCO_HOME under
      ``backups/`` (restorable via ``renco import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from renco_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from renco_constants import
        # get_renco_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from renco_cli.config import get_renco_home as _get_home
    ...
    try:
        from renco_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from renco_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_renco_home so the user sees ~/.renco/...
    try:
        from renco_constants import get_renco_home, display_renco_home

        home = get_renco_home()
        try:
            display_path = f"{display_renco_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  renco import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``renco update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``renco update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".renco-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    for proc in proc_iter:
        ...
        # the `-m renco_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``renco_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``renco.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``renco`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `renco` launch will complete it")
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Renco processes are running from this install's venv:",
    ]
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``renco update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from renco_cli._scan_venv_blockers import _is_pausable_gateway
...
def _ledger_reapable_backend_pids(
    ...
    (``renco_cli.process_identity``). A holder qualifies when ALL of:
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.renco-runtime`` interpreter child alive and holding the install open
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    renco.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Renco...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m renco_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: renco gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``renco gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from renco_cli import gateway_windows
        from renco_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    (``renco-gateway-helper`` is fine — profile units are
    ``renco-gateway-<profile>`` — but ``renco-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "renco-gateway" or svc_name.startswith("renco-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    print("    renco gateway status")
    if any(not name.startswith("ai.renco.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.renco.") for name in ordered):
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.renco.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``renco_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``renco_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``renco update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``renco-setup.exe``) executes
    ``$RENCO_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_renco_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``renco update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``renco update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from renco_cli.update_receipt import begin_update_receipt
    ...
    # running Renco runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another renco.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_renco_instances(scripts_dir)
            if concurrent:
    ...
    try:
        from renco_cli.update_receipt import record_step
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Renco backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Renco backend process(es) hold the venv; stopping their trees"
                )
    ...
    # process by design (a CLI `renco update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by renco.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Renco windows/gateways and re-run: renco update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Renco gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in renco_cli/config.py bricked
        # every user who ran ``renco update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``renco
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``renco update`` again later once a fix lands.")
                    else:
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && renco update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_renco_home added to renco_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``renco`` launch finishes the install
        ...
        # symbols in renco_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `renco` again "
        ...
        if not import_ok:
            ...
            print("    Run `renco update` again — if it persists, reinstall:")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_renco_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.renco/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit RENCO_HOME so
        # it is not affected by sync_skills()'s module-level RENCO_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's RENCO_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from renco_cli.profiles import backfill_profile_envs
        ...
        # ``renco update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``renco_cli.config`` and
        # ``renco_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from renco_cli.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'renco config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'renco config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: renco config migrate")
            else:
                print()
                print("Skipped. Run 'renco config migrate' later to configure.")
        else:
        ...
        # check `renco curator status`. Self-stamps after printing so it
        ...
        #
        # Purge stale cached Renco modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... renco-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... renco-gateway*)
            ...
            try:
                from renco_cli.gateway import _get_restart_exit_wait_budget
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: renco gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `renco update` restart it automatically, allow\n"
            ...
            # Restart EVERY ai.renco.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: renco gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: renco -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_renco_home() / ".update_exit_code"
                    try:
            ...
            try:
                from renco_cli.update_receipt import record_gateway_restart
        ...
        # Warn if legacy Renco gateway unit files are still installed.
        # When both renco.service (from a pre-rename install) and the
        # current renco-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `renco update` surfaces the issue until the user migrates.
        try:
            from renco_cli.gateway import (
                has_legacy_renco_units,
                _find_legacy_renco_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_renco_units():
                print()
                print("⚠ Legacy Renco gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_renco_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (renco.service) fight the current")
                print("  renco-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    renco gateway migrate-legacy")
                print()
        ...
        try:
            from renco_cli.update_receipt import (
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``renco_cli.gateway`` no longer imports,
