"""Tests for the ``renco hooks`` CLI subcommand."""
...
from renco_cli import hooks as hooks_cli
...
class TestHooksTest:
    def test_synthetic_payload_matches_production_shape(self, tmp_path):
        """`renco hooks test` must feed the script stdin in the same
        shape invoke_hook() would at runtime.  Prior to this fix,
        run_once bypassed _serialize_payload and the two paths diverged —
        scripts tested with `renco hooks test` saw different top-level
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `renco hooks doctor` used to run every
