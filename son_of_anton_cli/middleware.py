"""Son of Anton middleware contract helpers.
...
OBSERVER_SCHEMA_VERSION = "son-of-anton.observer.v1"
MIDDLEWARE_SCHEMA_VERSION = "son-of-anton.middleware.v1"
...
def apply_llm_request_middleware(
    ...
    **context: Any,
    ...
    provider kwargs before Son of Anton sends them.
    """
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from son_of_anton_cli.plugins import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from son_of_anton_cli.plugins import has_middleware
