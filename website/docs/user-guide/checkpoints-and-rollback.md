...
Renco Agent can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.

Enable checkpoints per-session with `--checkpoints`:

```bash
renco chat --checkpoints
```

Or enable globally in `~/.renco/config.yaml`:
...
| Command | Description |
|---------|-------------|
| `renco checkpoints` | Show total size, project count, per-project breakdown |
| `renco checkpoints status` | Same as bare `checkpoints` |
| `renco checkpoints list` | Alias for `status` |
| `renco checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `renco checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `renco checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Renco detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.renco/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/renco/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.renco/checkpoints/ at startup
  ...
  # `renco checkpoints prune` command below, with a confirmation prompt.
  auto_prune: true
  retention_days: 7
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `renco checkpoints prune` manually.
...
Renco responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.renco/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/renco-agent                                        20       2h ago  live
...
Clear with: renco checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
renco checkpoints prune --retention-days 3 --max-size-mb 200
```
...
`/rollback <N>` restores only the files Renco itself changed. Every successful
...
Renco last wrote (you edited it afterwards, or Renco never touched it) is
...
If the ledger is empty (a store created before this feature, or Renco hasn't
...
- **Directory scope** — Renco skips overly broad directories (root `/`, home `$HOME`).
...
~/.renco/checkpoints/
  ...
  │   ├── refs/renco/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `renco checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.renco/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.renco/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
renco checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `renco chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `renco checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Renco session in its own worktree/branch, with checkpoints as an extra layer.
