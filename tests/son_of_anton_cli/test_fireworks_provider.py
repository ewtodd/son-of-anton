...
from son_of_anton_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS, normalize_provider
...
class TestFireworksAliases:
    ...
    def test_providers_normalize_provider(self, alias):
        from son_of_anton_cli.providers import normalize_provider as normalize_in_providers
...
class TestFireworksOverlay:
    def test_overlay_exists(self):
        ...
        assert "fireworks" in SON_OF_ANTON_OVERLAYS
        overlay = SON_OF_ANTON_OVERLAYS["fireworks"]
...
class TestFireworksAuxiliary:
    ...
    def test_client_sends_attribution_headers(self, monkeypatch):
        ...
        assert headers["HTTP-Referer"] == "https://son-of-anton.nousresearch.com"
        assert headers["X-Title"] == "Son of Anton Agent"
        assert kwargs["base_url"] == "https://api.fireworks.ai/inference/v1"

    def test_client_sends_son_of_anton_user_agent(self, monkeypatch):
        ...
        headers = kwargs.get("default_headers", {})
        assert headers["User-Agent"].startswith("SonOfAntonAgent/")
