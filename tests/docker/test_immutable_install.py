...
Build the real image and verify at runtime:
  ...
  1. /opt/renco is not writable by the renco user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and RENCO_DISABLE_LAZY_INSTALLS are set
  3. /opt/renco/.install_method contains "docker" (code-scoped stamp)
  4. $RENCO_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $RENCO_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_renco(
    built_image: str, container_name: str,
) -> None:
    """The renco user must not be able to modify /opt/renco.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/renco as the renco user
        "touch /opt/renco/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"renco user can write to /opt/renco (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/renco/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"renco user can write to /opt/renco/.venv: {r.stdout}"
    )


def test_renco_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    RENCO_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$RENCO_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"RENCO_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $RENCO_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $RENCO_HOME was not healed on boot: "
        f"{r.stdout}"
    )
