...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/productivity/siyuan` |
...
3. Store it in `${RENCO_HOME:-~/.renco}/.env`:
   ```
...
```yaml
# In ~/.renco/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
