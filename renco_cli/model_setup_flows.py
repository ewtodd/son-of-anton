"""Per-provider model-selection wizard flows for ``renco setup`` / ``renco model``.

Extracted from ``renco_cli/main.py`` as part of the god-file decomposition
campaign (``~/.renco/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from renco_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``renco_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from renco_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``renco_cli.main`` at import time -> no import cycle.
"""
...
def _existing_api_key_for_model_flow(provider_id: str, pconfig) -> tuple[str, str]:
    ...
    from renco_cli.auth import _resolve_api_key_provider_secret
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    try:
        ...
        from renco_cli.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from renco_cli.main import _prompt_api_key
    from renco_constants import OPENROUTER_BASE_URL
    ...
    # in-flow (K/R/C) instead of having to edit ~/.renco/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `renco setup` from scratch. OpenRouter
    ...
    from renco_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.renco/.env by hand.
    ...
    from renco_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from renco_cli.auth import _save_model_choice, deactivate_provider
    from renco_cli.config import load_config, save_config
    ...
    if not presets:
        print("No MoA presets configured. Run `renco moa configure <name>` first.")
        return
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from renco_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from renco_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `renco auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``renco auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from renco_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from renco_cli.auth import (
    ...
    from renco_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from renco_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from renco_cli.auth import _save_model_choice, deactivate_provider
    from renco_cli.config import (
    ...
    from renco_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `renco model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from renco_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from renco_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from renco_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Renco will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Renco will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from renco_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from renco_cli.auth import _save_model_choice, deactivate_provider
    from renco_cli.config import load_config, normalize_extra_headers, save_config
...
def _model_flow_copilot(config, current_model=""):
    ...
    from renco_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from renco_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from renco_cli.config import save_env_value, load_config, save_config
    from renco_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from renco_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from renco_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Renco turns to `copilot --acp`.")
    print("  Renco currently starts its own ACP subprocess for each request.")
    print("  Renco uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set RENCO_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from renco_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from renco_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from renco_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `renco auth`)
    from renco_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from renco_cli.config import load_config, save_config, get_env_value
    from renco_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.renco/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``renco_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from renco_cli.main import _prompt_provider_choice
    from renco_cli.auth import ZAI_ENDPOINTS
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Renco typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Renco, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from renco_cli.main import _run_anthropic_oauth_flow
    from renco_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'renco model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
