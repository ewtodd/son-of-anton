...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``son_of_anton_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$SON_OF_ANTON_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Son of Anton")
    ...
    """
    name = getattr(args, "name", None) or "Son of Anton"
    description = getattr(args, "description", None) or "Your Son of Anton agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "son-of-anton slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"son-of-anton slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "son-of-anton slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "son-of-anton slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Son of Anton app\n"
