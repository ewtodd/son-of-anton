...
import { terminalBackgroundHex } from '@sonofanton/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $SON_OF_ANTON_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see SON_OF_ANTON_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['SON_OF_ANTON_TUI_BACKGROUND', process.env.SON_OF_ANTON_TUI_BACKGROUND ?? '(unset)'],
            ['SON_OF_ANTON_TUI_THEME', process.env.SON_OF_ANTON_TUI_THEME ?? '(unset)'],
