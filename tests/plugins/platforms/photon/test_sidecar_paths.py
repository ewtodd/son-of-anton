...
``/opt/renco``; ``resolve_sidecar_dir`` must run in place when the deps are
baked and current, and mirror the sidecar to the writable ``RENCO_HOME``
...
def test_adapter_import_does_not_resolve_sidecar_dir(monkeypatch) -> None:
    ...
    files to RENCO_HOME; the adapter and CLI resolve lazily on first use so
    a bare import (plugin discovery, `renco --help`, test collection) has
