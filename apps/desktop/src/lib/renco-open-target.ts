 ...
 * `renco://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `renco://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `renco://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type RencoOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const RENCO_PROTOCOL = 'renco:'
...
export function normalizeRencoOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('renco://') || trimmed.startsWith(`${RENCO_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // renco://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // renco://index-network/intent/1 → /index-network/intent/1
...
export function resolveRencoOpenPath(target: RencoOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeRencoOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeRencoOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeRencoOpenString(target.path)
 ...
 * (`renco://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromRencoDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveRencoOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveRencoOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `renco://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromRencoDeepLink('open', name, params)
}
