...
from son_of_anton_cli.auth import PROVIDER_REGISTRY, resolve_provider, resolve_api_key_provider_credentials
from son_of_anton_cli.models import _PROVIDER_MODELS, _PROVIDER_LABELS, _PROVIDER_ALIASES, normalize_provider
...
class TestOllamaCloudModelCatalog:
    ...
    def test_provider_model_ids_returns_dynamic_models(self, tmp_path, monkeypatch):
        ...
        with patch("son_of_anton_cli.models.fetch_api_models", return_value=["qwen3.5:397b"]), \
...
class TestOllamaCloudMergedDiscovery:
    def test_merges_live_and_models_dev(self, tmp_path, monkeypatch):
        ...
        with patch("son_of_anton_cli.models.fetch_api_models", return_value=["qwen3.5:397b", "glm-5"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
...
class TestOllamaCloudProvidersNew:
    def test_overlay_exists(self):
        ...
        assert "ollama-cloud" in SON_OF_ANTON_OVERLAYS
        overlay = SON_OF_ANTON_OVERLAYS["ollama-cloud"]
        assert overlay.transport == "openai_chat"
        assert overlay.base_url_env_var == "OLLAMA_BASE_URL"

    def test_alias_resolves(self):
        from son_of_anton_cli.providers import normalize_provider as np
...
class TestOllamaCloudSuffixStripping:
    ...
    def test_no_duplicate_when_live_clean_and_mdev_suffixed(self, tmp_path, monkeypatch):
        ...
        with patch("son_of_anton_cli.models.fetch_api_models", return_value=["kimi-k2.6", "glm-5.1"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_strip_suffix_helper(self):
        ...
        from son_of_anton_cli.models import _strip_ollama_cloud_suffix
