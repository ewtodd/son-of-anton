"""Tests for file write safety and SON_OF_ANTON_WRITE_SAFE_ROOT sandboxing.
...
class TestSafeWriteRoot:
    """SON_OF_ANTON_WRITE_SAFE_ROOT should sandbox writes to a specific subtree."""
    ...
    def test_safe_root_does_not_override_static_deny(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("SON_OF_ANTON_WRITE_SAFE_ROOT", os.path.expanduser("~"))
        assert _is_write_denied(os.path.expanduser("~/.ssh/id_rsa")) is True


class TestMultipleSafeWriteRoots:
    """SON_OF_ANTON_WRITE_SAFE_ROOT with multiple colon-separated directories."""
    ...
    def test_write_inside_first_root_allowed(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("SON_OF_ANTON_WRITE_SAFE_ROOT", f"{root_a}{os.pathsep}{root_b}")
        assert _is_write_denied(str(child)) is False
    ...
    def test_trailing_separator_ignored(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("SON_OF_ANTON_WRITE_SAFE_ROOT", f"{root}{os.pathsep}")
        assert _is_write_denied(str(inside)) is False
...
class TestGetWriteDeniedError:
    ...
    def test_credential_path_message(self):
        ...
        assert "SON_OF_ANTON_WRITE_SAFE_ROOT" not in err
    ...
    def test_safe_root_message(self, tmp_path: Path, monkeypatch):
        ...
        assert err is not None
        assert "outside SON_OF_ANTON_WRITE_SAFE_ROOT" in err
...
class TestSafeRootDenialMessageIntegration:
    ...
    message when SON_OF_ANTON_WRITE_SAFE_ROOT blocks a path.
    ...
    def test_write_file_safe_root_outside_shows_safe_root_message(
        ...
        assert "outside SON_OF_ANTON_WRITE_SAFE_ROOT" in res.error
...
class TestAtomicWrite:
    ...
    def test_no_temp_file_leaked_on_success(self, ops, tmp_path: Path):
        ...
        assert [p for p in os.listdir(tmp_path) if ".son-of-anton-tmp" in p] == []
...
class TestProtectedInstructionFiles:
    ...
    AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project-local .son-of-anton
    ...
    def test_project_local_son_of_anton_dir_is_gated(self, tmp_path, approvals):
        proj = tmp_path / "proj" / ".son-of-anton"
        proj.mkdir(parents=True)
        approvals["answer"] = "deny"
        res = self._write(proj / "config.yaml")
        assert res.get("error") and "BLOCKED" in res["error"]

    def test_checkout_nested_under_son_of_anton_dir_not_gated(self, tmp_path, approvals):
        """A repo living UNDER a .son-of-anton dir (e.g. ~/.son-of-anton/son-of-anton)
        must not have every write gated — only files directly inside a
        .son-of-anton dir count as project config."""
        repo = tmp_path / ".son-of-anton" / "some-repo" / "src"
        repo.mkdir(parents=True)
        res = self._write(repo / "module.py", "x = 1\n")
        assert not res.get("error"), res
        assert approvals["calls"] == []

    def test_real_son_of_anton_home_not_gated_by_this_check(
        self, tmp_path, approvals, monkeypatch
    ):
        """~/.son-of-anton itself is governed by existing guards, not this gate."""
        import tools.file_tools as ft
        fake_home = tmp_path / ".son-of-anton"
        (fake_home / "notes").mkdir(parents=True)
        monkeypatch.setattr(
            ft, "_get_real_son_of_anton_home", lambda: str(fake_home.resolve())
        )
