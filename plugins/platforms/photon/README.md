...
This plugin connects Son of Anton Agent to iMessage (and other Spectrum
...
TypeScript-only, Son of Anton runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Son of Anton channel,
...
Run `son-of-anton photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.son-of-anton/.env` (the same place every other
...
Management metadata lives in `~/.son-of-anton/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `son-of-anton photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Son of Anton
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Son of Anton coalesces
...
- **Native polls are supported.** Son of Anton posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `son-of-anton send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<son-of-anton-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `son-of-anton photon status`, a DM and a group roundtrip,
