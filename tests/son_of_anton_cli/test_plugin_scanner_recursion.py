...
def _enable(son_of_anton_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<son_of_anton_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        son_of_anton_home = Path(os.environ["SON_OF_ANTON_HOME"])  # set by hermetic conftest fixture
        user_plugins = son_of_anton_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(son_of_anton_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        son_of_anton_home = Path(os.environ["SON_OF_ANTON_HOME"])  # set by hermetic conftest fixture
        _write_plugin(son_of_anton_home / "plugins", ["p1"])
        _enable(son_of_anton_home, "p1")
...
class TestBackendGate:
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(son_of_anton_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(son_of_anton_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(son_of_anton_home, "bad-img-plugin")
