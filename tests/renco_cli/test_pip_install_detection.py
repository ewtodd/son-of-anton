...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $RENCO_HOME.

    Models a host git install whose $RENCO_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'renco update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("renco_cli.config.get_managed_system", return_value=None), \
         patch("renco_cli.config.get_renco_home", return_value=home):
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $RENCO_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
    with patch("renco_cli.config.get_renco_home", return_value=home):
        from renco_cli.config import stamp_install_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``renco update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("renco_cli.config.get_managed_system", return_value=None), \
         patch("renco_cli.config.get_renco_home", return_value=tmp_path), \
         patch("renco_constants.is_container", return_value=True):
        from renco_cli.config import detect_install_method
