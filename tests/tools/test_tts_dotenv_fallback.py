...
TTS provider tools must resolve API keys from ``~/.renco/.env`` (via
``renco_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``renco_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.renco/.env`` carries the
    ...
    ``renco_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``renco_cli.config.load_env`` to simulate ``~/.renco/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while renco_cli.config.get_env_value is patched must
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        # Simulate ~/.renco/.env carrying the key (load_env returns the dict
        ...
        with patch(
            ...
            # os.environ is empty.
            from renco_cli.config import get_env_value as live_get
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.renco/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
