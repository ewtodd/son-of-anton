...
1. **Update receipt** — a machine-readable JSON record of what one
   ``renco update`` run discovered, did, skipped (and why), written to
   ``<RENCO_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``renco_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``renco update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``renco update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_renco_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``renco update`` has many early-termination paths (Windows
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if any_stale:
        ...
        print("      renco gateway restart                # active profile")
        print("      renco -p <profile> gateway restart   # named profile")
    return any_stale
