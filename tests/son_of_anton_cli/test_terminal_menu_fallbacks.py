...
def test_prompt_model_selection_requires_expensive_confirmation(monkeypatch, capsys):
    ...
    monkeypatch.setattr("son_of_anton_cli.curses_ui.curses_radiolist", _raise_menu)
    monkeypatch.setattr(
        "son_of_anton_cli.model_cost_guard.expensive_model_warning",
...
def test_prompt_model_selection_uses_line_editor_for_custom_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "son_of_anton_cli.cli_output.line_input",
        lambda prompt_text: (
...
def test_remove_custom_provider_falls_back_on_menu_runtime_error(tmp_path, monkeypatch):
    from son_of_anton_cli.main import _remove_custom_provider
