...
import { getProfiles, rencoApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/renco'
...
import type { ProfileInfo } from '@/types/renco'
...
// /api/profiles/active `current`). "default" is the root ~/.renco. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'renco.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'renco.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await rencoApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new RENCO_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.rencoDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<RencoConnection | null> {
  const getConnection = window.rencoDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<RencoConnection | null> {
  const getConnectionFor = window.rencoDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'renco.desktop.showAllProfiles'
...
// Switch to the default (root ~/.renco) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.rencoDesktop?.touchBackend?.(target).catch(() => undefined)
}
