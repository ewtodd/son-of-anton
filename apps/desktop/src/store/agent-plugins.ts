...
/**
 * Feature store for backend (agent) plugins — the native Renco plugins plus
 ...
 * primitives `renco plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Renco. */
  portable?: boolean
}
 ...
 *  RENCO_HOME. Always refetches (it's a cheap local disk scan on the
