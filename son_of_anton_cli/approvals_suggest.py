"""``son-of-anton approvals suggest`` — mine approval history into allowlist proposals.

Son of Anton has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.son-of-anton/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, son-of-anton self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/son-of-anton home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_son_of_anton_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_son_of_anton_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  son-of-anton approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.son-of-anton/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``son-of-anton approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``son-of-anton approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from son_of_anton_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: son-of-anton approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `son-of-anton approvals <subcommand> -h` for details."
    )
    return 1
