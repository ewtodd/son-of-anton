"""
Lazy dependency installer for opt-in Son of Anton Agent backends.

Many Son of Anton features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``son-of-anton[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``son-of-anton tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``SON_OF_ANTON_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/son-of-anton`` read-only), setting
  ``SON_OF_ANTON_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Son of Anton, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (SON_OF_ANTON_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # Signal does not need a Python SDK — it talks to a local signal-cli
    # REST bridge over HTTP, so there is no lazy-install entry for it.
    ...
    # HF Agent Trace Viewer upload (son-of-anton trace upload / /upload-trace).
    #
    ...
    # presence, the `son-of-anton update` lazy-refresh pass re-asserts THIS pin on
...
def _allow_lazy_installs() -> bool:
    ...
    if os.environ.get("SON_OF_ANTON_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``son-of-anton update`` lazy-refresh pass.
    """
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``son-of-anton update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="son-of-anton-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``son_of_anton_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = son_of_anton_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $SON_OF_ANTON_HOME/bin is never on PATH, so a bare
        # which() misses the uv Son of Anton installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Son of Anton
    ...
    # deployment sets SON_OF_ANTON_MANAGED=true *and* SON_OF_ANTON_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from son_of_anton_cli.config import get_managed_system
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Son of Anton cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Son of Anton instead)."
            )
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``SON_OF_ANTON_DISABLE_LAZY_INSTALLS=1``) and sets
      ``SON_OF_ANTON_LAZY_INSTALL_TARGET``, installs are redirected to the writable
    ...
    * **Gated** — honors ``security.allow_lazy_installs`` and refuses to run
    ...
    if not _allow_lazy_installs():
        target = _lazy_install_target()
        if os.environ.get("SON_OF_ANTON_DISABLE_LAZY_INSTALLS") == "1" and target is None:
            reason = (
                "runtime installs are disabled on this deployment: the agent "
                "environment is immutable and no writable install target is "
                "configured (SON_OF_ANTON_LAZY_INSTALL_TARGET)"
            )
        else:
