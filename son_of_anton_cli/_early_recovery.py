"""Dependency-light venv recovery that runs BEFORE son_of_anton_cli.main's imports.

The ``son-of-anton`` console entry point is ``son_of_anton_cli.main:main``.  Importing
``son_of_anton_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``son_of_anton_cli.env_loader``, ``yaml`` via ``son_of_anton_cli.config``, ...).  In
...
a corrupted venv.  ``son_of_anton_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``son_of_anton_cli.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``son-of-anton acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``son_of_anton_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``son_of_anton_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``son-of-anton update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `son-of-anton update`: CLI and Desktop retries preserve
...
# every `son-of-anton` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    failure does not block every launch (``son-of-anton acp`` included).
    ...
    """
    try:
        from son_of_anton_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; son-of-anton will keep working from "
