...
Usage:
    renco pairing list              # Show all pending + approved users
    renco pairing approve <platform> <request-id|code>  # Approve a pairing request
    renco pairing revoke <platform> <user_id> # Revoke user access
    renco pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle renco pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: renco pairing {list|approve|revoke|clear-pending}")
        print("Run 'renco pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: renco pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.renco/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'renco pairing list' to see pending requests.\n")
