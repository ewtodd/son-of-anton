...
When ``SON_OF_ANTON_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.son-of-anton/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.son-of-anton            (has its own auth.json fixture)
    * Profile     -> tmp_path/.son-of-anton/profiles/coder   (active, SON_OF_ANTON_HOME points here)
    ...
    global_root = tmp_path / ".son-of-anton"
    global_root.mkdir()
...
# ``_load_provider_state`` itself, those helpers raise ``"Son of Anton is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (SON_OF_ANTON_HOME != ~/.son-of-anton, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    son_of_anton_home = tmp_path / "classic"
