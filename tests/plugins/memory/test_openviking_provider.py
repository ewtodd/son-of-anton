...
def test_openviking_provider_config_loader_uses_readonly_config(monkeypatch):
    ...
    config = openviking_module._load_renco_openviking_config()
...
""",
    ...
    settings = openviking_module._resolve_connection_settings(
        openviking_module._load_renco_openviking_config()
    )
...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(tmp_path, monkeypatch):
    ...
    from renco_cli import memory_setup
    ...
    OpenVikingMemoryProvider().post_setup(str(renco_home), config)
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Renco's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Renco
    venv instead of its own, and on Windows locks Renco venv DLLs so the
    venv cannot be rebuilt during `renco update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/renco/.venv/Lib/site-packages")
    monkeypatch.setenv("RENCO_PROFILE", "test-profile")
    ...
    assert child_env.get("RENCO_PROFILE") == "test-profile"
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Renco will retry on a later access "
...
def test_tool_add_resource_rejects_renco_credential_file_upload(tmp_path, monkeypatch):
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "renco"
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (renco-agent#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "renco"},
        ]
    }]
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for renco-agent#28296 review (M3): once a session is marked
...
def test_concurrent_providers_claim_unlocked_pending_owner_once(
    ...
    for provider in providers:
        ...
        provider._renco_home = str(tmp_path)
...
def test_is_available_false_without_any_endpoint(monkeypatch):
    _clear_openviking_env(monkeypatch)
    monkeypatch.setattr(
        openviking_module, "_load_renco_openviking_config", lambda: {}
    )
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Renco and
