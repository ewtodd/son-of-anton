...
``son_of_anton_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``son_of_anton_cli.config`` (get_env_value / save_env_value / remove_env_value)
and ``son_of_anton_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch
those source modules.
"""
import son_of_anton_cli.config as config_mod
import son_of_anton_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved, removed, existing):
    ...
    # Manifest writing reaches out to son_of_anton_cli.slack_cli + filesystem; stub it.
    import son_of_anton_cli.slack_cli as slack_cli_mod
