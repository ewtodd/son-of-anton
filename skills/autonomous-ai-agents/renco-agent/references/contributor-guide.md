...
For occasional contributors and PR authors. Full developer docs: https://renco-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.renco/config.yaml` (settings), `~/.renco/.env` (API keys) — both under `$RENCO_HOME` when it is set.
...
`_RENCO_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_renco_home()` for paths,
never hardcode `~/.renco`. For custom/local-only tools, write a plugin in
`~/.renco/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `renco_cli/commands.py`
...
- Tests auto-redirect `RENCO_HOME` to temp dirs — never touch real `~/.renco/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/renco_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_renco_home()` from `renco_constants` for all paths (profile-safe)
