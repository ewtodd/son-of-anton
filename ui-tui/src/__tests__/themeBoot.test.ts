...
// background into SON_OF_ANTON_TUI_BACKGROUND, which detectLightMode treats as a
...
describe('seedBootEnvironment', () => {
  it('seeds the cached background when no explicit signal outranks it', () => {
    ...
    expect(env.SON_OF_ANTON_TUI_BACKGROUND).toBe('#ffffff')
    expect(seeded).toEqual({ seededBackground: '#ffffff', seededPin: false })
  })

  it('never seeds over explicit user signals', () => {
    for (const preset of [{ SON_OF_ANTON_TUI_THEME: 'dark' }, { SON_OF_ANTON_TUI_LIGHT: '1' }] as NodeJS.ProcessEnv[]) {
      const env = { ...preset }
      const seeded = seedBootEnvironment(cache({ background: '#ffffff', mode: 'light' }), env)

      expect(env.SON_OF_ANTON_TUI_BACKGROUND).toBeUndefined()
    ...
    const env: NodeJS.ProcessEnv = { SON_OF_ANTON_TUI_BACKGROUND: '#123456' }

    expect(seedBootEnvironment(cache({ background: '#ffffff' }), env).seededBackground).toBeNull()
    expect(env.SON_OF_ANTON_TUI_BACKGROUND).toBe('#123456')
  })
  ...
  it('replays a cached config pin coherently with the physical background', () => {
    ...
    expect(env.SON_OF_ANTON_TUI_THEME).toBe('light')
    expect(env.SON_OF_ANTON_TUI_BACKGROUND).toBe('#1e1e1e')
  ...
  it('replays a pinned dark on a light physical background too', () => {
    ...
    expect(env.SON_OF_ANTON_TUI_THEME).toBe('dark')
    expect(env.SON_OF_ANTON_TUI_BACKGROUND).toBe('#ffffff')
...
describe('invalidateBootBackground', () => {
  ...
  it('leaves a trusted OSC answer that overwrote the seed alone', () => {
    ...
    env.SON_OF_ANTON_TUI_BACKGROUND = '#282828'

    expect(invalidateBootBackground(env)).toBe(false)
    expect(env.SON_OF_ANTON_TUI_BACKGROUND).toBe('#282828')
  })

  it('is a no-op when nothing was seeded', () => {
    const env: NodeJS.ProcessEnv = { SON_OF_ANTON_TUI_BACKGROUND: '#ffffff' }
