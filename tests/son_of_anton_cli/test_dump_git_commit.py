"""Tests for son_of_anton_cli.dump._get_git_commit — git SHA resolution for ``son-of-anton dump``.

``son-of-anton dump`` prints the running commit so support bug reports identify the
...
written by the Dockerfile's ``SON_OF_ANTON_GIT_SHA`` build-arg.
...
def test_get_git_commit_uses_live_git_when_available(tmp_path):
    ...
    from son_of_anton_cli import dump
    ...
    with patch("son_of_anton_cli.dump.subprocess.run", return_value=git_result) as mock_run, \
         patch("son_of_anton_cli.build_info.get_build_sha") as mock_build:
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``son-of-anton dump`` doesn't have to special-case
    ...
    with patch("son_of_anton_cli.dump.subprocess.run", return_value=git_result):
        live = dump._get_git_commit(repo_dir)

    # Baked-SHA path.
    failed = MagicMock(returncode=128, stdout="")
    with patch("son_of_anton_cli.dump.subprocess.run", return_value=failed), \
         patch("son_of_anton_cli.build_info.get_build_sha", return_value="b2f477a3"):
