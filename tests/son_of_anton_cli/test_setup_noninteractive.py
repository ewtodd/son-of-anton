...
import pytest
from son_of_anton_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `son-of-anton` should not prompt for input when no provider exists and stdin is headless."""
        from son_of_anton_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("son_of_anton_cli.main._has_any_provider_configured", return_value=False),
            patch("son_of_anton_cli.main.cmd_setup") as mock_setup,
        ...
        out = capsys.readouterr().out
        assert "son-of-anton config set model.provider custom" in out
