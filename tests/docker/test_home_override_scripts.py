...
def test_dashboard_service_resets_home(
    ...
    container with RENCO_DASHBOARD=1 and verify the dashboard process
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    RENCO_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "RENCO_DASHBOARD=1", "RENCO_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to renco:renco on every boot.
    ...
    # Verify files are now owned by renco.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "renco" in r.stdout, (
        f"expected renco-owned files after restart, got: {r.stdout!r} — "
