...
from renco_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import renco_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $RENCO_HOME/node is
...
def _has_renco_agent_browser() -> bool:
    ...
    # install.sh installs globally into $RENCO_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-RencoHome", str(get_renco_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = renco_subprocess_env(inherit_credentials=False)
