...
export const ZOOM_STORAGE_KEY = 'renco:desktop:zoomLevel'
 ...
 * but forgot to emit 'renco:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('renco:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
