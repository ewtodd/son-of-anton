...
def auth_json_path():
    """Return the Son of Anton auth store path, respecting SON_OF_ANTON_HOME overrides."""
    return get_son_of_anton_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`son-of-anton tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Son of Anton release, and
