...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update Son of Anton Desktop and try again.')
    }
...
export interface SonOfAntonWebSocketUrlOptions {
...
export function buildSonOfAntonWebSocketUrl(options: SonOfAntonWebSocketUrlOptions): string {
