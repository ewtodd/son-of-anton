...
class TestHasKnownPricing:
    ...
    def test_unknown_custom_model(self):
        assert _has_known_pricing("FP16_Son of Anton_4.5") is False
...
class TestTerminalFormatting:
    def test_terminal_format_has_sections(self, populated_db):
        ...
        assert "Son of Anton Insights" in text
...
class TestEdgeCases:
    ...
    def test_custom_model_shows_zero_cost(self, db):
        ...
        db.create_session(session_id="s1", source="cli", model="FP16_Son of Anton_4.5")
        ...
        assert "FP16_Son of Anton_4.5" in report["overview"]["models_without_pricing"]

        models = report["models"]
        custom = next(m for m in models if m["model"] == "FP16_Son of Anton_4.5")
