...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Renco Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${RENCO_HOME:-~/.renco}/.env`:
...
```bash
CANVAS="python $RENCO_HOME/skills/productivity/canvas/scripts/canvas_api.py"
