"""``renco verify`` subcommand parser.

Follows the pattern of ``renco_cli/subcommands/doctor.py``: parser built
...
# avoid paying an extra module import on every `renco` invocation.
...
def build_verify_parser(subparsers, *, cmd_verify: Callable) -> None:
    ...
    verify_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "(or load the saved manifest at .renco/environment.json), then "
    ...
    verify_parser.add_argument(
        "--save",
        action="store_true",
        help="Save the recipe as .renco/environment.json in the project",
    )
    verify_parser.add_argument(
        "--skip-start",
        action="store_true",
