...
Design notes / invariants (same contract as ``son_of_anton_cli/goals.py``):
...
- This module has zero hard dependency on ``cli.SonOfAntonCLI``, the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    """
    try:
        from son_of_anton_cli.goals import _get_session_db as _goals_db
...
def save_loop(session_id: str, state: LoopState) -> None:
    ...
    db = _get_session_db()
    if db is None:
        from son_of_anton_cli.goals import _warn_dropped_write
...
class LoopManager:
    ...
    def complete_tick(self, last_response: str) -> Dict[str, Any]:
        ...
        if s.until and (last_response or "").strip():
            try:
                from son_of_anton_cli.goals import judge_goal
