...
Usage:
  ...
  <server_name> matches the files in $RENCO_HOME/mcp-tokens/<server>.json etc.
...
def _renco_home():
    # Prefer Renco' own resolver (profile-safe); fall back to env then ~/.renco.
    try:
        from renco_constants import get_renco_home
        return str(get_renco_home())
    except Exception:
        return os.environ.get("RENCO_HOME") or os.path.expanduser("~/.renco")


def _tokens_dir():
    return os.path.join(_renco_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "renco-diag", "version": "1.0"}},
        },
        headers={
