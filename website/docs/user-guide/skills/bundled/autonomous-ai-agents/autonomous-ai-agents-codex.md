...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`renco-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-renco-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Renco terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Renco itself, `model.provider: openai-codex` uses Renco-managed Codex
OAuth from `~/.renco/auth.json` after `renco auth add openai-codex`. For the
...
## Renco Gateway Caveat

When invoking the Codex CLI from a Renco gateway/service context (for example,
