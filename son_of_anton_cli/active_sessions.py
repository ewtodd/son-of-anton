...
def active_session_limit_message(
    ...
    return (
        f"Son of Anton is at the active session limit ({active_count}/{max_sessions})."
        f"{detail} Try again when another session finishes."
    )


def _state_dir() -> Path:
    return Path(get_son_of_anton_home()) / "runtime"
...
class ActiveSessionLease:
    ...
    # root ``SON_OF_ANTON_HOME`` must release against the same registry even when
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile SON_OF_ANTON_HOME override (#85431).
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``son-of-anton dashboard`` / ``serve``) never trips that
