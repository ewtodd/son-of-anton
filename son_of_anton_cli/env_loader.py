"""Helpers for loading Son of Anton .env files consistently across entrypoints."""
...
# load_son_of_anton_dotenv() calls (user env + project env, gateway hot-reload,
...
# that were injected by an external secret source during load_son_of_anton_dotenv().
# Used by setup / `son-of-anton model` flows to label detected credentials so
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Son of Anton keys absent from ``.env``.
    ...
    parent Son of Anton process injects and that silently change *which provider
    ...
    ``tests/son_of_anton_cli/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_son_of_anton_dotenv()`` call.  Returns
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(son_of_anton_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_son_of_anton_dotenv()'s .op.env bootstrap: the 1Password
        ...
        local_env["SON_OF_ANTON_HOME"] = str(home)
...
def reset_secret_source_cache() -> None:
    """Forget which SON_OF_ANTON_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `son-of-anton setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``son_of_anton_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from son_of_anton_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_son_of_anton_dotenv(
    *,
    son_of_anton_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Son of Anton environment files with user config taking precedence.

    Behavior:
    - `~/.son-of-anton/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(son_of_anton_home or os.getenv("SON_OF_ANTON_HOME", Path.home() / ".son-of-anton"))
    ...
    #   EnvironmentFile=-/path/to/.son-of-anton/.op.env
    ...
    # 2. A fresh ``son-of-anton update`` retry just completed a deferred dependency
    ...
    from son_of_anton_cli import _early_recovery
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    ``load_son_of_anton_dotenv(son_of_anton_home=...)`` call would bridge the wrong
    ...
    """
    try:
        if Path(home_path).resolve() != _process_son_of_anton_home().resolve():
            return
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of SON_OF_ANTON_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Son of Anton reads
    ...
    owns the once-per-SON_OF_ANTON_HOME guard, the post-apply ASCII
    ...
    # a generic dict entry must not force crypto load on every son-of-anton launch.
    ...
    # so the 3-5 import-time load_son_of_anton_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `son-of-anton model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    if home_path == _process_son_of_anton_home():
        try:
...
def _process_son_of_anton_home() -> Path:
    """The SON_OF_ANTON_HOME the shared config cache is keyed to."""
    try:
