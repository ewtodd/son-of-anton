...
three virtual model IDs so the ``renco tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `renco tools` → Image "
                    "Generation → OpenAI to configure, or `renco setup` "
                    "to add the key."
                ),
