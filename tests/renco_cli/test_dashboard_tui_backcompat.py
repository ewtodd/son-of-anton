"""Regression test: `renco dashboard --tui` must not hard-crash.

Older Renco desktop app shells (<= 0.15.x) spawn the backend as::

    renco dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Renco couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real renco_cli.main parser in a subprocess.
