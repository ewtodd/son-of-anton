...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Son of Anton command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Son of Anton accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Son of Anton ``/command`` text."""
...
# Uses get_son_of_anton_home() so each profile gets its own Matrix store.
from son_of_anton_constants import get_son_of_anton_dir as _get_son_of_anton_dir

_STORE_DIR = _get_son_of_anton_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Son of Anton
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("SON_OF_ANTON_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("SON_OF_ANTON_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    ...
                    device_name="Son of Anton Agent",
                    device_id=self._device_id or None,
                )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "son-of-anton"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Son of Anton Agent" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in son_of_anton_cli/{setup,gateway}.py, and the
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        txn_id = f"son_of_anton_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces son_of_anton_cli/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where Son of Anton delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via son_of_anton_cli.gateway.get_env_value so setup-status
