...
Before the s6 migration, ``docker run nousresearch/renco-agent gateway
...
crash and a supervised dashboard alongside (when ``RENCO_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``RENCO_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``RENCO_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``renco`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'renco.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'renco.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → renco gateway run) can take
    ...
    # (main-wrapper.sh → python → renco gateway run) can take well over
    ...
    # top), NOT the static main-renco service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-renco
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The RENCO_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``renco gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``renco gateway
    ...
    # (main-wrapper.sh → python → renco gateway run) can take well
    ...
    # started either — so we'd see the CMD's `renco gateway run`
    ...
    # `renco gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*renco.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `renco gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-renco`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``RENCO_DASHBOARD=1`` is set, ``docker run <image> gateway
