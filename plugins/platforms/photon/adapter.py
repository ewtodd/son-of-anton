"""
Photon Spectrum (iMessage) platform adapter for Son of Anton Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Son of Anton dep
...
#
# Out-of-process senders (cron subprocesses, `son-of-anton send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_son_of_anton_home() honors profile overrides — never hardcode ~/.son-of-anton.
    from son_of_anton_constants import get_son_of_anton_home

    return get_son_of_anton_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (son-of-anton status, test collection, plugin discovery).
...
# artifacts so Son of Anton sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `son-of-anton photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `son-of-anton photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `son-of-anton setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, son-of-anton status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: son-of-anton photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: son-of-anton photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `son-of-anton update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``son-of-anton photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Son of Anton does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Son of Anton defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Son of Anton-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Son of Anton checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `son-of-anton photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                ...
                raise PhotonSidecarStartupError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `son-of-anton photon setup`)",
                    code="SIDECAR_DEPS_MISSING",
                    retryable=False,
                )
        # A `son-of-anton update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `son-of-anton send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`son-of-anton send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Son of Anton gateway (which spawns the sidecar and "
                    "records its address under <son-of-anton-home>/runtime/"
    ...
    headers = {"X-Son of Anton-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Son of Anton plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `son-of-anton photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: son-of-anton photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `son-of-anton gateway setup` alongside every other
    ...
    # Register CLI subcommands — `son-of-anton photon ...`
