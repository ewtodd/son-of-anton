...
from son_of_anton_cli.subcommands.dashboard import build_dashboard_parser
...
from son_of_anton_cli.subcommands.gui import build_gui_parser
...
from son_of_anton_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`son-of-anton config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `son-of-anton login` fails gracefully, not with argparse error ────
#
# `son-of-anton login` is a removed command; its handler (`login_command` in
# `son_of_anton_cli/auth.py`) prints a deprecation notice pointing at `son-of-anton auth` /
# `son-of-anton model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `son-of-anton --help`.
