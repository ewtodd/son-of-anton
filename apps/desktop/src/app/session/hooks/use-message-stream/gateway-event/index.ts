...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshRencoConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshRencoConfig — two REST calls (config + defaults) per event, per
