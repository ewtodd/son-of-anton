...
The notice prints the most recent curator run summary on `son-of-anton update`,
...
session start) so users normally never see the rename map. `son-of-anton update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(son_of_anton_main)

    yield {
        "curator": curator,
        "main": son_of_anton_main,
        "capsys": capsys,
    }
