...
def _is_son_of_anton_provider_credential(name: str) -> bool:
    """True if ``name`` is a Son of Anton-managed provider credential (API key,
    token, or similar) per ``_SON_OF_ANTON_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Son of Anton API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Son of Anton credential into the execute_code child.
    """
    try:
        from tools.environments.local import (
            _SON_OF_ANTON_PROVIDER_ENV_BLOCKLIST,
            _is_son_of_anton_internal_secret,
        )
        ...
        return True
    # Dynamically-generated Son of Anton-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_son_of_anton_internal_secret(name):
        return True
    return name in _SON_OF_ANTON_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Son of Anton-managed provider credentials (from
    ``_SON_OF_ANTON_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Son of Anton-managed
    ...
    Non-Son of Anton third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_son_of_anton_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Son of Anton provider "
                "credential %r (blocked by _SON_OF_ANTON_PROVIDER_ENV_BLOCKLIST). "
