"""Tests for renco_cli.managed_uv — one path, no guessing."""
...
def _runtime_info(
    ...
    from renco_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from renco_cli.managed_uv import RuntimeRepairResult
...
class TestManagedUvPath:
    ...
    def test_posix(self, tmp_path):
        with patch("renco_cli.managed_uv.get_renco_home", return_value=tmp_path):
            from renco_cli.managed_uv import managed_uv_path
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        with patch("renco_cli.managed_uv.get_renco_home", return_value=tmp_path), \
             patch("renco_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("renco_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from renco_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
        ...
        observed = []
        with patch(
            "renco_cli.managed_uv.get_renco_home",
            return_value=tmp_path,
        ), patch(
            "renco_cli.managed_uv._install_uv",
            side_effect=fake_install,
        ), patch(
            "renco_cli.managed_uv.repair_vulnerable_runtime",
            return_value=repair,
        ):
...
class TestEnsureUvUpdateBoundary:
    ...
    ``renco update`` runs the call site from the old, already-imported
    ``renco_cli.main`` against the freshly pulled ``managed_uv``. A release
    ...
    def test_failure_unpacks_without_raising(self, tmp_path):
        with patch("renco_cli.managed_uv.get_renco_home", return_value=tmp_path), \
             patch("renco_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("renco_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from renco_cli.managed_uv import ensure_uv
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from renco_cli.managed_uv import _UvResult
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\\renco\\uv.exe"), "pip"])
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("renco_cli.managed_uv.get_renco_home", return_value=tmp_path), \
             patch("renco_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")):
            from renco_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path, monkeypatch):
        ...
        from renco_cli.managed_uv import RuntimeRepairResult, update_managed_uv

        uv = tmp_path / "bin" / "uv"
        _make_executable(uv)
        # Fresh stamp under the isolated RENCO_HOME.
        import renco_constants
        stamp = renco_constants.get_renco_home() / "cache" / ".uv_self_update_stamp"
        stamp.parent.mkdir(parents=True, exist_ok=True)
        stamp.touch()

        with patch("renco_cli.managed_uv.get_renco_home", return_value=tmp_path), \
             patch("renco_cli.managed_uv.subprocess.run") as mock_run, \
             patch(
                 "renco_cli.managed_uv.repair_vulnerable_runtime",
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from renco_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from renco_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("RENCO_HOME", str(tmp_path / "profiles" / "alpha"))
        alpha = managed_python_install_dir(checkout)
        monkeypatch.setenv("RENCO_HOME", str(tmp_path / "profiles" / "beta"))
        beta = managed_python_install_dir(checkout)

        expected = checkout / ".renco-runtime" / "python"
        assert alpha == expected
        assert beta == expected

    def test_environment_is_private_and_sanitized(self, tmp_path):
        from renco_cli.managed_uv import managed_python_env
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from renco_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch(
                 "renco_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "renco_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
        ...
        assert not (root / ".renco-runtime").exists()
        mock_install.assert_not_called()

    def test_stage_candidate_sync_keeps_uv_project_config(self, tmp_path):
        from renco_cli.managed_uv import _stage_candidate_venv

        root = tmp_path / "checkout"
        root.mkdir()
        (root / "uv.lock").write_text("# lock\n", encoding="utf-8")
        generation = root / ".renco-runtime" / "python" / "gen"
        ...
        with patch("renco_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch(
                 "renco_cli.managed_uv._smoke_candidate_venv",
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        generation = root / ".renco-runtime" / "python" / "generation-test"
        ...
        with patch(
             ...
             patch(
                 "renco_cli.managed_uv._stage_candidate_venv",
                 return_value=None,
             ):
            result = repair_vulnerable_runtime("uv", project_root=root)
        ...
        reacquired = _acquire_repair_lock(root / ".renco-runtime")
    ...
    def test_successful_repair_removes_parked_backup(self, tmp_path):
        ...
        candidate_python.write_text("candidate interpreter", encoding="utf-8")
        fixed = _runtime_info(candidate_python, (3, 53, 1))
        candidate_venv = root / ".renco-runtime" / "venv-candidate"
...
class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        from renco_cli.managed_uv import _acquire_repair_lock, _release_repair_lock

        runtime_root = tmp_path / ".renco-runtime"
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from renco_cli.managed_uv import _cut_over_candidate

        root, live, sentinel = _make_runtime_install(tmp_path)
        runtime_root = root / ".renco-runtime"
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("renco_cli.managed_uv._install_uv_posix") as mock_posix:
            from renco_cli.managed_uv import _install_uv
            _install_uv(target)
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from renco_cli.managed_uv import _runtime_request
    ...
    def _run_generation(tmp_path, monkeypatch, current_version, candidate_version):
        ...
        import renco_cli.managed_uv as managed_uv
        from renco_cli.sqlite_runtime import SQLiteRuntimeInfo
...
class TestMinorLineFallForward:
    ...
    `renco update` with no path to a fixed runtime.
    """
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, tmp_path):
        ...
        with patch("renco_cli.managed_uv.get_renco_home", return_value=tmp_path), \
             patch("renco_cli.managed_uv._install_uv"), \
             patch(
                 "renco_cli.managed_uv._uv_version_string",
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch(
             ...
             patch(
                 "renco_cli.managed_uv._refresh_managed_uv_catalog",
    ...
    def test_retry_success_proceeds_to_staging(self, tmp_path):
        def second_attempt(root):
            generation = root / ".renco-runtime" / "python" / "generation-retry"
            candidate_python = generation / "bin" / "python"
            candidate_python.parent.mkdir(parents=True)
...
class TestDefaultLiveVenv:
    ...
    'not-applicable' on every renco update and stayed on journal_mode=DELETE
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from renco_cli.managed_uv import _default_live_venv
...
class TestVenvPythonUpdateBoundary:
    """``_venv_python`` must survive a renco_constants predating its symbol.

    ``renco update`` imports renco_constants from the OLD checkout, ``git
    pull`` replaces that file, and the freshly-pulled managed_uv then runs its
    lazy ``from renco_constants import venv_python_path`` against the module
    ...
    plainly contains it, which is what made the error so confusing:
        ...
        cannot import name 'venv_python_path' from 'renco_constants'
        (~/.renco/renco-agent/renco_constants.py)
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        from renco_cli.managed_uv import _venv_python
        ...
        monkeypatch.delattr(renco_constants, "venv_python_path", raising=False)
        ...
        expected = Path("/opt/renco/venv/Scripts/python.exe") \
            if sys.platform == "win32" else Path("/opt/renco/venv/bin/python")
        assert _venv_python(Path("/opt/renco/venv")) == expected

    def test_recovery_uses_the_shared_helper_not_a_second_copy(self, monkeypatch):
        """The reload must resolve through renco_constants, not open-code it.

        Hand-rolling `Scripts`/`bin` here is what #76105 deduped away and what
        `test_no_open_coded_venv_layout_remains_in_renco_cli` bans.
        """
        import renco_constants
        ...
        assert _venv_python(Path("/opt/renco/venv")) == sentinel
