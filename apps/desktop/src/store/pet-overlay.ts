 ...
 * app's bounds and stays visible while Renco is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'renco.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'renco.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.rencoDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.rencoDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.rencoDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.rencoDesktop?.petOverlay?.close()
}
