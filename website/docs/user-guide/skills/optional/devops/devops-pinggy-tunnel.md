...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/devops/pinggy-tunnel` |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Renco' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
