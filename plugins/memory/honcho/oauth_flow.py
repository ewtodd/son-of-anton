...
``son-of-anton://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Son of Anton OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "son-of-anton"
...
def begin_authorization(
    ...
    *,
    ...
    (``son-of-anton-desktop`` / ``son-of-anton-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Son of Anton.</div>"
)
...
def start_loopback_flow_background(
    *,
    config_path: Path | None = None,
    host: str | None = None,
    source: str = "son-of-anton-desktop",
    timeout: float = 300.0,
) -> dict[str, str]:
    ...
    # the request, where a context-local SON_OF_ANTON_HOME override can't reach.
