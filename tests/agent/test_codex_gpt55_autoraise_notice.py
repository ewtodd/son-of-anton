...
2. The per-profile dedupe marker (#54432) — the notice must show at most once
   ...
   Discord etc. The gate persists a marker under ``$SON_OF_ANTON_HOME``
...
def test_marker_lives_under_son_of_anton_home() -> None:
    marker = _codex_gpt55_autoraise_notice_marker()
    assert marker.parent == get_son_of_anton_home()
