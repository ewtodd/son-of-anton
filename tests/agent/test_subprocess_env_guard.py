...
``renco_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``RENCO_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "renco_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "renco_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / renco_subprocess_env /
