...
The ``son-of-anton update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m son_of_anton_cli.main gateway run`` — image name
    ``pythonw``, not ``son-of-anton.exe`` — so the ``taskkill /IM son-of-anton.exe`` guard
    ...
    # The son-of-anton.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM son-of-anton.exe' in text
