...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'renco_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'renco_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'renco_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Renco.app/Contents/MacOS/Renco', 'darwin'),
    '/Applications/Renco.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Renco.app/Contents/MacOS/Renco', 'darwin'),
    '/Users/x/Applications/Renco.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Renco\\Renco.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Renco'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\renco-desktop\\Renco.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\renco-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Renco.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_RencoXXXX/renco', 'linux', { APPIMAGE: '/home/x/Apps/Renco.AppImage' }),
    '/home/x/Apps/Renco.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/renco/linux-unpacked/renco', 'linux', {}), '/opt/renco/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/renco', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Renco.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Renco.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.renco/renco-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.renco/renco-agent',
    uninstallArgs: ['-m', 'renco_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/renco/linux-unpacked',
    rencoHome: '/home/x/.renco'
  })
  ...
  assert.match(script, /'-m' 'renco_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/renco\/linux-unpacked'/)
  assert.match(script, /export RENCO_HOME='\/home\/x\/\.renco'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.renco/renco-agent',
    agentRoot: '/home/x/.renco/renco-agent',
    uninstallArgs: ['-m', 'renco_cli.uninstall', '--mode', 'full'],
    appPath: null,
    rencoHome: '/home/x/.renco'
  })

  // System python + source on PYTHONPATH so import renco_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.renco\/renco-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'renco_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'renco_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    rencoHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'renco_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\renco',
    uninstallArgs: ['-m', 'renco_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Renco',
    rencoHome: 'C:\\Users\\x\\AppData\\Local\\renco'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import renco_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\renco;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "renco_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Renco" >nul 2>&1/)
