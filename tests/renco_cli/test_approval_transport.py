...
def _request():
    from renco_cli.approval_transport import ApprovalRequest
...
def test_transport_registry_is_manager_and_profile_isolated(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("RENCO_HOME", str(tmp_path / "profiles" / "work"))
    ...
    monkeypatch.setenv("RENCO_HOME", str(tmp_path / "profiles" / "personal"))
    assert first.get_approval_transport("phone") is None


def test_host_accepts_bound_sync_and_async_decisions():
    from renco_cli.approval_transport import invoke_approval_transport
...
def test_host_rejects_decision_completed_after_deadline(monkeypatch):
    import renco_cli.approval_transport as transport_module
...
def test_host_caps_hung_transport_workers():
    from renco_cli.approval_transport import (
...
def test_live_temp_home_fixture_plugin_routes_and_hardline_stays_core_owned(
    ...
    """Real discovery + config + guard path under an isolated RENCO_HOME."""
    import renco_cli.plugins as plugins_module
    from tools import approval
...
def present(request):
    output = Path(os.environ["RENCO_HOME"]) / "transport-invocations.jsonl"
...
"""
    ...
    token = approval.set_renco_interactive_context(True)
    approval.clear_session("local")
    approval._permanent_approved.clear()
    try:
        routed = approval.check_all_command_guards(
            "rm -rf /tmp/renco-approval-transport-fixture", "local"
        )
        manager.discover_and_load(force=True)
        reloaded = approval.check_all_command_guards(
            "rm -rf /tmp/renco-approval-transport-fixture-reloaded", "local"
        )
        gateway_token = approval.set_renco_interactive_context(False)
        monkeypatch.setenv("RENCO_GATEWAY_SESSION", "1")
        try:
            gateway_routed = approval.check_all_command_guards(
                "rm -rf /tmp/renco-approval-transport-fixture-gateway", "local"
            )
        finally:
            approval.reset_renco_interactive_context(gateway_token)
        hardline = approval.check_all_command_guards("rm -rf /", "local")
    finally:
        approval.reset_renco_interactive_context(token)
