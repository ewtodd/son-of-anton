"""Regression tests: `renco dashboard` validates RENCO_WEB_DIST before serving.

A custom RENCO_WEB_DIST without --skip-build previously skipped BOTH the
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "renco_cli.profiles.get_active_profile_name", lambda: "default"
    )
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    ...
    assert "RENCO_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("RENCO_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "renco_cli" / "web_dist"
