...
class PlatformEntry:
    ...
    # status displays (``renco setup``, ``renco status``, the dashboard
    ...
    # Env vars this platform needs (for ``renco setup`` display).
    ...
    # built-ins).  Used by ``renco gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved RENCO_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `renco`
        # invocation -- including plain `renco chat`, which never touches any
        ...
        # `renco setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``renco setup``/``gateway status``, channel
