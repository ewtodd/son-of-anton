...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Renco Agent |
| License | MIT |
...
import upstream classes into Renco itself.
...
```bash
mkdir -p ~/.renco/cache/darwinian-evolver && cd ~/.renco/cache/darwinian-evolver
...
```bash
cd ~/.renco/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.renco/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.renco/skills/research/darwinian-evolver
DE_DIR=~/.renco/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Renco core.
   Custom driver scripts under `~/.renco/skills/...` are user-side and fine.
