...
class _RecordingDB:
    """Stand-in for ``son_of_anton_state.SessionDB`` that counts ``close()`` calls."""
...
def test_lazy_recall_open_is_owned_by_the_agent(monkeypatch):
    ...
    monkeypatch.setattr("son_of_anton_state.SessionDB", _factory)
...
def build_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "set_son_of_anton_home_override", lambda _home: None)
    monkeypatch.setattr(server, "reset_son_of_anton_home_override", lambda _tok: None)
