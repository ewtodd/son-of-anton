...
Regression guard for the bug where `son-of-anton model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`son_of_anton_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in SON_OF_ANTON_OVERLAYS
        overlay = SON_OF_ANTON_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `son_of_anton_cli/main.py` (cmd_model): read
