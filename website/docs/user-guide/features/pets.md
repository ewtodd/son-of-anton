...
Renco can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<RENCO_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Renco renders the sprite at full fidelity when your
...
renco pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `renco pets list [query] [--limit N]` |
| List installed pets | `renco pets list --installed` |
| Install a pet | `renco pets install <slug> [--select] [--force]` |
...
| Preview/animate | `renco pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `renco pets off` |
| Remove an installed pet | `renco pets remove <slug>` |
| Diagnose setup | `renco pets doctor` |

`renco pets show` flags:
...
Beyond installing pre-made pets from the gallery, Renco can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `renco pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `renco tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `RENCO_PET_IMAGE_PROVIDER` env var (e.g. `RENCO_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Renco is
...
Run `renco pets doctor` — it reports:
...
- The [`renco-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-renco-agent.md)
