...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env RENCO_DASHBOARD_SESSION_TOKEN=abc123deadbeef RENCO_DESKTOP=1 renco dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /RENCO_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /RENCO_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Renco-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Renco-Session-Token: hdr_888'), /X-Renco-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Renco-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('renco', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v renco', 15000)
  assert.equal(args[args.length - 1], 'command -v renco')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `renco-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `renco-ssh-test-${process.pid}-${Date.now()}`)
