// Backend subcommand routing for the desktop-managed Renco process.
//
// The desktop app launches its own headless backend via `renco serve` — it
...
// app hasn't updated yet, or an older `renco` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Renco profile to pin via `--profile`.
 */
 ...
 * `-m renco_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `renco_cli/subcommands/dashboard.py` source registers
