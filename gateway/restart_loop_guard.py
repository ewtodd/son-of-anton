...
Defenses 1 and 2 (the ``_SON_OF_ANTON_GATEWAY`` guard on ``son-of-anton gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.son-of-anton.gateway")``,
...
State lives in ``<SON_OF_ANTON_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_son_of_anton_home() / "gateway" / "restart_loop.json"
