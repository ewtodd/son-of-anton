"""Bounded product contract for the first Renco shared-metrics slice."""
...
SCHEMA_KEY = "renco.metrics.schema_version"
SCHEMA_VERSION = "renco.metrics.event.v2"
MODEL_CALL_SCOPE = "renco.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "renco.task_run"
TOOL_CALL_SCOPE = "renco.tool_call"
CLIENT_ACTIVE_MARK = "renco.client.active"
TOOL_APPROVAL_MARK = "renco.tool_approval"
SKILL_LIFECYCLE_MARK = "renco.skill.lifecycle"
SKILL_LOAD_MARK = "renco.skill.load"
SUBSCRIBER_NAME = "renco.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "renco.client.active"
LEGACY_MODEL_CALL_METRIC = "renco.model_call.count"
MODEL_ROUTE_METRIC = "renco.model_route.count"
TASK_STARTED_METRIC = "renco.task_run.started"
TASK_FINISHED_METRIC = "renco.task_run.finished"
TOOL_CALL_METRIC = "renco.tool_call.count"
TOOL_APPROVAL_METRIC = "renco.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "renco.skill.lifecycle.count"
SKILL_LOAD_METRIC = "renco.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Renco installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(renco_version or "").strip()
    ...
    return {
        ...
        "renco_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("renco_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Renco logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("renco.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from renco_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Renco terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Renco registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "renco-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Renco tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Renco."""
