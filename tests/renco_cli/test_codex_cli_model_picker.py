...
Covers:
 - Normal path (tokens already in Renco auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Renco
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `renco auth openai-codex`. The old
...
@pytest.fixture()
def renco_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Renco auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(renco_auth_only_env):
    """openai-codex appears when tokens are already in Renco auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Renco
    auth store."""
    renco_home = tmp_path / ".renco"
    renco_home.mkdir()
