"""Tests for `renco memory status` CLI command.
...
def _run_cmd_status(capfd, mem_config=None, memory_tools=None):
    ...
    """
    from renco_cli.memory_setup import cmd_status

    config = {"memory": mem_config or {}}
    if memory_tools is None:
        memory_tools = {"memory"}

    with patch("renco_cli.config.load_config", return_value=config):
        with patch("renco_cli.memory_setup._get_available_providers", return_value=[]):
            with patch(
