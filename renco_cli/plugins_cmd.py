"""``renco plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.renco/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Renco subprocess resolution: :func:`shutil.which` first,
...
def _plugins_dir() -> Path:
    ...
    plugins = get_renco_home() / "plugins"
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.renco/.env``."""
...
def _print_python_dependencies(manifest: dict, console) -> None:
    ...
    Declaration seam ONLY — Renco never auto-installs plugin pip
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from renco_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_renco_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_renco_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_renco_home()}/.env later)[/dim]")
...
def _install_metadata_path() -> Path:
    return get_renco_home() / "plugins" / _INSTALL_METADATA_FILE
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Renco.",
                ) from None
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `renco plugins update {plugin_name}`."
            )
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from renco_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `renco plugins search <term>` to "
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (target / "plugin.json").exists() and not (
        ...
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Renco plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `renco plugins enable {installed_name}` to activate.[/dim]",
        )
    ...
    console.print("[dim]  renco gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`renco plugins install {recorded_source} --force "
        ...
        if allowed is not True:
            ...
            if scan_result.verdict == "dangerous":
                ...
                console.print(
                    ...
                    f"findings, then re-enable with `renco plugins enable {name}` "
                    f"if you trust them.",
                )
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``renco setup`` / ``renco plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``renco plugins enable`` /
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    if name in LEGACY_RELAY_PLUGIN_KEYS:
        console.print(
            ...
            f"by Renco core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."
        )
        sys.exit(1)
    ...
    # Renco core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from renco_cli.plugin_capabilities import parse_declared_capabilities
    ...
    for entry in _discover_all_plugins():
        ...
        if entry[5] == key or entry[0] == key:
            if entry[3] == "entrypoint":
                from renco_cli.plugins import discover_entrypoint_manifests
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from renco_cli.plugin_capabilities import CAPABILITY_REGISTRY
    ...
    *,
    ...
    if not (sys.stdin.isatty() and sys.stdout.isatty()):
        console.print(
            ...
            f"`renco plugins capabilities {plugin_id}` to review and "
            f"`renco plugins enable {plugin_id}` to grant interactively."
        )
        return False
    ...
    console.print(
        ...
        f"`renco plugins enable {plugin_id}` to grant later.[/dim]"
    )
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``renco plugins capabilities [<id>]`` — declared vs granted."""
    from rich.console import Console

    from renco_cli.plugin_capabilities import granted_capabilities
...
def _resolve_tool_override_grant(
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`renco plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]"
        )
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``renco_agent.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.renco/plugins``. Include package metadata here
    so ``renco plugins list`` can show and enable them.
    """
    from renco_cli.plugins import ENTRY_POINTS_GROUP
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``renco plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] renco plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] renco plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] renco plugins")
    console.print("[dim]Enable/disable:[/dim] renco plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Renco
    ...
    try:
        from renco_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        console.print("[dim]List installed plugins:[/dim] renco plugins list")
        sys.exit(1)
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``renco plugins enable web/firecrawl`` cleared
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.renco/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.renco/plugins/<name>``."""
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `renco plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``renco plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``renco_cli.main`` never covers them. After a
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    has local changes"), and the same autostash approach ``renco update``
    ...
    try:
        ...
        if dirty:
            ...
            push = _run_plugin_git(
                ...
                "-m", "renco-plugin-update-autostash",
            )
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.renco/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from renco_cli.plugin_dev import doctor_plugin
...
def cmd_search(
    ...
    *,
    ...
    from renco_cli.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        search_index,
    )
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: renco plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")


def plugins_command(args) -> None:
    """Dispatch renco plugins subcommands."""
    ...
    if action == "install":
        ...
        from renco_cli.plugin_packs import pack_command
