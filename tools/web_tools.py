...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Son of Anton config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Son of Anton' config/.env layer (``son-of-anton config set``, ``son-of-anton tools``)
...
def _web_requires_env() -> list[str]:
    ...
    set. The behavioral contract is: if the env var is set, the tool sees
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_son_of_anton_dir("cache/web", "web_cache")
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `son-of-anton plugins enable {disabled_key}` "
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `son-of-anton tools` to set one up."
                    ),
                }
        else:
            logger.info(
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `son-of-anton plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`son-of-anton plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
def _provider_is_ready(provider) -> bool:
    ...
    require a true availability probe — otherwise ``son-of-anton doctor`` paints a
...
if __name__ == "__main__":
    ...
    from son_of_anton_cli.config import get_env_value as _gev
