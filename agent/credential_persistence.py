...
``auth.json``.  They intentionally have no dependency on ``renco_cli.auth`` so
...
# Sources Renco owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Renco-owned OAuth/device-code state)
