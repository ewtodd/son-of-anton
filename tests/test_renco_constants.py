"""Tests for renco_constants module."""
...
from renco_constants import (
    ...
    find_renco_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_renco_root,
    get_renco_dir,
    get_renco_home,
    get_process_renco_home,
    heal_renco_managed_node,
...
class TestGetDefaultRencoRoot:
    """Tests for get_default_renco_root() — Docker/custom deployment awareness."""

    def test_no_renco_home_returns_native(self, tmp_path, monkeypatch):
        """When RENCO_HOME is not set, returns ~/.renco."""
        monkeypatch.delenv("RENCO_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_renco_root() == tmp_path / ".renco"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (RENCO_HOME=<root>/profiles/<name>),
        ...
        assert get_default_renco_root() == docker_root

    @pytest.mark.windows_only
    def test_no_renco_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\renco, not ~/.renco."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("RENCO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_renco_root() == local_appdata / "renco"

    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; RENCO_HOME / home changes invalidate.

        get_default_renco_root() resolves RENCO_HOME against the native
        ...
        (native home, RENCO_HOME) compared for free each call.
        """
        # RENCO_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_renco_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            renco_constants, "_default_renco_root_memo", None, raising=False
        )

        first = get_default_renco_root()
        ...
        # RENCO_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("RENCO_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_renco_root() == docker_root
        assert resolve_calls["n"] > before, (
            "RENCO_HOME change must force a fresh resolution"
        )
...
class TestGetRencoHome:
    """Tests for get_renco_home() platform-aware fallback."""

    @pytest.mark.windows_only
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When RENCO_HOME is unset on Windows, use %LOCALAPPDATA%\\renco."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("RENCO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(renco_constants, "_profile_fallback_warned", False)

        assert get_renco_home() == local_appdata / "renco"


class TestGetProcessRencoHome:
    """Tests for get_process_renco_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_renco_home() == home




class TestRencoManagedNode:
    @pytest.mark.windows_only
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_renco_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("RENCO_HOME", str(home))
        monkeypatch.setattr(renco_constants, "node_tool_runnable", lambda path: True)

        assert find_renco_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(renco_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(renco_constants, "heal_renco_managed_node", lambda: False)
        monkeypatch.setattr(
            renco_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert renco_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Renco-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(renco_constants, "heal_renco_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = renco_constants._RENCO_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = renco_constants.find_renco_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert renco_constants.find_renco_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert renco_constants.find_renco_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(renco_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(renco_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from renco_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".renco"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import renco_cli._subprocess_compat as subprocess_compat
...
class TestGetRencoDir:
    """Tests for ``get_renco_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_renco_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_renco_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert renco_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert renco_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert renco_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert renco_constants.windows_path_to_wsl("/home/alex") is None
        assert renco_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(renco_constants, "is_wsl", lambda: True)
        assert renco_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert renco_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert renco_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def _install_fake_psutil(self, monkeypatch, procs):
        ...
        monkeypatch.setattr(renco_constants.sys, "platform", "win32")
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(renco_constants.sys, "platform", "darwin")
        monkeypatch.setenv("RENCO_HOME", str(tmp_path))
        assert renco_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert renco_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("RENCO_HOME", str(home))
        monkeypatch.setenv(
            "RENCO_NODE_TARGET_MAJOR",
            str(renco_constants._RENCO_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(renco_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            renco_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            renco_constants,
        ...
        monkeypatch.setattr(
            renco_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(renco_constants._RENCO_NODE_TARGET_MAJOR)
        ...
        result = renco_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(renco_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_renco_managed_node() is False
        assert renco_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_renco_managed_node() is False
        assert renco_constants._managed_node_heal_attempted is True
