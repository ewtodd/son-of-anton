...
1. The standalone daemon path (``renco kanban daemon --force`` /
   :func:`renco_cli.kanban_db.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``renco kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_configured_max_in_progress_parsing(monkeypatch):
    import renco_cli.config as cfgmod
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import renco_cli.config as cfgmod
    import renco_cli.profiles as profmod
