...
son_of_anton_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `son-of-anton config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_son_of_anton_home = get_son_of_anton_home()`` at import time (module
    singleton), so monkeypatching SON_OF_ANTON_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SON_OF_ANTON_HOME.)
    """
    import cli

    cli._son_of_anton_home = home
