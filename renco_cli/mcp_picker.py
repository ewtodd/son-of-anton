"""MCP picker — interactive `renco mcp picker` (also the default `renco mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``renco mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `renco plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from renco_cli.cli_output import prompt_yes_no
from renco_cli.curses_ui import curses_single_select
from renco_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Renco session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from renco_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `renco mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: renco mcp install <name>    Picker: renco mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Renco is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Renco — run `renco update` "
...
def show_catalog() -> None:
    """`renco mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`renco mcp picker` (and default `renco mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`renco mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `renco mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
