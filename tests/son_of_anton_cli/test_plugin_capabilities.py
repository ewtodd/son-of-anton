...
def son_of_anton_home(tmp_path, monkeypatch):
    """Point SON_OF_ANTON_HOME at a tmp dir with an empty config.yaml."""
...
class TestDeclarationParsing:
    ...
    def test_entrypoint_companion_metadata_declares_capabilities_without_import(
        ...
        from son_of_anton_cli import plugins as plugins_mod
        from son_of_anton_cli.plugins import PluginManager
        ...
        plugin_ep = SimpleNamespace(
            ...
            group="son_of_anton_agent.plugins",
        ...
        capability_ep = SimpleNamespace(
            ...
            value="thread_namer.plugin:register",
            group="son_of_anton_agent.plugin_capabilities",
            load=load,
        )
        monkeypatch.setattr(
...
class TestConsentPersistence:
    def test_record_consent_persists_grant(self, son_of_anton_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        cfg = _read_cfg(son_of_anton_home)
    ...
    def test_record_consent_mirrors_nested_legacy_key(self, son_of_anton_home):
        record_consent(
            "capplug", ["llm.model_override"], ["llm.model_override"]
        )
        entry = _read_cfg(son_of_anton_home)["plugins"]["entries"]["capplug"]
        assert entry["llm"]["allow_model_override"] is True

    def test_granted_capabilities_roundtrip(self, son_of_anton_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        assert granted_capabilities("capplug") == frozenset({"tools.override"})

    def test_grant_is_union_with_previous(self, son_of_anton_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        record_consent(
            "capplug",
            ["llm.model_override"],
    ...
    def test_capability_granted_after_consent(self, son_of_anton_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        assert plugin_capability_granted("capplug", "tools.override") is True

    def test_declined_stays_off(self, son_of_anton_home):
...
class TestUpdateReconsent:
    def test_added_capability_is_pending(self, son_of_anton_home):
    ...
    def test_unchanged_set_needs_no_reconsent(self, son_of_anton_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        assert pending_capabilities("capplug", ["tools.override"]) == []
    ...
    def test_no_consent_record_counts_as_changed(self, son_of_anton_home):
        assert declared_set_changed("capplug", ["tools.override"]) is True

    def test_reconsent_grants_addition(self, son_of_anton_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        declared_v2 = ["tools.override", "llm.model_override"]
        record_consent(
...
class TestFailClosed:
    def test_missing_config_not_granted(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_path / "nonexistent"))
        assert plugin_capability_granted("capplug", "tools.override") is False
        assert granted_capabilities("capplug") == frozenset()

    def test_corrupt_granted_list_not_granted(self, son_of_anton_home):
    ...
    def test_corrupt_entry_types_not_granted(self, son_of_anton_home):
        (son_of_anton_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    capplug: 42\n", encoding="utf-8"
        )
        assert plugin_capability_granted("capplug", "tools.override") is False

    def test_unknown_capability_denied(self, son_of_anton_home):
        record_consent("capplug", ["tools.override"], ["tools.override"])
        assert plugin_capability_granted("capplug", "root.everything") is False

    def test_unknown_ids_in_granted_list_ignored(self, son_of_anton_home):
        (son_of_anton_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    capplug:\n"
    ...
    def test_corrupt_consent_hash_counts_as_changed(self, son_of_anton_home):
        (son_of_anton_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    capplug:\n"
...
class TestLegacyGateCompat:
    def test_legacy_allow_tool_override_still_works(self, son_of_anton_home):
        (son_of_anton_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    oldplug:\n"
    ...
    def test_legacy_nested_llm_key_still_works(self, son_of_anton_home):
        (son_of_anton_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    oldplug:\n"
    ...
    def test_legacy_false_stays_denied(self, son_of_anton_home):
        (son_of_anton_home / "config.yaml").write_text(
            "plugins:\n  entries:\n    oldplug:\n"
    ...
    def test_tool_override_gate_uses_canonical_path(self, son_of_anton_home):
        """PluginContext._tool_override_allowed honors capability grant."""
        from son_of_anton_cli.plugins import PluginContext, PluginManifest, PluginManager
    ...
    def test_tool_override_gate_denies_without_grant(self, son_of_anton_home):
        from son_of_anton_cli.plugins import PluginContext, PluginManifest, PluginManager
    ...
    def test_tool_override_gate_legacy_key(self, son_of_anton_home):
        from son_of_anton_cli.plugins import PluginContext, PluginManifest, PluginManager
    ...
    def test_bundled_plugin_trusted(self, son_of_anton_home):
        from son_of_anton_cli.plugins import PluginContext, PluginManifest, PluginManager
...
class TestHasCapability:
    def test_probe_granted(self, son_of_anton_home):
        from son_of_anton_cli.plugins import PluginContext, PluginManifest, PluginManager
...
class TestConsentFlow:
    ...
    def test_consent_yes_records_grant(self, son_of_anton_home, monkeypatch):
        from son_of_anton_cli.plugins_cmd import _run_capability_consent
    ...
    def test_consent_decline_leaves_ungranted(self, son_of_anton_home):
        from son_of_anton_cli.plugins_cmd import _run_capability_consent
    ...
    def test_non_interactive_fails_closed(self, son_of_anton_home):
        from son_of_anton_cli.plugins_cmd import _run_capability_consent
    ...
    def test_already_granted_skips_prompt(self, son_of_anton_home):
        from son_of_anton_cli.plugins_cmd import _run_capability_consent
