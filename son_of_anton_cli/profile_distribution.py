"""Profile distributions — shareable, packaged Son of Anton profiles via git.

A distribution is a Son of Anton profile published as a git repository (or
...
* ``son-of-anton profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``son-of-anton skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``son-of-anton profile``, not a parallel tree):

    son-of-anton profile install <source> [--name N] [--alias] [--force] [--yes]
    son-of-anton profile update  <name>  [--force-config] [--yes]
    son-of-anton profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "son_of_anton_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".son_of_anton_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "son-of-anton", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            son_of_anton_requires=str(data.get("son_of_anton_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.son_of_anton_requires:
            out["son_of_anton_requires"] = self.son_of_anton_requires
        if self.author:
...
def check_son_of_anton_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Son of Anton {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Son of Anton distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``son-of-anton profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Son of Anton profile distribution."
            )
...
def plan_install(
    ...
    from son_of_anton_cli import __version__ as son_of_anton_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Son of Anton distribution."
        )

    # Version check up-front so we fail fast
    check_son_of_anton_requires(manifest.son_of_anton_requires, son_of_anton_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.son-of-anton).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="son_of_anton_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `son-of-anton profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`son-of-anton profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="son_of_anton_dist_update_") as tmp:
        plan = plan_install(
