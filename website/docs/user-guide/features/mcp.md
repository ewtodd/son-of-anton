...
description: "Connect Renco Agent to external tool servers via MCP — and control exactly which MCP tools Renco loads"
---

# MCP (Model Context Protocol)

MCP lets Renco Agent connect to external tool servers so the agent can use tools that live outside Renco itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Renco to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

:::tip Coming from Claude Code?
The `mcpServers` block in your `~/.claude.json` maps to `mcp_servers` in Renco' `config.yaml` — and `renco import-agent claude-code` migrates it (along with skills and instructions) automatically. See [Import from Other Agents](../import-from-other-agents.md).
:::

## What MCP gives you

- Access to external tool ecosystems without writing a native Renco tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Renco to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.renco/config.yaml`:
...
4. Ask Renco to use the MCP-backed capability.
...
Renco will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

Renco ships a curated catalog of MCP servers that Nous staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
renco mcp                # interactive picker (default)
renco mcp catalog        # plain-text list, scriptable
renco mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from Renco
...
`optional-mcps/` in the renco-agent repo — presence in that directory means
Nous approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — Renco prompts at install time and writes the value to
  `~/.renco/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Renco points you at
  `renco auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Renco probes the MCP server to list every
...
written (if not). Re-run `renco mcp configure <name>` once the server is
reachable to refine.
...
the renco-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/ewtodd/renco/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Renco
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Renco`) for that entry instead of silently hiding it. Run `renco update`
to install the latest Renco when you see that.
...
from environment variables (which include everything in `~/.renco/.env`).
...
MCPs are never auto-updated. Re-run `renco mcp install <name>` to refresh
after a Renco update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/ewtodd/renco/tree/main/optional-mcps).
...
rejected), and Renco's bundled `github/*` skills driving the `gh` CLI are a
...
HTTP MCP servers are remote endpoints Renco connects to directly.
...
- you do not want Renco spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Renco handles discovery, client identification, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.

Renco identifies itself with a [Client ID Metadata Document](../../reference/mcp-config-reference.md#client-identification-cimd-and-dcr) on servers that support one, and falls back to Dynamic Client Registration on those that don't. Both are automatic; there is nothing to configure.

:::tip Figma remote MCP
Figma's hosted endpoint (`https://mcp.figma.com/mcp`) allowlists Dynamic Client Registration by **exact `client_name`** — bare `"Renco Agent"` 403s, while `"Claude Code"` and `"Codex"` succeed. Renco auto-sets `oauth.client_name: "Claude Code"` for `mcp.figma.com` so install/login works without a special trick:
...
Or: `renco mcp install figma`, then `renco mcp login figma`.
:::
...
On first connect, Renco prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.renco/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Renco runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal Renco prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches Renco on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where Renco expects them.
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `renco mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `renco mcp login` can list the tools and look like it worked, but every real tool call later times out. `renco mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `renco mcp login googledrive` — with the pre-registered client, Renco skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.renco/config.yaml` from inside a running Renco session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `renco mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Renco passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
- `value_from: profile` sends the active Renco profile name, resolved once at connect time — useful when multiple profiles on one machine talk to the same server and it needs to tell them apart.
...
Renco reads MCP config from `~/.renco/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Renco skips the server entirely |
...
For well-known MCP servers, `renco mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
renco mcp add codex --preset codex
```
...
You can pick any local name (`renco mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Renco registers MCP tools

Renco prefixes MCP tools so they do not collide with built-in names:
...
- Renco only registers resource utilities if the MCP session actually supports resource operations
- Renco only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Renco, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Renco skips the server completely and does not even attempt a connection.
...
You can also separately disable Renco-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Renco does not create an empty runtime MCP toolset for that server.
...
Renco discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Renco when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Renco receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Renco does not blindly pass your full shell environment.
...
Then verify your config and restart Renco.
...
Because Renco now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Renco may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Renco via the `sampling/createMessage` protocol. This allows an MCP server to ask Renco to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Renco as an MCP server

In addition to connecting **to** MCP servers, Renco can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Renco's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Renco
- You want a single MCP server that bridges to all of Renco's connected messaging platforms at once
- You already have a running Renco gateway with connected platforms
...
Add Renco to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Renco in a specific location:

```json
{
  "mcpServers": {
    "renco": {
      "command": "/home/user/.renco/renco-agent/venv/bin/renco",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Renco-specific channel browser:
...
The MCP server includes a live event bridge that polls Renco's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
renco mcp serve              # Normal mode
renco mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Renco's session store — `~/.renco/state.db` is the primary source, with `sessions.json` kept only as a legacy fallback. A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `renco send` CLI.
...
- The embedded `renco mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Renco, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
