"""Cross-process mutual exclusion for in-flight Renco updates.

Three different surfaces can start an update of the same install tree:

* ``renco update`` from a terminal,
* the dashboard's Update button (``POST /api/renco/update`` →
  ``_spawn_renco_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``renco-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``renco update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <RENCO_HOME>/.renco-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``renco update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Renco is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``renco-setup`` binary under ``~/.renco`` is only refreshed by a full
  installer run (``copy_self_to_renco_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Renco is still running") on every
  GUI update forever, with no Renco process actually running.
"""
...
MARKER_NAME = ".renco-update-in-progress"

# Set by an orchestrating updater (the Tauri `renco-setup --update` flow) to
# its own pid before spawning `renco update` as a child stage. The parent
...
HANDOFF_PID_ENV = "RENCO_UPDATE_HANDOFF_PID"
...
# to show "Renco is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Renco home (never the context-local profile override):
    the Rust updater resolves ``$RENCO_HOME`` or the platform default, and the
    ...
    return get_process_renco_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``renco update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``renco-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Renco update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `renco update` as a stage): we run under ITS claim
