 ...
 * product name (`Renco`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Renco'
 *
 * This is a hard failure with no obvious cause for the user — `renco desktop`
 ...
 * (the mac-unpacked Renco.app bundle) and Windows (win-unpacked), so we
 ...
 * Renco.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `renco desktop --build-only` (renco_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Renco.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Renco'}.exe`
