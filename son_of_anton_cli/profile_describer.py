...
Used by ``son-of-anton profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``son_of_anton_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Son of Anton Agent kanban board.
...
Rules:
  ...
  - Never write "Son of Anton Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from son_of_anton_constants import get_son_of_anton_home  # type: ignore
            profile_dir = Path(get_son_of_anton_home())
        else:
