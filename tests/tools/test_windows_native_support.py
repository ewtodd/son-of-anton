...
code paths Renco takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``renco_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect RENCO_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("renco_cli.stdio", None)
        # Fresh import now; tests import from renco_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import renco_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("renco_cli.stdio", None)

    def test_no_op_on_posix(self, monkeypatch):
        from renco_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("renco_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "renco_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, renco_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "renco_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call renco_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """renco_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from renco_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from renco_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self):
        ...
        Without it, the gateway-respawn watcher spawned by ``renco update``
        (which runs under renco-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# renco_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "renco_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the RENCO_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / renco_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "renco_cli/tools_config.py",
            "renco_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_renco_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_renco_home" in source
        assert 'cache_dir = get_renco_home() / "cache" / "terminal"' in source
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_renco_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "renco_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "renco_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        renco_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "renco_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from renco_cli._subprocess_compat import" in block
    ...
    def test_watcher_threads_hidden_console_spec_into_respawn(self):
        ...
        The spec supplies the stable cwd + env overlay (RENCO_HOME,
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "RENCO_HOME) from the restart spec."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import renco_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "renco_cli.main", "gateway", "run"]
    ...
    def test_windows_keeps_console_python_and_preserves_tail(self):
        ...
        it up — a pre-import so the lazy ``renco_cli.gateway`` import didn't
        re-run ``gateway/status``'s ``import msvcrt`` branch, and a mock of
        ``get_renco_home`` because the real one's ``Path.resolve()`` consults
        ...
        with mock.patch.object(
            gw, "_stable_gateway_working_dir", return_value="C:/renco"
        ), mock.patch(
            "renco_cli.config.get_renco_home", return_value="C:/renco"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
...
class TestGatewayRunRestartWatcherOuterPopenFallback:
    ...
    def test_outer_watcher_retries_without_breakaway_on_oserror(self, monkeypatch):
        ...
        monkeypatch.setattr(gr, "_resolve_renco_bin", lambda: ["renco"])
        ...
        assert "_RENCO_GATEWAY" not in kw1["env"]
    ...
    def test_outer_watcher_dual_failure_warns_without_leaking_secrets(
        ...
        monkeypatch.setenv("RENCO_TEST_SECRET", secret)
        ...
        assert secret in (kwargs_used.get("env") or {}).get("RENCO_TEST_SECRET", "")
