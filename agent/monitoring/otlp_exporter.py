...
configured under ``monitoring.export.otlp``. Lets an operator stream Son of Anton
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install son-of-anton[otlp]``), imported lazily so the
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"son-of-anton.event": kind or "unknown"}
    keep_by_kind = {
    ...
    for col in keep_by_kind.get(kind, ()):  # type: ignore[arg-type]
        ...
        if v is not None:
            ...
            attrs[f"son-of-anton.{col}"] = v
    return attrs


def export_batch(provider, batch: List[Dict[str, Any]]) -> int:
    """Map a batch of events to OTel spans. Returns spans created."""
    tracer = provider.get_tracer("son-of-anton.monitoring")
    n = 0
    for ev in batch:
        try:
            name = f"son-of-anton.{ev.get('event', 'event')}"
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'son-of-anton[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
