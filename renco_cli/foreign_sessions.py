...
``renco sessions import`` (and ``--resume @claude`` / ``--resume @codex``)
let a user pull a conversation they started in another agent CLI into
Renco and continue it here.
...
role-alternation invariant Renco enforces everywhere else:
...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the Renco SessionDB.

    Returns the new Renco session id.  The foreign file is only read.
...
def pick_foreign_session(
    ...
    if not sys.stdin.isatty():
        print(
            ...
            "  renco sessions import --from claude|codex <path>"
        )
        return None
    try:
...
def run_sessions_import(args, db=None) -> Optional[str]:
    """`renco sessions import` entry point. Returns new session id or None."""
    ...
    print(f"  Continue it with:  renco --resume {session_id}")
    return session_id
