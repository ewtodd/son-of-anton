 ...
 * console. It writes RENCO_HOME/.renco-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Renco within 30 minutes.
...
export function handoffResultPath(rencoHome: string): string {
  return path.join(rencoHome, '.renco-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(rencoHome)
  let raw: string
