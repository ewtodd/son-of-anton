 ...
 * the three user-facing uninstall modes to the `renco uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m renco_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `renco_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'renco_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Renco.app/Contents/MacOS/Renco  → …/Renco.app
 *   Windows: …\Renco\Renco.exe                 → …\Renco  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Renco.app/Contents/MacOS/Renco → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Renco.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Renco.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Renco$/i.test(dir) || /[\\/]renco-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/renco
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import renco_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, rencoHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import renco_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Renco install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "RENCO_HOME=${String(rencoHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
