"""Central registry for all son-of-anton tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (son_of_anton_constants itself is stdlib-only, so no cycle).
        from son_of_anton_constants import get_son_of_anton_home

        return Path(get_son_of_anton_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``son-of-anton tools``
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved SON_OF_ANTON_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("son_of_anton_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``son_of_anton_plugins.{name}``), not the exact module string.
                # A handler defined in ``son_of_anton_plugins.pkg.handlers`` is
                # still owned by the ``son_of_anton_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``son-of-anton tools enable foo``)
