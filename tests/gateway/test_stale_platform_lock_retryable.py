...
``gateway run --replace`` only kills same-SON_OF_ANTON_HOME PID-file holders.
...
def test_lock_conflict_names_owning_profile(adapter):
    ...
    existing = {
        ...
        "son_of_anton_home": "/opt/data/profiles/lead-gen-outreach",
    }
    ...
    assert adapter._fatal_error_message == (
        ...
        "(son-of-anton --profile lead-gen-outreach gateway stop)."
    )
    assert adapter._fatal_error_retryable is True
    assert adapter._fatal_error_code == "telegram-bot-token_lock"


def test_lock_conflict_infers_profile_from_legacy_son_of_anton_home(adapter):
    """Locks written before the profile field existed still attribute via son_of_anton_home."""
    existing = {
        "pid": 559,
        "start_time": 123,
        "son_of_anton_home": "/opt/data/profiles/lead-gen-outreach",
