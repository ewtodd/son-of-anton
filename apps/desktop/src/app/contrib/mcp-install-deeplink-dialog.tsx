...
import { getRencoConfigRecord, saveMcpServers } from '@/renco'
...
import { setRencoConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `renco://mcp/install` deep links. The payload is
 ...
 * confirming lets Renco spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getRencoConfigRecord())
      ...
      setRencoConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
