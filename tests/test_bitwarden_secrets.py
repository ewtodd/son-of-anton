...
binary download are exercised manually by `renco secrets bitwarden
...
def renco_home(tmp_path, monkeypatch):
    """Point Renco at an isolated home directory."""
    home = tmp_path / ".renco"
    home.mkdir()
    monkeypatch.setenv("RENCO_HOME", str(home))
    # Some modules cache get_renco_home; clear if needed.
    import renco_constants
    if hasattr(renco_constants, "_RENCO_HOME_CACHE"):
        renco_constants._RENCO_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(renco_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from renco_cli.env_loader import _apply_external_secret_sources
