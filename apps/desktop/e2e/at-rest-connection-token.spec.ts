 ...
 * dir (macOS `~/Library/Application Support/Renco/connection.json`, Windows
 * `AppData\Roaming\Renco\connection.json`). Anything that can read the file
 ...
 *      or in RENCO_HOME (logs included).
 ...
 * `X-Renco-Session-Token` header it receives — a dropped or mangled token
 ...
 * gateway token: `51c68d4ab` ("Add Renco desktop app (#20059)"), the commit
 ...
 */
const SENTINEL_TOKEN = 'renco-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'RencoE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-Renco-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote Renco gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-renco-session-token']
 ...
 * backend, so we skip spawning `renco serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startRenco() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    RENCO_DESKTOP_APP_NAME: STABLE_APP_NAME,
    RENCO_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { rencoDesktop?: Record<string, unknown> }).rencoDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture Renco should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { rencoDesktop: any }).rencoDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // RENCO_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.rencoHome, needles),
      'the gateway session token leaked into a RENCO_HOME file (logs included)',
    ).toEqual([])
