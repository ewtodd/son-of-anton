...
the shared /blueprint command handler (renco_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated RENCO_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("renco://blueprint/morning-brief?")
