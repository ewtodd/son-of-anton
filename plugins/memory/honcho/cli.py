...
Handles: son_of_anton honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "son-of-anton" else ""
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Son of Anton profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $SON_OF_ANTON_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_son_of_anton_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    cfg: dict, son_of_anton_host: dict
    ...
    pin = False
    for val in (
        son_of_anton_host.get("pinUserPeer"),
        son_of_anton_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in son_of_anton_host:
        aliases_src = son_of_anton_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in son_of_anton_host:
        prefix_src = son_of_anton_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(son_of_anton_host: dict) -> None:
    ...
    for key in _IDENTITY_MAPPING_KEYS:
        son_of_anton_host.pop(key, None)
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom son_of_anton_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    son_of_anton_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        son_of_anton_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(son_of_anton_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = son_of_anton_host.get("userPeerAliases")
    prefix = son_of_anton_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(son_of_anton_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(son_of_anton_host)
    son_of_anton_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        son_of_anton_host["userPeerAliases"] = aliases
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/son_of_anton (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Son of Anton persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = son_of_anton_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            son_of_anton_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(son_of_anton_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'son_of_anton honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'son_of_anton honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'son_of_anton honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(son_of_anton_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'son_of_anton honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = son_of_anton_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        son_of_anton_host["peerName"] = new_peer

    current_ai = son_of_anton_host.get("aiPeer") or cfg.get("aiPeer", "son-of-anton")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        son_of_anton_host["aiPeer"] = new_ai

    current_workspace = son_of_anton_host.get("workspace") or cfg.get("workspace", "son-of-anton")
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, son_of_anton_host)
    ...
    if run_mapping:
        peer_target = son_of_anton_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            _scrub_identity_mapping(son_of_anton_host)
            son_of_anton_host["pinUserPeer"] = True
            ...
            _scrub_identity_mapping(son_of_anton_host)
            son_of_anton_host["pinUserPeer"] = False
            if prior_aliases:
            ...
            _configure_raw_identity_mapping(
                son_of_anton_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = son_of_anton_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        son_of_anton_host["observationMode"] = new_obs
    else:
        son_of_anton_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(son_of_anton_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        son_of_anton_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        son_of_anton_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = son_of_anton_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        son_of_anton_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = son_of_anton_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        son_of_anton_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                son_of_anton_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(son_of_anton_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            son_of_anton_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        son_of_anton_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        son_of_anton_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        son_of_anton_host["dialecticReasoningLevel"] = new_reasoning
    else:
        son_of_anton_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = son_of_anton_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        son_of_anton_host["sessionStrategy"] = new_strat

    son_of_anton_host["enabled"] = True
    son_of_anton_host.setdefault("saveMessages", True)
    ...
    print("\n  Other commands:")
    print("    son_of_anton honcho status     -- show full config")
    print("    son_of_anton honcho mode       -- change recall/observation mode")
    print("    son_of_anton honcho tokens     -- tune context and dialectic budgets")
    print("    son_of_anton honcho peer       -- update peer names")
    print("    son_of_anton honcho map <name> -- map this directory to a session name\n")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if son_of_anton_cli internals move."""
    try:
        from son_of_anton_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(son_of_anton_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    son_of_anton_host["apiKey"] = cred.access_token
    son_of_anton_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        son_of_anton_host["peerName"] = cred.consent_peer_name
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: son_of_anton honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'son_of_anton honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'son_of_anton honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: son_of_anton honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        son_of_anton = hosts.get(_host_key(), {})
        user = son_of_anton.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = son_of_anton.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = son_of_anton.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = son_of_anton.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Son of Anton' identity in Honcho. Seed with 'son_of_anton honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: son_of_anton honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: son_of_anton honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = son_of_anton.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = son_of_anton.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = son_of_anton.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Son of Anton asks Honcho's AI peer a question")
        ...
        print("\n  Set with: son_of_anton honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'son_of_anton honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'son_of_anton honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    son_of_anton honcho identity --show        — show both peer representations")
        print("    son_of_anton honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Son of Anton + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Son of Anton\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Son of Anton persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  son_of_anton honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'son_of_anton honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'son_of_anton honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  son_of_anton honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'son-of-anton'")
        print("  with Honcho configured and no prior session history.")
        print("  (Son of Anton calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    son_of_anton honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'son_of_anton honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Son of Anton, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'son_of_anton honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    son_of_anton honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  son_of_anton honcho identity <path/to/SOUL.md>")
    ...
    print("    Son of Anton:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Son of Anton:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Son of Anton:   Honcho observes every message and updates representations")
    ...
    print("    Son of Anton:   per-session by default — each run gets its own session")
    print("              Map a custom name:  son_of_anton honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. son_of_anton honcho setup              — configure API key (required)")
        print("  2. son_of_anton honcho migrate            — re-run this walkthrough")
    else:
        print("  1. son_of_anton honcho status             — verify Honcho connection")
        print("  2. son_of_anton                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. son_of_anton honcho identity --show    — verify AI peer representation")
        print("  4. son_of_anton honcho tokens             — tune context and dialectic budgets")
        print("  5. son_of_anton honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'son_of_anton memory setup'...\n")
        from son_of_anton_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``son_of_anton honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``son_of_anton honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to son_of_anton memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Son of Anton Honcho",
    )
