...
``renco mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Renco desktop app. Use the "
            "terminal instead: `renco mcp install <name>` for catalog entries, "
            "`renco mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Renco desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `renco mcp catalog` "
