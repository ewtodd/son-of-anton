...
A package-manager install (NixOS, and anything else shipping Renco from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.renco/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("renco_cli.config.get_managed_system", lambda: "nixos")
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets RENCO_MANAGED *and* a writable target.

    Dockerfile sets RENCO_LAZY_INSTALL_TARGET and the NixOS container module
    passes RENCO_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("renco_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("renco_cli.config.get_managed_system", _raise)
