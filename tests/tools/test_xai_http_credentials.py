...
def _set_xai_oauth_unavailable(monkeypatch):
    from son_of_anton_cli import auth
...
def test_xai_credentials_fail_closed_without_profile_scope(tmp_path, monkeypatch):
    ...
    from son_of_anton_cli.config import invalidate_env_cache
...
def test_prefer_api_key_honors_son_of_anton_xai_base_url_with_validation(monkeypatch):
    """The preferred-key path reads the same override pair as the OAuth
    branch (SON_OF_ANTON_XAI_BASE_URL first, then XAI_BASE_URL) behind the same
    ...
    monkeypatch.setattr(
        ...
        lambda name, default=None: {
            ...
            "SON_OF_ANTON_XAI_BASE_URL": "https://staging.x.ai/v1",
