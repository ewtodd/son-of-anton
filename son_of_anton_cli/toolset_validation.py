...
the tool registry or launching Son of Anton (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``son-of-anton-cli`` to the non-existent ``son-of-anton``. ``resolve_toolset('son-of-anton')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``son-of-anton-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``son-of-anton`` instead of ``son-of-anton-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"son-of-anton-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `son-of-anton tools` to reconfigure."
        )
    return warnings
