...
# Connecting Desktop to Many Renco Instances

Register every Renco backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Renco Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Renco gateway…"**) deep-links straight to
...
list of Renco gateways. Its intro says it plainly: *"Manage this device and
every Renco gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Renco runtime managed by this app." | automatic |
| **Remote gateway** | "A Renco gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Renco install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Renco Cloud** | "A hosted instance discovered through your Renco Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Renco Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Renco Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **Renco Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Renco Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `renco serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Renco agents
...
connection is registered) dispatches `renco update` to every eligible
...
- **Renco Cloud** instances are skipped with a *"Managed by Renco Cloud"*
...
panel, ⌘K **Update Renco**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `renco serve` is running on the remote host, the
...
- **"Update Renco Desktop to chat with agents on other connections"** — the
