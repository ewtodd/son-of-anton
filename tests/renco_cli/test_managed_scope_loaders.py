...
renco_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # renco_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("RENCO_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import renco_time

    assert renco_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (RENCO_TIMEZONE, RENCO_REDACT_SECRETS, RENCO_MAX_ITERATIONS,
    ...
    # writing RENCO_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
