"""``son-of-anton login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``son-of-anton login`` was removed in favor of ``son-of-anton auth`` / ``son-of-anton model``
    (the runtime handler in ``son_of_anton_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``son-of-anton login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``son-of-anton --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``son-of-anton login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``son-of-anton model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `son-of-anton auth` to manage credentials, "
            "`son-of-anton model` to select a provider, or `son-of-anton setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `son-of-anton model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: son-of-anton-cli)"
    )
