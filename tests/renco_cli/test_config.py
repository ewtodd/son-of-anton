"""Tests for renco_cli configuration management."""
...
class TestGetRencoHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("RENCO_HOME", None)
            home = get_renco_home()
            assert home == Path.home() / ".renco"


class TestEnsureRencoHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from renco_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``renco logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from renco_cli import config as cfg_mod
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from renco_cli.config import _deep_merge
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"RENCO_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"RENCO_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"RENCO_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"RENCO_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("renco_cli.config.sanitize_env_file", return_value=2),
            patch(
                "renco_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("renco_cli.config.read_raw_config", return_value={}),
            patch("renco_cli.config.get_missing_env_vars", return_value=[]),
            patch("renco_cli.config.get_missing_config_fields", return_value=[]),
            patch("renco_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from renco_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """RENCO_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `renco setup`) lets a
        ...
        via config.yaml; RENCO_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from renco_cli.config import OPTIONAL_ENV_VARS
        assert "RENCO_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `renco setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `renco setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_renco_home(self):
        from renco_cli.config_migrations import support_floor_message
        from renco_constants import display_renco_home

        msg = support_floor_message()
        assert f"{display_renco_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from renco_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``RENCO_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Renco
    process on next startup via the dotenv → ``os.environ`` chain in
    ``renco_cli/env_loader.py``.
    ...
    def _renco_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "RENCO_SPOTIFY_CLIENT_ID",
            "RENCO_QWEN_BASE_URL",
            "RENCO_MAX_ITERATIONS",
        ],
    )
    def test_renco_integration_keys_still_writable(self, allowed_key):
        """``RENCO_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``RENCO_*`` convention must keep
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from renco_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"RENCO_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: renco\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "renco"
