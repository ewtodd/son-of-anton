...
``ui_meta['son-of-anton-bots']`` block in its profile.yaml), a bot's canonical
...
def _son_of_anton_root(home: Path) -> Path:
    """Root ~/.son-of-anton for both the default profile and named profiles."""
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @son-of-anton.
    return "son-of-anton" if name == "default" else name
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "```\n"
        "son-of-anton peer dm <peer>/<agent-name> < /tmp/dm.txt\n"
        "```\n"
        "Use `<peer>` alone for the peer's main agent. Run `son-of-anton peer list` "
        "for the live peer list."
    )


def _build_section(home: Path) -> str:
    root = _son_of_anton_root(home)
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Son of Anton profile is an agent teammate with "
        ...
        "```\n"
        f'son-of-anton -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file /tmp/dm.txt\n'
        "```\n"
        ...
        "which agent it came from. Run `son-of-anton profile list` for the LIVE teammate "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    not the ambient SON_OF_ANTON_HOME — build threads can lose the ContextVar
    override and the env var would then name the wrong profile.
    """
    resolved = str(home) if home else (os.getenv("SON_OF_ANTON_HOME") or os.path.expanduser("~/.son-of-anton"))
    with _lock:
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    resolved = Path(str(home) if home else (os.getenv("SON_OF_ANTON_HOME") or os.path.expanduser("~/.son-of-anton")))
    surface: dict = {}
    try:
        ...
        token = set_son_of_anton_home_override(str(resolved))
        try:
    ...
    try:
        ...
        surface["skills"] = []
    try:
        root = _son_of_anton_root(resolved)
    ...
    try:
        ...
        surface["peers"] = _peers(_son_of_anton_root(resolved))
    except Exception:
        surface["peers"] = []
    try:
