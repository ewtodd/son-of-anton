...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.rencoHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.rencoHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.rencoHome)
  const builder = await RealSessionBuilder.start(sandbox.rencoHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.rencoHome, mock.url)
  fs.appendFileSync(path.join(sandbox.rencoHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.rencoHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
