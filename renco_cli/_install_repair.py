...
- ``renco_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``renco_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``renco_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``renco_cli.main`` imports
``managed_uv``, ``renco_constants``, and friends only in its late path; the
...
:func:`renco_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from renco_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (renco_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``renco acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # renco_constants is stdlib-only, so the canonical layout helpers are safe
...
def _quarantine_running_renco_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Rename live renco*.exe shims aside so the installer can rewrite them.
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    moved = _quarantine_running_renco_exe(scripts_dir) if scripts_dir else []
    try:
        ...
        # the quarantined shims renamed aside and `renco` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``renco*.exe`` shims on Windows so they can be replaced
