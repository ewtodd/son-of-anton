"""Tests for son_of_anton_cli/completion.py — shell completion script generation."""
...
from son_of_anton_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real son-of-anton structure."""
    p = argparse.ArgumentParser(prog="son-of-anton")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_son_of_anton_completion()" in out
        assert "complete -F _son_of_anton_completion son-of-anton" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_son_of_anton_profiles'" in out
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'son-of-anton profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _son_of_anton_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_son_of_anton_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _son_of_anton_profiles"
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__son_of_anton_profiles" in out
        count = out.count("(__son_of_anton_profiles)")
