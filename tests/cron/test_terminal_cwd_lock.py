...
def test_run_job_releases_cwd_lock_when_body_raises(tmp_path):
    ...
    with patch("cron.scheduler._son_of_anton_home", tmp_path), \
         ...
         patch("son_of_anton_state.SessionDB", return_value=MagicMock()):
...
def test_cwd_lock_timeout_derivation(monkeypatch):
    """The bound tracks SON_OF_ANTON_CRON_TIMEOUT (+margin) with a floor, and
    ...
    monkeypatch.setenv("SON_OF_ANTON_CRON_TIMEOUT", "1800")
    assert sched._cwd_lock_timeout_seconds() == 1860.0
    monkeypatch.setenv("SON_OF_ANTON_CRON_TIMEOUT", "30")
    assert sched._cwd_lock_timeout_seconds() == 180.0
    monkeypatch.setenv("SON_OF_ANTON_CRON_TIMEOUT", "0")
    assert sched._cwd_lock_timeout_seconds() == 660.0
    monkeypatch.setenv("SON_OF_ANTON_CRON_TIMEOUT", "bogus")
    assert sched._cwd_lock_timeout_seconds() == 660.0
