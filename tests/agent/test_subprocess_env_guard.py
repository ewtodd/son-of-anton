...
``son_of_anton_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``SON_OF_ANTON_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "son_of_anton_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "son_of_anton_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / son_of_anton_subprocess_env /
