"""
Security advisory checker for Son of Anton Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``son-of-anton doctor --ack <id>``; the ack is persisted to
...
1. ``son-of-anton doctor`` (and ``son-of-anton doctor --ack <id>``)
...
run in environments where the rest of Son of Anton failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including son-of-anton when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.son-of-anton/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: son-of-anton doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Son of Anton requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'son-of-anton doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.son-of-anton/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_son_of_anton_home()) / "cache"
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `son-of-anton doctor` on the gateway host for details.")
