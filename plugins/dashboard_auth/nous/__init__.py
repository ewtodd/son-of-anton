...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      RENCO_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      RENCO_DASHBOARD_PORTAL_URL       — defaults to
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``renco-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``renco_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set RENCO_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via RENCO_DASHBOARD_PORTAL_URL for
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"renco_session_pkce\"]``
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between RENCO_DASHBOARD_PORTAL_URL / RENCO_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``RENCO_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("RENCO_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``RENCO_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("RENCO_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``RENCO_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set RENCO_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``renco_cli.web_server.should_require_auth`` +
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"RENCO_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
