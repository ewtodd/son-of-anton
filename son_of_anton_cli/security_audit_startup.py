...
logger = logging.getLogger("son-of-anton.security_audit")
...
def run_security_audit(
    *, son_of_anton_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(son_of_anton_home=son_of_anton_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
