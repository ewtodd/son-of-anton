"""CLI handlers for the ``son-of-anton proxy`` subcommand."""
...
from son_of_anton_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "son-of-anton proxy requires aiohttp. Run `son-of-anton setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"son-of-anton auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Son of Anton proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Son of Anton proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: son-of-anton proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``son-of-anton proxy <subcommand>``."""
    ...
    print(
        "son-of-anton proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  son-of-anton proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  son-of-anton proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  son-of-anton proxy providers\n"
