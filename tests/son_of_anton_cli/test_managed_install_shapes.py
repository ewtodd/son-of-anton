...
def test_env_var_names_the_managing_system(son_of_anton_home, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("SON_OF_ANTON_MANAGED", raising=False)
    else:
        monkeypatch.setenv("SON_OF_ANTON_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    son_of_anton_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the SON_OF_ANTON_MANAGED of the service."""
    (son_of_anton_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("SON_OF_ANTON_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(son_of_anton_home, monkeypatch):
    (son_of_anton_home / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("SON_OF_ANTON_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    son_of_anton_home, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("SON_OF_ANTON_MANAGED", managed_value)
    ...
    # `son-of-anton update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "son-of-anton update"
...
def test_unmanaged_install_offers_no_update_command(son_of_anton_home, monkeypatch):
    monkeypatch.delenv("SON_OF_ANTON_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(son_of_anton_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = son_of_anton_home / ".managed"
