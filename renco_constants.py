"""Shared constants for Renco Agent.
...
_UNSET = object()
_RENCO_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_RENCO_HOME_OVERRIDE", default=_UNSET
)
...
def set_renco_home_override(path: str | Path | None) -> Token:
    """Set a context-local Renco home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _RENCO_HOME_OVERRIDE.set(value)


def reset_renco_home_override(token: Token) -> None:
    """Restore the previous context-local Renco home override."""
    _RENCO_HOME_OVERRIDE.reset(token)


def get_renco_home_override() -> str | None:
    """Return the active context-local Renco home override, if any."""
    override = _RENCO_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_renco_home() -> Path:
    """Return the platform-native default Renco home path."""
...
def _renco_home_from_env() -> Path:
    """Resolve RENCO_HOME from the process environment only.

    Reads the ``RENCO_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_renco_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_renco_home`
    and :func:`get_process_renco_home` so the two never drift.
    """
    val = os.environ.get("RENCO_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_renco_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``RENCO_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_renco_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[RENCO_HOME fallback] RENCO_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass RENCO_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_renco_home() -> Path:
    """Return the Renco home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_renco_home_override`) → ``RENCO_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``RENCO_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``RENCO_HOME`` explicitly (see the systemd
    template in ``renco_cli/gateway.py`` and the kanban dispatcher in
    ``renco_cli/kanban_db.py``).  See https://github.com/ewtodd/renco/issues/18594.
    """
    override = get_renco_home_override()
    if override:
        return Path(override)

    if not os.environ.get("RENCO_HOME", "").strip():
        _warn_profile_fallback_once()

    return _renco_home_from_env()


def renco_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Renco home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_renco_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_renco_home() -> Path:
    """Return the Renco home for the running process, ignoring task overrides.

    Unlike :func:`get_renco_home`, this never follows the context-local
    override set by :func:`set_renco_home_override`.  It resolves only the
    process ``RENCO_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_renco_root(). The function resolves
# RENCO_HOME against the native home on every call (~80us of path
...
# Its result depends only on (RENCO_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates RENCO_HOME mid-process.
_default_renco_root_memo: "tuple[str, str, Path] | None" = None


def get_default_renco_root() -> Path:
    """Return the root Renco directory for profile-level operations.

    In standard deployments this is the platform-native Renco home
    (``~/.renco`` on POSIX, ``%LOCALAPPDATA%\\renco`` on native Windows).

    In Docker or custom deployments where ``RENCO_HOME`` points outside
    ``~/.renco`` (e.g. ``/opt/data``), returns ``RENCO_HOME`` directly
    — that IS the root.

    In profile mode where ``RENCO_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.renco/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_renco_root_memo
    native_home = _get_platform_default_renco_home()
    env_home = os.environ.get("RENCO_HOME", "")
    if _default_renco_root_memo is not None:
        memo_native, memo_env, memo_result = _default_renco_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # RENCO_HOME is under ~/.renco (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — RENCO_HOME itself is the root
                result = env_path
    _default_renco_root_memo = (str(native_home), env_home, result)
    return result
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``RENCO_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<RENCO_HOME>/skills`` last-resort
    """
    override = os.getenv("RENCO_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_renco_home() / "skills"
...
def get_renco_dir(
    ...
    *,
    ...
    """Resolve a Renco subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to RENCO_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to RENCO_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Renco home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local RENCO_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_renco_home()
...
def iter_renco_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Renco-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\renco\\node``. POSIX installs use
    ``$RENCO_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_renco_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with rencoManagedNodePathEntries() in
...
_RENCO_NODE_TARGET_MAJOR = int(os.environ.get("RENCO_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Renco-managed Node trees live under ``$RENCO_HOME/node`` (or a profile's
    ``RENCO_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_renco_node_executable``
    used to trust file presence alone, so ``renco update`` would pick that
...
def renco_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Renco-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_renco_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Renco-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `renco update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%RENCO_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the renco.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_RENCO_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_RENCO_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$RENCO_HOME/node`` on POSIX.
    ...
    ``RENCO_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_renco_managed_node() -> str | None:
    """Install a Renco-managed Node tree and return its npm path.
    ...
    Renco never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$RENCO_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_renco_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_renco_node_dirs():
...
def heal_renco_managed_node() -> bool:
    """Redownload Renco-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not renco_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "RENCO_HOME": str(get_renco_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_renco_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_RENCO_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_renco_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _RENCO_NODE_TARGET_MAJOR
    return False


def find_renco_node_executable(command: str) -> str | None:
    """Return a Renco-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_RENCO_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_renco_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Renco-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Renco-managed installs.

    This is for Renco-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_renco_node_executable(command)
    if managed:
        return managed
    if renco_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_renco_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Renco-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_renco_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``renco update`` — leaving a **dangling symlink** that ``which`` still
...
def display_renco_home() -> str:
    """Return a user-friendly display string for the current RENCO_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.renco``
        profile:  ``~/.renco/profiles/coder``
        custom:   ``/opt/renco-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.renco``.  For code that needs a real ``Path``, use
    :func:`get_renco_home` instead.
    """
    home = get_renco_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``RENCO_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/ewtodd/renco/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{RENCO_HOME}/home`` when the profile-home directory exists."""
    renco_home = get_renco_home_override() or (env or {}).get("RENCO_HOME") or os.getenv("RENCO_HOME")
    if not renco_home:
        return None
    profile_home = os.path.join(renco_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("RENCO_REAL_HOME") or os.getenv("RENCO_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Renco profile HOME.

    ``RENCO_HOME`` scopes Renco state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={RENCO_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{RENCO_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{RENCO_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Renco' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["RENCO_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Renco itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under RENCO_HOME.

    Replaces the ``get_renco_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, renco_logging.py, renco_time.py, etc.).
    """
    return get_renco_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under RENCO_HOME."""
    return get_renco_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under RENCO_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_renco_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._renco_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``renco_cli`` modules using three different Windows
    ...
    A few sites outside ``renco_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``renco_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Renco itself. An ImportError
...
# `renco_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Renco.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("renco_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `renco update` is exactly
    ...
    return [
        ...
        "    renco update",
        "If that also fails, reinstall: https://renco-agent.nousresearch.com",
    ]
