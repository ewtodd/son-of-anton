import type { SkinBranding, SkinColors } from '@renco/shared/skin'
...
// Mirrors @renco/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @renco/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Renco navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Renco look was never hand-authored: for years the
 ...
 *  answer when known (cached in RENCO_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.RENCO_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit RENCO_TUI_THEME / COLORFGBG signals above still win,
...
// `RENCO_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `RENCO_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `RENCO_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `RENCO_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Renco palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.RENCO_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.RENCO_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.RENCO_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into RENCO_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into RENCO_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Renco navy
  ...
  // the derived value already carries the identity seeds (e.g. Renco navy).
  const activeRow =
