...
def _codex_request_kwargs():
    return {
        "model": "gpt-5-codex",
        "instructions": "You are Son of Anton.",
...
def test_codex_preflight_defangs_harmony_tokens_before_and_after_middleware(monkeypatch):
    ...
    monkeypatch.setattr(
        "son_of_anton_cli.middleware.apply_llm_request_middleware",
        _request_middleware,
    )
    monkeypatch.setattr(
        "son_of_anton_cli.middleware.run_llm_execution_middleware",
...
def test_try_refresh_codex_client_credentials_handles_xai_oauth(monkeypatch):
    ...
    monkeypatch.setattr(
        "son_of_anton_cli.auth.resolve_xai_oauth_runtime_credentials",
        _fake_resolve,
    )
...
def test_try_refresh_copilot_client_credentials_rebuilds_client(monkeypatch):
    ...
    monkeypatch.setattr(
        "son_of_anton_cli.copilot_auth.evict_cached_exchanged_token",
        lambda _raw: None,
    )
    monkeypatch.setattr(
...
def test_try_refresh_copilot_client_credentials_falls_back_when_exchange_unavailable(monkeypatch):
    ...
    monkeypatch.setattr("son_of_anton_cli.copilot_auth.get_copilot_api_token", _boom)
    monkeypatch.setattr(run_agent, "OpenAI", _fake_openai)
...
def test_mid_turn_compaction_does_not_double_persist_in_place_rows(monkeypatch, tmp_path):
    ...
    def _fake_compress_context(messages, system_message, *, approx_tokens=None, task_id="default", focus_topic=None):
        ...
        return compacted, "You are Son of Anton."
