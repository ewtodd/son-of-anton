// Son of Anton Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Son of Anton): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Son of Anton -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Son of Anton-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `son-of-anton photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Son of Anton can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "son-of-anton-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Son of Anton'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Son of Anton allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Son of Anton, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-son-of-anton-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Son of Anton supplied them so a known-good
