"""``son-of-anton gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.son-of-anton/auth.json``) — the same path ``son-of-anton dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.son-of-anton/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _default_gateway_id() -> str:
    ...
    try:
        ...
        host = ""
    return f"gw-{host or 'son-of-anton'}"
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `son-of-anton auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from son_of_anton_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `son-of-anton gateway enroll` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `son-of-anton setup` (or `son-of-anton auth add nous`) first, then retry.")
        else:
    ...
    #    save_env_value writes them to ~/.son-of-anton/.env (0600 dir) and never logs.
    to_write = {
    ...
    from son_of_anton_cli.config import get_env_path
    ...
    # process is launched under (load_son_of_anton_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in son_of_anton_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_son_of_anton_root()).resolve()
        home = Path(get_son_of_anton_home()).resolve()
        try:
