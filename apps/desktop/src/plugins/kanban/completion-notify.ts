 ...
 *    desktop shell fires only while the user is AWAY from Renco. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@renco/plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Renco (the toast above covers the foreground case). Isolated:
