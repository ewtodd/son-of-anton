...
A package-manager install (NixOS, and anything else shipping Son of Anton from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.son-of-anton/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("son_of_anton_cli.config.get_managed_system", lambda: "nixos")
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets SON_OF_ANTON_MANAGED *and* a writable target.

    Dockerfile sets SON_OF_ANTON_LAZY_INSTALL_TARGET and the NixOS container module
    passes SON_OF_ANTON_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("son_of_anton_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("son_of_anton_cli.config.get_managed_system", _raise)
