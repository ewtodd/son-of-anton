...
metadata:
  son-of-anton:
    ...
    related_skills: [claude-code, son-of-anton]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Son of Anton terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Son of Anton itself, `model.provider: openai-codex` uses Son of Anton-managed Codex
OAuth from `~/.son-of-anton/auth.json` after `son-of-anton auth add openai-codex`. For the
...
## Son of Anton Gateway Caveat

When invoking the Codex CLI from a Son of Anton gateway/service context (for example,
