...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<son_of_anton_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active SON_OF_ANTON_HOME."""
    from son_of_anton_constants import get_son_of_anton_home

    flush_dir = get_son_of_anton_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<son_of_anton_home>/pending_messages/``), so a runtime cap rotation no
