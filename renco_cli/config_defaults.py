"""Default configuration data for Renco Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from renco_cli/config.py. Must not import from renco_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["renco-cli"],
    # SQLite journal mode used by every Renco database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Renco server processes.
    ...
    "session": {
        # Per-terminal `renco -c`: each CLI session drops a breadcrumb file
        # under $RENCO_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by renco_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `renco chat --run-budget N`.
        ...
        # an interactive `renco gateway restart` must never block for hours
        ...
        # the Renco-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Renco
        ...
        # identity slot (SOUL.md). Empty by default. The RENCO_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Renco adds a coding operating brief
        ...
        # `renco status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``RENCO_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             RENCO_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force RENCO_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Renco auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Renco sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Renco
        ...
        # bundled Renco image, which drops to the `renco` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `renco tools` picker's
    ...
    # Opt in via ``renco chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.renco/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: renco sweeps the checkpoint base at startup
        ...
        # ``renco checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .renco.md, or .cursorrules before Renco applies
    ...
    # Single-query (``renco -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Renco truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Renco' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); renco =
                                      # Renco' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Renco' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Renco does.
    ...
    "auxiliary": {
        ...
        # model; override via `renco model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `renco` (and `renco chat`) launches by default:
        ...
        # REPL and `--tui` (or RENCO_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `renco --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `renco -c` muscle memory.  Default off so existing
        # users aren't surprised.  RENCO_TUI_RESUME=<id> always wins.
        ...
        # model — see renco_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to RENCO_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/renco`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `renco pets`. Disabled until
        ...
        "pet": {
            ...
            # get_renco_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.renco/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.renco/cache/delegation/
    ...
    # response. If not, Renco feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Renco auto-pauses the goal and
    ...
    "skills": {
        ...
        # checkout, ``<root>/.renco/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``renco skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``renco skills trust`` / ``renco skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${RENCO_SKILL_DIR} and ${RENCO_SESSION_ID} in SKILL.md
        ...
        # (`renco skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.renco/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.renco/.curator_backups/blobs/. Enables `renco curator ledger`
        # and single-mutation `renco curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `renco curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `renco curator run --consolidate` overrides
        ...
        # normally restored on every `renco update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `renco curator purge` deletes
        ...
        # skipped), snapshot ~/.renco/skills/ into
        # ~/.renco/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `renco curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for renco-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Renco Agent*" header.
    ...
    # dangerous command. -q runs export RENCO_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; RENCO_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Renco' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.renco/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or RENCO_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .renco config) always require human
        ...
        # longer trigger the startup banner. Add via `renco doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``renco_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Renco to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $RENCO_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via RENCO_CRON_MAX_PARALLEL env var.
        ...
        # RENCO_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # RENCO_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via RENCO_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    "code_execution": {
        ...
        #     renco-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Renco tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.renco/logs/.
    ...
    # update model picker lists without shipping a renco-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://renco-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `renco model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Renco provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal RENCO_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.renco/sessions/sessions.json entirely.
        ...
        # the RENCO_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.renco.gateway` that defenses 1-2 don't
        ...
        # `renco update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``RENCO_GATEWAY_MAX_STARTS`` / ``RENCO_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.renco/.env,
        ...
        # behavior: files must live under the Renco cache, under
        ...
        # user. Bridged to RENCO_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Renco
        # cache (~/.renco/cache/{documents,images,audio,video,screenshots})
        ...
        # to RENCO_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # RENCO_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to RENCO_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.renco/state.db.
    ...
    "sessions": {
        ...
        # the default of ``renco sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.renco/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `renco sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `renco update` prints a one-line notice with
        ...
        # ~/.renco/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to RENCO_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # RENCO_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `renco sessions export`. Checked
    ...
    # ``renco doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `renco doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``renco update`` behaviour.
    "updates": {
        ...
        #     DBs) into <RENCO_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``renco backup``-style zip
        #     of RENCO_HOME into <RENCO_HOME>/backups/, restorable with
        #     ``renco import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `renco update` does with uncommitted local changes to the
        ...
        # When `renco update` finds the source checkout parked on a feature
    ...
    "lsp": {
        ...
        #              ``<RENCO_HOME>/lsp/bin/`` on first use.
    ...
    # rather than storing them in ~/.renco/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.renco/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.renco/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Renco may use this encrypted
            ...
            # ~/.renco/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.renco/.env (or the shell)
    ...
    #
    # Configure with `renco egress setup`.  Disabled by default — the rest of
    # Renco works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.renco/bin/ on
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Renco mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'renco model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Renco auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Renco, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @renco:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. RENCO_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #renco)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: renco-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'renco-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Renco API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Renco API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Renco API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # RENCO_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
