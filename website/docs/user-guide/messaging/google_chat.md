...
sidebar_position: 12
title: "Google Chat"
description: "Set up Renco Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Renco Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Renco
...
> Run `renco gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Renco can read (e.g.,
`~/.renco/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a renco restart)
...
- Principal: `renco-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Renco calls
...
- **App name**: whatever you want users to see ("Renco" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/renco-chat-events`.
...
`ADDED_TO_SPACE` event that Renco uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Renco

Add the Google Chat section to `~/.renco/.env`:
...
Send "hola" in the test DM. The bot posts a "Renco is thinking…" marker, then
...
`~/.renco/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Renco is thinking…").
...
Thread support: when a user replies inside a thread, Renco detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Renco session.
...
3. Download the JSON. Move it onto the host that runs Renco.
4. Register the client with Renco (run under the profile you want it scoped to):
...
renco -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Renco home (e.g.
`~/.renco/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.renco/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.renco/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Renco isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `renco gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Renco will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.renco/google_chat_user_tokens/<sanitized_email>.json` (filesystem
