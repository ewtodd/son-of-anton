...
``<SON_OF_ANTON_HOME>/state/gateway.lifecycle.json``:
...
def _process_son_of_anton_home() -> Path:
    """SON_OF_ANTON_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<SON_OF_ANTON_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``son_of_anton_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
