...
import { pathFromRencoDeepLink, resolveRencoOpenPath } from '@/lib/renco-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.rencoDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.rencoDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.rencoDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.rencoDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // renco://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.rencoDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future rencoDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveRencoOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.rencoDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (renco://index-network/intent/1, renco://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromRencoDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.rencoDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.rencoDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.rencoDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.rencoDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
