...
Extracted verbatim from renco_state.py so the SessionDB mixin modules
(renco_state_search / renco_state_schema / renco_state_portability) can
reference them without importing renco_state (which would be a cycle).
renco_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``renco sessions optimize-storage``. A legacy DB sits at
...
# `renco sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `renco db optimize`. This is
