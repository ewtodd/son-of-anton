...
guard is covered in ``tests/son_of_anton_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``son_of_anton_cli.model_switch.prewarm_picker_cache_async``
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from son_of_anton_cli.model_switch,
