"""Tests for SON_OF_ANTON_HOME credential-file read blocking in file_safety.

Regression for https://github.com/ewtodd/son-of-anton/issues/17656 —
``read_file`` was previously only sandboxed against ``SON_OF_ANTON_HOME`` itself,
...
for the credential stores while leaving arbitrary ``SON_OF_ANTON_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_son_of_anton_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "son_of_anton_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_son_of_anton_home_path", lambda: home)
    return home
...
def test_arbitrary_son_of_anton_home_file_not_blocked(fake_home):
    """Non-credential files inside SON_OF_ANTON_HOME stay readable."""
...
def test_identically_named_son_of_anton_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Son of Anton-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside SON_OF_ANTON_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside SON_OF_ANTON_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside SON_OF_ANTON_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, SON_OF_ANTON_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_son_of_anton_home_path", lambda: profile)
