"""Implementation of the ``renco bundles`` CLI subcommand.

Mirrors the structure of ``renco_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `renco bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  renco bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``renco bundles`` argparse tree.

    Called from ``renco_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `renco bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``renco bundles <subcommand>`` to the right handler."""
