...
from renco_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS, normalize_provider
...
class TestFireworksAliases:
    ...
    def test_providers_normalize_provider(self, alias):
        from renco_cli.providers import normalize_provider as normalize_in_providers
...
class TestFireworksOverlay:
    def test_overlay_exists(self):
        ...
        assert "fireworks" in RENCO_OVERLAYS
        overlay = RENCO_OVERLAYS["fireworks"]
...
class TestFireworksAuxiliary:
    ...
    def test_client_sends_attribution_headers(self, monkeypatch):
        ...
        assert headers["HTTP-Referer"] == "https://renco-agent.nousresearch.com"
        assert headers["X-Title"] == "Renco Agent"
        assert kwargs["base_url"] == "https://api.fireworks.ai/inference/v1"

    def test_client_sends_renco_user_agent(self, monkeypatch):
        ...
        headers = kwargs.get("default_headers", {})
        assert headers["User-Agent"].startswith("RencoAgent/")
