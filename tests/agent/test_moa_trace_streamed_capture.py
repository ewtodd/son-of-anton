...
path via ``renco chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp RENCO_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point RENCO_HOME at a temp dir and turn moa.save_traces on."""
    renco_home = tmp_path / ".renco"
    renco_home.mkdir()
    monkeypatch.setenv("RENCO_HOME", str(renco_home))

    # save_moa_turn reads config via renco_cli.config.load_config; stub it to
