"""``son-of-anton auth`` subcommand parser.
...
def build_auth_parser(subparsers, *, cmd_auth: Callable) -> None:
    ...
    auth_add.add_argument(
        "provider",
        help="Provider id (for example: deepseek, openai-api, custom)",
    )
    auth_add.add_argument(
        "--type",
        dest="auth_type",
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Son of Anton with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set SON_OF_ANTON_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
