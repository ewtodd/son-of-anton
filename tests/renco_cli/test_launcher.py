"""Tests for the top-level `./renco` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./renco` should use `renco_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "renco"
    called = []

    fake_main_module = types.ModuleType("renco_cli.main")

    def fake_main():
        called.append("renco_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "renco_cli.main", fake_main_module)
