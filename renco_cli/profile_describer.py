...
Used by ``renco profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``renco_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Renco Agent kanban board.
...
Rules:
  ...
  - Never write "Renco Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_renco_home())
        else:
