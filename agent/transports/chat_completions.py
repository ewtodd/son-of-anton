...
def _reasoning_config_for_model(model: str, reasoning_config: dict | None) -> dict | None:
    ...
    Renco' internal effort set extends the wire vocabulary with ``ultra``
...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate Renco/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    ...
    # Gemini 2.5 accepts thinkingBudget; don't guess a budget from Renco'
    ...
    # is stricter (low/high). Clamp Renco' wider effort set to what each
...
def _raise_gemini_thinking_max_tokens(
    ...
    global Renco cap of 4096 is enough for visible text, but Ultra/high
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(
        ...
        - Renco-internal scaffolding markers — any top-level message key
        ...
        for msg_idx, msg in enumerate(messages):
            ...
            # Drop all Renco-internal scaffolding markers (``_``-prefixed).
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if message_tool_calls:
            ...
            for tc in message_tool_calls:
                ...
                # preserve an explicit blank name for Renco's recovery path.
