"""Tests for renco_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate RENCO_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".renco"
    home.mkdir()
    ...
    import importlib
    from renco_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from renco_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://renco-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/ewtodd/renco"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import renco_cli.model_catalog as model_catalog
        from renco_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in renco_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``renco model`` CLI — not the live
        ...
        # ``RENCO_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` RENCO_HOME directly instead.
        import importlib
        from renco_cli import model_catalog
        from renco_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from renco_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["RENCO_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("renco_cli.models.check_nous_free_tier", return_value=False), patch(
                "renco_cli.models.union_with_portal_free_recommendations",
