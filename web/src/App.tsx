...
const SIDEBAR_COLLAPSED_KEY = "renco-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="renco-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="renco-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateRenco = status?.can_update_renco === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'renco update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "renco update";
    return (
      t.status.updateRencoConfirmMessage ??
      `This will run 'renco update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateRencoConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateRenco) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateRenco,
      runningLabel: t.status.updatingRenco,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Renco gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateRencoConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateRencoConfirmTitle ?? `${t.status.updateRenco}?`}
    />
    </>
  );
}
