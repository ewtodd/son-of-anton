 ...
 *    a `RencoPlugin` registers automatically (vite glob — drop a folder in).
 *    `renco-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `renco-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<renco home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<renco home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: RencoPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default RencoPlugin export — skipped`)
