...
## Layer 1 — Is this thing a Son of Anton plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Son of Anton process**. Its LLM calls go through Son of Anton's already-configured provider. | Already uses Portal if user's Son of Anton is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Son of Anton. That LLM call is already authenticated
via Son of Anton's provider config. The plugin's own server auth (e.g.
...
What Son of Anton Agent has that *feels* like OAuth — `son-of-anton login --provider nous`
opening a browser, user signs in, token lands in `~/.son-of-anton/auth.json` — is a
**Son of Anton-specific browser flow**. Under the hood it produces a credential
Son of Anton uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Son of Anton's existing Portal credential out of `~/.son-of-anton/auth.json`.
...
rides on the credential Son of Anton already holds.

Where this could live in Son of Anton:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Son of Anton itself.
...
1. First decide: Son of Anton plugin (runs inside Son of Anton) or separate app? If plugin,
   it already uses Portal via Son of Anton's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Son of Anton agent itself") is fully supported. Son of Anton treats agent
...
pitch in the Tool Gateway announcement is specifically about Son of Anton Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Son of Anton is configured to use Portal as its provider. It is
