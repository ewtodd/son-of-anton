...
def build_status_phrase(tool_name: str, args: dict | None, max_len: int = 49) -> str | None:
    ...
    name ("Son of Anton is running …"), so it starts lowercase with "is".
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Son of Anton' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("SON_OF_ANTON_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
