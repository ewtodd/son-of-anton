...
``kind: backend``) or ``~/.son-of-anton/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`son_of_anton_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.son-of-anton/.env``) so credentials set
    through Son of Anton' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``son-of-anton tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``son-of-anton tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``son_of_anton_cli/tools_config.py`` to inject this provider as a
