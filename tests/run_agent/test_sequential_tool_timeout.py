...
def _make_agent(tmp_path: Path) -> AIAgent:
    with (
        ...
        patch("run_agent._son_of_anton_home", tmp_path),
...
def test_sequential_tool_timeout_emits_result_and_continues(tmp_path, monkeypatch):
    ...
    messages: list[dict] = []
    monkeypatch.setenv("SON_OF_ANTON_CONCURRENT_TOOL_TIMEOUT_S", "1.0")
...
def test_sequential_tool_timeout_suppresses_late_terminal_event(tmp_path, monkeypatch):
    import son_of_anton_cli.lifecycle as lifecycle
    import model_tools
...
def test_sequential_timeout_does_not_cut_clarify_human_wait(
    ...
    outlast ``SON_OF_ANTON_CONCURRENT_TOOL_TIMEOUT_S`` (default 420s).
    """
    agent = _make_agent(tmp_path)
    monkeypatch.setenv("SON_OF_ANTON_CONCURRENT_TOOL_TIMEOUT_S", "1.0")
    monkeypatch.setattr(
