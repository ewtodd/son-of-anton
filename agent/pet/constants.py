...
row taxonomy is inferred from the atlas shape so Renco can render both legacy
...
# rows (1536x1872), while older Renco/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Renco' activity state names. They are not always identical to the
...
# Legacy Renco/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Renco activity names -> accepted row-name aliases in descending
