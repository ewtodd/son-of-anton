...
def main_mod(monkeypatch):
    import son_of_anton_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_son_of_anton_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import son_of_anton_cli.oneshot as oneshot
        from son_of_anton_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """son-of-anton -z bypasses SonOfAntonCLI, but recall still needs SessionDB."""
    from son_of_anton_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "son_of_anton_state", mod("son_of_anton_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "son_of_anton_cli.config",
        mod("son_of_anton_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "son_of_anton_cli.models",
        mod("son_of_anton_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "son_of_anton_cli.runtime_provider",
        mod(
            "son_of_anton_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "son_of_anton_cli.tools_config",
        mod("son_of_anton_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["SON_OF_ANTON_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/son-of-anton-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["SON_OF_ANTON_MODEL"] == "nous/son-of-anton-test"
    assert env["SON_OF_ANTON_INFERENCE_MODEL"] == "nous/son-of-anton-test"
    assert env["SON_OF_ANTON_TUI_PROVIDER"] == "nous"
    assert env["SON_OF_ANTON_INFERENCE_PROVIDER"] == "nous"
    assert env["SON_OF_ANTON_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["SON_OF_ANTON_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("son-of-anton-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_son_of_anton_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("SON_OF_ANTON_TUI_DIR", raising=False)
