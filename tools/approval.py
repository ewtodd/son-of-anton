...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("SON_OF_ANTON_YOLO_MODE", ""))
...
# Son of Anton session id (observability identity, distinct from the gateway
...
# os.environ["SON_OF_ANTON_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export SON_OF_ANTON_INTERACTIVE.
_son_of_anton_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "son_of_anton_interactive",
    default=None,
)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``SON_OF_ANTON_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _son_of_anton_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("SON_OF_ANTON_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Son of Anton session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SON_OF_ANTON_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("SON_OF_ANTON_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("SON_OF_ANTON_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("SON_OF_ANTON_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``son-of-anton chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``SON_OF_ANTON_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("SON_OF_ANTON_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("SON_OF_ANTON_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SON_OF_ANTON_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SON_OF_ANTON_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SON_OF_ANTON_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("SON_OF_ANTON_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``SON_OF_ANTON_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# go stale when SON_OF_ANTON_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_SON_OF_ANTON_ENV_PATH = (
    r'(?:~\/\.son-of-anton/|'
    r'(?:\$home|\$\{home\})/\.son-of-anton/|'
...
# theater. Mirrors _SON_OF_ANTON_ENV_PATH; matches the SON_OF_ANTON_HOME override form as
# well as ~/.son-of-anton/.
_SON_OF_ANTON_CONFIG_PATH = (
    r'(?:~\/\.son-of-anton/|'
    r'(?:\$home|\$\{home\})/\.son-of-anton/|'
    r'(?:\$son_of_anton_home|\$\{son_of_anton_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_SON_OF_ANTON_ENV_PATH}|'
    rf'{_SON_OF_ANTON_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_son_of_anton_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Son of Anton: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # `son-of-anton` and `gateway` (e.g. `son-of-anton -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bson-of-anton\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart son-of-anton gateway (kills running agents)"),
    (r'\bson-of-anton\s+update\b', "son-of-anton update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `son-of-anton gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `son-of-anton ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart son-of-anton-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart son-of-anton-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(son-of-anton|gateway|cli\.py)\b', "kill son-of-anton/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill son-of-anton` but not
    # `kill -9 $(pgrep -f son-of-anton)` because the substitution is opaque
    ...
    # the `son-of-anton gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.son-of-anton.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(son-of-anton|ai\.son-of-anton)\b', "stop/restart son-of-anton launchd service (kills running agents)"),
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_SON_OF_ANTON_CONFIG_PATH}|{_SON_OF_ANTON_ENV_PATH})', "in-place edit of Son of Anton config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_SON_OF_ANTON_CONFIG_PATH}|{_SON_OF_ANTON_ENV_PATH})', "in-place edit of Son of Anton config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_SON_OF_ANTON_CONFIG_PATH}|{_SON_OF_ANTON_ENV_PATH})', "in-place edit of Son of Anton config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.son-of-anton/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / SON_OF_ANTON_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Son of Anton home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\son-of-anton), so folding the user home
    # first would eat the prefix the Son of Anton-home fold needs.
    command = _rewrite_resolved_son_of_anton_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / SON_OF_ANTON_HOME such as ``/``,
...
def _rewrite_resolved_son_of_anton_home(command: str) -> str:
    """Rewrite the resolved absolute Son of Anton home prefix to ``~/.son-of-anton/``.
    ...
    try:
        from son_of_anton_constants import get_son_of_anton_home
        home = get_son_of_anton_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.son-of-anton")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``son-of-anton send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Son of Anton ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"son-of-anton-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Son of Anton YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    ...
    into one place:
      - process-scoped ``--yolo`` / ``SON_OF_ANTON_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            SON_OF_ANTON_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set SON_OF_ANTON_INTERACTIVE or "
                "SON_OF_ANTON_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set SON_OF_ANTON_INTERACTIVE or "
            "SON_OF_ANTON_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("SON_OF_ANTON_EXEC_ASK"):
        ...
        # can see (SON_OF_ANTON_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``SON_OF_ANTON_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from son_of_anton_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from son_of_anton_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (SON_OF_ANTON_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # SON_OF_ANTON_EXEC_ASK (and sometimes a session platform marker) can leak
