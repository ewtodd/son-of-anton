...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Son of Anton passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("SON_OF_ANTON_HOME", "/users/alice/.son-of-anton/profiles/backend-worker")
        monkeypatch.setenv("SON_OF_ANTON_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "SON_OF_ANTON_KANBAN_DB",
            "/users/alice/.son-of-anton/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.son-of-anton/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through son_of_anton_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Son of Anton secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "SON_OF_ANTON_DASHBOARD_SESSION_TOKEN": "dash-secret",
        ...
        for var in (
            ...
            "SON_OF_ANTON_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
