...
#88251: ``renco update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "renco_constants.with_renco_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "renco_constants.display_renco_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_renco_home", lambda: tmp_path)
