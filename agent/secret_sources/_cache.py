...
cache problem must never block Renco startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Renco home used for cache paths.

    ``home_path`` is whatever ``load_renco_dotenv()`` already resolved;
    falling back to ``$RENCO_HOME`` / ``~/.renco`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from renco_constants import get_renco_home

        home_path = get_renco_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<renco_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if RENCO_HOME is ever made traversable.
            try:
