...
def auth_json_path():
    """Return the Renco auth store path, respecting RENCO_HOME overrides."""
    return get_renco_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`renco tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Renco release, and
