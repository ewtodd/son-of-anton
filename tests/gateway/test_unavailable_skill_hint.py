...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`son_of_anton_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + SON_OF_ANTON_HOME so the real user config is untouched."""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `son-of-anton skills config` writes).
    monkeypatch.setattr(
    ...
    assert "son-of-anton skills config" in msg
