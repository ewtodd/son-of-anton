...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from renco_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # renco model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in RENCO_OVERLAYS
        overlay = RENCO_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from renco_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a RencoAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("RencoAgent/")

    def test_gmi_profile_declares_renco_user_agent(self):
        """The GMI plugin sets a RencoAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("RencoAgent/"), (
            f"expected GMI profile User-Agent to start with 'RencoAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("renco_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "renco_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["renco", "chat", "--provider", "gmi"])

        from renco_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("renco_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("renco_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("renco_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            from renco_cli.main import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_renco_home() / "config.yaml").read_text()) or {}
