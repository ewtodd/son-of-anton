...
which is already bounded by SON_OF_ANTON_CRON_TIMEOUT — the worker thread submitted
...
timeout (SON_OF_ANTON_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SON_OF_ANTON_CRON_SESSION_DB_TIMEOUT", "0.2")
        ...
        with patch("cron.scheduler._son_of_anton_home", tmp_path), \
             ...
             patch("son_of_anton_state.SessionDB"), \
             patch(
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed SON_OF_ANTON_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors SON_OF_ANTON_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("SON_OF_ANTON_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "SON_OF_ANTON_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("SON_OF_ANTON_CRON_SESSION_DB_TIMEOUT", raising=False)
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._son_of_anton_home", tmp_path), \
                 ...
                 patch("son_of_anton_cli.env_loader.load_son_of_anton_dotenv"), \
                 patch("son_of_anton_cli.env_loader.reset_secret_source_cache"), \
                 patch("son_of_anton_state.SessionDB", side_effect=_hanging_then_capture), \
                 patch(
                     "son_of_anton_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
