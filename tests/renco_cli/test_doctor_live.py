"""Tests for ``renco doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from renco_cli import doctor_live
from renco_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `renco doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("renco_cli.doctor.RENCO_HOME", tmp_path / "home")
        monkeypatch.setattr("renco_cli.doctor.PROJECT_ROOT", tmp_path / "root")
