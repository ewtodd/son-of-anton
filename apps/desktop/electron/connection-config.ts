 ...
 *     `X-Renco-Session-Token` header; WS uses `?token=`.
...
// renco_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - renco_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - renco_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / renco #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-renco_session_at', '__Secure-renco_session_at', 'renco_session_at']
const RT_COOKIE_VARIANTS = ['__Host-renco_session_rt', '__Secure-renco_session_rt', 'renco_session_rt']

// The Nous portal (NAS) does NOT use Renco gateway session cookies — it is a
...
// Keep this aligned with renco_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['renco', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Renco profile the remote SSH dashboard should actually run as.
...
// Renco Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteRencoPath = String(entry.remoteRencoPath || '').trim()

  if (remoteRencoPath) {
    out.remoteRencoPath = remoteRencoPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Renco installation. Preserve an explicit mapping
  // when it is a valid Renco profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_renco_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `RENCO_HOME` scopes it.
 *
 ...
 * and by the registry SSH branch of the `renco:api` handler — both routes
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://renco.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `renco:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a renco session ACCESS-token cookie
 ...
 * the Renco gateway session cookies, so cloud sign-in / discovery liveness
