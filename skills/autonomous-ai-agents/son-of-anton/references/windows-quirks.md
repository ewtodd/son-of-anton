...
Son of Anton runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`son-of-anton config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Son of Anton's env scrubber dropping
...
Son of Anton-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Son of Anton tool and most Windows APIs. Prefer forward slashes in code
