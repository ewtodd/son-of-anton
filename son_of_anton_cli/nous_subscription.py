...
from son_of_anton_cli.config import get_env_value, load_config
...
_DEFAULT_PLATFORM_TOOLSETS = {
    "cli": "son-of-anton-cli",
}

# Maps a tools_config provider's ``managed_nous_feature`` to the tool-pool
# coverage category (son_of_anton_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `son-of-anton tools` picker scope its entitlement gate to the selected backend, so a
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    with the runtime resolver is what stops ``son-of-anton status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Son of Anton-managed Node dirs (Windows installer / POSIX $SON_OF_ANTON_HOME/node)
        ...
        managed_path = with_son_of_anton_node_path().get("PATH", "")
        if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("SON_OF_ANTON_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Keyless Tavily is opt-in: selecting it in `son-of-anton tools` / setup writes
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("SON_OF_ANTON_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("SON_OF_ANTON_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from son_of_anton_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`son-of-anton tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``son-of-anton tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``son-of-anton model``'s Nous
    login, this:
