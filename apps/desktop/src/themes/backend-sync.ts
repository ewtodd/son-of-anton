/**
 * Live skin sync from the Renco backend.
 *
 * The backend resolves the active skin (built-in or `$RENCO_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Renco
 ...
 */
export function ingestBackendSkin(skin: RencoSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as RencoSkin)
