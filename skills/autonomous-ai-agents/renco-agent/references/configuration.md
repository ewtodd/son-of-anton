...
Edit with `renco config edit` or `renco config set section.key value`.
Full reference: https://renco-agent.nousresearch.com/docs/user-guide/configuration
...
`renco config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `renco tools` (interactive) or `renco tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_RENCO_CORE_TOOLS` is the default bundle most platforms inherit).
