...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{SON_OF_ANTON_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``SON_OF_ANTON_HOME`` is a **durable** store — on Son of Anton
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``son-of-anton gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (SON_OF_ANTON_HOME is a
    durable Fly volume on Son of Anton Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    SON_OF_ANTON_HOME volume, or an expired same-epoch orphan (#85433), is
