...
// SON_OF_ANTON_TUI_THEME=light (or SON_OF_ANTON_TUI_BACKGROUND set to something
...
const RELEVANT_ENV = [
  'SON_OF_ANTON_TUI_LIGHT',
  'SON_OF_ANTON_TUI_THEME',
  'SON_OF_ANTON_TUI_BACKGROUND',
  'COLORFGBG',
  'COLORTERM',
  'TERM_PROGRAM'
] as const
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('Son of Anton Agent')
...
describe('detectLightMode', () => {
  ...
  it('honors SON_OF_ANTON_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ SON_OF_ANTON_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ SON_OF_ANTON_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ SON_OF_ANTON_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ SON_OF_ANTON_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ SON_OF_ANTON_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets SON_OF_ANTON_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', SON_OF_ANTON_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors SON_OF_ANTON_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ SON_OF_ANTON_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ SON_OF_ANTON_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', SON_OF_ANTON_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', SON_OF_ANTON_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses SON_OF_ANTON_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ SON_OF_ANTON_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // SON_OF_ANTON_TUI_THEME=light would flip the base palette and make these
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ SON_OF_ANTON_TUI_BACKGROUND: '#ffffff' })
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ SON_OF_ANTON_TUI_BACKGROUND: '#ffffff' })
...
describe('derived tone ladder', () => {
  it('reproduces the original hand-tuned tones from seeds (reverse-engineered knobs)', async () => {
    ...
    const light = await importThemeWithEnv({ SON_OF_ANTON_TUI_BACKGROUND: '#ffffff' })
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  it('renders a dark-authored skin on light like minimumContrastRatio hosts do (the standardized look)', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ SON_OF_ANTON_TUI_BACKGROUND: '#ffffff' })
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ SON_OF_ANTON_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ SON_OF_ANTON_TUI_BACKGROUND: '#ffffff' })
  ...
  it('defaultThemeForCurrentBackground follows a late SON_OF_ANTON_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ SON_OF_ANTON_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
