...
2. ``son-of-anton lsp status`` surfaces a ``Backend warnings`` section when
...
def test_install_npm_works_without_extras(tmp_path, monkeypatch):
    ...
    install_targets = [c for c in cmd if not c.startswith("-") and c not in {
        "install", "--prefix", str(install_mod.son_of_anton_lsp_bin_dir().parent),
        "/usr/bin/npm",
    }]
...
def test_install_pip_finds_windows_scripts_launcher(tmp_path, monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        scripts_dir = install_mod.son_of_anton_lsp_bin_dir().parent / "python-packages" / "Scripts"
    ...
    assert (install_mod.son_of_anton_lsp_bin_dir() / "fake-language-server.exe").exists()


# ---------------------------------------------------------------------------
# Fix 2: ``son-of-anton lsp status`` surfaces shellcheck-missing for bash
# ---------------------------------------------------------------------------
