...
## 2026-07-13: Scope plugin manager state by Renco home/profile (keyed cache)

Status: Accepted

Context:
Renco supports multiple profiles via different Renco home directories.
Homes are switched two ways in a running process: the `RENCO_HOME`
environment variable (single-profile CLI/gateway processes), and the
context-local `set_renco_home_override()` (`renco_constants.py`), which
...
`get_renco_home() / "plugins"`, and context-engine plugins (e.g.
`renco-lcm`) capture profile-scoped state — such as the LCM database
path — at registration time. A single-slot cache meant:

1. Switching homes via `set_renco_home_override()` was invisible to a
   naive "did `RENCO_HOME` change" check, so the singleton silently kept
...
2. Even when a fresh `PluginManager` *was* created for a new home, plugin
   modules are imported into `sys.modules` as `renco_plugins.<slug>` by
   ...
   `renco_plugins.<slug>.<submodule>`, and Python's import machinery
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  Renco home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
  `get_plugin_manager()` resolves the current home via `get_renco_home()`
  (which itself already consults `get_renco_home_override()` before
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `RENCO_HOME` or `set_renco_home_override()`.
...
- Regression coverage exercises the real production path
  (`set_renco_home_override()`) rather than only the env-var path, and
