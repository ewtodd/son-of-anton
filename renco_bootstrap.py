"""Windows UTF-8 bootstrap for Renco entry points.
...
should be imported at the very top of every Renco entry point
(``renco``, ``renco-agent``, ``renco-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``renco_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``renco_bootstrap``, never ``renco_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Renco modules.

    Renco ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Renco source root
    ...
    """
    root = src_root or os.environ.get("RENCO_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``RENCO_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("RENCO_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import renco_bootstrap``
# (or ``from renco_bootstrap import apply_windows_utf8_bootstrap``) at
