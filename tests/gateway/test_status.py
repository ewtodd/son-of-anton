...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "renco-gateway"
    ...
    def test_get_running_pid_cached_invalidates_when_pid_file_changes(self, tmp_path, monkeypatch):
        ...
        def _write_record(pid: int, start_time: int) -> None:
            record = {
                "pid": pid,
                "kind": "renco-gateway",
                "argv": ["python", "-m", "renco_cli.main", "gateway"],
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level RENCO_HOME.
        ...
        the process-level RENCO_HOME, not the profile's directory.  See #56986.
        """
        ...
        monkeypatch.setenv("RENCO_HOME", str(process_home))
...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``renco gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "renco gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "renco -p coder gateway run --replace",
            "/opt/renco/.venv/bin/renco --profile coder gateway run --replace",
            "renco_home=/opt/data/profiles/coder renco gateway run --replace",
        ):
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        """A Windows argv renders RENCO_HOME with backslashes while the
        ...
        cmdline = r"renco_home=c:\opt\data\profiles\coder renco gateway run --replace"
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("RENCO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "renco-gateway",
            "argv": ["/Users/user/.renco/renco-agent/renco_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_self_reacquires_when_disk_start_time_null(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "renco-gateway",
            "argv": ["renco_cli/main.py", "--profile", "milena", "gateway", "run", "--replace"],
    ...
    def test_acquire_scoped_lock_self_reacquires_when_start_times_differ(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 111,
            "kind": "renco-gateway",
            "argv": ["renco_cli/main.py", "gateway", "run", "--replace"],
            "scope": "discord-bot-token",
        }))
    ...
    def test_acquire_scoped_lock_stamps_profile_label(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("RENCO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("RENCO_HOME", str(tmp_path / "profiles" / "lead-gen-outreach"))
    ...
    def test_acquire_scoped_lock_omits_profile_when_not_inferable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("RENCO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("RENCO_HOME", str(tmp_path / "some-custom-dir"))
...
class TestScopedLockOwnerLabel:
    ...
    def test_profile_label_for_root_home_is_default(self, monkeypatch):
        monkeypatch.setenv("RENCO_HOME", "/opt/data")
    ...
    def test_owner_label_prefers_explicit_profile_field(self):
        record = {"profile": "coder", "renco_home": "/opt/data/profiles/other"}
    ...
    def test_owner_label_ignores_invalid_profile_and_uses_home(self):
        ...
        # renco_home fallback — attribution degrades, never corrupts.
        record = {
            "profile": "bad; rm -rf /",
            "renco_home": "/opt/data/profiles/zerocool",
        }
        assert status.scoped_lock_owner_label(record) == "zerocool"

    def test_owner_label_falls_back_to_renco_home(self):
        # Locks written before the profile field existed still attribute.
        record = {"pid": 559, "renco_home": "/opt/data/profiles/zerocool"}
        assert status.scoped_lock_owner_label(record) == "zerocool"
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_renco_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's RENCO_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("RENCO_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
        ...
        payload = json.loads((tmp_path / ".gateway-takeover.json").read_text())
        assert payload["replacer_renco_home"] == str(tmp_path)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        RENCO_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.renco flap
        ...
        marker_path.write_text(json.dumps({
            ...
            "replacer_pid": 99999,
            "replacer_renco_home": str(tmp_path / "profiles" / "other"),
    ...
    def test_consume_accepts_legacy_marker_without_renco_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Renco versions have no
        ``replacer_renco_home`` field; an absent field is treated as same-home so
...
class TestScopedLockTakeover:
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        def terminate(pid, *, force=False):
            ...
            assert payload["target_renco_home"] == str(target_home)
            assert payload["replacer_renco_home"] == str(replacer_home)
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``renco gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00renco_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "renco_cli/main.py" in result
        assert calls == ["proc"]
...
class TestLaunchdPlistRespawnGovernance:
    def test_plist_has_throttle_interval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("RENCO_HOME", str(tmp_path))
        from renco_cli.gateway import generate_launchd_plist
...
class TestResolveGatewayLiveness:
    ...
    def test_profile_dir_scopes_every_read_to_that_profile(self, tmp_path):
        ...
        ignore the RENCO_HOME contextvar override (#56986), so the profile
