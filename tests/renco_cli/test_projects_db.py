"""Tests for the per-profile Projects store (renco_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Renco Agent", folders=["/tmp/renco"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "renco-agent"
    assert proj.name == "Renco Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/renco"
    assert [f.path for f in proj.folders] == ["/tmp/renco"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "renco-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' RENCO_HOME.
