...
Verifies that ``SonOfAntonCLI._handle_update_command`` correctly:
...
Also verifies that ``son_of_anton_cli.main._launch_tui`` correctly handles exit
...
def _make_self(modal_response):
    ...
    so we don't need a full ``SonOfAntonCLI`` construction.
...
def _call(self_):
    ...
    return SonOfAntonCLI._handle_update_command(self_)
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        patch("son_of_anton_cli.config.is_managed", return_value=True),
        patch(
            "son_of_anton_cli.config.format_managed_message",
...
def test_affirmative_answer_sets_pending_relaunch_and_returns_true(answer, capsys):
    ...
    with patch("son_of_anton_cli.config.is_managed", return_value=False):
        result = _call(self_)
