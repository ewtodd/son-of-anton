"""
Top-level argparse construction for the son-of-anton CLI.
...
# argparse runs (it sets ``SON_OF_ANTON_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``son_of_anton_cli.relaunch`` should carry over when
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="son-of-anton",
        description="Son of Anton Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `son-of-anton chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Son of Anton Agent",
    )
    ...
    # top-level parser: when the user writes `son-of-anton -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/son_of_anton_cli/
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Son of Anton itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
