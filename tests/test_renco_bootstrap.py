"""Tests for renco_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Renco entry point
(renco, renco-agent, renco-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Renco entry point imports renco_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported renco_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("renco_bootstrap", None)
    import renco_bootstrap  # noqa: WPS433
    return renco_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Renco CLI
...
class TestEntryPointsImportBootstrap:
    """Every Renco entry point must import renco_bootstrap as its
    ...
    # Entry points that invoke Renco as a process.  Each one must
    # import renco_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "renco_cli/main.py",   # renco CLI (console_script)
        "run_agent.py",          # renco-agent (console_script)
        "acp_adapter/entry.py",  # renco-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import renco_bootstrap' and that
        ...
        half-finished ``renco update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``renco_bootstrap`` as a top-level module) leaves renco
        ...
        """
        # Resolve relative to the renco-agent repo root.  Tests live
        # at tests/test_renco_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # renco start even when renco_bootstrap hasn't been
        ...
        assert first_import_name == "renco_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'renco_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import renco_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Renco's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("RENCO_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["RENCO_PYTHON_SRC_ROOT"] = env
            elif "RENCO_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["RENCO_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/renco")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("RENCO_PYTHON_SRC_ROOT", None)
            else:
                os.environ["RENCO_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/renco", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Renco root.  The guard must relocate Renco to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/renco"])
        assert result[0] == "/opt/renco"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Renco root.
        assert result.index("/opt/renco") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["RENCO_PYTHON_SRC_ROOT"] = "/env/renco"
            hb.harden_import_path()
            assert sys.path[0] == "/env/renco"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("RENCO_PYTHON_SRC_ROOT", None)
