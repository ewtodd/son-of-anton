...
/**
 * E2E smoke tests for the packaged Renco desktop app.
 *
 ...
 * (credential stripping, isolated RENCO_HOME + userData, unique app name).
 *
...
test('window opens with the Renco title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      rencoDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).rencoDesktop?.hud?.open({ sessionId: null })
  )
