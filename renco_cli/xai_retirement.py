...
Pure logic: walks a Renco config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `renco doctor` and a future `renco migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Renco config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Renco config and return retirement issues.
...
def apply_migration(
    ...
    from renco_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `renco migrate xai` reports nothing to migrate
