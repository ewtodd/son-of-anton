...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __rencoWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__rencoWatch || (win.__rencoWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('renco-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('renco-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __rencoWatchTag?: number }).__rencoWatchTag
