...
def checkpoint_base(tmp_path):
    """Isolated checkpoint base — never writes to ~/.son-of-anton/."""
...
class TestStoreInit:
    ...
    def test_legacy_migration_archives_prev2_repos(
        ...
        (fake_repo / "SON_OF_ANTON_WORKDIR").write_text(str(work_dir) + "\n")
...
class TestSafeRestore:
    """Safe restore: preserve user hand-edits, revert only Son of Anton-authored changes.
    ...
    def test_safe_restore_skips_file_user_edited_after_agent(self, mgr, work_dir):
        ...
        # Son of Anton writes the file, then the user modifies it afterwards.
        (work_dir / "main.py").write_text("agent version\n")
        mgr.record_agent_write(str(work_dir / "main.py"))
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # Content no longer matches what Son of Anton last wrote → preserved.
    ...
    def test_safe_restore_removes_agent_created_file_keeps_user_edit(self, mgr, work_dir):
        ...
        # Son of Anton creates a brand-new file after the checkpoint...
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # User edit preserved; Son of Anton-created file removed (not in checkpoint).
...
def _seed_legacy_repo(base: Path, name: str, workdir: Path, mtime: float = None) -> Path:
    ...
    (shadow / "SON_OF_ANTON_WORKDIR").write_text(str(workdir) + "\n")
    (shadow / "info").mkdir()
...
class TestPruneCheckpointsOrphanAllowlist:
    ...
    def test_end_to_end_timing_change_during_confirmation_prompt(self, tmp_path, monkeypatch):
        ...
        `son-of-anton checkpoints prune`: the preview shows one pre-v2 orphan; a
        ...
        be deleted.
        """
        import son_of_anton_cli.checkpoints as checkpoints_cli
