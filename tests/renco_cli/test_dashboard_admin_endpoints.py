...
def _client():
    ...
    # Keep the state DB under the isolated RENCO_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_renco_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        config_text = (renco_home / "config.yaml").read_text()
        env_text = (renco_home / ".env").read_text()
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.renco/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `renco auth remove`: clean up the backing
        source and suppress (provider, source).
        """
        from agent.credential_pool import load_pool
        from renco_cli.auth import is_source_suppressed
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `renco auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_renco_home):
        ...
        (get_renco_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_renco_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_renco_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "renco_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("renco-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # renco-index source advertises availability + featured search.
            idx = _Src("renco-index")
        ...
        assert {"official", "github", "renco-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "renco_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/renco/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/renco/update`` runs ``renco update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import renco_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/renco/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/renco/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr("renco_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `renco tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_renco_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_renco_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _RENCO_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_renco_action stores
    ...
    ws._spawn_renco_action(["gateway", "restart"], "gateway-restart")

    assert "_RENCO_GATEWAY" not in captured["env"]
    assert captured["env"]["RENCO_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_renco_home
):
    ...
    startup under RENCO_DESKTOP=1 so the stale orphan is cleared first.
    """
    import renco_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from renco_cli.gateway, so patch it
    # on that module.
    import renco_cli.gateway as g
