"""Safe Son of Anton Console command engine.

This module backs ``son-of-anton console`` and is intentionally narrower than the
full Son of Anton CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["SonOfAntonConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'son-of-anton doctor'")
        and last.startswith("Run 'son-of-anton setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="son-of-anton", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `son-of-anton "):
        return ""
    return summary
...
# dashboard opens a fresh SonOfAntonConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("son_of_anton_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["SonOfAntonConsoleEngine", list[str]], str]:
    def handler(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "SonOfAntonConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["SonOfAntonConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `son-of-anton {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `son-of-anton {usage}`?",
        )


class SonOfAntonConsoleEngine:
    """Curated line-command executor for Son of Anton Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "son-of-anton":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Son of Anton Console does not run shell syntax. Use one supported "
                    "Son of Anton command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Son of Anton component status.", _status)
        self.register(("version",), "version", "Show Son of Anton version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Son of Anton logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Son of Anton configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Son of Anton Console."""

        extracted = {
            "dump": (
                "son_of_anton_cli.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "debug": (
                "son_of_anton_cli.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "son_of_anton_cli.subcommands.prompt_size",
            ...
            "insights": (
                "son_of_anton_cli.subcommands.insights",
            ...
            "security": (
                "son_of_anton_cli.subcommands.security",
            ...
            "backup": (
                "son_of_anton_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "son_of_anton_cli.subcommands.import_cmd",
            ...
            "config": (
                "son_of_anton_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "son_of_anton_cli.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "son_of_anton_cli.subcommands.plugins",
            ...
            "skills": (
                "son_of_anton_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "mcp": (
                "son_of_anton_cli.subcommands.mcp",
                "build_mcp_parser",
                "cmd_mcp",
                [
                    ("list",),
            ...
            "memory": (
                "son_of_anton_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "auth": (
                "son_of_anton_cli.subcommands.auth",
                "build_auth_parser",
                "cmd_auth",
                [
                    ("list",),
            ...
            "pairing": (
                "son_of_anton_cli.subcommands.pairing",
            ...
            "slack": (
                "son_of_anton_cli.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Son of Anton configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "son_of_anton_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("son_of_anton_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "son_of_anton_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("son_of_anton_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("son_of_anton_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Son of Anton Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Son of Anton Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        blocked_pairs = {
            ...
            ("portal", "open"): "`portal open` opens a browser and is not available in Son of Anton Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Son of Anton Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Son of Anton Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Son of Anton Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Son of Anton Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Son of Anton Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Son of Anton Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Son of Anton Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Son of Anton Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Son of Anton Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Son of Anton Console.")
...
def _version(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "version")
    from son_of_anton_cli._startup_fast import print_fast_version_info

    return _capture_output(lambda: print_fast_version_info(check_updates=True))


def _status(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Son of Anton Console.")
    ...
    from son_of_anton_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from son_of_anton_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _config_set(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
...
def _config_migrate(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from son_of_anton_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _cron_list(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from son_of_anton_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from son_of_anton_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from son-of-anton console")
...
def _cron_resume(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: SonOfAntonConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``son-of-anton console`` REPL."""
    ...
    engine = SonOfAntonConsoleEngine()
    if interactive:
        print("Son of Anton Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("son-of-anton> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
