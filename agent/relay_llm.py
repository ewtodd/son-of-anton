"""Core NeMo Relay adapters for physical Renco provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Renco knows the API mode."""
...
def _relay_metadata(
    ...
    if not isinstance(relay_metadata, dict):
        relay_metadata = {}
    relay_metadata.setdefault("renco.provider", provider_name)
...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited Renco turn when present."""
...
def stream_current(
    ...
    *,
    ...
    """Run a provider stream under the inherited Renco turn when present.
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from Renco's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # Renco-side callbacks run while the native pipeline drives
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Renco."""
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "Renco Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
