...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with son-of-anton)
2. User plugins: ``$SON_OF_ANTON_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``son_of_anton_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SON_OF_ANTON_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$SON_OF_ANTON_HOME/plugins/model-providers/`` if it exists."""
    try:
        from son_of_anton_constants import get_son_of_anton_home

        d = get_son_of_anton_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple SON_OF_ANTON_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_son_of_anton_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``son_of_anton_agent.plugins``
    entry-point group so they self-register.

    A distribution ships::

        [project.entry-points."son_of_anton_agent.plugins"]
        acme-inference = "acme_son_of_anton_plugin:register"
    ...
    * **Provider targets only.** The ``son_of_anton_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$SON_OF_ANTON_HOME``) keep their
    ...
    try:
        from son_of_anton_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$SON_OF_ANTON_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``son_of_anton_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$SON_OF_ANTON_HOME`` profile of the
    ...
    # 2. User plugins — under $SON_OF_ANTON_HOME/plugins/model-providers/<name>/.
