...
mirroring the Nous provider's precedence convention:
  ...
  Environment overrides::
      ...
      RENCO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      RENCO_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      RENCO_DASHBOARD_BASIC_AUTH_SECRET
      RENCO_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "RENCO_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or RENCO_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "RENCO_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "RENCO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "RENCO_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "RENCO_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "RENCO_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # RENCO_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
