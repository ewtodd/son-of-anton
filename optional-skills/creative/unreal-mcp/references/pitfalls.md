...
### 1. Start order: editor first, Renco second

Renco probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Renco session.
...
restart the editor, restart the Renco session.
...
server fails to bind (Output Log) or Renco' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.renco/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Renco

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Renco' connection
lives in `~/.renco/config.yaml` via `renco mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Renco — just don't
mistake it for the Renco setup step.
...
explicitly warns against overlapping calls. Renco executes same-turn tool
...
### 9. Timeouts: Renco gives up before Unreal does

Renco' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Renco runs elsewhere (SSH backend,
