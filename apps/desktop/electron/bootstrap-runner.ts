 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Renco Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.renco-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(rencoHome) {
  return path.join(rencoHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.renco/renco-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(rencoHome) {
  if (!rencoHome) {
    return null
  }

  const candidate = path.join(rencoHome, 'renco-agent', 'scripts', installScriptName())
...
function cachedScriptPath(rencoHome, commit) {
  return path.join(bootstrapCacheDir(rencoHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/ewtodd/renco/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(rencoHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(rencoHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, rencoHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass RENCO_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          RENCO_HOME: rencoHome || process.env.RENCO_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, rencoHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, rencoHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--renco-home', rencoHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, rencoHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, rencoHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, rencoHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(rencoHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, rencoHome, emit })
