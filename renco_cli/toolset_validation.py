...
the tool registry or launching Renco (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``renco-cli`` to the non-existent ``renco``. ``resolve_toolset('renco')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``renco-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``renco`` instead of ``renco-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `renco tools` to reconfigure."
        )
    return warnings
