...
const RENCO_PATHS_MIME = 'application/x-renco-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(RENCO_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(RENCO_PATHS_MIME)
  ...
  const getPath = window.rencoDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.rencoDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (renco --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.rencoDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[renco-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.rencoDesktop?.terminal?.write(sessionId, `${command}\r`)
