...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (RENCO_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via RENCO_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in RENCO_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from renco_cli.providers import RENCO_OVERLAYS
        assert "bedrock" in RENCO_OVERLAYS
