...
"No LLM provider configured. Run `son-of-anton model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real SON_OF_ANTON home.
...
# found" (the b200/son-of-anton-ultra-sft report). The stored MODEL NAME is the one
...
ULTRA_LEGACY_CONFIG = {
    ...
    "custom_providers": [
        {
            "name": "son-of-anton-ultra",
            "base_url": ULTRA_URL,
            "api_key": "sk-ultra",
            "model": "son-of-anton-ultra-sft",
        }
    ],
}
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("son-of-anton-ultra-sft")
            == "custom:son-of-anton-ultra"
        )
