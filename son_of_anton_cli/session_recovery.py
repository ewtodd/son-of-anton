"""Offline, non-destructive recovery for a damaged Son of Anton session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``son_of_anton_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``son_of_anton_state._backup_db_file``.
    """
    from son_of_anton_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="son-of-anton-session-recovery-",
        dir=str(work_root),
    )
...
def _recover_via_lost_and_found(
    *,
    ...
    from son_of_anton_cli.session_lost_and_found import (
