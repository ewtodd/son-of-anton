...
def isolated_home(monkeypatch):
    """Temp RENCO_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="renco_test_31179_")
    renco_home = os.path.join(test_home, ".renco")
    os.makedirs(renco_home)
...
def _fresh_modules():
    """Drop cached renco modules so each test reloads against current env."""
    _drop_reload_targets()
