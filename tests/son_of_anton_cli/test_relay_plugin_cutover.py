"""Regression tests for migration from the removed Son of Anton Relay plugin."""
...
from son_of_anton_cli.doctor import collect_relay_plugin_cutover_findings
from son_of_anton_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV
...
def test_doctor_reports_legacy_exporter_env_without_new_config(monkeypatch):
    ...
    findings = dict(
        collect_relay_plugin_cutover_findings(
            {},
            {
                "SON_OF_ANTON_NEMO_RELAY_ATIF_ENABLED": "true",
                "SON_OF_ANTON_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S": "30",
            },
        )
    )

    assert "now ignored" in findings["SON_OF_ANTON_NEMO_RELAY_ATIF_ENABLED"]
    assert RELAY_PLUGINS_CONFIG_ENV in findings["SON_OF_ANTON_NEMO_RELAY_ATIF_ENABLED"]
    assert "now ignored" in findings["SON_OF_ANTON_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S"]
    assert (
        RELAY_PLUGINS_CONFIG_ENV
        in findings["SON_OF_ANTON_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S"]
    )
...
def test_enable_rejects_removed_relay_plugin_without_discovery(name, capsys):
    with (
        patch("son_of_anton_cli.plugins_cmd._resolve_plugin_key_and_source") as resolve,
        patch("son_of_anton_cli.plugins_cmd._save_enabled_set") as save_enabled,
    ):
        from son_of_anton_cli.plugins_cmd import cmd_enable
...
def test_enable_rejects_alias_resolving_to_removed_relay_plugin(capsys):
    with (
        patch(
            "son_of_anton_cli.plugins_cmd._resolve_plugin_key_and_source",
