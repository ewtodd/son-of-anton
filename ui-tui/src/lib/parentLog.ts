...
const logDir = join(process.env.RENCO_HOME?.trim() || join(homedir(), '.renco'), 'logs')
...
// a real ~/.renco (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('renco-tui: parent lifecycle log unavailable\n')
    }
  }
}
