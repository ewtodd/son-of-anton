...
import { getRencoConfig, getRencoConfigDefaults } from '@/renco'
...
interface RencoConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useRencoConfig({ activeSessionIdRef }: RencoConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshRencoConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getRencoConfig(), getRencoConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Renco' built-in medium over the user's config.
  ...
  return { refreshRencoConfig, sttEnabled, voiceMaxRecordingSeconds }
}
