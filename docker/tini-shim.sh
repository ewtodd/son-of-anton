...
# Renco WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: RENCO_TINI_SHIM_TARGET overrides the `/init` path (and
# RENCO_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${RENCO_TINI_SHIM_TARGET:-/init}"
WRAPPER="${RENCO_TINI_SHIM_WRAPPER:-/opt/renco/docker/main-wrapper.sh}"
