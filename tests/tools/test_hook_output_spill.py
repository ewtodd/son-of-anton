...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("son_of_anton_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="son-of-anton-spill-test-")
    ...
    def test_default_directory_uses_son_of_anton_home(self):
        """When no directory override, spill under SON_OF_ANTON_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="son-of-anton-home-")
        try:
            with patch.dict(os.environ, {"SON_OF_ANTON_HOME": test_home}):
                # Also patch get_son_of_anton_home to the env var to mirror production.
            ...
            # ~/.son-of-anton/hook_outputs depending on get_son_of_anton_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.son-of-anton/hook_outputs/sess")),
            ]
