...
# cwd. Without an explicit PYTHONPATH the child resolves ``renco_cli`` /
...
def _make_running_kanban_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("RENCO_PROFILE", "parent-worker")
    monkeypatch.setenv("RENCO_KANBAN_WORKSPACE", str(workspace))
    monkeypatch.setenv("RENCO_KANBAN_ATTACHMENTS_ROOT", str(attachments_root))
    ...
    monkeypatch.setenv("RENCO_KANBAN_RUN_ID", str(run_id))
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    The parent process may be a dispatcher worker with RENCO_KANBAN_TASK set;
    the child is only a subagent, not the run owner.
    """
    monkeypatch.setenv("RENCO_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("RENCO_KANBAN_RUN_ID", "123")
    home = tmp_path / ".renco"
    home.mkdir()
    monkeypatch.setenv("RENCO_HOME", str(home))
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    ``os.environ`` and therefore never wrote RENCO_DELEGATED_CHILD_CONTEXT into
    the sandbox env.
    """
    home = tmp_path / ".renco"
    home.mkdir()
    ...
    monkeypatch.setenv("RENCO_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("RENCO_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("RENCO_KANBAN_DB", str(home / "kanban.db"))
    monkeypatch.setenv("RENCO_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("RENCO_KANBAN_CLAIM_LOCK", "lock")
    monkeypatch.delenv("RENCO_DELEGATED_CHILD_CONTEXT", raising=False)
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("RENCO_KANBAN_"),
            is_windows=False,
        )

    assert os.environ.get("RENCO_DELEGATED_CHILD_CONTEXT") is None
    assert env["RENCO_HOME"] == str(home)
    assert env["RENCO_DELEGATED_CHILD_CONTEXT"] == "1"
    assert "RENCO_KANBAN_TASK" not in env
    assert "RENCO_KANBAN_RUN_ID" not in env
    assert "RENCO_KANBAN_DB" not in env
    assert "RENCO_KANBAN_WORKSPACE" not in env
    assert "RENCO_KANBAN_CLAIM_LOCK" not in env
