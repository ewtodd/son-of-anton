...
This plugin connects Renco Agent to iMessage (and other Spectrum
...
TypeScript-only, Renco runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Renco channel,
...
Run `renco photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.renco/.env` (the same place every other
...
Management metadata lives in `~/.renco/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `renco photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Renco
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Renco coalesces
...
- **Native polls are supported.** Renco posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `renco send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<renco-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `renco photon status`, a DM and a group roundtrip,
