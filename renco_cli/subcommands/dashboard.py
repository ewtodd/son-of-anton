"""``renco dashboard`` / ``renco serve`` subcommand parsers.
...
``renco_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `renco dashboard` / `renco serve` cmdlines and SIGTERM them directly —
    # the same path `renco update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Renco web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Renco web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Renco Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Renco desktop app shells (<= 0.15.x) spawn the
    # backend as `renco dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Renco couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Renco Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Renco backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Renco backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports RENCO_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `renco dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.renco/.env.
    # Nested subparser so bare `renco dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "RENCO_DASHBOARD_OAUTH_CLIENT_ID into ~/.renco/.env, and prints "
            "how to engage the login gate. Requires being logged in (renco setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://renco.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via RENCO_DASHBOARD_PORTAL_URL. Mainly for "
