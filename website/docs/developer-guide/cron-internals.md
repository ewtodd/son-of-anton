...
description: "How Renco stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `renco_cli/cron.py` | CLI `renco cron` subcommands |
...
Jobs are stored in `~/.renco/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `renco cron` commands are run or during active CLI sessions.
...
```python
# ~/.renco/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.renco/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `renco cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `renco cron` CLI provides direct job management:

```bash
renco cron list                    # Show all jobs
renco cron create                  # Interactive job creation (alias: add)
renco cron edit <job_id>           # Edit job configuration
renco cron pause <job_id>          # Pause a running job
renco cron resume <job_id>         # Resume a paused job
renco cron run <job_id>            # Trigger immediate execution
renco cron remove <job_id>         # Delete a job
```
