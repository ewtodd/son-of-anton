...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.renco/kanban.db`. Your task id is in "
    "`$RENCO_KANBAN_TASK`; your workspace is `$RENCO_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $RENCO_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $RENCO_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${RENCO_KANBAN_BRANCH:-wt/$RENCO_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$RENCO_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`renco profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `renco kanban <verb>` for board operations. Use "
