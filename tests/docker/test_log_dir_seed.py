"""Runtime smoke test for Docker $RENCO_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the renco user after container boot.
...
registered later runs its log service as the dropped renco user and
...
def test_logs_gateways_seeded_and_renco_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by renco after boot."""
    ...
    assert "logs=renco" in r.stdout, (
        f"logs/ not owned by renco: {r.stdout}"
    )
    assert "gateways=renco" in r.stdout, (
        f"logs/gateways/ not owned by renco: {r.stdout}"
    )
