...
def _is_dispatcher_owned_worker() -> bool:
    """False when SON_OF_ANTON_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_son_of_anton_home() under the active profile.
...
#   - cli.py, son_of_anton_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("SON_OF_ANTON_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by SON_OF_ANTON_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like son-of-anton-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("son-of-anton-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (son-of-anton-*) include _SON_OF_ANTON_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("son-of-anton-")
    ...
    # window). Core Son of Anton tools (toolsets._SON_OF_ANTON_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from son_of_anton_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to son-of-anton's coercion layer.
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from son_of_anton_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from son_of_anton_cli.plugins import _dispatch_pre_tool_call_hooks
        ...
        try:
            ...
            if skip_tool_execution_middleware:
                ...
                from son_of_anton_cli.middleware import run_tool_execution_middleware
