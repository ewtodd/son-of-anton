...
def test_output_path_rejects_renco_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    renco_home = tmp_path / "renco-home"
    renco_home.mkdir()
    monkeypatch.setattr(file_safety, "_renco_home_path", lambda: renco_home)
    monkeypatch.setattr(file_safety, "_renco_root_path", lambda: renco_home)

    target = renco_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    renco_home = tmp_path / "renco-home"
    token_dir = renco_home / "mcp-tokens"
