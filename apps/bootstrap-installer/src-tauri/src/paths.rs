...
//!
//! Mirrors `renco_constants.get_renco_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\renco
//!   macOS:   ~/.renco
//!   Linux:   ~/.renco  (override via $RENCO_HOME)
//!
//! NOTE (macOS): Python's get_renco_home(), scripts/install.sh, and the
//! Electron desktop's resolveRencoHome() ALL use ~/.renco on macOS — there
...
/// Returns the canonical Renco home directory, respecting $RENCO_HOME if set.
pub fn renco_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("RENCO_HOME") {
    ...
    {
        // %LOCALAPPDATA%\renco — matches scripts/install.ps1's $RencoHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("renco");
        }
    }

    // macOS + Linux + fallback: ~/.renco (matches Python get_renco_home(),
    // install.sh, and the Electron desktop's resolveRencoHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".renco");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    renco_home().join("bootstrap-cache")
}
...
/// RENCO_HOME so it survives repo checkout deletion (unlike anything under
/// renco-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\renco\renco-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_renco`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under RENCO_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves RENCO_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    renco_home().join(".renco-update-in-progress")
}
...
pub fn copy_self_to_renco_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to RENCO_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_RENCO_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".renco-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under RENCO_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[renco-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("RENCO_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_renco_home() -> String {
    renco_home().to_string_lossy().into_owned()
}
