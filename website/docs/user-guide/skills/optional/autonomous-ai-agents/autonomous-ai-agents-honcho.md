---
title: "Honcho — Configure and troubleshoot Honcho memory for Renco"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Renco"
---
...
Configure and troubleshoot Honcho memory for Renco.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `renco skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Renco

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Renco profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/renco#running-honcho-locally-with-renco

### Verify

```bash
renco honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Renco profile's AI peer
...
Honcho models conversations as interactions between **peers**. Renco creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Renco instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Renco run |
| `global` | Single session across all directories |

Manual override: `renco honcho map my-project-name`
...
Each Renco profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block renco.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `renco.coder` host block in `honcho.json`
...
```bash
renco honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Renco when Honcho memory is active.
...
Config file: `$RENCO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `renco honcho setup`. Ensure `memory.provider: honcho` is in `~/.renco/config.yaml`.

### Memory not persisting across sessions
Check `renco honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `renco profile create <name> --clone`. For existing profiles: `renco honcho sync`.
...
| Command | Description |
|---------|-------------|
| `renco honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `renco honcho status` | Show resolved config, connection test, peer info for active profile |
| `renco honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `renco honcho disable` | Disable Honcho for the active profile |
| `renco honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `renco honcho peers` | Show peer identities across all profiles |
| `renco honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `renco honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `renco honcho sessions` | List known directory-to-session-name mappings |
| `renco honcho map <name>` | Map current working directory to a Honcho session name |
| `renco honcho identity` | Seed AI peer identity or show both peer representations |
| `renco honcho sync` | Create host blocks for all Renco profiles that don't have one yet |
| `renco honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Renco + Honcho |
| `renco memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `renco memory status` | Show active memory provider and config |
| `renco memory off` | Disable external memory provider |
