...
from son_of_anton_constants import get_default_son_of_anton_root, get_son_of_anton_home, display_son_of_anton_home
from utils import (
    _preserve_file_mode,
...
from son_of_anton_cli.sizefmt import format_bytes as _format_size
...
_EXCLUDED_DIRS = {
    "son-of-anton",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under SON_OF_ANTON_HOME) —
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Son of Anton backup slot."""
...
@contextmanager
def _backup_operation_lock(son_of_anton_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = son_of_anton_home / ".backup.lock"
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside SON_OF_ANTON_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to son-of-anton root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``son-of-anton`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/son-of-anton/`` — must be preserved.
        if part == "son-of-anton" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SON_OF_ANTON_HOME.
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # son_of_anton_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Son of Anton home directory."""
    son_of_anton_root = get_default_son_of_anton_root()

    if not son_of_anton_root.is_dir():
        print(f"Error: Son of Anton home directory not found at {son_of_anton_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(son_of_anton_root):
            _run_backup_locked(args, son_of_anton_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, son_of_anton_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"son-of-anton-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"son-of-anton-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_son_of_anton_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(son_of_anton_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(son_of_anton_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``son-of-anton`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "son-of-anton" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(son_of_anton_root)
    ...
    # outside SON_OF_ANTON_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_son_of_anton_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: son-of-anton import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Son of Anton backup.
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a son-of-anton dir name
        if prefix in {".son-of-anton", "son-of-anton"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``son-of-anton import``, which is the
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``son-of-anton import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Son of Anton' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_son_of_anton_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Son of Anton configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under SON_OF_ANTON_HOME.
            ...
            target = son_of_anton_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(son_of_anton_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_son_of_anton_home()}."
            )
        ...
        try:
            from son_of_anton_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Son of Anton configuration has been restored.")
...
_QUICK_STATE_FILES = (
    ...
    # are at $SON_OF_ANTON_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside SON_OF_ANTON_HOME and the entry is silently
...
def _quick_snapshot_root(son_of_anton_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    Args:
        max_file_size: When set, individual files larger than this many bytes
            ...
            never stall ``son-of-anton update`` or silently eat disk — the small
            pairing/cron/config files the snapshot exists to protect are
            always captured. ``None`` (default) copies everything, which
            preserves manual ``/snapshot`` and ``son-of-anton backup --quick``
            behavior.
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: son-of-anton snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(son_of_anton_home)
    if not root.exists():
        return []
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``son-of-anton update``.
    ...
    Args:
        ...
        son_of_anton_home: Override for the Son of Anton home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(son_of_anton_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for son-of-anton backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_son_of_anton_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, son_of_anton_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(son_of_anton_root):
            return _write_full_zip_backup_locked(out_path, son_of_anton_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, son_of_anton_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``son_of_anton_root`` to ``out_path``.
...
def _pre_update_backup_dir(son_of_anton_home: Optional[Path] = None) -> Path:
    home = son_of_anton_home or get_son_of_anton_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SON_OF_ANTON_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SON_OF_ANTON_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``son-of-anton update``) should continue even if the backup fails.
    """
    son_of_anton_root = son_of_anton_home or get_default_son_of_anton_root()
    if not son_of_anton_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(son_of_anton_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, son_of_anton_root)
    if result is None:
        return None
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SON_OF_ANTON_HOME under ``backups/`` before a
    ``son-of-anton claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``son-of-anton import <archive>``.  Writes to
    ``<SON_OF_ANTON_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `son-of-anton import` and the
