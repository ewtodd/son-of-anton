...
import type { RencoGitWorktree, RencoRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: RencoGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, RencoRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, RencoGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<RencoRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<RencoGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<RencoRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<RencoGitWorktree[]>>()
const $noRepoStatus = atom<RencoRepoStatus | null>(null)
const $noWorktrees = atom<RencoGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<RencoRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<RencoGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: RencoRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: RencoGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<RencoRepoStatus | null>
  seq: number
  target: string
}
