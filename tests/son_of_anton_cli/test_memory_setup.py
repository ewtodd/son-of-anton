...
import son_of_anton_cli.memory_setup as memory_setup
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_son_of_anton_home", lambda: tmp_path)
    monkeypatch.setattr("son_of_anton_cli.config.load_config", lambda: {"memory": {}})
...
def test_install_dependencies_force_reinstalls_versioned_specs(tmp_path, monkeypatch):
    ...
    so a downgraded/stripped bridge package is restored on son-of-anton update."""
...
def test_cmd_status_memory_tool_gate_disabled(capsys, monkeypatch):
    ...
    monkeypatch.setattr("son_of_anton_cli.config.load_config", lambda: _cfg)
    # check_memory_requirements() reads the readonly loader, not load_config.
    monkeypatch.setattr(
        "son_of_anton_cli.config.load_config_readonly", lambda: _cfg, raising=False
    )
