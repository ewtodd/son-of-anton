...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Renco's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Renco instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
renco memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `renco` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Renco syncs them back at session init.
...
The `renco honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `renco memory setup honcho` (or run `renco memory setup` and pick it from the list); the `renco honcho` subcommand then appears on the next invocation.

```bash
renco memory setup honcho    # Configure Honcho directly (works before activation)
renco honcho status          # Connection status, config, and key settings
renco honcho setup           # Redirects to `renco memory setup` (post-activation alias)
renco honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
renco honcho peer            # Show or update peer names + dialectic reasoning level
renco honcho mode            # Show or set recall mode (hybrid/context/tools)
renco honcho tokens          # Show or set token budget for context and dialectic
renco honcho identity        # Seed or show the AI peer's Honcho identity
renco honcho sync            # Sync Honcho config to all existing profiles
renco honcho peers           # Show peer identities across all profiles
renco honcho sessions        # List known Honcho session mappings
renco honcho map             # Map current directory to a Honcho session name
renco honcho enable          # Enable Honcho for the active profile
renco honcho disable         # Disable Honcho for the active profile
renco honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `renco honcho`

If you previously used the standalone `renco honcho setup`:
...
No re-login or re-setup needed. Run `renco memory setup` and select "honcho" — the wizard detects your existing config.
