...
Every aux picker (``son-of-anton model`` → Configure auxiliary models, the
``son-of-anton tools`` vision picker, and any future one) must route through
``son_of_anton_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
def configured_home(tmp_path, monkeypatch):
    """A SON_OF_ANTON_HOME with one ``providers:`` entry and one legacy
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from son_of_anton_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    with patch("son_of_anton_cli.model_switch.list_authenticated_providers", _capture):
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    """
    import son_of_anton_cli.main as main
    import son_of_anton_cli.tools_config as tools_config
    ...
    with (
        patch("son_of_anton_cli.model_switch.list_authenticated_providers", _direct),
        patch("son_of_anton_cli.inventory.build_aux_picker_rows", _substrate),
        patch("son_of_anton_cli.main._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through son_of_anton_cli.inventory.build_aux_picker_rows so custom "
