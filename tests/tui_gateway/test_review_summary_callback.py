    ...
    with patch.dict(
        "sys.modules",
        {
            "son_of_anton_constants": MagicMock(
                get_son_of_anton_home=MagicMock(return_value="/tmp/son_of_anton_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'son-of-anton-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'son-of-anton-release' patched"
    }
