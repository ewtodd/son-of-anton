...
_FORCE_SYNC_ENV = "SON_OF_ANTON_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.son-of-anton") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.son-of-anton to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.son-of-anton", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``SON_OF_ANTON_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, son_of_anton_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.son-of-anton/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(son_of_anton_home=son_of_anton_home)

    def _sync_back_transaction(self, son_of_anton_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .son-of-anton/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (son_of_anton_home or get_son_of_anton_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="son-of-anton-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.son-of-anton/skills/a.md`` →
        ``~/.son-of-anton/skills/a.md``, a new remote file at
        ``/root/.son-of-anton/skills/b.md`` maps to ``~/.son-of-anton/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
