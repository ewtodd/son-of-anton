...
# per-profile log file (~/.son-of-anton/logs/mcp-stderr.log), tagged with the
...
def sdk_httpx():
    ...
    side by side with Son of Anton' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Son of Anton' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Son of Anton explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Son of Anton and
    its subprocesses can consume those credentials without duplicating them
    in every MCP server's ``env:`` block.
    """
    try:
        from son_of_anton_cli.env_loader import get_secret_source
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            son_of_anton_home = os.path.expanduser(
                os.getenv(
                    "SON_OF_ANTON_HOME", os.path.join(os.path.expanduser("~"), ".son-of-anton")
                )
            )
            candidates = [
                os.path.join(son_of_anton_home, "node", "bin", resolved_command),
                ...
                # image (which the Son of Anton Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Son of Anton MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Son of Anton gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Son of Anton document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`son_of_anton_cli.mcp_config._unwrap_exception_group` with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Son of Anton' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (SON_OF_ANTON_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit SON_OF_ANTON_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Son of Anton' logging (agent.log via son_of_anton_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `son-of-anton setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Son of Anton process (kill -9, crash, force-quit)
        ...
        # ~/.son-of-anton/logs/mcp-stderr.log.
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"son-of-anton-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Son of Anton' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Son of Anton is fully
                ...
                if not self._ready.is_set():
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``son-of-anton mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Son of Anton' pinned ``httpx``, and the two define unrelated
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `son-of-anton mcp login {server_name}` (or delete the tokens "
        f"file under ~/.son-of-anton/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Son of Anton processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_son_of_anton_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local SON_OF_ANTON_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_son_of_anton_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_son_of_anton_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local SON_OF_ANTON_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_son_of_anton_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Son of Anton config file.
    ...
    ``os.environ`` (which includes ``~/.son-of-anton/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Son of Anton' MEDIA: tag convention and out to messaging
            ...
            # Son of Anton' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Son of Anton's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Son of Anton uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Son of Anton registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``son-of-anton tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Son of Anton registry.
