 ...
 * composer with the reply scrolling above it, so Renco can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.rencoDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.rencoDesktop?.hud
...
export const reportHudSession = (sessionId: null | string): void => window.rencoDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.rencoDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
