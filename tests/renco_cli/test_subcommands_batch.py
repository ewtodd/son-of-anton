...
def test_config_get_unset_subcommands_parse():
    """`renco config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="renco")
...
# ── deprecated `renco login` fails gracefully, not with argparse error ────
#
# `renco login` is a removed command; its handler (`login_command` in
# `renco_cli/auth.py`) prints a deprecation notice pointing at `renco auth` /
# `renco model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `renco --help`.
