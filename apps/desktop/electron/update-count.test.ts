...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'renco-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Renco Test')
    git('config', 'user.email', 'renco@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/ewtodd/renco.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/ewtodd/renco/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/ewtodd/renco.git',
