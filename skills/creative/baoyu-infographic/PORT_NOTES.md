...
| Change | Upstream | Son of Anton |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `son-of-anton` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Son of Anton infra |
...
| File operations | Bash commands | Son of Anton file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains Son of Anton-specific adaptations.
