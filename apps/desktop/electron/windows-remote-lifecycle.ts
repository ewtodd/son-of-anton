...
const READY_RE = /^RENCO_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitRencoPath = '') {
  const explicit = psLiteral(explicitRencoPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$rencoHome=$env:RENCO_HOME',
    'if(-not $rencoHome){$rencoHome=Join-Path $env:LOCALAPPDATA "renco"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command renco.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $rencoHome "renco-agent\\venv\\Scripts\\renco.exe")',
    '$candidates+=(Join-Path $HOME "renco-agent\\.venv\\Scripts\\renco.exe")',
    '$renco=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $renco){throw "Renco is not installed on the remote Windows host."}',
    'if($explicit -and $renco -ne $explicit){throw "The configured Renco path is not an executable file."}',
    '$python=Join-Path (Split-Path $renco) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Renco Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;rencoHome=$rencoHome;rencoPath=$renco;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitRencoPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitRencoPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'renco_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.rencoPath === 'string' &&
    typeof lock.rencoHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.rencoPath === runtime.rencoPath &&
    lock.rencoHome === runtime.rencoHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteRencoPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.rencoPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Renco on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.rencoPath = inspection.path
  const rencoVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located renco at ${runtime.rencoPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, rencoPath: runtime.rencoPath })
    )
  } catch (error) {
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForRenco(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Renco SSH"', 'powershell.exe -NoLogo')
