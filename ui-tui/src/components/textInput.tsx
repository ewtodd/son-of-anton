import type { InputEvent, Key } from '@sonofanton/ink'
import * as Ink from '@sonofanton/ink'
 ...
 * but that accepts `key.meta` on macOS — and son-of-anton-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.SON_OF_ANTON_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which son-of-anton-ink decodes as meta+'d'; without this branch it
