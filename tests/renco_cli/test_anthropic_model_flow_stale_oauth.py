"""Tests for Bug #12905 fix — stale OAuth token detection in renco model flow.

Bug 3: `renco model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.renco/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("renco_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from renco_cli.main import _model_flow_anthropic
        cfg = {}
