...
def container_env(tmp_path, monkeypatch):
    """Set up a fake RENCO_HOME with .container-mode file."""
    renco_home = tmp_path / ".renco"
    renco_home.mkdir()
    monkeypatch.setenv("RENCO_HOME", str(renco_home))
    monkeypatch.delenv("RENCO_DEV", raising=False)

    container_mode = renco_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=renco-agent\n"
        "exec_user=renco\n"
        "renco_bin=/data/current-package/bin/renco\n"
    )
    return renco_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("renco_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "renco-agent"
    assert info["exec_user"] == "renco"
    assert info["renco_bin"] == "/data/current-package/bin/renco"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from renco_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "renco"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "renco-agent" in cmd
    assert "/data/current-package/bin/renco" in cmd
    assert "chat" in cmd
