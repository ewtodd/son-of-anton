"""ACP auth helpers — detect and advertise Renco authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "renco-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Renco runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Renco can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Renco.
    ...
    not have Renco provider credentials configured yet, so Renco always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Renco using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Renco provider",
            description=(
                "Open Renco' interactive model/provider setup in a terminal. "
                "Use this when Renco has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
