...
measured in the production session DB) through `renco chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    RENCO_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Renco home with credentials for the
...
""".strip(),
            ...
            env.update({
                ...
                "RENCO_HOME": str(HOME),
                "RENCO_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "renco_cli.main", "chat", "--query", q,
