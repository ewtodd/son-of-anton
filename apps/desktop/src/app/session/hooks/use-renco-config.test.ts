...
import { getRencoConfig } from '@/renco'
...
import { useRencoConfig } from './use-renco-config'

vi.mock('@/renco', () => ({
  getRencoConfig: vi.fn(),
  getRencoConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'renco.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getRencoConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getRencoConfig>>)

describe('useRencoConfig refreshRencoConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Renco' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useRencoConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshRencoConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useRencoConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getRencoConfig>>>()
    vi.mocked(getRencoConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useRencoConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshRencoConfig(true)
    })
    expect(getRencoConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getRencoConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getRencoConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getRencoConfig>>>()
    vi.mocked(getRencoConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshRencoConfig(true)
      refreshC = result.current.refreshRencoConfig(true)
    })
