...
``renco_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``renco_cli.config`` (get_env_value / save_env_value / remove_env_value)
and ``renco_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch
those source modules.
"""
import renco_cli.config as config_mod
import renco_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved, removed, existing):
    ...
    # Manifest writing reaches out to renco_cli.slack_cli + filesystem; stub it.
    import renco_cli.slack_cli as slack_cli_mod
