#!/usr/bin/env python3
"""
SQLite State Store for Son of Anton Agent.
...
from son_of_anton_state_portability import SessionPortabilityMixin
from son_of_anton_state_schema import SessionSchemaMixin
from son_of_anton_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``son-of-anton -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_son_of_anton_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.son-of-anton`` even when a test fixture later
    redirects ``SON_OF_ANTON_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_son_of_anton_home()`` fresh so a runtime
       ``SON_OF_ANTON_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_son_of_anton_home() / "state.db"
...
# ~/.son-of-anton/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects SON_OF_ANTON_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without SON_OF_ANTON_HOME, a stale worktree
# without the re-pin, or a developer shell that exports SON_OF_ANTON_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "SON_OF_ANTON_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.son-of-anton``).  The test conftest injects the pre-sandbox production
#: root here so custom-``SON_OF_ANTON_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Son of Anton root for the guard.

    Deliberately avoids ``Path.home()`` / ``son_of_anton_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``son_of_anton_state``
...
#: redirects ``SON_OF_ANTON_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Son of Anton test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``SON_OF_ANTON_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "SON_OF_ANTON_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``SON_OF_ANTON_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``son-of-anton`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Son of Anton home *root*.
    ...
    under ``~/.son-of-anton/son-of-anton/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``SON_OF_ANTON_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``SON_OF_ANTON_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Son of Anton root {root}). "
                "Tests must run against a temporary SON_OF_ANTON_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "SON_OF_ANTON_HOME. If this test genuinely needs the live "
...
# son_of_anton_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``son-of-anton sessions optimize`` (FTS merge + VACUUM)
    ...
    ``son_of_anton_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``son_of_anton_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``son-of-anton update`` can actually do for this install (#75153).
    """
    try:
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Son of Anton"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Son of Anton-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `son-of-anton doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see son_of_anton_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with son_of_anton_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``son-of-anton doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Son of Anton host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``son-of-anton serve`` backend, interactive CLI sessions,
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``son_of_anton_cli.sqlite_safe_read``).
    ...
    try:
        from son_of_anton_cli.sqlite_safe_read import has_live_connection
    except ImportError:
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Son of Anton home tree (``get_son_of_anton_home()``) — the safe repair scope:
      Son of Anton owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``son_of_anton_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_son_of_anton_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Son of Anton needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (son_of_anton_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_son_of_anton_state.py:600-632)
        ...
        probe_session_id = f"_son_of_anton_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.son-of-anton/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``son-of-anton sessions optimize-storage`` on a capable host).
#
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship son_of_anton_state without son_of_anton_cli). A
    ...
    """
    try:
        from son_of_anton_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug(
            "son_of_anton_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch son_of_anton_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``son_of_anton_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `son-of-anton snapshot list` / "
                "`son-of-anton snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (son-of-anton doctor, dashboards) ──────────
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple son-of-anton processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Son of Anton
    ...
    # `son-of-anton update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `son-of-anton snapshot list` / "
                    f"`son-of-anton snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`son-of-anton db optimize`),
            ...
            # ``son_of_anton_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `son-of-anton update` to rebuild the venv with a "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Son of Anton process held the "
    ...
    #
    # `optimize_fts_storage()` (the `son-of-anton sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`son-of-anton sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``son-of-anton sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``son-of-anton --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: son_of_anton_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``son-of-anton -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``son-of-anton sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``SON_OF_ANTON_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Son of Anton, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Son of Anton session.

        A Son of Anton session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Son of Anton session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `son-of-anton sessions vacuum` runs in a transient
