...
Before the fix, ``renco whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``renco gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    renco = home / ".renco"
    renco.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("RENCO_HOME", str(renco))
...
def _env_value(renco_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from renco_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("renco_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``renco whatsapp`` with an existing paired session and
