---
sidebar_position: 8
title: "Use Voice Mode with Renco"
description: "A practical guide to setting up and using Renco voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Renco
...
- you want Renco sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Renco.
...
## Step 1: make sure normal Renco works first
...
Renco supports both local and cloud speech stacks.
...
### If you use `renco setup`

If you choose NeuTTS in the setup wizard, Renco checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Renco transcribes and responds
...
- asking Renco to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Renco loop.

## Tuning CLI behavior

### Silence threshold

If Renco starts/stops too aggressively, tune:
...
Renco stays a normal chat bot, but can speak replies.
...
- you want Renco to function like a portable research or ops assistant
...
Renco joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Renco detects speech boundaries
- transcripts are posted in the associated text channel
- Renco responds in text and audio
...
1. get text Renco working
2. run `renco setup tts` to enable voice support
