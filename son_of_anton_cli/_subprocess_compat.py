...
Son of Anton is developed on Linux / macOS and tested natively on Windows too.
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_SON_OF_ANTON_GATEWAY_BREAKAWAY"
...
# Electron exits, so the gateway never gets respawned after a `son-of-anton
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Son of Anton' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``son-of-anton update`` on slow-WMI machines
