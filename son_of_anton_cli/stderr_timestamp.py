...
def _is_son_of_anton_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for Son of Anton ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current Son of Anton gateway shapes
...
def _prepare_child_command(
    ...
    ``--external-supervisor`` on the inner ``gateway run`` so ``son-of-anton update``
    ...
    if not _is_launchd_supervised(environ):
        return argv
    if not _is_son_of_anton_gateway_run_argv(argv):
        return argv
