...
* ``son_of_anton_cli.providers.determine_api_mode`` correctly mapped
  ``api.anthropic.com`` to ``anthropic_messages``.
* ``son_of_anton_cli.runtime_provider._detect_api_mode_for_url`` did NOT, so
...
class TestExplicitRuntimeForAnthropic:
    ...
    Exercised whenever the user (or a Son of Anton subcommand) passes an
...
class TestPoolEntryForAnthropic:
    ...
    has added an OAuth credential via ``son-of-anton auth add anthropic
