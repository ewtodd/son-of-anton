"""Renco-managed uv and Python runtime repair.

Renco owns its own uv binary at ``$RENCO_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$RENCO_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Renco
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.renco-runtime/python``. A
...
from renco_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".renco-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where Renco keeps *its* uv binary.

    ``$RENCO_HOME/bin/uv`` on POSIX, ``$RENCO_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Renco-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Renco keeps databases "
            "out of WAL mode on this SQLite build. The next `renco update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``renco update`` runs
    the call site from the *old*, already-imported ``renco_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``renco update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under RENCO_HOME
    caches the last successful self-update time.
    """
    try:
        from renco_constants import get_renco_home

        stamp = get_renco_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``renco update`` so the managed copy stays current.
...
def _reload_renco_constants():
    """Re-execute ``renco_constants`` from disk and return the fresh module.

    ``renco update`` imports ``renco_constants`` from the OLD checkout,
    ...
    return importlib.reload(renco_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Renco generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `renco update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("renco_cli.main")
    ...
    if holders:
        ...
        return True, f"other Renco processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Renco-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``renco update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``renco update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$RENCO_HOME/bin/`` instead of ``~/.local/bin/``.
    """
