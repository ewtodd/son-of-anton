...
- _seed_from_env reads API keys from ~/.son-of-anton/.env when not in os.environ
- _resolve_api_key_provider_secret falls back to credential_pool when env vars are empty
- ~/.son-of-anton/.env takes priority over os.environ for Son of Anton-managed credentials
...
def _make_pconfig(provider_id="deepseek", env_vars=None):
    ...
    """
    from son_of_anton_cli.auth import ProviderConfig
...
@pytest.fixture
def isolated_son_of_anton_home(tmp_path, monkeypatch):
    """Point SON_OF_ANTON_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.son-of-anton/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.son-of-anton/.env, not just os.environ.
    ...
    load_son_of_anton_dotenv, the credential pool must still discover it.
    """

    def test_deepseek_key_from_dotenv_only(self, isolated_son_of_anton_home):
        """Key in .env but not os.environ → _seed_from_env adds a pool entry."""
        _write_env_file(isolated_son_of_anton_home, DEEPSEEK_API_KEY="sk-dotenv-only-12345")
    ...
    def test_empty_dotenv_no_entries(self, isolated_son_of_anton_home):
    ...
    def test_dotenv_wins_over_stale_os_environ(self, isolated_son_of_anton_home, monkeypatch):
        """Regression for #20591: a fresh key rotated into ~/.son-of-anton/.env must
        ...
        """
        _write_env_file(isolated_son_of_anton_home, DEEPSEEK_API_KEY="sk-dotenv-fresh")
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.son-of-anton/.env."""

    def test_key_from_dotenv_only(self, isolated_son_of_anton_home):
        """Key in .env but not os.environ → _resolve returns it with the env var source."""
        _write_env_file(isolated_son_of_anton_home, DEEPSEEK_API_KEY="sk-dotenv-resolve-789")
        assert "DEEPSEEK_API_KEY" not in os.environ
    ...
    def test_dotenv_wins_over_stale_os_environ_on_resolve(
        self, isolated_son_of_anton_home, monkeypatch
    ):
        """Regression for #20591: when both ~/.son-of-anton/.env and os.environ define
        ...
        """
        _write_env_file(isolated_son_of_anton_home, DEEPSEEK_API_KEY="dotenv-fresh-deepseek")
    ...
    def test_get_anthropic_key_prefers_dotenv_over_stale_os_environ(
        ...
        prefer ~/.son-of-anton/.env over a stale shell export. This path resolves
        ...
        """
        _write_env_file(isolated_son_of_anton_home, ANTHROPIC_API_KEY="dotenv-fresh-anthropic")
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_son_of_anton_home):
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_son_of_anton_home):
    ...
    def test_env_var_takes_priority_over_pool(self, isolated_son_of_anton_home, monkeypatch):
    ...
    def test_dotenv_takes_priority_over_pool(self, isolated_son_of_anton_home):
        """Key in .env beats credential pool — pool only fires when both env sources are empty."""
        _write_env_file(isolated_son_of_anton_home, DEEPSEEK_API_KEY="sk-dotenv-priority-xyz")
        assert "DEEPSEEK_API_KEY" not in os.environ
...
class TestAnthropicEnvAuthTypeClassification:
    ...
    def test_oauth_token_classified_as_oauth(self, isolated_son_of_anton_home):
        """sk-ant-oat- token from CLAUDE_CODE_OAUTH_TOKEN → AUTH_TYPE_OAUTH."""
        from agent.credential_pool import AUTH_TYPE_OAUTH
        _write_env_file(isolated_son_of_anton_home, CLAUDE_CODE_OAUTH_TOKEN="sk-ant-oat-fake-12345")
        entry = self._seed("CLAUDE_CODE_OAUTH_TOKEN", "sk-ant-oat-fake-12345")
        assert entry.auth_type == AUTH_TYPE_OAUTH

    def test_admin_key_classified_as_api_key(self, isolated_son_of_anton_home):
        ...
        _write_env_file(isolated_son_of_anton_home, ANTHROPIC_API_KEY="sk-ant-admin-fake-12345")
        entry = self._seed("ANTHROPIC_API_KEY", "sk-ant-admin-fake-12345")
        assert entry.auth_type == AUTH_TYPE_API_KEY

    def test_standard_api_key_classified_as_api_key(self, isolated_son_of_anton_home):
        """sk-ant-api- key → AUTH_TYPE_API_KEY (unchanged behaviour)."""
        from agent.credential_pool import AUTH_TYPE_API_KEY
        _write_env_file(isolated_son_of_anton_home, ANTHROPIC_API_KEY="sk-ant-api-fake-12345")
