...
**three** code paths in Son of Anton that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Son of Anton — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Son of Anton
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Son of Anton is
   refusing my value."
...
son-of-anton config get delegation.max_concurrent_children

# 2. Did Son of Anton' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.son-of-anton/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with son-of-anton on path)
...
appears, **the cap is the model**, not Son of Anton.
