...
// no longer bundles the Renco Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Renco.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Renco'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Renco.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Renco'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default RENCO_HOME for non-sandboxed runs -- matches main.ts's
// resolveRencoHome(). On Windows it's %LOCALAPPDATA%\renco; elsewhere
// it's ~/.renco. The fresh-install sandbox launchFresh() sets its own
// RENCO_HOME and never touches this.
const DEFAULT_RENCO_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'renco')
  }
  return path.join(os.homedir(), '.renco')
})()
const VENV_ROOT = path.join(DEFAULT_RENCO_HOME, 'renco-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'renco-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.RENCO_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Renco-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Renco-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Renco-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Renco-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.RENCO_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.RENCO_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const rencoHome = path.join(sandbox, 'renco-home')
  ...
  env.RENCO_DESKTOP_CWD = cwd
  env.RENCO_DESKTOP_IGNORE_EXISTING = '1'
  env.RENCO_DESKTOP_TEST_MODE = 'fresh-install'
  env.RENCO_DESKTOP_USER_DATA_DIR = userDataDir
  env.RENCO_HOME = rencoHome
  delete env.RENCO_DESKTOP_RENCO
  delete env.RENCO_DESKTOP_RENCO_ROOT
  ...
  console.log(`  RENCO_HOME: ${rencoHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(rencoHome, 'renco-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Renco Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of renco_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'renco-agent', 'renco_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Renco
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + RENCO_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  RENCO_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
