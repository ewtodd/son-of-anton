...
Profile-aware location: ``$RENCO_HOME/logs/dashboard-auth.log``.
...
from ``renco_constants`` or other renco_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$RENCO_HOME/logs/dashboard-auth.log``.

    Uses ``renco_constants.get_renco_home()`` (a leaf module — no import
    ...
    return get_renco_home() / "logs" / "dashboard-auth.log"
