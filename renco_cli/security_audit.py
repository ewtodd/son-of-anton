"""On-demand supply-chain audit for Renco Agent installs.

Scans three surfaces a Renco user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Renco venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.renco/plugins``
...
def _discover_plugins(renco_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.renco/plugins``.
...
def _discover_components(
    *,
    ...
    home = renco_home or Path(get_renco_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `renco security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, renco_home=home
    )
