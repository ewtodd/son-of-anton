...
export function canUseQuickEntry(): boolean {
  return typeof window !== 'undefined' && typeof window.rencoDesktop?.quickEntry?.getSettings === 'function'
}
...
export async function loadQuickEntrySettings(): Promise<void> {
  ...
  try {
    applyStatus(await window.rencoDesktop.quickEntry.getSettings())
  } catch {
...
export async function saveQuickEntrySettings(patch: { enabled?: boolean; shortcut?: string }): Promise<void> {
  ...
  try {
    applyStatus(await window.rencoDesktop.quickEntry.setSettings(patch))
  } catch {
...
export function initQuickEntryBridge(): () => void {
  const api = typeof window === 'undefined' ? undefined : window.rencoDesktop?.quickEntry
