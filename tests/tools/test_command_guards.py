...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects SON_OF_ANTON_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("SON_OF_ANTON_INTERACTIVE", "SON_OF_ANTON_GATEWAY_SESSION", "SON_OF_ANTON_EXEC_ASK", "SON_OF_ANTON_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SON_OF_ANTON_INTERACTIVE"] = "1"
        session_key = os.getenv("SON_OF_ANTON_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SON_OF_ANTON_INTERACTIVE or SON_OF_ANTON_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        os.environ["SON_OF_ANTON_SESSION_KEY"] = session_key
        try:
            check_all_command_guards(command, "local")
        finally:
            os.environ.pop("SON_OF_ANTON_GATEWAY_SESSION", None)
            os.environ.pop("SON_OF_ANTON_EXEC_ASK", None)
            os.environ.pop("SON_OF_ANTON_SESSION_KEY", None)
