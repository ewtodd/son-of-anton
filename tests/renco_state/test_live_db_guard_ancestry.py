...
entirely on the process environment: ``RENCO_HOME`` says *where* to write and
...
# hardcoded ~/.renco — that root is %LOCALAPPDATA%\renco on Windows) and
...
root = renco_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        renco_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``RENCO_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("RENCO_HOME", "RENCO_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no RENCO_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``RENCO_HOME`` is gone, and the env-only guard
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{renco_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert renco_state._process_looks_like_pytest(self._FakeProc(cmdline))

    @pytest.mark.parametrize(
        "cmdline",
        [
            ["renco", "gateway", "start"],
            ["/usr/bin/python", "-m", "renco_cli.main", "sessions", "list"],
            # A path that merely *contains* "pytest" is not a pytest process:
            # tmp paths like /tmp/pytest-of-dev/... show up in real argv.
            ["renco", "run", "--file", "/tmp/pytest-of-dev/test0/input.txt"],
        ],
    )
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not renco_state._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not renco_state._process_looks_like_pytest(_Denied())
