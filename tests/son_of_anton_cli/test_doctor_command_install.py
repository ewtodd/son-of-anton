"""Tests for the Command Installation check in son-of-anton doctor."""
...
import son_of_anton_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal SON_OF_ANTON_HOME + PROJECT_ROOT for doctor tests."""
    ...
    son_of_anton_bin = venv_bin_dir / "son-of-anton"
    son_of_anton_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    son_of_anton_bin.chmod(0o755)
    ...
    return home, project, son_of_anton_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, son_of_anton_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "son-of-anton"
        wrong_target = tmp_path / "wrong_son_of_anton"
        ...
        assert cmd_link.resolve() == son_of_anton_bin.resolve()
