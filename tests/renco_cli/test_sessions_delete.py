...
def test_sessions_delete_accepts_unique_id_prefix(monkeypatch, capsys):
    ...
    monkeypatch.setattr(renco_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        ["renco", "sessions", "delete", "20260315_092437_c9a6", "--yes"],
    )
...
def _run_prune(monkeypatch, capsys, argv_tail, candidates=None, skipped_open=0):
    """Run `renco sessions prune <argv_tail>` against a FakeDB, capturing
    ...
    monkeypatch.setattr(renco_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys, "argv", ["renco", "sessions", "prune", *argv_tail]
    )
...
def test_sessions_prune_bare_keeps_90_day_default(monkeypatch, capsys):
    """A truly bare `renco sessions prune` keeps the implicit 90-day cutoff."""
...
def test_sessions_prune_preview_shows_oldest_newest(monkeypatch, capsys):
    ...
    from renco_cli.session_filters import format_epoch
...
def test_sessions_prune_surfaces_matching_open_sessions(monkeypatch, capsys):
    ...
    assert "renco sessions delete <id>" in out
