"""Tests for tools.wake_word — the "Hey Renco" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey renco"}) == "hey renco"
    assert ww.wake_phrase({}) == "hey renco"
...
def test_looks_like_path():
    assert ww._looks_like_path("models/hey_renco.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "renco_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey renco"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey renco"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_renco": object()}

        def predict(self, frame):
            return {"hey_renco": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_renco.onnx"}}
    )
    assert calls["download"] == [["/models/hey_renco.onnx"]]
    assert eng._labels == ["hey_renco"]


def test_bundled_hey_renco_model_ships_on_disk():
    # The "hey renco" wake word works out of the box only if the model is
