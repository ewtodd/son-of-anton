...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"son-of-anton send: {file_path} is not a text file. --file reads the "
            ...
            print(f"son-of-anton send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"son-of-anton send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"son-of-anton send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"son-of-anton send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"son-of-anton send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `son-of-anton gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.son-of-anton/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_son_of_anton_env() -> None:
    """Populate ``os.environ`` from ``~/.son-of-anton/.env`` AND bridge top-level
    ...
    startup that ``son-of-anton send`` must replicate when invoked standalone:
    ...
    intentionally reimplement the minimum needed here so ``son-of-anton send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.son-of-anton/.env and ~/.son-of-anton/config.yaml into os.environ so the
    ...
    if message is None or not message.strip():
        print(
            "son-of-anton send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `son-of-anton send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Son of Anton "
