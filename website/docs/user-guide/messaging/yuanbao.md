...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Renco Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Renco to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.renco/.env`:
...
All standard Renco commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.renco/logs/gateway.log`
...
4. Consider enabling verbose logging: `renco gateway run -vv`
...
Yuanbao has a maximum message size. Renco automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
renco chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
