...
def renco_home(monkeypatch, tmp_path):
    """Isolate RENCO_HOME so the tests don't pollute the real config.
...
class TestPatchCRLFPreservation:
    def test_patch_on_crlf_file_stays_pure_crlf(self, renco_home, tmp_path):
    ...
    def test_patch_multiline_replacement_on_crlf(self, renco_home, tmp_path):
...
class TestWriteFileCRLFPreservation:
    ...
    def test_overwrite_lf_file_stays_lf(self, renco_home, tmp_path):
