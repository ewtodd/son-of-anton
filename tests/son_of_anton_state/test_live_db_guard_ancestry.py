...
entirely on the process environment: ``SON_OF_ANTON_HOME`` says *where* to write and
...
# hardcoded ~/.son-of-anton — that root is %LOCALAPPDATA%\son-of-anton on Windows) and
...
root = son_of_anton_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        son_of_anton_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``SON_OF_ANTON_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("SON_OF_ANTON_HOME", "SON_OF_ANTON_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no SON_OF_ANTON_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``SON_OF_ANTON_HOME`` is gone, and the env-only guard
    ...
    def test_child_inheriting_pytest_env_still_refuses_production_db(self):
        ...
        env = dict(os.environ)
        env.pop("SON_OF_ANTON_HOME", None)
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{son_of_anton_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert son_of_anton_state._process_looks_like_pytest(self._FakeProc(cmdline))

    @pytest.mark.parametrize(
        "cmdline",
        [
            ["son-of-anton", "gateway", "start"],
            ["/usr/bin/python", "-m", "son_of_anton_cli.main", "sessions", "list"],
            # A path that merely *contains* "pytest" is not a pytest process:
            # tmp paths like /tmp/pytest-of-dev/... show up in real argv.
            ["son-of-anton", "run", "--file", "/tmp/pytest-of-dev/test0/input.txt"],
        ],
    )
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not son_of_anton_state._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not son_of_anton_state._process_looks_like_pytest(_Denied())
