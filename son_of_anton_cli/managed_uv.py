...
The Python backing the install is different: it is shared by every Son of Anton
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.son-of-anton-runtime/python``. A
...
from son_of_anton_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".son-of-anton-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Son of Anton-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Son of Anton keeps databases "
            "out of WAL mode on this SQLite build. The next `son-of-anton update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``son-of-anton update`` runs
    the call site from the *old*, already-imported ``son_of_anton_cli.main`` against
...
def _reload_son_of_anton_constants():
    """Re-execute ``son_of_anton_constants`` from disk and return the fresh module.

    ``son-of-anton update`` imports ``son_of_anton_constants`` from the OLD checkout,
    ...
    return importlib.reload(son_of_anton_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Son of Anton generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `son-of-anton update` with no path to a fixed
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Son of Anton-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``son-of-anton update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``son-of-anton update`` repairs the runtime (issue class:
...
def repair_vulnerable_runtime(
    ...
    *,
    ...
    try:
        ...
        print(
            "  ⚠ Son of Anton venv links SQLite "
