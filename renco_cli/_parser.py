"""
Top-level argparse construction for the renco CLI.
...
# argparse runs (it sets ``RENCO_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``renco_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    renco                        Start interactive chat
    renco chat -q "Hello"        Single query mode
    renco --tui                  Launch the modern TUI (or set display.interface: tui)
    renco --cli                  Force the classic REPL (overrides display.interface: tui)
    renco -c                     Resume the most recent session
    renco -c "my project"        Resume a session by name (latest in lineage)
    renco --resume <session_id>  Resume a specific session by ID
    renco --resume latest        Resume the most recent session (same as -c)
    renco --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    renco setup                  Run setup wizard
    renco logout                 Clear stored authentication
    renco auth add <provider>    Add a pooled credential
    renco auth list              List pooled credentials
    renco auth remove <p> <t>    Remove pooled credential by index, id, or label
    renco auth reset <provider>  Clear exhaustion status for a provider
    renco model                  Select default model
    renco fallback [list]        Show fallback provider chain
    renco fallback add           Add a fallback provider (same picker as `renco model`)
    renco fallback remove        Remove a fallback provider from the chain
    renco config                 View configuration
    renco config edit            Edit config in $EDITOR
    renco config set model gpt-4 Set a config value
    renco gateway                Run messaging gateway
    renco -s renco-agent-dev,github-auth
    renco -w                     Start in isolated git worktree
    renco gateway install        Install gateway background service
    renco sessions list          List past sessions
    renco sessions browse        Interactive session picker
    renco sessions rename ID T   Rename/title a session
    renco logs                   View agent.log (last 50 lines)
    renco logs -f                Follow agent.log in real time
    renco logs errors            View errors.log
    renco logs --since 1h        Lines from the last hour
    renco debug share             Upload debug report for support
    renco console                Open the safe Renco command console
    renco update                 Update to latest version
    renco dashboard              Start web UI dashboard (port 9119)
    renco dashboard --stop       Stop running dashboard processes
    renco dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="renco",
        description="Renco Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `renco chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via RENCO_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `renco setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to RENCO_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.renco/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Renco Agent",
    )
    ...
    # top-level parser: when the user writes `renco -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/renco_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also RENCO_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.renco/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Renco itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
