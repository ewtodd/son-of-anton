...
def renco_home(tmp_path, monkeypatch):
    """Point RENCO_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.renco/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, renco_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, renco_home, windows_default_encoding):
        ...
        assert not (renco_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, renco_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, renco_home):
        auth_path = renco_home / "auth.json"
...
# --- sibling readers of the same ~/.renco/auth.json in other modules -------
#
# _load_auth_store lives in renco_cli/auth.py, but several other modules read
# the same ~/.renco/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, renco_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(renco_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nous_provider_reads_non_ascii_store(self, renco_home, windows_default_encoding, monkeypatch):
        ...
        # RENCO_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", renco_home / "auth.json")
    ...
    def test_read_shared_nous_state_reads_non_ascii_store(
        ...
        """renco_cli.auth._read_shared_nous_state must read a non-ASCII store.
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, renco_home, monkeypatch, windows_default_encoding
    ):
        """renco_cli.main._has_any_provider_configured reads auth.json.
        ...
        # renco_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_nous_state(
        self, renco_home, windows_default_encoding
    ):
