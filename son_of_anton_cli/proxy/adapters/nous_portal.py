...
Reads the user's Nous OAuth state from ``~/.son-of-anton/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `son-of-anton auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `son-of-anton auth add nous` to re-authenticate."
                )
    ...
    # to son_of_anton_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
