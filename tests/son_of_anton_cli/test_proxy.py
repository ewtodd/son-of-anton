"""Tests for the `son-of-anton proxy` subcommand and its upstream adapters."""
...
from son_of_anton_cli.proxy.adapters import ADAPTERS, get_adapter
from son_of_anton_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from son_of_anton_cli.proxy.adapters.nous_portal import NousPortalAdapter
from son_of_anton_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(son_of_anton_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic SON_OF_ANTON_HOME."""
...
def test_nous_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "son_of_anton_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic SON_OF_ANTON_HOME."""
    auth_path = son_of_anton_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("son_of_anton_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from son_of_anton_cli.proxy.server import create_app  # noqa: E402
