...
title: "Desktop Plugin SDK (@renco/plugin-sdk)"
description: "Extend the native Renco Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Renco Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `RencoPlugin`.
It imports one module — `@renco/plugin-sdk` — and gets everything: the app's
...
`$RENCO_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Renco. This page is the **native
desktop app** (`renco desktop`) SDK — the `@renco/plugin-sdk` module and
`$RENCO_HOME/desktop-plugins/`. The **web dashboard** (`renco dashboard`) has
its own, unrelated plugin system on `window.__RENCO_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Renco Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$RENCO_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$RENCO_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `RencoPlugin` contract, appear in **Settings → Plugins**,
...
Create `$RENCO_HOME/desktop-plugins/hello/plugin.js` (that's `~/.renco/...`
by default, or `~/.renco/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.renco/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@renco/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Renco' }),
      jsx('div', {
...
The only importable specifiers are `@renco/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `RencoPlugin`:

```ts
interface RencoPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `renco.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@renco/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@renco/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@renco/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@renco/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@renco/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@renco/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@renco/plugin-sdk'
...
`window.renco.send('get-price eth')` (or a declarative
`<button data-renco-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@renco/plugin-sdk'
...
for requests; `targetProfile` is the backend Renco profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Renco
...
ctx.os.notify({
  ...
  // Body click → focus Renco + navigate. Same vocabulary as OS deep links:
  activate: 'renco://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'renco://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `renco://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `renco://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@renco/plugin-sdk'
...
```javascript
import { queryClient } from '@renco/plugin-sdk'
...
a package into `~/.renco/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.renco/plugins` is not reachable as a filesystem —
...
the `renco://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Renco plugin and declare it in a
`manifest.json`:
...
renco-agent codebase directly (`renco_state`, `renco_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.renco/`)
...
(`renco.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `RencoPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@renco/plugin-sdk` alias.
2. It's still lint-fenced to `@renco/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@renco/plugin-sdk`, `react`,
...
| Plugin contract | `RencoPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `RencoOpenTarget`, `Contribution` |
...
### Agents: the `renco-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`renco-desktop-plugins`** skill — it carries the same contract as this page in
...
`$RENCO_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `renco logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@renco/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.renco/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.renco/logs/errors.log` for
