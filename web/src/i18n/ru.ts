...
export const ru: Translations = {
  ...
  status: {
    ...
    stopped: "Остановлено",
    updateRenco: "Обновить Renco",
    updatingRenco: "Обновление Renco…",
  ...
  pluginsPage: {
    ...
    headline:
      "Поиск, установка, включение и обновление плагинов Renco (аналог `renco plugins`).",
    ...
    removeConfirm: "Удалить этот плагин из ~/.renco/plugins/?",
    removeHint: "Удалять можно только плагины, установленные пользователем в ~/.renco/plugins.",
  ...
  skills: {
    ...
    filters: "Фильтры",
    noSkills: "Навыки не найдены. Навыки загружаются из ~/.renco/skills/",
  ...
  env: {
    ...
    customHint: "Произвольные переменные окружения, сохранённые в вашем .env, которые Renco не распознаёт. Используйте их для внедрения переменных окружения для навыков, серверов MCP или собственных инструментов.",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Коллекционные значки Renco, полученные на основе реальной истории сессий. Известные, но ещё не полученные достижения отображаются как «Обнаруженные»; «Секретные» достижения остаются скрытыми до появления первого подходящего поведения.",
      scan_subtitle:
        "Анализ истории сессий Renco. Первое сканирование может занять 5–10 секунд при большой истории.",
    },
    actions: {
    ...
    stats: {
      ...
      latest: "Последнее",
      latest_hint_empty: "запускайте Renco чаще",
      none_yet: "Пока нет",
    },
    state: {
      unlocked: "Разблокировано",
    ...
    guide: {
      ...
      secret_body:
        "Секретные достижения скрывают свой точный триггер. Как только Renco обнаруживает связанный сигнал, карточка становится «Обнаруженной» и показывает требование.",
      scan_status_header: "Статус сканирования",
      scan_status_body:
        "Renco сканирует локальную историю один раз, затем карточки появятся автоматически. Если это занимает несколько секунд — ничего не зависло.",
