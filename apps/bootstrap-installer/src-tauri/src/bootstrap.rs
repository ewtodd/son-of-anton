...
//! RENCO_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for RENCO_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub renco_home: Option<String>,
}
...
/// Spawn the locally-built Renco desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_renco_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_renco_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Renco desktop at {}. The desktop build step \
             may have been skipped or failed. Run `renco desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Renco desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Renco
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_renco_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Renco.exe"),
            ("win-arm64-unpacked", "Renco.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Renco.app/Contents/MacOS", "Renco"),
            ("mac-arm64/Renco.app/Contents/MacOS", "Renco"),
        ]
    } else {
        &[("linux-unpacked", "renco")]
    };
...
pub(crate) fn resolve_renco_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_renco_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Renco.app/Contents/MacOS/Renco -> .../Renco.app
...
/// path so a bare re-open just opens Renco instead of re-running setup.
pub(crate) fn renco_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".renco-bootstrap-complete").exists()
        && resolve_renco_desktop_exe(install_root).is_some()
}
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    // writeFileAtomic(). renco_is_installed() only checks existence, so a
    // partial direct write would incorrectly enable the launcher fast path.
    let tmp_path = install_root.join(".renco-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_renco_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Renco desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_renco_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::renco_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $RencoHome which Stage-Repository clones
    // the repo INTO at $RencoHome\renco-agent. Mirrors renco_constants.
    let renco_home = args
        .renco_home
        .clone()
        .unwrap_or_else(|| crate::paths::renco_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&renco_home).join("renco-agent");
    ...
    // Copy ourselves to RENCO_HOME/renco-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_renco_home() {
        tracing::warn!(?err, "failed to copy installer into RENCO_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    renco_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, renco_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Renco"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Renco.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Renco.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("renco");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Renco.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_renco_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_renco_desktop_app(&root)
    ...
    #[test]
    fn resolve_renco_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_renco_desktop_app(&root).is_none(),
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".renco-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".renco-bootstrap-complete");
        let tmp_path = root.join(".renco-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    #[test]
    fn renco_is_installed_treats_marker_existence_as_sufficient() {
        ...
        make_release_tree(&root);
        std::fs::write(root.join(".renco-bootstrap-complete"), b"").unwrap();
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".renco-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".renco-bootstrap-complete.tmp").exists(),
