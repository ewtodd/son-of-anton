...
const SKIN_KEY = 'renco-desktop-theme-v2'
const MODE_KEY = 'renco-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'renco-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'renco-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'renco-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.rencoTheme = skinName
  root.dataset.rencoMode = rendered
  ...
  window.rencoDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('renco-boot-background', chromeBg)
    window.localStorage.setItem('renco-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.rencoThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.rencoDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Renco authoring/activating a skin from a
