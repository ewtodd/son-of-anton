...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("SON_OF_ANTON_STREAM_STALE_GIVEUP", 5)
...
def _resolve_direct_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    ``SON_OF_ANTON_API_CALL_STALE_TIMEOUT`` → reasoning-model floor → context-size
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        for msg in messages:
            ...
            # substitute it before dropping the key (Son of Anton bookkeeping,
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        _base_timeout = (
            ...
            else env_float("SON_OF_ANTON_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # SON_OF_ANTON_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("SON_OF_ANTON_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode SON_OF_ANTON_STREAM_READ_TIMEOUT.
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # already-received tool-call chunk back to Son of Anton. Preserve only
        ...
        if agent.provider == "moa":
            # Son of Anton interrupts the managed stream; Relay retains sole
        ...
        # completed response. Relay records that attempt while Son of Anton preserves
    ...
    def _call():
        ...
        _max_stream_retries = env_int("SON_OF_ANTON_STREAM_RETRIES", 2)
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("SON_OF_ANTON_STREAM_STALE_TIMEOUT", 180.0)
    ...
    # unless the user explicitly set SON_OF_ANTON_STREAM_STALE_TIMEOUT; override the
    ...
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        ...
        try:
            ...
            pass
        _stream_stale_timeout = env_float("SON_OF_ANTON_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
        logger.debug(
