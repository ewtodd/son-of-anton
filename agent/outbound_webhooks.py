...
(``gateway/platforms/webhook.py``): inbound wakes Son of Anton when the world
changes; outbound tells the world when Son of Anton does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Son of Anton-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``SON_OF_ANTON_SAFE_MODE=1`` still skips registration, matching
...
Config schema (``~/.son-of-anton/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/son-of-anton-events
          events: [on_session_end, subagent_stop]
          # secret literal (discouraged) or env var name (preferred):
          secret_env: SON_OF_ANTON_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `son-of-anton hooks list`
...
Headers::
    ...
    User-Agent:              Son of Anton-Agent-Outbound-Webhook
    X-Son of Anton-Event:          <hook event name>
    X-Son of Anton-Delivery:       <delivery_id>
    X-Son of Anton-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("SON_OF_ANTON_SAFE_MODE"):
        logger.info("SON_OF_ANTON_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Son of Anton-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Son of Anton-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
