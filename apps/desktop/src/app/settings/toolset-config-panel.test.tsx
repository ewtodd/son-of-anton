...
import type { ToolsetConfig } from '@/types/renco'
...
const getRencoConfigSchema = vi.fn()
...
vi.mock('@/renco', () => ({
  ...
  getRencoConfigSchema: () => getRencoConfigSchema(),
...
beforeEach(() => {
  ...
  getRencoConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getRencoConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveRencoConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveRencoConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
