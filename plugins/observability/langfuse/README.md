...
This plugin ships bundled with Renco but is **opt-in** — it only loads when
...
renco tools  # → Langfuse Observability
...
Set these in `~/.renco/.env` (or via `renco tools`):
...
renco chat -q "hello"              # then check Langfuse for a "Renco turn" trace
```

Generation observations include the Renco system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `RENCO_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
RENCO_LANGFUSE_ENV=production       # environment tag
RENCO_LANGFUSE_RELEASE=v1.0.0       # release tag
RENCO_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
RENCO_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
RENCO_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
RENCO_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`RENCO_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
renco plugins disable observability/langfuse
```
