...
# that run *before* son_of_anton_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("SON_OF_ANTON_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".son-of-anton", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``SON_OF_ANTON_TUI=1``, then a real-TTY gate (a
    ...
    pipes run ``son-of-anton … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("SON_OF_ANTON_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``SON_OF_ANTON_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("SON_OF_ANTON_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`son-of-anton --tui … >log`, CI capture):
...
from son_of_anton_cli.subcommands.sync import build_sync_parser
from son_of_anton_cli.subcommands.gateway import build_gateway_parser
from son_of_anton_cli.subcommands.profile import build_profile_parser
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any son-of-anton module import.
#
# Many modules cache SON_OF_ANTON_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SON_OF_ANTON_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.son-of-anton/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SON_OF_ANTON_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo son-of-anton -p <name>` against the invoking user's home.
        ...
        candidate = home / ".son-of-anton" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`son-of-anton chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors son_of_anton_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SON_OF_ANTON_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.son-of-anton/profiles/coder or
    # /opt/data/profiles/coder).  If SON_OF_ANTON_HOME points to the son-of-anton root
    # instead (e.g. systemd hardcodes SON_OF_ANTON_HOME=/root/.son-of-anton), we must
    # still read active_profile — the user may have switched profiles via
    # `son-of-anton profile use` and the gateway should honour that choice.
    # See issue #22502.
    son_of_anton_home_env = os.environ.get("SON_OF_ANTON_HOME", "")
    if profile_name is None and son_of_anton_home_env:
        if Path(son_of_anton_home_env).parent.name == "profiles":
            return
    ...
    if profile_name is None and not os.environ.get("SON_OF_ANTON_S6_SUPERVISED_CHILD"):
        try:
            ...
            active_path = get_default_son_of_anton_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set SON_OF_ANTON_HOME
    if profile_name is not None:
        try:
            from son_of_anton_cli.profiles import resolve_profile_env

            son_of_anton_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            son_of_anton_home = _resolve_sudo_user_profile_env(profile_name)
            ...
            # A bug in profiles.py must NEVER prevent son-of-anton from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["SON_OF_ANTON_HOME"] = son_of_anton_home
...
def _has_any_provider_configured() -> bool:
    ...
    # Only count these if Son of Anton has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Son of Anton to use their tokens.
    if _has_son_of_anton_config:
        try:
...
def _print_tui_exit_summary(
    ...
    print(f"  son-of-anton --tui --resume {target}")
    if title:
        print(f'  son-of-anton --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @sonofanton/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@son-of-anton" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/son-of-anton-ink/package.json",
    "packages/son-of-anton-ink/index.js",
    "packages/son-of-anton-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``SON_OF_ANTON_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("SON_OF_ANTON_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``SON_OF_ANTON_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SON_OF_ANTON_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SON_OF_ANTON_HOME": son_of_anton_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(son_of_anton_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(son_of_anton_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if son_of_anton_cli_dir is None:
        son_of_anton_cli_dir = Path(__file__).parent
    bundled = son_of_anton_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``son-of-anton update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("SON_OF_ANTON_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Son of Anton checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `son-of-anton update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Son of Anton checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `son-of-anton --tui`\n"
        "If the checkout is still inconsistent, run `son-of-anton update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (SON_OF_ANTON_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SON_OF_ANTON_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $SON_OF_ANTON_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Son of Anton installed,
        ...
        if not path and bin == "node":
            try:
                from son_of_anton_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("SON_OF_ANTON_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with SON_OF_ANTON_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset SON_OF_ANTON_TUI_DIR (e.g. `unset SON_OF_ANTON_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # son_of_anton_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_son_of_anton_node_path()),
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Son of Anton-managed
    ...
    if tui_dev:
        # Keep the local @sonofanton/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @sonofanton/ink resolves through
        # packages/son-of-anton-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "son-of-anton-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from son_of_anton_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("SON_OF_ANTON_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["SON_OF_ANTON_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("SON_OF_ANTON_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["SON_OF_ANTON_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("SON_OF_ANTON_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["SON_OF_ANTON_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["SON_OF_ANTON_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a son-of-anton process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from son_of_anton_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="son-of-anton-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["SON_OF_ANTON_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SON_OF_ANTON_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["SON_OF_ANTON_MODEL"] = model
        env["SON_OF_ANTON_INFERENCE_MODEL"] = model
    if provider:
        env["SON_OF_ANTON_TUI_PROVIDER"] = provider
        env["SON_OF_ANTON_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SON_OF_ANTON_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SON_OF_ANTON_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SON_OF_ANTON_TUI_SKILLS"] = value
    if query:
        env["SON_OF_ANTON_TUI_QUERY"] = query
    if image:
        env["SON_OF_ANTON_TUI_IMAGE"] = image
    if checkpoints:
        env["SON_OF_ANTON_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SON_OF_ANTON_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SON_OF_ANTON_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SON_OF_ANTON_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SON_OF_ANTON_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SON_OF_ANTON_ACCEPT_HOOKS"] = "1"
    ...
    # SON_OF_ANTON_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `son-of-anton --tui` try to
    ...
    # still set SON_OF_ANTON_TUI_RESUME themselves.
    env.pop("SON_OF_ANTON_TUI_RESUME", None)
    if resume_session_id:
        env["SON_OF_ANTON_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `son-of-anton update` so
...
def _resolve_use_tui(args) -> bool:
    ...
    jobs and pipelines run ``son-of-anton … chat -q`` with stdout on a pipe;
    ...
    *explicitly* passes ``--tui`` still gets the informative bail-out.
    ...
    try:
        ...
        return False
    if os.environ.get("SON_OF_ANTON_TUI") == "1":
        return True
    try:
        from son_of_anton_cli.config import load_config
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Son of Anton will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        ...
        print("    4. Re-run:               son-of-anton model")
        print()
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.son-of-anton/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `son-of-anton sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set SON_OF_ANTON_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "SON_OF_ANTON_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="son-of-anton sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="son-of-anton sync now")
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `son-of-anton profile` — show current profile status
        from son_of_anton_cli.profiles import format_profile_label

        profile_name = get_active_profile_name()
        dhh = display_son_of_anton_home()
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → son-of-anton -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.son-of-anton)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  son-of-anton profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     son-of-anton -p {name} chat")
                else:
        ...
        from son_of_anton_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from son_of_anton_constants import get_son_of_anton_home as _hh
        ...
        from son_of_anton_cli import profile_describer as _pd
        ...
        from son_of_anton_cli.profiles import profile_exists, validate_alias_name
        ...
        from son_of_anton_cli.profiles import normalize_profile_name, rename_profile
        ...
        from son_of_anton_cli.profiles import export_profile
        ...
        from son_of_anton_cli.profiles import import_profile
        ...
        import tempfile
        from son_of_anton_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="son_of_anton_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  son-of-anton -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      son-of-anton -p {plan.manifest.name} chat")
        ...
        from son_of_anton_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `son-of-anton profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"son-of-anton -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from son_of_anton_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("son_of_anton_requires"):
            print(f"Requires:     Son of Anton {data['son_of_anton_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from son_of_anton_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.son_of_anton_requires:
        print(f"  Requires: Son of Anton {mf.son_of_anton_requires}")
...
def cmd_gateway_enroll(args):
    ...
    from son_of_anton_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("SON_OF_ANTON_TUI") == "1")
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `son-of-anton --tui` is the hot path on phones. The full parser setup imports
...
def main():
    ...
    # =========================================================================
    # profile command  (parser built in son_of_anton_cli/subcommands/profile.py)
    # =========================================================================
