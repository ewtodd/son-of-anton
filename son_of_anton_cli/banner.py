...
Pure display functions with no SonOfAntonCLI state dependency.
"""
import json
import logging
import os
...
# (e.g. nix-built son-of-anton — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/ewtodd/son-of-anton.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/son-of-anton"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/nousresearch/son-of-anton/"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "son-of-anton-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `son-of-anton update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Son of Anton update is available.

    Two paths: if ``SON_OF_ANTON_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = son_of_anton_home / ".update_check"
    embedded_rev = os.environ.get("SON_OF_ANTON_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # SON_OF_ANTON_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/son-of-anton/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from son_of_anton_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $SON_OF_ANTON_HOME/son-of-anton/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = son_of_anton_home / "son-of-anton"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Son of Anton git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$SON_OF_ANTON_HOME/son-of-anton/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``son_of_anton_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from son_of_anton_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/ewtodd/son-of-anton/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Son of Anton checkout. Cached per-process. Release URL always points at the
    canonical ewtodd/son-of-anton repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Son of Anton Agent v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from son_of_anton_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
...
def _banner_snapshot_path() -> Path:
    return get_son_of_anton_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_son_of_anton_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if not (model or "").strip() or (model or "").strip().lower() == "unknown":
        ...
        left_lines.append(
            ...
            f"[{dim}]— run /model or son-of-anton setup"
        )
    else:
    ...
    try:
        from son_of_anton_cli.codex_runtime_switch import get_current_runtime
        from son_of_anton_cli.config import load_config as _load_cfg
        if get_current_runtime(_load_cfg()) == "codex_app_server":
            right_lines.append(
                f"[bold {accent}]Runtime: [{text}]codex app-server "
                f"[{dim}](terminal/file ops/MCP run inside codex)"
            )
    except Exception:
        pass
