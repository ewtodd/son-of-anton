...
# xAI's public API is OpenAI-compatible for the endpoints Son of Anton commonly
# uses. The Responses endpoint is included because Son of Anton' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Son of Anton-managed OAuth credentials."""

    auth_hint = "son-of-anton auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`son-of-anton auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`son-of-anton auth reset xai-oauth` or re-authenticate with "
                    "`son-of-anton auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `son-of-anton auth add xai-oauth --type oauth`."
            )
