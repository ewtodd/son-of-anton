...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updateRenco } from '@/renco'
import type { ActionStatusResponse, AnalyticsResponse, StatusResponse } from '@/renco'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `renco logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateRenco()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.rencoActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
