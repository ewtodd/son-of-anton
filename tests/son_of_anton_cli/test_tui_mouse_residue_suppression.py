...
The Python launcher (`son-of-anton --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Son of Anton session
...
from son_of_anton_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["son-of-anton", "--tui"])
        monkeypatch.delenv("SON_OF_ANTON_TUI", raising=False)
        monkeypatch.setenv("SON_OF_ANTON_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["son-of-anton", "--tui"])
        monkeypatch.delenv("SON_OF_ANTON_TUI", raising=False)
        monkeypatch.delenv("SON_OF_ANTON_TUI_NO_EARLY_DISABLE", raising=False)
