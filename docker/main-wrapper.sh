#!/bin/sh
# shellcheck shell=sh
# /opt/renco/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching RENCO_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `renco` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `renco <args>` (subcommand passthrough)
#
# Drop to renco via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${RENCO_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${RENCO_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export RENCO_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset RENCO_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid renco "$@"; exec "$@"; }
...
# an arbitrary non-root, non-renco UID, the bootstrap was skipped and the
# baked image dirs (owned by the renco build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u renco)" ]; then
    cat >&2 <<EOF
[renco] ERROR: container started with --user $cur_uid (an arbitrary, non-renco UID) — not supported.
...
# to the renco user's home before dropping privileges so libraries that
...
_renco_orig_cwd="${RENCO_ORIG_CWD:-$PWD}"
...
# the user's command so `renco chat` starts in the Docker -w
