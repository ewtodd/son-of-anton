...
import type { SkillInfo, ToolsetInfo } from '@/types/renco'
...
export function SkillsView({
  ...
  const registryBridge = window.rencoDesktop?.connections
  const rosterBridge = window.rencoDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Renco (default)' : p.name,
      value: p.name
    }))
