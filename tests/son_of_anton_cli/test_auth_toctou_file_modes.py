"""Regression tests for TOCTOU-safe credential file writers in ``son_of_anton_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.son-of-anton/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # home" reference — which currently tracks SON_OF_ANTON_HOME via
    # get_default_son_of_anton_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("SON_OF_ANTON_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from son_of_anton_cli import auth as auth_mod
