...
``~/.son-of-anton/logs/subagent-timeout-<sid>-<ts>.log``. This gives users a
...
class TestDumpSubagentTimeoutDiagnostic:
    ...
    def test_writes_log_with_expected_sections(self, son_of_anton_home):
        ...
        p = Path(path)
        assert p.is_file()
        # File lives under SON_OF_ANTON_HOME/logs/
        assert p.parent == son_of_anton_home / "logs"
    ...
    def test_returns_none_on_unwritable_logs_dir(self, tmp_path, monkeypatch):
        # Point SON_OF_ANTON_HOME at an unwritable path so logs/ can't be created
        # (simulates permission-denied). Helper must not raise.
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        bogus = tmp_path / "does-not-exist" / ".son-of-anton"
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(bogus))
        child = _StubChild()
...
class TestRunSingleChildTimeoutDump:
    ...
    def test_zero_api_calls_writes_dump_and_surfaces_path(self, son_of_anton_home, monkeypatch):
        ...
        assert dump_path.parent == son_of_anton_home / "logs"
    ...
    def test_non_timeout_error_has_null_timeout_metadata(self, son_of_anton_home, monkeypatch):
