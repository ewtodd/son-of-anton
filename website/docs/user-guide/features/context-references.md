...
Type `@` followed by a reference to inject content directly into your message. Renco expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Renco env: `$RENCO_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$RENCO_HOME/skills/.hub/`
