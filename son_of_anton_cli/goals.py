"""Persistent session goals — the Ralph loop for Son of Anton.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Son of Anton feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.SonOfAntonCLI`` or the gateway
...
# creation, and the first son_of_anton_cli.config import (journal-mode
...
def _bootstrap_session_db(home: str, done: threading.Event) -> None:
    ...
    try:
        ...
        # SON_OF_ANTON_HOME). It then caches the wrong profile's DB under this
        # profile's key.
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current SON_OF_ANTON_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``son_of_anton_home`` path so profile switches still pick up the right DB.
    ...
    try:
        ...
        home = str(get_son_of_anton_home())
    except Exception as exc:  # pragma: no cover
...
def run_kanban_goal_loop(
    *,
    ...
    (``son-of-anton -p <profile> chat -q "work kanban task <id>"``). The worker's
