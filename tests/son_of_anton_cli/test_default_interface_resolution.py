...
`son-of-anton` launches the classic prompt_toolkit REPL by default, but users can
flip ``display.interface: tui`` in config.yaml to make the modern Ink TUI the
default for bare ``son-of-anton`` / ``son-of-anton chat``. Explicit flags always win:

    --cli                forces the classic REPL (highest precedence)
    --tui                forces the TUI
    (no TTY)             forces the classic REPL — ambient prefs don't apply
    SON_OF_ANTON_TUI=1         the env default
...
``son-of-anton … chat -q`` on a pipe, and the TUI's no-TTY bail-out exits 0
...
These tests pin that precedence at every layer that makes the decision:
  ...
  * ``_wants_tui_early(argv)``  — the dependency-free early resolver used by
    ...
    ``son_of_anton_cli.config`` are importable.
...
class TestWantsTuiEarly:
    ...
    def test_missing_config_defaults_to_cli(self, tmp_path, monkeypatch):
        # SON_OF_ANTON_HOME points at an empty dir — no config.yaml.
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_path))
        monkeypatch.setattr(m, "_EARLY_INTERFACE_CACHE", None)
        assert m._wants_tui_early([]) is False
...
class TestParserFlags:
    ...
    def test_cli_and_tui_are_relaunch_inherited(self):
        from son_of_anton_cli.relaunch import _INHERITED_FLAGS_TABLE
