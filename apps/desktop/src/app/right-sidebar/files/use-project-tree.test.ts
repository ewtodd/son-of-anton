...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { rencoDesktop: { readDir: typeof readDir } }).rencoDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): RencoReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { rencoDesktop: unknown }).rencoDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: RencoReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<RencoReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { rencoDesktop: unknown }).rencoDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.rencoDesktop is missing', async () => {
    delete (window as unknown as { rencoDesktop?: unknown }).rencoDesktop
