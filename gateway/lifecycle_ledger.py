...
``<RENCO_HOME>/state/gateway.lifecycle.json``:
...
def _process_renco_home() -> Path:
    """RENCO_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<RENCO_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``renco_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
