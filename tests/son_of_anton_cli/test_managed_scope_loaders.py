...
son_of_anton_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # son_of_anton_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("SON_OF_ANTON_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import son_of_anton_time

    assert son_of_anton_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (SON_OF_ANTON_TIMEZONE, SON_OF_ANTON_REDACT_SECRETS, SON_OF_ANTON_MAX_ITERATIONS,
    ...
    # writing SON_OF_ANTON_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
