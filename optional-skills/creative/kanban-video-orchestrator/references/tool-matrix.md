...
Maps each role archetype to the Renco skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public renco-agent
...
## Renco skills relevant to video production

### Visual / rendering skills (`renco-agent/skills/creative/`)
...
### Design / pre-production skills (`renco-agent/skills/creative/`)
...
### Audio / media skills (`renco-agent/skills/creative/` + `skills/media/`)
...
These are **not** Renco skills but external CLIs / APIs that profiles invoke.
...
## Built-in Renco tools for media review

These are native Renco tools — not invoked via terminal but through their own
...
key is present in `${RENCO_HOME:-~/.renco}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Renco profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${RENCO_HOME:-~/.renco}/.env` → environment variable.
...
When a new Renco-public video skill ships:
