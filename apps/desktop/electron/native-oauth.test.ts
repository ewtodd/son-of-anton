...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/renco', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/renco/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/renco'), 'https://gw.example.com/renco/auth/native/refresh')
})
