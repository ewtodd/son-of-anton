"""CLI handlers for ``son-of-anton migrate ...``.

Currently exposes only ``son-of-anton migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``son-of-anton migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: son-of-anton migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `son-of-anton migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `son-of-anton doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
