"""Tests for the ``son-of-anton send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``son_of_anton_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from son_of_anton_cli import send_cmd
...
def test_load_son_of_anton_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``son-of-anton send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``son-of-anton config set``) lives
    ...
    (son_of_anton_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_son_of_anton_home() to re-resolve under the patched env.
    from importlib import reload

    import son_of_anton_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_son_of_anton_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_son_of_anton_env_utf8_bom_preserves_first_key(tmp_path, monkeypatch):
    ...
    `son-of-anton send` failed to authenticate.
    """
    import os
...
def test_load_son_of_anton_env_bomless_utf8_still_loads(tmp_path, monkeypatch):
    """BOM-less UTF-8 .env files must keep loading after the utf-8-sig switch."""
    import os

    son_of_anton_home = tmp_path / ".son-of-anton"
    son_of_anton_home.mkdir()
    (son_of_anton_home / ".env").write_bytes(b"SEND_PLAIN_TOKEN=plain-val\n")
...
def test_load_son_of_anton_env_latin1_fallback_still_loads(tmp_path, monkeypatch):
...
def test_load_son_of_anton_env_latin1_fallback_overrides_shell(tmp_path, monkeypatch):
    ...
    (son_of_anton_home / ".env").write_bytes(b"SEND_OVR_TOKEN=caf\xe9-file\n")
...
def test_load_son_of_anton_env_fallback_read_error_is_swallowed(tmp_path, monkeypatch):
    ...
    (son_of_anton_home / ".env").write_bytes(b"SEND_ERR_TOKEN=caf\xe9\n")
...
def test_load_son_of_anton_env_bom_only_env_is_noop(tmp_path, monkeypatch):
    """A .env containing only a BOM must load zero vars without error."""
    import os

    son_of_anton_home = tmp_path / ".son-of-anton"
    son_of_anton_home.mkdir()
    (son_of_anton_home / ".env").write_bytes(b"\xef\xbb\xbf")
