...
description: "通过 Termux 在 Android 手机上直接运行 Renco Agent"
---

# 在 Android 上通过 Termux 运行 Renco

这是在 Android 手机上通过 [Termux](https://termux.dev/) 直接运行 Renco Agent 的已验证路径。
...
这并不妨碍 Renco 作为手机原生 CLI agent 正常工作——只是意味着推荐的移动端安装有意比桌面/服务器安装更精简。

---

## 方式一：一行安装命令

Renco 现已内置 Termux 感知的安装路径：
...
- 将 `renco` 链接到 `$PREFIX/bin`，使其保留在 Termux PATH 中
...
### 5. 将 `renco` 添加到 Termux PATH

```bash
ln -sf "$PWD/venv/bin/renco" "$PREFIX/bin/renco"
```

`$PREFIX/bin` 在 Termux 中已默认在 PATH 中，因此这样做可以让 `renco` 命令在新 shell 中持续可用，无需每次重新激活虚拟环境。
...
### `renco doctor` 提示缺少 ripgrep 或 Node
