...
def test_cli_and_cron_share_plugin_target_normalization(plugin_platform, monkeypatch, capsys):
    ...
    from son_of_anton_cli.send_cmd import cmd_send
    ...
    monkeypatch.setattr("son_of_anton_cli.send_cmd._load_son_of_anton_env", lambda: None)
    with patch("gateway.config.load_gateway_config", return_value=config), \
         patch("tools.interrupt.is_interrupted", return_value=False), \
         patch("gateway.mirror.mirror_to_session", return_value=True), \
...
'''
    ...
    env.update({
        ...
        "SON_OF_ANTON_KANBAN_TASK": "fixture",
