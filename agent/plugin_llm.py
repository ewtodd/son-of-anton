...
Plugins built on Renco Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Renco
...
:class:`~renco_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Renco' main config
...
handles every provider, fallback chain, and per-task override Renco
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`renco_cli.plugins.PluginContext`
