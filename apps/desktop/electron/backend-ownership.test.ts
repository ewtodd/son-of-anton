...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'renco serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/renco serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m renco_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/renco --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Renco Runtime\\renco.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('renco chat --query serve'), false)
