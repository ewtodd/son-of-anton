"""``son-of-anton update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``son-of-anton.exe`` gets quarantined,
...
from son_of_anton_cli.update_cmd import _editable_install_is_current
