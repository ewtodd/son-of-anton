...
Covers:
- Defense 1: gateway stop/restart refuse when _SON_OF_ANTON_GATEWAY=1
...
from son_of_anton_cli.cron import (
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "son-of-anton gateway restart",
        "son-of-anton gateway stop",
        "son-of-anton  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "SON_OF_ANTON GATEWAY RESTART",           # uppercase
    ])
    def test_son_of_anton_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.son-of-anton.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.son-of-anton/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l son-of-anton-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.son-of-anton.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.son-of-anton.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.son-of-anton/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated son-of-anton note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "son-of-anton cron list",
        "son-of-anton update",
        "son-of-anton config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `son-of-anton gateway start` is benign — starting a gateway from inside a
        ...
        "son-of-anton gateway start",
        "son-of-anton gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway son-of-anton
        # services must not be falsely blocked (the old `.*son-of-anton` matched any
        # son-of-anton token).
        "launchctl unload ai.son-of-anton.update-checker.plist",
        "launchctl restart ai.son-of-anton.daemon",
        ...
        "systemctl restart son-of-anton-meta.service",
        "systemctl restart son-of-anton-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_son_of_anton_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade son-of-anton then run son-of-anton gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_son_of_anton_gateway_once.sh). The script must live under
        # SON_OF_ANTON_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_path / ".son-of-anton"))
        scripts_dir = tmp_path / ".son-of-anton" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify son-of-anton gateway stop/restart refuse when _SON_OF_ANTON_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_SON_OF_ANTON_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SON_OF_ANTON_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _SON_OF_ANTON_GATEWAY=1.

    Issue #37453: systemctl --user restart son-of-anton-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart son-of-anton-gateway",
        "systemctl --user restart son-of-anton-gateway",
        "systemctl stop son-of-anton-gateway.service",
        "son-of-anton gateway restart",
        "launchctl kickstart gui/501/ai.son-of-anton.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.son-of-anton.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.son-of-anton/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.son-of-anton.gateway.restart-once.plist",
        "pkill -f son-of-anton.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart son-of-anton-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.son-of-anton.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-son-of-anton label: label-independent detection is the point
        # (#62891 second reproduction used `ai.son-of-anton.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.son-of-anton.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.son-of-anton.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.son-of-anton.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-son-of-anton systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run son-of-anton gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("son-of-anton gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeson-of-anton gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under SON_OF_ANTON_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.son-of-anton.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_python_script_with_pathlib_division_not_blocked(self, tmp_path):
        ...
        ".son-of-anton") must NOT be blocked.
        ...
        script.write_text(
            ...
            'ENV = Path.home() / ".son-of-anton" / ".env"\n'
            'print("digest ok")\n'
        )
        check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("son-of-anton gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/son-of-anton\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 son-of-anton gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_son_of_anton_home()` falls back to Path.home(), which raises
        RuntimeError when neither SON_OF_ANTON_HOME nor HOME resolves
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    son_of_anton_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run son-of-anton gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_path / ".son-of-anton"))
        (tmp_path / ".son-of-anton").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run son-of-anton gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell son-of-anton gateway restart"',
        'psql -c "\\! systemctl restart son-of-anton-gateway"',
        "grep 'systemctl restart son-of-anton-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart son-of-anton-gateway",
        'grep "$(systemctl restart son-of-anton-gateway)" f',
        "grep 'restart' log; systemctl restart son-of-anton-gateway",
        'sqlite3 db "SELECT 1"; son-of-anton gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "son-of-anton gateway restart",
        "sudo systemctl stop son-of-anton-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart son-of-anton-gateway%'\""
        )
