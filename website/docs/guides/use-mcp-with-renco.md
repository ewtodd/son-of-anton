---
sidebar_position: 6
title: "Use MCP with Renco"
description: "A practical guide to connecting MCP servers to Renco Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Renco

This guide shows how to actually use MCP with Renco Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Renco tool
- you want Renco to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Renco to internal APIs, databases, or company systems without modifying Renco core

Do not use MCP when:
- a built-in Renco tool already solves the job well
...
- Renco remains the agent
- MCP servers contribute tools
- Renco discovers those tools at startup or reload time
...
If you installed Renco with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Renco banner/status should show MCP integration when configured
- ask Renco what tools it has available
...
## WSL2: bridge Renco in WSL to Windows Chrome

This is the practical setup when:

- Renco runs inside WSL2
...
In this setup, Renco does **not** connect to Chrome directly. Instead:

- Renco runs in WSL
- Renco starts a local stdio MCP server
...
```text
Renco (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Renco stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Renco core browser transport
...
```bash
renco mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
renco mcp test chrome-devtools-win
```

Then start a fresh Renco session or run:
...
Once loaded, Renco can use the MCP-prefixed browser tools directly. For example:
...
If Renco runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Renco from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Renco from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Renco:
...
2. Renco-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Renco will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Renco to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Renco to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Renco remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
renco mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
renco mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `renco mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Renco to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Renco to merge, publish, deploy, or spawn runtimes.
...
Now Renco can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Renco core.
...
Because Renco now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
