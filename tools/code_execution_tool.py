...
Lets the LLM write a Python script that calls Son of Anton tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `son_of_anton_tools.py` stub module with UDS RPC functions
...
# SON_OF_ANTON_KANBAN_DB, SON_OF_ANTON_*_WEBHOOK).  The child only needs the few
# location/profile vars in _SON_OF_ANTON_CHILD_ALLOWED below; SON_OF_ANTON_RPC_SOCKET /
# SON_OF_ANTON_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational SON_OF_ANTON_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set son_of_anton_cli treats as the
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    # broad "SON_OF_ANTON_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like SON_OF_ANTON_KANBAN_DB / SON_OF_ANTON_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if k.startswith("SON_OF_ANTON_"):
            ...
            # allowlist — a deliberately-dropped SON_OF_ANTON_* var.
            _dropped_son_of_anton.append(k)
    if _dropped_son_of_anton:
        logger.debug(
            "execute_code: dropped %d non-allowlisted SON_OF_ANTON_* var(s) from the "
            ...
            len(_dropped_son_of_anton),
            ", ".join(sorted(_dropped_son_of_anton)),
        )
...
# ---------------------------------------------------------------------------
# son_of_anton_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Son of Anton-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Son of Anton-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    son_of_anton_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'son_of_anton_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_son_of_anton_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the son_of_anton_tools.py stub module.
...
"""Auto-generated Son of Anton tools RPC stubs."""
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("SON_OF_ANTON_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Son of Anton tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("SON_OF_ANTON_RPC_DIR") or os.path.join(tempfile.gettempdir(), "son_of_anton_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated son_of_anton_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/son_of_anton_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_son_of_anton_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/son_of_anton_tools.py", tools_src)
        ...
        env_prefix = (
            f"SON_OF_ANTON_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"SON_OF_ANTON_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("SON_OF_ANTON_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    to a subset of Son of Anton tools.
    ...
    # --- Set up temp directory with son_of_anton_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="son_of_anton_sandbox_")
    ...
    try:
        ...
        # SON_OF_ANTON_TIMEZONE is an internal Son of Anton setting and must not leak
        # into child processes.
        _tz_name = os.getenv("SON_OF_ANTON_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("SON_OF_ANTON_TIMEZONE", None)
        ...
        # ``son_of_anton_tools.py`` always lives in the staging directory, so that
        # directory must be importable even when project mode changes CWD.
        # Son of Anton's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Son of Anton's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        #
        # Before re-injecting PYTHONPATH, strip Son of Anton-owned entries that
        # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
        # so it passes the scrub).  They are redundant for same-Son of Anton-
        ...
        _strip_son_of_anton_owned_pythonpath(child_env)
        _son_of_anton_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir]
        if _uses_son_of_anton_python_environment(_child_python):
            _pp_parts.append(_son_of_anton_root)
        elif _child_python not in _external_env_logged:
            # Import behavior changes silently otherwise — surface it (once
            # per interpreter path) so "import son_of_anton_constants suddenly
            # fails" reports are diagnosable without log spam.
            _external_env_logged.add(_child_python)
            logger.info(
                "execute_code: child interpreter %s is outside the Son of Anton "
                "environment; son-of-anton root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.son-of-anton/.env')).
...
def _load_config() -> dict:
    ...
    ``son-of-anton --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (son-of-anton's python). Reproducible and the
...
# Interpreter paths already reported as outside the Son of Anton environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the son-of-anton root from every
...
def _uses_son_of_anton_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Son of Anton's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``son-of-anton tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and son-of-anton's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.son-of-anton/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Son of Anton tools programmatically. "
        ...
        f"Available via `from son_of_anton_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from son_of_anton_tools import {import_str}` "
