...
Renco Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Renco can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`renco fallback` reuses the provider picker from `renco model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.

If you'd rather edit the YAML directly, add a top-level `fallback_providers` list to `~/.renco/config.yaml`:
...
`fallback_providers` (plural, list) is the current config shape and supports multiple fallbacks tried in order. `fallback_model` (singular) is the legacy single-fallback key — Renco still honors it for back-compat, but `renco fallback` writes the current `fallback_providers` key and migrates legacy config on write. When both are set, `fallback_providers` takes priority.
:::
...
| Nous Portal | `nous` | `renco setup --portal` (fresh) or `renco auth add nous` (OAuth) |
| OpenAI Codex | `openai-codex` | `renco model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth) |
...
| xAI Grok OAuth (SuperGrok) | `xai-oauth` (alias `grok-oauth`) | `renco model` → xAI Grok OAuth (browser login; SuperGrok subscription) |
| AWS Bedrock | `bedrock` | Standard boto3 auth (`AWS_REGION` + `AWS_PROFILE` or `AWS_ACCESS_KEY_ID`) |
| Qwen Portal (OAuth) | `qwen-oauth` | `renco model` (Qwen Portal OAuth; optional: `RENCO_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `renco model` (MiniMax portal OAuth) |
...
:::tip
There are no environment variables for the primary fallback chain — configure it exclusively through `config.yaml` or `renco fallback`. This is intentional: fallback configuration is a deliberate choice, not something a stale shell export should override.
:::
...
Renco uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `renco kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), Renco first tries the main provider + main model for that auxiliary task. If that route is unavailable or later fails with a capacity-style error, Renco now honors user-configured fallback policy before using the built-in discovery chain:
...
Every task above follows the same **provider / model / base_url** pattern. Each task can also declare its own `fallback_chain`; if omitted, `provider: auto` uses the top-level `fallback_providers` chain before Renco' built-in auxiliary discovery chain.
...
| `"codex"` | Force Codex OAuth | `renco model` → ChatGPT or Codex Subscription |
...
`base_url` takes precedence over `provider`. Renco uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.

---

## Auxiliary Capacity-Error Fallback

When you set an explicit auxiliary provider (e.g. `auxiliary.vision.provider: glm`), Renco treats that as your preferred choice — but if the provider literally cannot serve the request because of a **capacity error** (HTTP 402 payment required, HTTP 429 daily-quota exhaustion, connection failure), Renco falls back through a layered chain instead of failing silently:
...
4. **Warn + re-raise** — if every layer fails, Renco logs `Auxiliary <task>: ... all fallbacks exhausted` at WARNING level and re-raises the original error
...
Renco recognizes these as capacity-equivalent to 402 credit exhaustion (not transient rate limits):
...
If your provider returns a different phrase for daily-quota exhaustion and Renco doesn't trigger fallback, that's a bug — open an issue with the exact error string.
...
If no provider is available for compression, Renco drops middle conversation turns without generating a summary rather than failing the session.
