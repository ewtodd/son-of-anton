"""CLI handlers for ``renco migrate ...``.

Currently exposes only ``renco migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``renco migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: renco migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `renco migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `renco doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
