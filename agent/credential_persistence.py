...
``auth.json``.  They intentionally have no dependency on ``son_of_anton_cli.auth`` so
...
# Sources Son of Anton owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Son of Anton-owned OAuth/device-code state)
