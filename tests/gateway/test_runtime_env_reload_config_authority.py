...
RENCO_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env RENCO_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``renco config get terminal.backend`` still says local.
    """
    renco_home = tmp_path / ".renco"
    renco_home.mkdir()
    (renco_home / "config.yaml").write_text(
    ...
    (renco_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
