"""``renco logs`` — view and filter Renco log files.
...
Usage examples::
    ...
    renco logs                    # last 50 lines of agent.log
    renco logs -f                 # follow agent.log in real time
    renco logs errors             # last 50 lines of errors.log
    renco logs gateway -n 100    # last 100 lines of gateway.log
    renco logs gui -f            # follow gui.log (dashboard/pty/ws)
    renco logs desktop -f        # follow desktop.log (Electron app boot/backend)
    renco logs --level WARNING    # only WARNING+ lines
    renco logs --session abc123   # filter by session ID substring
    renco logs --component tools  # only tool-related lines
    renco logs --since 1h         # lines from the last hour
    renco logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_renco_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Renco runs — try 'renco chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_renco_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_renco_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_renco_home()}/logs/")
        return

    print(f"Log files in {display_renco_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'renco chat' to generate logs)")
