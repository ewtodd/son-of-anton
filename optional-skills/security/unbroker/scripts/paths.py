...
All per-subject data lives under PDD_DATA_DIR (default: $RENCO_HOME/unbroker),
which is the same trust boundary Renco uses for .env and OAuth tokens.
"""
...
def renco_home() -> Path:
    return Path(os.environ.get("RENCO_HOME") or (Path.home() / ".renco"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else renco_home() / "unbroker"
