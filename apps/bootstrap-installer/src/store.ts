...
export const $rencoHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.renco/logs/bootstrap-installer.log')
    $rencoHome.set('~/.renco')
  ...
  try {
    const [logPath, rencoHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_renco_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $rencoHome.set(rencoHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Renco" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Renco-Setup.exe --update);
...
export async function launchRencoDesktop(): Promise<void> {
  ...
  await invoke('launch_renco_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Renco repository' },
