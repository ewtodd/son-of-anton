"""Slash command definitions and autocomplete for the Renco CLI.
...
from renco_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``renco_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``renco_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Renco config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Renco works on across turns until achieved", "Session",
    ...
    CommandDef("update", "Update Renco Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Renco Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`renco_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/renco`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Renco ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Renco
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``renco skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``renco skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``renco skills list`` and
...
# unrelated command landed. These claim their slots right after /renco,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /renco <command>).
...
# native slot, the alias spelling stays reachable via /renco reset).
...
# ``/renco <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/renco" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /renco topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /renco moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /renco debug on Slack.
#   - egress: Docker-only proxy status; reachable as /renco egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /renco init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /renco version on
...
#   - diff: git working-tree diff; reached via /renco diff on Slack so it
...
#     /renco update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /renco heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /renco refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /renco pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /renco whoami on Slack.
...
#     /renco platform on Slack. Demoted when /save became gateway-available
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/renco <verb>`` subcommand.
    ...
    can still reach them via ``/renco <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/renco`` is always reserved as the first entry so the
    legacy ``/renco <subcommand>`` form keeps working for anything that
    ...
    # Reserve /renco as the catch-all top-level command.
    entries.append(("renco", "Talk to Renco or run a subcommand", "[subcommand] [args]"))
    seen.add("renco")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_RENCO_ONLY:
            # Intentionally Slack-via-/renco only (see _SLACK_VIA_RENCO_ONLY).
            return
    ...
    # everything except /renco, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://renco-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /renco handler.

    Maps both canonical names and aliases so /renco bg do stuff works
    the same as /renco background do stuff.

    Plugin-registered slash commands are included so ``/renco <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from renco_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via renco_cli.personality."""
        try:
