"""``son-of-anton worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``son_of_anton_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    son-of-anton worktree list             # audit: verdict + reason per tree
    son-of-anton worktree prune            # reap safe trees + merged branches
    son-of-anton worktree prune --dry-run  # show the plan, change nothing
    son-of-anton worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `son-of-anton worktree prune`."
        )
