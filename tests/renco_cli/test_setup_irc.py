"""Tests for IRC gateway configuration via `renco setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Renco install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Renco install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#renco")
            monkeypatch.setenv("IRC_NICKNAME", "renco-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `renco setup gateway` experience with IRC present."""
