#!/usr/bin/env python3
"""Canvas LMS API CLI for Renco Agent.
...
def _check_config():
    ...
    if missing:
        renco_env = os.path.join(
            os.environ.get("RENCO_HOME", os.path.expanduser("~/.renco")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {renco_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Renco Agent"
    )
