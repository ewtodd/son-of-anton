...
| Variable | Meaning |
|----------|---------|
| `RENCO_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `RENCO_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `RENCO_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Renco itself — they're private to these helpers. The variables Renco *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `renco --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_renco_root)" || { echo "htui: not in a Renco checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$RENCO_MAIN_CHECKOUT/.venv/bin/python" -m renco_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `RENCO_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_renco_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `RENCO_TUI_DIR` are mutually exclusive
`RENCO_TUI_DIR` points Renco at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `renco --tui --dev` exits with an error. Run `unset RENCO_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_renco_root)" || { echo "hgui: not in a Renco checkout" >&2; return 1; }
  deps="${RENCO_GUI_DEPS_CHECKOUT:-$RENCO_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _renco_link_deps "$desktop" "$deps/apps/desktop"
    _renco_link_deps "$root" "$deps"
  else
  ...
  trap '_renco_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    RENCO_DESKTOP_RENCO_ROOT="$root" \
    RENCO_DESKTOP_PYTHON="$RENCO_MAIN_CHECKOUT/.venv/bin/python" \
    RENCO_DESKTOP_IGNORE_EXISTING=1 \
    RENCO_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `RENCO_DESKTOP_RENCO_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `renco`. |
| `RENCO_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `RENCO_DESKTOP_IGNORE_EXISTING` | Ignores any `renco` on `PATH` so it can't shadow the worktree. |
| `RENCO_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Renco checkout.
_renco_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/renco_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_renco_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'renco_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `renco --tui --dev` and the `RENCO_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/ewtodd/renco/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `RENCO_*` variable Renco reads
