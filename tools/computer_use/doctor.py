"""
`renco computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Renco side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Renco provider secrets stripped.
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """Renco-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    Honors `RENCO_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if json_output:
        ...
        # attach Renco identity under renco_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["renco_identity"] = identity
