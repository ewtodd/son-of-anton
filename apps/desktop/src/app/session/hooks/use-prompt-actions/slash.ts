import { skillInvocationText } from '@renco/shared'
...
// 120s RPC budget (RENCO_TUI_RPC_TIMEOUT_MS default) instead.
...
const renderWakeStatus = (status: WakeStatusResponse): string => {
  const lines = [
    ...
    `Phrase: "${status.phrase?.trim() || 'hey renco'}"`,
  ...
  return useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        // lease. Sending it through slash.exec creates a separate RencoCLI in
