"""Meta Model API (Muse Spark) provider plugin for Renco Agent.
...
Bundled from https://github.com/albertodepaola/renco-meta-provider. Renco'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to renco' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Renco' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in renco_cli/providers.py and the retention hint in
