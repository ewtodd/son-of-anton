...
409), and is driven by:
  ...
  * the ``son-of-anton sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``son_of_anton_cli/``) so the
...
is toggled via ``son-of-anton sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.son-of-anton/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``son-of-anton sync enable`` — it is
...
# namespace from son-of-anton's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the SON_OF_ANTON_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: SON_OF_ANTON_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("SON_OF_ANTON_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Son of Anton Cloud instance can be set
...
# precedence as base_url: the SON_OF_ANTON_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   SON_OF_ANTON_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   SON_OF_ANTON_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   SON_OF_ANTON_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``SON_OF_ANTON_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Son of Anton Cloud deployment sets to opt its instances into sync by
    ...
    """
    return _sync_config_bool("SON_OF_ANTON_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``SON_OF_ANTON_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``son-of-anton sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "SON_OF_ANTON_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``SON_OF_ANTON_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``son-of-anton sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Son of Anton Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("SON_OF_ANTON_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.son-of-anton/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.son-of-anton/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Son of Anton Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``SON_OF_ANTON_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.son-of-anton/skills/ containing
...
def stable_device_id() -> str:
    ...
    ~/.son-of-anton/skills/.sync_device_id.
    ...
    Use ``set_device_name()`` / ``son-of-anton sync device --name`` to set an explicit
    label."""
    ...
    # Son of Anton Cloud (and any templated deployment) can seed the label
    # declaratively via SON_OF_ANTON_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `son-of-anton sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("SON_OF_ANTON_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.son-of-anton/skills/.sync_device_id, overwriting
...
# ~/.son-of-anton/skills/.sync_state as JSON.
#
...
# path under ~/.son-of-anton/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.son-of-anton/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Son of Anton speaks "
            f"{WIRE_VERSION} — update Son of Anton to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "son-of-anton skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (son-of-anton sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.son-of-anton/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "son-of-anton skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (SON_OF_ANTON_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``son-of-anton sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.son-of-anton/skills/_org/
...
#
# `son-of-anton sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.son-of-anton/skills/_org/<org_id>/``.
