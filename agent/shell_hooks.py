...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`son_of_anton_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.son-of-anton/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``SON_OF_ANTON_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``son_of_anton_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Son of Anton-canonical

    # Inject context for pre_llm_call:
    {"context": "Today is Friday"}

    # Modify tool input for pre_tool_call (Son of Anton-canonical):
...
# son_of_anton_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``son_of_anton_cli.config.load_config``
    ...
    setting.  ``SON_OF_ANTON_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / SON_OF_ANTON_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("SON_OF_ANTON_SAFE_MODE"):
        logger.info("SON_OF_ANTON_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / SON_OF_ANTON_ACCEPT_HOOKS=1 / "
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from son_of_anton_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _evaluate_result(
    ...
    * non-JSON / unparseable stdout on a ``fail_closed`` blocking hook —
    ...
    test helper (:func:`run_once`) so ``son-of-anton hooks test`` reflects
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Son of Anton wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Son of Anton
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`son_of_anton_cli.plugins.get_pre_tool_call_block_message`.  This is
    ...
    if event == "pre_verify":
        # "continue" (Son of Anton) / "block" (Claude-Code Stop: block the stop) both
...
def allowlist_path() -> Path:
    ...
    return get_son_of_anton_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / SON_OF_ANTON_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Son of Anton is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    if accept_hooks_arg:
        return True
    env = os.environ.get("SON_OF_ANTON_ACCEPT_HOOKS", "").strip().lower()
