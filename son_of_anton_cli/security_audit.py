"""On-demand supply-chain audit for Son of Anton Agent installs.

Scans three surfaces a Son of Anton user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Son of Anton venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.son-of-anton/plugins``
...
def _discover_plugins(son_of_anton_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.son-of-anton/plugins``.
...
def _discover_components(
    *,
    ...
    home = son_of_anton_home or Path(get_son_of_anton_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `son-of-anton security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, son_of_anton_home=home
    )
