"""On-demand worktree + branch reclaim (``renco worktree`` / ``/worktree prune``).

The startup pruner in ``cli._prune_stale_worktrees`` is deliberately
conservative and silent: it runs before the banner on every ``renco -w``
...
- **Orphaned local branches** beyond the two auto-generated prefixes the
  startup pass deletes (``renco/renco-*``, ``pr-*``): salvage lanes, port
...
- untracked-only dirt is ARCHIVED to ``~/.renco/archive/worktree-prune/``
...
def _archive_untracked(tree: Path, untracked: List[str]) -> Optional[Path]:
    ...
    dest = (
        Path.home() / ".renco" / "archive" / "worktree-prune"
        / f"{tree.name}-{stamp}"
    )
    try:
...
def audit_worktrees(repo_root: str, *, with_sizes: bool = True) -> List[TreeRecord]:
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        ...
        if lock_state == "live":
            rec("keep", "in use by a running renco session")
            continue
...
def audit_branches(repo_root: str) -> List[BranchRecord]:
    ...
    Generalizes the startup pass's prefix list (``renco/renco-*``/``pr-*``)
    ...
    if workers > 1:
        try:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=workers, thread_name_prefix="renco-branch-gc"
            ) as pool:
