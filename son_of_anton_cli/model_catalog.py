...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/ewtodd/son-of-anton/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Son of Anton silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`son-of-anton update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``son-of-anton update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.son-of-anton/cache/model_catalog.json`` via the same atomic writer the
