...
def test_recovered_update_retry_skips_external_secret_sources(tmp_path, monkeypatch):
    ...
    import son_of_anton_cli.env_loader as env_loader
    ...
    loaded = load_son_of_anton_dotenv(son_of_anton_home=home)
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    from son_of_anton_cli.env_loader import _sanitize_env_file_if_needed

    env_file = tmp_path / ".env"
    content = "SON_OF_ANTON_TEST_KEY=hello_utf32\nSECOND_KEY=world\n"
    raw = codecs.BOM_UTF32_LE + content.encode("utf-32-le")
    env_file.write_bytes(raw)

    with caplog.at_level(logging.WARNING, logger="son_of_anton_cli.env_loader"):
...
def test_known_keys_absent_from_user_env_are_cleared(tmp_path, monkeypatch):
    """Known Son of Anton keys inherited from parent process are removed when absent
    ...
    monkeypatch.setenv("SON_OF_ANTON_ACP_AUTH_METHOD", "cursor_login")
    ...
    load_son_of_anton_dotenv(son_of_anton_home=home)
    ...
    # SON_OF_ANTON_ACP_AUTH_METHOD and COPILOT_CLI_PATH are NOT in the profile .env → cleared
    assert "SON_OF_ANTON_ACP_AUTH_METHOD" not in os.environ
...
def test_empty_assignment_in_user_env_is_preserved(tmp_path, monkeypatch):
    ...
    Empty ``SON_OF_ANTON_ACP_AUTH_METHOD=`` tells the ACP adapter to skip
    ...
    home = tmp_path / "son-of-anton"
    home.mkdir()
    (home / ".env").write_text("SON_OF_ANTON_ACP_AUTH_METHOD=\n", encoding="utf-8")
    ...
    assert "SON_OF_ANTON_ACP_AUTH_METHOD" in os.environ
    assert os.environ["SON_OF_ANTON_ACP_AUTH_METHOD"] == ""
    # COPILOT_CLI_PATH is absent from .env → cleared
    assert "COPILOT_CLI_PATH" not in os.environ


def test_no_user_env_does_not_clear_anything(tmp_path, monkeypatch):
    """When no profile .env exists (bare profile), load_son_of_anton_dotenv must not
    ...
    assert os.getenv("SON_OF_ANTON_ACP_AUTH_METHOD") == "cursor_login"
    assert os.getenv("PATH") == "/usr/bin:/bin"


def test_known_key_explicitly_set_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Son of Anton key that IS explicitly set in the profile .env survives
    the cleanup (overrides the inherited value).
    """
    home = tmp_path / "son-of-anton"
    home.mkdir()
    (home / ".env").write_text(
        "SON_OF_ANTON_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    ...
    assert os.getenv("SON_OF_ANTON_ACP_AUTH_METHOD") == "claude_code_cli"


def test_export_prefixed_known_key_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Son of Anton key defined with the bash-compatible ``export KEY=value``
    ...
    home.mkdir()
    (home / ".env").write_text(
        "export SON_OF_ANTON_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    monkeypatch.setenv("SON_OF_ANTON_ACP_AUTH_METHOD", "cursor_login")
    load_son_of_anton_dotenv(son_of_anton_home=home)
    assert os.getenv("SON_OF_ANTON_ACP_AUTH_METHOD") == "claude_code_cli"
...
def test_cleanup_scope_is_the_profile_managed_set():
    ...
    """
    from son_of_anton_cli.env_loader import _PROFILE_MANAGED_ENV_KEYS
...
#
# load_son_of_anton_dotenv loads .env with override=True, so a stale
...
# the shared son_of_anton_cli.config.apply_terminal_config_to_env helper.
# ---------------------------------------------------------------------------
...
def _seed_terminal_home(tmp_path, monkeypatch, *, config_yaml=None, env_text=None):
    ...
    # The bridge is scoped to the process SON_OF_ANTON_HOME (a different profile's
...
def test_config_yaml_terminal_backend_overrides_stale_env(tmp_path, monkeypatch):
    """Regression for #29186: a leftover TERMINAL_ENV=docker in ~/.son-of-anton/.env
...
def test_other_profile_home_does_not_bridge_process_config(tmp_path, monkeypatch):
    ...
    load_son_of_anton_dotenv(son_of_anton_home=other_home)
