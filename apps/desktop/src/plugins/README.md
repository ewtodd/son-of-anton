...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `RencoPlugin` and
...
`$RENCO_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`renco-desktop-plugins` skill.
