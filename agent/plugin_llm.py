...
Plugins built on Son of Anton Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Son of Anton
...
:class:`~son_of_anton_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Son of Anton' main config
...
handles every provider, fallback chain, and per-task override Son of Anton
supports.
"""
...
def _resolve_task_ownership(plugin_id: str) -> tuple[frozenset, frozenset]:
    ...
    try:
        ...
        pass
    try:
        from son_of_anton_cli.main import _AUX_TASKS
...
class PluginLlm:
    ...
    Instances are constructed by :class:`son_of_anton_cli.plugins.PluginContext`
