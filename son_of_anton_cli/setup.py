...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # Image and resource limits use defaults; tune via `son-of-anton setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `son-of-anton egress setup` then `son-of-anton egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `son-of-anton egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'son-of-anton[vercel]'")
        ...
        try:
            ...
            # which() misses the uv Son of Anton installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'son-of-anton[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
