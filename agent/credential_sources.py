"""Unified removal contract for every credential source Son of Anton reads from.

Son of Anton seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.son-of-anton/.env
    claude_code   — ~/.claude/.credentials.json
    son_of_anton_pkce   — ~/.son-of-anton/.anthropic_oauth.json
    ...
    manual        — user ran `son-of-anton auth add`
...
unify here is **removal**:
    ...
    ``son-of-anton auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), son_of_anton_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.son-of-anton/.env  → clear it
    ...
    """
    from son_of_anton_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in son_of_anton_cli/config.py. read_text() with no
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.son-of-anton/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Son of Anton.",
            f"  The pool entry is now suppressed — Son of Anton will ignore "
            f"{env_var} until you run `son-of-anton auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Son of Anton stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `son-of-anton auth add anthropic` to re-enable if needed.",
    ])


def _remove_son_of_anton_pkce(provider: str, removed) -> RemovalResult:
    """~/.son-of-anton/.anthropic_oauth.json is ours — delete it outright."""
    from son_of_anton_constants import get_son_of_anton_home

    result = RemovalResult()
    oauth_file = get_son_of_anton_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Son of Anton Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``son-of-anton auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `son-of-anton model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Son of Anton auth store is not enough — _seed_from_singletons() would
    ...
    via ``son-of-anton auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `son-of-anton auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Son of Anton stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `son-of-anton auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="son_of_anton_pkce",
        remove_fn=_remove_son_of_anton_pkce,
        description="~/.son-of-anton/.anthropic_oauth.json",
    ))
    register(RemovalStep(
