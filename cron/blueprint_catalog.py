...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``renco://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``renco://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"renco://blueprint/{quote(blueprint.key)}{qs}"
