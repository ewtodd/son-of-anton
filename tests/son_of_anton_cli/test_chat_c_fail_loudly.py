...
Regression for #86794: a background/quiet `son-of-anton chat -c "<title>" -q "..."`
...
def isolated_home(tmp_path, monkeypatch):
    """Point SON_OF_ANTON_HOME at a temp dir so session creation stays isolated."""
...
def _create_titled_session(title):
    from son_of_anton_cli.main import _create_titled_session as fn
...
class TestChatCFailLoudlyOnStderr:
    ...
    def test_missing_session_fails_on_stderr(self, isolated_home, monkeypatch):
        ...
        assert not stderr_lines[0].startswith("Use 'son-of-anton sessions list'")
