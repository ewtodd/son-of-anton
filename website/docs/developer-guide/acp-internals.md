...
The ACP adapter wraps Renco' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
renco acp / renco-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.renco/.env
  -> configure stderr logging
  -> construct RencoACPAgent
...
- `allow_once` -> Renco `once`
- `allow_always` -> Renco `always`
- reject options -> Renco `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Renco tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["renco-acp"])
...
Instead it reuses Renco' runtime resolver:

- `acp_adapter/auth.py`
- `renco_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Renco provider/credentials. It also always advertises a terminal setup auth method (`renco-setup`, args `--setup`) so first-run ACP clients can open Renco' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.renco/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `renco-acp` toolset definition
- `renco_cli/main.py` — `renco acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `renco-acp` script
