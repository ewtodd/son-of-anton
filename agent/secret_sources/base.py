...
``~/.son-of-anton/.env`` has loaded and BEFORE the rest of Son of Anton reads
``os.environ``.
...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per SON_OF_ANTON_HOME) by the orchestrator in
...
_SOURCE_ENVIRONMENT = ContextVar("son_of_anton_secret_source_environment", default=None)
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``son-of-anton secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``son-of-anton secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def remediation(self, kind: Optional["ErrorKind"], cfg: dict) -> str:
        ...
        Called by the startup status printer (and ``son-of-anton secrets ...
        ...
        *what to run* next to fix it — not just what broke.  Sources
        ...
        ``son-of-anton secrets bitwarden token`` for AUTH_FAILED).  Return an
        ...
        generic = {
            ErrorKind.NOT_CONFIGURED: (
                f"Run `son-of-anton secrets {self.name} setup` to finish configuration."
            ),
            ErrorKind.BINARY_MISSING: (
                f"Run `son-of-anton secrets {self.name} setup` to install the helper CLI."
            ),
            ErrorKind.AUTH_FAILED: (
                f"Credentials rejected — run `son-of-anton secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.AUTH_EXPIRED: (
                f"Credentials expired — run `son-of-anton secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.NETWORK: (
...
# codes that must not reach Son of Anton' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential Son of Anton knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into Son of Anton
      output.
