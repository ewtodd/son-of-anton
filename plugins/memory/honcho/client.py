...
Resolution order for config file:
  1. $RENCO_HOME/honcho.json  (instance-local, enables isolated Renco instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Renco profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Renco profile.

    Resolution order:
      1. RENCO_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``renco_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"renco"`` (default profile)
    """
    explicit = os.environ.get("RENCO_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named Renco
    # profile share that same host.  Keep named profiles isolated; only the
    # default Renco profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $RENCO_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.renco/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_renco_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_renco_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.renco
    ...
    # resolve_config_path()/get_renco_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $RENCO_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Renco profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.renco block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the Renco-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Renco session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Renco session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_renco_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_renco_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'renco honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.renco_home) if config.renco_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/renco_home) identity reappears with a
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'renco honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `renco honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                renco_cfg = load_config()
                honcho_cfg = renco_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
