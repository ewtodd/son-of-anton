...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "son-of-anton"
    ...
    monkeypatch.setattr(uninstall, "get_son_of_anton_home", lambda: son_of_anton_home)
    monkeypatch.setattr(uninstall, "_is_default_son_of_anton_home", lambda home: False)
    ...
    assert str(son_of_anton_home) in output
    assert project_root.exists()
    assert son_of_anton_home.exists()
