...
Complements tests/renco_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/renco_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/renco_cli/test_kanban_db.py.
    monkeypatch.setenv("RENCO_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_default_spawn_does_not_auto_load_any_skill(kanban_home, monkeypatch):
    ...
    renco subprocess (which would hang trying to call an LLM).
    """
    captured = {}
    ...
    env = captured["env"]
    assert env.get("RENCO_KANBAN_TASK") == tid
    assert env.get("RENCO_PROFILE") == "some-profile"
...
def test_cli_daemon_help_marks_deprecated():
    ...
    from renco_cli import kanban as kb_cli
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `renco kanban --help` output"
    )
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``renco_cli.kanban_db`` fresh and uses that single
