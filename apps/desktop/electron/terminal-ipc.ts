...
// registry, and the renco:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // RENCO_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.RENCO_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Renco' TUI into
    ...
    env.TERM_PROGRAM = 'Renco'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a renco/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from RENCO_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.RENCO_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `renco:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('renco:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('renco:terminal:write', (_event, id, data) => {
  ...
  ipcMain.handle('renco:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('renco:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('renco:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
