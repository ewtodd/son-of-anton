...
`renco config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.renco/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `RENCO_HOME`, secret scope, and
