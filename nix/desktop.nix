# nix/desktop.nix — Renco Desktop (Electron) app build + wrapper
#
# `rencoAgent` is the fully-built `.#default` package — it ships the
# `renco` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `RENCO_DESKTOP_RENCO` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Renco CLI").
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "renco-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "renco-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = rencoNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "renco-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe rencoNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/renco-desktop $out/bin
    cp -r ${renderer}/* $out/share/renco-desktop/
    ...
    substituteInPlace $out/share/renco-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/renco-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # RENCO_DESKTOP_RENCO to the absolute path of the nix-built `renco`
    # binary so the desktop's resolver step 4 ("existing Renco CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/renco-desktop \
      --add-flags "$out/share/renco-desktop" \
      --set RENCO_DESKTOP_RENCO "${lib.getExe rencoAgent}" \
      --set ELECTRON_IS_DEV 0

    # XDG launcher entry
    mkdir -p $out/share/applications $out/share/icons/hicolor/1024x1024/apps
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/renco.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../renco_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/renco-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/renco.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/renco.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Renco Agent";
    homepage = "https://github.com/ewtodd/renco";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "renco-desktop";
  };
}
