...
it is automatically downloaded from GitHub releases to $SON_OF_ANTON_HOME/bin/tirith.
...
def _get_son_of_anton_home() -> str:
    """Return the Son of Anton home directory, respecting SON_OF_ANTON_HOME env var."""
    return str(get_son_of_anton_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_son_of_anton_home(), ".tirith-install-failed")
...
def _son_of_anton_bin_dir() -> str:
    """Return $SON_OF_ANTON_HOME/bin, creating it if needed."""
    d = os.path.join(_get_son_of_anton_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SON_OF_ANTON_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_son_of_anton_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SON_OF_ANTON_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SON_OF_ANTON_HOME/bin/tirith
    ...
    son_of_anton_bin = os.path.join(_son_of_anton_bin_dir(), "tirith")
    if os.path.isfile(son_of_anton_bin) and os.access(son_of_anton_bin, os.X_OK):
        _resolved_path = son_of_anton_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return son_of_anton_bin
