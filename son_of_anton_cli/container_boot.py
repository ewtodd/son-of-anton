...
``$SON_OF_ANTON_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-son-of-anton-setup
(the stage2 hook) has chowned the volume and seeded $SON_OF_ANTON_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$SON_OF_ANTON_HOME``,
    not under ``profiles/``). The dispatcher in ``son_of_anton_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``son-of-anton gateway start`` (no ``-p``) targets. Without it,
    bare ``son-of-anton gateway start`` inside the container would land on
    ...
    ``$SON_OF_ANTON_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        son_of_anton_home: The container's SON_OF_ANTON_HOME (typically /opt/data).
            Profiles live under ``<son_of_anton_home>/profiles/<name>/``;
            the default profile lives at ``<son_of_anton_home>`` itself.
    ...
    # ``son-of-anton gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(son_of_anton_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(son_of_anton_home)
    ...
    actions.append(ReconcileAction(
        ...
        prior_exit=_read_prior_exit_label(son_of_anton_home),
    ))

    profiles_root = son_of_anton_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `son-of-anton profile create` (config.yaml
            # is not — that comes later via `son-of-anton setup`). Use it as the
            ...
            # via ``son-of-anton -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(son_of_anton_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = son_of_anton_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("SON_OF_ANTON_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the son-of-anton args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/son-of-anton/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/son-of-anton/docker/main-wrapper.sh
    ...
    ``son-of-anton`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``son-of-anton <subcommand>``, so an
    explicit leading ``son-of-anton`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `son-of-anton <subcommand>`; peel an explicit son-of-anton.
    if args and Path(args[0]).name == "son-of-anton":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``son-of-anton dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted SON_OF_ANTON_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `son-of-anton -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit son-of-anton
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the son-of-anton user) won't hit EACCES. See
...
def _write_reconcile_log(
    son_of_anton_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $SON_OF_ANTON_HOME/logs/container-boot.log.
    ...
    """
    import time
    log_dir = son_of_anton_home / "logs"
...
def main() -> int:
    ...
    # bind-mounted SON_OF_ANTON_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        son_of_anton_home=son_of_anton_home, scandir=scandir,
    )
    for a in actions:
        print(
