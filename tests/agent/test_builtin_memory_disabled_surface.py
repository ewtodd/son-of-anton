...
``get_tool_definitions``) against a temp ``RENCO_HOME``, not mocks.
"""
...
class TestBuiltinMemoryToolAvailability:
    def test_tool_hidden_when_both_stores_disabled(self, renco_home):
        _write_memory_config(
            renco_home, memory_enabled=False, user_profile_enabled=False
        )
        assert "memory" not in _memory_tool_names()

    def test_tool_present_when_only_user_profile_enabled(self, renco_home):
        _write_memory_config(
            renco_home, memory_enabled=False, user_profile_enabled=True
        )
    ...
    def test_tool_present_when_only_memory_enabled(self, renco_home):
        _write_memory_config(
            renco_home, memory_enabled=True, user_profile_enabled=False
        )
        definition = _memory_tool_definition()
    ...
    def test_tool_present_by_default(self, renco_home):
    ...
    def test_config_flip_updates_tool_without_manual_cache_clear(self, renco_home):
        _write_memory_config(
            renco_home, memory_enabled=False, user_profile_enabled=False
        )
        assert "memory" not in _memory_tool_names()
    ...
    def test_unreadable_config_fails_open(self, renco_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "renco_cli.config.load_config_readonly", _boom, raising=False
        )
...
class TestIndependentStoreWriteGates:
    ...
    def test_user_profile_only_rejects_memory_write(self, renco_home):
    ...
    def test_memory_only_rejects_user_profile_write(self, renco_home):
        from tools.memory_tool import memory_tool
    ...
    def test_staged_write_cannot_bypass_target_gate(self, renco_home):
    ...
    def test_invalid_target_error_is_bounded_and_carries_recovery_hint(self, renco_home):
