...
Pre-#67193 renco-setup binaries reuse ``bootstrap-cache/install-<branch>.ps1``
...
``Access denied``). Those binaries have no self-update path, so ``renco
...
def _run(home, root, branch="main"):
    with patch.object(cli_main, "get_renco_home", return_value=str(home)), patch.object(
...
def test_never_raises_on_io_error(tmp_path):
    home, root = _setup(tmp_path)
    with patch.object(cli_main, "get_renco_home", side_effect=OSError("boom")):
