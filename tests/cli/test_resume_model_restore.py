...
SessionDB.session_gateway_runtime (son_of_anton_state.py) — the round trip that
makes `son-of-anton --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare SonOfAntonCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.SonOfAntonCLI)
