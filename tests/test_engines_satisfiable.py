...
`npm ci` / `npm install` — the installer's workspace step, `renco update`'s
...
`renco update` left installs in a mixed state. These tests encode the
...
class TestEnginesAreSatisfiable:
    ...
    def test_managed_node_bundles_an_npm_the_engines_accept(self):
        ...
        11.10–11.16 band — fresh Renco-managed installs then die at
        ...
        assert _satisfies_range(stock_npm, npm_range), (
            ...
            "Renco-managed install cannot run npm ci."
        )
