...
# Wire-name used when Son of Anton keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Son of Anton's configured provider
# (Firecrawl / Tavily / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "son_of_anton_web_search"
...
# rename on the wire (son_of_anton_<name>), map back in normalize_response so
# Son of Anton dispatch is unaffected.
_OPENCODE_RESERVED_TOOL_NAMES = ("web_search", "search_files")
_RESERVED_TOOL_ALIAS_PREFIX = "son_of_anton_"
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Son of Anton web providers.
        #
        ...
        #    keep Son of Anton dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``son_of_anton_web_search`` so Grok cannot hijack the name. The alias
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Son of Anton dispatches
                ...
                # (son_of_anton_web_search / son_of_anton_search_files, #85589).
