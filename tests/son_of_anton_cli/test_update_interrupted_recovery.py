...
``son-of-anton update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
def _stub_install_env(monkeypatch, m, seen):
    ...
    monkeypatch.setattr("son_of_anton_cli.managed_uv.ensure_uv", lambda: None)
    # The install executor moved to son_of_anton_cli._install_repair (shared between
    ...
    import son_of_anton_cli._install_repair as ir
...
def test_recovery_self_lock_does_not_clear_core_marker_via_import_probes(
    ...
    monkeypatch.setattr(m, "_son_of_anton_exe_shims", lambda d: [shim])
    monkeypatch.setattr(
        m,
