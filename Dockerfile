...
# node:26 image instead (Renco pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/renco is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/renco/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when renco
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# renco process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('renco')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main renco process, the dashboard,
...
# 'Renco WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via RENCO_UID at runtime
RUN useradd -u 10000 -m -d /opt/data renco
...
#
# ui-tui/packages/renco-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/renco-ink/ ui-tui/packages/renco-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as renco-ink above).
...
# lock on the @renco/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Renco when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Renco production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/renco/.venv at first
...
# gives the non-root renco user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link renco-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/renco are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the renco user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/renco/bin && \
    cp /opt/renco/docker/renco-exec-shim.sh /opt/renco/bin/renco && \
    chmod 0755 /opt/renco/bin/renco && \
    printf 'docker\n' > /opt/renco/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $RENCO_HOME. $RENCO_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``renco update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the renco user via
# `s6-setuidgid renco` in its run script. If RENCO_UID is unset, services
# run as the default renco user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `renco dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the RENCO_GIT_SHA build-arg to
# /opt/renco/.renco_build_sha at build time, and have
# renco_cli/build_info.py read it at runtime.  Both `renco dump` and
...
ARG RENCO_GIT_SHA=
RUN if [ -n "${RENCO_GIT_SHA}" ]; then \
        printf '%s\n' "${RENCO_GIT_SHA}" > /opt/renco/.renco_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-renco + dashboard.
...
# `exec renco`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $RENCO_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/renco/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-renco-setup && \
    chmod +x /etc/cont-init.d/01-renco-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV RENCO_WEB_DIST=/opt/renco/renco_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/renco/ui-tui/dist/entry.js`) and skip the
...
ENV RENCO_TUI_DIR=/opt/renco/ui-tui
ENV RENCO_HOME=/opt/data
ENV RENCO_WRITE_SAFE_ROOT=/opt/data
ENV RENCO_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/renco (root-owned, read-only) so a runtime
...
# is seeded + chowned to the renco user by docker/stage2-hook.sh and lives
...
ENV RENCO_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> renco ...` they default to root, and any file the
# command writes under $RENCO_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/renco/bin/renco, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid renco`
# when invoked as root. Non-root callers (supervised processes,
# `--user renco`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/renco/.venv/bin/renco). See the shim source for
# the opt-out env var (RENCO_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/renco-exec-shim.sh /opt/renco/bin/renco
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/renco/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> renco ...` and any
# subprocess that doesn't activate the venv first still find renco.
#
# /opt/renco/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/renco/bin:/opt/renco/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. renco
# subcommand vs. no-args), drops to the renco user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/renco/docker/entrypoint-dispatch.sh" ]
CMD [ ]
