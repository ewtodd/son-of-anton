"""
Configuration management for Renco Agent.

Config files are stored in ~/.renco/ for easy access:
- ~/.renco/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.renco/.env         - API keys and secrets

This module provides:
- renco config          - Show current configuration
- renco config edit     - Open config in editor
- renco config get      - Print a resolved configuration value
- renco config set      - Set a specific value
- renco config unset    - Remove a user configuration value
- renco config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``renco config set`` (which rewrites
    ...
    ``config.yaml`` in place: renco never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.renco/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``renco logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  renco config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Renco makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Renco itself starts
...
#   on every plugin install / ``renco update``.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Renco runtime location "
            "(RENCO_HOME, RENCO_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # RENCO_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `renco plugins enable
    # observability/langfuse` or `renco tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "RENCO_LANGFUSE_ENV",
    "RENCO_LANGFUSE_RELEASE",
    "RENCO_LANGFUSE_SAMPLE_RATE",
    "RENCO_LANGFUSE_MAX_CHARS",
    "RENCO_LANGFUSE_CAPTURE",
    "RENCO_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "RENCO_ACP_AUTH_METHOD",
    "RENCO_ACP_AUTO_APPROVE",
    "RENCO_COPILOT_ACP_COMMAND",
    "RENCO_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from renco_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set RENCO_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("RENCO_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_renco_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # RENCO_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Renco is running in package-manager-managed mode.

    Two signals: the RENCO_MANAGED env var (set by the systemd service),
    or a .managed marker file in RENCO_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Renco through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``renco_cli/`` — i.e. the git checkout for source
    installs, ``/opt/renco`` inside the published image. It is a property of
    the running interpreter, NOT of ``$RENCO_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Renco was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$RENCO_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. RENCO_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.renco``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$RENCO_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.renco:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$RENCO_HOME``, a
    ...
    ``docker`` and ``renco update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/renco-agent`` image bakes a ``docker``
        stamp into ``/opt/renco`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $RENCO_HOME.
    try:
    ...
    #    host install whose shared $RENCO_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``renco update``.
    try:
        method = (
            (get_renco_home() / ".install_method")
    ...
    # detect Nix installs that don't set RENCO_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``renco_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$RENCO_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/renco`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``renco update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# renco_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Renco, it doesn't update
...
✗ ``renco update`` doesn't apply inside the Docker container.

Renco Agent runs as a published image (nousresearch/renco-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/renco-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate renco-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/renco-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/renco-agent/tags
  • Your config and session history live under ``$RENCO_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``renco update`` inside Docker.
...
def format_managed_message(action: str = "modify this Renco installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Renco installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Renco."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from RENCO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, renco_bin
    or None if container mode is not active, we're already inside the
    container, or RENCO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("RENCO_DEV") == "1":
        return None
    ...
    container_mode_file = get_renco_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "renco-agent")
    exec_user = info.get("exec_user", "renco")
    renco_bin = info.get("renco_bin", "/data/current-package/bin/renco")
...
# Re-export from renco_constants — canonical definition lives there.
from renco_constants import get_renco_home, get_process_renco_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_renco_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_renco_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_renco_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the RENCO_UID / RENCO_GID env vars set by Docker deployments.

    Docker containers running Renco commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level RENCO_HOME once, but
    subdirectories created at runtime by ``ensure_renco_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("RENCO_UID", "").strip()
    gid_str = os.environ.get("RENCO_GID", "").strip()
    try:
...
def _chown_to_renco_uid(path) -> None:
    """Chown ``path`` to ``RENCO_UID:RENCO_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_renco_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_renco_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the renco group can
    share state with the gateway service.

    The mode can be overridden via the RENCO_HOME_MODE environment variable
    (e.g. RENCO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse RENCO_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("RENCO_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("RENCO_CONTAINER") or os.environ.get("RENCO_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set RENCO_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into RENCO_HOME, upgrading legacy empty templates.
...
# ensure_renco_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_RENCO_HOME_ENSURED: set = set()


def ensure_renco_home():
    """Ensure ~/.renco directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``RENCO_HOME`` we only re-run
    ...
    ``get_renco_home()`` and therefore re-run for the new path.
    """
    home = get_renco_home()
    key = str(home)

    if key in _RENCO_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``renco profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old RENCO_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_renco_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _RENCO_HOME_ENSURED.add(key)


def _ensure_renco_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"RENCO_HOME {home} does not exist."
        )
...
from renco_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `renco config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.renco.config`` entries, then checks
    ...
    try:
        ...
        # should never break `renco update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Renco' own config writer has historically emitted it
    ...
    # Renco (``_save_discovered_models_to_config``), not hand-curated.
    # Older Renco versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Renco versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``renco_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "known_plugin_toolsets", # written/read by renco_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Renco won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, renco send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'renco doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "renco update / renco -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from renco_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ renco config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in renco_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from renco_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: renco config set <key> <value>")
    ...
    # `renco update`, but only the version bump is persisted.
    ...
    # metadata.renco.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.renco/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_renco_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``RencoCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``renco config set``
    ...
    backends) — while display paths (``renco status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the RENCO_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _renco_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in renco_cli.personality.
from renco_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`renco_cli.personality` — the single owner of
...
def render_personality_prompt(value: Any) -> str:
    ...
    from renco_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``RENCO_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`renco_cli.personality` (single owner).
    """
    from renco_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.renco/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_renco_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.renco/config.yaml.
    ...
    (which change ``RENCO_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Renco host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/renco/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_renco_dotenv()
...
#   openai-codex (OAuth — renco auth) — OpenAI Codex
#   nous         (OAuth — renco auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.renco/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Renco writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.renco/.env.
    ...
    interactive menu render (`renco tools`, `renco setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `renco tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.renco/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.renco/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``RENCO_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"RENCO_CUSTOM_{slug}_API_KEY" if slug else "RENCO_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.renco/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.renco/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Renco — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Renco vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.renco/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.renco/.env`` over ``os.environ``.

    Used for Renco-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Renco Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from renco_cli.auth import get_anthropic_key
    ...
    # Warn on stale RENCO_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("RENCO_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale RENCO_MAX_ITERATIONS={_env_ghost} "
                f"(run 'renco doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  renco config edit     # Edit config file", Colors.DIM))
    print(color("  renco config set <key> <value>", Colors.DIM))
    print(color("  renco setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``RENCO_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("RENCO_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `renco cron list`, then pin the "
        "intended values with `renco cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (renco_cli/plugins_cmd.py, renco_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `renco config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``renco config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``renco config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``renco config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (renco config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   renco config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   renco config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``renco config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``renco config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    renco config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    renco config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `renco config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_renco_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `renco config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `renco config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: renco config get <key> [--json]")
            print()
            print("Examples:")
            print("  renco config get model")
            print("  renco config get terminal.backend")
            print("  renco config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: renco config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  renco config set model anthropic/claude-sonnet-4")
            print("  renco config set terminal.backend docker")
            print("  renco config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: renco config unset <key>")
            print()
            print("Examples:")
            print("  renco config unset model")
            print("  renco config unset terminal.backend")
            print("  renco config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'renco config migrate' to add them")
        ...
        print("  renco config           Show current configuration")
        print("  renco config edit      Open config in editor")
        print("  renco config get <key>          Print a resolved config value")
        print("  renco config set <key> <value>   Set a config value")
        print("  renco config unset <key>        Remove a config value")
        print("  renco config check     Check for missing/outdated config")
        print("  renco config migrate   Update config with new options")
        print("  renco config path      Show config file path")
        print("  renco config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``renco config`` UI so users
