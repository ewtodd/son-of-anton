"""Tests for renco_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $RENCO_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Renco-managed node dir.
"""
...
def _make_renco_node(renco_home: Path) -> Path:
    """Create a fake $RENCO_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = renco_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    renco_home = fake_home / ".renco"
    _make_renco_node(renco_home)
    ...
    removed = uninstall.remove_node_symlinks(renco_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    renco_home = fake_home / ".renco"
    node_bin = _make_renco_node(renco_home)
