...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `renco send`."
---

# Pipe Script Output to Messaging Platforms

`renco send` is a small, scriptable CLI that pushes a message to any
messaging platform Renco is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | renco send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `renco
...
renco send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
renco send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
renco send --list

# Filter by platform
renco send --list telegram
```
...
Any platform Renco ships adapters for works as a target:
...
`renco send` resolves the message body in this order:

1. **Positional argument** — `renco send --to telegram "hi"`
2. **`--file PATH`** — `renco send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | renco send --to telegram`

When stdin is a TTY (no pipe), Renco does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  renco send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `renco send` reuses your Renco config, the same script works on
any host where Renco is installed — no need to export bot tokens into
...
`renco send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  renco send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | renco send \
...
```bash
./train.py --epochs 200 && \
  renco send --to telegram "training done" || \
  renco send --to telegram "training failed (exit $?)"
```
...
renco send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(renco send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `renco send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `renco send` calls
the platform's REST endpoint directly using credentials from
`~/.renco/.env` and `~/.renco/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `renco gateway start` and retry.
...
# Machine-readable
renco send --list --json
```

The listing is built from `~/.renco/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`renco gateway
...
| Approach | Multi-platform | Reuses Renco creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `renco send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`renco send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `renco send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `renco send` shares with cron delivery.
