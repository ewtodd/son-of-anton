"""Implementation of the ``son-of-anton bundles`` CLI subcommand.

Mirrors the structure of ``son_of_anton_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `son-of-anton bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  son-of-anton bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``son-of-anton bundles`` argparse tree.

    Called from ``son_of_anton_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `son-of-anton bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``son-of-anton bundles <subcommand>`` to the right handler."""
