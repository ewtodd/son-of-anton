...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Renco Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Renco terminal calls (desktop app flow only)
...
### Service Account (recommended for Renco)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${RENCO_HOME:-~/.renco}/.env` (the skill will prompt for this on first load).
...
## Renco Execution Pattern (desktop app flow)

Renco terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/renco-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/renco-op.sock"
