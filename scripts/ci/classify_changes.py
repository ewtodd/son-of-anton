...
# tree generates llms.txt — the index every LLM (Renco included, via the
# renco-agent skill) reads to learn what Renco can do — and
...
_MCP_CATALOG_FILES = {"renco_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Renco-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``renco serve`` backend, the Docker image)
