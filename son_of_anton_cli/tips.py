...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or son-of-anton -c.",
    ...
    "Ctrl+Z suspends Son of Anton to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "son-of-anton -c resumes your most recent CLI session. son-of-anton -c \"project name\" resumes by title.",
    "son-of-anton -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "son-of-anton -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "son-of-anton chat -t web,terminal enables only specific toolsets for a focused session.",
    "son-of-anton chat -s github-pr-workflow preloads a skill at launch.",
    "son-of-anton chat -q \"query\" runs a single non-interactive query and exits.",
    "son-of-anton chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "son-of-anton chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "son-of-anton --yolo bypasses all dangerous command approval prompts for the entire session.",
    "son-of-anton chat --source telegram tags the session for filtering in son-of-anton sessions list.",
    "son-of-anton -p work chat runs under a specific profile without changing your default.",
    ...
    "son-of-anton sessions prune --older-than 30 cleans up old sessions.",
    "son-of-anton skills search react --source skills-sh searches the skills.sh public directory.",
    "son-of-anton skills check scans installed hub skills for upstream updates.",
    "son-of-anton skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "son-of-anton skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "son-of-anton mcp add github --command npx adds MCP servers from the command line.",
    "son-of-anton mcp serve runs Son of Anton itself as an MCP server for other agents.",
    "son-of-anton auth add lets you add multiple API keys for credential pool rotation.",
    "son-of-anton completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "son-of-anton logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "son-of-anton backup creates a zip backup of your entire Son of Anton home directory.",
    "son-of-anton profile create coder creates an isolated profile that becomes its own command.",
    "son-of-anton profile create work --clone copies your current config and keys to a new profile.",
    "son-of-anton update syncs new bundled skills to ALL profiles automatically.",
    "son-of-anton gateway install sets up Son of Anton as a system service (systemd/launchd).",
    "son-of-anton memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "son-of-anton webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Son of Anton tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'son-of-anton profile create coder' creates the 'coder' command.",
    "son-of-anton profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Son of Anton prints a resume command with session ID and stats.",
    "son-of-anton sessions export backup.jsonl exports all sessions for backup or analysis.",
    "son-of-anton -r SESSION_ID resumes any specific past session by its ID.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: son-of-anton cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Son of Anton runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to Son of Anton via a local macOS server.",
    ...
    "Checkpoints use shadow repos in ~/.son-of-anton/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "son-of-anton chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "son-of-anton plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "The skills audit log at ~/.son-of-anton/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Son of Anton state via SON_OF_ANTON_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "SON_OF_ANTON_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Son of Anton auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Son of Anton config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Son of Anton is working.',
    ...
    "SON_OF_ANTON_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.son-of-anton/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.son-of-anton/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'son-of-anton curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "son-of-anton curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'son-of-anton curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    ...
    # --- TUI & Dashboard ---
    'SON_OF_ANTON_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    ...
    'son-of-anton -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'son-of-anton chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'son-of-anton chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'son-of-anton chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    ...
    'SON_OF_ANTON_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SON_OF_ANTON_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.son-of-anton/plugins/ — trust-gated by design.',
    "SON_OF_ANTON_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SON_OF_ANTON_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SON_OF_ANTON_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SON_OF_ANTON_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SON_OF_ANTON_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SON_OF_ANTON_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
