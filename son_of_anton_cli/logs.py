...
def tail_log(
    ...
    *,
    ...
    log_path = get_son_of_anton_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Son of Anton runs — try 'son-of-anton chat' first)")
        sys.exit(1)
    ...
    if component:
        from son_of_anton_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_son_of_anton_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_son_of_anton_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_son_of_anton_home()}/logs/")
        return

    print(f"Log files in {display_son_of_anton_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'son-of-anton chat' to generate logs)")
