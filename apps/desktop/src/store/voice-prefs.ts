...
import { getRencoConfigRecord, saveRencoConfig } from '@/renco'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getRencoConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveRencoConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
