"""Tests for _purge_stale_son_of_anton_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `son-of-anton update`.

Field failure (2026-08-20, Teknium's Linux box): `son-of-anton update` pulled a
checkout where son_of_anton_cli/gateway.py newly imports `line_input` from
son_of_anton_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from son_of_anton_cli.gateway
...
the Son of Anton package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Son of Anton modules from the cache; later
...
def test_purge_evicts_son_of_anton_prefixed_modules():
    victims = [
        "son_of_anton_cli.cli_output",
        "son_of_anton_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
        "agent.memory_store",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_son_of_anton_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_son_of_anton_modules()
    assert sys.modules.get("son_of_anton_cli.update_cmd") is update_cmd
    assert sys.modules.get("son_of_anton_cli.main") is cli_main
    assert "son_of_anton_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["son_of_anton_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_son_of_anton_modules()
    finally:
        sys.modules.pop("son_of_anton_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    name = "son_of_anton_cli.cli_output"
    ...
    try:
        ...
        try:
            from son_of_anton_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
