...
Renco runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Renco
...
1. Renco captures a baseline of current diagnostics for the file.
...
…). Renco auto-detects the binary on PATH or in
`<RENCO_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Renco at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<RENCO_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`renco lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Renco installs both packages together when you
run `renco lsp install typescript` or auto-install fires on first
use.

## CLI

```
renco lsp status          # service state + per-server install status
renco lsp list            # registry, optionally --installed-only
renco lsp install <id>    # eagerly install one server
renco lsp install-all     # try every server with a known recipe
renco lsp restart         # tear down running clients
renco lsp which <id>      # print resolved binary path
```

`renco lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <RENCO_HOME>/lsp/bin
...
When `install_strategy: auto`, Renco installs binaries into
`<RENCO_HOME>/lsp/bin/`. NPM packages land in
`<RENCO_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Renco-owned and is
...
**`renco lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<RENCO_HOME>/lsp/bin/`. Run
`renco lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `renco lsp status`**
...
When `renco lsp status` shows a `Backend warnings` section, install
...
`~/.renco/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.renco/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `renco lsp restart` to clear the set;
