...
Extracted verbatim from son_of_anton_state.py so the SessionDB mixin modules
(son_of_anton_state_search / son_of_anton_state_schema / son_of_anton_state_portability) can
reference them without importing son_of_anton_state (which would be a cycle).
son_of_anton_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``son-of-anton sessions optimize-storage``. A legacy DB sits at
...
# `son-of-anton sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `son-of-anton db optimize`. This is
