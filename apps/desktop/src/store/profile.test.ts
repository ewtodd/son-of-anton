...
const { getProfiles } = await import('@/renco')
...
const remoteConn = (over: Partial<RencoConnection> = {}): RencoConnection =>
  ({ baseUrl: 'https://renco-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as RencoConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { rencoDesktop: { getConnection } })
