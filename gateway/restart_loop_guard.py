...
Defenses 1 and 2 (the ``_RENCO_GATEWAY`` guard on ``renco gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.renco.gateway")``,
...
State lives in ``<RENCO_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_renco_home() / "gateway" / "restart_loop.json"
