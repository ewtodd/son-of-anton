# Renco Observer Hooks

Renco observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Renco' planner, model providers, memory, tool registry,
...
Renco also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "renco.observer.v1"
```

Hook callbacks are fail-open. Renco catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Renco only builds
...
The native NeMo Relay SDK integration maps Renco session, turn, LLM, and tool
