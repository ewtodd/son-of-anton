"""
son-of-anton fallback — manage the fallback provider chain.
...
Subcommands:
  son-of-anton fallback [list]   Show the current fallback chain (default when no subcommand)
  son-of-anton fallback add      Pick provider + model via the same picker as `son-of-anton model`,
                           then append the selection to the chain
  son-of-anton fallback remove   Pick an entry to delete from the chain
  son-of-anton fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.son-of-anton/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from son_of_anton_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `son-of-anton model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  son-of-anton fallback add")
        print()
        return
    ...
    print("  Docs: https://son-of-anton.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `son-of-anton model`, then append the selection to the chain."""
    from son_of_anton_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `son-of-anton model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `son-of-anton fallback list` to view, or `son-of-anton fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``son-of-anton fallback [subcommand]``."""
