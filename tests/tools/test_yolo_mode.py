"""Tests for --yolo (RENCO_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When RENCO_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("RENCO_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With RENCO_YOLO_MODE, dangerous commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """RENCO_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("RENCO_YOLO_MODE") is None or True  # no-op, documents intent
    ...
    def test_false_like_yolo_values_do_not_bypass_dangerous_command(self, monkeypatch, value):
        ...
        monkeypatch.setenv("RENCO_YOLO_MODE", value)
        monkeypatch.setenv("RENCO_INTERACTIVE", "1")
        monkeypatch.setenv("RENCO_SESSION_KEY", "test-session")
