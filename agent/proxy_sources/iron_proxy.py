...
* The ``iron-proxy`` binary is auto-installed into ``<renco_home>/bin/iron-proxy``
  on first use.  Renco pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<renco_home>/proxy/ca.{crt,key}`` is generated on
  first ``renco egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<renco_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``renco egress start``), pidfile
  at ``<renco_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<renco_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``renco egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<renco_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "RENCO_IRON_PROXY_MGMT_KEY"
...
# credential (Renco' auth.py keys Google on both GEMINI_API_KEY and
...
def _renco_bin_dir() -> Path:
    from renco_constants import get_renco_home

    return get_renco_home() / "bin"
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.renco/proxy/`` just to
    check whether a pidfile exists.
    """
    from renco_constants import get_renco_home

    return get_renco_home() / "proxy"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<renco_home>/bin/iron-proxy``  (our managed copy — preferred)
    ...
    managed = _renco_bin_dir() / _platform_binary_name()
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``renco proxy install`` surface lets them
    propagate so the wizard can show a clear error.
    """

    bin_dir = _renco_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="renco-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "renco-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``renco egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="renco-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
...
def mint_proxy_token(prefix: str = "renco-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<renco_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``RENCO_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``renco egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="renco-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `renco egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `renco egress setup` and use "
            "`renco egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `renco egress setup`, "
            "then `renco egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``renco egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `renco egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<renco_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a Renco dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``renco egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``renco egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_RENCO_IRON_PROXY_NONCE_ENV = "RENCO_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_RENCO_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `renco egress install`."
        )
    ...
    env[_RENCO_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the Renco
    # process.
    #
    ...
    # ``renco egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``renco egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `renco egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`renco egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `renco secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
