...
def _seed_config(tmp_path, monkeypatch):
    hh = tmp_path / ".son-of-anton"
    hh.mkdir()
    (hh / "config.yaml").write_text("display:\n  show_reasoning: true\n")
    monkeypatch.setenv("SON_OF_ANTON_HOME", str(hh))
    # cli captures _son_of_anton_home at import; force it to the temp home.
    import cli

    monkeypatch.setattr(cli, "_son_of_anton_home", hh, raising=False)
    return hh
