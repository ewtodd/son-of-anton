"""Regression for #86721 — a one-shot `son-of-anton cron run` invocation's
...
def test_recover_interrupted_executions_reaps_the_stale_claim_from_a_dead_process(
    ...
    # exactly what a subsequent `son-of-anton cron run` invocation would trigger
...
def test_try_dispatch_background_run_calls_recovery_before_claiming(monkeypatch):
    ...
    # caller ("son-of-anton cron run", tests) early return documented just
