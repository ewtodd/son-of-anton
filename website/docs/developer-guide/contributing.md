...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Renco Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Renco Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Renco core? Start with [Build a Renco Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Renco itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Renco instance (throwaway RENCO_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m renco_cli.main
scripts/dev-sandbox.sh --persistent python -m renco_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.renco/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.renco` — use `get_renco_home()` from `renco_constants` for code paths and `display_renco_home()` for user-facing messages. See [AGENTS.md](https://github.com/ewtodd/renco/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Renco can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Renco to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/ewtodd/renco/blob/main/LICENSE).
