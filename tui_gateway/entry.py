...
# from shadowing Renco's own top-level modules.  ``renco_bootstrap`` lives at
...
import renco_bootstrap

renco_bootstrap.harden_import_path()
...
# shared owner in renco_cli.mcp_startup — the wait/in-flight/join helpers
...
# renco_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `RENCO_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("RENCO_TUI_SIDECAR_URL")
...
# tested; override via ``RENCO_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("RENCO_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``RENCO_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``renco --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``renco_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from renco_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → renco_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's RENCO_HOME first), so a launch profile with no
    ...
    try:
        from renco_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``renco --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``renco dashboard`` spawn theirs via
    ``renco_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``renco_cli.mcp_startup`` thread used by the
    ...
    try:
        from renco_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from renco_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's RENCO_HOME override — the shared owner in
    ``renco_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins Renco activates mid-conversation.
