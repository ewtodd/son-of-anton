...
* ``urllib``-based ``/models`` catalog discovery probe
  (``son_of_anton_cli.models.probe_api_models`` via ``_custom_provider_ssl_context``).
...
from son_of_anton_cli.models import _custom_provider_ssl_context
...
class TestResolveRequestsVerifyProviderScoped:
    ...
    def test_no_base_url_does_not_consult_config(self, clean_env, bundle_file):
        ...
        with patch("son_of_anton_cli.config.get_compatible_custom_providers", probe):
