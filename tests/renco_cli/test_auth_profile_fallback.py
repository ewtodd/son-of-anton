...
When ``RENCO_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.renco/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.renco            (has its own auth.json fixture)
    * Profile     -> tmp_path/.renco/profiles/coder   (active, RENCO_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Renco is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (RENCO_HOME != ~/.renco, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    renco_home = tmp_path / "classic"
    renco_home.mkdir()
