...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/productivity/shopify` |
...
4. Save to `${RENCO_HOME:-~/.renco}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Renco","productType":"Apparel","tags":["test"]}}'
```
