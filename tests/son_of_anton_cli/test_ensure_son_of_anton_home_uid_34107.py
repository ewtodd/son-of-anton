"""Regression tests for #34107 — Docker UID/GID handling in ensure_son_of_anton_home.

When Son of Anton runs in Docker with ``SON_OF_ANTON_UID=1000`` / ``SON_OF_ANTON_GID=911``,
the entrypoint chowns the top-level ``SON_OF_ANTON_HOME`` once at startup. But
...
The fix is a ``_chown_to_son_of_anton_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_son_of_anton_uid_gid
# ---------------------------------------------------------------------------


class TestResolveSonOfAntonUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("SON_OF_ANTON_UID", "1000")
        monkeypatch.setenv("SON_OF_ANTON_GID", "911")
        from son_of_anton_cli.config import _resolve_son_of_anton_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_son_of_anton_uid
# ---------------------------------------------------------------------------


class TestChownToSonOfAntonUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_son_of_anton_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_son_of_anton_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SON_OF_ANTON_UID", raising=False)
        monkeypatch.delenv("SON_OF_ANTON_GID", raising=False)
        from son_of_anton_cli import config as cfg
