...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Renco gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.rencoDesktop
  Object.defineProperty(window, 'rencoDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'rencoDesktop', { configurable: true, value: original })
}
