"""Tests for the Command Installation check in renco doctor."""
...
import renco_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal RENCO_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".renco"
    ...
    renco_bin = venv_bin_dir / "renco"
    renco_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    renco_bin.chmod(0o755)
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, renco_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "renco"
        wrong_target = tmp_path / "wrong_renco"
        ...
        assert cmd_link.resolve() == renco_bin.resolve()
