...
When running Renco inside Docker, the default `network_mode: host` gives the
...
The Renco [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   RENCO_UID=$(id -u) RENCO_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=renco,renco-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://renco-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Renco trust model and vulnerability reporting
