...
billing screens drive. Companion to ``son_of_anton_cli/nous_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Son of Anton already holds for inference
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``SON_OF_ANTON_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → the
...
# son_of_anton_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``SON_OF_ANTON_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("SON_OF_ANTON_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nous Portal — run `son-of-anton portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    try:
        from son_of_anton_cli.auth import AuthError, resolve_nous_access_token
    except ImportError:
