// Launching the Renco TUI in the user's OWN terminal emulator.
//
...
// `renco --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Renco runtime is often a venv Python invoked
//     as `python -m renco_cli.main`, not a `renco` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Renco
