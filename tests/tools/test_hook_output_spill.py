...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("renco_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="renco-spill-test-")
    ...
    def test_default_directory_uses_renco_home(self):
        """When no directory override, spill under RENCO_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="renco-home-")
        try:
            with patch.dict(os.environ, {"RENCO_HOME": test_home}):
                # Also patch get_renco_home to the env var to mirror production.
            ...
            # ~/.renco/hook_outputs depending on get_renco_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.renco/hook_outputs/sess")),
            ]
