...
``son_of_anton_cli.env_loader._apply_external_secret_sources()``.
...
def get_source(name: str, *, scope: Optional[str] = None) -> Optional[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        return _SCOPED_SOURCES.get(scope or son_of_anton_home_key(), {}).get(
            name
...
def list_sources(*, scope: Optional[str] = None) -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(scope or son_of_anton_home_key(), {}))
...
def list_plugin_sources() -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(son_of_anton_home_key(), {}))
        return list(merged.values())
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``son-of-anton`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's SON_OF_ANTON_HOME is ``~/.son-of-anton/profiles/<name>``; the
    default profile (``~/.son-of-anton``) returns "".
    """
    if home_path is not None:
        resolved = Path(home_path)
        if resolved.parent.name == "profiles" and resolved.name:
            return resolved.name
    for env_name in ("SON_OF_ANTON_PROFILE_NAME", "SON_OF_ANTON_PROFILE"):
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(
        secrets_cfg, scope=son_of_anton_home_key(home_path)
    )
    if not enabled:
        return report
