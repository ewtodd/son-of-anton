...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "son-of-anton-gateway"
    ...
    def test_get_running_pid_cached_invalidates_when_pid_file_changes(self, tmp_path, monkeypatch):
        ...
        def _write_record(pid: int, start_time: int) -> None:
            record = {
                ...
                "argv": ["python", "-m", "son_of_anton_cli.main", "gateway"],
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level SON_OF_ANTON_HOME.
        ...
        the process-level SON_OF_ANTON_HOME, not the profile's directory.  See #56986.
        """
        ...
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(process_home))
...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``son-of-anton gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "son-of-anton gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "son-of-anton -p coder gateway run --replace",
            "/opt/son-of-anton/.venv/bin/son-of-anton --profile coder gateway run --replace",
            "son_of_anton_home=/opt/data/profiles/coder son-of-anton gateway run --replace",
        ):
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        """A Windows argv renders SON_OF_ANTON_HOME with backslashes while the
        ...
        cmdline = r"son_of_anton_home=c:\opt\data\profiles\coder son-of-anton gateway run --replace"
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SON_OF_ANTON_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "son-of-anton-gateway",
            "argv": ["/Users/user/.son-of-anton/son-of-anton/son_of_anton_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_self_reacquires_when_disk_start_time_null(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "son-of-anton-gateway",
            "argv": ["son_of_anton_cli/main.py", "--profile", "milena", "gateway", "run", "--replace"],
    ...
    def test_acquire_scoped_lock_self_reacquires_when_start_times_differ(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 111,
            "kind": "son-of-anton-gateway",
            "argv": ["son_of_anton_cli/main.py", "gateway", "run", "--replace"],
            "scope": "discord-bot-token",
        }))
    ...
    def test_acquire_scoped_lock_stamps_profile_label(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SON_OF_ANTON_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_path / "profiles" / "lead-gen-outreach"))
    ...
    def test_acquire_scoped_lock_omits_profile_when_not_inferable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SON_OF_ANTON_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_path / "some-custom-dir"))
...
class TestScopedLockOwnerLabel:
    ...
    def test_profile_label_for_root_home_is_default(self, monkeypatch):
        monkeypatch.setenv("SON_OF_ANTON_HOME", "/opt/data")
    ...
    def test_owner_label_prefers_explicit_profile_field(self):
        record = {"profile": "coder", "son_of_anton_home": "/opt/data/profiles/other"}
    ...
    def test_owner_label_ignores_invalid_profile_and_uses_home(self):
        ...
        # son_of_anton_home fallback — attribution degrades, never corrupts.
        record = {
            "profile": "bad; rm -rf /",
            "son_of_anton_home": "/opt/data/profiles/zerocool",
        }
        assert status.scoped_lock_owner_label(record) == "zerocool"

    def test_owner_label_falls_back_to_son_of_anton_home(self):
        # Locks written before the profile field existed still attribute.
        record = {"pid": 559, "son_of_anton_home": "/opt/data/profiles/zerocool"}
        assert status.scoped_lock_owner_label(record) == "zerocool"
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_son_of_anton_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's SON_OF_ANTON_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
        ...
        payload = json.loads((tmp_path / ".gateway-takeover.json").read_text())
        assert payload["replacer_son_of_anton_home"] == str(tmp_path)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        SON_OF_ANTON_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.son-of-anton flap
        ...
        marker_path.write_text(json.dumps({
            ...
            "replacer_pid": 99999,
            "replacer_son_of_anton_home": str(tmp_path / "profiles" / "other"),
    ...
    def test_consume_accepts_legacy_marker_without_son_of_anton_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Son of Anton versions have no
        ``replacer_son_of_anton_home`` field; an absent field is treated as same-home so
...
class TestScopedLockTakeover:
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        def terminate(pid, *, force=False):
            ...
            assert payload["target_son_of_anton_home"] == str(target_home)
            assert payload["replacer_son_of_anton_home"] == str(replacer_home)
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``son-of-anton gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00son_of_anton_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "son_of_anton_cli/main.py" in result
        assert calls == ["proc"]
...
class TestLaunchdPlistRespawnGovernance:
    def test_plist_has_throttle_interval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_path))
        from son_of_anton_cli.gateway import generate_launchd_plist
...
class TestResolveGatewayLiveness:
    ...
    def test_profile_dir_scopes_every_read_to_that_profile(self, tmp_path):
        ...
        ignore the SON_OF_ANTON_HOME contextvar override (#56986), so the profile
