...
A pre-run snapshot of ``~/.son-of-anton/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.son-of-anton/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.son-of-anton/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_son_of_anton_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.son-of-anton/cron/jobs.json``)."""
    return get_son_of_anton_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.son-of-anton/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.son-of-anton/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.son-of-anton/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.son-of-anton/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `son-of-anton curator rollback --list` to see available snapshots)",
            None,
        )
