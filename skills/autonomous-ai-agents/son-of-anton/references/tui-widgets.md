...
Author widget apps for the Son of Anton TUI (`son-of-anton --tui`): glanceable ambient
...
- The TUI must be in use (`son-of-anton --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.son-of-anton/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.son-of-anton/logs/tui_gateway_crash.log` if a widget never appears.
