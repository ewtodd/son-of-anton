"""Tests for renco_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
def receipt_home(tmp_path, monkeypatch):
    """Isolated RENCO_HOME for receipt writes."""
    home = tmp_path / ".renco"
    home.mkdir()
    monkeypatch.setattr(
        "renco_cli.config.get_renco_home", lambda: home, raising=False
    )
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["renco-gateway"],
        ...
        assert gr["restarted_services"] == ["renco-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another renco.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "renco.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(renco_main, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            renco_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            renco_main,
        ...
        monkeypatch.setattr(
            renco_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import renco_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "renco_cli.profiles._get_default_renco_home", lambda: home
        )
        monkeypatch.setattr(
            "renco_cli.profiles._get_profiles_root",
    ...
    def test_matrix_returns_true_only_on_stale(self, capsys):
        ...
        assert "STALE" in out
        assert "renco -p <profile> gateway restart" in out
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("renco_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
