...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"son-of-anton[termux]"' in text
    assert '"son-of-anton[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"son-of-anton[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
