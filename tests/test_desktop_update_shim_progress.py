...
def progress(tmp_path):
    ...
    status = tmp_path / "renco-update-status"
...
# Stands in for `renco update`, and reports the stage that was on screen
...
n="$(cat "$RENCO_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$RENCO_TEST_CALLS"
for f in "$TMPDIR"/renco-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$RENCO_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$RENCO_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "renco-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    renco = install_root / "venv" / "bin" / "renco"
    renco.write_text(FAKE_RENCO)
    renco.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "RENCO_TEST_CAPTURE": str(capture),
        "RENCO_TEST_CALLS": str(calls),
        "RENCO_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".renco-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`renco update` is the longest wait in the hand-off and the one the
