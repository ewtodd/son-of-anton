...
export function markerPath(rencoHome) {
  return path.join(rencoHome, '.renco-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(rencoHome)
  let raw
 ...
 *
 * The Tauri-based renco-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(rencoHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(rencoHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Renco to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(rencoHome, opts)
