...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Renco E2E'], { cwd: repo })
...
function configureRepoCwd(rencoHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(rencoHome, mockUrl)
  fs.appendFileSync(path.join(rencoHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(rencoHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.rencoHome, mock.url, repo)
