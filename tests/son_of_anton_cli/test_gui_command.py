"""Tests for ``son-of-anton gui`` desktop launcher wiring."""
...
from son_of_anton_cli import main as cli_main
...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    call live, and on a Linux dev box it wrote a ``son-of-anton.desktop`` pointing
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("SON_OF_ANTON_DESKTOP_PASSWORD_STORE", raising=False)
...
def _make_packaged_executable(root: Path, monkeypatch) -> Path:
    ...
    if sys.platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Son of Anton.app" / "Contents" / "MacOS" / "Son of Anton"
    elif sys.platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Son of Anton.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "son-of-anton"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("son_of_anton_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("son_of_anton_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("son_of_anton_cli.main._desktop_build_needed", return_value=True), \
         patch("son_of_anton_cli.main._write_desktop_build_stamp"), \
         patch("son_of_anton_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("son_of_anton_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("son_of_anton_cli.main._register_linux_desktop_entry"), \
         patch("son_of_anton_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Son of Anton-managed
    Node ahead of that bare PATH or the install dies with ``node: not found``.
    """
    import os

    from son_of_anton_constants import iter_son_of_anton_node_dirs

    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    # A managed Node tree on disk so with_son_of_anton_node_path() actually prepends it.
    home = tmp_path / "son-of-anton-home"
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["son-of-anton"], 0)
    ...
    with patch("son_of_anton_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("son_of_anton_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("son_of_anton_cli.main._desktop_build_needed", return_value=True), \
         patch("son_of_anton_cli.main._write_desktop_build_stamp"), \
         patch("son_of_anton_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("son_of_anton_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("son_of_anton_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(skip_build=False))

    managed_dirs = [str(p) for p in iter_son_of_anton_node_dirs() if p.is_dir()]
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Son of Anton.app/Contents/MacOS/Son of Anton in place. Re-downloading Electron can't
    ...
    with patch("son_of_anton_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("son_of_anton_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("son_of_anton_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("son_of_anton_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("son_of_anton_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("son_of_anton_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Son of Anton.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "Son of Anton.app"
    _write_info_plist(app, "com.nousresearch.son-of-anton")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Son of Anton").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "Son of Anton Helper.app"
    _write_info_plist(helper, "com.nousresearch.son-of-anton.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Son of Anton.app component, silently skipping every .node/.dylib/crashpad
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`son-of-anton desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch)

    registered: list[Path] = []
    monkeypatch.setattr("son_of_anton_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "son_of_anton_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: registered.append(project_root) or (tmp_path / "son-of-anton.desktop"),
    )

    launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)

    with patch("son_of_anton_cli.main._desktop_build_needed", return_value=False), \
         patch("son_of_anton_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("son_of_anton_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("son_of_anton_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("son_of_anton_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("son_of_anton_cli.linux_desktop_entry.install_desktop_entry", boom)
    ...
    with patch("son_of_anton_cli.main._desktop_build_needed", return_value=False), \
         patch("son_of_anton_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("son_of_anton_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("son_of_anton_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("son_of_anton_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("son_of_anton_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_desktop_launch_options_normalizes_password_store(raw, expected):
    ...
    with patch("son_of_anton_cli.config.load_config", return_value=cfg):
...
def _clear_keychain_env(monkeypatch):
    for var in (
        ...
        "SON_OF_ANTON_DESKTOP_PASSWORD_STORE",
    ):
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("son_of_anton_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert cli_main._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("son_of_anton_cli.main.subprocess.run", return_value=ping_fail):
        assert cli_main._detect_linux_password_store() is None
    with patch("son_of_anton_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert cli_main._detect_linux_password_store() is None
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("son_of_anton_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("son_of_anton_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("son_of_anton_cli.main._desktop_build_needed", return_value=True), \
         patch("son_of_anton_cli.main._write_desktop_build_stamp"), \
         patch("son_of_anton_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("son_of_anton_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("son_of_anton_cli.config.load_config", return_value={}), \
         patch("son_of_anton_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("son_of_anton_cli.main._detect_linux_password_store", return_value="gnome-libsecret"), \
         patch("son_of_anton_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SON_OF_ANTON_DESKTOP_PASSWORD_STORE"] == "gnome-libsecret"
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("son_of_anton_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("son_of_anton_cli.config.load_config", return_value={}), \
         patch("son_of_anton_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("son_of_anton_cli.main._detect_linux_password_store", return_value="kwallet6"), \
         patch("son_of_anton_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SON_OF_ANTON_DESKTOP_PASSWORD_STORE"] == "kwallet6"
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with patch("son_of_anton_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("son_of_anton_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("son_of_anton_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("son_of_anton_cli.config.load_config", return_value=cfg), \
         patch("son_of_anton_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("son_of_anton_cli.main._detect_linux_password_store") as mock_detect, \
         patch("son_of_anton_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_explicit_password_store_env_wins_over_config_and_detection(tmp_path, monkeypatch):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("SON_OF_ANTON_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SON_OF_ANTON_DESKTOP_PASSWORD_STORE"] == "basic"
...
def test_gui_password_store_bridge_is_linux_only(tmp_path, monkeypatch):
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert "SON_OF_ANTON_DESKTOP_PASSWORD_STORE" not in launch_env
