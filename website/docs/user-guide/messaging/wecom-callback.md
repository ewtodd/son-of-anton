...
Connect Renco to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Renco supports two WeCom integration modes:
...
> Run `renco gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Renco decrypts the message, queues it for the agent
...
(Use `renco gateway start` only after `renco gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Renco and the token the
...
in `~/.renco/.env` values around `=` will also break signature checks. After
fixing, restart `renco gateway run`.
...
Check `renco gateway run` logs for the bound host/port. If the adapter bound to
