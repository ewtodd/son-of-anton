...
# boundary, blocking `renco send` message bodies, multi-line
# `git commit -m` messages, and heredoc text that merely MENTION
# shutdown/reboot commands.
_QUOTED_NEWLINE_DATA_ALLOW = [
    # renco send with a multi-line message body (the reported symptom)
    'renco send -t telegram -s "spark1" "console output:\nsudo reboot\ndone"',
    'renco send -t telegram "line1\nshutdown -h now\nline3"',
...
_QUOTED_NEWLINE_THREATS_BLOCK = [
    ...
    'renco send -t telegram "$(sudo reboot)"',
    'echo "`shutdown -h now`"',
...
def test_quoted_newline_data_not_blocked_by_full_guard_chain(clean_session):
    ...
    command = 'renco send -t telegram "status:\nsudo reboot happened at 3am"'
...
def test_yolo_env_var_cannot_bypass_hardline(clean_session, monkeypatch):
    """RENCO_YOLO_MODE=1 must not bypass the hardline floor."""
...
def test_approvals_mode_off_cannot_bypass_hardline(clean_session, monkeypatch, tmp_path):
    ...
    # _get_approval_mode() reads from renco config; simplest path: monkeypatch the helper.
