...
renco kanban watch --tenant <project-slug>

# Snapshot of the board
renco kanban list --tenant <project-slug>
renco kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
renco kanban stats --tenant <project-slug>

# Visual dashboard (browser)
renco dashboard

# Inspect a specific task (includes comments + events)
renco kanban show <task-id>

# Follow a single task's event stream
renco kanban tail <task-id>
```

Verify available subcommands with `renco kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `renco kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `renco kanban block <id>` then `renco kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `renco kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `renco kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `renco profile list` (profile exists?), `renco status` (gateway/dispatcher up?), and verify tenant. |
...
renco kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
renco kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(renco kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
renco kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
renco kanban block <task-id>
renco kanban archive <task-id>

# Diagnose what happened
renco kanban show <task-id>      # task body, comments, recent events
renco kanban tail <task-id>      # follow the live event stream
renco kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `renco kanban create` for the director
...
while true; do
    clear
    renco kanban list --tenant <slug>
    echo "---"
    renco kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `renco kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `renco profile list` and start
