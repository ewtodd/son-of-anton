...
* The ``iron-proxy`` binary is auto-installed into ``<son_of_anton_home>/bin/iron-proxy``
  on first use.  Son of Anton pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<son_of_anton_home>/proxy/ca.{crt,key}`` is generated on
  first ``son-of-anton egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<son_of_anton_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``son-of-anton egress start``), pidfile
  at ``<son_of_anton_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<son_of_anton_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``son-of-anton egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<son_of_anton_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "SON_OF_ANTON_IRON_PROXY_MGMT_KEY"
...
# credential (Son of Anton' auth.py keys Google on both GEMINI_API_KEY and
...
def _son_of_anton_bin_dir() -> Path:
    from son_of_anton_constants import get_son_of_anton_home

    return get_son_of_anton_home() / "bin"
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.son-of-anton/proxy/`` just to
    check whether a pidfile exists.
    """
    from son_of_anton_constants import get_son_of_anton_home

    return get_son_of_anton_home() / "proxy"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<son_of_anton_home>/bin/iron-proxy``  (our managed copy — preferred)
    ...
    managed = _son_of_anton_bin_dir() / _platform_binary_name()
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``son-of-anton proxy install`` surface lets them
    propagate so the wizard can show a clear error.
    """

    bin_dir = _son_of_anton_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="son-of-anton-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "son-of-anton"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``son-of-anton egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="son-of-anton-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
        ...
        subprocess.run(  # noqa: S603
            [
                ...
                "-key", str(tmp_key),
                "-sha256", "-days", "3650",
                "-subj", "/CN=son-of-anton iron-proxy CA",
...
def mint_proxy_token(prefix: str = "son-of-anton-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<son_of_anton_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``SON_OF_ANTON_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``son-of-anton egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="son-of-anton-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `son-of-anton egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `son-of-anton egress setup` and use "
            "`son-of-anton egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `son-of-anton egress setup`, "
            "then `son-of-anton egress restart`."
        )
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "run `son-of-anton egress restart`."
            ) from exc
        raise RuntimeError(
        ...
        raise RuntimeError(
            ...
            "`son-of-anton egress restart` once."
        ) from exc
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``son-of-anton egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `son-of-anton egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<son_of_anton_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a Son of Anton dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``son-of-anton egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``son-of-anton egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_SON_OF_ANTON_IRON_PROXY_NONCE_ENV = "SON_OF_ANTON_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_SON_OF_ANTON_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `son-of-anton egress install`."
        )
    ...
    if not cfg.exists():
        raise RuntimeError(
            ...
            "Run `son-of-anton egress setup` first."
        )
    ...
    # Son of Anton process.
    _proxy_nonce = hashlib.sha256(os.urandom(16)).hexdigest()
    env[_SON_OF_ANTON_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the Son of Anton
    # process.
    #
    ...
    # ``son-of-anton egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``son-of-anton egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `son-of-anton egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`son-of-anton egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `son-of-anton secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
