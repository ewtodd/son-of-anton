...
Let the renco agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where renco runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `renco meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `renco meet` CLI |
...
| `cli.py` | `renco meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$RENCO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `renco meet node {run,list,approve,remove,status,ping}` |
...
renco meet install                                      # pip + Chromium
renco meet setup                                        # preflight
renco meet auth                                         # optional
renco meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
renco meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.renco/.env
renco meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
renco meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
renco meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.renco/.env
renco meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, renco will **not** switch your system audio input automatically — the
...
renco plugins enable google_meet
renco meet node run --display-name my-mac --host 0.0.0.0 --port 18789
