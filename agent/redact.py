...
_REDACT_ENABLED = os.getenv("SON_OF_ANTON_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Son of Anton — used by
    ``son-of-anton config``, ``son-of-anton status``, ``son-of-anton dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical son-of-anton log line
...
# (``security.redact_secrets: false`` / SON_OF_ANTON_REDACT_SECRETS) applies to
