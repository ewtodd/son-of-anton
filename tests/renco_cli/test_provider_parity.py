...
the SAME provider universe as ``renco model`` (the CLI/TUI picker).
...
def test_every_renco_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `renco model` universe."""
    ...
    assert not missing, (
        "providers shown in `renco model` but not configurable in the desktop "
