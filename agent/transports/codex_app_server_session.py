...
Owns one Codex thread per Renco session. Drives `turn/start`, consumes
...
# Renco' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_RENCO_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Renco/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Renco session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _RENCO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("RENCO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="renco",
            client_title="Renco Agent",
            client_version=_get_renco_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Renco' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Renco' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own renco-tools callback we
            # auto-accept — the user already approved Renco' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Renco
        approval-mode/timeout logic. The Renco-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Renco approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Renco approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Renco returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Renco approval mode/timeout resolution
    ...
    # "prompt expired"; the Renco-side messaging already distinguishes them.
    return "decline"
...
def _get_renco_version() -> str:
    """Best-effort Renco version string for codex's userAgent line."""
    try:
