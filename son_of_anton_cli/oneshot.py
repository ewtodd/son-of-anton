...
configured for "cli" in `son-of-anton tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (SON_OF_ANTON_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `son-of-anton chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"son-of-anton -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "son-of-anton -z: --toolsets all enables every toolset; "
    ...
    if not valid:
        return None, "son-of-anton -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    env_model_early = os.getenv("SON_OF_ANTON_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "son-of-anton -z: --provider requires --model (or SON_OF_ANTON_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"son-of-anton -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("son-of-anton -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``son-of-anton -z`` / oneshot mode.

    Oneshot bypasses ``SonOfAntonCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when son-of-anton is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from son_of_anton_cli.config import load_config
    from son_of_anton_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("SON_OF_ANTON_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from son_of_anton_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("SON_OF_ANTON_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # SonOfAntonCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    from son_of_anton_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                SON_OF_ANTON_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via SON_OF_ANTON_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via SON_OF_ANTON_YOLO_MODE=1
