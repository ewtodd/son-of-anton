"""
Renco Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.renco/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Renco PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing renco-agent or renco PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Renco Agent" comment and following line
                if '# Renco Agent' in line or '# renco-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('renco' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing renco
                if 'renco' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Renco-owned file, and
...
def remove_wrapper_script():
    """Remove the renco wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "renco",
        Path.home() / ".local" / "bin" / "renco-acp",
        Path.home() / ".local" / "bin" / "renco-agent",
        Path("/usr/local/bin/renco"),
        Path("/usr/local/bin/renco-acp"),
        Path("/usr/local/bin/renco-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains renco_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'renco_cli' in content or 'renco-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(renco_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``renco`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Renco home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (renco_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``renco gateway run`` processes
    ...
    try:
        from renco_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from renco_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``RENCO_HOME`` and ``RENCO_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\renco\git\cmd``, ``%LOCALAPPDATA%\renco\git\bin``,
#      ``%LOCALAPPDATA%\renco\git\usr\bin``, ``%LOCALAPPDATA%\renco\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\renco\git\`` and Node to
#      ``%LOCALAPPDATA%\renco\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``renco dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\renco\gateway-service\`` and sometimes
...
def _renco_path_markers(renco_home: Path) -> list[str]:
    """Path-entry substrings that identify Renco-owned User-PATH entries."""
    root = str(renco_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare renco-agent install dir.
    markers = [root + "\\renco-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if RENCO_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "renco".  We don't want to catch
    # unrelated entries like "crenco-foo" or "ephermeral", so we look for
    # backslash-renco as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(renco_home: Path) -> list[str]:
    """Strip Renco-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _renco_path_markers(renco_home)
            kept: list[str] = []
...
def remove_renco_env_vars_windows() -> list[str]:
    """Delete RENCO_HOME and RENCO_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("RENCO_HOME", "RENCO_GIT_BASH_PATH"):
                try:
...
def _is_default_renco_home(renco_home: Path) -> bool:
    """Return True when ``renco_home`` points at the default (non-profile) root."""
    try:
        from renco_constants import get_default_renco_root
        return renco_home.resolve() == get_default_renco_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its RENCO_HOME directory.

    We shell out to ``renco -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current RENCO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m renco_cli.main` so we don't depend on a `renco`
    #    wrapper that may be half-removed mid-uninstall.
    renco_invocation = [_sys.executable, "-m", "renco_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                renco_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's RENCO_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``renco uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$RENCO_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from renco_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(renco_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Renco Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Renco Chat GUI installation was found.")
        print(f"  Checked: {renco_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Renco agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(renco_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Renco agent at {renco_home / 'renco-agent'}")
        print(f"  • Your config, sessions, and secrets under {renco_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Renco agent is still installed. Run 'renco' to use the CLI,")
    print("or 'renco uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.renco/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.renco/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie RENCO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_renco_home(renco_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.renco data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Renco Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {renco_home / 'config.yaml'}")
    print(f"  Secrets: {renco_home / '.env'}")
    print(f"  Data:    {renco_home / 'cron/'}, {renco_home / 'sessions/'}, {renco_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their RENCO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Renco data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Renco code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, renco_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Renco PATH entries from shell configs / Windows User PATH")
    print("  • Renco wrapper scripts and Renco-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Renco config/data: {renco_home}")
        if _is_default_renco_home(renco_home):
        ...
        print(f"  • Keep Renco config/data: {renco_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``renco`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$RENCO_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in renco_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\renco\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(renco_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Renco-owned PATH entries in User environment")

        log_info("Removing RENCO_HOME / RENCO_GIT_BASH_PATH User env vars...")
        removed_env = remove_renco_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Renco-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing renco command...")
    ...
    #     (only when they still point into this Renco home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Renco-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(renco_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Renco-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(renco_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE RENCO_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from renco_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(renco_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.renco/, just remove the renco-agent subdir
            if renco_home in project_root.parents or project_root.parent == renco_home:
    ...
    #     under RENCO_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(renco_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(renco_home)
    ...
    # 5. Optionally remove ~/.renco/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile RENCO_HOME dirs live under
        ...
        try:
            if renco_home.exists():
                shutil.rmtree(renco_home)
                log_success(f"Removed {renco_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {renco_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {renco_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://renco-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://renco-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Renco Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m renco_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import renco_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``renco_constants`` + ``renco_cli.colors``
    (and lazily ``renco_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m renco_cli.uninstall")
    parser.add_argument(
        "--mode",
