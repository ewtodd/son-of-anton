...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.son-of-anton/.env``."""
...
def _print_python_dependencies(manifest: dict, console) -> None:
    ...
    Declaration seam ONLY — Son of Anton never auto-installs plugin pip
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from son_of_anton_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `son-of-anton plugins search <term>` to "
...
def cmd_install(
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `son-of-anton plugins enable {installed_name}` to activate.[/dim]",
        )
    ...
    console.print("[dim]  son-of-anton gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`son-of-anton plugins install {recorded_source} --force "
        ...
        if allowed is not True:
            ...
            if scan_result.verdict == "dangerous":
                ...
                console.print(
                    ...
                    f"findings, then re-enable with `son-of-anton plugins enable {name}` "
                    f"if you trust them.",
                )
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``son-of-anton setup`` / ``son-of-anton plugins disable basic`` can park the plugin
...
def cmd_capabilities(name: Optional[str] = None) -> None:
    """``son-of-anton plugins capabilities [<id>]`` — declared vs granted."""
    from rich.console import Console

    from son_of_anton_cli.plugin_capabilities import granted_capabilities
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``son-of-anton plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] son-of-anton plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] son-of-anton plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] son-of-anton plugins")
    console.print("[dim]Enable/disable:[/dim] son-of-anton plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Son of Anton
    ...
    try:
        from son_of_anton_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        console.print("[dim]List installed plugins:[/dim] son-of-anton plugins list")
        sys.exit(1)
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``son-of-anton plugins enable web/firecrawl`` cleared
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.son-of-anton/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.son-of-anton/plugins/<name>``."""
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `son-of-anton plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``son-of-anton plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``son_of_anton_cli.main`` never covers them. After a
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    has local changes"), and the same autostash approach ``son-of-anton update``
    ...
    try:
        ...
        if dirty:
            ...
            push = _run_plugin_git(
                ...
                "-m", "son-of-anton-plugin-update-autostash",
            )
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.son-of-anton/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from son_of_anton_cli.plugin_dev import doctor_plugin
...
def cmd_search(
    ...
    *,
    ...
    from son_of_anton_cli.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        search_index,
    )
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: son-of-anton plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")


def plugins_command(args) -> None:
    """Dispatch son-of-anton plugins subcommands."""
    ...
    if action == "install":
        ...
        from son_of_anton_cli.plugin_packs import pack_command
