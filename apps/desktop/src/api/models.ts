...
import { capabilityScoped, rencoApi, type ProfileScope, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getGlobalModelInfo(profile?: null | string): Promise<ModelInfoResponse> {
  return rencoApi<ModelInfoResponse>({
...
export function getUsageAnalytics(days = 30, profile?: ProfileScope): Promise<AnalyticsResponse> {
  return window.rencoDesktop.api<AnalyticsResponse>({
...
export function getGlobalModelOptions(
  ...
  return rencoApi<ModelOptionsResponse>({
    ...profileScoped(profile),
...
// curation `renco model` does — for Nous it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(
  provider: string,
  profile?: null | string
): Promise<RecommendedDefaultModel> {
  return rencoApi<RecommendedDefaultModel>({
    ...profileScoped(profile),
...
export function setGlobalModel(
  ...
  return rencoApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body: {
...
export function getAuxiliaryModels(profile?: null | string): Promise<AuxiliaryModelsResponse> {
  return rencoApi<AuxiliaryModelsResponse>({
    ...profileScoped(profile),
...
export function getMoaModels(profile?: null | string): Promise<MoaConfigResponse> {
  return rencoApi<MoaConfigResponse>({
    ...profileScoped(profile),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(
  ...
  return rencoApi<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(profile),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function setModelAssignment(
  ...
  return rencoApi<ModelAssignmentResponse>({
    ...profileScoped(profile),
    path: '/api/model/set',
    method: 'POST',
    body
