"""Install and remove the Linux desktop entry (``son-of-anton.desktop``).

``son-of-anton desktop`` builds and launches the Electron app. On Linux, a
...
``son-of-anton uninstall --gui`` removes the entry again.

Two values must be absolute for the entry to work:

  - ``Exec`` — the launcher runs without shell ``PATH`` customizations, so
    a bare ``son-of-anton desktop`` fails when son-of-anton lives in ``~/.local/bin``
...
DESKTOP_ENTRY_NAME = "son-of-anton.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``son-of-anton.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``son-of-anton desktop``.

    Prefer the real ``son-of-anton`` executable (argv[0] or PATH). When Son of Anton
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from son_of_anton_cli.relaunch import resolve_son_of_anton_bin

    bin_path = resolve_son_of_anton_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "son_of_anton_cli.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Son of Anton\n"
        "GenericName=Son of Anton Desktop\n"
        "Comment=Launch Son of Anton Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Son of Anton\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Son of Anton desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "son-of-anton"
