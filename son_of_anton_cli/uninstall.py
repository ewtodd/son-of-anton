"""
Son of Anton Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.son-of-anton/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Son of Anton PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing son-of-anton or son-of-anton PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Son of Anton Agent" comment and following line
                if '# Son of Anton Agent' in line or '# son-of-anton' in line:
                    skip_next = True
                    continue
                if skip_next and ('son-of-anton' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing son-of-anton
                if 'son-of-anton' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Son of Anton-owned file, and
...
def remove_wrapper_script():
    """Remove the son-of-anton wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "son-of-anton",
        Path.home() / ".local" / "bin" / "son-of-anton-acp",
        Path.home() / ".local" / "bin" / "son-of-anton",
        Path("/usr/local/bin/son-of-anton"),
        Path("/usr/local/bin/son-of-anton-acp"),
        Path("/usr/local/bin/son-of-anton"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains son_of_anton_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'son_of_anton_cli' in content or 'son-of-anton' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(son_of_anton_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``son-of-anton`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Son of Anton home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (son_of_anton_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``son-of-anton gateway run`` processes
    ...
    try:
        from son_of_anton_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from son_of_anton_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SON_OF_ANTON_HOME`` and ``SON_OF_ANTON_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\son-of-anton\git\cmd``, ``%LOCALAPPDATA%\son-of-anton\git\bin``,
#      ``%LOCALAPPDATA%\son-of-anton\git\usr\bin``, ``%LOCALAPPDATA%\son-of-anton\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\son-of-anton\git\`` and Node to
#      ``%LOCALAPPDATA%\son-of-anton\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``son-of-anton dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\son-of-anton\gateway-service\`` and sometimes
...
def _son_of_anton_path_markers(son_of_anton_home: Path) -> list[str]:
    """Path-entry substrings that identify Son of Anton-owned User-PATH entries."""
    root = str(son_of_anton_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare son-of-anton install dir.
    markers = [root + "\\son-of-anton", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SON_OF_ANTON_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "son-of-anton".  We don't want to catch
    # unrelated entries like "cson-of-anton-foo" or "ephermeral", so we look for
    # backslash-son-of-anton as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(son_of_anton_home: Path) -> list[str]:
    """Strip Son of Anton-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _son_of_anton_path_markers(son_of_anton_home)
            kept: list[str] = []
...
def remove_son_of_anton_env_vars_windows() -> list[str]:
    """Delete SON_OF_ANTON_HOME and SON_OF_ANTON_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SON_OF_ANTON_HOME", "SON_OF_ANTON_GIT_BASH_PATH"):
                try:
...
def _is_default_son_of_anton_home(son_of_anton_home: Path) -> bool:
    """Return True when ``son_of_anton_home`` points at the default (non-profile) root."""
    try:
        from son_of_anton_constants import get_default_son_of_anton_root
        return son_of_anton_home.resolve() == get_default_son_of_anton_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SON_OF_ANTON_HOME directory.

    We shell out to ``son-of-anton -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SON_OF_ANTON_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m son_of_anton_cli.main` so we don't depend on a `son-of-anton`
    #    wrapper that may be half-removed mid-uninstall.
    son_of_anton_invocation = [_sys.executable, "-m", "son_of_anton_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                son_of_anton_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's SON_OF_ANTON_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``son-of-anton uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SON_OF_ANTON_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from son_of_anton_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(son_of_anton_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚛ Son of Anton Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Son of Anton Chat GUI installation was found.")
        print(f"  Checked: {son_of_anton_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Son of Anton agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(son_of_anton_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Son of Anton agent at {son_of_anton_home / 'son-of-anton'}")
        print(f"  • Your config, sessions, and secrets under {son_of_anton_home}")
        print()
    ...
    print()
    uninstall_gui(son_of_anton_home)
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Son of Anton agent is still installed. Run 'son-of-anton' to use the CLI,")
    print("or 'son-of-anton uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.son-of-anton/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.son-of-anton/ for future reinstall
    """
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            ...
            son_of_anton_home=son_of_anton_home,
            full_uninstall=bool(getattr(args, "full", False)),
        )
        return

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie SON_OF_ANTON_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_son_of_anton_home(son_of_anton_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.son-of-anton data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚛ Son of Anton Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {son_of_anton_home / 'config.yaml'}")
    print(f"  Secrets: {son_of_anton_home / '.env'}")
    print(f"  Data:    {son_of_anton_home / 'cron/'}, {son_of_anton_home / 'sessions/'}, {son_of_anton_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SON_OF_ANTON_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Son of Anton data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Son of Anton code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, son_of_anton_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Son of Anton PATH entries from shell configs / Windows User PATH")
    print("  • Son of Anton wrapper scripts and Son of Anton-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Son of Anton config/data: {son_of_anton_home}")
        if _is_default_son_of_anton_home(son_of_anton_home):
        ...
        print(f"  • Keep Son of Anton config/data: {son_of_anton_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``son-of-anton`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SON_OF_ANTON_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in son_of_anton_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\son-of-anton\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(son_of_anton_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Son of Anton-owned PATH entries in User environment")

        log_info("Removing SON_OF_ANTON_HOME / SON_OF_ANTON_GIT_BASH_PATH User env vars...")
        removed_env = remove_son_of_anton_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Son of Anton-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing son-of-anton command...")
    ...
    #     (only when they still point into this Son of Anton home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Son of Anton-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(son_of_anton_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Son of Anton-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(son_of_anton_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SON_OF_ANTON_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from son_of_anton_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(son_of_anton_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.son-of-anton/, just remove the son-of-anton subdir
            if son_of_anton_home in project_root.parents or project_root.parent == son_of_anton_home:
    ...
    #     under SON_OF_ANTON_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(son_of_anton_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(son_of_anton_home)
    ...
    # 5. Optionally remove ~/.son-of-anton/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SON_OF_ANTON_HOME dirs live under
        ...
        try:
            if son_of_anton_home.exists():
                shutil.rmtree(son_of_anton_home)
                log_success(f"Removed {son_of_anton_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {son_of_anton_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {son_of_anton_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {son_of_anton_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://son-of-anton.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://son-of-anton.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Son of Anton Agent! ⚛")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m son_of_anton_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import son_of_anton_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``son_of_anton_constants`` + ``son_of_anton_cli.colors``
    (and lazily ``son_of_anton_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m son_of_anton_cli.uninstall")
    parser.add_argument(
        "--mode",
