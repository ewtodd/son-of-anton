...
Runs a real subprocess with a temp RENCO_HOME whose config.yaml contains a
``${ENV_VAR}`` reference, plus a managed-scope overlay dir (RENCO_MANAGED_DIR).
...
Subprocess (not in-process monkeypatching) so module-level ``_renco_home``
globals, managed-scope caches, and ``_under_pytest`` guards behave like
production: ``RENCO_MANAGED_DIR`` is set explicitly, which bypasses the
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    code = textwrap.dedent(
        ...
        import os
        from pathlib import Path
        saved = Path(server._renco_home, "config.yaml").read_text(encoding="utf-8")
    ...
    out = _run_py(
        ...
        {
            "RENCO_HOME": str(home),
            "RENCO_MANAGED_DIR": str(managed_dir),
...
def test_writeback_roundtrip_byte_identical_when_unchanged(tmp_path):
    ...
    code = textwrap.dedent(
        ...
        p = Path(__import__('os').environ['RENCO_HOME']) / 'config.yaml'
    ...
    out = _run_py(code, {"RENCO_HOME": str(home)}, tmp_path)
