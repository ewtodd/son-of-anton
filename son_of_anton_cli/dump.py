...
from son_of_anton_cli.config import get_son_of_anton_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.son-of-anton/.env.
    ...
    exports. ``son-of-anton debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.son_of_anton_build_sha`` by the Dockerfile's
    ``SON_OF_ANTON_GIT_SHA`` build-arg (see ``son_of_anton_cli/build_info.py``).
...
def _gateway_status() -> str:
    ...
    try:
        from son_of_anton_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(son_of_anton_home: Path) -> int:
...
def _cron_summary(son_of_anton_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = son_of_anton_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    load_son_of_anton_dotenv(
        son_of_anton_home=env_path.parent,
    ...
    lines = []
    lines.append("--- son-of-anton dump ---")
    ...
    lines.append(f"son_of_anton_home:      {display_son_of_anton_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.son-of-anton/.env: a managed
        ...
        # A credential added via `son-of-anton auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `son-of-anton auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["son-of-anton-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(son_of_anton_home)}")
    lines.append(f"  skills:             {_count_skills(son_of_anton_home)}")
