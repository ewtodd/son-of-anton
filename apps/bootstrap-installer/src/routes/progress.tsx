...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Renco' : 'Setting up Renco Agent'

  const description = isUpdate
    ? 'Renco is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Renco installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="renco-fade-in flex h-full flex-col">
