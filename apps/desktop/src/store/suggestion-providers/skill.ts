 ...
 *  Working in `~/www/renco-agent`, the draft says "renco-agent" constantly
 *  — a "Use skill: renco-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`renco-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/renco-agent-dev`, `plugin:skill`). */
