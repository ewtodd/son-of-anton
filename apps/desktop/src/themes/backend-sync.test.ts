...
describe('ingestBackendSkin', () => {
  ...
  it('seed does not paint, but a later same-name skin.changed applies (missed-activation recovery)', () => {
    ...
    // restarted). Renco re-affirms it — `renco config set display.skin neon`
    // or a `renco skin set` recolor. That explicit event must repaint even
    ...
    ingestBackendSkin(skin('forest'), { apply: true }) // Renco authored a new skin
    expect($pendingSkinApply.get()).toBe('forest')
  })
  ...
  it('applies a runtime switch back to default (repaints the desktop to its own default)', () => {
    ...
    ingestBackendSkin(skin('default'), { apply: true }) // Renco switched back to default
