...
import type { RencoReadDirResult } from '@/global'
import type * as RencoModule from '@/renco'
...
// getStatus would supply the connected backend's renco_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ renco_home: '/remote/box/.renco' }))

vi.mock('@/renco', async importActual => ({
  ...(await importActual<typeof RencoModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const agentPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<RencoReadDirResult>>()
...
describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend renco_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.renco/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.renco/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.renco/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.renco/plugins')
    // The remote backend's renco_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.renco/desktop-plugins')
  })
  ...
  it('probes desktop/plugin.js inside agent-plugin packages (unified packaging)', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.renco/plugins'
        ? { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.renco/plugins/my-feature' }] }
        : { entries: [] }
    )
    ...
    expect(readFileText).toHaveBeenCalledWith('/local/.renco/plugins/my-feature/desktop/plugin.js')
    // The Python half's files must never be probed as a desktop entry.
    expect(readFileText).not.toHaveBeenCalledWith('/local/.renco/plugins/my-feature/plugin.js')
  })

  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.rencoDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.renco/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.renco/plugins'
        ? { entries: [{ isDirectory: true, name: 'uni', path: '/local/.renco/plugins/uni' }] }
        : { entries: [] }
    )
    ...
    try {
      ...
      // ~/.renco/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend renco_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.renco/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.renco/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.renco/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.renco/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.renco/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'renco-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "renco-bots", name: "Bot Mode", register() {} }',
        'renco-bots',
        { file: '/local/.renco/desktop-plugins/renco-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['renco-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['renco-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.renco/desktop-plugins/renco-bots/plugin.js'
      })
