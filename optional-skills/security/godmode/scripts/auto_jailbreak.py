...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("RENCO_HOME", os.path.expanduser("~/.renco")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("RENCO_HOME", Path.home() / ".renco")) / "skills" / "red-teaming" / "godmode"
...
RENCO_HOME = Path(os.getenv("RENCO_HOME", Path.home() / ".renco"))
CONFIG_PATH = RENCO_HOME / "config.yaml"
PREFILL_PATH = RENCO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Renco models — already uncensored, just needs clean prompt
    "renco": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "renco" in model_lower or "nous" in model_lower:
        return "renco"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Renco config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.renco/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Renco for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Renco for changes to take effect.")
