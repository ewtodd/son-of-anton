"""Unified provider-credential lifecycle across every store Renco reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.renco/.env``                     — the canonical secret store
    2. ``~/.renco/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.renco/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `renco auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``renco auth add``.
    """
    from renco_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from renco_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from renco_cli.config import load_env, remove_env_value
