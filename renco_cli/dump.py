"""
Dump command for renco CLI.

Outputs a compact, plain-text summary of the user's Renco setup
...
from renco_cli.config import get_renco_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.renco/.env.
    ...
    exports. ``renco debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.renco_build_sha`` by the Dockerfile's
    ``RENCO_GIT_SHA`` build-arg (see ``renco_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``renco dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from renco_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(renco_home: Path) -> int:
...
def _cron_summary(renco_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = renco_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"renco_home:      {display_renco_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.renco/.env: a managed
        ...
        # A credential added via `renco auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `renco auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["renco-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(renco_home)}")
    lines.append(f"  skills:             {_count_skills(renco_home)}")
