...
## Layer 1 — Is this thing a Renco plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Renco process**. Its LLM calls go through Renco's already-configured provider. | Already uses Portal if user's Renco is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Renco. That LLM call is already authenticated
via Renco's provider config. The plugin's own server auth (e.g.
...
What Renco Agent has that *feels* like OAuth — `renco login --provider nous`
opening a browser, user signs in, token lands in `~/.renco/auth.json` — is a
**Renco-specific browser flow**. Under the hood it produces a credential
Renco uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Renco's existing Portal credential out of `~/.renco/auth.json`.
...
rides on the credential Renco already holds.

Where this could live in Renco:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Renco itself.
...
1. First decide: Renco plugin (runs inside Renco) or separate app? If plugin,
   it already uses Portal via Renco's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Renco agent itself") is fully supported. Renco treats agent
...
pitch in the Tool Gateway announcement is specifically about Renco Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Renco is configured to use Portal as its provider. It is
