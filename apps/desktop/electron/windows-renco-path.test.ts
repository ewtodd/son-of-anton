// Unit tests for the pure Windows `renco` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvRencoCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `renco` shim shadows
//      the real renco.cmd/renco.exe.
...
//   3. resolveVenvRencoCommand() — must probe the venv python via
//      canImportRencoCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvRenco: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvRenco: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvRenco: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvRenco: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvRenco: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvRenco: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvRencoCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportRencoCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    rencoHome: '/fake/renco-home',
...
test('resolveVenvRencoCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvRencoCommand('/root/venv/Scripts/renco.exe', [], deps), null)
})

test('resolveVenvRencoCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvRencoCommand('/root/venv/Scripts/renco.cmd', [], deps), null)
})

test('resolveVenvRencoCommand: returns null when the basename is not renco/renco.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvRencoCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvRencoCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvRencoCommand('/root/venv/bin/renco.exe', [], deps), null)
})

test('resolveVenvRencoCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvRencoCommand('/root/venv/Scripts/renco.exe', [], deps), null)
})

test('resolveVenvRencoCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportRencoCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvRencoCommand('/root/venv/Scripts/renco.exe', ['serve'], deps)
...
test('resolveVenvRencoCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvRencoCommand('/root/venv/Scripts/renco.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'renco_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvRencoCommand: is case-insensitive on renco.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvRencoCommand('/root/venv/Scripts/RENCO.EXE', [], deps))
  assert.ok(resolveVenvRencoCommand('/root/venv/SCRIPTS/renco.exe', [], deps))
})
