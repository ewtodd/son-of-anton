"""Tests for son_of_anton_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("son_of_anton_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("son_of_anton_cli.config.load_config")
    ...
    @patch("son_of_anton_cli.config.load_config")
    @patch.dict("os.environ", {"SON_OF_ANTON_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """SON_OF_ANTON_SESSION_PLATFORM should be used when SON_OF_ANTON_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_son_of_anton_platform_takes_precedence(self, tmp_path, monkeypatch):
        """SON_OF_ANTON_PLATFORM should win over SON_OF_ANTON_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from son_of_anton_cli.skills_config import _get_categories
        skills = [
