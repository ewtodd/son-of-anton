...
One read-only pass that answers, BEFORE any mutation: what Renco runtimes
...
`detect_install_method`) — so `renco update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `renco update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `renco serve`; it respawns
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Renco runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "renco update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set) -> str:
    ...
    if pid in service_pids:
        try:
            from renco_cli.gateway import is_macos, supports_systemd_services
...
def _restart_mechanism(supervisor: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"renco -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        from renco_cli.gateway import _get_service_pids
    ...
    try:
        from renco_cli.gateway import find_profile_gateway_processes
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Renco services: none detected — code swap only.")
        return
