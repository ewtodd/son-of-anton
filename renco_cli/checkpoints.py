"""`renco checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.renco/checkpoints/``.  Actions:

    renco checkpoints               # same as `status`
    renco checkpoints status        # total size, project count, breakdown
    renco checkpoints list          # per-project checkpoint counts + workdir
    renco checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    renco checkpoints clear [-f]    # nuke the entire base (asks first)
    renco checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    renco checkpoints
    renco checkpoints prune --retention-days 3 --max-size-mb 200
    renco checkpoints clear -f
...
from renco_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: renco checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``renco checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `renco checkpoints` → status
