---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Renco"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in Renco"
---
...
Set up Actual Computer (actual.inc) inference in Renco.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `renco skills install official/devops/actual-setup` |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Renco inference
...
- User has an `ac_` key and wants Renco routed through their Actual cluster.
...
- Renco has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Renco — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.renco/.env`.
...
3. Select provider + model:
   ```bash
   renco config set model.provider actual
   renco config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   renco chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Renco at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.renco/.env`, then:
   ```bash
   renco config set model.provider actual
   renco config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   renco chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by Renco since the first-class provider).**
   ...
   request still 400s this way on an old Renco, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Renco' default toolset
   ...
   servers emit a bare `data: [DONE]` — Renco reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Renco the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
renco chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
renco chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
