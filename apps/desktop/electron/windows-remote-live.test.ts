...
//   RENCO_WIN_SSH_HOST   ssh alias/host of the Windows box
//   RENCO_WIN_SSH_USER   remote user
//   RENCO_WIN_SSH_RENCO absolute path to the remote renco.exe under test
const liveHost = process.env.RENCO_WIN_SSH_HOST || ''
const liveUser = process.env.RENCO_WIN_SSH_USER || ''
const configuredRenco = process.env.RENCO_WIN_SSH_RENCO || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Renco-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredRenco)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredRenco.replace('renco.exe', 'python.exe')}' -m renco_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredRenco.replace('renco.exe', 'python.exe')
        const terminate = `& '${python}' -m renco_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.rencoPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m renco_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m renco_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
