"""``renco sync`` subcommand parser — Skill Sync.

Cloned from ``renco_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    renco sync status                 show gate/opt-in/head state
    renco sync pull                   pull and materialize opted-in skills
    renco sync push                   push opted-in skills
    renco sync now                    reconcile: pull then push
    renco sync enable <skill>         opt a skill into sync
    renco sync disable <skill>        opt a skill out of sync
    renco sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    renco sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  renco sync status            what is synced, and from where\n"
            "  renco sync enable my-skill   include a skill in your sync\n"
            "  renco sync now               pull, then push\n"
            "  renco sync propose my-skill  share a skill with your team\n"
        ),
