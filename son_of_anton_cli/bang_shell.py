...
def bang_shell_enabled() -> bool:
    ...
    if env_var_enabled("SON_OF_ANTON_GATEWAY_SESSION"):
        return False
    if env_var_enabled("SON_OF_ANTON_CRON_SESSION"):
        return False
    if (os.getenv("SON_OF_ANTON_SESSION_PLATFORM") or "").strip():
        return False
    return True
...
def _bang_env() -> dict:
    """Environment for a bang command, with Son of Anton-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Son of Anton session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
