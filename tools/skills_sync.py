...
Copies bundled skills from the repo's skills/ directory into ~/.son-of-anton/skills/
...
The manifest lives at ~/.son-of-anton/skills/.bundled_manifest.
"""
...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
from son_of_anton_constants import get_bundled_skills_dir, get_son_of_anton_home
...
# cron, kanban workers) import this module once under the launch SON_OF_ANTON_HOME
# and later scope requests to a different profile via
# set_son_of_anton_home_override(). Frozen module constants would then resolve —
...
# re-resolve from the live profile-scoped SON_OF_ANTON_HOME on every call.
_SON_OF_ANTON_HOME_AT_IMPORT = SON_OF_ANTON_HOME
_SKILLS_DIR_AT_IMPORT = SKILLS_DIR
_MANIFEST_FILE_AT_IMPORT = MANIFEST_FILE


def _son_of_anton_home() -> Path:
    """Return the active profile's SON_OF_ANTON_HOME at call time."""
    configured = Path(SON_OF_ANTON_HOME)
    if configured != _SON_OF_ANTON_HOME_AT_IMPORT:
        return configured
...
def _skills_dir() -> Path:
    ...
    if configured != _SKILLS_DIR_AT_IMPORT:
        return configured
    return _son_of_anton_home() / "skills"
...
# Marker file written by `son-of-anton profile create --no-skills` (named profiles)
...
def _get_bundled_dir() -> Path:
    ...
    Checks SON_OF_ANTON_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.son-of-anton/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.son-of-anton/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``son-of-anton skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `son-of-anton skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.son-of-anton/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.son-of-anton) that wrote the
    ...
    if (_son_of_anton_home() / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.son-of-anton/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `son-of-anton update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `son-of-anton skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``SON_OF_ANTON_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.son-of-anton/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # path join, a missing ``SON_OF_ANTON_HOME`` default, a malicious
    # bundled-manifest entry, or a mid-flight exception that leaves a
    # stale path in scope — this guard turns the resulting
    # ``shutil.rmtree(~/.son-of-anton)`` into a loud, recoverable ``ValueError``
    ...
    # ``SON_OF_ANTON_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `son-of-anton skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `son-of-anton update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``son-of-anton update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``son-of-anton update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``son-of-anton skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `son-of-anton skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes SON_OF_ANTON_HOME/.no-bundled-skills so the
    installer, ``son-of-anton update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``son-of-anton skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = _son_of_anton_home() / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            _son_of_anton_home().mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding "
                "(`son-of-anton skills opt-out`).\n"
            ...
            changed = existed
            message = (
                "Opted back in. The next `son-of-anton update` (or `son-of-anton skills "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.son-of-anton/skills/ ...")
