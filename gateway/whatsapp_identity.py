...
def expand_whatsapp_aliases(identifier: str) -> Set[str]:
    ...
    bridge's ``$SON_OF_ANTON_HOME/whatsapp/session/lid-mapping-*.json`` files,
    ...
    session_dir = get_son_of_anton_dir("platforms/whatsapp/session", "whatsapp/session")
...
def canonical_whatsapp_identifier(identifier: str) -> str:
    ...
    same session-key identity Son of Anton itself uses.

    Plugins that need per-sender behaviour (role-based routing,
    authorisation, per-contact policy) should use this so their
    bookkeeping lines up with Son of Anton' session bookkeeping even when
