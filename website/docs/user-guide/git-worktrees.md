...
title: "Git Worktrees"
description: "Run multiple Renco agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Renco Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Renco so each session has a clean, isolated working directory.

## Why Use Worktrees with Renco?

Renco treats the **current working directory** as the project root:

- CLI: the directory where you run `renco` or `renco chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.renco/config.yaml`
...
Renco creates `.worktrees/my-experiment/` inside the repo (branch
`renco/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `renco-<id>`
...
git worktree add ../repo-feature feature/renco-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/renco-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Renco there:

```bash
cd ../repo-feature

# Start Renco in the worktree
renco
```
...
git worktree add ../repo-experiment-a feature/renco-a
git worktree add ../repo-experiment-b feature/renco-b
```
...
- Works on its own branch (`feature/renco-a` vs `feature/renco-b`).
...
- Renco checkpoint data under `~/.renco/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Renco experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/renco-checkpoints-docs`, `feature/renco-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Renco from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `renco -w` (Automatic Worktree Mode)

Renco has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `renco/renco-<hash>`).
...
```bash
renco -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `renco -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Renco session its own clean checkout.
