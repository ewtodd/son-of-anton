...
from renco_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_renco_tools_mcp_entry,
    _format_toml_value,
...
# ---- end-to-end migrate(, expose_renco_tools=False) ----
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from renco_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_renco_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(renco_cfg, codex_home=tmp_path, expose_renco_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Renco' managed block, re-running
        ...
        migrate({"mcp_servers": {"renco-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.renco-mcp]" in final
    ...
    def test_summary_reports_migration_count(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_renco_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``renco codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "renco_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_renco_tools=False)
...
# ---- Bug C: RENCO_HOME tempdir leak into ~/.codex/config.toml ----


class TestRencoHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_renco_tools_mcp_entry()`` read ``RENCO_HOME``
    ...
    once codex spawned the renco-tools MCP subprocess.
    """




    def test_real_renco_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate RENCO_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.renco"
        monkeypatch.setenv("RENCO_HOME", real_path)
        entry = _build_renco_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("RENCO_HOME") == real_path

    def test_unset_renco_home_omits_env_key(self, monkeypatch):
        """When RENCO_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever RENCO_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_renco_tools_mcp_entry()
        env = entry.get("env", {})
        assert "RENCO_HOME" not in env, (
            f"RENCO_HOME should not be set when env var is unset, got: "
            f"{env.get('RENCO_HOME')!r}"
        )
