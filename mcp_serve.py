"""
Son of Anton MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Son of Anton-specific extra)

Usage:
    son-of-anton mcp serve
    son-of-anton mcp serve --verbose
...
logger = logging.getLogger("son_of_anton.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SON_OF_ANTON_HOME."""
    try:
        from son_of_anton_constants import get_son_of_anton_home
        return get_son_of_anton_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SON_OF_ANTON_HOME", Path.home() / ".son-of-anton")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from son_of_anton_constants import get_son_of_anton_home
        directory_file = get_son_of_anton_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SON_OF_ANTON_HOME", Path.home() / ".son-of-anton")
...
class EventBridge:
    ...
    This is the Son of Anton equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from son_of_anton_constants import get_son_of_anton_home
            db_file = get_son_of_anton_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SON_OF_ANTON_HOME", Path.home() / ".son-of-anton")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Son of Anton MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "son-of-anton",
        instructions=(
            "Son of Anton messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Son of Anton MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
