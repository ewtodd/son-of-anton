"""Pydantic request/response models for the Renco dashboard web server.

Extracted verbatim from ``renco_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from renco_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``renco model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `renco import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`renco -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the RENCO_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under RENCO_HOME.
    output: str = ""
