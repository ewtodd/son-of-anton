"""`renco meet node ...` subcommand tree.

Wired into the existing ``renco meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``renco meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="renco-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             renco meet node approve <name> ws://<host>:{args.port} {token}")
        try:
