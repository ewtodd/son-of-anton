...
sidebar_position: 1
title: "CLI 命令参考"
description: "Renco 终端命令及命令族的权威参考"
---
...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 Renco profile（配置文件）。覆盖 `renco profile use` 设置的粘性默认值。 |
...
| `--ignore-user-config` | 忽略 `~/.renco/config.yaml`，回退到内置默认值。`.env` 中的凭据仍会加载。 |
| `--ignore-rules` | 跳过 `AGENTS.md`、`SOUL.md`、`.cursorrules`、memory（记忆）和预加载 skill 的自动注入。 |
| `--tui` | 启动 [TUI](../user-guide/tui.md) 而非经典 CLI。等同于 `RENCO_TUI=1`。 |
...
| 命令 | 用途 |
|---------|---------|
| `renco chat` | 与 agent 进行交互式或单次聊天。 |
| `renco model` | 交互式选择默认 provider 和模型。 |
| `renco fallback` | 管理主模型出错时依次尝试的 fallback provider。 |
| `renco gateway` | 运行或管理消息 gateway 服务。 |
| `renco proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](../user-guide/features/subscription-proxy.md)。 |
| `renco lsp` | 管理 Language Server Protocol 集成（为 write_file/patch 提供语义诊断）。 |
| `renco setup` | 全部或部分配置的交互式设置向导。 |
| `renco whatsapp` | 配置并配对 WhatsApp 桥接。 |
| `renco slack` | Slack 辅助工具（当前功能：生成将每条命令注册为原生斜杠命令的 app manifest）。 |
| `renco auth` | 管理凭据——添加、列出、删除、重置、设置策略。处理 Codex/Nous/Anthropic 的 OAuth 流程。 |
| `renco login` / `logout` | **已弃用** — 请改用 `renco auth`。 |
| `renco status` | 显示 agent、auth 和平台状态。 |
| `renco cron` | 检查并触发 cron 调度器。 |
| `renco kanban` | 多 profile 协作看板（任务、链接、调度器）。 |
| `renco webhook` | 管理用于事件驱动激活的动态 webhook 订阅。 |
| `renco hooks` | 检查、审批或删除 `config.yaml` 中声明的 shell 脚本 hook。 |
| `renco doctor` | 诊断配置和依赖问题。 |
| `renco security audit` | 对 venv、plugin 依赖和固定 MCP 服务器进行按需供应链审计（OSV.dev）。 |
| `renco dump` | 可直接复制粘贴的设置摘要，用于支持/调试。 |
| `renco debug` | 调试工具——上传日志和系统信息以获取支持。 |
| `renco backup` | 将 Renco 主目录备份为 zip 文件。 |
| `renco checkpoints` | 检查/修剪/清除 `~/.renco/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
| `renco import` | 从 zip 文件恢复 Renco 备份。 |
| `renco logs` | 查看、跟踪和过滤 agent/gateway/错误日志文件。 |
| `renco config` | 显示、编辑、迁移和查询配置文件。 |
| `renco pairing` | 审批或撤销消息配对码。 |
| `renco skills` | 浏览、安装、发布、审计和配置 skill。 |
| `renco bundles` | 将多个 skill 归组到单个 `/<name>` 斜杠命令下。参见 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。 |
| `renco curator` | 后台 skill 维护——状态、运行、暂停、固定。参见 [Curator](../user-guide/features/curator.md)。 |
| `renco memory` | 配置外部 memory provider。当对应 provider 激活时，特定于 plugin 的子命令（如 `renco honcho`）会自动注册。 |
| `renco acp` | 将 Renco 作为 ACP 服务器运行，用于编辑器集成。 |
| `renco mcp` | 管理 MCP 服务器配置，并将 Renco 作为 MCP 服务器运行。 |
| `renco plugins` | 管理 Renco Agent plugin（安装、启用、禁用、删除）。 |
| `renco portal` | Nous Portal 状态、订阅链接和 Tool Gateway 路由。参见 [Tool Gateway](../user-guide/features/tool-gateway.md)。 |
| `renco tools` | 按平台配置已启用的工具。 |
| `renco computer-use` | 安装或检查 cua-driver 后端（macOS Computer Use）。 |
| `renco sessions` | 浏览、导出、修剪、重命名和删除会话。 |
| `renco insights` | 显示 token/费用/活动分析。 |
| `renco claw` | OpenClaw 迁移辅助工具。 |
| `renco dashboard` | 启动用于管理配置、API 密钥和会话的 Web 控制台。 |
| `renco profile` | 管理 profile——多个隔离的 Renco 实例。 |
| `renco completion` | 打印 shell 补全脚本（bash/zsh/fish）。 |
| `renco --version` | 显示版本信息。 |
| `renco update` | 拉取最新代码并重新安装依赖。`--check` 预览而不安装；`--backup` 在拉取前对 `RENCO_HOME` 进行快照。 |
| `renco uninstall` | 从系统中删除 Renco。 |
...
| `--ignore-user-config` | 忽略 `~/.renco/config.yaml`，使用内置默认值。`.env` 中的凭据仍会加载。适用于隔离的 CI 运行、可复现的 bug 报告和第三方集成。 |
...
### `renco -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`renco -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
单次运行覆盖（不修改 `~/.renco/config.yaml`）：

| 标志 | 等效环境变量 | 用途 |
|---|---|---|
| `-m` / `--model <model>` | `RENCO_INFERENCE_MODEL` | 覆盖本次运行的模型 |
...
相同的 agent、相同的工具、相同的 skill——只是剥离了所有交互式/装饰性层。如果你还需要在记录中包含工具输出，请改用 `renco chat -q`；`-z` 专门用于"我只需要最终答案"的场景。

## `renco model`

交互式 provider + 模型选择器。**这是添加新 provider、设置 API 密钥和运行 OAuth 流程的命令。** 从终端运行——不要在活跃的 Renco 聊天会话内部运行。
...
:::warning renco model 与 /model——了解区别
**`renco model`**（从终端运行，在任何 Renco 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 Renco 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 Renco 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `renco model`。
:::
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `renco model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `RENCO_HOME`。当你并行运行多个 profile 并希望在 `renco update` 后全部重启时很有用。 |
| `--no-supervise` | 在 `run` 时：在 s6-overlay Docker 镜像内部，跳过 s6 自动监管，退回到 pre-s6 前台语义——gateway 作为容器主进程运行，无自动重启。在 s6 镜像之外为空操作。等同于设置 `RENCO_GATEWAY_NO_SUPERVISE=1`。 |

:::tip WSL 用户
使用 `renco gateway run` 而非 `renco gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s renco 'renco gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-renco-gateway-start-fails)。
:::
...
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容别名——在已有安装上裸运行 `renco setup` 现在默认执行此操作。 |
...
关于 gateway 本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一键设置路径，请参阅上方的 `renco setup --portal`。
...
```bash
renco slack manifest              # 将 manifest 打印到 stdout
renco slack manifest --write      # 写入 ~/.renco/slack-manifest.json
renco slack manifest --slashes-only  # 仅输出 features.slash_commands 数组
```
...
| 标志 | 默认值 | 用途 |
|------|---------|---------|
| `--write [PATH]` | stdout | 写入文件而非 stdout。裸 `--write` 写入 `$RENCO_HOME/slack-manifest.json`。 |
| `--name NAME` | `Renco` | Slack 中的机器人显示名称。 |
| `--description DESC` | 默认简介 | Slack app 目录中显示的机器人描述。 |
| `--slashes-only` | 关闭 | 仅输出 `features.slash_commands`，用于合并到手动维护的 manifest 中。 |

`renco update` 后重新运行 `renco slack manifest --write` 以获取新增命令。


## `renco login` / `renco logout` *（已弃用）*

:::caution
`renco login` 已被移除。请使用 `renco auth` 管理 OAuth 凭据，使用 `renco model` 选择 provider，或使用 `renco setup` 进行完整的交互式设置。
:::
...
```bash
renco auth                                              # 交互式向导
renco auth list                                         # 显示所有池
renco auth list openrouter                              # 显示特定 provider
renco auth add openrouter --api-key sk-or-v1-xxx        # 添加 API 密钥
renco auth add anthropic --type oauth                   # 添加 OAuth 凭据
renco auth remove openrouter 2                          # 按索引删除
renco auth reset openrouter                             # 清除冷却时间
renco auth status anthropic                             # 显示某 provider 的认证状态
renco auth logout anthropic                             # 登出并清除已存储的认证状态
renco auth spotify                                      # 通过 PKCE 将 Renco 与 Spotify 认证
```
...
多 profile、多项目协作看板。每个安装可托管多个看板（每个项目、仓库或领域一个）；每个看板是独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装从名为 `default` 的单个看板开始，其数据库为 `~/.renco/kanban.db`（向后兼容）；其他看板位于 `~/.renco/kanban/boards/<slug>/kanban.db`。嵌入在 gateway 中的调度器每次 tick 扫描所有看板。

**全局标志（适用于以下所有操作）：**

| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `renco kanban boards switch`、`RENCO_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `renco kanban`。Worker 的环境中固定了 `RENCO_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
| `boards switch <slug>` / `boards use` | 将 `<slug>` 持久化为活跃看板（写入 `~/.renco/kanban/current`）。 |
...
renco kanban list                  # 显示 atm10-server 的任务
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `RENCO_KANBAN_BOARD` 环境变量 → `~/.renco/kanban/current` 文件 → `default`。

所有操作也可作为 gateway 中的斜杠命令使用（`/kanban …`），参数界面相同——包括 `boards` 子命令和 `--board` 标志。

完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/renco-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
...
| `--script` | 位于 `~/.renco/scripts/` 下的过滤/转换脚本。webhook payload 以 JSON 形式通过 stdin 传入；JSON stdout 会替换 payload，空 stdout、`[SILENT]` 或非零退出码会忽略该 webhook。参见[脚本过滤与转换](../user-guide/messaging/webhooks.md#script-filters-and-transforms)。 |

订阅持久化到 `~/.renco/webhook_subscriptions.json`，webhook 适配器无需重启 gateway 即可热重载。
...
输出整个 Renco 设置的紧凑纯文本摘要。专为复制粘贴到 Discord、GitHub issue 或 Telegram 寻求支持而设计——无 ANSI 颜色、无特殊格式，只有数据。
...
| 部分 | 详情 |
|---------|---------|
| **Header** | Renco 版本、发布日期、git commit hash |
| **Environment** | 操作系统、Python 版本、OpenAI SDK 版本 |
| **Identity** | 活跃 profile 名称、RENCO_HOME 路径 |
...
:::tip
`renco dump` 专为分享而设计。交互式诊断请使用 `renco doctor`。可视化概览请使用 `renco status`。
:::
...
报告包含系统信息（操作系统、Python 版本、Renco 版本）、近期 agent 和 gateway 日志（每文件 512 KB 限制）以及脱敏的 API 密钥状态。密钥始终脱敏——不会上传任何密钥。
...
```bash
renco debug share              # 上传调试报告，打印 URL
renco debug share --lines 500  # 包含更多日志行
renco debug share --expire 30  # 粘贴保留 30 天
renco debug share --local      # 在终端打印报告（不上传）
```
...
创建 Renco 配置、skill、会话和数据的 zip 归档。备份不包含 renco-agent 代码库本身。

| 选项 | 说明 |
|--------|-------------|
| `-o`, `--output <path>` | zip 文件的输出路径（默认：`~/renco-backup-<timestamp>.zip`）。 |
| `-q`, `--quick` | 快速快照：仅包含关键状态文件（config.yaml、state.db、.env、auth、cron 任务）。比完整备份快得多。 |
| `-l`, `--label <name>` | 快照标签（仅与 `--quick` 配合使用）。 |

备份使用 SQLite 的 `backup()` API 进行安全复制，因此即使 Renco 正在运行也能正确工作（WAL 模式安全）。
...
- `renco-agent` 代码本身（这是用户数据备份，不是仓库快照）。

### 示例

```bash
renco backup                           # 完整备份到 ~/renco-backup-*.zip
renco backup -o /tmp/renco.zip        # 完整备份到指定路径
renco backup --quick                   # 仅状态快速快照
renco backup --quick --label "pre-upgrade"  # 带标签的快速快照
```
...
检查和管理 `~/.renco/checkpoints/` 处的影子 git 存储——会话内 `/rollback` 命令的存储层。可随时安全运行；不需要 agent 正在运行。

| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `renco checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
```bash
renco checkpoints                                  # 状态概览
renco checkpoints prune --retention-days 3         # 激进清理
renco checkpoints prune --max-size-mb 200          # 一次性收紧大小上限
renco checkpoints clear-legacy -f                  # 删除 v1 归档目录
renco checkpoints clear -f                         # 清除所有内容
```
...
将之前创建的 Renco 备份恢复到 Renco 主目录。归档中的所有文件会覆盖 Renco 主目录中的现有文件；`--force` 仅跳过当目标已有 Renco 安装时触发的确认提示。
...
### 示例
```bash
renco import ~/renco-backup-20260423.zip           # 覆盖现有配置前提示确认
renco import ~/renco-backup-20260423.zip --force   # 不提示直接覆盖
```
...
查看、跟踪和过滤 Renco 日志文件。所有日志存储在 `~/.renco/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
Renco 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`renco logs list` 子命令显示所有日志文件，包括已轮转的。
...
renco skills install https://sharethis.chat/SKILL.md                     # 直接 URL（含引用的支持文件）
renco skills install https://example.com/SKILL.md --name my-skill        # frontmatter 无名称时覆盖名称
renco skills check
renco skills update
renco skills config
...
- `--source well-known` 允许你将 Renco 指向暴露 `/.well-known/skills/index.json` 的站点。
...
Skill bundle 将多个 skill 归组到一个 `/<bundle-name>` 斜杠命令下。调用 bundle 会将每个引用的 skill 加载到单个合并的用户消息中。存储位置：`~/.renco/skill-bundles/<slug>.yaml`。YAML schema 和行为请参阅 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。
...
| `reload` | 重新扫描 `~/.renco/skill-bundles/` 并报告新增/删除的 bundle |
...
| `backup` | 手动对 `~/.renco/skills/` 进行 tar.gz 快照（curator 在每次真实运行前也会自动快照） |
| `rollback` | 从快照恢复 `~/.renco/skills/`（默认使用最新快照） |
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `renco update` 后的第一次 tick 时立即执行 curator。使用 `renco curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `renco model` 相同的选择器）并追加到链末尾 |
...
检查 `~/.renco/config.yaml` 中声明的 shell 脚本 hook，针对合成 payload 测试它们，并管理 `~/.renco/shell-hooks-allowlist.json` 处的首次使用同意许可名单。
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `renco <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `renco honcho`）。未激活的 provider 不暴露其子命令。运行 `renco --help` 查看当前已连接的命令。
:::
...
将 Renco 作为 ACP（Agent Client Protocol）stdio 服务器启动，用于编辑器集成。
...
管理 MCP（Model Context Protocol）服务器配置，并将 Renco 作为 MCP 服务器运行。

| 子命令 | 说明 |
|------------|-------------|
| `serve [-v\|--verbose]` | 将 Renco 作为 MCP 服务器运行——向其他 agent 暴露对话。 |
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 Renco 中使用 MCP](../guides/use-mcp-with-renco.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-renco-as-an-mcp-server)。
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `renco plugins` 会打开包含两个部分的复合交互界面：
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 Renco Plugin](../developer-guide/plugins/index.md)。
...
`renco computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时 `renco tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`renco update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 Renco 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 Renco。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.renco`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。

| 选项 | 说明 |
|--------|-------------|
| `--dry-run` | 预览将迁移的内容而不写入任何内容。 |
| `--preset <name>` | 迁移预设：`full`（所有兼容设置）或 `user-data`（排除基础设施配置）。两种预设都不导入密钥——需要显式传入 `--migrate-secrets`。 |
| `--overwrite` | 在冲突时覆盖现有 Renco 文件（默认：当计划有冲突时拒绝应用）。 |
| `--migrate-secrets` | 在迁移中包含 API 密钥。即使在 `--preset full` 下也需要显式指定。 |
| `--no-backup` | 跳过迁移前对 `~/.renco/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.renco/backups/pre-migration-*.zip`；可用 `renco import` 恢复）。 |
...
迁移涵盖 30+ 个类别，包括 persona、memory、skill、模型 provider、消息平台、agent 行为、会话策略、MCP 服务器、TTS 等。条目要么**直接导入**到 Renco 等效项，要么**归档**以供手动审查。
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `cd ~/.renco/renco-agent && uv pip install -e ".[web]"`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页始终可用，但额外需要 `pty` extra（`cd ~/.renco/renco-agent && uv pip install -e ".[web,pty]"`）以及 POSIX PTY 环境（如 Linux、macOS 或 WSL2）。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
| `--stop` | — | 停止正在运行的 `renco dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `renco dashboard` 进程并退出。 |
...
管理 profile——多个隔离的 Renco 实例，每个实例拥有自己的 config、会话、skill 和主目录。
...
将 shell 补全脚本打印到 stdout。在 shell profile 中 source 输出内容，即可对 Renco 命令、子命令和 profile 名称进行 Tab 补全。
...
```bash
renco update [--check] [--backup] [--restart-gateway]
```

拉取最新的 `renco-agent` 代码并在受管理的 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。使用 `--check` 查看你的检出是否落后于 `origin/main`，而不安装。

| 选项 | 说明 |
|--------|-------------|
| `--check` | 并排打印当前 commit 和最新 `origin/main` commit，同步时退出码为 0，落后时为 1。不拉取、不安装、不重启任何内容。 |
| `--backup` | 在拉取前创建 `RENCO_HOME` 的带标签预更新快照（config、auth、会话、skill、配对数据）。默认**关闭**——之前的始终备份行为在大型主目录上每次更新会增加数分钟。通过 `config.yaml` 中的 `update.backup: true` 永久开启。 |
| `--restart-gateway` | 成功更新后重启正在运行的 gateway 服务。如果安装了多个 profile，隐含 `--all` 语义。 |

附加行为：

- **配对数据快照。** 即使 `--backup` 关闭，`renco update` 也会在 `git pull` 前对 `~/.renco/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `renco backup restore --state pre-update` 回滚。
- **旧版 `renco.service` 警告。** 如果 Renco 检测到预重命名的 `renco.service` systemd 单元（而非当前的 `renco-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
| 命令 | 说明 |
|---------|-------------|
| `renco --version` | 打印版本信息。 |
| `renco update` | 拉取最新变更并重新安装依赖。 |
| `renco uninstall [--full] [--yes]` | 删除 Renco，可选择删除所有 config/数据。 |
