...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[renco] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[renco`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[renco`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        RencoHome = $(if ($paths) { $paths.renco_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\renco\renco-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-renco'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-renco') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
