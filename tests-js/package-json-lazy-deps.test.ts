 ...
 *
 * The root ``package.json`` is installed by ``renco update`` on every user,
 ...
 *   workspace graph entirely. ``renco update`` and ``renco doctor --fix``
 ...
 *   ``renco tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``renco update`` for
 ...
 * fails — read the lazy-install guidance in the ``renco-agent-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in renco_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`renco tools` → Browser Automation → Camofox.'
  )
})
