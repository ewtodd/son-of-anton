...
// (#73044). Exercises the REAL renco + store/gateway + store/profile chain:
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/renco')
...
const conn = (over: Partial<RencoConnection> = {}): RencoConnection =>
  ({
    authMode: 'oauth',
