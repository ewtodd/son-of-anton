...
``renco://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Renco OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "renco-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``renco-desktop`` / ``renco-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Renco.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local RENCO_HOME override can't reach.
