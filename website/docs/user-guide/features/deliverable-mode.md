...
When Renco Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.renco/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.renco/config.yaml` (switchable per session
via `/personality`).
...
If you use Renco' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.renco/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Renco Agent's deliverable
