...
entries via ``son-of-anton mcp catalog`` or the interactive ``son-of-anton mcp picker``,
and install them with ``son-of-anton mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into son-of-anton. Presence in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``son-of-anton mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.son-of-anton/.env`` (the
...
from son_of_anton_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.son-of-anton/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Son of Anton install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Son of Anton understands version {_MANIFEST_VERSION})"
        )
    ...
    if t_type == "http" and a_type == "api_key":
        ...
        # at connect time.
        from son_of_anton_cli.mcp_config import _env_key_for_server
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Son of Anton is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Son of Anton
        understands. Update Son of Anton to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_son_of_anton_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.son-of-anton/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.son-of-anton/.env via save_env_value()."""
...
def _build_server_config(
    ...
    by son_of_anton_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from son_of_anton_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from son_of_anton_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``son-of-anton mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `son-of-anton mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `son-of-anton mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `son-of-anton mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `son-of-anton mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `son-of-anton auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`son-of-anton auth {entry.auth.provider}` if you have not "
    ...
    from son_of_anton_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new Son of Anton session to load its tools.",
        Colors.GREEN,
    ))
