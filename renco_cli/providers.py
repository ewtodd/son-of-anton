"""
Single source of truth for provider identity in Renco Agent.
...
2. **Renco overlays** — transport type, auth patterns, aggregator flags,
...
# -- Renco overlay ----------------------------------------------------------
# Renco-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class RencoOverlay:
    """Renco-specific provider metadata layered on top of models.dev."""
...
RENCO_OVERLAYS: Dict[str, RencoOverlay] = {
    ...
    "openai-codex": RencoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": RencoOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": RencoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": RencoOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="RENCO_QWEN_BASE_URL",
    ),
    "lmstudio": RencoOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": RencoOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": RencoOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": RencoOverlay(
    ...
    "kimi-for-coding": RencoOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": RencoOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": RencoOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": RencoOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": RencoOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": RencoOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": RencoOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": RencoOverlay(
        transport="openai_chat",
    ...
    "opencode": RencoOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": RencoOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": RencoOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": RencoOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": RencoOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "renco", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Renco overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Renco overlay
    ...
    overlay = RENCO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + renco extra
    ...
    if overlay is not None:
        # Renco-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and RENCO_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Renco overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in RENCO_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Renco provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from renco_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
