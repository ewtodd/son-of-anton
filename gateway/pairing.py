...
def _sync_live_adapter_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    Pairing revoke updates the platform's allowlist env, but when the
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    """Remove ``user_id`` (and platform alias equivalents) from the allowlist.

    Matching must mirror PairingStore / authz alias rules: approve
    mirrors a normalized principal into the platform allowlist, while revoke
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from son_of_anton_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{SON_OF_ANTON_HOME}/pairing`` while newer code/docs may
    write ``{SON_OF_ANTON_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own SON_OF_ANTON_HOME using the same legacy/consolidated layout rules
    as ``son-of-anton -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current SON_OF_ANTON_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp SON_OF_ANTON_HOME
        ...
        if profile:
            ...
            self._dir = get_son_of_anton_dir(
                "platforms/pairing",
                "pairing",
                home=profile_home,
            )
        ...
        if profile:
            ...
            # ``son-of-anton -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `son-of-anton` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> son-of-anton pairing approve ...` as root, "
                    "re-run with `docker exec -u son-of-anton <container> ...` and "
                    "chown the existing file to the son-of-anton user, or restart the "
