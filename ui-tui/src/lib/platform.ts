 ...
 * son-of-anton-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``son_of_anton_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but son-of-anton-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS son-of-anton-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on son-of-anton-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is son-of-anton-ink's Alt signal and accepting it would
