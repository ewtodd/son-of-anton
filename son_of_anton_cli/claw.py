"""son-of-anton claw — OpenClaw migration commands.

Usage:
    son-of-anton claw migrate              # Preview then migrate (always shows preview first)
    son-of-anton claw migrate --dry-run    # Preview only, no changes
    son-of-anton claw migrate --yes        # Skip confirmation prompt
    son-of-anton claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    son-of-anton claw migrate --no-backup  # Skip pre-migration snapshot
    son-of-anton claw cleanup              # Archive leftover OpenClaw directories
    son-of-anton claw cleanup --dry-run    # Preview what would be archived
"""
...
from son_of_anton_cli.config import get_son_of_anton_home, get_config_path, load_config, save_config
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print()
    print_error(
        "Son of Anton gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_son_of_anton.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_son_of_anton", script_path)
...
def claw_command(args):
    """Route son-of-anton claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: son-of-anton claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Son of Anton")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'son-of-anton claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Son of Anton migration."""
    ...
    # migrate-son-of-anton posture (two-phase: run once without secrets, rerun with
    ...
    print(
        color(
            "│          ⚕ Son of Anton — OpenClaw Migration                 │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: son-of-anton claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.son-of-anton/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: son-of-anton claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Son of Anton home ─────────
    # Delegates to son_of_anton_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `son-of-anton import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from son_of_anton_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(son_of_anton_home=son_of_anton_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: son-of-anton import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Son of Anton home."
            )
    ...
    # 'son-of-anton claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: son-of-anton claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: son-of-anton claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  son-of-anton claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  son-of-anton claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  son-of-anton config set OPENROUTER_API_KEY sk-or-v1-...")
