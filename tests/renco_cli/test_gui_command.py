"""Tests for ``renco gui`` desktop launcher wiring."""
...
from renco_cli import main as cli_main
...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    call live, and on a Linux dev box it wrote a ``renco.desktop`` pointing
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("RENCO_DESKTOP_PASSWORD_STORE", raising=False)
...
def _make_packaged_executable(root: Path, monkeypatch) -> Path:
    ...
    if sys.platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Renco.app" / "Contents" / "MacOS" / "Renco"
    elif sys.platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Renco.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "renco"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("renco_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("renco_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("renco_cli.main._desktop_build_needed", return_value=True), \
         patch("renco_cli.main._write_desktop_build_stamp"), \
         patch("renco_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("renco_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("renco_cli.main._register_linux_desktop_entry"), \
         patch("renco_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Renco-managed
    Node ahead of that bare PATH or the install dies with ``node: not found``.
    """
    import os

    from renco_constants import iter_renco_node_dirs

    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    # A managed Node tree on disk so with_renco_node_path() actually prepends it.
    home = tmp_path / "renco-home"
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["renco"], 0)
    ...
    with patch("renco_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("renco_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("renco_cli.main._desktop_build_needed", return_value=True), \
         patch("renco_cli.main._write_desktop_build_stamp"), \
         patch("renco_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("renco_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("renco_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(skip_build=False))

    managed_dirs = [str(p) for p in iter_renco_node_dirs() if p.is_dir()]
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Renco.app/Contents/MacOS/Renco in place. Re-downloading Electron can't
    ...
    with patch("renco_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("renco_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("renco_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("renco_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("renco_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("renco_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Renco.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "Renco.app"
    _write_info_plist(app, "com.nousresearch.renco")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Renco").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "Renco Helper.app"
    _write_info_plist(helper, "com.nousresearch.renco.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Renco.app component, silently skipping every .node/.dylib/crashpad
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`renco desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch)

    registered: list[Path] = []
    monkeypatch.setattr("renco_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "renco_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: registered.append(project_root) or (tmp_path / "renco.desktop"),
    )

    launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)

    with patch("renco_cli.main._desktop_build_needed", return_value=False), \
         patch("renco_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("renco_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("renco_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("renco_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("renco_cli.linux_desktop_entry.install_desktop_entry", boom)
    ...
    with patch("renco_cli.main._desktop_build_needed", return_value=False), \
         patch("renco_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("renco_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("renco_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("renco_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("renco_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_desktop_launch_options_normalizes_password_store(raw, expected):
    ...
    with patch("renco_cli.config.load_config", return_value=cfg):
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("renco_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert cli_main._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("renco_cli.main.subprocess.run", return_value=ping_fail):
        assert cli_main._detect_linux_password_store() is None
    with patch("renco_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert cli_main._detect_linux_password_store() is None
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("renco_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("renco_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("renco_cli.main._desktop_build_needed", return_value=True), \
         patch("renco_cli.main._write_desktop_build_stamp"), \
         patch("renco_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("renco_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("renco_cli.config.load_config", return_value={}), \
         patch("renco_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("renco_cli.main._detect_linux_password_store", return_value="gnome-libsecret"), \
         patch("renco_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["RENCO_DESKTOP_PASSWORD_STORE"] == "gnome-libsecret"
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("renco_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("renco_cli.config.load_config", return_value={}), \
         patch("renco_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("renco_cli.main._detect_linux_password_store", return_value="kwallet6"), \
         patch("renco_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["RENCO_DESKTOP_PASSWORD_STORE"] == "kwallet6"
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with patch("renco_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("renco_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("renco_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("renco_cli.config.load_config", return_value=cfg), \
         patch("renco_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("renco_cli.main._detect_linux_password_store") as mock_detect, \
         patch("renco_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_explicit_password_store_env_wins_over_config_and_detection(tmp_path, monkeypatch):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("RENCO_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["RENCO_DESKTOP_PASSWORD_STORE"] == "basic"
...
def test_gui_password_store_bridge_is_linux_only(tmp_path, monkeypatch):
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert "RENCO_DESKTOP_PASSWORD_STORE" not in launch_env
