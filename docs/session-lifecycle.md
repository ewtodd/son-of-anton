...
The canonical transcript store is SQLite via `SessionDB` (from `renco_state`). The
...
Counts consecutive restarts via a JSON file (`{RENCO_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `renco update`, `renco gateway restart`,
or `/restart`.
