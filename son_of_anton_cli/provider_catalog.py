...
The provider list shown by ``son-of-anton model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`son_of_anton_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``son-of-anton model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`son_of_anton_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``son-of-anton model`` shows.
"""
...
# son_of_anton_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `son-of-anton model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``son-of-anton model`` universe.
    ...
    # Son of Anton overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = SON_OF_ANTON_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Son of Anton overlay, then api_key.
        auth_type = (
