...
interface RencoConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function RencoConsoleModal({ open, onClose }: RencoConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("renco> ");
  const inputPromptRef = useRef("renco> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "renco> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Renco Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="renco-console-title"
    >
      <div
        className={cn(
          themedBody,
