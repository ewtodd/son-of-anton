...
import type { FrameEvent } from '@renco/ink'
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `renco-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? dump?.diagPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    try {
      process.stderr.write(`renco-tui lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
    } catch {
  ...
  onSignal: signal => {
    ...
    resetTerminalModes()
    process.stderr.write(`renco-tui lifecycle: received ${signal}\n`)
  },
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write(
      `renco-tui lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`
    )
    process.stderr.write(dumpNotice(snap, dump))
    process.stderr.write('renco-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  ...
  onWarn: snap => {
    ...
    process.stderr.write(
      `renco-tui: heap climbing fast (${formatBytes(snap.heapUsed)}) — a large tool output or long session may be straining memory\n`
    )
  }
})

if (process.env.RENCO_HEAPDUMP_ON_START === '1') {
