/**
 * Shared E2E fixtures for the Renco desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → renco serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `renco desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `renco-e2e-${prefix}-${Math.random()}`))
  const rencoHome = path.join(root, 'renco-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(rencoHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(rencoHome, 'config.yaml')
 ...
 */
export function writeEnvFile(rencoHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(rencoHome, '.env')
 ...
 */
function writeEmptyConfig(rencoHome: string): void {
  const configPath = path.join(rencoHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - RENCO_HOME → sandbox renco-home (isolated config/sessions)
 *  - RENCO_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - RENCO_DESKTOP_IGNORE_EXISTING=1 → don't pick up `renco` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - RENCO_DESKTOP_RENCO_ROOT → REPO_ROOT (dev checkout resolution)
 *  - RENCO_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    RENCO_HOME: sandbox.rencoHome,
    RENCO_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    RENCO_DESKTOP_IGNORE_EXISTING: '1',
    RENCO_DESKTOP_RENCO_ROOT: REPO_ROOT,
    RENCO_DESKTOP_APP_NAME: `RencoE2E-${Date.now()}`,
    ...
    RENCO_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated RENCO_HOME + userData
 * @param env      - the process environment (already has RENCO_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.rencoHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.rencoHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via RENCO_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`renco serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.rencoHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { RENCO_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Renco backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Renco.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Renco.app', 'Contents', 'MacOS', 'Renco')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'renco')
}
 ...
 * progress without spawning a real Renco backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * RENCO_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    RENCO_DESKTOP_BOOT_FAKE: '1',
    RENCO_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + renco-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).RENCO_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).RENCO_DESKTOP_RENCO
  delete (env as Record<string, string | undefined>).RENCO_DESKTOP_RENCO_ROOT
