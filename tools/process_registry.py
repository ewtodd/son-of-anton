...
CHECKPOINT_PATH = get_son_of_anton_home() / "processes.json"
...
# ---------------------------------------------------------------------------
# When Son of Anton runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
#
# Wrapping the spawn in ``systemd-run --user --scope --unit=son-of-anton-worker-<pid>``
...
def _build_systemd_scope_argv(
    ...
    if binary is None:
        ...
        return shell_argv
    unit_name = f"son-of-anton-worker-{unit_suffix}"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from son_of_anton_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_local(
        ...
        if use_pty:
            ...
            try:
                ...
                if pty_use_systemd_scope:
                    ...
                    session.systemd_unit = f"son-of-anton-worker-{session.id}.scope"
        ...
        if use_systemd_scope:
            ...
            session.systemd_unit = f"son-of-anton-worker-{unit_suffix}.scope"
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/son_of_anton_bg_{session.id}.log"
        pid_path = f"{temp_dir}/son_of_anton_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/son_of_anton_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``son_of_anton_cli.goals``) to support
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `son-of-anton update` restarting the gateway) is still
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Son of Anton desktop app.",
            }
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.son-of-anton/processes.json with the raw command
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Son of Anton'}"
    elif _reason == "lost":
