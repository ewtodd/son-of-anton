"""Runtime-backed validation behind ``son-of-anton plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary SON_OF_ANTON_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="son-of-anton-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            os.environ,
            {
                ...
                "SON_OF_ANTON_BUNDLED_PLUGINS": str(bundled),
                "SON_OF_ANTON_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    modules_before = {
        ...
        if name == "son_of_anton_plugins" or name.startswith("son_of_anton_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Son of Anton discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "son_of_anton_plugins" or name.startswith("son_of_anton_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_son_of_anton_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".son-of-anton" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from son_of_anton_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Son of Anton supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Son of Anton never auto-installs plugin dependencies; "
    ...
    if schema:
        from son_of_anton_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Son of Anton' real scanner and registration path."""
    try:
