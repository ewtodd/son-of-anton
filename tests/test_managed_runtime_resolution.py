"""Guard: Renco-owned subprocesses must not resolve managed runtimes by bare PATH.

Renco installs runtimes for itself — ``uv`` at ``$RENCO_HOME/bin/uv``, Node at
``$RENCO_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Renco's own
...
* when a system copy also exists, the one Renco does not own wins — which is
...
``iter_renco_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Renco provisions into RENCO_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Renco-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Renco-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$RENCO_HOME/bin (uv) and $RENCO_HOME/node are not on an "
        ...
        "  node/npm -> renco_constants.find_node_executable()\n"
        "  PATH env -> renco_constants.iter_renco_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(renco_constants, helper))


def test_managed_uv_helpers_exist():
    from renco_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
