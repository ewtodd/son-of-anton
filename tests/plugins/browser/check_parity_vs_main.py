...
Run from the PR worktree:
    ...
    cd ~/.renco/renco-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.renco/renco-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.renco/renco-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a renco-agent checkout"
)
...
# Isolated RENCO_HOME for the config write.
