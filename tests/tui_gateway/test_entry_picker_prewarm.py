...
guard is covered in ``tests/renco_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``renco_cli.model_switch.prewarm_picker_cache_async``
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from renco_cli.model_switch,
