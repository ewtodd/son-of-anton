...
sidebar_position: 1
title: "Architecture"
description: "Renco Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Renco Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ renco_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # RencoCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── renco_state.py           # SQLite session/state database with FTS5
├── renco_constants.py       # RENCO_HOME, profile-aware paths
...
│
├── renco_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `renco` subcommands (large file)
...
│   ├── skills_config.py      # renco skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # renco tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # renco gateway start/stop
│
...
```text
User input → RencoCLI.process_input()
...
Three discovery sources: `~/.renco/plugins/` (user), `.renco/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `renco plugins` or `config.yaml`.
...
Exposes Renco as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`renco -p <name>`) gets its own RENCO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
