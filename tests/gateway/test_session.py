...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_son_of_anton_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("son_of_anton_constants.display_son_of_anton_home", return_value="~/.son-of-anton/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.son-of-anton/profiles/coder/cron/output/" in prompt
...
class TestWhatsAppSessionKeyConsistency:
    ...
    def test_whatsapp_group_participant_aliases_share_session_key(self, tmp_path, monkeypatch):
        ...
        per-user sessions."""
        tmp_home = tmp_path / "son-of-anton-home"
        ...
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_home))
