...
React + Ink terminal UI for Renco. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `RENCO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `renco setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.renco/.renco_history` or under `RENCO_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `renco setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/renco-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external renco setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.renco/tui-parent.log
    ...
    types/
      renco-ink.d.ts            type declarations for @renco/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent RencoCLI subprocess for slash commands
```
