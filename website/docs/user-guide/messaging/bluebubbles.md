...
Connect Renco to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Renco and the BlueBubbles server
...
Or set environment variables directly in `~/.renco/.env`:
...
By default, Renco responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Renco uses conservative defaults for `Renco` and `@Renco agent` variants.
...
When someone messages your iMessage, Renco automatically sends them a pairing code. Approve it with:
```bash
renco pairing approve bluebubbles <CODE>
```
Use `renco pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.renco/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.renco/.env`):
```bash
...
Renco will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Renco
Renco → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Renco sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Renco wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.renco/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Renco resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `renco logs gateway` for webhook errors (or `renco logs -f` to follow in real-time)
