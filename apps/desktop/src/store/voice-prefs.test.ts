...
vi.mock('@/renco', () => ({
  getRencoConfigRecord: vi.fn(async () => ({})),
  saveRencoConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye renco', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye renco')
  })
