"""Tests for son-of-anton backup and import commands."""
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew son_of_anton_cli.backup's datetime forward instead of sleeping.
    ...
    import son_of_anton_cli.backup as _backup

    shim = getattr(_backup.datetime, "_son_of_anton_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _son_of_anton_test_shim = True
...
def _make_son_of_anton_tree(root: Path) -> None:
    """Create a realistic ~/.son-of-anton directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "son_of_anton_state.db"):
    ...
    # son-of-anton repo (should be EXCLUDED)
    (root / "son-of-anton").mkdir(exist_ok=True)
    (root / "son-of-anton" / "run_agent.py").write_text("# big file\n")
    (root / "son-of-anton" / ".git").mkdir()
    (root / "son-of-anton" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_son_of_anton_agent(self):
        from son_of_anton_cli.backup import _should_exclude
        assert _should_exclude(Path("son-of-anton/run_agent.py"))
        assert _should_exclude(Path("son-of-anton/.git/HEAD"))
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        _make_son_of_anton_tree(son_of_anton_home)
        ...
        import son_of_anton_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = son_of_anton_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, son_of_anton_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside SON_OF_ANTON_HOME."""
        son_of_anton_home = tmp_path / ".son-of-anton"
        son_of_anton_home.mkdir()
        _make_son_of_anton_tree(son_of_anton_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(son_of_anton_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Son of Anton backup."""
        from son_of_anton_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Son of Anton configuration has been restored." in out
        assert "son-of-anton gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        son_of_anton_home = tmp_path / ".son-of-anton"
        (son_of_anton_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(son_of_anton_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (son_of_anton_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (son_of_anton_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            son_of_anton_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (son_of_anton_home / "gateway.pid").write_text("4242")
        (son_of_anton_home / "processes.json").write_text('{"live": true}')
        ...
        assert (son_of_anton_home / "gateway.pid").read_text() == "4242"
        assert (son_of_anton_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (son_of_anton_home / "cron.pid").exists()
        assert not (son_of_anton_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (son_of_anton_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".son-of-anton"
        src_home.mkdir(parents=True)
        _make_son_of_anton_tree(src_home)

        monkeypatch.setenv("SON_OF_ANTON_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from son_of_anton_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".son-of-anton"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(dst_home))
        ...
        # son-of-anton should NOT be present
        assert not (dst_home / "son-of-anton").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from son_of_anton_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from son_of_anton_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".son-of-anton/", "")
            zf.writestr(".son-of-anton/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_son_of_anton_home(self, tmp_path, monkeypatch):
        """Backup handles empty son-of-anton home (no files to back up)."""
        son_of_anton_home = tmp_path / ".son-of-anton"
        son_of_anton_home.mkdir()
        # Only excluded dirs, no actual files
        (son_of_anton_home / "__pycache__").mkdir()
        (son_of_anton_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        son_of_anton_home = tmp_path / ".son-of-anton"
        son_of_anton_home.mkdir()
        (son_of_anton_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = son_of_anton_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        son_of_anton_home = tmp_path / ".son-of-anton"
        son_of_anton_home.mkdir()
        (son_of_anton_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(son_of_anton_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (son_of_anton_home / "config.yaml").exists()
        assert (son_of_anton_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`son-of-anton import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the SON_OF_ANTON_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (son_of_anton_home / "config.yaml").write_text(original)
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(son_of_anton_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (son_of_anton_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(son_of_anton_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside SON_OF_ANTON_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        son_of_anton_home = dst_home / ".son-of-anton"
        son_of_anton_home.mkdir()
    ...
    def test_symlinked_target_keeps_its_symlink(self, tmp_path, monkeypatch):
        ...
        son_of_anton_home = tmp_path / ".son-of-anton"
        son_of_anton_home.mkdir()
        store = son_of_anton_home / "store"
        store.mkdir()
        real = store / "config.yaml"
        real.write_text("model: original\n")
        link = son_of_anton_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        son_of_anton_home = tmp_path / ".son-of-anton"
        son_of_anton_home.mkdir()
        target = son_of_anton_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo son-of-anton import`` onto a user-owned (or Docker/NAS volume-owned)
        SON_OF_ANTON_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "son_of_anton_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``son-of-anton import`` is the
        ...
        son_of_anton_home = tmp_path / ".son-of-anton"
        son_of_anton_home.mkdir()
        target = son_of_anton_home / "helper.sh"
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from son_of_anton_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from son_of_anton_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from son_of_anton_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def son_of_anton_home(self, tmp_path):
        """Create a fake SON_OF_ANTON_HOME with critical state files."""
    ...
    def test_state_db_safely_copied(self, son_of_anton_home):
        from son_of_anton_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(son_of_anton_home=son_of_anton_home)
        db_copy = son_of_anton_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, son_of_anton_home, monkeypatch, capsys):
        ...
        snap_id = backup_mod.create_quick_snapshot(son_of_anton_home=son_of_anton_home)
        err = capsys.readouterr().out
        assert "SQLite safe copy FAILED" in err or "CRITICAL" in err
        assert "state.db" in err
        # Other small files may still snapshot
        if snap_id:
            manifest = (son_of_anton_home / "state-snapshots" / snap_id / "manifest.json")
    ...
    def test_snapshot_includes_pairing_directories(self, son_of_anton_home):
        ...
        (son_of_anton_home / "platforms" / "pairing").mkdir(parents=True)
        (son_of_anton_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (son_of_anton_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (son_of_anton_home / "pairing").mkdir()
        (son_of_anton_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (son_of_anton_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = son_of_anton_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (son-of-anton update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, son_of_anton_home, capsys):
        ...
        """
        import json
        from son_of_anton_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", son_of_anton_home=son_of_anton_home)
        assert first_id is not None
        first_dir = son_of_anton_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", son_of_anton_home=son_of_anton_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = son_of_anton_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, son_of_anton_home=son_of_anton_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, son_of_anton_home):
        ...
        from son_of_anton_cli.backup import create_quick_snapshot, restore_quick_snapshot

        board_dir = son_of_anton_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(son_of_anton_home=son_of_anton_home)
        copy = (
            son_of_anton_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, son_of_anton_home=son_of_anton_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, son_of_anton_home, monkeypatch):
        ...
        import son_of_anton_cli.backup as bk
        from son_of_anton_cli.backup import create_quick_snapshot

        board = son_of_anton_home / "kanban" / "boards" / "work"
        ...
        copy = son_of_anton_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``son-of-anton update``
    ...
    def son_of_anton_home(self, tmp_path):
        ...
        root.mkdir()
        _make_son_of_anton_tree(root)
        return root


    def test_backup_contents_match_full_backup(self, son_of_anton_home):
        """Pre-update backup should include the same user data that
        ``son-of-anton backup`` would, and should exclude the same directories."""
        from son_of_anton_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(son_of_anton_home=son_of_anton_home)
        ...
        # son-of-anton repo excluded
        assert not any(n.startswith("son-of-anton/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, son_of_anton_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(son_of_anton_home=son_of_anton_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (son_of_anton_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, son_of_anton_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside SON_OF_ANTON_HOME."""
        from son_of_anton_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def son_of_anton_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".son-of-anton"
        root.mkdir()
        _make_son_of_anton_tree(root)
        # Point SON_OF_ANTON_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve SON_OF_ANTON_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(son_of_anton_home, value):
        import yaml
        (son_of_anton_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(son_of_anton_home):
        d = son_of_anton_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(son_of_anton_home):
        d = son_of_anton_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, son_of_anton_home, capsys):
        ...
        self._set_mode(son_of_anton_home, "off")
        from son_of_anton_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(son_of_anton_home)
        assert not self._zips(son_of_anton_home)



    def test_config_full_mode(self, son_of_anton_home, capsys):
        self._set_mode(son_of_anton_home, "full")
        from son_of_anton_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(son_of_anton_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (son-of-anton claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``son-of-anton claw migrate`` runs before mutating ~/.son-of-anton/."""
    ...
    def test_restorable_with_son_of_anton_import(self, son_of_anton_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Son of Anton
        backup — `son-of-anton import` should accept it."""
        from son_of_anton_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(son_of_anton_home=son_of_anton_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, son_of_anton_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from son_of_anton_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(son_of_anton_home=son_of_anton_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(son_of_anton_home=son_of_anton_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`son-of-anton update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, son_of_anton_home: Path, label="pre-update"):
        from son_of_anton_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, son_of_anton_home=son_of_anton_home, keep=5)
    ...
    def test_restores_when_emptied_after_migration(self, tmp_path):
        ...
        son_of_anton_home = tmp_path / ".son-of-anton"
        jobs_path = son_of_anton_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(son_of_anton_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, son_of_anton_home=son_of_anton_home)
        assert result is not None
...
# location, NOT under SON_OF_ANTON_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, son_of_anton_home: Path) -> None:
        son_of_anton_home.mkdir(parents=True, exist_ok=True)
        (son_of_anton_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (son_of_anton_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (son_of_anton_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        son_of_anton_home = tmp_path / ".son-of-anton"
        self._make_min_tree(son_of_anton_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under SON_OF_ANTON_HOME,
        ...
        # External state did NOT leak into SON_OF_ANTON_HOME.
        assert not (son_of_anton_home / "_external").exists()
