"""Tests for --ignore-user-config and --ignore-rules flags on `son-of-anton chat`.
...
files. In Son of Anton the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.son-of-anton/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → SonOfAntonCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("SON_OF_ANTON_IGNORE_USER_CONFIG", "SON_OF_ANTON_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``SON_OF_ANTON_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<son_of_anton_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With SON_OF_ANTON_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._son_of_anton_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_son_of_anton_home", tmp_path)
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        """With SON_OF_ANTON_IGNORE_USER_CONFIG=1, user config.yaml is ignored.

        The built-in default ``model.default`` is empty string (no user override),
        and the user's ``agent.system_prompt`` is not seen.
        """
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("SON_OF_ANTON_IGNORE_USER_CONFIG", "1")
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("SON_OF_ANTON_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``SonOfAntonCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting SON_OF_ANTON_IGNORE_RULES=1 flips SonOfAntonCLI.ignore_rules True."""
        monkeypatch.setenv("SON_OF_ANTON_IGNORE_RULES", "1")

        # Import SonOfAntonCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of SonOfAntonCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.SonOfAntonCLI)
        # Replicate the exact logic from cli.py SonOfAntonCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("SON_OF_ANTON_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``son_of_anton_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("SON_OF_ANTON_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("SON_OF_ANTON_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("SON_OF_ANTON_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("SON_OF_ANTON_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "SON_OF_ANTON_IGNORE_USER_CONFIG" not in os.environ
        assert "SON_OF_ANTON_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_flags_present_in_chat_parser(self):
        ...
        import argparse
        parser = argparse.ArgumentParser(prog="son-of-anton")
