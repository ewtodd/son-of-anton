...
* The requested key is passed to the child ONLY via the ``SON_OF_ANTON_SECRET_KEY``
...
# son_of_anton_cli.env_loader can consume both providers identically.
...
def _run_helper(
    ...
    The key is passed as DATA via ``SON_OF_ANTON_SECRET_KEY`` — never interpolated
    ...
    if source_env is os.environ:
        ...
        env = dict(source_env)
    env["SON_OF_ANTON_SECRET_KEY"] = secret_key
...
def get_command_secret(
    *,
    ...
    ``SON_OF_ANTON_SECRET_KEY``.  Returns None on any failure — never raises."""
...
class CommandSource(SecretSource):
    ...
    Bulk shape: the helper enumerates a KEY=VALUE blob in one run.  Config::
        ...
        secrets:
          command:
            enabled: true
            command: "cat /run/user/1000/son-of-anton-secrets.env"
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "Son of Anton discards helper stderr so diagnostics can't leak "
                "secret material."
            )
