...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: '正在加载 Renco 设置',
      loadingSessions: '正在加载最近会话',
      retryingRemoteBackend: '正在重新连接远程 Renco 后端…',
      startingDesktopConnection: '正在启动桌面连接',
      startingRencoDesktop: '正在启动 Renco 桌面版…'
    },
    errors: {
      backgroundExited: 'Renco 后台进程已退出。',
      backgroundExitedDuringStartup: 'Renco 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Renco 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    installMethodUnsupportedTitle: '不受支持的安装方式',
    updateRenco: '更新 Renco',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 Renco Desktop。',
    ...
    native: {
      ...
      rejectAction: '拒绝',
      inputTitle: '需要输入',
      inputBody: 'Renco 正在等待你的回应。',
      turnDoneTitle: 'Renco 已完成',
      turnDoneBody: '',
      turnErrorTitle: '本轮失败',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Renco 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 Renco 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      openFolder: '打开插件文件夹',
      rescan: '重新扫描',
      reveal: '在文件管理器中显示',
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '你安装到 Renco 后端的插件——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        appliesTo: '应用于：',
        empty: '尚未安装智能体插件。',
        ...
        updateBackendToManage: '请更新 Renco 后端以便在桌面端管理此插件。',
      ...
      installModal: {
        ...
        agentLabel: '智能体插件',
        desktopLabel: '桌面 UI',
        agentTargetLocal: profile => `安装到 ${profile} 后端（~/.renco/plugins/）`,
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 Renco 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: 'Renco 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'Renco 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '后台回合错误。'
        ...
        plugin: {
          label: '插件通知',
          description: 'Renco 在后台时，桌面插件发送了通知。'
        }
      },
      test: '发送测试通知',
      testTitle: 'Renco',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Renco 跟随系统设置。',
      ...
      reactionsTitle: '消息回应',
      reactionsDesc: 'iMessage 风格的表情回应 — 你可以给消息添加回应，Renco 也能回应你的消息。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 Renco 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 Renco，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 Renco 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'Renco 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'Renco 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'Renco 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Renco 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Renco 运行时已更新,但桌面应用本身仍是旧构建——在应用更新之前,新的界面功能(如 Bot Mode)不会显示。请运行下方的更新以重新构建应用。如果此警告仍未消除,请从最新的桌面安装程序重新安装。',
      ...
      automaticUpdatesDesc: 'Renco 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    quickEntry: {
      enabledTitle: '快速输入',
      enabledDesc: '用全局快捷键在任何地方唤出一个小输入框，无需打开 Renco 即可发送提示。',
    ...
    connections: {
      title: '已注册网关',
      intro: '管理本机以及通过远程、SSH 或 Renco Cloud 连接可访问的每个 Renco 网关。',
      stagedNote:
      ...
      updateSkippedCloud: '由 Renco Cloud 托管',
      kindLocal: '本地',
      kindRemote: '远程网关',
      kindCloud: 'Renco Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: '由本应用管理的 Renco 运行时。',
      kindRemoteDesc: '可通过 HTTP(S) 访问的 Renco 网关——局域网、Tailscale 或互联网。',
      kindCloudDesc: '通过你的 Renco Cloud 账户发现的托管实例。',
      kindSshDesc: '通过 SSH 访问的 Renco 安装。',
      labelTitle: '名称',
      ...
      headersDesc:
        '随发往此网关的每个 HTTP 和 WebSocket 请求一起发送——用于 Cloudflare Access 等访问代理（CF-Access-Client-Id / CF-Access-Client-Secret）。值加密存储。由 Renco 管理的请求头（Authorization、Cookie、Host 等）会被忽略。',
      ...
      cloudAddHint: '提示：在上方登录 Renco Cloud 可自动发现你的智能体——此表单仅用于手动注册已知的实例 URL。',
      save: '保存连接',
      saving: '保存中…',
      cancel: '取消',
      empty: '尚未注册任何连接。'
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Renco Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Renco 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
      envOverrideTitle: '环境变量正在控制此桌面会话。',
      envOverrideDesc: '取消设置 RENCO_DESKTOP_REMOTE_URL 和 RENCO_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。',
      modeTitle: '连接模式',
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 Renco 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc: '将此桌面外壳连接到远程 Renco 后端。',
      remoteAuthHint: '托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      cloudTitle: 'Renco Cloud',
      cloudDesc: '只需登录 Renco Cloud 一次，即可从你账户下的智能体中选择——无需粘贴 URL。',
      cloudSignInTitle: 'Renco Cloud',
      cloudSignIn: '登录 Renco Cloud',
      cloudSignedIn: '已登录 Renco Cloud',
      cloudNeedsSignIn: '登录 Renco Cloud 以发现你账户下的智能体。',
      ...
      cloudDiscoverFailed: '无法加载你的 Renco Cloud 智能体',
      cloudConnectFailed: '无法连接到该智能体',
      cloudSignInFailed: 'Renco Cloud 登录失败',
      cloudSignedOutTitle: '已退出 Renco Cloud',
      cloudSignedOutMessage: '已清除 Renco Cloud 会话。',
      ...
      remoteUrlDesc: '远程 dashboard 后端的基础 URL。支持路径前缀，例如 /renco。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Renco Desktop 将使用已保存设置重新连接（界面保持打开）。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · Renco ${version}` : ''}`,
      ...
      sshTitle: '通过 SSH 连接',
      sshDesc:
        'Renco 会通过 SSH 在远程启动并以隧道连接到本应用——无需自行启动或暴露任何服务。前提：已具备到该主机的密钥 SSH 访问。',
      ...
      sshRencoPathTitle: 'Renco 路径（可选）',
      sshRencoPathDesc: '远程 renco 可执行文件的完整路径。留空 = 自动检测。',
      sshRencoPathPlaceholder: '自动检测',
      sshTestConnection: '测试 SSH',
      sshConnect: '连接',
      sshButtonsHint: '“保存”将在下次启动时生效，“连接”则立即重新连接。',
      sshReachable: (host, platform) => `可连接：${host}（${platform}）——已找到 Renco`,
      sshIncompleteHost: '连接前请输入 SSH 主机。',
      sshErrUnreachable: '无法通过 SSH 连接到该主机。请检查主机、端口和网络。',
      sshErrAuth:
        'SSH 认证失败。请将密钥加载到 ssh-agent（ssh-add），或在 ~/.ssh/config 中设置 IdentityFile——Renco 以非交互方式运行 ssh。',
      sshErrHostKey: '自上次连接以来主机密钥已更改。请确认这是预期的，然后运行 ssh-keygen -R <host> 并重新连接。',
      sshErrNotInstalled:
        '远程主机上未安装 Renco。请在远程安装（curl -fsSL https://renco-agent.nousresearch.com/install.sh | sh）或设置 Renco 路径。',
      sshErrPlatform: '不支持的远程平台。Renco Desktop 的 SSH 模式支持 Linux、macOS 和 Windows 远程主机。',
      sshErrTimeout: 'SSH 连接超时。主机可能无法访问或处于休眠状态。',
      sshErrUpdateRequired: '使用 Desktop SSH 连接前，请更新远程主机上的 Renco。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        '一个链接请求将此 MCP 服务器添加到 Renco。请检查下方的完整配置——它来自该链接，而非 Renco。',
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。Renco 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
      ...
      localEndpoint: {
        title: '本地 / 自定义端点',
        description: '将 Renco 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在加载提供方...'
    },
    sessions: {
  ...
  skills: {
    ...
    skillArchivedMessage: '可通过 renco curator restore 恢复。',
    hub: {
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 Renco 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 Renco 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 Renco 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 Renco 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 Renco 桌面端' },
    ...
    rencoActiveSessions: (version, count) => `Renco ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
    openBrowser: '打开浏览器',
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'Bot 用户 ID', placeholder: '@renco:example.org' },
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 Renco 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Renco 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Renco 指向该 URL。',
      ...
      weixin:
        '运行 `renco gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。Renco 会通过腾讯 iLink Bot API 连接并保存凭据。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 Renco 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Renco 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。Renco 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 Renco 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 Renco 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
      ...
      worktreeStaleBackend: '请更新 Renco 后端以在此远程连接上创建工作树 —— 该后端早于 git 工作树 API。',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 Renco…',
    placeholderReconnecting: '正在重新连接 Renco…',
    ...
    attachUrlDesc: 'Renco 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 Renco 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    latestBody: '你正在运行最新版本。',
    latestBodyBackend: '后端正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 Renco 已可安装。',
    availableTitleBackend: '后端有可用更新',
    availableBodyBackend: '已连接的 Renco 后端有新版本可安装。',
    availableBodyNoChangelog: '已有新版本可用。此安装方式无法显示更新日志。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 Renco，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 Renco 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 Renco 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
      'Renco 更新器会在自己的窗口中接管，并在完成后自动重新打开 Renco。更新期间请不要自行重新打开 Renco。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Renco 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 Renco 会自动重新打开。',
    errorTitle: '更新未完成',
    errorBody: '没有数据丢失。你可以现在重试。',
    blockerTitle: '关闭本地预览以更新 Renco？',
    blockerBody: 'Renco 需要在更新前停止这些本地预览。这不会修改或删除你的文件。',
    foreignBlockerTitle: '关闭其他进程以更新 Renco',
    foreignBlockerBody: 'Renco 无法安全地自动关闭这些进程。请关闭拥有这些进程的应用、终端或服务，然后重试更新。',
    mixedBlockerBody: 'Renco 可以关闭下方列出的本地预览。其他进程必须手动关闭，更新才能继续。',
  ...
  install: {
    ...
    setupChoiceTitle: '设置 Renco Desktop',
    setupChoiceDesc: '将此应用连接到你已运行的 Renco 网关，或在这台电脑上本地安装 Renco。',
    connectExistingTitle: '连接到现有 Renco',
    connectExistingShort: '连接现有环境',
    connectExistingDesc: '使用会话令牌或浏览器登录连接远程后端。不会启动本地安装。',
    installLocalTitle: '本地安装 Renco',
    installLocalDesc: '下载 Renco，创建 Python 环境，并在这台电脑上运行后端。',
    localStartUnavailable: '无法启动本地安装。请重启 Renco Desktop 后重试。',
    remoteSetupTitle: '连接到现有 Renco',
    remoteSetupDesc: '输入网关 URL。Renco Desktop 会检测需要令牌还是浏览器登录。',
    remoteUrlTitle: '网关 URL',
    remoteUrlDesc: '使用 Renco 网关的基础 URL；远程地址请包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/renco',
    probing: '正在检测网关认证方式...',
    probeError: '无法连接到该 Renco 网关。',
    identityProvider: '你的身份提供方',
    authTitle: '认证',
    ...
    backToSetup: '返回',
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 Renco Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 Renco CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 Renco 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。Renco 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    headerTitle: '开始设置 Renco Agent',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: 'Renco 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 Renco…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 Renco 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 Renco 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — Renco 会自动连接',
    ...
    authorizeThere: '请在那里授权 Renco。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 Renco，连接会自动完成，无需复制或粘贴。`,
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。Renco 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'Renco 正在重启...',
      askRestart: '让 Renco 重启服务器',
      lookingRestart: taskId => `Renco 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'Renco 正在后台工作。可在预览控制台查看进度。',
      ...
      blankPageBody: '在上方输入地址开始浏览，或让 Renco 打开一个页面。',
      finishedRestarting: message => `Renco 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'Renco 无法重启服务器。',
      stillWorking: 'Renco 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    ...
    approval: {
      gatewayDisconnected: 'Renco 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.renco/config.yaml)。Renco 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'Renco 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'Renco 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 Renco 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 Renco 桌面版以保存图片',
    ...
    handoff: {
      ...
      timedOut: '等待网关超时。`renco gateway` 是否正在运行？'
    }
  },
