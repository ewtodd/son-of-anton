#!/usr/bin/env bash
# Canonical test runner for renco-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.renco/...)
#
# Usage:
...
# (RENCO_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.renco/renco-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.renco/renco-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${RENCO_PYTHON:-}" ] && [ -x "$RENCO_PYTHON" ] \
    && "$RENCO_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: RENCO_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `renco` binary rather
  # than the devShell hook.
  PYTHON="$RENCO_PYTHON"
  echo "▶ no local venv — using Nix dev venv via RENCO_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and RENCO_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.renco/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.renco"
...
#
#   * RENCO_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * RENCO_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     RENCO_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# RENCO_RUN_SLOW_PET_TESTS / RENCO_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no RENCO_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in RENCO_TEST_IMAGE RENCO_TEST_WORKERS RENCO_TEST_PATHS \
  RENCO_TEST_FILE_TIMEOUT RENCO_TEST_FILE_RETRIES RENCO_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${RENCO_RUN_SLOW_PET_TESTS:+RENCO_RUN_SLOW_PET_TESTS="$RENCO_RUN_SLOW_PET_TESTS"} \
  ${RENCO_E2E_BROWSER:+RENCO_E2E_BROWSER="$RENCO_E2E_BROWSER"} \
