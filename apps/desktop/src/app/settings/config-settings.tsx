...
import { getElevenLabsVoices, getRencoConfigSchema, saveRencoConfig } from '@/renco'
...
import type { ConfigFieldSchema, RencoConfigRecord } from '@/types/renco'

import { rencoConfigCacheWriter, useRencoConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<RencoConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useRencoConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => rencoConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['renco-config-schema'] : ['renco-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getRencoConfigSchema(scopeProfile ?? undefined),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveRencoConfig(config, scopeProfile ?? undefined)
  ...
  const applyConfig = (next: RencoConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: RencoConfigRecord) => {
