...
def test_slow_log_emitted_at_zero_threshold(db, monkeypatch, caplog):
    monkeypatch.setenv("SON_OF_ANTON_SEARCH_SLOW_MS", "0")
    with caplog.at_level(logging.INFO, logger="son_of_anton_state"):
...
def test_no_log_under_threshold(db, monkeypatch, caplog):
    monkeypatch.setenv("SON_OF_ANTON_SEARCH_SLOW_MS", "60000")
    with caplog.at_level(logging.INFO, logger="son_of_anton_state"):
