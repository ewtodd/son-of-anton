...
// background into RENCO_TUI_BACKGROUND, which detectLightMode treats as a
...
describe('seedBootEnvironment', () => {
  ...
  it('never seeds over explicit user signals', () => {
    ...
    const env: NodeJS.ProcessEnv = { RENCO_TUI_BACKGROUND: '#123456' }

    expect(seedBootEnvironment(cache({ background: '#ffffff' }), env).seededBackground).toBeNull()
    expect(env.RENCO_TUI_BACKGROUND).toBe('#123456')
  })
  ...
  it('replays a cached config pin coherently with the physical background', () => {
    ...
    expect(env.RENCO_TUI_THEME).toBe('light')
    expect(env.RENCO_TUI_BACKGROUND).toBe('#1e1e1e')
  ...
  it('replays a pinned dark on a light physical background too', () => {
    ...
    expect(env.RENCO_TUI_THEME).toBe('dark')
    expect(env.RENCO_TUI_BACKGROUND).toBe('#ffffff')
...
describe('invalidateBootBackground', () => {
  ...
  it('leaves a trusted OSC answer that overwrote the seed alone', () => {
    ...
    env.RENCO_TUI_BACKGROUND = '#282828'

    expect(invalidateBootBackground(env)).toBe(false)
    expect(env.RENCO_TUI_BACKGROUND).toBe('#282828')
  })

  it('is a no-op when nothing was seeded', () => {
    const env: NodeJS.ProcessEnv = { RENCO_TUI_BACKGROUND: '#ffffff' }
