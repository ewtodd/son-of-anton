...
Canonical owners:
  ...
  * ``son_of_anton_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._son_of_anton_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/son-of-anton/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "son_of_anton_cli/managed_scope.py",
