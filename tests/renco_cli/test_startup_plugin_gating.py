...
``renco_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``renco --help``, ``renco --version``,
``renco logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``renco --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from renco_cli import main as _main
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``renco photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain renco-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level renco <name> CLI command via PluginContext.
