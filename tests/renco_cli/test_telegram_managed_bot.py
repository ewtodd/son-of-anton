"""Tests for renco_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("renco_")
        assert name.endswith("_bot")
        assert len(name) == len("renco_") + 16 + len("_bot")
        assert len(name) <= 32
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="Renco & Friends",
        )
        assert "Renco+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "renco_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/RencoSetupBot/renco_abcdefghijklmnop_bot?name=Renco+Agent",
            "qr_payload": "https://t.me/newbot/RencoSetupBot/renco_abcdefghijklmnop_bot?name=Renco+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "renco_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Renco Agent")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="renco_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/RencoSetupBot/renco_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/RencoSetupBot/renco_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "renco_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "renco_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("renco_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("renco_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("renco_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from renco_cli.setup import _setup_telegram_auto

        assert callable(_setup_telegram_auto)

    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from renco_cli import setup

        seen = {}
        profile_home = tmp_path / ".renco" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_renco_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "renco_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from renco_cli.setup import _profile_name_from_renco_home

        assert (
            _profile_name_from_renco_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\renco\profiles\oracle")
            )
            == "oracle"
        )
