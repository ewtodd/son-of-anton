...
def _run_renco(home: Path, *args: str, marker: bool = False) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["RENCO_HOME"] = str(home)
    env["RENCO_KANBAN_HOME"] = str(home)
    for name in (
        "RENCO_KANBAN_BOARD",
        "RENCO_KANBAN_DB",
    ...
    if marker:
        env["RENCO_DELEGATED_CHILD_CONTEXT"] = "1"
    else:
        env.pop("RENCO_DELEGATED_CHILD_CONTEXT", None)
...
def test_delegated_child_kanban_cli_refusal_returns_nonzero_exit_status(tmp_path):
    ...
    created = _run_renco(home, "kanban", "create", "exit status probe", "--json")
