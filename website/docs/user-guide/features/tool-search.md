...
**Tool Search** is Renco' opt-in progressive-disclosure layer for that
...
:::info Built-in Renco tools never defer
The tools that make up Renco' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_RENCO_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Renco **unwraps the bridge** and
...
- **No JS sandbox.** Renco uses the simpler "structured tools" mode
