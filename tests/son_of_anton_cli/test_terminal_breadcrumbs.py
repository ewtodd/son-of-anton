"""Tests for son_of_anton_cli/terminal_breadcrumbs.py — per-terminal ``son-of-anton -c``.

Covers terminal id derivation (tty vs env vars vs none), breadcrumb
write/read roundtrip under a temp SON_OF_ANTON_HOME, stale-session fallback
...
from son_of_anton_cli import terminal_breadcrumbs as tb
...
def test_breadcrumb_roundtrip(son_of_anton_home, monkeypatch, no_terminal_env):
    ...
    files = list((son_of_anton_home / "terminal-sessions").iterdir())
    assert [f.name for f in files] == ["tty-dev-pts-7"]


def test_write_skipped_without_terminal_identity(son_of_anton_home, monkeypatch, no_terminal_env):
    _fake_no_tty(monkeypatch)
    tb.write_breadcrumb("20260815_120000_abc123")
    assert not (son_of_anton_home / "terminal-sessions").exists()
    assert tb.read_breadcrumb() is None


def test_two_terminals_do_not_clobber(son_of_anton_home, monkeypatch, no_terminal_env):
...
def test_stale_breadcrumb_ignored_and_pruned(son_of_anton_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/1")
    directory = son_of_anton_home / "terminal-sessions"
...
def test_corrupt_breadcrumb_returns_none(son_of_anton_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/1")
    directory = son_of_anton_home / "terminal-sessions"
    directory.mkdir(parents=True)
...
def test_resolve_picks_this_terminals_session(son_of_anton_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(son_of_anton_home, "20260815_100000_aaaaaa")
    _make_session(son_of_anton_home, "20260815_110000_bbbbbb")  # newer, other terminal
    tb.write_breadcrumb("20260815_100000_aaaaaa")
    assert tb.resolve_breadcrumb_session() == "20260815_100000_aaaaaa"


def test_resolve_falls_back_when_session_deleted(son_of_anton_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(son_of_anton_home, "20260815_110000_bbbbbb")
    tb.write_breadcrumb("20260815_100000_deleted")  # never existed / deleted
    assert tb.resolve_breadcrumb_session() is None


def test_resolve_projects_through_compression_chain(son_of_anton_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(son_of_anton_home, "20260815_100000_parent")
...
def test_config_gate_off_disables_writes_and_resolution(
    son_of_anton_home, monkeypatch, no_terminal_env
):
