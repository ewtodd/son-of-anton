...
def test_foreign_active_plugin_configuration_is_left_unchanged(
    ...
    try:
        ...
        assert relay.events == []
        assert "already active outside Son of Anton native ownership" in caplog.text
...
def test_legacy_exporter_env_without_plugins_toml_warns_and_stays_disabled(
    ...
    monkeypatch.delenv(relay_runtime.RELAY_PLUGINS_CONFIG_ENV, raising=False)
    monkeypatch.setenv("SON_OF_ANTON_NEMO_RELAY_ATOF_ENABLED", "1")
    monkeypatch.setenv("SON_OF_ANTON_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S", "30")
    relay = _FakeRelay()
    ...
    try:
        ...
        assert relay.events == []
        assert "no SON_OF_ANTON_NEMO_RELAY_PLUGINS_TOML was provided" in caplog.text
        assert "SON_OF_ANTON_NEMO_RELAY_ATOF_ENABLED" in caplog.text
        assert "SON_OF_ANTON_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S" in caplog.text
    finally:
        host.shutdown()
...
def test_initialization_failure_is_fail_open(explicit_static_config, caplog):
    ...
    try:
        ...
        assert "Son of Anton Relay plugin initialization failed" in caplog.text
    finally:
        host.shutdown()
...
def test_session_close_does_not_flush_during_concurrent_managed_publication(
    ...
    event_loop_thread = threading.Thread(
        ...
        name="son-of-anton-relay-session-close-regression",
        daemon=True,
    )
...
""".strip(),
    ...
    try:
        ...
        assert relay.events == []
        assert "Son of Anton [[dynamic_plugins]] records are unsupported" in caplog.text
...
agent_name = "Son of Anton Native Test"
agent_version = "test"
""".strip(),
        encoding="utf-8",
    )
