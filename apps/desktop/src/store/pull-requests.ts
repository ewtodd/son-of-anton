...
import type { RencoBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/renco'
...
export const $pullRequestsByBranch = atom<Record<string, RencoBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'renco.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('renco.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: RencoBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
