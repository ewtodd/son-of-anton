"""``son-of-anton sync`` subcommand parser — Skill Sync.

Cloned from ``son_of_anton_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    son-of-anton sync status                 show gate/opt-in/head state
    son-of-anton sync pull                   pull and materialize opted-in skills
    son-of-anton sync push                   push opted-in skills
    son-of-anton sync now                    reconcile: pull then push
    son-of-anton sync enable <skill>         opt a skill into sync
    son-of-anton sync disable <skill>        opt a skill out of sync
    son-of-anton sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    son-of-anton sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  son-of-anton sync status            what is synced, and from where\n"
            "  son-of-anton sync enable my-skill   include a skill in your sync\n"
            "  son-of-anton sync now               pull, then push\n"
            "  son-of-anton sync propose my-skill  share a skill with your team\n"
        ),
