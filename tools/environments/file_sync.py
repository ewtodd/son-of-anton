...
_FORCE_SYNC_ENV = "RENCO_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.renco") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.renco to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``RENCO_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, renco_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.renco/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(renco_home=renco_home)

    def _sync_back_transaction(self, renco_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="renco-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.renco/skills/a.md`` →
        ``~/.renco/skills/a.md``, a new remote file at
        ``/root/.renco/skills/b.md`` maps to ``~/.renco/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
