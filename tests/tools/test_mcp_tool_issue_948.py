...
def test_resolve_stdio_command_falls_back_to_renco_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool.shutil.which", return_value=None), \
         patch.dict("os.environ", {"RENCO_HOME": str(tmp_path)}, clear=False):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$RENCO_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the Renco Docker image
    ...
    # the other candidates ($RENCO_HOME/node/bin/npx and ~/.local/bin/npx)
