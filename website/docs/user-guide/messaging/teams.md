...
description: "Set up Renco Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Renco Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `renco gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Renco strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create renco-bot --allow-anonymous
devtunnel port create renco-bot -p 3978 --protocol http  # replace 3978 with TEAMS_PORT if changed
devtunnel host renco-bot
...
The public tunnel URL uses HTTPS, but Renco' local webhook listener uses plain HTTP. The tunnel terminates TLS and forwards HTTP to port `3978`; do not configure the local tunnel port as HTTPS.
...
**Docker** (must run from the directory that contains `docker-compose.yml` — usually your cloned `renco-agent` repo, not `~`):

```bash
cd /path/to/renco-agent
RENCO_UID=$(id -u) RENCO_GID=$(id -g) docker compose up -d gateway
```

**Native / systemd install** (typical `renco` one-liner installer under `~/.renco/renco-agent`):

```bash
renco gateway restart
# or foreground: renco gateway run
```

The Teams SDK is optional; when Teams is enabled, the gateway lazy-installs it into Renco' own venv on first start (do **not** use system `pip install` on Ubuntu 24.04 — that hits PEP 668 `externally-managed-environment`). To install manually into the Renco venv:

```bash
~/.renco/renco-agent/venv/bin/pip install microsoft-teams-apps aiohttp
...
Alternatively, configure via `~/.renco/config.yaml`:
...
For a permanent server, terminate TLS at a reverse proxy and forward requests to the plain HTTP Renco listener, normally `http://127.0.0.1:3978`. Register the proxy's public HTTPS endpoint with Teams:
...
Make sure the public HTTPS endpoint is reachable from the internet and uses a valid TLS certificate. Teams rejects self-signed certificates. Keep the Renco listener behind the proxy; port `3978` does not serve HTTPS itself.
...
| Problem | Solution |
|---------|----------|
| `Can't find a suitable configuration file` from `docker compose` | You are not in the repo that has `docker-compose.yml`, or you are on a native install — use `renco gateway restart` instead, or `cd` into the clone first |
| `requirements not met` / `Teams SDK missing` / `No adapter available for teams` | Restart gateway so lazy-install can run, or install into the **Renco venv**: `~/.renco/renco-agent/venv/bin/pip install microsoft-teams-apps aiohttp`. System `pip` fails on Ubuntu 24.04 (PEP 668) and would not affect the service anyway |
...
| Logs show `"UNKNOWN / HTTP/1.0" 400` when Teams sends a message | The tunnel or reverse proxy is forwarding HTTPS to Renco' plain HTTP listener. Terminate TLS at the proxy and forward HTTP to port `3978` |
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.renco/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create renco-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs renco` / `renco gateway status -l` for tracebacks |
...
- Store credentials in `~/.renco/.env` with permissions `600` (`chmod 600 ~/.renco/.env`)
