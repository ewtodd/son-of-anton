...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/mcp/mcp-oauth-remote-gateway` |
...
| Related skills | [`renco-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-renco-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/docs/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
# MCP OAuth on a Remote Renco Gateway

## Overview

Renco' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Renco process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Renco runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Renco.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Renco' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Renco is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Renco** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Renco process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the renco user, an
empty or missing tokens directory (`$RENCO_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Renco detects a remote session it prints two
...
Both require an interactive terminal to the Renco host. The rest of this skill
is for when there is NO interactive TTY — Renco running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Renco Dashboard (try this BEFORE manual token surgery)

A remote Renco gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `renco dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'renco dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "RENCO_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`RENCO_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$RENCO_HOME/.env`. Those
...
Renco' `RencoTokenStorage` would have written, so on `/reload-mcp` Renco finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "RENCO|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if Renco' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find RENCO_HOME and the config path

```bash
RENCO_HOME=$(python3 -c 'from renco_constants import get_renco_home; print(get_renco_home())')
echo "config: $RENCO_HOME/config.yaml"
echo "tokens: $RENCO_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Renco Agent (manual OAuth)",
...
### 8. Write tokens in Renco' exact schema

`tools/mcp_oauth.py::RencoTokenStorage` expects two files under
`$RENCO_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "renco-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though Renco (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Renco sees `auth: oauth`, calls `RencoTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Renco process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Renco needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Renco does natively).
...
- **Don't claim Renco doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Renco. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Renco' config.
