"""Tests for the top-level `./son-of-anton` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./son-of-anton` should use `son_of_anton_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "son-of-anton"
    called = []

    fake_main_module = types.ModuleType("son_of_anton_cli.main")

    def fake_main():
        called.append("son_of_anton_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "son_of_anton_cli.main", fake_main_module)
    ...
    assert called == ["son_of_anton_cli.main"]
