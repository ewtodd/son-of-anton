...
from renco_cli import kanban as kc
...
def review_worker(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> str:
    ...
    monkeypatch.setenv("RENCO_PROFILE", "builder")
    ...
    monkeypatch.setenv("RENCO_KANBAN_TASK", task_id)
    monkeypatch.setenv("RENCO_KANBAN_RUN_ID", str(task.current_run_id))
    return task_id
...
def test_review_tools_redact_handoff_and_route_changes(
    ...
    monkeypatch.setenv("RENCO_PROFILE", "reviewer")
    monkeypatch.setenv("RENCO_KANBAN_RUN_ID", str(review.current_run_id))
...
def test_review_tools_are_gated_and_visible_to_kanban_workers(
    ...
    definitions = registry.get_definitions(
        set(resolve_toolset("renco-cli")), quiet=True
    )
    names = {
    ...
    from agent.transports.renco_tools_mcp_server import EXPOSED_TOOLS
...
def test_review_cli_round_trip_preserves_handoff(
    ...
    monkeypatch.setenv("RENCO_KANBAN_TASK", task_id)
    monkeypatch.setenv("RENCO_KANBAN_RUN_ID", str(implementation.current_run_id))
...
def test_goal_mode_review_handoff_cannot_bypass_judge(
    ...
    monkeypatch.setenv("RENCO_KANBAN_TASK", tool_task)
    monkeypatch.setenv("RENCO_KANBAN_RUN_ID", str(claimed.current_run_id))
    ...
    monkeypatch.setenv("RENCO_KANBAN_TASK", cli_task)
    monkeypatch.setenv("RENCO_KANBAN_RUN_ID", str(cli_claimed.current_run_id))
