"""Centralized logging setup for Son of Anton Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.son-of-anton/logs/`` (profile-aware via ``get_son_of_anton_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``son_of_anton_cli.web_server``,
    ``son_of_anton_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_son_of_anton_session_injector", False):
    ...
    _session_record_factory._son_of_anton_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``son-of-anton logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``son-of-anton logs --component gateway``.
    "gateway": ("gateway", "son_of_anton_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("son_of_anton_cli", "cli"),
    "cron": ("cron",),
    "gui": (
...
def setup_logging(
    *,
    son_of_anton_home: Optional[Path] = None,
    ...
    """Configure the Son of Anton logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    son_of_anton_home
        Override for the Son of Anton home directory.  Falls back to
        ``get_son_of_anton_home()`` (profile-aware).
    log_level
    ...
    home = son_of_anton_home or get_son_of_anton_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_son_of_anton_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._son_of_anton_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the son-of-anton group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from son_of_anton_cli.config import is_managed
...
# the module header): when several Son of Anton processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._son_of_anton_queue = True  # type: ignore[attr-defined]
