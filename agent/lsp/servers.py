...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<SON_OF_ANTON_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<SON_OF_ANTON_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    session_path = os.path.join(
        son_of_anton_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(son_of_anton_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName Son of Anton -HostProfileId son-of-anton -HostVersion 1.0.0 "
...
def son_of_anton_lsp_session_dir() -> str:
