"""
Interactive setup wizard for Renco Agent.
...
_DOCS_BASE = "https://renco-agent.nousresearch.com/docs"
...
# display_renco_home imported lazily at call sites (stale-module safety during renco update)
...
from renco_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from renco_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Renco Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Renco using environment variables or config commands:")
    print_info("  renco config set model.provider custom")
    print_info("  renco config set model.base_url http://localhost:8080/v1")
    print_info("  renco config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'renco setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``RENCO_NONINTERACTIVE=1`` (see ``renco_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("RENCO_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``RENCO_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from renco_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'renco setup')")


def _print_setup_summary(config: dict, renco_home):
    ...
    try:
        from renco_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Renco cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    renco model            (pick any provider/model)")
        print_info("    renco setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'renco setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from renco_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `renco tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from renco_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'renco setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'renco setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'renco tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'renco setup terminal'"))
    ...
    # Spotify (OAuth via renco auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'renco setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {renco_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('renco setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('renco setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('renco setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('renco setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('renco setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('renco config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('renco config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('renco config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('renco', Colors.GREEN)}              Start chatting")
    print(f"   {color('renco gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('renco doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `renco tools` and `renco setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``renco model``)
    ...
    provider added to ``renco model`` is automatically available here.
    ...
    # Delegate to the shared renco model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from renco_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: renco model")
    ...
    # on demand via `renco auth add`, `renco setup` vision, and
    # `renco setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``renco model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from renco_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.renco/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Renco auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via renco setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'renco setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Renco runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `renco setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `renco setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `renco egress setup` then `renco egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `renco egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'renco-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $RENCO_HOME/bin is never on PATH, so a bare
            # which() misses the uv Renco installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'renco-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into RENCO_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("RENCO_MAX_ITERATIONS")
    ...
    print_info("  Run `renco setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # RENCO_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.renco/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from renco_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_renco_home(Path(get_renco_home()))
    except Exception:
        pass
...
def _profile_name_from_renco_home(renco_home) -> str | None:
    """Return the active profile name when RENCO_HOME is a profile dir."""
    if renco_home.parent.name == "profiles":
        return renco_home.name
    return None
...
def _setup_bluebubbles():
    ...
    print_info("Connects Renco to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from renco_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://renco-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://renco-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from renco_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Renco from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'renco setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     renco config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `renco import` or `renco setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `renco setup tools` and `renco tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Renco profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``renco_cli.auth`` — lists every supported
    ...
    """
    try:
        from renco_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from renco_cli.config
    ...
    if section_key == "model":
        ...
        from renco_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_renco migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_renco", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Renco.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Renco to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Renco at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Renco at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Renco at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Renco at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Renco equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Renco config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Renco.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Renco's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(renco_home: Path) -> bool:
    ...
    print_info("Renco can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: renco claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=renco_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: renco claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Renco configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Renco config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Renco (use renco claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``renco setup --portal`` and ``renco portal``. This is the
    ...
    Renco session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``renco setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``renco model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``renco portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Renco Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `renco portal` == quick setup's Nous step.
    try:
        from renco_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `renco portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `renco portal`: %s", exc)
        print()
    ...
    print_info("  Run `renco portal info` to inspect routing.")
    print_info("  Run `renco` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      renco setup           — full or quick (auto-detected)
      renco setup model     — just model/provider
      renco setup tts       — just text-to-speech
      renco setup terminal  — just terminal backend
      renco setup gateway   — just messaging platforms
      renco setup tools     — just tool configuration
      renco setup telemetry — just local shared metrics
      renco setup agent     — just agent settings
    """
    from renco_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Renco Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Renco Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Renco Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, renco_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Renco configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'renco setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(renco_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Renco?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, renco_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, renco_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {renco_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'renco config edit'")
    ...
    # Tune later with `renco setup agent`.
    if not is_existing:
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from renco_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    _print_setup_summary(config, renco_home)


def _run_first_time_quick_setup(config: dict, renco_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``renco setup <section>``
    or switch providers with ``renco model``.
    """
    from renco_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'renco setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `renco import` from another machine).
        from renco_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    renco setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  renco setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``renco tools`` (which
    ...
    try:
        ...
        from renco_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``renco setup agent`` /
    ``renco config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, renco_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `renco update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        renco tools")
        print_info("  Seed skills:         renco skills opt-in --sync")
        print_info("  Add MCP servers:     renco mcp add")
        print_info("  Enable plugins:      renco plugins")
        print_info("  Tune agent settings: renco setup agent")
        print()
        _print_setup_summary(config, renco_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, renco_home)


def _blank_slate_walkthrough(config: dict, renco_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`renco update` runs from re-injecting them. Opt back in any")
            print_info("time with `renco skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `renco tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `renco plugins list` / `renco plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `renco plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `renco mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `renco mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   renco tools")
    print_info("  Seed skills:         renco skills opt-in --sync")
    print_info("  Add MCP servers:     renco mcp add")
    print_info("  Tune agent settings: renco setup agent")
    print()
...
def _run_quick_setup(config: dict, renco_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'renco setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Renco to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'renco setup gateway'.")
