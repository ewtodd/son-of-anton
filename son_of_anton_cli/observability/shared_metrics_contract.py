"""Bounded product contract for the first Son of Anton shared-metrics slice."""
...
SCHEMA_KEY = "son-of-anton.metrics.schema_version"
SCHEMA_VERSION = "son-of-anton.metrics.event.v2"
MODEL_CALL_SCOPE = "son-of-anton.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "son-of-anton.task_run"
TOOL_CALL_SCOPE = "son-of-anton.tool_call"
CLIENT_ACTIVE_MARK = "son-of-anton.client.active"
TOOL_APPROVAL_MARK = "son-of-anton.tool_approval"
SKILL_LIFECYCLE_MARK = "son-of-anton.skill.lifecycle"
SKILL_LOAD_MARK = "son-of-anton.skill.load"
SUBSCRIBER_NAME = "son-of-anton.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "son-of-anton.client.active"
LEGACY_MODEL_CALL_METRIC = "son-of-anton.model_call.count"
MODEL_ROUTE_METRIC = "son-of-anton.model_route.count"
TASK_STARTED_METRIC = "son-of-anton.task_run.started"
TASK_FINISHED_METRIC = "son-of-anton.task_run.finished"
TOOL_CALL_METRIC = "son-of-anton.tool_call.count"
TOOL_APPROVAL_METRIC = "son-of-anton.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "son-of-anton.skill.lifecycle.count"
SKILL_LOAD_METRIC = "son-of-anton.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Son of Anton installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(son_of_anton_version or "").strip()
    ...
    return {
        ...
        "son_of_anton_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("son_of_anton_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Son of Anton logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    relay_metadata = set(metadata) - {
        RUNTIME_INSTANCE_KEY,
        RUNTIME_SCHEMA_KEY,
        "son-of-anton.call_role",
    }
    if relay_metadata - {"otel.status_code"} or metadata.get(
        "otel.status_code", "OK"
    ) not in {"OK", "ERROR"}:
        return None
    call_role = metadata.get("son-of-anton.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from son_of_anton_cli.platforms import get_all_platforms

        if value in get_all_platforms():
            return "gateway"
    except Exception:
        pass
    if value in {"discord", "slack", "signal"}:
        return "gateway"
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Son of Anton terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Son of Anton registry toolset metadata to a low-cardinality category."""
    ...
    if toolset == "x_search":
        return "web"
    if toolset.startswith(("discord", "slack", "signal")):
        return "communication"
    return "other"


def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Son of Anton tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Son of Anton."""
