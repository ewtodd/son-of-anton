...
Converted from the former ``.github/actions/renco-smoke-test`` composite
...
def test_renco_help(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"renco --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"renco --help produced a traceback: {r.stderr[-2000:]!r}"
    )
...
def test_dashboard_subcommand_present(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"renco dashboard --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
...
def test_renco_help_under_wrapped_init(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"renco --help failed under `docker run --init` (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"renco --help produced a traceback under --init: {r.stderr[-2000:]!r}"
    )
