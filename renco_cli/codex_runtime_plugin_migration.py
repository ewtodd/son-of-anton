"""Migrate Renco' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Renco' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Renco mcp_servers.<n>.command/args/env  → codex stdio transport
  Renco mcp_servers.<n>.url/headers       → codex streamable_http transport
  Renco mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Renco mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Renco-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Renco' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Renco config.")
...
# Renco keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Renco' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, renco_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Renco MCP server config to the codex inline-table dict
    ...
    if not isinstance(renco_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(renco_cfg.get("command"))
    has_url = bool(renco_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (RENCO_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Renco)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Renco' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Renco' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Renco
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="renco-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a RENCO_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed renco-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.renco`` symlink that happens to live under ``/private/var/folders``
...
def _build_renco_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Renco' own
    ...
    so a renco installed under /opt/, /usr/local/, or a venv all work.
    RENCO_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # RENCO_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_renco_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever RENCO_HOME its launcher sets
    ...
    # — that would override the launcher's RENCO_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("RENCO_HOME", tmp_path) would
    ...
    renco_home = os.environ.get("RENCO_HOME") or ""
    if renco_home and _looks_like_test_tempdir(renco_home):
        renco_home = ""
    if renco_home:
        env["RENCO_HOME"] = renco_home
    # PYTHONPATH passes through so a worktree-launched renco finds the
    ...
    env["RENCO_QUIET"] = "1"
    env["RENCO_REDACT_SECRETS"] = env.get("RENCO_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_renco_tools: bool = True,
) -> MigrationReport:
    """Translate Renco mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        renco_config: full ~/.renco/config.yaml dict
        ...
        expose_renco_tools: when True (default), register Renco' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Renco for tools
    ...
    renco_servers = (renco_config or {}).get("mcp_servers") or {}
    if not isinstance(renco_servers, dict):
        report.errors.append(
            "mcp_servers in Renco config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in renco_servers.items():
    ...
    # Inject Renco' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Renco for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/renco_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_renco_tools:
        translated["renco-tools"] = _build_renco_tools_mcp_entry()
        if "renco-tools" not in report.migrated:
            report.migrated.append("renco-tools")
