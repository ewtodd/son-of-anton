...
use the notepad, and the `son-of-anton cron notepad` CLI handler.
"""
...
def cron_env(tmp_path, monkeypatch, notepad):
    """Isolated cron environment with temp SON_OF_ANTON_HOME (mirrors test_cron_context_from)."""
...
class TestPromptInjection:
    def test_nonempty_notepad_rendered_into_prompt(self, cron_env, notepad):
        ...
        assert f"son-of-anton cron notepad {job['id']} set" in prompt
...
class TestNotepadCli:
    ...
    def test_cli_set_get_list_delete(self, notepad, capsys):
        from son_of_anton_cli.cron import cron_notepad
