# A2A — Agent-to-Agent protocol for Son of Anton
...
peer (another Son of Anton, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
son-of-anton gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Son of Anton serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.son-of-anton/a2a_audit.jsonl`.
- Conversations persist to `~/.son-of-anton/a2a_conversations/` — they survive context
