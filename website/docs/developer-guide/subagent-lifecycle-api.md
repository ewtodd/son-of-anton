...
Plugins can launch and supervise fresh Renco child sessions without importing
...
active agent turn fails closed with `No active Renco parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Renco can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Renco's existing unsafe-tool block remains enforced.
