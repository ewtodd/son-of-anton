"""MCP picker — interactive `son-of-anton mcp picker` (also the default `son-of-anton mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``son-of-anton mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `son-of-anton plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from son_of_anton_cli.cli_output import prompt_yes_no
from son_of_anton_cli.curses_ui import curses_single_select
from son_of_anton_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Son of Anton session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from son_of_anton_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `son-of-anton mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: son-of-anton mcp install <name>    Picker: son-of-anton mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Son of Anton is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Son of Anton — run `son-of-anton update` "
...
def show_catalog() -> None:
    """`son-of-anton mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`son-of-anton mcp picker` (and default `son-of-anton mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`son-of-anton mcp install <name>` — non-interactive entry-point.

    Returns 0 on success, non-zero on failure (so the CLI can propagate
    exit codes).
    """
    from son_of_anton_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        print(color(
            f"  ✗ '{identifier}' is not in the catalog. "
            "Run `son-of-anton mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
