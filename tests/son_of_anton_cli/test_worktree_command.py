...
def test_new_named_creates_and_retargets(repo):
    ...
    assert cli_mod._active_worktree["branch"] == "son-of-anton/fix-login"
...
@requires_git
def test_new_unnamed_uses_random_son_of_anton_prefix(repo):
    out = _run(_Stub(), "/worktree new")
    assert "Worktree ready" in out
    name = os.path.basename(cli_mod._active_worktree["path"])
    assert name.startswith("son-of-anton-")
