...
@pytest.fixture
def tmp_renco_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    home = tmp_path / "renco"
    home.mkdir()
...
def test_store_and_load_photon_token(tmp_renco_home: Path) -> None:
    photon_auth.store_photon_token("abc123def456")
    assert photon_auth.load_photon_token() == "abc123def456"

    auth_json = json.loads((tmp_renco_home / "auth.json").read_text())
...
def test_save_auth_never_world_readable(tmp_renco_home: Path) -> None:
    """auth.json must be created 0o600 — no window at process umask."""
    photon_auth.store_photon_token("secret-token")
    mode = (tmp_renco_home / "auth.json").stat().st_mode & 0o777
    assert mode == 0o600


def test_store_project_credentials_round_trip(
    tmp_renco_home: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
# cross-process lock renco_cli/auth.py's ~15 other writers all use, so a
...
def test_list_projects_unwraps_list(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_get(url: str, **kwargs: Any) -> _FakeResponse:
        return _FakeResponse(json_body=[{"id": "p1", "name": "Renco Agent"}])
...
def test_find_project_by_name_case_insensitive(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_get(url: str, **kwargs: Any) -> _FakeResponse:
        return _FakeResponse(json_body={"data": [
            {"id": "p1", "name": "Other"},
            {"id": "p2", "name": "renco agent"},
        ]})

    monkeypatch.setattr(photon_auth.httpx, "get", fake_get)
    proj = photon_auth.find_project_by_name("tok", "Renco Agent")
...
def test_create_project_omits_spectrum_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(photon_auth.httpx, "post", fake_post)
    data = photon_auth.create_project("tok", name="Renco Agent")
    ...
    assert captured["body"]["name"] == "Renco Agent"
