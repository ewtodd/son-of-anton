...
`son-of-anton --tui --resume latest --in ./dir` (and the classic-CLI equivalents)
...
def test_top_level_in_value_not_mistaken_for_subcommand(monkeypatch):
    # `son-of-anton --in chat` — "chat" is the flag's value, not the subcommand.
    import sys

    import son_of_anton_cli.main as mod

    monkeypatch.setattr(sys, "argv", ["son-of-anton", "--in", "chat", "--resume", "latest"])
