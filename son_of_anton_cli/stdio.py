...
on Japanese locales, etc.).  Son of Anton's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``son_of_anton_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Son of Anton is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SON_OF_ANTON_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SON_OF_ANTON_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Son of Anton-managed Git install directories so
    ...
    # already-running shells don't see the change — which means son-of-anton
    ...
    # install without Son of Anton-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Son of Anton's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Son of Anton's
    ``son-of-anton config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Son of Anton (User env var in Windows Settings, or
    export in a PowerShell profile) and Son of Anton picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Son of Anton-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\son-of-anton\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``son-of-anton`` from right after install) retain their old PATH.

    Any subprocess Son of Anton spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Son of Anton-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "son-of-anton", "git", "cmd"),
        os.path.join(local_appdata, "son-of-anton", "git", "bin"),
        os.path.join(local_appdata, "son-of-anton", "git", "usr", "bin"),
        # Son of Anton venv Scripts directory — host of the son-of-anton.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes son-of-anton, but harmless to include.
        os.path.join(local_appdata, "son-of-anton", "son-of-anton", "venv", "Scripts"),
