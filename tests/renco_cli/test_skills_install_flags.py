"""
Tests for --yes / --force flag separation in `renco skills install`.
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    monkeypatch.setattr("renco_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["renco", "skills", "install", "official/email/agentmail", "--yes"],
    )
...
def test_cli_skills_install_no_flags(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["renco", "skills", "install", "test/skill"],
    )
