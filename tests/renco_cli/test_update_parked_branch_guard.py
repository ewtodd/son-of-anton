"""Regression tests for the parked-branch guard in ``renco update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``renco
...
def _no_config(monkeypatch):
    ...
    import renco_cli.config as renco_config

    monkeypatch.setattr(renco_config, "load_config", lambda: {})
...
def test_skip_warning_names_branch_behind_count_and_commands(repo_pair, capsys):
    ...
    assert f"git -C {repo_pair} checkout main && renco update" in out
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(renco_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    """
    monkeypatch.setattr(renco_main, "PROJECT_ROOT", repo)
    monkeypatch.setattr(renco_main, "_resolve_update_branch", lambda args: "main")
    ...
    monkeypatch.setattr(renco_main, "_discard_lockfile_churn", lambda *a, **k: None)
    ...
    monkeypatch.setattr(renco_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    monkeypatch.setattr(renco_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(renco_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        renco_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(renco_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(renco_main, "_capture_active_tool_dependencies", lambda: [])
