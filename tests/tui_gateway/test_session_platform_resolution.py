...
The desktop app's chat panel uses ``son-of-anton serve`` (the ``tui_gateway``
...
``tui_gateway`` session-creation time:
  ...
  SON_OF_ANTON_DESKTOP=1, SON_OF_ANTON_DESKTOP_TERMINAL unset  -> platform="desktop"
  SON_OF_ANTON_DESKTOP=1, SON_OF_ANTON_DESKTOP_TERMINAL=1     -> platform="tui"  (embedded pane)
...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("SON_OF_ANTON_DESKTOP_TERMINAL", raising=False)
    return monkeypatch
...
class TestResolveSessionPlatform:
    ...
    def test_desktop_chat_backend_gets_desktop_tag(self, clean_env):
        clean_env.setenv("SON_OF_ANTON_DESKTOP", "1")
        _srv = _reload_resolver()
    ...
    def test_truthy_variants_recognized(self, clean_env, val):
        clean_env.setenv("SON_OF_ANTON_DESKTOP", val)
        _srv = _reload_resolver()
        assert _srv._resolve_session_platform() == "desktop"
    ...
    def test_embedded_terminal_overrides_desktop_when_both_set(self, clean_env):
        ...
        clean_env.setenv("SON_OF_ANTON_DESKTOP", "1")
        clean_env.setenv("SON_OF_ANTON_DESKTOP_TERMINAL", "true")
        _srv = _reload_resolver()
        assert _srv._resolve_session_platform() == "tui"
