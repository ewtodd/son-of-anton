...
_PRIVATE_BROWSER_SENTINEL = "_RENCO_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# renco: pin this named session to its own tab (once per daemon process)
def _renco_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "renco-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_renco_ensure_own_tab()
del _renco_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Renco's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Renco's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_renco_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `renco tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Renco' own bin dir ($RENCO_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from renco_constants import get_renco_home

        return str(Path(get_renco_home()) / "bin")
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Renco' managed uv (bootstrapped on demand via
    ``renco_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$RENCO_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Renco-managed copy, or resolution
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_renco_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `renco tools` → Browser Automation."
        )
