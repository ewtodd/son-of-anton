...
- files inside the Renco home tree are repaired with ``chmod u+rw``
...
import renco_state
from renco_state import SessionDB, preflight_db_writability
...
def renco_home(tmp_path, monkeypatch):
    """Isolated RENCO_HOME so the repair scope covers tmp DBs."""
    home = tmp_path / ".renco"
    home.mkdir()
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, renco_home):
        db = renco_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, renco_home):
        db = renco_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, renco_home):
        sub = renco_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, renco_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, renco_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, renco_home):
        db = renco_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, renco_home):
        db_path = renco_home / "state.db"
    ...
    def test_sessiondb_actionable_error_outside_home(
        ...
        renco_state._set_last_init_error(None)
        try:
            with pytest.raises(sqlite3.OperationalError) as exc_info:
                SessionDB(db_path)
            msg = str(exc_info.value)
