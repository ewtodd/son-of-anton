...
warn / fail). Renco already scans hub-installed *skills* via
``tools/skills_guard.py``; this module extends the same static-analysis
engine to ``renco plugins install`` and ``renco plugins update``, which
...
- Exempts the "reads own env secret" / "HTTP call with key" pattern
  ...
  foreign credential-store access (~/.ssh, ~/.aws, ~/.renco/.env),
...
#
# ``renco_env_access`` (a reference to ``~/.renco/.env``) is the DOCUMENTED
...
def scan_plugin(plugin_dir: Path, source: str = "") -> ScanResult:
    ...
    Args:
        plugin_dir: Path to the plugin directory (typically the temp clone,
            before it is moved into ``~/.renco/plugins/``).
