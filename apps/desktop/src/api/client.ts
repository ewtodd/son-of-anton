import { JsonRpcGatewayClient } from '@renco/shared'

import type { RencoApiRequest } from '@/global'
...
// "Timed out connecting to Renco backend" that hangs the UI (#48504).
//
...
export class RencoGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Renco gateway connection closed',
      connectErrorMessage: 'Could not connect to Renco gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Renco gateway is not connected',
...
// process to route them to that host (renco:api's registry branch). Null
...
export function rencoApi<T>(request: RencoApiRequest): Promise<T> {
  return window.rencoDesktop.api<T>({ ...connectionScoped(), ...request })
}
