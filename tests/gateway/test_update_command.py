...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _renco_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._renco_home", tmp_path), \
        ...
        with patch("gateway.run._renco_home", tmp_path):
            ...
            # project_root will be the real renco-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_resolve_renco_bin_fallback(self):
        """_resolve_renco_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_renco_bin

        fake_spec = MagicMock()
        with patch("shutil.which", return_value=None), \
             patch("importlib.util.find_spec", return_value=fake_spec):
            result = _resolve_renco_bin()

        assert result == [sys.executable, "-m", "renco_cli.main"]
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._renco_home", renco_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/renco" if x == "renco" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = renco_home / ".update_pending.json"
        ...
        assert not (renco_home / ".update_exit_code").exists()
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        assert "Starting Renco update" in result
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("RENCO_MANAGED", "")
        ...
        # Later steps may legitimately return success ("Starting Renco
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (renco_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = renco_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (renco_home / ".update_output.txt").write_text("done")
        (renco_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (renco_home / ".update_pending.json").write_text(json.dumps(pending))
        (renco_home / ".update_output.txt").write_text(
    ...
    async def test_cleans_up_on_error(self, tmp_path):
        ...
        pending_path = renco_home / ".update_pending.json"
        output_path = renco_home / ".update_output.txt"
        exit_code_path = renco_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (renco_home / ".update_pending.claimed.json").exists()
    ...
    async def test_completion_notification_tolerates_invalid_utf8_output(self, tmp_path):
        ...
        assert "Renco update finished" in sent_text
        assert not pending_path.exists()
        assert not output_path.exists()
        assert not exit_code_path.exists()
...
class TestWatchUpdateProgress:
    ...
    async def test_invalid_utf8_update_output_does_not_crash_watcher(self, tmp_path):
        ...
        (renco_home / ".update_pending.json").write_text(json.dumps({
            "platform": "telegram",
            "chat_id": "67890",
            "user_id": "12345",
        }))
        (renco_home / ".update_output.txt").write_bytes(
        ...
        assert "Renco update finished" in sent
        assert not (renco_home / ".update_pending.json").exists()
