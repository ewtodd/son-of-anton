"""``son-of-anton skin`` — list, switch, and tweak skins from the CLI.
...
def _skin_set(key: str, value: str, skin: str | None) -> int:
    ...
    print(f"✓ {key} = {value} in {display_son_of_anton_home()}/skins/{target}.yaml (live within ~1s)")
    return 0
...
def skin_command(args) -> None:
    """Dispatch ``son-of-anton skin <verb>``."""
