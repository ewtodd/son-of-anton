...
profile's SON_OF_ANTON_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.son-of-anton/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active SON_OF_ANTON_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever SON_OF_ANTON_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    SON_OF_ANTON_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import son_of_anton_constants

    monkeypatch.setattr(
        son_of_anton_constants, "_get_platform_default_son_of_anton_home", lambda: root
    )
    monkeypatch.setenv("SON_OF_ANTON_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile SON_OF_ANTON_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "son_of_anton_home"
    ...
    assert son_of_anton_constants.get_son_of_anton_home().resolve() == profile_home.resolve()
    assert son_of_anton_constants.get_default_son_of_anton_root().resolve() == root.resolve()

    # cron/jobs.py computes SON_OF_ANTON_DIR from get_son_of_anton_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.SON_OF_ANTON_DIR.resolve() == profile_home.resolve()
        assert (
