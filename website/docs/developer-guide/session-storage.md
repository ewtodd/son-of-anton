...
Renco Agent uses a SQLite database (`~/.renco/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `renco_state.py`


## Architecture Overview

```
~/.renco/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `renco_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `renco_state.py` for the exact SQL.
...
Multiple renco processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.renco/state.db
...
Default path: `~/.renco/state.db`

This is derived from `renco_constants.get_renco_home()` which resolves to
`~/.renco/` by default, or the value of `RENCO_HOME` environment variable.
