"""Shell completion script generation for renco CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_renco_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Renco Agent bash completion
# Add to ~/.bashrc:
#   eval "$(renco completion bash)"

_renco_profiles() {{
    local profiles_dir="$HOME/.renco/profiles"
...
_renco_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_renco_profiles)" -- "$cur"))
        return
    fi
...
complete -F _renco_completion renco
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef renco
# Renco Agent zsh completion
# Add to ~/.zshrc:
...
_renco_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.renco/profiles" ]]; then
        profiles+=($HOME/.renco/profiles/*(N/:t))
    fi
...
_renco() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_renco_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'renco command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Renco Agent fish completion",
        "# Add to your config:",
        "#   renco completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __renco_profiles",
        "    echo default",
        "    if test -d $HOME/.renco/profiles",
        "        for d in $HOME/.renco/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c renco -f -s p -l profile"
        " -d 'Profile name' -xa '(__renco_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__renco_profiles)' -d 'Profile name'"
                )
