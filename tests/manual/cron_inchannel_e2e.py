...
Uses a throwaway SON_OF_ANTON_HOME so it never touches ~/.son-of-anton. No real names.
"""
...
def _fresh_home():
    """Point SON_OF_ANTON_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_son_of_anton_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["SON_OF_ANTON_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER SON_OF_ANTON_HOME is set.
...
# a different get_son_of_anton_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SON_OF_ANTON_HOME (throwaway): {HOME}")
