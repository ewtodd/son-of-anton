...
/** Why a known Renco command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Renco profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Renco Agent version', surface: exec() },
...
function isKnownRencoSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Renco' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownRencoSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Renco and were never asked for. It is
