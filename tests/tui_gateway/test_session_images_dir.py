...
before ``prompt.submit`` installs the session's profile RENCO_HOME override, so
...
def test_profile_home_session_writes_under_profile(tmp_path):
    ...
    profile_home = tmp_path / ".renco" / "profiles" / "coder"
...
def test_launch_home_fallback_when_no_profile(tmp_path):
    ...
    launch_home = tmp_path / ".renco"
    session = {}

    with patch("tui_gateway.server._renco_home", launch_home):
...
def test_two_profiles_are_isolated(tmp_path):
    ...
    home_a = tmp_path / ".renco" / "profiles" / "a"
    home_b = tmp_path / ".renco" / "profiles" / "b"
