...
``~/.son-of-anton/skills/.curator_ledger.jsonl`` describing who changed what, with
before/after file manifests whose contents are stored content-addressed
(sha256-deduped) under ``~/.son-of-anton/.curator_backups/blobs/``.
...
Design decisions (Teknium-approved):
  ...
  - The ledger covers ALL actors, tagged ``curator`` / ``agent`` / ``user``.
    ...
    ``son-of-anton curator rollback <entry-id>``.
...
def ledger_path() -> Path:
    return get_son_of_anton_home() / "skills" / ".curator_ledger.jsonl"


def blobs_dir() -> Path:
    return get_son_of_anton_home() / ".curator_backups" / "blobs"
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """All paths in an entry must live under SON_OF_ANTON_HOME. Defense in depth —
