"""Cross-process update mutual exclusion (``son_of_anton_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``son-of-anton
...
dashboard-spawned ``son-of-anton update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".son-of-anton-update-in-progress"


def test_marker_path_follows_process_son_of_anton_home(tmp_path, monkeypatch):
    """The lock must land where the Rust updater and Electron gate look.

    All three resolve the *process* SON_OF_ANTON_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("SON_OF_ANTON_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".son-of-anton-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``son-of-anton update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Son of Anton is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``son-of-anton-setup`` under ``~/.son-of-anton`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("son_of_anton_cli.update_lock._pid_alive", lambda pid: True)
