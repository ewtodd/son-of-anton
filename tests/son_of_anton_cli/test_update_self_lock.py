...
ancestors by design — a CLI ``son-of-anton update`` IS the venv python.  Before
...
Windows ``son-of-anton update`` exited 2 before even fetching and the update
...
import son_of_anton_cli.update_cmd as update_cmd
...
def test_recovered_update_retry_builds_parser_without_native_secret_modules(
    ...
    monkeypatch.setattr(sys, "argv", ["son-of-anton", "update", "--yes"])
    sys.modules.pop("cryptography.hazmat.bindings._rust", None)
    ...
    def test_import_main_does_not_load_self_locking_modules(self):
        result = self._run(
            textwrap.dedent(
                ...
                from son_of_anton_cli.update_cmd import _SELF_LOCKING_NATIVE_MODULES
                loaded = [
    ...
    def test_update_dispatch_does_not_load_cryptography(self):
        result = self._run(
            textwrap.dedent(
                ...
                with patch("son_of_anton_cli.main._cmd_update_check", lambda *a, **k: 0):
                    try:
                        m.main()
                    except SystemExit:
                        pass
