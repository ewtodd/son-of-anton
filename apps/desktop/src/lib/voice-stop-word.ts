...
//
// When someone is in a hands-free "Hey Renco" voice chat, the natural way to
...
// phrase (optionally addressed to Renco), so a real turn that merely contains
...
// Optional address prefixes so "renco stop" / "ok stop" / "hey renco, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey renco', 'hey renco,', 'renco', 'renco,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("renco") is not a stop command on its own.
      continue
    }
 ...
 * to Renco). Returns false for anything that merely contains "stop" as part of
