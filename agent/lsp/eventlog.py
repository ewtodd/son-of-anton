...
Grep recipe::
    ...
    tail -f ~/.son-of-anton/logs/agent.log | rg 'lsp\\['
"""
...
event_log = logging.getLogger("son-of-anton.lint.lsp")
...
def log_server_unavailable(server_id: str, binary_or_pkg: str) -> None:
    ...
    if _announce_once(_announced_unavailable, key):
        _emit(
            ...
            "(install via `son-of-anton lsp install <id>` or set lsp.servers.<id>.command)",
        )
    else:
