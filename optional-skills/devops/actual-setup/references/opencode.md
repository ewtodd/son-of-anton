...
is typically only in `~/.renco/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Renco,
...
`renco-custom-providers` skill, pitfall 2). Renco hits this because it forwards
