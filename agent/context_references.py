...
_SENSITIVE_SON_OF_ANTON_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    son_of_anton_home = get_son_of_anton_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(son_of_anton_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(son_of_anton_home / rel for rel in _SENSITIVE_SON_OF_ANTON_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Son of Anton path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.son-of-anton/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Son of Anton path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
