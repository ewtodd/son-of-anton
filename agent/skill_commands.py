...
#
# When a user invokes a /skill (or /bundle), Son of Anton expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_son_of_anton_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``SON_OF_ANTON_PLATFORM`` env var and
    ``SON_OF_ANTON_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("SON_OF_ANTON_PLATFORM")
            or get_session_env("SON_OF_ANTON_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("SON_OF_ANTON_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Return the effective Son of Anton home the skill scan should be scoped to.

    A gateway session can switch between profiles that each carry their own
    ``skills.external_dirs`` (via ``set_son_of_anton_home_override``), but the
    ...
    return str(get_son_of_anton_home())
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.son-of-anton/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core Son of Anton slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Son of Anton command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's Son of Anton home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.son-of-anton/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``son-of-anton -s <skill>`` or
    a deployment's ``SON_OF_ANTON_TUI_SKILLS`` env var could force-load a skill an
