...
Covers:
  ...
  - Namespace spoofing (son-of-anton:, foreign, already-colon'd) is rejected
  ...
  - `son-of-anton plugins show` output includes emits/listens
"""
...
@pytest.mark.parametrize(
    "bad_event",
    [
        "son-of-anton:x",   # reserved core prefix
...
def test_emit_rejects_namespaced_names(bad_event):
    ...
    for name in (bad_event, f"b:{bad_event}", "son-of-anton:x", "a:x", "b:x"):
...
def test_subscribe_is_unrestricted():
    """Any plugin may subscribe to any event, including son-of-anton: and foreign."""
    manager = _fresh_manager()
    ctx_a = _make_ctx(manager, "plugin_a", key="a")
    got = []
    # None of these raise — only emit is namespace-gated.
    ctx_a.subscribe("son-of-anton:core_event", lambda **p: got.append("son-of-anton"))
    ctx_a.subscribe("b:ping", lambda **p: got.append("b"))
    assert "son-of-anton:core_event" in manager._subscriptions
    assert "b:ping" in manager._subscriptions
