...
import { disconnectOAuthProvider, listOAuthProviders } from '@/renco'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/renco'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.rencoDesktop?.terminal)
...
//      in renco_cli/provider_catalog.py) — the SAME provider identity
//      `renco model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Renco can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Renco can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Renco never deletes creds another tool owns behind a silent API call.
