...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Renco MCP catalog. The
...
Two halves, in this order: the editor side must be up before Renco connects.
...
### One-time, Renco side
    ...
    renco mcp install unreal-engine
...
`~/.renco/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Renco — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Renco connects
...
2. Start the Renco session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Renco
they appear as:
...
then happens at `renco mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Renco runs batched calls
...
- **Start order matters.** Editor + server up first, then the Renco
...
- **Timeouts on long operations.** Renco' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.renco/config.yaml` for
