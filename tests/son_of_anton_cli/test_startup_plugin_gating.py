...
``son_of_anton_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``son-of-anton --help``, ``son-of-anton --version``,
``son-of-anton logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``son-of-anton --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from son_of_anton_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["son-of-anton", "--help"]
    buf = io.StringIO()
    try:
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``son-of-anton photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain son-of-anton-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level son-of-anton <name> CLI command via PluginContext.
