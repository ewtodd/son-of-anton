"""Run a real Renco CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_renco_executable(renco_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "renco",
        Path(".venv") / "Scripts" / "renco.exe",
    ):
        candidate = renco_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("renco")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Renco executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Renco source checkout containing .venv/bin/renco",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated RENCO_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["renco.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['renco.client.active']}"
        )
    [model] = by_name["renco.model_route.count"]
    expected_model = {
        "name": "renco.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['renco.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["renco.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['renco.task_run.started']}"
        )
    [terminal] = by_name["renco.task_run.finished"]
    ...
    [tool] = by_name["renco.tool_call.count"]
    ...
    lifecycle = by_name["renco.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["renco.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Renco development environment requires jsonschema"
        ) from exc
    ...
    if metrics["renco.client.active"] != [
        {
            "name": "renco.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['renco.client.active']}"
        )
    [model] = metrics["renco.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['renco.model_route.count']}"
        )
    [terminal] = metrics["renco.task_run.finished"]
    ...
    [tool] = metrics["renco.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["renco.skill.lifecycle.count"]
    ...
    loads = metrics["renco.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    renco_repo = args.renco_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    renco = _resolve_renco_executable(renco_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="renco-relay-shared-metrics-"))
    home = root / "renco-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["RENCO_HOME"] = str(home)
        python_paths = [str(renco_repo)]
    ...
    (root / "renco.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "renco.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Renco exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Renco model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Renco did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Renco did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from renco_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "renco.shared_metrics.v2.schema.json",
    )

    print("Renco -> NeMo Relay shared-metrics smoke test passed")
