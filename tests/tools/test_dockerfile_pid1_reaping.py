...
- Signal forwarding runs through the init so ``docker stop`` triggers
  son-of-anton's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, son-of-anton accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, son-of-anton accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and son-of-anton would run without zombie reaping.
    """
    ...
    assert "/opt/son-of-anton/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/son-of-anton/docker/main-wrapper.sh" in dispatcher, (
