...
Used by ``renco kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``renco_cli/goals.py`` — same aux
...
RENCO_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("RENCO_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Renco Agent board.
...
def _profile_author() -> str:
    """Mirror of ``renco_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("RENCO_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=RENCO_KANBAN_SPECIFY_MAX_TOKENS,
