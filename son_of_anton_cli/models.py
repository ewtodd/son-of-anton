...
Add, remove, or reorder entries here — both `son-of-anton setup` and
`son-of-anton` provider-selection will pick up the change automatically.
"""
...
from son_of_anton_cli.urllib_security import open_credentialed_url, url_origin
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from son_of_anton_cli.config import get_custom_provider_tls_settings
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Son of Anton that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Son of Anton that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_son_of_anton_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$SON_OF_ANTON_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `son-of-anton model` TUI
...
# In-repo fallback for the model Son of Anton silently lands on when the user never
...
# copy, `son-of-anton -p`, a cron job, or a freshly booted gateway answers
...
def _reasoning_caps_disk_path() -> Path:
    from son_of_anton_constants import get_son_of_anton_home
    return get_son_of_anton_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _SON_OF_ANTON_USER_AGENT}
    try:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from son_of_anton_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return son-of-anton-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; son-of-anton's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    env_base = None
    try:
        from son_of_anton_cli.auth import _nous_inference_env_override
...
def _get_ollama_request_headers() -> dict[str, str]:
    ...
    try:
        from son_of_anton_cli.config import normalize_extra_headers
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _SON_OF_ANTON_USER_AGENT}
...
def _resolve_static_model_alias(
    ...
    try:
        from son_of_anton_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Son of Anton' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``son_of_anton_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Son of Anton should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``son-of-anton auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.son-of-anton/.env``.
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from son_of_anton_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from son_of_anton_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Son of Anton release.
...
#
# Cache strategy:
#   - One JSON file at $SON_OF_ANTON_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from son_of_anton_constants import get_son_of_anton_home
    return get_son_of_anton_home() / "provider_models_cache.json"
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SON_OF_ANTON_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Son of Anton' default Claude IDs elsewhere use
...
def opencode_zen_free_headers() -> dict:
    ...
    return {
        ...
        "User-Agent": f"SonOfAntonAgent/{_v}",
    }
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _SON_OF_ANTON_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"son-of-anton/{_SON_OF_ANTON_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=son-of-anton"
    ...
    *,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Son of Anton will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Son of Anton will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Son of Anton cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Son of Anton actually ships.  (#46850)
            #
