...
Also works when ``son-of-anton`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from son_of_anton_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched son-of-anton."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_son_of_anton_bin() -> Optional[str]:
    """Find the son-of-anton entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("son-of-anton")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m son_of_anton_cli.main``.
    ...
    ``son-of-anton.exe`` on PATH or the ``sys.executable -m son_of_anton_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    path_bin = shutil.which("son-of-anton")
    if path_bin:
        return path_bin
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with son-of-anton.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_son_of_anton_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "son_of_anton_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh son-of-anton invocation.
    ...
    relaunch contract wants: "run son-of-anton again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``son-of-anton.exe`` (a Python console-script shim that wraps
    ``python -m son_of_anton_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "son-of-anton exited, then
    new son-of-anton started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``son-of-anton`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nSon of Anton relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run son-of-anton.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
