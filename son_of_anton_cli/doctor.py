"""
Doctor command for son-of-anton CLI.

Diagnoses issues with Son of Anton Agent setup.
"""
...
from son_of_anton_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
SON_OF_ANTON_HOME = get_son_of_anton_home()
_DHH = display_son_of_anton_home()  # user-facing display path (e.g. ~/.son-of-anton or ~/.son-of-anton/profiles/coder)

# Load environment variables from ~/.son-of-anton/.env so API key checks work
_env_path = get_env_path()
load_son_of_anton_dotenv(son_of_anton_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from son_of_anton_cli.colors import Colors, color
from son_of_anton_cli.models import _SON_OF_ANTON_USER_AGENT
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Son of Anton containers"
        ...
        action = "run `son-of-anton update`"
    return (
...
def _son_of_anton_database_paths(son_of_anton_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Son of Anton-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from son_of_anton_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, son_of_anton_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((son_of_anton_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(son_of_anton_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``son_of_anton_cli.sqlite_safe_read``).
    ...
    """
    from son_of_anton_cli.sqlite_safe_read import (
        has_live_connection,
...
def _report_database_journal_modes(
    ...
    from son_of_anton_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = son_of_anton_home if son_of_anton_home is not None else SON_OF_ANTON_HOME
    try:
        databases = _son_of_anton_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Son of Anton databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.son-of-anton/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("SON_OF_ANTON_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from son_of_anton_cli.auth import get_minimax_oauth_auth_status
...
from son_of_anton_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'son-of-anton sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. SON_OF_ANTON_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("SON_OF_ANTON_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("SON_OF_ANTON_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Son of Anton Relay plugin."""
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches son_of_anton_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``son-of-anton --version`` reporting a stale version while
    ...
    """
    try:
        from son_of_anton_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != son_of_anton_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'son-of-anton update', or set "
            "son_of_anton_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `son-of-anton doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from son_of_anton_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the SON_OF_ANTON_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("SON_OF_ANTON_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via SON_OF_ANTON_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `son-of-anton`.
    os.environ.setdefault("SON_OF_ANTON_INTERACTIVE", "1")

    # Handle `son-of-anton doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.son-of-anton/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Son of Anton Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`son-of-anton doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        from son_of_anton_cli.config import load_config
        from son_of_anton_cli.mcp_security import validate_mcp_server_entry
    ...
    try:
        import sqlite3
        from son_of_anton_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Son of Anton already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and son_of_anton_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.son-of-anton/.env (primary location for user config)
    env_path = SON_OF_ANTON_HOME / '.env'
    if env_path.exists():
        ...
        # matches son_of_anton_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'son-of-anton setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'son-of-anton setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'son-of-anton setup' to create one")
                issues.append("Run 'son-of-anton setup' to create .env")
    
    # Check ~/.son-of-anton/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from son_of_anton_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from son_of_anton_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'son-of-anton config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.son-of-anton/.env or run 'son-of-anton setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'son-of-anton setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'son-of-anton config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from son_of_anton_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'son-of-anton setup' to migrate config")
                else:
                    issues.append("Run 'son-of-anton doctor --fix' or 'son-of-anton setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'son-of-anton doctor --fix'")
        except Exception:
            pass

        # Detect stale SON_OF_ANTON_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives SON_OF_ANTON_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from son_of_anton_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("SON_OF_ANTON_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"SON_OF_ANTON_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `son-of-anton setup` run)",
                )
                if should_fix:
                    if remove_env_value("SON_OF_ANTON_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale SON_OF_ANTON_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove SON_OF_ANTON_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the SON_OF_ANTON_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale SON_OF_ANTON_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'son-of-anton doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from son_of_anton_cli.config import load_env as _load_env_depr
            from son_of_anton_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        from son_of_anton_cli.config import load_config
        from son_of_anton_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Son of Anton' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = son_of_anton_home / subdir_name
    ...
    soul_path = son_of_anton_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Son of Anton a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Son of Anton Agent Persona\n\n"
                "<!-- Edit this file to customize how Son of Anton communicates. -->\n\n"
                "You are Son of Anton, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = son_of_anton_home / "memories"
    ...
    state_db_path = son_of_anton_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from son_of_anton_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'son-of-anton doctor --fix' "
                        "(or 'son-of-anton sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from son_of_anton_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'son-of-anton doctor --fix' "
                        "(or 'son-of-anton sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Son of Anton
        ...
        try:
            from son_of_anton_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'son-of-anton sessions optimize-storage' "
    ...
    wal_path = son_of_anton_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'son-of-anton doctor --fix' to checkpoint")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'son-of-anton[vercel]')",
                "Install the Vercel optional dependency: pip install 'son-of-anton[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/Son of Anton-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `son-of-anton update` does, so a session's first browser call
            ...
            # `son-of-anton update` wiped node_modules (issue #48521).
            check_warn(
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Son of Anton one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'son-of-anton config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _SON_OF_ANTON_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``son-of-anton doctor``.
    ...
    # never the legitimate source for `son-of-anton doctor`.
    ...
    try:
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'son-of-anton setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = SON_OF_ANTON_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: son-of-anton skills list)")
    ...
    try:
        from son_of_anton_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = SON_OF_ANTON_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: son-of-anton memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: son-of-anton memory setup",
                    "No Honcho API key — run 'son-of-anton memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run son-of-anton memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: son-of-anton memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: son-of-anton memory setup")
        except Exception as _e:
    ...
    try:
        from son_of_anton_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'son-of-anton doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
