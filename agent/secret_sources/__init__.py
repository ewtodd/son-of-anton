...
credentials at process startup, _after_ ~/.son-of-anton/.env has loaded.
...
Currently bundled:
  ...
  - ``bitwarden`` — Bitwarden Secrets Manager (`bws` CLI).  See
    ...
    ``son_of_anton_cli.secrets_cli`` for the user-facing setup wizard.
  - ``onepassword`` — 1Password ``op://`` secret references (`op` CLI).
    See ``agent.secret_sources.onepassword`` for the integration and
    ``son_of_anton_cli.onepassword_secrets_cli`` for the user-facing commands.
