...
def _reply_anchor_for_event(event) -> str | None:
    ...
    if (
        ...
        and raw_message.get("_son_of_anton_no_thread_response")
    ):
...
from son_of_anton_constants import get_default_son_of_anton_root, get_son_of_anton_dir, get_son_of_anton_home
...
IMAGE_CACHE_DIR = get_son_of_anton_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_son_of_anton_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_son_of_anton_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from son_of_anton_cli.config import load_config_readonly as _load_config
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; SonOfAntonAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_son_of_anton_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_son_of_anton_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_son_of_anton_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_son_of_anton_dir("cache/screenshots", "browser_screenshots")
...
_SON_OF_ANTON_ROOT = get_default_son_of_anton_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "SON_OF_ANTON_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "SON_OF_ANTON_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "SON_OF_ANTON_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "SON_OF_ANTON_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _SON_OF_ANTON_HOME / "image_cache",
    _SON_OF_ANTON_HOME / "audio_cache",
    _SON_OF_ANTON_HOME / "video_cache",
    _SON_OF_ANTON_HOME / "document_cache",
    _SON_OF_ANTON_HOME / "browser_screenshots",
    ...
    _SON_OF_ANTON_HOME / "cache" / "images",
    _SON_OF_ANTON_HOME / "cache" / "audio",
    _SON_OF_ANTON_HOME / "cache" / "videos",
    _SON_OF_ANTON_HOME / "cache" / "documents",
    _SON_OF_ANTON_HOME / "cache" / "screenshots",
)
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("SON_OF_ANTON_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _SON_OF_ANTON_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Son of Anton profile and shared Son of Anton root both contain control
    ...
    # SON_OF_ANTON_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.son-of-anton
    ...
    for son_of_anton_root in (_SON_OF_ANTON_HOME, _SON_OF_ANTON_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(son_of_anton_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Son of Anton
    secrets (``~/.son-of-anton/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Son of Anton cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.son-of-anton/...``
...
def _translate_docker_container_media_path(candidate: Path) -> Optional[Path]:
    ...
    auto-mounted Son of Anton cache dirs (``/root/.son-of-anton/...``), the default
    ...
    # In-process gateways (Desktop backend, `son-of-anton serve`) may not have
    ...
    # are longer prefixes, so /root/.son-of-anton/... still translates to the host
    ...
    if default_home is not None and not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.son-of-anton/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.son-of-anton"):
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    # credential/secret stores under the Son of Anton root (~/.son-of-anton/.env,
    ...
    # ``MEDIA:~/.son-of-anton/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?son-of-anton\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+son-of-anton[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``SON_OF_ANTON_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-SON_OF_ANTON_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_hard_cap_seconds: float = _float_env(
            "SON_OF_ANTON_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-SON_OF_ANTON_HOME holder may be replaced only when the runner
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Son of Anton home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.son-of-anton/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          SON_OF_ANTON_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          SON_OF_ANTON_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          SON_OF_ANTON_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("SON_OF_ANTON_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("SON_OF_ANTON_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("SON_OF_ANTON_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _son_of_anton_run_generation is set on
