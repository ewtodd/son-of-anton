...
const SCAN_MODULE = 'renco_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Renco processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'renco_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Renco process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Renco installation is free.\n' +
    '\n' +
    'Close other Renco windows and terminals, then retry.  If the problem\n' +
    'persists, run `renco update` in a terminal for detailed diagnostics.'
  )
}
