    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `son-of-anton update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    try:
        ...
        pass
    # Son of Anton owns auxiliary retry + provider/model fallback policy (the
    ...
    # Son of Anton sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Son of Anton control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="son-of-anton-protected-aux-provider",
        daemon=True,
    ).start()
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        from son_of_anton_cli.models import fetch_models_with_pricing
...
# in lockstep with son_of_anton_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``son_of_anton_cli.__version__`` is
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("SON_OF_ANTON_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``son_of_anton_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.son-of-anton/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from son_of_anton_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from son_of_anton_cli.auth import _agent_key_is_usable
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from son_of_anton_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    if runtime is not None:
        ...
        if not api_key:
            logger.warning(
                ...
                "(run: son-of-anton auth add nous)."
            )
            _mark_provider_unhealthy("nous", ttl=60)
            return None, None
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from son_of_anton_cli.config import load_config
        from son_of_anton_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `son-of-anton setup` or `son-of-anton model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=son_of_anton_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second son-of-anton
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_son_of_anton_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``son_of_anton_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._son_of_anton_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Son of Anton' built-in discovery
    ...
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: son-of-anton model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.son-of-anton/.env",
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_son_of_anton_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_son_of_anton_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Son of Anton owns
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from son_of_anton_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: son-of-anton model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from son_of_anton_cli.models import opencode_zen_free_runtime as _oc_free_rt
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`son_of_anton_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Son of Anton reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Son of Anton reasoning config for direct model calls
