...
import son_of_anton_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`son-of-anton gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `son-of-anton gateway restart` in the background; when a
        ...
        assert "son-of-anton gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``SON_OF_ANTON_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../son_of_anton_test`` SON_OF_ANTON_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "son-of-anton-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir SON_OF_ANTON_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="SON_OF_ANTON_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/son_of_anton_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="SON_OF_ANTON_HOME=/tmp/son-of-anton-e2e-41264"\n'
        assert (
            gateway_cli._temp_home_in_service_definition(unit)
            == "/tmp/son-of-anton-e2e-41264"
        )
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="SON_OF_ANTON_HOME={tmp_path}/son-of-anton-home"\n'
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".son-of-anton" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `son-of-anton gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import son_of_anton_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.son-of-anton.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                ...
                "<string>/Users/alice/.son-of-anton</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_son_of_anton_home", lambda: tmp_path)
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.son-of-anton.gateway"])
            ),
        )
...
class TestSystemUnitSonOfAntonHome:
    """SON_OF_ANTON_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".son-of-anton" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".son-of-anton")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_son_of_anton = target_home / ".son-of-anton"
        root_home = tmp_path / "root"
        root_son_of_anton = root_home / ".son-of-anton"
        managed_bin = target_son_of_anton / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        root_son_of_anton.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(root_son_of_anton))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        )
        monkeypatch.setattr(gateway_cli, "get_son_of_anton_home", lambda: root_son_of_anton)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "son_of_anton_constants.iter_son_of_anton_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "son_of_anton_constants.son_of_anton_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'SON_OF_ANTON_HOME=/home/alice/.son-of-anton' in unit
        assert '/root/.son-of-anton' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's SON_OF_ANTON_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        son_of_anton_home = str(gateway_cli.get_son_of_anton_home().resolve())
        assert f'SON_OF_ANTON_HOME={son_of_anton_home}' in unit


class TestSystemUnitRefreshSyncsSonOfAntonHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.son-of-anton."""

    def test_refresh_adopts_unit_son_of_anton_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_son_of_anton = root_home / ".son-of-anton"
        alice_son_of_anton = alice_home / ".son-of-anton"
        root_son_of_anton.mkdir(parents=True)
        alice_son_of_anton.mkdir(parents=True)
        (root_son_of_anton / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_son_of_anton / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's SON_OF_ANTON_HOME + drain timeout).
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(alice_son_of_anton))
        ...
        # Simulate sudo without inherited SON_OF_ANTON_HOME (falls back to root).
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(root_son_of_anton))
        ...
        assert os.environ["SON_OF_ANTON_HOME"] == str(alice_son_of_anton)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned SON_OF_ANTON_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_son_of_anton_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_son_of_anton_home_from_systemd_unit",
...
class TestSonOfAntonHomeForTargetUser:
    """Unit tests for _son_of_anton_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("SON_OF_ANTON_HOME", raising=False)

        result = gateway_cli._son_of_anton_home_for_target_user("/home/alice")
        assert result == "/home/alice/.son-of-anton"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "son-of-anton gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".son-of-anton" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_son_of_anton_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        ...
        assert f'SON_OF_ANTON_HOME={target_home / ".son-of-anton" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".son-of-anton" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_son_of_anton_home", lambda: profile_dir)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".son-of-anton" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.son-of-anton.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".son-of-anton" / "son-of-anton"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".son-of-anton" / "son-of-anton")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".son-of-anton" / "son-of-anton"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SON_OF_ANTON_HOME", str(root_home / ".son-of-anton"))
        monkeypatch.setattr(gateway_cli, "get_son_of_anton_home", lambda: root_home / ".son-of-anton")
        ...
        # WorkingDirectory is anchored at the target user's SON_OF_ANTON_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.son-of-anton" in unit
        assert "WorkingDirectory=/home/alice/.son-of-anton/son-of-anton" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "son-of-anton-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "son-of-anton-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'son-of-anton gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacySonOfAntonUnitDetection:
    """Tests for _find_legacy_son_of_anton_units / has_legacy_son_of_anton_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``son-of-anton.service`` unit behind when the service was
    renamed to ``son-of-anton-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``son-of-anton.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``son-of-anton-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Son of Anton Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m son_of_anton_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "son-of-anton.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "son-of-anton.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_son_of_anton_units()
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m son_of_anton_cli.main gateway run
          - python path/to/son_of_anton_cli/main.py gateway run
          - son-of-anton gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m son_of_anton_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/son-of-anton/son_of_anton_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/son-of-anton gateway run --replace",
            "ExecStart=/venv/bin/python /opt/son-of-anton/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "son-of-anton.service" if i == 0 else "son-of-anton.service"  # same name
            # Test each variant fresh
            (user_dir / "son-of-anton.service").write_text(
                f"[Unit]\nDescription=Old Son of Anton\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_son_of_anton_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "son-of-anton.service" in out
        assert "son-of-anton gateway migrate-legacy" in out



class TestRemoveLegacySonOfAntonUnits:
    """Tests for remove_legacy_son_of_anton_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "son-of-anton.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_son_of_anton_units(interactive=False)
        ...
        assert any("--user stop son-of-anton.service" in c for c in cmds_joined)
        assert any("--user disable son-of-anton.service" in c for c in cmds_joined)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (son-of-anton-gateway-coder.service)
        ...
        profile_unit = user_dir / "son-of-anton-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "son-of-anton-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `son-of-anton gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import son_of_anton_cli.main as cli_main
        ...
        # public, just confirm that `son-of-anton gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import son_of_anton_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "son_of_anton_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_son_of_anton_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "son_of_anton_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_son_of_anton_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_son_of_anton_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_son_of_anton_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_son_of_anton_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_son_of_anton_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "son-of-anton-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "son-of-anton-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "son-of-anton-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``son-of-anton gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "son-of-anton-gateway")
        ...
        assert "sudo systemctl start son-of-anton-gateway" in out
        assert "sudo son-of-anton gateway uninstall --system" in out
        assert "son-of-anton gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``son-of-anton gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (SON_OF_ANTON_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """



    def test_user_unit_workingdirectory_is_son_of_anton_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".son-of-anton"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_son_of_anton_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_son_of_anton_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.son-of-anton.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.son-of-anton.gateway"
