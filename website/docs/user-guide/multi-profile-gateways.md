...
If you only run one Renco agent, you don't need this page — see
...
[Connecting Desktop to Many Renco Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Renco agents that should all
...
(`ai.renco.gateway-<name>.plist`) or systemd user service
(`renco-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
renco profile create coder
renco profile create personal-bot
renco profile create research
...
Equivalently, in the default profile's `~/.renco/config.yaml`:
...
You do **not** run `renco gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `renco gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.renco/profiles/coder/.env`; the default listener key is rejected.
...
home). `renco status` reports the multiplexer and the profiles it serves;
`renco status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/renco-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: renco-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    renco gateway "$action"
  else
    renco -p "$profile" gateway "$action"
  fi
}
...
```bash
renco-gateways start      # start every configured profile
renco-gateways stop       # stop every configured profile
renco-gateways restart    # restart all
renco-gateways status     # status across all
renco-gateways list       # delegates to `renco gateway list`
```

:::tip
The `default` profile is targeted with `renco gateway <action>` (no `-p`),
not `renco -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `renco -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.renco.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/renco-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.renco.gateway.plist` /
`renco-gateway.service`.
...
```bash
# Default profile
tail -f ~/.renco/logs/gateway.log
tail -f ~/.renco/logs/gateway.error.log

# Named profile
tail -f ~/.renco/profiles/<name>/logs/gateway.log
tail -f ~/.renco/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.renco/logs/gateway.log ~/.renco/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
renco logs -f                  # follow default profile
renco -p coder logs -f         # follow one profile
renco logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
renco profile list             # profiles + model + gateway state
renco-gateways status          # full status across every profile
launchctl list | grep renco    # macOS — PIDs and labels
systemctl --user list-units 'renco-gateway-*'   # Linux — units
```
...
The default profile uses `~/.renco/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
renco config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.renco/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=renco --why="gateways running" \
  sleep infinity &
...
`renco-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.renco/.env ~/.renco/profiles/*/.env
```

## Updating the code

`renco update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `renco gateway start` after a previous `renco gateway stop`. The
...
```bash
ps -ef | grep "renco_cli.*-p <profile>"
cat ~/.renco/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.renco.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.renco.gateway-<profile>.plist

# Linux
systemctl --user restart renco-gateway-<profile>.service
```

### Health check

```bash
renco doctor                  # default profile
renco -p <profile> doctor     # one profile
```
