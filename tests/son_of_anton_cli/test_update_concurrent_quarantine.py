"""Tests for issue #26670 — concurrent son-of-anton.exe detection and improved
quarantine retry / reboot-deferred fallback during `son-of-anton update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_son_of_anton_instances
# helper (and need the autouse stub in tests/son_of_anton_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_son_of_anton_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "son-of-anton.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "son-of-anton.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_son_of_anton_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "son-of-anton.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_son_of_anton_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "son-of-anton.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_son_of_anton_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("son-of-anton.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_son_of_anton_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "son_of_anton_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "son_of_anton_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m son_of_anton_cli.main gateway run"
