...
description: "Plugins shipped with Renco Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Renco ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.renco/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Renco Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.renco/plugins/<name>/`
3. **Project** — `./.renco/plugins/<name>/` (requires `RENCO_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `renco_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `renco memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `renco plugins list` and the interactive `renco plugins` UI), but none load until you explicitly enable them:

```bash
renco plugins enable disk-cleanup
```

Or via `~/.renco/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Renco. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
renco plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `renco plugins enable <name>`.
...
| `renco-achievements` | dashboard tab | Steam-style collectible badges generated from your real Renco session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `renco memory` and `renco plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `RENCO_HOME` or `/tmp/renco-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under RENCO_HOME | always | Never |
...
**State** — everything lives at `$RENCO_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `RENCO_HOME` or `/tmp/renco-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `renco plugins enable disk-cleanup` (or check the box in `renco plugins`).

**Disabling again:** `renco plugins disable disk-cleanup`.
...
**Enabling:** `renco plugins enable security-guidance` (or check the box in `renco plugins`).

**Disabling again:** `renco plugins disable security-guidance`.
...
Traces Renco turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Renco' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `renco logs`.
...
```bash
renco tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Renco and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
renco plugins enable observability/langfuse
```

Then put the credentials in `~/.renco/.env`:

```bash
RENCO_LANGFUSE_PUBLIC_KEY=pk-lf-...
RENCO_LANGFUSE_SECRET_KEY=sk-lf-...
RENCO_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Renco turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `RENCO_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Renco session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `renco chat` session lives under one Langfuse session.

**Verify:**

```bash
renco plugins list                 # observability/langfuse should show "enabled"
renco chat -q "hello"              # check the Langfuse UI for a "Renco turn" trace
```
...
| `RENCO_LANGFUSE_RELEASE` | — | Release/version tag |
| `RENCO_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `RENCO_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `RENCO_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Renco-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Renco-prefixed wins when both are set.
...
**Disabling:** `renco plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled Renco plugin. Do not run `renco plugins enable observability/nemo_relay`; Renco core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `RENCO_NEMO_RELAY_PLUGINS_TOML` to that file before starting Renco. The policy is process-wide for every profile hosted by that Renco process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `RENCO_NEMO_RELAY_ATOF_*` and `RENCO_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `renco doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `renco.session.segment` plus `renco.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.renco/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
renco plugins enable google_meet
renco meet setup   # preflight: playwright, chromium, auth file
renco meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `renco plugins disable google_meet`. Any saved transcripts stay in `~/.renco/workspace/meetings/` until you remove them.

### renco-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Renco session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Renco feature changes.

**How it works:**

- Scans your entire `~/.renco/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$RENCO_HOME/plugins/renco-achievements/state.json`
...
| Secret | Hidden until Renco detects the first related signal in your history |

**API** — routes mount under `/api/plugins/renco-achievements/`:
...
**State files** — live under `$RENCO_HOME/plugins/renco-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Renco updates. |
...
**Enabling:** Nothing to enable — `renco-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `renco dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/renco-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.renco/plugins/renco-achievements/` that ships no dashboard. The plugin's state files under `$RENCO_HOME/plugins/renco-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Renco plugin — see [Build a Renco Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.renco/plugins/<name>/`
- Manifest source is reported as `bundled` in `renco plugins list`
