...
| List/filter by metadata (age, source, cost, tokens, workspace) | `renco sessions list` / `stats` via terminal |
| Rename | `renco sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `renco sessions archive <filters>` |
| Delete (destructive) | `renco sessions delete` / `renco sessions prune <filters>` |
| Export before deleting anything valuable | `renco sessions export --session-id <id> --format md` |
...
`renco sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `renco sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `renco sessions list`
