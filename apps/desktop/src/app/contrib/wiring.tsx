...
import { getLatestSessionMessages } from '@/renco'
...
import { useRencoConfig } from '../session/hooks/use-renco-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshRencoConfig, sttEnabled, voiceMaxRecordingSeconds } = useRencoConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('renco:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('renco:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshRencoConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshRencoConfig])
