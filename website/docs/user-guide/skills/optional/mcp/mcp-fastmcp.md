...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/mcp/fastmcp` |
...
| Related skills | [`renco-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-renco-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Renco or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Renco. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.renco/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.renco/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Renco integration, either:

- configure the server in `~/.renco/config.yaml` using the `native-mcp` skill, or
...
### Renco cannot see the deployed server

The server-building part may be correct while the Renco config is not. Load the `native-mcp` skill and configure the server in `~/.renco/config.yaml`, then restart Renco.
