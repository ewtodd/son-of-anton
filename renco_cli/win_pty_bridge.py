"""Windows ConPTY bridge for the `renco dashboard` chat tab.

Drop-in counterpart to ``renco_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``renco_cli.web_server`` consumes: ``spawn``, ``read``,
