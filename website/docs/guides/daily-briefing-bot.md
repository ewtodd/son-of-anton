...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `renco setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Renco spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.renco/cron/output/` and you can read them anytime.
:::
...
Renco will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Renco Agent
```
...
Before creating cron jobs, ensure Renco has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Renco what you want:
...
Renco will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Renco to delegate each topic to a sub-agent:
...
Renco will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
