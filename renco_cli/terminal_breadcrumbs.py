"""Per-terminal session breadcrumbs for ``renco -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$RENCO_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``renco -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_renco_home() / "terminal-sessions"
