...
const logDir = join(process.env.SON_OF_ANTON_HOME?.trim() || join(homedir(), '.son-of-anton'), 'logs')
...
// a real ~/.son-of-anton (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('son-of-anton-tui: parent lifecycle log unavailable\n')
    }
  }
}
