...
def bang_shell_enabled() -> bool:
    ...
    if env_var_enabled("RENCO_GATEWAY_SESSION"):
        return False
    if env_var_enabled("RENCO_CRON_SESSION"):
        return False
    if (os.getenv("RENCO_SESSION_PLATFORM") or "").strip():
        return False
    return True
...
def _bang_env() -> dict:
    """Environment for a bang command, with Renco-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Renco session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
