import type { InputEvent, Key } from '@renco/ink'
import * as Ink from '@renco/ink'
 ...
 * but that accepts `key.meta` on macOS — and renco-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.RENCO_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which renco-ink decodes as meta+'d'; without this branch it
