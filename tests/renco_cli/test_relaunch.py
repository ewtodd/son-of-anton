"""Tests for renco_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from renco_cli import relaunch as relaunch_mod


class TestResolveRencoBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/renco"
        ...
        assert relaunch_mod.resolve_renco_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'renco' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_renco_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/renco" if name == "renco" else None
        )
        assert relaunch_mod.resolve_renco_bin() == "/usr/bin/renco"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_renco_bin", lambda: "/usr/bin/renco")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/renco"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/renco", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/renco", ["/usr/bin/renco", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        renco).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_renco_bin", lambda: r"C:\Users\test\renco.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\renco.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\renco.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_renco_bin", lambda: r"C:\renco.exe")
...
class TestResolveRencoBinWindowsPyGuard:
    """On Windows, resolve_renco_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a renco.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\renco.exe" if name == "renco" else None,
        )

        bin_path = relaunch_mod.resolve_renco_bin()
        # Must NOT be the .py — must be the renco.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\renco.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "renco"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_renco_bin() == str(script)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_renco_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND renco.exe
        ...
        assert relaunch_mod.resolve_renco_bin() is None
