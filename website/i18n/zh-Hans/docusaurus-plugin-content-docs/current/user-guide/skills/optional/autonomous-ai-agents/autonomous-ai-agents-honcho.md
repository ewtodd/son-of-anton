...
description: "配置并使用 Honcho 记忆功能与 Renco -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。"
---
...
配置并使用 Honcho 记忆功能与 Renco -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。适用于设置 Honcho、排查记忆问题、通过 Honcho peers 管理配置文件，或调整观察、召回和辩证设置。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `renco skills install official/autonomous-ai-agents/honcho` 安装 |
...
| 相关 skills | [`renco-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-renco-agent) |
...
Honcho 提供 AI 原生的跨会话用户建模。它在多次对话中学习用户特征，并为每个 Renco 配置文件提供独立的 peer 身份，同时共享统一的用户视图。
...
参见：https://docs.honcho.dev/v3/guides/integrations/renco#running-honcho-locally-with-renco
...
3. **AI peer 卡片** -- 此 Renco 配置文件的 AI peer 身份卡片
...
Honcho 将对话建模为 **peers** 之间的交互。Renco 每个会话创建两个 peers：

- **用户 peer**（`peerName`）：代表人类用户。Honcho 从观察到的消息中构建用户表示。
- **AI peer**（`aiPeer`）：代表此 Renco 实例。每个配置文件拥有自己的 AI peer，使 agents 形成独立视角。
...
| `per-session` | 每次 Renco 运行创建新的 Honcho 会话 |
| `global` | 跨所有目录使用单一会话 |

手动覆盖：`renco honcho map my-project-name`
...
每个 Renco 配置文件拥有自己的 Honcho AI peer，同时共享同一工作区（用户上下文）。这意味着：
...
`--clone` 对 Honcho 的作用：
1. 在 `honcho.json` 中创建 `renco.coder` host 块
...
配置文件：`$RENCO_HOME/honcho.json`（配置文件本地）或 `~/.honcho/config.json`（全局）。
...
运行 `renco honcho setup`。确保 `~/.renco/config.yaml` 中包含 `memory.provider: honcho`。

### 记忆未跨会话持久化
检查 `renco honcho status` -- 验证 `saveMessages: true` 且 `writeFrequency` 不是 `session`（该选项仅在退出时写入）。

### 配置文件未获得自己的 peer
创建时使用 `--clone`：`renco profile create <name> --clone`。对于现有配置文件：`renco honcho sync`。
...
| 命令 | 描述 |
|---------|-------------|
| `renco honcho setup` | 交互式设置向导（云端/本地、身份、观察、召回、会话） |
| `renco honcho status` | 显示当前配置文件的已解析配置、连接测试、peer 信息 |
| `renco honcho enable` | 为当前配置文件启用 Honcho（如需则创建 host 块） |
| `renco honcho disable` | 为当前配置文件禁用 Honcho |
| `renco honcho peer` | 显示或更新 peer 名称（`--user <name>`、`--ai <name>`、`--reasoning <level>`） |
| `renco honcho peers` | 显示所有配置文件的 peer 身份 |
| `renco honcho mode` | 显示或设置召回模式（`hybrid`、`context`、`tools`） |
| `renco honcho tokens` | 显示或设置 token 预算（`--context <N>`、`--dialectic <N>`） |
| `renco honcho sessions` | 列出已知的目录到会话名称映射 |
| `renco honcho map <name>` | 将当前工作目录映射到 Honcho 会话名称 |
| `renco honcho identity` | 为 AI peer 身份播种，或显示两个 peer 的表示 |
| `renco honcho sync` | 为所有尚未拥有 host 块的 Renco 配置文件创建 host 块 |
| `renco honcho migrate` | 从 OpenClaw 原生记忆迁移到 Renco + Honcho 的分步指南 |
| `renco memory setup` | 通用记忆提供商选择器（选择 "honcho" 运行相同向导） |
| `renco memory status` | 显示当前活跃的记忆提供商及配置 |
| `renco memory off` | 禁用外部记忆提供商 |