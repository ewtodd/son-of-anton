...
def test_root_session_no_compression(db):
    ...
    assert prov["currentRencoSessionId"] == "root1"
    assert prov["rootRencoSessionId"] == "root1"
    assert prov["parentRencoSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_renco_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentRencoSessionId"] == "old"
    assert prov["rootRencoSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousRencoSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootRencoSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"renco"}
    assert "sessionProvenance" in meta["renco"]
    assert meta["renco"]["sessionProvenance"]["currentRencoSessionId"] == "root1"
