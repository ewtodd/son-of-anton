...
from tools.code_execution_tool import (
    ...
    _uses_son_of_anton_python_environment,
...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        self.assertIn("son_of_anton_sandbox_", result["output"])
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The son-of-anton venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_son_of_anton_tools(self):
        """Regression: son_of_anton_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from son_of_anton_tools import terminal`.
        """
        import tempfile
    ...
    def test_strict_mode_can_still_import_son_of_anton_tools(self):
...
class TestSecurityInvariantsAcrossModes(unittest.TestCase):
    ...
    def test_tool_whitelist_enforced_in_strict_mode(self):
        ...
        code = (
            "import son_of_anton_tools as ht\n"
...
# ---------------------------------------------------------------------------
# _python_environment_prefix / _uses_son_of_anton_python_environment
# ---------------------------------------------------------------------------
...
class TestPythonEnvironmentPrefix(unittest.TestCase):
    ...
    def test_failure_is_not_cached(self):
        ...
        A sticky cached failure would silently drop the son-of-anton root from
...
class TestUsesSonOfAntonPythonEnvironment(unittest.TestCase):
    """Unit tests for _uses_son_of_anton_python_environment."""
    ...
    def test_true_for_current_interpreter(self):
        ...
        self.assertTrue(_uses_son_of_anton_python_environment(sys.executable))
    ...
    def test_true_for_current_interpreter_without_probe(self):
        ...
        must never drop the son-of-anton root for the interpreter Son of Anton itself runs.
        """
        with patch("subprocess.run",
                   side_effect=subprocess.TimeoutExpired(cmd=[], timeout=5)) as mock_run:
            self.assertTrue(_uses_son_of_anton_python_environment(sys.executable))
    ...
    def test_false_for_different_prefix(self):
        ...
        with patch("tools.code_execution_tool._python_environment_prefix",
            ...
            self.assertFalse(_uses_son_of_anton_python_environment("/other/python"))

    def test_false_when_prefix_is_empty(self):
        """If prefix cannot be determined (error path), treat as external."""
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=""):
            self.assertFalse(_uses_son_of_anton_python_environment("/bad/python"))

    def test_true_when_prefix_matches_sys_prefix(self):
        son_of_anton_prefix = os.path.realpath(sys.prefix)
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=son_of_anton_prefix):
            self.assertTrue(_uses_son_of_anton_python_environment("/same/env/python"))


# ---------------------------------------------------------------------------
# PYTHONPATH composition — son-of-anton root included only for same-env interpreters
# ---------------------------------------------------------------------------

class TestPythonPathComposition(unittest.TestCase):
    """Verify son-of-anton root inclusion in PYTHONPATH depends on env match.

    Patches ``_uses_son_of_anton_python_environment`` directly so these tests are
    ...
    def _son_of_anton_root(self) -> str:
    ...
    def test_son_of_anton_root_included_when_same_env(self):
        """When interpreter is in the Son of Anton env, son-of-anton root is in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=True)
        parts = pythonpath.split(os.pathsep)
        self.assertIn(self._son_of_anton_root(), parts,
                      "son-of-anton root must be in PYTHONPATH for same-env interpreters")

    def test_son_of_anton_root_excluded_when_external_env(self):
        """When interpreter is external, son-of-anton root must NOT be in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=False)
        parts = pythonpath.split(os.pathsep)
        self.assertNotIn(self._son_of_anton_root(), parts,
                         "son-of-anton root must not leak into an external interpreter's PYTHONPATH")
