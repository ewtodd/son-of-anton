...
from son_of_anton_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_nous_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    """
    try:
        from son_of_anton_cli.nous_account import nous_portal_topup_url
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Dev override: when SON_OF_ANTON_DEV_CREDITS_FIXTURE selects a fixture state, /usage
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so SON_OF_ANTON_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — SON_OF_ANTON_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="nous",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Son of Anton's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `son-of-anton auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `son-of-anton auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
    ...
    if code == "reset":
        ...
        # persisted pool cooldowns so Son of Anton doesn't keep the credential
        # frozen behind the now-stale ``last_error_reset_at`` (issue #43747).
        try:
            from son_of_anton_cli.auth import clear_codex_pool_quota_cooldowns
