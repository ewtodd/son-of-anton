"""
Son of Anton Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with son-of-anton;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.son-of-anton/plugins/<name>/``
3. **Project plugins** – ``./.son-of-anton/plugins/<name>/`` (opt-in via
   ``SON_OF_ANTON_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``son_of_anton_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``SON_OF_ANTON_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("SON_OF_ANTON_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``SON_OF_ANTON_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.son-of-anton/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("SON_OF_ANTON_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When SON_OF_ANTON_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Son of Anton loggers.
    """
    ...
    logger.debug(
        "SON_OF_ANTON_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Son of Anton' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by son_of_anton_cli.kanban_db when a task
    ...
    # separate `son-of-anton -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `son-of-anton kanban dispatch`),
    ...
    # ``son-of-anton kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: son_of_anton_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINTS_GROUP = "son_of_anton_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "son_of_anton_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``son_of_anton_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- son-of-anton-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- son-of-anton-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- son-of-anton-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``son-of-anton:<event>``.
SON_OF_ANTON_EVENT_NAMESPACE = "son-of-anton"
...
_NS_PARENT = "son_of_anton_plugins"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Son of Anton home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "son-of-anton", "depends",
}

# Highest manifest schema version this Son of Anton understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Son of Anton "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `son-of-anton plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    # lookups and by ``son-of-anton plugins list``. For a flat plugin at
    ...
    # ``son_of_anton_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Son of Anton never
    ...
    # (``son-of-anton plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Son of Anton paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_son_of_anton_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Son of Anton config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from son_of_anton_cli.platform_actions import PlatformActions
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Son of Anton core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``son-of-anton honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``son-of-anton <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`son_of_anton_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from son_of_anton_cli.dashboard_auth import (
            DashboardAuthProvider,
            register_provider,
        )
        from son_of_anton_cli.dashboard_auth.registry import restore_registration
        from son_of_anton_cli.dashboard_auth.registry import snapshot_registration
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_son_of_anton_dotenv()`` startup — after
        ``~/.son-of-anton/.env`` loads, before Son of Anton reads credentials — when
        ...
        NOTE ON TIMING: ``load_son_of_anton_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_son_of_anton_dotenv``) so the first process sees them (#64177).
    ...
    def register_slack_action_handler(
        ...
        Son of Anton' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: son_of_anton_cli.main imports plugins indirectly
        from son_of_anton_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``son-of-anton:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``son-of-anton:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, SON_OF_ANTON_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{SON_OF_ANTON_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``son-of-anton:<event>``
    ...
    def register_skill(
        ...
        ``~/.son-of-anton/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or son_of_anton_home_key()
        ...
        # by resolved son-of-anton home). The ledger is therefore keyed per manager
        # — i.e. per (son_of_anton_home, plugin_id) — and every release/restore
        ...
        # contributed, so `son-of-anton plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("SON_OF_ANTON_SAFE_MODE"):
                logger.info("SON_OF_ANTON_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_son_of_anton_dotenv() already ran at import time. Re-pull so the
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from son_of_anton_cli.env_loader import load_son_of_anton_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Son of Anton plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Son of Anton core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Son of Anton core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Son of Anton Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with son-of-anton and must
            ...
            # path actually asks for that platform. Every platform Son of Anton ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `son-of-anton plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from son_of_anton_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_son_of_anton_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_son_of_anton_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.son-of-anton/plugins/)
        user_dir = get_son_of_anton_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.son-of-anton/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("SON_OF_ANTON_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".son-of-anton" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set SON_OF_ANTON_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from son_of_anton_cli.agent_plugins import _discover_mcp
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Son of Anton process (e.g. a pip
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``son-of-anton plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`son-of-anton plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``son-of-anton tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `son-of-anton plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Son of Anton validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Son of Anton does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `son-of-anton plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from son_of_anton_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_son_of_anton_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``son_of_anton_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``son_of_anton_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # from a different Son of Anton home (profile switch reusing a slug
        # like "son-of-anton-lcm") or from an earlier force=True reload in the
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            ...
            args=(dispatch_queue,),
            name="son-of-anton-plugin-events",
            daemon=True,
        )
...
# Keyed cache: resolved Son of Anton home -> PluginManager. Son of Anton supports
# multiple profiles via different SON_OF_ANTON_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_son_of_anton_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# son_of_anton_constants.set_son_of_anton_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_son_of_anton_home() / "plugins"`` and some
    plugins (notably context engines such as son-of-anton-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Son of Anton home (env var *or* the context-local
    ``set_son_of_anton_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Son of Anton home instead of
    being one process-wide singleton.
    """
    try:
        return get_son_of_anton_home().expanduser().resolve()
    except Exception:
...
def get_plugin_manager() -> PluginManager:
    ...
    profile switch — via ``SON_OF_ANTON_HOME`` or the context-local
    ``set_son_of_anton_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=son_of_anton_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_son_of_anton_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from son_of_anton_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="son-of-anton-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``son-of-anton tools`` TUI so plugin-provided toolsets appear
