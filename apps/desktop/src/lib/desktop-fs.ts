import type {
  RencoConnection,
  RencoReadDirResult,
  RencoReadFileTextResult,
  RencoSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: RencoConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: RencoConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<RencoReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<RencoReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<RencoReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<RencoReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.rencoDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: RencoSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
