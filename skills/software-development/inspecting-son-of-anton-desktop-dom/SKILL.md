---
name: inspecting-son-of-anton-desktop-dom
description: "Read the live Son of Anton desktop DOM/CSS over CDP."
version: 1.0.0
...
# Inspecting the live Son of Anton desktop DOM
...
`debugging-son-of-anton-desktop`), or anything where the real question is "does this
look right".
...
- **no `SON_OF_ANTON_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`SON_OF_ANTON_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${SON_OF_ANTON_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
SON_OF_ANTON_HOME=/tmp/cdp-probe-home \
SON_OF_ANTON_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
SON_OF_ANTON_DESKTOP_CDP_PORT=9333 \
  npx electron . --user-data-dir=/tmp/cdp-probe-userdata
```

The separate `--user-data-dir` dodges Electron's single-instance lock, so it
cannot collide with a running `hgui`; the separate `SON_OF_ANTON_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `SON_OF_ANTON_HOME` baked in, if you
...
- **A throwaway `SON_OF_ANTON_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `son-of-anton:api` and may exit on its own. The renderer still mounts and the DOM is
