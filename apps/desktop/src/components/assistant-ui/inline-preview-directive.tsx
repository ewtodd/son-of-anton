 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.renco.send(prompt)` (or declarative
 * `data-renco-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'renco-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'renco-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `renco.send(prompt)`.
 ...
 *  wires `data-renco-send` so declarative HTML works with zero script:
 *  `<button data-renco-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.renco={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-renco-send]"):null;' +
    'if(el)send(el.getAttribute("data-renco-send")||"")},true)})()</script>'
  )
}
...
function InlineHtmlFrame({
  ...
  useEffect(() => {
    ...
    void Promise.resolve(window.rencoDesktop?.readFileText(path))
      .then(result => {
        if (!alive) {
          return
        }
