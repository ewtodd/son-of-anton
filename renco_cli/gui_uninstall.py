"""
Renco Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``renco desktop`` / ``renco gui``)
     Built inside the agent checkout under ``$RENCO_HOME/renco-agent/``:
       ...
       - ``$RENCO_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Renco.app`` or ``~/Applications/Renco.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Renco`` (NSIS per-user)
...
the app name ("Renco"), separate from ``$RENCO_HOME``:
  - macOS:   ``~/Library/Application Support/Renco``
  - Windows: ``%APPDATA%\\Renco``
  - Linux:   ``$XDG_CONFIG_HOME/Renco`` (default ``~/.config/Renco``)
...
``renco uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(renco_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return renco_home / "renco-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Renco"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Renco"
...
def source_built_gui_artifacts(renco_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``renco desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``renco-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(renco_home)
    ...
    return [
        ...
        renco_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Renco" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Renco.app"),
            home / "Applications" / "Renco.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Renco).
            local_base / "Programs" / "Renco",
            # Older / alternate layout some builds used.
            local_base / "renco-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Renco")
    else:
        ...
        # ``uninstall_gui``.
        from renco_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `renco desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Renco.desktop",
        ]
    return paths


def agent_is_installed(renco_home: Path) -> bool:
    """Return True when a usable Python agent install exists under RENCO_HOME.
    ...
    if (agent_root / "renco_cli").is_dir():
        return True
...
def gui_is_installed(renco_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(renco_home):
        if p.exists():
            return True
...
def gui_install_summary(renco_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = renco_home if renco_home is not None else get_renco_home()
...
def uninstall_gui(renco_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``renco-agent/renco_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$RENCO_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Renco disappears from the launcher.
        try:
            from renco_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove renco' or "
            "'sudo dnf remove renco'). AppImage builds are a single file you "
