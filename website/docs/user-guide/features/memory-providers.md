...
Renco Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
renco memory setup      # interactive picker + configuration
renco memory status     # check what's active
renco memory off        # disable external provider
```

You can also select the active memory provider via `renco plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.renco/config.yaml`:
...
When a memory provider is active, Renco automatically:
...
**Setup Wizard:**
```bash
renco memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `renco honcho setup` command still works (it now redirects to `renco memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$RENCO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$RENCO_HOME/honcho.json` > `~/.renco/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/ewtodd/renco/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/renco).
...
:::tip Migrating from `renco honcho`
If you previously used `renco honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Renco profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Renco profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Renco profile. Host key `renco` → default; `renco.<profile>` for others. |
...
```bash
renco profile create coder --clone
```

`--clone` creates a `renco.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Renco profile, creates host blocks for any profile without one, inherits settings from the default `renco` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `renco memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/ewtodd/renco/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/renco).
...
`renco memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.renco/.env`, and for named profiles use
...
`OPENVIKING_AGENT` is Renco' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
renco memory setup    # select "mem0" → "Platform"
# Or manually:
...
**Setup (OSS):**
```bash
renco memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
renco memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```
...
**Config:** `$RENCO_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.renco/.env`.
...
| `user_id` | `renco-user` | User identifier |
...
**Switching modes:** Re-run `renco memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Local mode UI:** `hindsight-embed -p renco ui start`

**Config:** `$RENCO_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `renco` | Memory bank identifier |
...
See [plugin README](https://github.com/ewtodd/renco/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
renco memory setup    # select "holographic"
# Or manually:
renco config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.renco-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$RENCO_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
renco memory setup    # select "retaindb"
# Or manually:
renco config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.renco/.env
```
...
- Knowledge tree stored at `$RENCO_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=renco), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
Before running `renco memory setup`, set `base_url` in
...
Then run `renco memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$RENCO_HOME/supermemory.json`
...
| `container_tag` | `renco` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `renco-{identity}` → `renco-coder`) to isolate memories per Renco profile
...
| **Requires** | `pip install renco-memori` + `renco-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install renco-memori
renco-memori install
renco config set memory.provider memori
renco memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `renco-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$RENCO_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$RENCO_HOME/` so each profile has its own credentials
