...
Usage (via cron with --no-agent):
    ...
    renco cron create renco-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$RENCO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name renco-issues --repo ewtodd/renco --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Renco .env file
(``${RENCO_HOME:-~/.renco}/.env``) to avoid the 60 req/hr
