...
Connects Renco Agent to WeChat personal accounts via Tencent's iLink Bot API.
...
def _account_dir(renco_home: str) -> Path:
    path = Path(renco_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(renco_home: str, account_id: str) -> Path:
    return _account_dir(renco_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(renco_home, account_id)
...
def load_weixin_account(renco_home: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, renco_home: str):
        self._root = _account_dir(renco_home)
...
def _sync_buf_path(renco_home: str, account_id: str) -> Path:
    return _account_dir(renco_home) / f"{account_id}.sync.json"


def _load_sync_buf(renco_home: str, account_id: str) -> str:
    path = _sync_buf_path(renco_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(renco_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(renco_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    """Native Renco adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._token_store = ContextTokenStore(renco_home)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(renco_home, self._account_id)
            if persisted:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Renco regardless of this "
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._renco_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._renco_home, self._account_id, sync_buf)
    ...
    async def send(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"renco-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = f"renco-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
...
async def send_weixin_direct(
    *,
    ...
    token_store = ContextTokenStore(str(get_renco_home()))
