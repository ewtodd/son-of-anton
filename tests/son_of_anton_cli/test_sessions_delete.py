...
def test_sessions_delete_accepts_unique_id_prefix(monkeypatch, capsys):
    ...
    monkeypatch.setattr(son_of_anton_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        ["son-of-anton", "sessions", "delete", "20260315_092437_c9a6", "--yes"],
    )
...
def _run_prune(monkeypatch, capsys, argv_tail, candidates=None, skipped_open=0):
    """Run `son-of-anton sessions prune <argv_tail>` against a FakeDB, capturing
    ...
    monkeypatch.setattr(son_of_anton_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys, "argv", ["son-of-anton", "sessions", "prune", *argv_tail]
    )
...
def test_sessions_prune_bare_keeps_90_day_default(monkeypatch, capsys):
    """A truly bare `son-of-anton sessions prune` keeps the implicit 90-day cutoff."""
...
def test_sessions_prune_preview_shows_oldest_newest(monkeypatch, capsys):
    ...
    from son_of_anton_cli.session_filters import format_epoch
...
def test_sessions_prune_surfaces_matching_open_sessions(monkeypatch, capsys):
    ...
    assert "son-of-anton sessions delete <id>" in out
