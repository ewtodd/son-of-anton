...
class TestSlackExecApproval:
    ...
    async def test_sends_blocks_with_buttons(self):
        ...
        assert "son_of_anton_approve_once" in action_ids
        assert "son_of_anton_approve_session" in action_ids
        assert "son_of_anton_approve_always" in action_ids
        assert "son_of_anton_deny" in action_ids
    ...
    async def test_smart_deny_owner_override_hides_persistent_buttons(self):
        ...
        assert [element["action_id"] for element in elements] == [
            "son_of_anton_approve_once", "son_of_anton_deny",
        ]
...
class TestSlackApprovalAction:
    ...
    async def test_truncates_inflated_original_text(self):
        ...
        action = {"action_id": "son_of_anton_approve_once", "value": "session-key"}
...
class TestSlackReactionForwarding:
    ...
    async def test_reaction_synthesizes_message_in_thread(self):
        ...
        assert synth["_son_of_anton_reaction"]["name"] == "thumbsup"
        assert synth["_son_of_anton_reaction"]["action"] == "added"
        assert synth["_son_of_anton_reaction"]["reacted_to_ts"] == "2000.0"
        # Pre-authorized as addressed-to-the-bot (skips mention gate only).
        assert synth["_son_of_anton_force_process"] is True
    ...
    async def test_target_channel_handoff_routes_top_level(self):
        ...
        assert synth["_son_of_anton_no_thread_response"] is True
        assert synth["_son_of_anton_reaction_source_channel"] == "C1"
    ...
    async def test_target_channel_with_thread_routes_to_thread(self):
        ...
        assert "_son_of_anton_no_thread_response" not in forwarded[0]
