...
- Explicit targets (e.g., "discord:123456789")
- Platform home channels (e.g., "discord" → home channel)
...
# delivery.  Hard API limits vary per platform; the headroom covers the "full
...
class DeliveryTarget:
    ...
    - "discord:123456" → specific Discord chat
    """
    platform: Platform
...
class DeliveryRouter:
    ...
    def __init__(self, config: GatewayConfig, adapters: Dict[Platform, Any] = None,
        ...
        self.output_dir = get_son_of_anton_home() / "cron" / "output"
    ...
    def _save_full_output(self, content: str, job_id: str) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_dir = get_son_of_anton_home() / "cron" / "output"
    ...
    def _filter_silence_narration_enabled(self) -> bool:
        ...
        ``SON_OF_ANTON_FILTER_SILENCE_NARRATION`` env var overrides config when set;
        otherwise the ``gateway.filter_silence_narration`` config flag wins
        (default True).
        """
        env = os.getenv("SON_OF_ANTON_FILTER_SILENCE_NARRATION")
        if env is not None:
