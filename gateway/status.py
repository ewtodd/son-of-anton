...
The PID file lives at ``{SON_OF_ANTON_HOME}/gateway.pid``.  SON_OF_ANTON_HOME defaults to
``~/.son-of-anton`` but can be overridden via the environment variable.  This means
separate SON_OF_ANTON_HOME directories naturally get separate PID files — a property
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_son_of_anton_home() / "gateway-starts.log"
...
def _get_process_son_of_anton_home() -> Path:
    """Return the process-level SON_OF_ANTON_HOME, skipping context-local overrides.
    ...
    ``get_son_of_anton_home()`` honors ``_SON_OF_ANTON_HOME_OVERRIDE`` contextvar used for
...
def _canonical_son_of_anton_home(path: Path | str) -> Path:
    """Return a stable absolute SON_OF_ANTON_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_son_of_anton_home(left: Path | str, right: Path | str) -> bool:
    """Compare SON_OF_ANTON_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_son_of_anton_home(left))) == os.path.normcase(
        str(_canonical_son_of_anton_home(right))
    )
...
def _profile_label_for_home(home: Path | str) -> Optional[str]:
    ...
    try:
        ...
        if _same_son_of_anton_home(canonical, get_default_son_of_anton_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_son_of_anton_home(canonical, _get_platform_default_son_of_anton_home()):
            return "default"
    except Exception:
        pass
    return None
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SON_OF_ANTON_HOME."""
    home = _get_process_son_of_anton_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SON_OF_ANTON_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "son-of-anton" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Son of Anton gateway lifecycle subcommand from a command line.
    ...
    test also matched ``son_of_anton_cli.main gateway status`` and even unrelated
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `son-of-anton gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Son of Anton gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a SON_OF_ANTON_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.son-of-anton`` or ``$SON_OF_ANTON_HOME``)
...
def _build_pid_record() -> dict:
    return {
        ...
        # SON_OF_ANTON_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "son_of_anton_home": str(_canonical_son_of_anton_home(_get_process_son_of_anton_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``son_of_anton_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `son-of-anton update` (and the
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `son-of-anton status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``SON_OF_ANTON_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    SON_OF_ANTON_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    # is inferable; readers fall back to deriving it from son_of_anton_home.
    profile = _profile_label_for_home(_get_process_son_of_anton_home())
    if profile:
...
# enabled in the user's systemd (e.g. ``son-of-anton.service`` + ``son-of-anton-
# gateway.service``).
#
...
def _get_takeover_marker_path(son_of_anton_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``son_of_anton_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = son_of_anton_home or _get_process_son_of_anton_home()
    return _canonical_son_of_anton_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-SON_OF_ANTON_HOME --replace while
    ...
    our_home = _get_process_son_of_anton_home()
    target_home = record.get("target_son_of_anton_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_son_of_anton_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_son_of_anton_home")
        if replacer_home is not None and not _same_son_of_anton_home(
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's SON_OF_ANTON_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_son_of_anton_home(
            target_home or _get_process_son_of_anton_home()
        )
        ...
        record = {
            ...
            "target_son_of_anton_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_son_of_anton_home": str(
                _canonical_son_of_anton_home(_get_process_son_of_anton_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target SON_OF_ANTON_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("son_of_anton_home")
    if not isinstance(pid_record_home, str) or not _same_son_of_anton_home(
