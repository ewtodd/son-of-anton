 ...
 * `iter_renco_node_dirs()` in renco_constants.py, which the Electron main
 * process cannot import.
 */
function rencoManagedNodePathEntries(
  rencoHome,
  ...
  const root = pathModule.join(rencoHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const rencoNodeDirs = rencoManagedNodePathEntries(rencoHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([rencoNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeRencoHomeRoot(rencoHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!rencoHome) {
    return rencoHome
  }

  const resolved = pathModule.resolve(String(rencoHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). renco_bootstrap sets this inside the child too, but only
