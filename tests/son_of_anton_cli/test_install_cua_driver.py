...
* Be supported-platform-only — no-op silently elsewhere so ``son-of-anton update``
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Son of Anton
    ...
    def test_non_upgrade_repairs_incompatible_existing_driver(self):
        ...
        incompatible = {
            "ready": False,
            "version": "0.19.4",
            "reason": "Son of Anton computer use requires cua-driver 0.20.0 or newer",
        }
    ...
    def test_incompatible_explicit_override_is_not_replaced(self, monkeypatch):
        ...
        monkeypatch.setenv("SON_OF_ANTON_CUA_DRIVER_CMD", "/opt/custom/cua-driver")
        incompatible = {
            "ready": False,
            "version": "0.19.4",
            "reason": "Son of Anton computer use requires cua-driver 0.20.0 or newer",
    ...
    def test_missing_explicit_override_does_not_install_standard_driver(
        ...
        monkeypatch.setenv("SON_OF_ANTON_CUA_DRIVER_CMD", "/missing/custom/cua-driver")
        with patch.object(
                 tools_config,
                 "_resolved_cua_driver_cmd",
                 return_value=None,
...
class TestRequireConfirmedUpdate:
    """`son-of-anton update` passes require_confirmed_update=True: the full
    ...
    Explicit `son-of-anton computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `son-of-anton computer-use install --upgrade` (default flag): the old
    ...
    def test_incompatible_driver_repairs_despite_indeterminate_check(self):
        """Son of Anton' own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `son-of-anton update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `son-of-anton update`.
    """
...
class TestCuaVersionSummary:
    """`son-of-anton computer-use status` prints one line, whatever the binary says.

    A binary chosen by SON_OF_ANTON_CUA_DRIVER_CMD is under no obligation to answer
