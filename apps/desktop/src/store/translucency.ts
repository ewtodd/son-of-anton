 ...
 * `@renco/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-renco-glass]`
 * block in styles.css).
 */
...
} from '@renco/shared/translucency'
 ...
 * sets `rencoDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.rencoDesktop?.glassSupported === 'boolean'
    ? window.rencoDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.rencoDesktop?.translucencySupported === 'boolean'
    ? window.rencoDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-renco-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-renco-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-renco-glass-on', glassLive)
  root.toggleAttribute('data-renco-glass', glassOn)
  root.toggleAttribute('data-renco-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-renco-glass-scope', scope)
  } else {
    root.removeAttribute('data-renco-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.rencoDesktop?.setTranslucency?.(state)
  })
