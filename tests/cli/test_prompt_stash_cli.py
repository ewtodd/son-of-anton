"""Tests for the Ctrl+S prompt stash wiring inside SonOfAntonCLI.

The state machine itself is covered by tests/cli/test_prompt_stash.py. These
tests verify the cli.py side:
  - SonOfAntonCLI.__init__ creates a PromptStash
...
def _make_cli(**kwargs):
    """Create a SonOfAntonCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), patch.dict(
        ...
        with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), patch.dict(
            ...
            return _cli_mod.SonOfAntonCLI(**kwargs)
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports son_of_anton_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``SonOfAntonCLI.run()``'s local ``KeyBindings`` instance, so they are not
