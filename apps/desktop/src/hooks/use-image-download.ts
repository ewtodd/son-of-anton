...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'renco:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.rencoDesktop?.saveImageFromUrl) {
        if (await window.rencoDesktop.saveImageFromUrl(src)) {
