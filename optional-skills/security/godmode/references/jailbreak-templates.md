...
## 5. GODMODE FAST — Renco 4 405B (Zero Refusal)

**Model:** `nousresearch/renco-4-405b`
**Strategy:** Renco models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Renco Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.renco/config.yaml`:
