...
def _receipt_dir() -> Path:
    ...
    return get_son_of_anton_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``son-of-anton update`` has many early-termination paths (Windows
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if any_stale:
        ...
        print("      son-of-anton gateway restart                # active profile")
        print("      son-of-anton -p <profile> gateway restart   # named profile")
    return any_stale
