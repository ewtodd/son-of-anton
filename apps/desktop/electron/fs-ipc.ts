...
// hardening, RENCO_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('renco:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('renco:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('renco:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('renco:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<RENCO_HOME>/desktop-plugins`,
  // resolved from the main-process RENCO_HOME (see resolveRencoHome) — NOT from
  // the connected backend. A remote backend reports its own `renco_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped renco_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(rencoHome, 'profiles', profile) : rencoHome
  ...
  ipcMain.handle('renco:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL agent-plugin root (`<RENCO_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('renco:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('renco:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('renco:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('renco:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('renco:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('renco:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
