"""
Profile management for multiple isolated Son of Anton instances.

Each profile is a fully independent SON_OF_ANTON_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.son-of-anton/profiles/<name>/`` by default.

The "default" profile is ``~/.son-of-anton`` itself — backward compatible,
zero migration needed.

Usage::

    son-of-anton profile create coder          # fresh profile + bundled skills
    son-of-anton profile create coder --clone  # also copy config, .env, SOUL.md, skills
    son-of-anton profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    son-of-anton -p coder chat                 # or via flag
    son-of-anton profile use coder             # set as sticky default
    son-of-anton profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See son_of_anton_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.son-of-anton``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   son-of-anton  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `son-of-anton backup` archives
...
# Marker file written by `son-of-anton profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `son-of-anton
...
# `son-of-anton skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Son of Anton infrastructure directories that only the default profile
         (``~/.son-of-anton``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_son_of_anton_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.son-of-anton) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "son-of-anton",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when SON_OF_ANTON_HOME equals the
...
# into the export. The set below identifies the *known Son of Anton profile
# artifacts* at the root of SON_OF_ANTON_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``son_of_anton_constants``.
...
_RESERVED_NAMES = frozenset({
    "son-of-anton", "default", "test", "tmp", "root", "sudo",
})

# Son of Anton subcommands that cannot be used as profile names/aliases
_SON_OF_ANTON_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the son-of-anton root, NOT to the current SON_OF_ANTON_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SON_OF_ANTON_HOME points outside
    ``~/.son-of-anton``, profiles live under ``SON_OF_ANTON_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_son_of_anton_home() / "profiles"


def _get_default_son_of_anton_home() -> Path:
    """Return the default (pre-profile) SON_OF_ANTON_HOME path.

    In standard deployments this is ``~/.son-of-anton``.
    In Docker/custom deployments where SON_OF_ANTON_HOME is outside ``~/.son-of-anton``
    (e.g. ``/opt/data``), returns SON_OF_ANTON_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_son_of_anton_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``son-of-anton``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``son-of-anton`` profile inside ``~/.son-of-anton/``) or get refused
    at alias-creation time anyway. ``default`` is a special pass-through —
    it's a valid alias for the built-in root profile.
    """
    if name == "default":
        return  # special alias for ~/.son-of-anton
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Son of Anton installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SON_OF_ANTON_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_son_of_anton_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, son-of-anton subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SON_OF_ANTON_SUBCOMMANDS:
        return f"'{canon}' conflicts with a son-of-anton subcommand"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Son of Anton. If we
    ...
    try:
        ...
        from son_of_anton_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `son-of-anton doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``son-of-anton -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``son-of-anton profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``son-of-anton -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``son-of-anton profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SON_OF_ANTON_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Son of Anton config
...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    break ``son-of-anton profile list``.
    """
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, son_of_anton_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SON_OF_ANTON_HOME. When
    ...
    The returned ``son_of_anton_home`` is the path to pass to
    ``set_son_of_anton_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_son_of_anton_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``son-of-anton update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.son-of-anton)."
        )
    ...
    if clone_from is not None or clone_all or clone_config:
        if clone_from is None:
            ...
            source_dir = get_son_of_anton_home()
        else:
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.son-of-anton/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `son-of-anton -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Son of Anton profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `son-of-anton update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`son-of-anton profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `son-of-anton update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running Son of Anton (or predate schema
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SON_OF_ANTON_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``son-of-anton profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SON_OF_ANTON_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_son_of_anton_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Son of Anton *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Son of Anton backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``SON_OF_ANTON_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `son-of-anton -p <canon> profile
    ...
    # interpreter.
    _SON_OF_ANTON_CONSOLE_SCRIPT_NAMES = frozenset({"son-of-anton", "son-of-anton", "son-of-anton-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Son of Anton process: either an entrypoint marker in argv, a
            # resolved executable named `son-of-anton`, or a python interpreter
            # directly exec'ing a `son-of-anton`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_son_of_anton = (
                any(marker in joined for marker in son_of_anton_markers)
                or exe_name == "son-of-anton"
                or exe_name.startswith("son-of-anton")
            )
            if not is_son_of_anton and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: son-of-anton, son-of-anton,
                # son-of-anton-acp) rather than a bare `startswith("son-of-anton")` --
                ...
                # "son-of-anton-notes.py" or "son-of-anton-unrelated-tool" as the shim,
                ...
                is_son_of_anton = script_stem in _SON_OF_ANTON_CONSOLE_SCRIPT_NAMES
            if not is_son_of_anton:
                continue
            ...
            # ...or by SON_OF_ANTON_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("SON_OF_ANTON_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.son-of-anton).\n"
            "To remove everything, use: son-of-anton uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_son_of_anton_home
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SON_OF_ANTON_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SON_OF_ANTON_HOME")
    try:
        os.environ["SON_OF_ANTON_HOME"] = str(profile_dir)
        from son_of_anton_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SON_OF_ANTON_HOME"] = old_home
        elif "SON_OF_ANTON_HOME" in os.environ:
            del os.environ["SON_OF_ANTON_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.son-of-anton/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: son-of-anton profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SON_OF_ANTON_HOME.

    Returns ``"default"`` if SON_OF_ANTON_HOME is not set or points to ``~/.son-of-anton``.
    Returns the profile name if SON_OF_ANTON_HOME points into ``~/.son-of-anton/profiles/<name>``.
    Returns ``"custom"`` if SON_OF_ANTON_HOME is set to an unrecognized path.
    """
    from son_of_anton_constants import get_son_of_anton_home
    son_of_anton_home = get_son_of_anton_home()
    resolved = son_of_anton_home.resolve()

    default_resolved = _get_default_son_of_anton_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      SON_OF_ANTON_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Son of Anton file the
      user may have lying alongside SON_OF_ANTON_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``son-of-anton sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``SON_OF_ANTON_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.son-of-anton itself — its parent is ~/ and its
        # directory name is ".son-of-anton", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: son-of-anton profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.son-of-anton itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.son-of-anton). "
            "Specify a different name: son-of-anton profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="son_of_anton_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"son_of_anton_{old_name}"
    legacy_old_host = f"son-of-anton.{old_name}"
    new_host = f"son_of_anton_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_son_of_anton_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("son_of_anton_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SON_OF_ANTON_HOME path string.

    Called early in the CLI entry point, before any son-of-anton modules
    are imported, to set the SON_OF_ANTON_HOME environment variable.

    When SON_OF_ANTON_HOME is already set, the configured spelling IS the
