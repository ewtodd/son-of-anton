...
- 96vc: ACP didn't set RENCO_EXEC_ASK, so `check_all_command_guards`
...
1. Setting RENCO_EXEC_ASK inside _run_agent (wraps the agent call).
...
def _isolate_approval_state(monkeypatch):
    ...
    developer's real ``~/.renco/config.yaml`` command allowlist into
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set RENCO_INTERACTIVE so
    ...
    (RENCO_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When RENCO_INTERACTIVE is set and an approval callback is
        ...
        # Without RENCO_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without RENCO_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With RENCO_INTERACTIVE: callback IS called, approval flows through it
        ...
        assert called_with, (
            "with RENCO_INTERACTIVE the approval path should consult the "
