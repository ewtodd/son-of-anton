...
chat completions — use your Renco-managed provider subscription as their
...
| Use case | "Use Renco as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Renco stores
the refresh token in `~/.renco/auth.json` — the same place all Renco
...
Model:      Renco-4-70B    # or Renco-4.3-36B, Renco-4-405B
```
...
interface in `renco_cli/proxy/adapters/`.
...
If you see `not logged in`, run `renco portal`. If you see
...
`renco_cli/proxy/adapters/<provider>.py` and registering it in
