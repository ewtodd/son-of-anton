...
has to be thrown away or summarized. Renco has always done this in one batch:
...
Renco folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Renco's normal session flush is
...
With `micro_compact` unset or `false` Renco behaves exactly as it always has:
...
Defaults to `$RENCO_HOME/logs/agent.log`. It reports passes, outcome mix, net
