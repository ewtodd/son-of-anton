...
`SON_OF_ANTON_REDACT_SECRETS` at module-import time. `son_of_anton_cli/main.py` at
...
in .env), the toggle was silently ignored in both `son-of-anton chat` and
`son-of-anton gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `SON_OF_ANTON_REDACT_SECRETS`
env var in `son_of_anton_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports son_of_anton_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("SON_OF_ANTON_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import son_of_anton_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('SON_OF_ANTON_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["SON_OF_ANTON_HOME"] = str(son_of_anton_home)
    env.pop("SON_OF_ANTON_REDACT_SECRETS", None)
...
def test_redact_secrets_default_true_when_unset(tmp_path):
    ...
    `SON_OF_ANTON_REDACT_SECRETS=false`).
    """
    son_of_anton_home = tmp_path / ".son-of-anton"
    son_of_anton_home.mkdir()
    (son_of_anton_home / "config.yaml").write_text("{}\n")  # empty config
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env SON_OF_ANTON_REDACT_SECRETS takes precedence over config.yaml."""
    son_of_anton_home = tmp_path / ".son-of-anton"
    son_of_anton_home.mkdir()
    (son_of_anton_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (son_of_anton_home / ".env").write_text("SON_OF_ANTON_REDACT_SECRETS=true\n")
