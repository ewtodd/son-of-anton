...
campaign (``~/.son-of-anton/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from son_of_anton_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``son_of_anton_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from son_of_anton_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``son_of_anton_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.son-of-anton/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `son-of-anton setup` from scratch. OpenRouter
    ...
    from son_of_anton_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.son-of-anton/.env by hand.
    ...
    from son_of_anton_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from son_of_anton_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from son_of_anton_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `son-of-anton auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``son-of-anton auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from son_of_anton_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from son_of_anton_cli.auth import (
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from son_of_anton_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from son_of_anton_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from son_of_anton_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Son of Anton will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Son of Anton will install it now (the preflight below "
...
def _model_flow_copilot(config, current_model=""):
    ...
    from son_of_anton_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from son_of_anton_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from son_of_anton_cli.config import save_env_value, load_config, save_config
    from son_of_anton_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from son_of_anton_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from son_of_anton_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Son of Anton turns to `copilot --acp`.")
    print("  Son of Anton currently starts its own ACP subprocess for each request.")
    print("  Son of Anton uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set SON_OF_ANTON_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from son_of_anton_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from son_of_anton_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from son_of_anton_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `son-of-anton auth`)
    from son_of_anton_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from son_of_anton_cli.config import load_config, save_config, get_env_value
    from son_of_anton_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.son-of-anton/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``son_of_anton_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from son_of_anton_cli.main import _prompt_provider_choice
    from son_of_anton_cli.auth import ZAI_ENDPOINTS
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from son_of_anton_cli.main import _run_anthropic_oauth_flow
    from son_of_anton_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'son-of-anton model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
