...
A pack is a single YAML file (``renco-pack.yaml``) that pins a set of
...
* Capability consent is NEVER bulk-granted: after each plugin installs,
  ...
  as a normal ``renco plugins install`` (#64228).
    ...
    for entry in pack.plugins:
        ...
        try:
            if index_entries is None:
                from renco_cli.plugin_index import load_index

                index_entries, _src = load_index()
            from renco_cli.plugin_index import resolve_name
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print(
            "[dim]Install them manually, e.g. `renco skills install <id>`.[/dim]"
        )
    console.print(
...
def install_pack_plugins(
    ...
    *,
    ...
    single install (:func:`renco_cli.plugins_cmd._run_capability_consent`).
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-renco-pack") -> tuple[str, List[str]]:
    ...
    doc: dict[str, Any] = {
        "name": pack_name,
        "description": "Exported by `renco plugins pack export`.",
        "version": "1.0.0",
        "plugins": entries,
    }
    if config:
...
def cmd_pack_show(source: str) -> None:
    """``renco plugins pack show <path-or-url>`` — dry-run review."""
    ...
    console.print("\n[dim]Dry run only. Install with `renco plugins pack install ...`.[/dim]")


def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``renco plugins pack install <path-or-url>``.
...
def cmd_pack_export(*, enabled_only: bool = False, name: str = "my-renco-pack") -> None:
    """``renco plugins pack export [--enabled-only]`` — pack YAML on stdout."""
    from rich.console import Console
...
def pack_command(args) -> None:
    """Dispatch ``renco plugins pack <action>``."""
    ...
    if action == "install":
        ...
        cmd_pack_export(
            ...
            name=getattr(args, "name", None) or "my-renco-pack",
        )
    elif action == "show":
        ...
        Console().print(
            "[red]Error:[/red] Usage: renco plugins pack {install|export|show}"
        )
        sys.exit(1)
