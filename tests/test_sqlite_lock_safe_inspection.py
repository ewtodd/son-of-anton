"""POSIX advisory locks must survive Renco' own database inspection.
...
Renco used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `renco sessions
...
out across Renco' inspection calls.
"""
...
def clean_registry():
    ...
    import renco_cli.sqlite_safe_read as mod
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_renco_fail_close", False):
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import renco_cli.sqlite_safe_read as ssr
