 ...
 * packaged binary) with a real `renco serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → renco serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Renco title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Renco')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { rencoDesktop?: Record<string, unknown> }).rencoDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → renco serve is spawned → WS connects → config
