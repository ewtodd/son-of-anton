...
import { terminalBackgroundHex } from '@renco/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $RENCO_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see RENCO_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['RENCO_TUI_BACKGROUND', process.env.RENCO_TUI_BACKGROUND ?? '(unset)'],
            ['RENCO_TUI_THEME', process.env.RENCO_TUI_THEME ?? '(unset)'],
