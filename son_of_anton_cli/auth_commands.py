...
_OAUTH_CAPABLE_PROVIDERS = {"openai-codex", "xai-oauth", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `son-of-anton --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``son-of-anton auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``son-of-anton auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`son-of-anton auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Son of Anton reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `son-of-anton auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `son-of-anton auth` is called bare."""
