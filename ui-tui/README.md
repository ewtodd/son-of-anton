...
React + Ink terminal UI for Son of Anton. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SON_OF_ANTON_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `son-of-anton setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.son-of-anton/.son_of_anton_history` or under `SON_OF_ANTON_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `son-of-anton setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/son-of-anton-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external son-of-anton setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.son-of-anton/tui-parent.log
    ...
    types/
      son-of-anton-ink.d.ts            type declarations for @sonofanton/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SonOfAntonCLI subprocess for slash commands
```
