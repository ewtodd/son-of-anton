...
Usage (via cron with --no-agent):
    ...
    renco cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$RENCO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Renco-Watcher/1.0"})
