...
This page covers all commands related to [Renco profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `renco profile` without a subcommand shows help.
...
```bash
renco profile use <name>
```

Sets `<name>` as the active profile. All subsequent `renco` commands (without `-p`) will use this profile.
...
renco profile use default
```

## `renco profile create`

```bash
renco profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `renco profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `renco update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.renco`), use `renco skills opt-out` / `renco skills opt-in`. |
...
renco profile create work2 --clone-from work

# Clone everything from a specific profile
renco profile create work2-backup --clone-from work --clone-all
```

## `renco profile describe`

```bash
renco profile describe [<name>] [options]
```
...
renco profile describe researcher

# Set it explicitly
renco profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
renco profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
renco profile describe --all --auto
```

## `renco profile delete`

```bash
renco profile delete <name> [options]
```
...
```bash
renco profile delete mybot
renco profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.renco`) cannot be deleted — use `renco uninstall` to remove everything.
:::

## `renco profile show`

```bash
renco profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Renco home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ renco profile show work
Profile: work
Path:    ~/.renco/profiles/work
...
```bash
renco profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Renco installation.
...
```bash
renco profile alias work
# Creates/updates ~/.local/bin/work

renco profile alias work --name mywork
# Creates ~/.local/bin/mywork

renco profile alias work --remove
# Removes the wrapper script
```

## `renco profile rename`

```bash
renco profile rename <old-name> <new-name>
```
...
```bash
renco profile rename mybot assistant
# ~/.renco/profiles/mybot → ~/.renco/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `renco profile export`

```bash
renco profile export <name> [options]
```
...
```bash
renco profile export work
# Creates work.tar.gz in the current directory

renco profile export work -o ./work-2026-03-29.tar.gz
```
...
## `renco profile import`

```bash
renco profile import <archive> [options]
```
...
```bash
renco profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

renco profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `renco -p telemetry`). |
...
renco profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
renco profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
renco profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
renco profile install ./telemetry/
```

### `renco profile update`

```bash
renco profile update <name> [--force-config] [--yes]
```
...
```bash
renco profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Renco version, author, env var requirements, the source URL/path, and
...
`renco profile list` also shows the distribution name and version in a
`Distribution` column, and `renco profile show <name>` / `delete <name>`
...
renco profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
renco profile install https://github.com/your-org/internal-assistant.git
```
...
`renco_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Renco version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Renco can clone from.
3. Tell recipients to run `renco profile install <your-repo-url>`.
...
## `renco -p` / `renco --profile`

```bash
renco -p <name> <command> [options]
renco --profile <name> <command> [options]
```

Global flag to run any Renco command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
renco -p work chat -q "Check the server status"
renco --profile dev gateway start
renco -p personal skills list
renco -p work config edit
```

## `renco completion`

```bash
renco completion <shell>
```
...
- `renco profile <TAB>` — subcommands (list, use, create, etc.)
- `renco profile use <TAB>` — profile names
- `renco -p <TAB>` — profile names
