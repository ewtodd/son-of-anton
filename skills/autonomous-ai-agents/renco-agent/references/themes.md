# Themes / Skins — Author a Renco Color Theme

Author a Renco **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`renco_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.renco/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Renco itself.
...
- Write access to the Renco home dir — `~/.renco` by default, or `$RENCO_HOME`
  / the active profile's dir. Skins live in `<renco-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `renco config set`).
...
3. `write_file` it to `<renco-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `renco config set display.skin default` (or they can `/skin default`).
...
```
renco skin set <key> <hex>      # e.g. renco skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `renco skin set background "#08201f"` changes
only the background; `renco skin use <name>` / `renco skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.renco`** when a profile is active — resolve the real home
  from `$RENCO_HOME` first, falling back to `~/.renco`.
...
- **Never hand-edit `config.yaml` to activate.** Use `renco config set
...
- **You apply it, not the user.** `renco config set display.skin <name>` is
...
- `read_file` the written `<renco-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `renco config get display.skin` and confirm it reports `<name>`.
