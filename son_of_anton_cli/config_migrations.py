...
``son_of_anton_cli.config.migrate_config``. Each step is a function
...
``son_of_anton_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``son_of_anton_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``son_of_anton_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``son_of_anton_cli.config`` (e.g.
``patch("son_of_anton_cli.config.read_raw_config", ...)``) keep working, because
...
#: SON_OF_ANTON_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def _cfg():
    """Return the live ``son_of_anton_cli.config`` module (lazy, cycle-free)."""
    from son_of_anton_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$SON_OF_ANTON_HOME/plugins/`` for currently installed user plugins.
        grandfathered: List[str] = []
        try:
            user_plugins_dir = get_son_of_anton_home() / "plugins"
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `son-of-anton plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    #   3. Creates `~/.son-of-anton/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_son_of_anton_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_son_of_anton_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `son-of-anton config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `son-of-anton config set`"
                )
...
# behavior set it to true explicitly via `son-of-anton config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from son_of_anton_cli.relay_plugin_cutover import legacy_relay_plugin_keys
    ...
    message = (
        ...
        "SON_OF_ANTON_NEMO_RELAY_PLUGINS_TOML."
    )
