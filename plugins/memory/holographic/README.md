...
```bash
son-of-anton memory setup    # select "holographic"
```

Or manually:
```bash
son-of-anton config set memory.provider holographic
```
...
|-----|---------|-------------|
| `db_path` | `$SON_OF_ANTON_HOME/memory_store.db` | SQLite database path |
