...
subparser, placing the flag BEFORE the subcommand (e.g. 'son-of-anton --yolo chat')
...
def _build_parser():
    """Build the son-of-anton argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("son_of_anton_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "son_of_anton_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets SON_OF_ANTON_YOLO_MODE regardless of flag position.
    ...
    def test_yolo_before_chat_sets_env(self):
        ...
        assert os.environ.get("SON_OF_ANTON_YOLO_MODE") == "1"
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `son-of-anton gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: son_of_anton_cli.main is a very heavy
    # import (previously 11 separate `python -m son_of_anton_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `son-of-anton <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `son-of-anton -t web chat` silently dropped the
    ...
    Uses the real `son_of_anton_cli._parser.build_top_level_parser()` rather than
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `son-of-anton <flag> <value> chat`:\n  "
