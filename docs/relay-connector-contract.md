...
This document is the formal interface between the **Renco gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `renco gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`renco gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `renco` CLI.
