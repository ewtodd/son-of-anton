...
1. **User-local:** `~/.son-of-anton/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the son-of-anton repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with son-of-anton
...
- Don't use for: personal skills in `~/.son-of-anton/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `son-of-anton skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Son of Anton Agent
license: MIT
...
- Credit the **human first**, then "Son of Anton Agent" as secondary collaborator: `Ben Barclay (benbarclay), Son of Anton Agent`.
- Never `author: Son of Anton Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Son of Anton Agent`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.son-of-anton/skills/`.
...
|---|---|
| Son of Anton tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.son-of-anton/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Son of Anton Agent` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.son-of-anton.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through Son of Anton tools
