"""
Profile management for multiple isolated Renco instances.

Each profile is a fully independent RENCO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.renco/profiles/<name>/`` by default.

The "default" profile is ``~/.renco`` itself — backward compatible,
zero migration needed.

Usage::

    renco profile create coder          # fresh profile + bundled skills
    renco profile create coder --clone  # also copy config, .env, SOUL.md, skills
    renco profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    renco -p coder chat                 # or via flag
    renco profile use coder             # set as sticky default
    renco profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See renco_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `renco profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `renco
...
# `renco skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Renco infrastructure directories that only the default profile
         (``~/.renco``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_renco_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.renco) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "renco-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when RENCO_HOME equals the
...
# into the export. The set below identifies the *known Renco profile
# artifacts* at the root of RENCO_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``renco_constants``.
...
_RESERVED_NAMES = frozenset({
    "renco", "default", "test", "tmp", "root", "sudo",
})

# Renco subcommands that cannot be used as profile names/aliases
_RENCO_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the renco root, NOT to the current RENCO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where RENCO_HOME points outside
    ``~/.renco``, profiles live under ``RENCO_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_renco_home() / "profiles"


def _get_default_renco_home() -> Path:
    """Return the default (pre-profile) RENCO_HOME path.

    In standard deployments this is ``~/.renco``.
    In Docker/custom deployments where RENCO_HOME is outside ``~/.renco``
    (e.g. ``/opt/data``), returns RENCO_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_renco_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Renco installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its RENCO_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_renco_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, renco subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _RENCO_SUBCOMMANDS:
        return f"'{canon}' conflicts with a renco subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            renco_exe = shutil.which("renco") or "renco"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(renco_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Renco. If we
    ...
    try:
        ...
        # not be migrated. The next `renco doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``renco -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``renco profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``renco -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``renco profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``RENCO_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Renco config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_renco_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, renco_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own RENCO_HOME. When
    ...
    The returned ``renco_home`` is the path to pass to
    ``set_renco_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_renco_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``renco update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.renco)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.renco/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `renco -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Renco profile.\n"
    ...
    if not soul_path.exists():
        try:
            from renco_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `renco update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`renco profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Renco (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `renco profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `renco -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches RENCO_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``renco profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "RENCO_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_renco_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Renco *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Renco backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``RENCO_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `renco -p <canon> profile
    ...
    renco_markers = ("renco_cli.main", "renco-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('renco')` resolution) spawns the
    ...
    # interpreter, not "renco", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _RENCO_CONSOLE_SCRIPT_NAMES = frozenset({"renco", "renco-agent", "renco-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Renco process: either an entrypoint marker in argv, a
            # resolved executable named `renco`, or a python interpreter
            # directly exec'ing a `renco`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_renco = (
                any(marker in joined for marker in renco_markers)
                or exe_name == "renco"
                or exe_name.startswith("renco")
            )
            if not is_renco and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: renco, renco-agent,
                # renco-acp) rather than a bare `startswith("renco")` --
                ...
                # "renco-notes.py" or "renco-unrelated-tool" as the shim,
                ...
                is_renco = script_stem in _RENCO_CONSOLE_SCRIPT_NAMES
            if not is_renco:
                continue
            ...
            # ...or by RENCO_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("RENCO_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.renco).\n"
            "To remove everything, use: renco uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_renco_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``RENCO_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from renco_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set RENCO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("RENCO_HOME")
    try:
        os.environ["RENCO_HOME"] = str(profile_dir)
        from renco_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["RENCO_HOME"] = old_home
        elif "RENCO_HOME" in os.environ:
            del os.environ["RENCO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.renco/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: renco profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from RENCO_HOME.

    Returns ``"default"`` if RENCO_HOME is not set or points to ``~/.renco``.
    Returns the profile name if RENCO_HOME points into ``~/.renco/profiles/<name>``.
    Returns ``"custom"`` if RENCO_HOME is set to an unrecognized path.
    """
    from renco_constants import get_renco_home
    renco_home = get_renco_home()
    resolved = renco_home.resolve()

    default_resolved = _get_default_renco_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      RENCO_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Renco file the
      user may have lying alongside RENCO_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``renco sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``RENCO_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.renco itself — its parent is ~/ and its
        # directory name is ".renco", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: renco profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.renco itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.renco). "
            "Specify a different name: renco profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="renco_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"renco_{old_name}"
    legacy_old_host = f"renco.{old_name}"
    new_host = f"renco_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_renco_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("renco_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a RENCO_HOME path string.

    Called early in the CLI entry point, before any renco modules
    are imported, to set the RENCO_HOME environment variable.

    When RENCO_HOME is already set, the configured spelling IS the
