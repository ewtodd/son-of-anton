...
- Job store (cron/jobs.py): the field is validated at the storage choke
  point against the canonical Son of Anton effort grammar (parse_reasoning_effort
  in son_of_anton_constants — the SAME parser every other effort surface uses).
...
class TestCronjobToolReasoningEffort:
    ...
    via `son-of-anton cron create/edit --reasoning-effort` only."""
    ...
    def test_schema_does_not_expose_reasoning_effort(self):
        ...
        keeps the parameter for the CLI lane (son_of_anton_cli/cron.py), but the
