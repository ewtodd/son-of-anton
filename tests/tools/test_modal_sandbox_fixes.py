...
try:
    ...
    pytest.skip("son-of-anton tools not importable (missing deps)", allow_module_level=True)
...
class TestCwdHandling:
    ...
    def test_home_path_replaced_for_modal(self, monkeypatch):
        ...
        monkeypatch.setenv("TERMINAL_CWD", "/home/dakota/github/son-of-anton")
...
class TestDockerHostBindApproval:
    ...
    def _isolate_approval_state(monkeypatch):
        ...
        hermetic SON_OF_ANTON_HOME fixture runs), so on a dev machine whose real
        ...
        SON_OF_ANTON_YOLO_MODE off the environment when the module is imported at
        collection time, before conftest's per-test env blanking runs. A test
        run launched from a --yolo Son of Anton session (or any shell exporting
        SON_OF_ANTON_YOLO_MODE=1) freezes True and every guard auto-approves.
