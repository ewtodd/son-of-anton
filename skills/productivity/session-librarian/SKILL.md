...
| List/filter by metadata (age, source, cost, tokens, workspace) | `son-of-anton sessions list` / `stats` via terminal |
| Rename | `son-of-anton sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `son-of-anton sessions archive <filters>` |
| Delete (destructive) | `son-of-anton sessions delete` / `son-of-anton sessions prune <filters>` |
| Export before deleting anything valuable | `son-of-anton sessions export --session-id <id> --format md` |
...
`son-of-anton sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `son-of-anton sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `son-of-anton sessions list`
