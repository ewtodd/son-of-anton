...
The ``renco update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m renco_cli.main gateway run`` — image name
    ``pythonw``, not ``renco.exe`` — so the ``taskkill /IM renco.exe`` guard
    ...
    # The renco.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM renco.exe' in text
