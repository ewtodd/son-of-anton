...
1. The venv-repair import probes (early recovery + `son-of-anton update`) must
...
2. ``son-of-anton doctor`` must fail the certificate check in that state, and
   ``son-of-anton doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `son-of-anton update`'s venv repair must apply
...
# =========================================================================
# 2. son-of-anton doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "son-of-anton doctor --fix" in message
