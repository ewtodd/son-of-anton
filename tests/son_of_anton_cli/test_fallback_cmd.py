"""Tests for `son-of-anton fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate SON_OF_ANTON_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".son-of-anton" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from son_of_anton_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nous", "model": "Son of Anton-4-Llama-3.1-405B"},
            ]
        }
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from son_of_anton_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        from son_of_anton_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
        out = capsys.readouterr().out
        assert "No fallback providers configured" in out
        assert "son-of-anton fallback add" in out
    ...
    def test_list_with_entries(self, isolated_home, capsys):
        _write_config(isolated_home, {
            ...
            "fallback_providers": [
                {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
                {"provider": "nous", "model": "Son of Anton-4"},
            ],
        })
        from son_of_anton_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("son_of_anton_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("son_of_anton_cli.main._require_tty"):
            from son_of_anton_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nous / Son of Anton-4")
        with patch("son_of_anton_cli.setup._curses_prompt_choice", return_value=1):
            from son_of_anton_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from son_of_anton_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `son-of-anton fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "son_of_anton_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
