"""``son-of-anton import-agent`` subcommand parser.

Follows the ``son-of-anton claw`` pattern (see ``son_of_anton_cli/subcommands/claw.py``):
parser building lives here, the handler is injected to avoid importing
``main``, and the import logic itself lives in ``son_of_anton_cli/agent_import.py``.
"""
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into Son of Anton",
        description=(
            "One-command import of another coding agent's setup into Son of Anton. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Son of Anton equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'son-of-anton setup' for those."
        ),
    )
    parser.add_argument(
        "agent",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing Son of Anton items on name conflicts (default: skip)",
    )
    parser.add_argument(
