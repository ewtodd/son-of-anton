...
```
Son of Anton Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${SON_OF_ANTON_HOME:-$HOME/.son-of-anton}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Son of Anton config (if missing)
...
3. **Restart Son of Anton session** to pick up the new MCP server
