"""Dependency-light venv recovery that runs BEFORE renco_cli.main's imports.

The ``renco`` console entry point is ``renco_cli.main:main``.  Importing
``renco_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``renco_cli.env_loader``, ``yaml`` via ``renco_cli.config``, ...).  In
...
a corrupted venv.  ``renco_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``renco_cli.main`` to become
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Renco already vendors
    (``~/.renco/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".renco" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``renco acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``renco_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``renco_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``renco update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `renco update`: CLI and Desktop retries preserve
...
# every `renco` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``renco update`` (or the self-lock preflight, #83569) left the
    ...
    ``renco_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``renco acp`` included).
    ...
    """
    try:
        from renco_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; renco will keep working from "
