...
late-binding seam in :mod:`renco_cli.web_deps` so tests that
...
from renco_cli.web_deps import late
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    the repo-scan policy, the RENCO_HOME junk filters — through the
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's RENCO_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``renco profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from RENCO_HOME).
    """
    from renco_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``renco profile use``).
...
async def update_profile_soul(name: str, body: ProfileSoulUpdate):
    ...
    try:
        ...
        # profiles seed SOUL.md at the umask default (renco_cli.profiles
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the RENCO_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``renco profile describe
    <name> --auto``.
    ...
    try:
        from renco_cli import profile_describer
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps renco_cli.profiles.export_profile /
# import_profile (the same machinery behind `renco profile export|import`).
...
async def export_profile_endpoint(name: str, body: ProfileExport):
    ...
    if not output:
        ...
        staging = get_renco_home() / "profile-exports"
        try:
