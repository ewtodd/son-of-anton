#!/usr/bin/env python3
"""Read which OS window sits directly underneath the Renco desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the Renco window as a JSON string."""
    if callback is None:
        return tool_error(
            "read_window_below is only available in the Renco desktop app."
        )
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        ...
        "Renco desktop window — what the user is working in behind this app. "
        ...
        "permission — never prompted for, noted in `note`). Other Renco "
        "windows are skipped: the nearest non-Renco window is reported. "
