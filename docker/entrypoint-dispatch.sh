...
if [ "$$" -eq 1 ]; then
    exec /init /opt/renco/docker/main-wrapper.sh "$@"
fi

echo "[renco] WARNING: container entrypoint is not PID 1; skipping s6-overlay /init and falling back to direct bootstrap. Supervised services are unavailable in this runtime, but the requested command will still run." >&2
...
/opt/renco/docker/stage2-hook.sh
exec /opt/renco/docker/main-wrapper.sh "$@"
