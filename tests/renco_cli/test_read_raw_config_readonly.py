...
@pytest.fixture()
def isolated_renco_home():
    """Per-test RENCO_HOME dir (already redirected by the autouse conftest
...
def test_freshness_after_config_edit(isolated_renco_home):
    from renco_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_renco_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_renco_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_renco_home):
    from renco_cli.config import read_raw_config_readonly

    cfg = isolated_renco_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
