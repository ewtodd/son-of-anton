...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<renco_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active RENCO_HOME."""
    from renco_constants import get_renco_home

    flush_dir = get_renco_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<renco_home>/pending_messages/``), so a runtime cap rotation no
