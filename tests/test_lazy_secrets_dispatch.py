...
1. `son-of-anton secrets bitwarden setup --help` works (dispatch path)
2. `son-of-anton update --check` works (update path)
3. `son-of-anton secrets bitwarden disable` works (handler execution)
4. `son-of-anton secrets onepassword status` works (lazy backend loads on demand)
...
def _run_son_of_anton(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run son-of-anton CLI as a subprocess from repo root.
    ...
    connectivity — but ``son-of-anton update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-son-of-anton-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-son-of-anton-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_1": "git@",
        }
    )
    return subprocess.run(
        [sys.executable, "-m", "son_of_anton_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_setup_help(self) -> None:
        """`son-of-anton secrets bitwarden setup --help` must exit 0 and print usage.

        This is the exact path that triggered the #86781 self-lock loop on
        Windows: setup/parser nested under lazy-loaded backend.
        """
        result = _run_son_of_anton(["secrets", "bitwarden", "setup", "--help"])
    ...
    def test_bitwarden_status(self) -> None:
        """`son-of-anton secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_son_of_anton(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`son-of-anton secrets bitwarden disable` must exit 0."""
        result = _run_son_of_anton(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`son-of-anton secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_son_of_anton(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`son-of-anton secrets onepassword setup --help` must exit 0."""
        result = _run_son_of_anton(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `son-of-anton update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`son-of-anton update --check` must not load cryptography._rust."""
        result = _run_son_of_anton(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['son-of-anton','update','--check']
...
sys.argv = ['son-of-anton', 'update', '--check']

import son_of_anton_cli.main as m
...
# resolves the son_of_anton_cli.main module's lazily re-exported attribute —
# so the patch must land on son_of_anton_cli.main._cmd_update_check.
with patch('son_of_anton_cli.main._cmd_update_check', capture_update_check):
    try:
        m.main()
