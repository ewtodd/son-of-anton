...
_NOUS_BUNDLE_FORMAT = "son-of-anton-debug-share/1"
...
def build_nous_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    ...
    The JSON shape is what the discord-support viewer (Repo 3) parses::
        ...
        {"format": "son-of-anton-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``son-of-anton debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "son-of-anton debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `son-of-anton debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  son-of-anton debug delete <url>")

    print("\nShare these links with the Son of Anton team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Son of Anton version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``son-of-anton debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from son_of_anton_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "son-of-anton debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `son-of-anton debug share --local` to print the report instead, "
            "or `son-of-anton debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
