#!/usr/bin/env python3
"""
SQLite State Store for Renco Agent.
...
from renco_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from renco_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from renco_state_portability import SessionPortabilityMixin
from renco_state_schema import SessionSchemaMixin
from renco_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``renco -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_renco_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.renco`` even when a test fixture later
    redirects ``RENCO_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_renco_home()`` fresh so a runtime
       ``RENCO_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_renco_home() / "state.db"
...
# ~/.renco/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects RENCO_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without RENCO_HOME, a stale worktree
# without the re-pin, or a developer shell that exports RENCO_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "RENCO_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.renco``).  The test conftest injects the pre-sandbox production
#: root here so custom-``RENCO_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Renco root for the guard.

    Deliberately avoids ``Path.home()`` / ``renco_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``renco_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "renco"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "renco"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".renco"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``RENCO_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Renco test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``RENCO_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "RENCO_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``RENCO_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``renco`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Renco home *root*.
    ...
    under ``~/.renco/renco-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``RENCO_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``RENCO_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Renco root {root}). "
                "Tests must run against a temporary RENCO_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "RENCO_HOME. If this test genuinely needs the live "
...
# renco_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``renco sessions optimize`` (FTS merge + VACUUM)
    ...
    ``renco_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``renco_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``renco update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Renco-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Renco"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Renco-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `renco doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see renco_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with renco_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``renco doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Renco host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``renco serve`` backend, interactive CLI sessions,
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``renco_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Renco home tree (``get_renco_home()``) — the safe repair scope:
      Renco owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``renco_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_renco_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Renco needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (renco_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_renco_state.py:600-632)
        ...
        probe_session_id = f"_renco_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.renco/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``renco sessions optimize-storage`` on a capable host).
#
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship renco_state without renco_cli). A
    ...
    try:
        ...
        logger.debug(
            "renco_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch renco_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``renco_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `renco snapshot list` / "
                "`renco snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (renco doctor, dashboards) ──────────
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple renco processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Renco
    ...
    # `renco update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `renco snapshot list` / "
                    f"`renco snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`renco db optimize`),
            ...
            # ``renco_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `renco update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `renco sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Renco process held the "
    ...
    #
    # `optimize_fts_storage()` (the `renco sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`renco sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``renco sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``renco --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: renco_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``renco -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``renco sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``RENCO_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Renco, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Renco session.

        A Renco session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Renco session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `renco sessions vacuum` runs in a transient
