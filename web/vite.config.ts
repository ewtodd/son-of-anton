...
const BACKEND = process.env.RENCO_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `renco dashboard` server injects a one-shot
 * session token into `index.html` (see `renco_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__RENCO_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function rencoDevToken(): Plugin {
  const TOKEN_RE = /window\.__RENCO_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__RENCO_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "renco:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[renco] Could not find session token in ${BACKEND} — ` +
              `is \`renco dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__RENCO_SESSION_TOKEN__="${match[1]}";` +
              `window.__RENCO_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[renco] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`renco dashboard\` or set RENCO_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@renco/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `renco dashboard` must serve these; Vite has no
