"""Lightweight internationalization (i18n) for Son of Anton static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Son of Anton itself -- approval prompts, a handful of gateway slash
...
def _locales_dir() -> Path:
    ...
    1. ``SON_OF_ANTON_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("SON_OF_ANTON_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "SON_OF_ANTON_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def reset_language_cache() -> None:
    ...
    Call after :func:`son_of_anton_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("SON_OF_ANTON_LANGUAGE")
    if env_lang:
