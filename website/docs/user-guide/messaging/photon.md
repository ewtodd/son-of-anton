...
Connect Renco to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Renco — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Renco runs it in a
...
renco photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Renco Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.renco/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.renco/.env`
...
(device token, dashboard project id) lives in `~/.renco/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Renco
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Renco replies with a pairing code. Approve it with:

```bash
renco pairing approve photon <CODE>
```

Use `renco pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.renco/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.renco/.env`):
...
By default Renco responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Renco` and
`@Renco agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Renco will reply.
...
assigned iMessage line Renco uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run renco photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `renco photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `renco photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `renco photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Renco sends images, voice
  ...
  media.
- **Native polls are supported.** Renco sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Renco sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `renco send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<renco-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Renco is set
...
| `PHOTON_MENTION_PATTERNS` | Renco wake words  | JSON list / comma / newline regex patterns for group mentions |
