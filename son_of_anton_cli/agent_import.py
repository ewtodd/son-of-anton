"""son-of-anton import-agent — import Claude Code / Codex CLI setups into Son of Anton.

Usage:
    son-of-anton import-agent                       # auto-detect ~/.claude or ~/.codex
    son-of-anton import-agent claude-code           # import from ~/.claude
    son-of-anton import-agent codex                 # import from ~/.codex
    son-of-anton import-agent claude-code --dry-run # preview only, no changes
    son-of-anton import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``son-of-anton claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_son_of_anton.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in SON_OF_ANTON_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → SON_OF_ANTON_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in SON_OF_ANTON_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in SON_OF_ANTON_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → SON_OF_ANTON_HOME/skills/codex-imports/<name>/
...
deliberately via ``son-of-anton setup`` or config.yaml.
"""
...
# Same entry delimiter as the Son of Anton memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `son-of-anton config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`son-of-anton config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_son_of_anton.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Son of Anton command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Son of Anton glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Son of Anton equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if name in existing and not self.overwrite:
                self.record(kind, name, f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Son of Anton config")
                continue

            son_of_anton_srv: Dict[str, Any] = {}
            if srv.get("command"):
                son_of_anton_srv["command"] = srv["command"]
                if srv.get("args"):
                    son_of_anton_srv["args"] = srv["args"]
                env_kept, env_stripped = sanitize_mcp_env(srv.get("env"))
                if env_kept:
                    son_of_anton_srv["env"] = env_kept
                if env_stripped:
                    self.stripped_secrets.extend(
                        f"mcp_servers.{name}.env.{k}" for k in env_stripped
                    )
                if srv.get("cwd"):
                    son_of_anton_srv["cwd"] = srv["cwd"]
            if srv.get("url"):
                son_of_anton_srv["url"] = srv["url"]
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → SON_OF_ANTON_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``son-of-anton import-agent`` (invoked from son_of_anton_cli.main)."""
    from son_of_anton_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: son-of-anton import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: son-of-anton import-agent claude-code   or   son-of-anton import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚛ Son of Anton — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: son-of-anton import-agent "
    ...
    print_info(f"Target:      {son_of_anton_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'son-of-anton setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: son-of-anton import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'son-of-anton setup' "
               "to configure providers, or add them to ~/.son-of-anton/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from son_of_anton_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'son-of-anton setup' or ~/.son-of-anton/.env.")
        print()
