...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``renco logs``.
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # Renco only initiates compaction in "renco" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Renco preflight compression for codex app-server "
                "(mode=%s); Renco will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Renco (via _emit_status), so it always
