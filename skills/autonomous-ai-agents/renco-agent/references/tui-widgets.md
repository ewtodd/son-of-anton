...
Author widget apps for the Renco TUI (`renco --tui`): glanceable ambient
...
- The TUI must be in use (`renco --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.renco/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.renco/logs/tui_gateway_crash.log` if a widget never appears.
