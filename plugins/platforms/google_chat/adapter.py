...
# ``model_tools`` import time, so ``son-of-anton status``, ``son-of-anton chat``, etc.
...
# ``son_of_anton_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Son of Anton is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a son-of-anton MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "son-of-anton-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SON_OF_ANTON_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _son_of_anton_home = _get_son_of_anton_home()
        except (ModuleNotFoundError, ImportError):
            _son_of_anton_home = _Path.home() / ".son-of-anton"
        self._thread_count_store = _ThreadCountStore(
            _son_of_anton_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Son of Anton is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SON_OF_ANTON_HOME", str(_Path.home() / ".son-of-anton"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SON_OF_ANTON_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a son-of-anton MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Son of Anton is thinking…' marker message.
        ...
        "Son of Anton is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Son of Anton is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Son of Anton is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Son of Anton is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``son-of-anton setup``.

    The setup wizard at ``son_of_anton_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.son-of-anton/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. son-of-anton-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.son-of-anton/.env")
    print_info("Restart the gateway: son-of-anton gateway restart")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `son-of-anton setup` to install Google Chat support.",
        ...
        # the "Son of Anton is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Son of Anton is thinking…' marker message "
