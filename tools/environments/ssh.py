...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "renco-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.renco"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.renco directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.renco"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="renco-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .renco/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths (e.g. home/user/.renco/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.renco".lstrip("/")
        ssh_cmd = self._build_ssh_command()
