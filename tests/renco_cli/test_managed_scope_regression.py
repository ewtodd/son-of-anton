...
def renco_home(tmp_path, monkeypatch):
    ...
    # /etc/renco on the dev/CI box can't influence the test.
    monkeypatch.setenv("RENCO_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(renco_home, monkeypatch):
...
def test_env_expansion_in_user_config(renco_home, monkeypatch):
    from renco_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_renco_dotenv(renco_home=str(home))
