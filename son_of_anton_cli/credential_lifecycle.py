"""Unified provider-credential lifecycle across every store Son of Anton reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.son-of-anton/.env``                     — the canonical secret store
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `son-of-anton auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``son-of-anton auth add``.
    """
    from son_of_anton_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from son_of_anton_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from son_of_anton_cli.config import load_env, remove_env_value
