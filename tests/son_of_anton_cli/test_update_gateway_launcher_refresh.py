...
2. ``son_of_anton_cli.main._refresh_windows_gateway_launchers`` — ``son-of-anton
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "son_of_anton_cli.main", "gateway", "run"]
    with mock.patch.object(
        gateway_windows, "_stable_gateway_working_dir", return_value=str(tmp_path)
    ), mock.patch("son_of_anton_cli.config.get_son_of_anton_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: son-of-anton update regenerates launchers
# ---------------------------------------------------------------------------
