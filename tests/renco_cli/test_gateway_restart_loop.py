...
Covers:
- Defense 1: gateway stop/restart refuse when _RENCO_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "renco gateway stop",
        "renco  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "RENCO GATEWAY RESTART",           # uppercase
    ])
    def test_renco_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.renco.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.renco/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l renco-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.renco.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.renco.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.renco/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated renco note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "renco cron list",
        "renco update",
        "renco config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `renco gateway start` is benign — starting a gateway from inside a
        ...
        "renco gateway start",
        "renco gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway renco
        # services must not be falsely blocked (the old `.*renco` matched any
        # renco token).
        "launchctl unload ai.renco.update-checker.plist",
        "launchctl restart ai.renco.daemon",
        ...
        "systemctl restart renco-meta.service",
        "systemctl restart renco-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_renco_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade renco then run renco gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_renco_gateway_once.sh). The script must live under
        # RENCO_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("RENCO_HOME", str(tmp_path / ".renco"))
        scripts_dir = tmp_path / ".renco" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify renco gateway stop/restart refuse when _RENCO_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_RENCO_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_RENCO_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _RENCO_GATEWAY=1.

    Issue #37453: systemctl --user restart renco-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart renco-gateway",
        "systemctl --user restart renco-gateway",
        "systemctl stop renco-gateway.service",
        "renco gateway restart",
        "launchctl kickstart gui/501/ai.renco.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.renco.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.renco/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.renco.gateway.restart-once.plist",
        "pkill -f renco.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart renco-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.renco.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-renco label: label-independent detection is the point
        # (#62891 second reproduction used `ai.renco.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.renco.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.renco.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.renco.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-renco systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run renco gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("renco gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xferenco gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under RENCO_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.renco.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("renco gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/renco\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 renco gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_renco_home()` falls back to Path.home(), which raises
        RuntimeError when neither RENCO_HOME nor HOME resolves
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    renco_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run renco gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("RENCO_HOME", str(tmp_path / ".renco"))
        (tmp_path / ".renco").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run renco gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell renco gateway restart"',
        'psql -c "\\! systemctl restart renco-gateway"',
        "grep 'systemctl restart renco-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart renco-gateway",
        'grep "$(systemctl restart renco-gateway)" f',
        "grep 'restart' log; systemctl restart renco-gateway",
        'sqlite3 db "SELECT 1"; renco gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "renco gateway restart",
        "sudo systemctl stop renco-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart renco-gateway%'\""
        )
