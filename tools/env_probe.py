...
machine as Son of Anton itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Son of Anton ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike Son of Anton's own uv call sites: this
    ...
    # local.py puts the Son of Anton-managed $SON_OF_ANTON_HOME/bin there, so a managed-only
