...
CODEX_OAUTH_USER_AGENT = f"son-of-anton-cli/{_SON_OF_ANTON_CLI_VERSION}"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop Son of Anton finding a setup-token
    ...
    "opencode-free": ProviderConfig(
        ...
        # secret to configure. Select via `son-of-anton model` / `/model free`.
        api_key_env_vars=(),
    ),
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from son_of_anton_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``son-of-anton auth``.
    """
    return (
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("SON_OF_ANTON_OAUTH_TRACE", "").strip().lower()
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``son-of-anton auth add`` OAuth paths that create credential-pool
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `son-of-anton logout`.
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``SON_OF_ANTON_PORTAL_BASE_URL`` /
    ``NOUS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Son of Anton at a non-production Nous Portal (e.g. a hosted agent
    provisioned on nous-account-service's `staging` environment, which stamps
    ``SON_OF_ANTON_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("SON_OF_ANTON_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: son-of-anton auth add nous",
        provider="nous",
        code=reason,
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("SON_OF_ANTON_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `son-of-anton model` walks users into a broken Qwen setup flow.
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("SON_OF_ANTON_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set SON_OF_ANTON_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("SON_OF_ANTON_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("SON_OF_ANTON_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("SON_OF_ANTON_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `son-of-anton auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `son-of-anton auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `son-of-anton auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `son-of-anton auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.son-of-anton/.env, and return it.
    ...
    print("       App name:     anything (e.g. son-of-anton)")
    ...
    # Persist so subsequent `son-of-anton auth spotify` runs skip the wizard.
    save_env_value("SON_OF_ANTON_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("SON_OF_ANTON_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved SON_OF_ANTON_SPOTIFY_CLIENT_ID to ~/.son-of-anton/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "SON_OF_ANTON_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Son of Anton:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Son of Anton is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.son-of-anton/auth.json (not ~/.codex/)
#
# Son of Anton maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Son of Anton auth store (~/.son-of-anton/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `son-of-anton auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `son-of-anton auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `son-of-anton auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `son-of-anton auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``son-of-anton setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``son-of-anton auth add openai-codex``
      ...
      ``son-of-anton auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Son of Anton auth store (~/.son-of-anton/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``son-of-anton auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Son of Anton auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Son of Anton auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run son-of-anton auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `son-of-anton auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Son of Anton auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Son of Anton keeps its OWN
        ...
        # the Codex CLI (or another Son of Anton process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Son of Anton's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("SON_OF_ANTON_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("SON_OF_ANTON_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Son of Anton processes
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Son of Anton persists a Codex 429's ``reset_at`` locally and freezes the
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("SON_OF_ANTON_CA_BUNDLE")
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``SON_OF_ANTON_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    ...
    """
    override = os.getenv("SON_OF_ANTON_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from son_of_anton_constants import get_default_son_of_anton_root
    return get_default_son_of_anton_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Son of Anton root, refuse rather than silently corrupt cross-profile
    # state. Tests must set SON_OF_ANTON_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from son_of_anton_constants import get_default_son_of_anton_root
        real_home_shared = (
            get_default_son_of_anton_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set SON_OF_ANTON_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No SON_OF_ANTON_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Son of Anton
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Son of Anton models — they're not reliable for agentic tool-calling
            if "son-of-anton" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "Son of Anton is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )

        # SON_OF_ANTON_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "son-of-anton-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``son-of-anton auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``son-of-anton auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `son-of-anton auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("Son of Anton is not logged into Nous Portal.",
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("SON_OF_ANTON_PORTAL_BASE_URL")
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `son-of-anton auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `son-of-anton auth` and
    # `son-of-anton model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``son-of-anton doctor`` runs the live
    ...
    try:
        from son_of_anton_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Son of Anton' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `son-of-anton doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def login_command(args) -> None:
    """Deprecated: use 'son-of-anton model' or 'son-of-anton setup' instead."""
    print("The 'son-of-anton login' command has been removed.")
    print("Use 'son-of-anton auth' to manage credentials,")
    print("'son-of-anton model' to select a provider, or 'son-of-anton setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.son-of-anton/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Son of Anton-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Son of Anton auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Son of Anton will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("SON_OF_ANTON_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Son of Anton will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Son of Anton gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Son of Anton creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Son of Anton auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Son of Anton auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Son of Anton creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``son-of-anton auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``son-of-anton auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Son of Anton auth store (~/.son-of-anton/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Son of Anton login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `son-of-anton model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``son-of-anton status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Son of Anton login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `son-of-anton model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``SON_OF_ANTON_PORTAL_BASE_URL`` set
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Son of Anton builds still see
        ...
        if not selected_model:
            ...
            print("  Run `son-of-anton model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Son of Anton will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `son-of-anton model` or configure an API key to use Son of Anton.")
        else:
