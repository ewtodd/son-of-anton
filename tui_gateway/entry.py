...
# from shadowing Son of Anton's own top-level modules.  ``son_of_anton_bootstrap`` lives at
...
import son_of_anton_bootstrap

son_of_anton_bootstrap.harden_import_path()
...
# shared owner in son_of_anton_cli.mcp_startup — the wait/in-flight/join helpers
...
# son_of_anton_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `SON_OF_ANTON_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("SON_OF_ANTON_TUI_SIDECAR_URL")
...
# tested; override via ``SON_OF_ANTON_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("SON_OF_ANTON_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``SON_OF_ANTON_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``son-of-anton --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``son_of_anton_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from son_of_anton_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → son_of_anton_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's SON_OF_ANTON_HOME first), so a launch profile with no
    ...
    try:
        from son_of_anton_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``son-of-anton --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``son-of-anton dashboard`` spawn theirs via
    ``son_of_anton_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``son_of_anton_cli.mcp_startup`` thread used by the
    ...
    try:
        from son_of_anton_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from son_of_anton_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's SON_OF_ANTON_HOME override — the shared owner in
    ``son_of_anton_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins Son of Anton activates mid-conversation.
