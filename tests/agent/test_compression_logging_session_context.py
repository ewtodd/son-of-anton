...
which moves ``SON_OF_ANTON_SESSION_ID`` env + ContextVar). The ``[session_id]`` tag on
log lines comes from a SEPARATE mechanism — ``son_of_anton_logging._session_context``
...
def test_logging_session_context_follows_compression_rotation(tmp_path: Path) -> None:
    ...
    son_of_anton_logging.set_session_context(parent_sid)
    try:
        ...
        current = getattr(son_of_anton_logging._session_context, "session_id", None)
        ...
        son_of_anton_logging.clear_session_context()
