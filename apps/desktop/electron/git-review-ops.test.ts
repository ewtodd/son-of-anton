...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'renco-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'renco-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Renco Test'], { cwd: dir })
