...
Scope of the repair is deliberately narrow. Son of Anton only upgrades an npm that
lives inside its **own** managed Node tree (``$SON_OF_ANTON_HOME/node``), installing
...
and their other projects; Son of Anton never modifies those. When the failing npm is
one of those foreign installs, Son of Anton instead provisions its own managed Node
...
from son_of_anton_constants import (
    bootstrap_son_of_anton_managed_node,
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Son of Anton-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$SON_OF_ANTON_HOME/node/bin/npm``, which itself links into
    ...
    if not npm:
        return None
    prefix = get_son_of_anton_home() / "node"
    try:
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$SON_OF_ANTON_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Son of Anton-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Son of Anton-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="son-of-anton-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Son of Anton could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Son of Anton-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$SON_OF_ANTON_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning a Son of Anton-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_son_of_anton_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Son of Anton-managed install, or
    ...
    if prefix is not None:
        # Son of Anton owns this npm — upgrade it in place. Only an npm-range
