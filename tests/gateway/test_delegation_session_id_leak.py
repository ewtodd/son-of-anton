...
def _isolate_session_context():
    saved_env = os.environ.get("SON_OF_ANTON_SESSION_ID")
    saved_ctx = _SESSION_ID.get()
    _SESSION_ID.set(_UNSET)
    os.environ.pop("SON_OF_ANTON_SESSION_ID", None)
    try:
        yield
    finally:
        ...
        if saved_env is None:
            ...
            os.environ["SON_OF_ANTON_SESSION_ID"] = saved_env
...
def _construct_child(session_id: str) -> tuple[object, str | None]:
    ...
    with delegated_child_context():
        ...
        return _SESSION_ID.get(), os.environ.get("SON_OF_ANTON_SESSION_ID")
...
def test_root_agent_keeps_contextvar_and_environment_in_sync():
    ...
    assert os.environ["SON_OF_ANTON_SESSION_ID"] == "parent-session"
    assert get_session_env("SON_OF_ANTON_SESSION_ID") == "parent-session"
...
def test_child_execution_binds_own_id_then_restores_parent():
    ...
    with delegated_child_context("child-session"):
        ...
        assert get_session_env("SON_OF_ANTON_SESSION_ID") == "child-session"
        assert os.environ["SON_OF_ANTON_SESSION_ID"] == "parent-session"
...
def test_child_subprocess_environment_receives_child_id():
    ...
    with delegated_child_context("child-session"):
        child_env = build_subprocess_env(
            base={"SON_OF_ANTON_SESSION_ID": "foreign-session"},
        )

    assert child_env["SON_OF_ANTON_SESSION_ID"] == "child-session"
    assert os.environ["SON_OF_ANTON_SESSION_ID"] == "parent-session"
    assert _SESSION_ID.get() == "parent-session"
...
def test_parallel_children_keep_parent_environment_and_own_contexts():
    ...
    def run_child(child_id: str) -> tuple[str, str, str | None, object]:
        with delegated_child_context(child_id):
            ...
            observed_environment = os.environ.get("SON_OF_ANTON_SESSION_ID")
        return (
            observed_context,
...
def test_nested_child_scope_restores_outer_child_then_parent():
    ...
    with delegated_child_context("child-outer"):
        assert get_session_env("SON_OF_ANTON_SESSION_ID") == "child-outer"
        with delegated_child_context("child-inner"):
            assert get_session_env("SON_OF_ANTON_SESSION_ID") == "child-inner"
        assert get_session_env("SON_OF_ANTON_SESSION_ID") == "child-outer"
...
def test_root_rotation_still_updates_both_paths_after_child():
    ...
    assert os.environ["SON_OF_ANTON_SESSION_ID"] == "parent-v2"
