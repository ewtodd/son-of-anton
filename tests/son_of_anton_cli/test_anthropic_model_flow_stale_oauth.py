"""Tests for Bug #12905 fix — stale OAuth token detection in son-of-anton model flow.

Bug 3: `son-of-anton model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.son-of-anton/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("son_of_anton_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from son_of_anton_cli.main import _model_flow_anthropic
        cfg = {}
