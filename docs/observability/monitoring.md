...
Renco gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Renco's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `renco.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `renco.platform.up/degraded` with bounded `error_code` attributes |
...
`renco.gateway.active_agents`, `renco.gateway.background_work`, and
`renco.gateway.background_delegations` are complementary. `active_agents`
...
```bash
renco monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'renco-agent[otlp]'`),
...
a plain `renco gateway run`: the exporter lives in the gateway process, so
...
`renco.gateway.up`, `renco.platform.up`, and `renco.platform.degraded`.
...
absent_over_time(renco_gateway_up[5m])

# Locally owned bridge is explicitly down.
renco_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
renco_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
renco_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
renco_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(renco_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `renco.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
renco.status = failed|unknown
renco.delivery_outcome = failed|not_configured
renco.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `renco.job_key`;
...
Renco Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/renco_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/renco_otel_capture.jsonl --wait 8
...
`renco.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
renco monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `renco monitoring` CLI intentionally exposes `status` only. This first
release covers only Renco Agent-owned service-health and operational-diagnostic
signals, including Renco Agent-owned Relay transport health. Team Gateway's
