...
| Version | `1.1.3` |
| Author | xdevplatform + openclaw + Renco Agent |
| License | MIT |
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Renco tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Renco Docker layout, `/opt/data` is `RENCO_HOME`, but Renco tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Renco-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Renco tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Renco subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Renco adaptation: reformatted for Renco skill conventions; safety guardrails preserved verbatim.
