...
import type { RencoReviewFile, RencoReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Renco' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'renco.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'renco.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'renco.desktop.reviewTreeMode'
const SELECTED_KEY = 'renco.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<RencoReviewFile[]>([])
...
export const $reviewShipInfo = atom<RencoReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['rencoDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: RencoReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly RencoReviewFile[], path: string): RencoReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.rencoDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.rencoDesktop?.openExternal?.(url)
    }
