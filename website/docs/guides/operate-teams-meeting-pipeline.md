...
```bash
renco teams-pipeline token-health
renco teams-pipeline token-health --force-refresh
```
...
```bash
renco teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
renco teams-pipeline maintain-subscriptions
renco teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Renco cron (recommended if you already run the Renco gateway)

Renco ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.renco/scripts/`. First create the script:

```bash
mkdir -p ~/.renco/scripts
cat > ~/.renco/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec renco teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.renco/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
renco cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/renco-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Renco Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=renco
EnvironmentFile=/etc/renco/env
ExecStart=/usr/local/bin/renco teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/renco-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Renco Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now renco-teams-pipeline-maintain.timer
systemctl list-timers renco-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/renco teams-pipeline maintain-subscriptions >> /var/log/renco/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.renco/.env` at the top of a wrapper script that crontab calls.
...
```bash
renco teams-pipeline subscriptions   # should show expirationDateTime advanced
renco teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
renco teams-pipeline list
renco teams-pipeline list --status failed
renco teams-pipeline show <job-id>
```

### Replay a stored job

```bash
renco teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
renco teams-pipeline fetch --meeting-id <meeting-id>
renco teams-pipeline fetch --join-web-url "<join-url>"
renco teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `renco teams-pipeline maintain-subscriptions --dry-run`
- inspect `renco teams-pipeline list --status failed`
...
- run `renco teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `renco teams-pipeline run`
...
- [ ] `renco teams-pipeline validate` returns an OK snapshot
- [ ] `renco teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Renco cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
