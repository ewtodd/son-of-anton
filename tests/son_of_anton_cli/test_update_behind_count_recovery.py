...
import son_of_anton_cli.banner as banner
...
def test_check_via_rev_recovers_exact_count():
    with patch(
        "son_of_anton_cli.banner.subprocess.run", return_value=_ls_remote_result(SHA_B)
...
def test_check_via_rev_up_to_date_short_circuits_compare():
    with patch(
        "son_of_anton_cli.banner.subprocess.run", return_value=_ls_remote_result(SHA_A)
...
def _shallow_git(head_sha, fetch_head_sha):
    def fake_run(cmd, **kwargs):
        if cmd[:4] == ["git", "remote", "get-url", "origin"]:
            return MagicMock(
                returncode=0,
                stdout="https://github.com/ewtodd/son-of-anton.git\n",
            )
...
def test_shallow_checkout_recovers_exact_count(tmp_path):
    ...
    """
    repo_dir = tmp_path / "son-of-anton"
    repo_dir.mkdir()

    with patch(
        "son_of_anton_cli.banner.subprocess.run", side_effect=_shallow_git(SHA_A, SHA_B)
...
def test_shallow_checkout_equal_tips_up_to_date_without_compare(tmp_path):
    ...
    with patch(
        "son_of_anton_cli.banner.subprocess.run", side_effect=_shallow_git(SHA_A, SHA_A)
    ), patch.object(banner, "_github_compare_behind") as compare:
