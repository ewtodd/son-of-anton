...
Resolution order for config file:
  1. $SON_OF_ANTON_HOME/honcho.json  (instance-local, enables isolated Son of Anton instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Son of Anton profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Son of Anton profile.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $SON_OF_ANTON_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.son-of-anton/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_son_of_anton_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.son-of-anton
    ...
    # resolve_config_path()/get_son_of_anton_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $SON_OF_ANTON_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Son of Anton profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.son-of-anton block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the Son of Anton-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Son of Anton session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Son of Anton session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_son_of_anton_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_son_of_anton_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'son-of-anton honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.son_of_anton_home) if config.son_of_anton_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/son_of_anton_home) identity reappears with a
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'son-of-anton honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                son_of_anton_cfg = load_config()
                honcho_cfg = son_of_anton_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
