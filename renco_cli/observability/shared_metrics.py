"""Durable aggregation and local export for Renco shared metrics."""
...
from renco_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "renco.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_renco_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "renco_version": row["renco_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Renco processes.
            with write_txn(connection):
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "renco_version": period_row["renco_version"],
