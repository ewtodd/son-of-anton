...
the actual `setup.sh` that creates the project workspace, configures Renco
...
2. **Create profiles** — `renco profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.renco/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `renco kanban create` assigned to the director
...
```bash
renco profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.renco/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.renco/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Renco
...
Each profile gets a `SOUL.md` at `~/.renco/profiles/<name>/SOUL.md` that
...
```bash
renco kanban create "Direct production of <video title>" \
    --assignee director \
...
the Renco `.env` (`${RENCO_HOME:-$HOME/.renco}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _renco_env="${RENCO_HOME:-$HOME/.renco}/.env"
    if grep -q "^${var}=" "$_renco_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_renco_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_renco_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY renco ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY renco OPENROUTER_API_KEY || exit 1
# ...
```
