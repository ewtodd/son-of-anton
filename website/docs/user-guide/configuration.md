...
sidebar_position: 2
title: "Configuration"
description: "Configure Renco Agent — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.renco/` directory for easy access.

:::tip Easiest path to a working `config.yaml`
Run `renco setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
renco config              # View current configuration
renco config edit         # Open config.yaml in your editor
renco config get KEY      # Print a resolved value
renco config set KEY VAL  # Set a specific value
renco config unset KEY    # Remove a user-set value
renco config check        # Check for missing options (after updates)
renco config migrate      # Interactively add missing options

# Examples:
renco config get model
renco config set model anthropic/claude-opus-4
renco config set terminal.backend docker
renco config unset terminal.backend
renco config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `renco config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `renco chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.renco/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.renco/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Long-running Renco server surfaces (including the gateway and
`renco serve --isolated`) apply the configured `RLIMIT_NOFILE` soft limit
...
The default is `4096`. Renco clamps the target to the operating system's hard
...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the legacy `RENCO_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the legacy `RENCO_API_CALL_STALE_TIMEOUT` env var.

Leaving these unset keeps the legacy defaults (`RENCO_API_TIMEOUT=1800`s, `RENCO_API_CALL_STALE_TIMEOUT=90`s, native Anthropic 900s). The non-streaming stale detector is auto-disabled for local endpoints when left implicit and can scale upward for very large contexts. Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/ewtodd/renco/blob/main/cli-config.yaml.example).

## Update Behavior

`renco update` settings live under `updates` in `config.yaml`:

```yaml
updates:
  pre_update_backup: quick       # quick (state snapshot, default) | full (snapshot + RENCO_HOME zip) | off
...
`terminal.font_family` controls the embedded terminal in Renco Desktop. It accepts either one locally installed family name (for example, `MesloLGS NF`) or a CSS font stack. Renco appends its bundled JetBrains Mono stack as a fallback, and an empty value keeps the default. You can edit the same profile-scoped setting in **Settings → Appearance → Terminal Font**; no Google Fonts download or system-font permission is required.

For cloud sandboxes such as Modal, Daytona, and Vercel Sandbox, `container_persistent: true` means Renco will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
find the credentials and config they already use in your normal shell. Renco
state is still profile-scoped through `RENCO_HOME`; `HOME` is not how profiles
select config, memory, sessions, or skills.

Renco does **not** change your system-wide `HOME`, your shell startup files, or
the operating system account home. This setting only controls the environment
passed to subprocesses that Renco launches through tools such as `terminal`,
...
|---|---|---|---|
| `auto` | Keep the real OS-user `HOME` | Use `{RENCO_HOME}/home` | Recommended default. Host CLIs keep working; container state persists. |
...
| `profile` | Use `{RENCO_HOME}/home` when it exists | Use `{RENCO_HOME}/home` when it exists | Strict per-profile CLI config isolation, but normal `~/.ssh`, `~/.gitconfig`, `~/.azure`, `~/.config/gh`, Claude/Codex auth, npm state, etc. will not be visible unless you initialize or link them inside the profile home. |
...
In that mode tool subprocesses use `{RENCO_HOME}/home` as `HOME`. Renco also
sets `RENCO_REAL_HOME` so scripts can still locate the actual user home when
they need it. Container backends keep using `{RENCO_HOME}/home` in `auto` mode
because that directory lives on the persistent Renco data volume.

Scripts that need to distinguish profile state from the real user home should
prefer `RENCO_HOME` for Renco data and `RENCO_REAL_HOME` for the account home:
...
real_home = Path(os.environ.get("RENCO_REAL_HOME", os.environ["HOME"]))
```

:::warning
The agent has the same filesystem access as your user account. Use `renco tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Single persistent container, shared across Renco processes.** Renco starts ONE long-lived container on first use and routes every terminal, file, and `execute_code` call through `docker exec` into that same container — across sessions, `/new`, `/reset`, and `delegate_task` subagents. Working-directory changes, installed packages, files in `/workspace`, and **background processes** all carry over from one tool call to the next, and from one Renco process to the next. When you close a TUI session, run `/quit`, or start a new `renco` invocation, the container keeps running and the next Renco process reuses it via a labeled lookup. See **Container lifecycle** below for the exact teardown rules.
...
terminal:
  ...
  docker_persist_across_processes: true   # Reuse container across Renco restarts
...
**`docker_env`** vs **`docker_forward_env`**: the former injects literal `KEY=value` pairs you specify in the config (the values live in your `config.yaml` or are passed as a JSON dict via `TERMINAL_DOCKER_ENV='{"DEBUG":"1"}'`). The latter forwards values from your shell or `~/.renco/.env`, so the actual secret never appears in the config file. Use `docker_forward_env` for tokens and `docker_env` for static knobs the container needs.

**`terminal.docker_extra_args`** (also overridable via `TERMINAL_DOCKER_EXTRA_ARGS='["--gpus=all"]'`) lets you pass arbitrary `docker run` flags that Renco doesn't surface as first-class keys — `--gpus`, `--network`, `--add-host`, alternative `--security-opt` overrides, etc. Each entry must be a string; the list is appended last to the assembled `docker run` invocation so it can override Renco' defaults if needed. Use sparingly — flags that conflict with the sandbox hardening (capability drops, `--user`, the workspace bind mount) will silently weaken isolation.

**`terminal.docker_network`** (default `true`; env: `TERMINAL_DOCKER_NETWORK`) — set to `false` to run the sandbox container with `--network=none`, cutting off all network egress from agent commands. This applies to the execution container used by `terminal`, `execute_code`, and the file tools. Because containers persist across Renco processes, flipping this to `false` while an older networked container exists will remove that container and start a fresh air-gapped one (a warning is logged); background processes running inside it are lost. Prefer this key over passing `--network=none` through `docker_extra_args`.

**Requirements:** Docker Desktop or Docker Engine installed and running. Renco probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `RENCO_DOCKER_BINARY=podman` (or the full path) to force it when both are installed.

#### Container lifecycle

Every Renco-managed container is tagged with three labels so subsequent processes (and the orphan reaper) can identify it:

- `renco-agent=1` — marks it as Renco-managed
- `renco-task-id=<sanitized task_id>` — keys the per-task reuse probe
- `renco-profile=<sanitized profile name>` — scopes reuse and reaping to the active Renco profile

On startup, Renco runs `docker ps --filter label=renco-task-id=<id> --filter label=renco-profile=<profile>` and **attaches to the existing container** when it finds one. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused — filesystem state and any installed packages survive, but in-container background processes do not.

When a Renco process exits — `/quit`, closing a TUI session, gateway shutdown, even SIGKILL — the cleanup path is a **no-op for the container in default mode**. The container keeps running. The next Renco process attaches to it in milliseconds via the label probe. This is the behavior the "one long-lived container shared across sessions" contract requires: it's the only way background processes (npm watchers, dev servers, long-running pytest) survive across sessions.
...
| Orphan reaper at next startup | Sweeps **Exited** renco-labeled containers older than `2 × lifetime_seconds` (default 600s = 10 min), scoped to the current profile. **Running containers are never touched** — sibling-process safety. Set `docker_orphan_reaper: false` to disable. |
...
- **Switching profiles** isolates containers from each other — a container labeled `renco-profile=work` is invisible to a Renco process running under `renco-profile=research`. The orphan reaper is profile-scoped too, so cross-profile containers don't get reaped accidentally, but they also won't get cleaned up automatically until you start Renco again under their original profile.

Parallel subagents spawned via `delegate_task(tasks=[...])` share this one container — concurrent `cd`, env mutations, and writes to the same path will collide. If a subagent needs an isolated sandbox, it must register a per-task image override via `register_task_env_overrides()`, which RL and benchmark environments (TerminalBench2, RencoSweEnv, etc.) do automatically for their per-task Docker images.
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.renco/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
| `RENCO_DOCKER_BINARY` | _none_ | Force a specific docker/podman binary path |
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.renco/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.renco/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `renco-{task_id}`.

**Disk limit:** Daytona enforces a 10 GiB maximum. Requests above this are capped with a warning.

### Vercel Sandbox Backend

Runs commands in a [Vercel Sandbox](https://vercel.com/docs/vercel-sandbox) cloud microVM. Renco uses the normal terminal and file tool surfaces; there are no Vercel-specific model-facing tools.
...
```bash
pip install 'renco-agent[vercel]'
```

**Required authentication:** Configure access-token auth with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This is the supported setup for deployments and normal long-running Renco processes on Render, Railway, Docker, and similar hosts.

For one-off local development, Renco also accepts short-lived Vercel OIDC tokens:

```bash
VERCEL_OIDC_TOKEN="$(vc project token <project-name>)" renco chat
```

From a linked Vercel project directory, you can omit the project name:

```bash
VERCEL_OIDC_TOKEN="$(vc project token)" renco chat
```

OIDC tokens are short-lived and should not be used as the documented deployment path.

**Runtime:** `terminal.vercel_runtime` supports `node24`, `node22`, and `python3.13`. If unset, Renco defaults to `node24`.

**Persistence:** When `container_persistent: true`, Renco snapshots the sandbox filesystem during cleanup and restores a later sandbox for the same task from that snapshot. Snapshot contents can include Renco-synced credentials, skills, and cache files that were copied into the sandbox. This preserves filesystem state only; it does not preserve live sandbox identity, PID space, shell state, or running background processes.

**Background commands:** `terminal(background=true)` uses Renco' generic non-local background process flow. You can spawn, poll, wait, view logs, and kill processes through the normal process tool while the sandbox is alive. Renco does not provide native Vercel detached-process recovery after cleanup or restart.

**Disk sizing:** Vercel Sandbox does not currently support Renco' `container_disk` resource knob. Leave `container_disk` unset or at the shared default `51200`; non-default values fail diagnostics and backend creation instead of being silently ignored.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/renco-agent` (HPC convention) → `~/.renco/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `renco config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Renco logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `renco doctor` to check.
...
- This covers Renco state (`~/.renco/`), **not** arbitrary working-tree files inside the sandbox — have the agent copy important artifacts out explicitly (e.g. `scp`, `modal volume put`) before the sandbox is destroyed.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.renco/cache/documents:/output" # Gateway-visible exports
```
...
`/home/user/.renco/cache/documents:/output`.

- Write files inside Docker to `/output/...`
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.renco/cache/documents/report.txt`
...
Renco resolves each listed variable from your current shell first, then falls back to `~/.renco/.env` if it was saved with `renco config set`.
...
When enabled, Renco appends `--user $(id -u):$(id -g)` to the `docker run` command so files written into bind-mounted directories (`/workspace`, `/root`, anything in `docker_volumes`) are owned by your host user, not root. The trade-off: the container can no longer `apt install` or write to root-owned paths like `/root/.npm` — use a base image whose `HOME` is owned by a non-root user (or add your required tooling at image build time) if you need both.
...
Docker sandboxes stay isolated by default. Renco does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Renco from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Renco from
...
```bash
renco config set terminal.persistent_shell false
```
...
- `renco config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `renco config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
renco config set skills.config.myplugin.path ~/myplugin-data
```
...
When the agent uses `skill_manage` to create, edit, patch, or delete a skill, Renco can optionally scan the new/updated content for dangerous keyword patterns (credential harvesting, obvious prompt injection, exfil instructions). The scanner is **off by default** — real agent workflows that legitimately touch `~/.ssh/` or mention `$OPENAI_API_KEY` were tripping the heuristic too often. Turn it back on if you want the scanner to prompt you before the agent's skill writes land:
...
Controls how much content Renco loads from each automatic context file before applying head/tail truncation. This applies to files injected into the system prompt such as `SOUL.md`, `.renco.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. It does **not** affect the `read_file` tool.
...
Three related caps control how much raw output a tool can return before Renco truncates it:
...
- **`max_bytes`** — When a `terminal` command produces more than this many characters of combined stdout/stderr, Renco keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (≈12-15K tokens across typical tokenisers).
...
Renco also flags **provider-side elision**: when an MCP or web tool result embeds its own truncation markers (`...N more items`, `"has_more": true`, "saved to sandbox" notes), a one-line notice is appended to the result warning that the visible data is incomplete and should be paged/fetched before treating any enumeration as complete.
...
`renco tools`), so a toolset listed here is always removed — even if a
...
```yaml
worktree: true    # Always create a worktree (same as renco -w)
...
Renco automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
If another turn still holds the session lease when this budget expires, Renco
fails closed: it does not load the transcript or run the model for the waiting
message. The user receives a rejection notice and must resend. Renco does not
...
- **Needs-attention escalation.** A platform continuously in the retry queue past `agent.reconnect_attention_after` (default `7200` seconds = 2 hours, `0` disables) gets `needs_attention: true` and a `retrying_since` timestamp in gateway runtime status (`renco status`), plus a WARNING log. Retries continue unchanged — this is a signal, not a circuit breaker. The flag clears on successful reconnect.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `renco plugins` → Provider Plugins → Context Engine.

See [Memory Providers](/user-guide/features/memory-providers) for the analogous single-select system for memory plugins.

## Iteration Budget

When the agent is working on a complex task with many tool calls, it can burn through its iteration budget (default: 500 turns). Renco does **not** inject mid-task pressure warnings — earlier builds warned the model at 70%/90% budget, which caused models to abandon complex tasks prematurely and was removed in April 2026.

Instead, when the budget is actually exhausted (500/500), Renco injects one message asking the model to wrap up and allows a single **grace call** so it can deliver a final response. If that grace call still doesn't produce text, the agent is asked to summarise what it accomplished.
...
`agent.max_turns` is **unlimited by default** — the turn cap caused more problems than it solved (silent mid-task truncation), so out of the box Renco runs a conversation turn to completion. To impose a cap, set a positive integer. To be explicit about "no limit", any of these case-insensitive spellings work: `"none"`, `"null"`, `"unlimited"`, `"infinite"`, `"infinity"`, `"inf"`, `0`, `-1` (they resolve to a `sys.maxsize` sentinel so the loop never exits on a turn count).

`agent.api_max_retries` controls how many times Renco retries a provider API call on transient errors (rate limits, connection drops, 5xx) **before** fallback-provider switching engages. The default is `3` — four attempts total. If you have [fallback providers](/user-guide/features/fallback-providers) configured and want to fail over faster, drop this to `0` so the first transient error on your primary immediately hands off to the fallback instead of churning retries against the flaky endpoint.
...
`verify_on_stop` accepts `true` (on everywhere), `false` (off — the default), or `"auto"` (legacy surface-aware behavior: on for interactive coding surfaces — CLI, TUI, desktop — and programmatic callers; off for messaging surfaces like Telegram/Discord where the verification narrative reads as chat noise). Off is the default everywhere: fresh installs ship `false` and the config migration turned it off on existing installs, so enabling it is an explicit opt-in. The `RENCO_VERIFY_ON_STOP` env var overrides the config value when set.
...
When a standing goal is active, Renco judges whether each assistant response satisfies it. If not, it feeds a continuation prompt back into the same session and keeps working until the goal is done, the turn budget is exhausted, or the user pauses/clears it. The turn budget is the real backstop — judge failures fail **open** (continue) so a flaky judge never wedges progress.

```yaml
goals:
  max_turns: 20   # Max continuation turns before Renco auto-pauses the goal (default: 20)
```

`max_turns` caps how many continuation turns a goal can drive before Renco auto-pauses it and asks the user to `/goal resume`. It protects against judge false negatives (goal actually done but judge says continue) and unbounded model spend on fuzzy or unachievable goals. See [Goals](/user-guide/features/goals) for the full feature.

### API Timeouts

Renco has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `RENCO_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Raised to a 900s ceiling (`agent.local_stream_stale_timeout`) | `RENCO_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 90s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `RENCO_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `RENCO_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Renco raises this to 30 minutes when it detects a local endpoint. If you explicitly set `RENCO_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.

The **stale stream detection** kills connections that receive SSE keep-alive pings but no actual content. For local providers (which don't send keep-alive pings during prefill) the default is raised to a finite 900-second ceiling instead of the 180s base — configurable via `agent.local_stream_stale_timeout` or the `RENCO_LOCAL_STREAM_STALE_TIMEOUT` env var.

The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Renco disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `RENCO_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
Renco turns on cross-session prompt caching automatically when the active provider supports it — no user config needed.

For Claude on **native Anthropic**, **OpenRouter**, and **Nous Portal**, Renco attaches `cache_control` breakpoints with the 1-hour TTL (`ttl: "1h"`) on the system prompt and skill blocks. The first send within a fresh hour pays full input rates; subsequent sends across any session within the same hour pull from the cache at the discounted cached-read rate. This means the system prompt, loaded skill content, and the early portion of any long-context include get reused across `renco` sessions and across forked subagents for the first hour.

The Qwen Cloud (Alibaba DashScope) upstream caps cache TTL at 5 minutes, so Renco uses the 5-minute breakpoint TTL there instead. Other Claude-via-third-party paths (AWS Bedrock, Azure Foundry) fall back to the provider's own caching defaults. xAI Grok uses a separate session-pinned conversation-id mechanism — see [xAI prompt caching](/integrations/providers#xai-grok--responses-api--prompt-caching).
...
The one explicit knob is the cache TTL tier Renco requests on Anthropic-style breakpoints:
...
`cache_ttl` selects the breakpoint TTL Renco attaches for Claude via the native Anthropic API, OpenRouter, and Nous Portal. Only the two Anthropic-supported tiers (`"5m"`, `"1h"`) are honored — any other value is ignored. Providers with their own caps (e.g. Qwen Cloud, which maxes at 5 minutes) still clamp to what the upstream allows.

## Auxiliary Models

Renco uses "auxiliary" models for side tasks like image analysis, web page summarization, browser screenshot analysis, session-title generation, and context compression. By default (`auxiliary.*.provider: "auto"`), Renco routes every auxiliary task to your **main chat model** — the same provider/model you picked in `renco model`. You don't need to configure anything to get started, but be aware that on expensive reasoning models (Opus, MiniMax M2.7, etc.) auxiliary tasks add meaningful cost. If you want cheap-and-fast side tasks regardless of your main model, set `auxiliary.<task>.provider` and `auxiliary.<task>.model` explicitly (for example, Gemini Flash on OpenRouter for vision and web extraction).
...
Instead of hand-editing YAML, run `renco model` and pick **"Configure auxiliary models"** from the menu. You'll get an interactive per-task picker:
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Renco Agent — Auxiliary Models Tutorial"
...
Every model slot in Renco — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Renco ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Renco uses that provider's built-in auth and base URL.
...
:::tip MiniMax OAuth
`minimax-oauth` logs in via browser OAuth (no API key needed). Run `renco model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](../guides/minimax-oauth.md).
:::

:::tip xAI Grok OAuth
`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `renco model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md), and if Renco is on a remote host see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md).
:::
...
auxiliary:
  ...
  # Kanban triage specifier — `renco kanban specify <id>` (or the
...
Each auxiliary task can optionally define a `fallback_chain` — a list of provider/model entries that Renco tries when the primary auxiliary provider fails due to rate limits, connectivity issues, or payment restrictions:
...
When the primary auxiliary provider (`openrouter` / `openai/gpt-4o-mini`) returns a rate-limit, connection timeout, or payment-required error, Renco walks the `fallback_chain` in order. It skips entries whose provider matches the already-failed provider, and tries each remaining entry until one succeeds or the chain is exhausted. If all fallbacks fail, Renco falls back to the main agent model as a final safety net.
...
The shape mirrors what OpenRouter accepts in the chat completions request body. Renco forwards the entire `extra_body` verbatim, so any other OpenRouter request-body field documented at [openrouter.ai/docs](https://openrouter.ai/docs) works the same way.
...
Or via environment variable (in `~/.renco/.env`):
...
| `"nous"` | Force Nous Portal | `renco auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `renco model` → ChatGPT or Codex Subscription |
| `"minimax-oauth"` | Force MiniMax OAuth (browser login, no API key). Uses MiniMax-M2.7-highspeed for auxiliary tasks. | `renco model` → MiniMax (OAuth) |
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok or X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `renco model` → xAI Grok OAuth (SuperGrok / Premium+) |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `renco model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Renco uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
```
Run `renco model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
`provider: "main"` uses whatever provider Renco uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `renco config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
field — OpenRouter ignores it for them. Renco transparently routes your
...
For other models routed through OpenRouter, Renco reads the live model
...
automatically without waiting for a Renco update; when the catalog is
unreachable or a model isn't listed, Renco falls back to its built-in
...
:::note
There is no `renco config set` support for `reasoning_overrides` keys — edit the YAML file directly. This is because model names often contain dots (e.g. `claude-opus-4.5`), which conflict with the CLI's dotted-key syntax.
:::
...
Renco detects when the agent is stuck in an unproductive tool-calling loop — the same tool call failing repeatedly, the same tool failing over and over, or an idempotent call returning the same result with no progress. By default it injects a **warning** into the tool result so the model self-corrects; it does not hard-stop, since a person watching the CLI/TUI can intervene.
...
- `+X -Y` line deltas appear only when the tool result already reports a diff (currently `patch`). Renco never shells out to git to compute them, so a `write_file` edit is counted without a delta.
...
When `display.file_mutation_verifier` is `true` (default), Renco appends a one-line advisory to the assistant's final response whenever a `write_file` or `patch` call failed during the turn and was never superseded by a successful write to the same path. This catches the "batch of parallel patches, half silently fail, model summarises success" class of over-claim without requiring you to manually run `git status` after every edit.
...
Set `file_mutation_verifier: false` (or `RENCO_FILE_MUTATION_VERIFIER=0`) to suppress the footer. The verifier only fires when real failures are outstanding at turn end — a model that retries a failed patch and succeeds within the same turn will not trigger it for that file.
...
- **Write denied** — path is on the credential denylist or outside `RENCO_WRITE_SAFE_ROOT` (see [File write safety](./security.md#file-write-safety))
...
⚠️ File-mutation verifier: 2 file(s) were NOT modified this turn despite any wording above that may suggest otherwise. Run `git status` or `read_file` to confirm.
  • ~/.renco/cron/jobs.json — [patch] Write denied: '…' is outside RENCO_WRITE_SAFE_ROOT (/path/to/project)
  • ~/.renco/scripts/monitor.py — [write_file] Write denied: '…' is outside RENCO_WRITE_SAFE_ROOT (/path/to/project)
```

If writes to Renco state (cron jobs, skills, scripts under `~/.renco/`) are failing, check whether `RENCO_WRITE_SAFE_ROOT` is set in your environment. For cron changes, use the `cronjob` tool or `renco cron edit` instead of patching `jobs.json` directly.
...
You can also set this per-session with the `RENCO_LANGUAGE` env var, which overrides the config value.
...
When `display.runtime_footer.enabled: true`, Renco appends a small runtime-context footer to the **final** message of each gateway turn. The current footer can show the model, context-window percentage, and current working directory. Off by default; opt in per-gateway if your team wants every reply to include this provenance.
...
`interim_assistant_messages` is gateway-only. When enabled, Renco sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
stt:
  ...
  # prompt: "Renco, Teknium, Nous Research, kanban"   # Static vocabulary hint (see below)
  local:
...
- `groq` uses Groq's Whisper-compatible endpoint and reads `GROQ_API_KEY`. Pass `stt.groq.language` (or the global `RENCO_LOCAL_STT_LANGUAGE` env var) to skip auto-detection and reduce latency.
...
An explicitly selected `stt.provider` is honored strictly — if it's unavailable, transcription errors with guidance to run `renco tools` rather than switching providers. Only when no provider has ever been selected does Renco auto-detect in this order: `local` → `groq` → `openai`.
...
stt:
  provider: "local"
  prompt: "Renco, Teknium, Nous Research, kanban, Ollama"
```
...
When the cap is reached, Renco returns a direct limit message naming which
surfaces hold the slots. Existing active sessions keep their normal behavior.
Run `renco status` to see the current slot usage and every holder.

The canonical key is top-level `max_concurrent_sessions`. Renco also accepts
`gateway.max_concurrent_sessions` as a fallback, but the top-level key wins when
both are set.

The cap is enforced with a local runtime lease file and is best-effort: Renco
fails open if the registry cannot be read or locked so users are not stranded.
It is intended for a single host/profile runtime, not a shared `$RENCO_HOME`
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Renco to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Renco still keys DMs by chat/DM ID as usual.
...
Control what Renco does when an unknown user sends a direct message:
...
- `pair` is the default for chat-style DM platforms. Renco denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status renco-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.renco/renco-agent && git pull && uv pip install -e .
  gpu:
    type: exec
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (Renco's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `renco tools`:
...
web:
  ...
  # Pin Exa/Parallel to a tier (set by the renco tools Free/Paid rows).
...
**Backend selection:** The runtime always uses the stored `web.backend` selection (set via `renco tools`; `nous` routes through the managed Tool Gateway). Only if no web backend has ever been selected is one auto-detected from available API keys: if only `SEARXNG_URL` is set, SearXNG is used; if only `EXA_API_KEY` is set, Exa; if only `TAVILY_API_KEY` is set, Tavily; if only `PARALLEL_API_KEY` is set, Parallel; if only `KEENABLE_API_KEY` is set, Keenable. With **no selection and no credentials at all**, requests rotate round-robin across the keyless free-tier ring (Exa / Parallel / Tavily / Firecrawl / Keenable) with automatic next-in-line failover on rate limits — see the [Web Search guide](/user-guide/features/web-search) for details. Once a selection exists, adding a key to `.env` does not change the route. Selecting Tavily, Firecrawl, or Keenable in `renco tools` also works without a key.
...
**Exa:** Set `EXA_API_KEY` in `~/.renco/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.renco/browser_recordings/
  # Optional CDP override — when set, Renco attaches directly to your own
  ...
  camofox:
    ...
    session_key: ""              # Optional session key sent when Renco creates a tab
...
Control how Renco handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `RENCO_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: renco chat --checkpoints). Default: false (opt-in).
...
Renco uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.renco/SOUL.md` or `$RENCO_HOME/SOUL.md` |
| `.renco.md` / `RENCO.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Renco falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.renco.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Renco automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`renco`)** | Current directory where you run the command |
| **Messaging gateway** | `terminal.cwd` from `~/.renco/config.yaml`; if unset, home directory `~` |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```yaml
# In ~/.renco/config.yaml:
terminal:
  cwd: /home/myuser/projects
```

`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.renco/.env` are legacy compatibility fallbacks. New configurations should use `terminal.cwd`.
...
- `seen` — internal state. Renco latches each shown hint here so it never fires again; the profile-build offer is also recorded here once shown. Don't hand-edit it — wipe the whole `onboarding` section if you want to re-see all hints.
...
dashboard:
  ...
  public_url: ""              # Full public authority for OAuth redirect_uri (env: RENCO_DASHBOARD_PUBLIC_URL)
...
- `oauth` / `basic_auth` / `drain_auth` — auth provider config read by the bundled dashboard-auth plugins. The drain secret itself is **not** set here; it's provisioned via the `RENCO_DASHBOARD_DRAIN_SECRET` env var. See [Web Dashboard](/user-guide/features/web-dashboard) for full auth setup.
