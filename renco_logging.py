"""Centralized logging setup for Renco Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.renco/logs/`` (profile-aware via ``get_renco_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``renco_cli.web_server``,
    ``renco_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Renco (TUI, gateway, ``hy_memory`` server, MCP
...
from renco_constants import get_config_path, get_renco_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_renco_session_injector", False):
    ...
    _session_record_factory._renco_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``renco logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``renco logs --component gateway``.
    "gateway": ("gateway", "renco_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("renco_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "renco_cli.web_server",
        "renco_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Renco logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    renco_home
        Override for the Renco home directory.  Falls back to
        ``get_renco_home()`` (profile-aware).
    log_level
    ...
    home = renco_home or get_renco_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_renco_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._renco_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the renco group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from renco_cli.config import is_managed
...
# the module header): when several Renco processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._renco_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="renco-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_renco_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse renco_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when renco_cli.config isn't importable (bare
        # renco_logging consumers).
        try:
            from renco_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
