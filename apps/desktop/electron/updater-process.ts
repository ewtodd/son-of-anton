 ...
 *
 * The staged Tauri `renco-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `renco update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `renco update`, swaps/relaunches
 * the app, and writes .renco-update-result.json. With the app gone before
 * the update starts, the RENCO_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `renco update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into RENCO_HOME on *every* platform
 * (`renco-setup.exe` on Windows, `renco-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_renco_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `renco update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(rencoHome, 'renco-setup.exe')
 ...
 *
 * `copy_self_to_renco_home` deliberately no-ops during `--update`
 ...
 * "Another Renco update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `renco update`, which
 ...
 * the installer is written to RENCO_HOME at install/repair time, making mtime
