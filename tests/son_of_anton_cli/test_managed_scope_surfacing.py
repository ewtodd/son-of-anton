"""Surfacing tests — managed scope shown in `config show` and `son-of-anton doctor`."""
import pytest
...
def test_config_show_no_managed_scope_silent(tmp_path, monkeypatch, capsys):
    ...
    home.mkdir()
    monkeypatch.setenv("SON_OF_ANTON_HOME", str(home))
    monkeypatch.setenv("SON_OF_ANTON_MANAGED_DIR", str(tmp_path / "nope"))
    (home / "config.yaml").write_text("model:\n  default: user/model\n", encoding="utf-8")
    import son_of_anton_cli.config as cfg
    from son_of_anton_cli import managed_scope
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("SON_OF_ANTON_MANAGED_DIR", str(tmp_path / "nope"))
    from son_of_anton_cli import managed_scope, doctor
