...
* Be supported-platform-only — no-op silently elsewhere so ``renco update``
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Renco
    ...
    def test_missing_explicit_override_does_not_install_standard_driver(
        ...
        monkeypatch.setenv("RENCO_CUA_DRIVER_CMD", "/missing/custom/cua-driver")
        with patch.object(
                 tools_config,
                 "_resolved_cua_driver_cmd",
                 return_value=None,
...
class TestRequireConfirmedUpdate:
    """`renco update` passes require_confirmed_update=True: the full
    ...
    Explicit `renco computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `renco computer-use install --upgrade` (default flag): the old
    ...
    def test_incompatible_driver_repairs_despite_indeterminate_check(self):
        """Renco' own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `renco update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `renco update`.
    """
...
class TestCuaVersionSummary:
    """`renco computer-use status` prints one line, whatever the binary says.

    A binary chosen by RENCO_CUA_DRIVER_CMD is under no obligation to answer
