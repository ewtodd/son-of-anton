...
4. Store the token in `${SON_OF_ANTON_HOME:-~/.son-of-anton}/.env` (or via `son-of-anton setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Son of Anton. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for Son of Anton
...
- **`AIRTABLE_API_KEY` flows from `${SON_OF_ANTON_HOME:-~/.son-of-anton}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
