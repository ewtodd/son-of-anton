...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``son-of-anton model`` CLI picker. Adds network calls
    ...
    try:
        from son_of_anton_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`son_of_anton_cli.providers.custom_provider_aliases` — lets the frontend
    do a membership check instead.
    """
    from son_of_anton_cli.providers import custom_provider_aliases
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from son_of_anton_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `son-of-anton model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Son of Anton.
    """
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Son of Anton overlay declares it keyless."""
    try:
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `son-of-anton model` to configure ({auth_type})"
        )
