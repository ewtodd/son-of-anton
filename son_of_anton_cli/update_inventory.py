...
@dataclass
class RuntimeRecord:
    """One running (or expected) Son of Anton runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "son-of-anton update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set) -> str:
    ...
    if pid in service_pids:
        try:
            from son_of_anton_cli.gateway import is_macos, supports_systemd_services
...
def _restart_mechanism(supervisor: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"son-of-anton -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        from son_of_anton_cli.gateway import _get_service_pids
    ...
    try:
        from son_of_anton_cli.gateway import find_profile_gateway_processes
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Son of Anton services: none detected — code swap only.")
        return
