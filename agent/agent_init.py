...
from son_of_anton_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.son-of-anton/.env``. See ewtodd/son-of-anton#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'son-of-anton memory status'. Note: "
        "systemd/gateway services do not inherit ~/.son-of-anton/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from son_of_anton_cli.providers import SON_OF_ANTON_OVERLAYS, get_provider

        overlay = SON_OF_ANTON_OVERLAYS.get(provider)
    ...
    try:
        ...
        from son_of_anton_cli.models import normalize_provider as normalize_model_provider
        from son_of_anton_cli.providers import normalize_provider as normalize_registry_provider
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .son-of-anton.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / SON_OF_ANTON_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.son-of-anton/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from son_of_anton_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # both live under ~/.son-of-anton/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from son_of_anton_logging import setup_logging, setup_verbose_logging
    setup_logging(son_of_anton_home=_ra()._son_of_anton_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that son_of_anton_logging.setup_logging() does not
        ...
        # handler level inside son_of_anton_logging.py, not here.
        pass
    ...
    # A multiplexed gateway may enter a different SON_OF_ANTON_HOME after
    ...
    # Session logs go into ~/.son-of-anton/sessions/ alongside gateway sessions
    son_of_anton_home = get_son_of_anton_home()
    agent.logs_dir = son_of_anton_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.son-of-anton/sessions/session_{sid}.json)
    ...
    try:
        from son_of_anton_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `son-of-anton --resume <id>` can restore the bypass (CLI resume paths read
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from son_of_anton_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from son_of_anton_cli.plugins import get_plugin_context_engine
    ...
    # Nous Son of Anton 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from son_of_anton_cli.model_switch import _check_son_of_anton_model_warning

            _son_of_anton_warn = _check_son_of_anton_model_warning(agent.model or "")
            if _son_of_anton_warn:
                _user_msg = (
                    "⚠ Nous Research Son of Anton 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_son_of_anton_warn)
        except Exception:
            pass
