"""Tests for renco_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_renco_home):
    """Drop the example-dashboard fixture into the per-test RENCO_HOME
    ...
    ``RENCO_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, renco-achievements, model providers
    ...
    from renco_cli import web_server

    user_plugins_dir = get_renco_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `renco plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down RENCO_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Renco vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints RENCO_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import renco_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("RENCO_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("RENCO_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_renco_home):
        """Create a TestClient and isolate the state DB under the test RENCO_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import renco_state
        from renco_constants import get_renco_home
        from renco_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(renco_state, "DEFAULT_DB_PATH", get_renco_home() / "state.db")
    ...
    def test_get_status_loads_gateway_config_off_event_loop(self, monkeypatch):
        ...
        import renco_cli.web_server as web_server
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `renco update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `renco update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(renco_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = renco_state.SessionDB.__init__
        ...
        # the class on renco_state covers every open the helper makes.
        monkeypatch.setattr(renco_state.SessionDB, "__init__", counting_init)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        RENCO_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from renco_constants import get_renco_home
        from renco_cli.config import load_env
        ...
        config_path = get_renco_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from renco_constants import get_renco_home
        from renco_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide RENCO_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from renco_constants import get_renco_home

        path = get_renco_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_renco_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["renco"]["workspace"] == "myws"
        assert cfg["hosts"]["renco"]["peerName"] == "eri"
        assert cfg["hosts"]["renco"]["environment"] == "local"
        assert cfg["hosts"]["renco"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["renco"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from renco_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_renco_returns_docker_guidance_without_spawning(self, monkeypatch):
        import renco_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn renco update")
        ...
        monkeypatch.setattr(web_server, "_spawn_renco_action", fail_spawn)
        web_server._ACTION_PROCS.pop("renco-update", None)
        web_server._ACTION_RESULTS.pop("renco-update", None)

        resp = self.client.post("/api/renco/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "renco-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/renco-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/renco-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/renco-agent:latest" in line for line in status_data["lines"])

    def test_update_renco_returns_apt_guidance_without_spawning(self, monkeypatch):
        import renco_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn renco update")
        ...
        assert data["update_command"] == "pkg upgrade renco-agent"
        assert spawned is False

        check = self.client.get("/api/renco/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade renco-agent"
        assert "Termux APT" in check_data["message"]

    def test_update_renco_spawns_with_action_id(self, monkeypatch):
        import renco_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_renco_action", fake_spawn)
        web_server._ACTION_PROCS.pop("renco-update", None)
        web_server._ACTION_RESULTS.pop("renco-update", None)
        ...
        assert calls == [
            (["update"], "renco-update", {"RENCO_ACTION_ID": "a" * 32})
        ]

    def test_update_renco_reuses_running_action(self, monkeypatch):
        import renco_cli.web_server as web_server
        ...
        web_server._ACTION_PROCS["renco-update"] = Proc()
        web_server._ACTION_IDS["renco-update"] = "b" * 32

        try:
            resp = self.client.post("/api/renco/update")
        finally:
            web_server._ACTION_PROCS.pop("renco-update", None)
            web_server._ACTION_IDS.pop("renco-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/renco_cli/test_setup_hidden_env.py.
        """
        from renco_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Renco provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from renco_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from renco_cli.config import save_env_value
        from renco_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "renco_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/RencoSetupBot/renco_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/RencoSetupBot/renco_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "renco_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_renco_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from renco_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from renco_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``renco model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from renco_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from renco_cli import profiles as profiles_mod
        from renco_cli.config import custom_endpoint_key_env
        from renco_constants import get_renco_home

        default_home = get_renco_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from renco_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from renco_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from renco_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from renco_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from renco_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_renco_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from renco_constants import get_renco_home
        import renco_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import renco_cli.tools_config as tools_config
        from renco_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        """
        import renco_cli.tools_config as tools_config
        from renco_cli.nous_account import NousPortalAccountInfo

        # Logged out of Nous Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "renco_cli.nous_subscription.get_nous_portal_account_info",
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "renco"
        save_config(config)
        ...
        assert "renco@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from renco_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from renco_cli.web_server import _denormalize_config_from_web
...
class TestStatusInstallId:
    ...
    Renco home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_renco_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """RENCO_HOME=<root> and RENCO_HOME=<root>/profiles/<name> resolve to
        the same id file — profiles share one physical install identity."""
        import renco_cli.web_server as ws

        root = tmp_path / "renco-root"
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from renco_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.renco/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_renco_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(renco_home, name="ocean"):
        themes_dir = renco_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="renco-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_renco_home):
        ...
        monkeypatch.setattr(
            renco_state, "DEFAULT_DB_PATH", get_renco_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``renco_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in renco_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_renco_home):
        ...
        # Pin the SessionDB to the isolated RENCO_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_renco_home, _install_example_plugin):
        ...
        installs it into the per-test ``RENCO_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into RENCO_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, renco-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (renco-achievements is loaded by default).
        resp = self.client.get("/api/plugins/renco-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        RENCO_HOME override scopes a request to another profile."""
        from renco_constants import (
            reset_renco_home_override,
            set_renco_home_override,
        )
        ...
        monkeypatch.setenv("RENCO_HOME", str(launch_home))
        from renco_cli import web_server
        token = set_renco_home_override(str(other))
        try:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets RENCO_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the renco root's plugins/ directory."""
        root = tmp_path / "renco-root"
...
# or ``sh -c 'printf …'``) instead of the real ``renco --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"renco-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "RENCO_CWD": str(tmp_path),
            "RENCO_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["RENCO_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from renco_cli.pty_bridge import PtyUnavailableError
        ...
        import renco_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from renco_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import renco_cli.main as cli_main
    import renco_cli.web_server as ws
    ...
    gateway_url = env.get("RENCO_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_renco_home, _install_example_plugin):
        ...
        the per-test ``RENCO_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_renco_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("RENCO_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under RENCO_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("RENCO_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/renco"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/renco/ds-assets/bg.png)" in prefixed.text
        assert "url(/renco/fonts-terminal/x.woff2)" in prefixed.text
