...
`mcp_tool.py`, and `renco_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# RencoMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_renco_provider_class() -> Optional[type]:
    ...
    class RencoMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._renco_server_name = server_name
            self._renco_home = ""
            ...
            self._renco_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`RencoTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, RencoTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not Renco' — mcp 2.0 builds on httpx2,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._renco_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import RencoTokenStorage
                        if isinstance(storage, RencoTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._renco_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import RencoTokenStorage
            if not isinstance(storage, RencoTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``renco mcp reauth``.
            ...
            back to ``renco mcp reauth``.
            """
            try:
                if self._renco_preregistered:
                    return
                ...
                # walking back into the same refusal. `renco mcp login`
                ...
                if cimd_url and rejected_id == cimd_url:
                    logger.warning(
                        ...
                        self._renco_server_name, cimd_url,
                    )
    ...
    return RencoMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_RENCO_PROVIDER_CLS: Optional[type] = _make_renco_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._renco_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`RencoMCPOAuthProvider` directly using the helpers
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `renco mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``renco mcp remove <name>`` and (indirectly) by
        ``renco mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, renco_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, renco_home=renco_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, renco_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, renco_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, renco_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(renco_home) / f"{_safe_filename(server_name)}.json"
            try:
