...
# ``model_tools`` import time, so ``renco status``, ``renco chat``, etc.
...
# ``renco_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Renco is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a renco MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "renco-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${RENCO_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _renco_home = _get_renco_home()
        except (ModuleNotFoundError, ImportError):
            _renco_home = _Path.home() / ".renco"
        self._thread_count_store = _ThreadCountStore(
            _renco_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Renco is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("RENCO_HOME", str(_Path.home() / ".renco"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${RENCO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a renco MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Renco is thinking…' marker message.
        ...
        "Renco is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Renco is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Renco is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Renco is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``renco setup``.

    The setup wizard at ``renco_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.renco/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. renco-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.renco/.env")
    print_info("Restart the gateway: renco gateway restart")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `renco setup` to install Google Chat support.",
        ...
        # the "Renco is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Renco is thinking…' marker message "
