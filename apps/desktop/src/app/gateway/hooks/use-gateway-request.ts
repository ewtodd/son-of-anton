...
import type { RencoGateway } from '@/renco'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as RencoGateway | null
  const gatewayRef = useRef<RencoGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.rencoDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<RencoGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as RencoGateway | null
      }),
    []
  )
