...
def _needs_server_members_intent(
    ...
    """Return True when Son of Anton must request Discord's Server Members intent.
...
def _format_privileged_intents_guidance(*, needs_members: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "Docs: https://son-of-anton.nousresearch.com/docs/user-guide/messaging/discord",
        ]
    )
    return "\n".join(lines)
...
def _discord_ready_timeout_seconds() -> float:
    ...
    if raw:
        try:
            ...
            logger.warning(
                "Ignoring invalid SON_OF_ANTON_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="SON_OF_ANTON_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="SON_OF_ANTON_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_son_of_anton_home())
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Son of Anton gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents Son of Anton requested (#79430): Message
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``SON_OF_ANTON_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("SON_OF_ANTON_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_son_of_anton_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Son of Anton response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Son of Anton work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Son of Anton/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:son-of-anton|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Son of Anton manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "son_of_anton_voice",
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Son of Anton pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Son of Anton session")
        ...
        @tree.command(name="status", description="Show Son of Anton session status")
        ...
        @tree.command(name="stop", description="Stop the running Son of Anton agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.son-of-anton/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Son of Anton Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Son of Anton gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Son of Anton session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Son of Anton in the thread",
        ...
        # son_of_anton_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from son_of_anton_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from son_of_anton_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Son of Anton skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from son_of_anton_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Son of Anton session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Son of Anton: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Son of Anton has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Son of Anton"
        if len(content) > 80:
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_son_of_anton_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Son of Anton: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Son of Anton semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "Son of Anton session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Son of Anton handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Son of Anton to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Son of Anton needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``son-of-anton update --gateway``
        ...
        *,
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Son of Anton could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_son_of_anton_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``son-of-anton pairing approve``).
    ...
    # so users approved via ``son-of-anton pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``son-of-anton update`` prompts.
...
# is not in this process (e.g. ``son-of-anton cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "SON_OF_ANTON_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "SON_OF_ANTON_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
