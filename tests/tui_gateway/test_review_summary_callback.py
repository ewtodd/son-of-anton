    ...
    with patch.dict(
        "sys.modules",
        {
            "renco_constants": MagicMock(
                get_renco_home=MagicMock(return_value="/tmp/renco_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'renco-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'renco-release' patched"
    }
