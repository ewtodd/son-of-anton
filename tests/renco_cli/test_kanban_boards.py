"""Tests for the multi-board kanban layer (``renco kanban boards …``).
...
* Current-board persistence via ``<root>/kanban/current`` and
  ``RENCO_KANBAN_BOARD`` env var.
* ``connect(board=)`` isolation — writes on one board don't leak.
* ``create_board`` / ``list_boards`` / ``remove_board`` round trip.
* CLI surface: ``renco kanban boards list/create/switch/rm``.
* ``_default_spawn`` injects ``RENCO_KANBAN_BOARD`` into worker env.
"""
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated RENCO_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test RENCO_HOME plus a path-init cache reset
    ...
    # Also reset renco_constants cache so get_default_renco_root() re-reads.
    try:
        import renco_constants
        renco_constants._cached_default_renco_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "renco-agent", "proj_1", "a",
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``RENCO_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("RENCO_KANBAN_DB", str(forced))
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("RENCO_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``RENCO_KANBAN_BOARD`` / DB / workspaces on spawn.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``renco kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "renco_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
