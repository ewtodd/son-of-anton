...
import { RENCO_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'renco:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.rencoDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(RENCO_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
