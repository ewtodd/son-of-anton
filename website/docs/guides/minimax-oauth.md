...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Renco Agent — no API key required"
---

# MiniMax OAuth

Renco Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Renco automatically refreshes your session.
...
# → Renco opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.renco/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.renco/.env
```
...
```bash
renco auth add minimax-oauth --no-browser
```

Renco will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Renco implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Renco generates a PKCE verifier / challenge pair and a random state value.
...
4. Renco polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.renco/auth.json` under the `minimax-oauth` key.
...
```bash
renco config set model.default MiniMax-M2.7
renco config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.renco/config.yaml` will contain entries similar to:
...
```bash
renco --provider minimax-oauth    # canonical
renco --provider minimax-portal   # alias
renco --provider minimax-global   # alias
renco --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `renco setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
renco --provider minimax-oauth
```
...
Renco refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Renco marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Renco marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `renco auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Renco raises a timeout error.

**Fix:** re-run `renco auth add minimax-oauth` (or `renco model`). The flow starts fresh.

### State mismatch (possible CSRF)

Renco detected that the `state` value returned by the authorization server does not match what it sent.
...
If `renco` cannot open a browser window, use `--no-browser`:

```bash
renco auth add minimax-oauth --no-browser
```

Renco prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `renco model` and select MiniMax (OAuth), or run `renco auth add minimax-oauth`.
...
```bash
renco auth logout minimax-oauth
```
...
- [renco doctor](../reference/cli-commands.md)
