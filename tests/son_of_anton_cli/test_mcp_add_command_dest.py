"""Regression test: ``son-of-anton mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``son_of_anton_cli/_parser.py``).  The dispatcher in ``son_of_anton_cli/main.py``
...
result, ``son-of-anton mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the son-of-anton parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`son-of-anton mcp add foo --url ...` must keep args.command == "mcp".
