...
binary download are exercised manually by `son-of-anton secrets bitwarden
...
def son_of_anton_home(tmp_path, monkeypatch):
    """Point Son of Anton at an isolated home directory."""
    home = tmp_path / ".son-of-anton"
    home.mkdir()
    monkeypatch.setenv("SON_OF_ANTON_HOME", str(home))
    # Some modules cache get_son_of_anton_home; clear if needed.
    import son_of_anton_constants
    if hasattr(son_of_anton_constants, "_SON_OF_ANTON_HOME_CACHE"):
        son_of_anton_constants._SON_OF_ANTON_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(son_of_anton_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from son_of_anton_cli.env_loader import _apply_external_secret_sources
