...
def session_db(tmp_path):
    ...
    os.environ["SON_OF_ANTON_HOME"] = str(tmp_path / ".son-of-anton")
    os.makedirs(tmp_path / ".son-of-anton", exist_ok=True)
    from son_of_anton_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".son-of-anton" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal SonOfAntonCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        SonOfAntonCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        SonOfAntonCLI._handle_branch_command(cli_instance, "/branch refactor approach")
