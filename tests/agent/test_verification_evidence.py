...
def test_masked_ad_hoc_script_is_not_verification_evidence(tmp_path, monkeypatch):
    ...
    script = Path(tempfile.gettempdir()) / f"son-of-anton-ad-hoc-{tmp_path.name}.py"
...
def test_windows_backslash_ad_hoc_script_path_is_matched(tmp_path, monkeypatch):
    ...
    def mock_is_temp_script(token, root):
        return "son-of-anton-ad-hoc" in token and ".py" in token
    ...
    win_script = r"C:\Users\test\AppData\Local\Temp\son-of-anton-ad-hoc-check.py"
