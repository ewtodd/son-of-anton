"""
Security advisory checker for Renco Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``renco doctor --ack <id>``; the ack is persisted to
...
1. ``renco doctor`` (and ``renco doctor --ack <id>``)
...
run in environments where the rest of Renco failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including renco when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.renco/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: renco doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Renco requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'renco doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.renco/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_renco_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``renco doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `renco doctor` on the gateway host for details.")
