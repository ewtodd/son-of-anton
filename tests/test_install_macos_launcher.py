"""Regression coverage for the user-facing macOS Renco launcher."""
...
def test_venv_launcher_bypasses_uv_console_script_that_requires_realpath(tmp_path: Path) -> None:
    """Stock macOS must start Renco even when its uv console script needs realpath."""
    ...
    (install_dir / "renco").write_text("# source entrypoint\n", encoding="utf-8")
    _make_executable(
        venv_bin / "renco",
        "#!/bin/sh\n"
        f'PATH="{minimal_path}"\n'
