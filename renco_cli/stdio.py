...
on Japanese locales, etc.).  Renco's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``renco_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Renco is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``RENCO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("RENCO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Renco-managed Git install directories so
    ...
    # already-running shells don't see the change — which means renco
    ...
    # install without Renco-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Renco's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Renco's
    ``renco config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Renco (User env var in Windows Settings, or
    export in a PowerShell profile) and Renco picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Renco-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\renco\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``renco`` from right after install) retain their old PATH.

    Any subprocess Renco spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Renco-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "renco", "git", "cmd"),
        os.path.join(local_appdata, "renco", "git", "bin"),
        os.path.join(local_appdata, "renco", "git", "usr", "bin"),
        # Renco venv Scripts directory — host of the renco.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes renco, but harmless to include.
        os.path.join(local_appdata, "renco", "renco-agent", "venv", "Scripts"),
