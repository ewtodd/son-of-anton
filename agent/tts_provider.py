...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Renco with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.renco/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Renco
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``renco tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``renco setup`` for
