...
gateway process memory + sidecar child env — so cron/`renco send`
standalone sends structurally could not authenticate. The adapter now
persists ``<renco-home>/runtime/photon-sidecar.json`` after the sidecar
...
async def test_standalone_send_consumes_record_when_env_missing(
    ...
    assert ":9111/" in url
    assert headers["X-Renco-Sidecar-Token"] == "record-token"
