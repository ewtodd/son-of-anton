...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("son_of_anton_cli.config.load_config", return_value=config or {}),
...
def test_explanation_persistence_turn_lease_cause_is_specific():
    ...
    assert "son-of-anton doctor" not in lower
...
def test_explanation_persistence_corrupt_cause_never_says_free_space():
    ...
    assert "son-of-anton doctor" in lower
...
def test_classify_persistence_error_reuses_disk_full_markers():
    """The disk bucket delegates to son_of_anton_state.is_disk_full_error, so
...
def test_classify_persistence_error_turn_lease_lost_is_distinct():
    from son_of_anton_state import SessionTurnLeaseLostError, classify_persistence_error
...
def test_persistence_error_causes_tuple_matches_classifier():
    ...
    from son_of_anton_state import PERSISTENCE_ERROR_CAUSES, classify_persistence_error
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("SON_OF_ANTON_TURN_COMPLETION_EXPLAINER", None)
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"SON_OF_ANTON_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
...
def test_explainer_config_read_once_then_cached():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("SON_OF_ANTON_TURN_COMPLETION_EXPLAINER", None)
        with patch("son_of_anton_cli.config.load_config", counting_load):
