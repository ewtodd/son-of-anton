# A2A — Agent-to-Agent protocol for Renco
...
peer (another Renco, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
renco gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Renco serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.renco/a2a_audit.jsonl`.
- Conversations persist to `~/.renco/a2a_conversations/` — they survive context
