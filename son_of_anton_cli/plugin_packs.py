    ...
    for entry in pack.plugins:
        ...
        try:
            if index_entries is None:
                from son_of_anton_cli.plugin_index import load_index

                index_entries, _src = load_index()
            from son_of_anton_cli.plugin_index import resolve_name
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print(
            "[dim]Install them manually, e.g. `son-of-anton skills install <id>`.[/dim]"
        )
    console.print(
...
def install_pack_plugins(
    ...
    *,
    ...
    single install (:func:`son_of_anton_cli.plugins_cmd._run_capability_consent`).
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-son-of-anton-pack") -> tuple[str, List[str]]:
    ...
    doc: dict[str, Any] = {
        "name": pack_name,
        "description": "Exported by `son-of-anton plugins pack export`.",
        "version": "1.0.0",
        "plugins": entries,
    }
    if config:
...
def cmd_pack_show(source: str) -> None:
    """``son-of-anton plugins pack show <path-or-url>`` — dry-run review."""
    ...
    console.print("\n[dim]Dry run only. Install with `son-of-anton plugins pack install ...`.[/dim]")


def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``son-of-anton plugins pack install <path-or-url>``.
...
def pack_command(args) -> None:
    """Dispatch ``son-of-anton plugins pack <action>``."""
    ...
    if action == "install":
        ...
        cmd_pack_export(
            ...
            name=getattr(args, "name", None) or "my-son-of-anton-pack",
        )
    elif action == "show":
        ...
        Console().print(
            "[red]Error:[/red] Usage: son-of-anton plugins pack {install|export|show}"
        )
        sys.exit(1)
