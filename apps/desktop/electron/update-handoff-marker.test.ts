...
function sandbox(tag: string) {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), `renco-handoff-marker-${tag}-`))
  const installRoot = path.join(home, 'renco-agent')
...
function markerStartedAt(home: string): number {
  const [, startedAt] = fs.readFileSync(path.join(home, '.renco-update-in-progress'), 'utf8').split('\n')
...
function runPosix(installRoot: string, startedAt?: string) {
  ...
  if (startedAt === undefined) {
    delete env.RENCO_UPDATE_STARTED_AT
  } else {
    env.RENCO_UPDATE_STARTED_AT = startedAt
  }
...
function assertScriptHandoff(run: (installRoot: string, startedAt?: string) => ReturnType<typeof spawnSync>) {
  ...
  fs.writeFileSync(path.join(refreshed.home, '.renco-update-in-progress'), '999999\n1\n')
