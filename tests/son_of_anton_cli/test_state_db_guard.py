...
from son_of_anton_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    SON_OF_ANTON_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def son_of_anton_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("son_of_anton_cli.config") or mod == "son_of_anton_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, son_of_anton_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, son_of_anton_home, capsys, monkeypatch):
        ...
        def create_then_zero(**kwargs):
            ...
            live = son_of_anton_home / "state.db"
