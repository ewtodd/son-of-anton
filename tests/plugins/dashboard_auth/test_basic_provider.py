...
def _clear_basic_env(monkeypatch):
    for var in (
        ...
        "RENCO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH",
...
class TestRegister:
    ...
    def test_registers_with_env_plaintext_password(self, basic, monkeypatch):
        monkeypatch.setenv("RENCO_DASHBOARD_BASIC_AUTH_USERNAME", "admin")
        monkeypatch.setenv("RENCO_DASHBOARD_BASIC_AUTH_PASSWORD", "hunter2")
        monkeypatch.setattr(basic, "_load_config_basic_auth_section", lambda: {})
        ctx = MagicMock()
        basic.register(ctx)
    ...
    def test_env_password_overrides_config(self, basic, monkeypatch):
        ...
        monkeypatch.setenv("RENCO_DASHBOARD_BASIC_AUTH_PASSWORD", "env-pw")
        ctx = MagicMock()
        basic.register(ctx)
        provider = ctx.register_dashboard_auth_provider.call_args.args[0]
        # env password works ...
    ...
    def test_explicit_secret_makes_sessions_portable(self, basic, monkeypatch):
        ...
        monkeypatch.setenv("RENCO_DASHBOARD_BASIC_AUTH_USERNAME", "admin")
        monkeypatch.setenv("RENCO_DASHBOARD_BASIC_AUTH_PASSWORD", "hunter2")
        monkeypatch.setenv("RENCO_DASHBOARD_BASIC_AUTH_SECRET", shared)
