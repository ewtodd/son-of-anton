"""Tests for issue #26670 — concurrent renco.exe detection and improved
quarantine retry / reboot-deferred fallback during `renco update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_renco_instances
# helper (and need the autouse stub in tests/renco_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "renco.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "renco.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_renco_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "renco.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_renco_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "renco.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_renco_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("renco.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_renco_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "renco_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "renco_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m renco_cli.main gateway run"
