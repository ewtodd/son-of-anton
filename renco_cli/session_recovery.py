"""Offline, non-destructive recovery for a damaged Renco session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``renco_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``renco_state._backup_db_file``.
    """
    from renco_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Renco process using this profile and retry. "
                "This includes the interactive `renco` CLI session this "
                ...
                "Renco process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `renco` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from renco_cli.session_lost_and_found import (
