"""
Tests for mcp_serve — Son of Anton MCP server.
...
def _isolate_son_of_anton_home(tmp_path, monkeypatch):
    """Redirect SON_OF_ANTON_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking son_of_anton_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <SON_OF_ANTON_HOME>/state.db for its mtime gate; the autouse
        # fixture points SON_OF_ANTON_HOME at tmp_path.
        db_path = tmp_path / "state.db"
