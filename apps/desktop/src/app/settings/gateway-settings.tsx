...
// Renco Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.rencoDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.rencoDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteRencoPath: state.sshRemoteRencoPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.rencoDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.rencoDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'renco-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.rencoDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.rencoDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.rencoDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.rencoDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.rencoDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.rencoDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.rencoDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.rencoDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.rencoDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.rencoDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Renco Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/renco"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteRencoPath: event.target.value }))}
                placeholder={g.sshRencoPathPlaceholder}
                value={state.sshRemoteRencoPath}
              />
            }
            description={g.sshRencoPathDesc}
            title={g.sshRencoPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.rencoDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
