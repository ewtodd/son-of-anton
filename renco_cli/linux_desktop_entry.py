"""Install and remove the Linux desktop entry (``renco.desktop``).

``renco desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "renco.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``renco.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``renco desktop``.

    Prefer the real ``renco`` executable (argv[0] or PATH). When Renco
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from renco_cli.relaunch import resolve_renco_bin

    bin_path = resolve_renco_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "renco_cli.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Renco\n"
        "GenericName=Renco Desktop\n"
        "Comment=Launch Renco Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Renco\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Renco desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "renco"
