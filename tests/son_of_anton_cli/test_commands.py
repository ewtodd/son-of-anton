...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /son-of-anton <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_son_of_anton_norm = {_norm(n) for n in _SLACK_VIA_SON_OF_ANTON_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_son_of_anton_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `son-of-anton slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "son_of_anton_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from son_of_anton_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch son_of_anton_cli.plugins.get_plugin_commands() to a fixed dict."""
        from son_of_anton_cli import plugins as _plugins_mod
