...
//!
//! Driven when the installer is launched as `Renco-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Renco desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `renco update`
//!      or repair bootstrap can race locked files),
//!   2. run `renco update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in renco_cli/main.py),
//!   3. run `renco desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `renco update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_renco) and
...
/// `renco update` exit code meaning "another renco process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// renco_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `renco update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `renco update` claims
/// the same file (see `renco_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `renco update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in renco_cli/update_lock.py — all three read
...
/// a dashboard-spawned `renco update`) still blocks.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let renco_home = crate::paths::renco_home();
    let install_root = renco_home.join("renco-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_renco`
    ...
    //
    // The same marker is the cross-process update lock (renco_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `renco update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Renco update is already running (PID {}, started {} ago). \
    ...
    let renco = resolve_renco(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the renco CLI under {}. Is Renco installed? \
    ...
    // async on Windows. If it still holds the venv shim, `renco update`
    ...
    // ---- stage 2: renco update -----------------------------------------
    // Pass --branch so `renco update` targets the branch this installer was
    ...
    // without --branch, `renco update` switches the checkout to `main` (a
    ...
    // --force skips `renco update`'s Windows running-exe guard (which would
    ...
    // time `renco update` runs there is no legitimate renco.exe to protect,
    // and the guard would only produce a false "Renco is still running" stop.
    //
    ...
    // probe only checks the renco.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Renco windows" message.
    ...
    // Retry-once for the update-boundary crash. `renco update` lazily imports
    ...
    // second `renco update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Renco" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Renco is still running. Close all Renco windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "renco update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::renco_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: renco desktop --build-only ----------------------------
    // `renco update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `renco update` already does above, and `renco update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `renco desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Renco manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_renco_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Renco to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend renco.exe (or the desktop Renco.exe
            ...
            // force-kill every Renco.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Renco still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_renco();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_renco(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Renco.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Renco.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `renco.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `renco.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\renco.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startRenco()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// renco.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named renco.exe.)
fn force_kill_other_renco() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `renco <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `renco update` is precisely the shape that
...
/// Path to the venv renco shim under an install root, regardless of existence.
fn venv_renco(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("renco.exe")
    } else {
        install_root.join("venv").join("bin").join("renco")
    }
}

/// Resolve the renco CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `renco` on PATH.
fn resolve_renco(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_renco(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "renco.exe" } else { "renco" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let renco_home = crate::paths::renco_home();
    let mut envs = vec![(
        "RENCO_HOME".to_string(),
        renco_home.as_os_str().to_os_string(),
    )];
    // `renco update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `renco update` child claims that SAME lock (renco_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Renco is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in renco_cli/update_lock.py.
    envs.push((
        "RENCO_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        renco_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_renco_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Renco.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.renco-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.renco-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::renco_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_renco_is_under_install_root() {
        let root = Path::new("/x/renco-agent");
        let shim = venv_renco(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/renco-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/renco-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "RENCO_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the renco update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_renco(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/renco-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".renco-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `renco update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(rencoHome, child.pid) races ahead
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Renco.app"]),
            Some(PathBuf::from("/Applications/Renco.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Renco.app");
        let tmp = base.join("Renco.app.renco-update-new");
        let old = base.join("Renco.app.renco-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Renco.app");
        let tmp = base.join("Renco.app.renco-update-new"); // intentionally absent
        let old = base.join("Renco.app.renco-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Renco.app");
        let tmp = base.join("Renco.app.renco-update-new"); // absent
        let old = base.join("Renco.app.renco-update-old");
        write_marker(&target, "OLD");
