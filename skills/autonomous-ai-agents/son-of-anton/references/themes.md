# Themes / Skins — Author a Son of Anton Color Theme

Author a Son of Anton **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`son_of_anton_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.son-of-anton/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Son of Anton itself.
...
- Write access to the Son of Anton home dir — `~/.son-of-anton` by default, or `$SON_OF_ANTON_HOME`
  / the active profile's dir. Skins live in `<son-of-anton-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `son-of-anton config set`).
...
3. `write_file` it to `<son-of-anton-home>/skins/<name>.yaml`.
...
`son_of_anton_cli/skin_engine.py`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `son-of-anton config set display.skin default` (or they can `/skin default`).
...
```
son-of-anton skin set <key> <hex>      # e.g. son-of-anton skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `son-of-anton skin set background "#08201f"` changes
only the background; `son-of-anton skin use <name>` / `son-of-anton skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.son-of-anton`** when a profile is active — resolve the real home
  from `$SON_OF_ANTON_HOME` first, falling back to `~/.son-of-anton`.
...
- **Never hand-edit `config.yaml` to activate.** Use `son-of-anton config set
...
- **You apply it, not the user.** `son-of-anton config set display.skin <name>` is
...
- `read_file` the written `<son-of-anton-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `son-of-anton config get display.skin` and confirm it reports `<name>`.
