...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  Son of Anton never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  Son of Anton continues with whatever credentials ``.env`` already had.
...
are cached in-process and on disk under ``<son_of_anton_home>/cache/op_cache.json``
so back-to-back short-lived ``son-of-anton`` invocations don't re-shell ``op`` for
...
# In-process cache.  The key folds in str(home_path) so a SON_OF_ANTON_HOME switch
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured Son of Anton to source it from, so normalize to that name here.
    if token_value:
...
def apply_onepassword_secrets(
    *,
    ...
    Called by ``load_son_of_anton_dotenv()`` after the .env files have loaded.
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            ...
            if isinstance(cfg, dict):
                token_env = str(cfg.get("service_account_token_env") or token_env)
            return (
                "Run `son-of-anton secrets onepassword token` to paste a fresh "
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`son-of-anton secrets onepassword token`) so
