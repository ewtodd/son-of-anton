...
export interface RencoReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForRencoReady`).
   */
...
export async function waitForRencoReady(baseUrl: string, options: RencoReadyOptions): Promise<void> {
  ...
  throw new Error(`Renco backend did not become ready: ${detail}`)
}
