...
Older images made /opt/renco/ui-tui and /opt/renco/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV RENCO_TUI_DIR=/opt/renco/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R renco:renco /opt/renco/ui-tui" not in text
    assert "chown -R renco:renco /opt/renco/node_modules" not in text
