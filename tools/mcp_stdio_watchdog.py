...
<url>``) is spawned as a direct child of the Son of Anton process. Son of Anton's own
...
Son of Anton process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Son of Anton startup's opt-in
