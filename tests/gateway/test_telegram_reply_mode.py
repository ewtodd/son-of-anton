...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/ewtodd/renco/issues/23994:
    reply_to_mode 'off' was ignored when sending via Renco-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/ewtodd/renco/issues/87051:
    ...
    response in a different chat lane than the Renco topic the session runs
    in. The no-anchor branch must prefer the Renco topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_renco_topic_thread_id(self):
