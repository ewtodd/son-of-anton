"""Tests for Codex auth — tokens stored in Son of Anton auth store (~/.son-of-anton/auth.json)."""
...
def _setup_son_of_anton_auth(son_of_anton_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Son of Anton auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_son_of_anton_auth(son_of_anton_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (son_of_anton_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("SON_OF_ANTON_HOME", str(son_of_anton_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((son_of_anton_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``son-of-anton auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``son-of-anton auth add openai-codex``.
    ...
    (son_of_anton_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `son-of-anton auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``son-of-anton auth add openai-codex`` for accounts B and C
    ...
    (son_of_anton_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``son-of-anton auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Son of Anton auth store, not ~/.codex/."""
    son_of_anton_home = tmp_path / "son-of-anton"
    ...
    _save_codex_tokens({"access_token": "son-of-anton-at", "refresh_token": "son-of-anton-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Son of Anton store
    assert not (codex_home / "auth.json").exists()

    # Son of Anton auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "son-of-anton-at"


def test_resolve_returns_son_of_anton_auth_store_source(tmp_path, monkeypatch):
    son_of_anton_home = tmp_path / "son-of-anton"
    _setup_son_of_anton_auth(son_of_anton_home)
    monkeypatch.setenv("SON_OF_ANTON_HOME", str(son_of_anton_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "son-of-anton-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run son-of-anton auth".
    """
    from son_of_anton_cli.auth import (
    ...
    assert "son-of-anton auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from son_of_anton_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from son_of_anton_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch son_of_anton_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("son_of_anton_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
