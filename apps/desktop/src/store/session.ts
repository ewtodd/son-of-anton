import type { ConnectionState } from '@renco/shared'
...
import type { SessionInfo, UsageStats } from '@/types/renco'
...
const COMPOSER_MODEL_KEY = 'renco.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'renco.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'renco.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'renco.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'renco.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'renco.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'renco.desktop.lastRoute'
...
function workspaceCwdKey(connection: RencoConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.rencoDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.rencoDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<RencoConnection | null>(null)
...
export const setConnection = (next: Updater<RencoConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useRencoConfig.
