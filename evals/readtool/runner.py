"""Run the read-tool eval through the REAL Renco AIAgent.

For each task: fresh temp RENCO_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    renco_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".renco"
    ...
    try:
        ...
        shutil.rmtree(renco_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.renco/.env; set +a  — then relaunch."
        )
