"""Tests for son_of_anton_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $SON_OF_ANTON_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Son of Anton-managed node dir.
"""
...
import son_of_anton_cli.uninstall as uninstall
...
def _make_son_of_anton_node(son_of_anton_home: Path) -> Path:
    """Create a fake $SON_OF_ANTON_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = son_of_anton_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    son_of_anton_home = fake_home / ".son-of-anton"
    _make_son_of_anton_node(son_of_anton_home)
    ...
    removed = uninstall.remove_node_symlinks(son_of_anton_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    son_of_anton_home = fake_home / ".son-of-anton"
    node_bin = _make_son_of_anton_node(son_of_anton_home)
