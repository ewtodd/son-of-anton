...
``renco_cli.env_loader._apply_external_secret_sources()``.
...
def get_source(name: str, *, scope: Optional[str] = None) -> Optional[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        return _SCOPED_SOURCES.get(scope or renco_home_key(), {}).get(
            name
...
def list_sources(*, scope: Optional[str] = None) -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(scope or renco_home_key(), {}))
...
def list_plugin_sources() -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(renco_home_key(), {}))
        return list(merged.values())
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``renco`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's RENCO_HOME is ``~/.renco/profiles/<name>``; the
    default profile (``~/.renco``) returns "".
    """
    if home_path is not None:
        resolved = Path(home_path)
        if resolved.parent.name == "profiles" and resolved.name:
            return resolved.name
    for env_name in ("RENCO_PROFILE_NAME", "RENCO_PROFILE"):
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(
        secrets_cfg, scope=renco_home_key(home_path)
    )
    if not enabled:
        return report
