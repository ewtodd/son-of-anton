"""``son-of-anton cron`` subcommand parser.

Extracted verbatim from ``son_of_anton_cli/main.py:main()`` — same arguments, same
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        "--deliver",
        help="Delivery target: origin, local, discord, signal, slack, or platform:chat_id",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.son-of-anton/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        ...
        help=(
            ...
            "~/.son-of-anton/scripts/ that runs each tick BEFORE the agent. "
    ...
    cron_create.add_argument(
        "--provider",
        dest="model_provider",
        help="Inference provider paired with --model (e.g. 'deepseek', 'custom').",
    )
    cron_create.add_argument(
        "--reasoning-effort",
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.son-of-anton/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        ...
        dest="monitor_script",
        help=(
            "Set/replace the monitor source script (see `son-of-anton cron create "
