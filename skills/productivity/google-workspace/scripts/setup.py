#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Son of Anton Agent.
...
# Ensure sibling modules (_son_of_anton_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _son_of_anton_home import display_son_of_anton_home, get_son_of_anton_home

SON_OF_ANTON_HOME = get_son_of_anton_home()
TOKEN_PATH = SON_OF_ANTON_HOME / "google_token.json"
CLIENT_SECRET_PATH = SON_OF_ANTON_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = SON_OF_ANTON_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Son of Anton profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Son of Anton Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Son of Anton Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  son-of-anton setup")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Son of Anton home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_son_of_anton_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Son of Anton")
