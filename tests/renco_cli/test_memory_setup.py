...
import renco_cli.memory_setup as memory_setup
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_renco_home", lambda: tmp_path)
    monkeypatch.setattr("renco_cli.config.load_config", lambda: {"memory": {}})
...
def test_install_dependencies_force_reinstalls_versioned_specs(tmp_path, monkeypatch):
    ...
    so a downgraded/stripped bridge package is restored on renco update."""
...
def test_cmd_status_memory_tool_gate_disabled(capsys, monkeypatch):
    ...
    monkeypatch.setattr("renco_cli.config.load_config", lambda: _cfg)
    # check_memory_requirements() reads the readonly loader, not load_config.
    monkeypatch.setattr(
        "renco_cli.config.load_config_readonly", lambda: _cfg, raising=False
    )
