...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects RENCO_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("RENCO_INTERACTIVE", "RENCO_GATEWAY_SESSION", "RENCO_EXEC_ASK", "RENCO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["RENCO_INTERACTIVE"] = "1"
        session_key = os.getenv("RENCO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No RENCO_INTERACTIVE or RENCO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
