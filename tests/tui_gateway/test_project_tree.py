...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="son-of-anton/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is SON_OF_ANTON_HOME (config/state) must not spawn a
    # phantom project; it lands in the Home bucket. A real repo alongside it
    # still groups normally.
    resolve = _resolver(
        {
            "/home/me/.son-of-anton": ("/home/me/.son-of-anton", "/home/me/.son-of-anton"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.son-of-anton", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.son-of-anton"
...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.son-of-anton")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.son-of-anton"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/son-of-anton": ("/www/son-of-anton", "/www/son-of-anton")})
    sessions = [
        _session("/www/son-of-anton", branch="main"),
        _session("/www/son-of-anton-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/son-of-anton"]
    assert _lane_ids(project) == ["/www/son-of-anton::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/son-of-anton"
