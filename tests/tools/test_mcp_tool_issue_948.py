...
def test_resolve_stdio_command_falls_back_to_son_of_anton_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool.shutil.which", return_value=None), \
         patch.dict("os.environ", {"SON_OF_ANTON_HOME": str(tmp_path)}, clear=False):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$SON_OF_ANTON_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the Son of Anton Docker image
    ...
    # the other candidates ($SON_OF_ANTON_HOME/node/bin/npx and ~/.local/bin/npx)
