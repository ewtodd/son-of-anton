...
export function setZoomPercent(percent: number): void {
  window.rencoDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.rencoDesktop?.zoom) {
  void window.rencoDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.rencoDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
