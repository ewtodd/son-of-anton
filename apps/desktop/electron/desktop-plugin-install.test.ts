...
describe('resolvePluginGitUrl', () => {
  it('maps owner/repo shorthand to github git url', () => {
    expect(resolvePluginGitUrl('NousResearch/renco-example-plugins')).toEqual({
      gitUrl: 'https://github.com/NousResearch/renco-example-plugins.git',
      subdir: null
    })
  })
...
describe('resolveSubdirWithin', () => {
  it('rejects path traversal', () => {
    const root = mkdtemp('renco-plugin-root-')
...
describe('findDesktopEntry', () => {
  it('finds root plugin.js', () => {
    const root = mkdtemp('renco-plugin-detect-')
...
describe('detectPluginComponents', () => {
  ...
  it('detects agent-only layout', async () => {
    const root = mkdtemp('renco-plugin-agent-')
    roots.push(root)
  ...
  it('detects dual layout', async () => {
    const root = mkdtemp('renco-plugin-dual-')
    roots.push(root)
    fs.mkdirSync(path.join(root, 'desktop'), { recursive: true })
