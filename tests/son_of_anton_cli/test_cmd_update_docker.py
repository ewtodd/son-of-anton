"""Tests for ``son-of-anton update`` / ``--check`` inside the Docker container.
...
fix, ``son-of-anton update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Son of Anton, not an update to the running container, so the message
...
from son_of_anton_cli.main import _cmd_update_check, cmd_update


# ---------- cmd_update (apply path) ----------


@patch("son_of_anton_cli.config.is_managed", return_value=False)
@patch("son_of_anton_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``son-of-anton update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull nousresearch/son-of-anton:latest" in out
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull nousresearch/son-of-anton:latest" in msg
    ...
    assert "SON_OF_ANTON_HOME" in msg or "/opt/data" in msg, (
