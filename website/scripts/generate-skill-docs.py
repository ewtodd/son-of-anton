...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/ewtodd/renco/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    renco_meta = (fm.get("metadata") or {}).get("renco") or {}
    tags = renco_meta.get("tags") or []
    related = renco_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `renco skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Renco Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Renco ships with a large built-in skill library copied into `~/.renco/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Renco also syncs bundled skills on `renco update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.renco/skills/` tree, it is still shipped with Renco; restore it with `renco skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with renco-agent — install via renco skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with renco-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "renco skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "renco skills install official/blockchain/solana",
        "renco skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "renco skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
