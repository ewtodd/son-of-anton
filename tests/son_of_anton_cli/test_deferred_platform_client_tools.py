...
loader so ``son-of-anton chat`` doesn't import ~20 gateway SDKs. The a2a plugin ships
...
``son-of-anton tools`` checklist. The same tools worked in gateway/web processes only
...
def _write_platform_plugin(
    ...
    *,
    ...
    """
    from son_of_anton_cli.plugins import PluginManifest
...
def clean_registry():
    ...
    ``probeplat`` name, so the imported ``son_of_anton_plugins.*`` modules have to go
    ...
    for name in set(sys.modules) - before_modules:
        if name.startswith("son_of_anton_plugins."):
...
class TestA2AClientToolsInCliProcess:
    ...
    def test_a2a_appears_in_the_son_of_anton_tools_checklist(self):
    ...
    def test_platform_bundle_includes_the_client_tools(self):
        ...
        assert A2A_CLIENT_TOOLS.issubset(set(resolve_toolset("son-of-anton-a2a")))
...
class TestDeferredPlatformToolPreregistration:
    ...
    def test_tools_stay_attributed_after_materialization(
        ...
        """`son-of-anton plugins list` must still credit the pre-registered tools.
    ...
    def test_partially_registered_tools_are_still_attributed(
        ...
        `son-of-anton plugins list` under-report what the process is carrying, and
    ...
    def test_failed_materialization_tears_down_pre_registered_tools(
        ...
        adapter. Attribution and the registry therefore agree at zero: `son-of-anton
