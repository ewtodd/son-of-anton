#!/usr/bin/env python3
"""Live test harness for Renco Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any renco imports.
ORIGINAL_HOME = os.environ.get("RENCO_HOME")
ORIGINAL_AUTH = Path.home() / ".renco" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.renco/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.renco/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="renco_ts_live_"))
    renco_home = home_dir / ".renco"
    renco_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, renco_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".renco" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, renco_home / ".env")
        ...
        load_renco_dotenv(renco_home=str(Path.home() / ".renco"))
    ...
    (renco_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return renco_home
...
def reset_module_state():
    """Drop cached modules so the new RENCO_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "renco_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["RENCO_HOME"] = str(home)
...
def main():
    ...
    # Restore original RENCO_HOME
    if ORIGINAL_HOME is not None:
        os.environ["RENCO_HOME"] = ORIGINAL_HOME
    else:
