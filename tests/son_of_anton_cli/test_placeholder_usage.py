...
from son_of_anton_cli.config import config_command, show_config
from son_of_anton_cli.setup import _print_setup_summary
...
def test_config_set_usage_marks_placeholders(capsys):
    ...
    assert "Usage: son-of-anton config set" in out
    assert "<key>" in out
    assert "<value>" in out
...
def test_show_config_marks_placeholders(tmp_path, capsys):
    ...
    out = capsys.readouterr().out
    assert "son-of-anton config set <key> <value>" in out
