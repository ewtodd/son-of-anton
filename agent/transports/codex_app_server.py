...
projection into Renco' display, approval bridging, transcript projection into
...
"codex_app_server"`. Renco' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Renco secret — gateway
        ...
        spawn_env = renco_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("RENCO_KANBAN_TASK"):
            kanban_db = spawn_env.get("RENCO_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "RENCO_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("RENCO_HOME", os.path.expanduser("~/.renco")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "renco",
        client_title: str = "Renco Agent",
