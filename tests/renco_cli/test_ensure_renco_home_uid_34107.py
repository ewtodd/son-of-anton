"""Regression tests for #34107 — Docker UID/GID handling in ensure_renco_home.

When Renco runs in Docker with ``RENCO_UID=1000`` / ``RENCO_GID=911``,
the entrypoint chowns the top-level ``RENCO_HOME`` once at startup. But
...
The fix is a ``_chown_to_renco_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_renco_uid_gid
# ---------------------------------------------------------------------------


class TestResolveRencoUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("RENCO_UID", "1000")
        monkeypatch.setenv("RENCO_GID", "911")
        from renco_cli.config import _resolve_renco_uid_gid
...
class TestChownToRencoUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_renco_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("RENCO_UID", raising=False)
        monkeypatch.delenv("RENCO_GID", raising=False)
        from renco_cli import config as cfg
