"""Anthropic Messages API adapter for Son of Anton Agent.

Translates between Son of Anton's internal OpenAI-style message format and
...
try:
    import son_of_anton_cli as _son_of_anton_cli

    _SON_OF_ANTON_VERSION = str(_son_of_anton_cli.__version__)
except Exception:
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in son_of_anton_cli/runtime_provider.py.
    """
...
# Son of Anton effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to son-of-anton's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + SonOfAntonAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://son-of-anton.nousresearch.com")
        headers.setdefault("X-Title", "Son of Anton Agent")
        headers.setdefault("User-Agent", f"SonOfAntonAgent/{_SON_OF_ANTON_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Son of Anton loads that into the process env from
    # ``~/.son-of-anton/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Son of Anton notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Son of Anton historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``son-of-anton models``) never mutates ``~/.son-of-anton/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Son of Anton)
      ...
      5. Anthropic credential_pool OAuth entry (~/.son-of-anton/auth.json)
    ...
    # 1. Son of Anton-managed OAuth/setup token env var
    ...
    # 5. Son of Anton credential_pool OAuth entry.
...
# ── Son of Anton-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.son-of-anton/.anthropic_oauth.json (our own file).
...
def _get_son_of_anton_oauth_file() -> Path:
    return get_son_of_anton_home() / ".anthropic_oauth.json"
...
def run_son_of_anton_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Son of Anton-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Son of Anton with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from son_of_anton_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_son_of_anton_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Son of Anton-managed OAuth credentials from ~/.son-of-anton/.anthropic_oauth.json."""
    oauth_file = _get_son_of_anton_oauth_file()
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Son of Anton OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See son-of-anton#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See son-of-anton: extended-thinking + parallel
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Son of Anton Agent", "Claude Code")
                text = text.replace("Son of Anton agent", "Claude Code")
                text = text.replace("son-of-anton", "claude-code")
        ...
        #      a) bare Son of Anton-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Son of Anton surfaces in its CLI. We
