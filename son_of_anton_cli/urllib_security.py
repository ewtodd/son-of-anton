...
def _resolved_https_context() -> ssl.SSLContext | None:
    """Return the explicit CA context for Son of Anton-owned urllib openers."""
    ca_bundle = next(
        (
            value
...
def _secure_opener_from_installed_policy(original_url: str, *, ssl_context=None):
    ...
    ``ssl_verify``) apply to this request. When it is None, Son of Anton-owned
    ...
    setattr(
        secured,
        "_son_of_anton_initial_addheaders",
...
def open_credentialed_url(
    ...
    *,
    ...
    if opener_factory is None:
        ...
        for name, value in getattr(opener, "_son_of_anton_initial_addheaders", ()):
