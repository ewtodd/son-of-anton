...
@pytest.fixture
def son_of_anton_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.son-of-anton/`` with a config.yaml referencing one external dir."""
...
def test_cache_invalidates_on_mtime_change(son_of_anton_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = son_of_anton_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different SON_OF_ANTON_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".son-of-anton"
    ...
    home_b = tmp_path / "home_b" / ".son-of-anton"
