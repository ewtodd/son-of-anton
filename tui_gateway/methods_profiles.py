...
handlers delegate to the same `son_of_anton_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Son of Anton profiles (name, path, model, description, skill count).
    ...
    def _latest_profile_session_rows(profile_path):
        ...
        sessions never surface in conversation lists (son-of-anton#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's son-of-anton-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``son-of-anton setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (son_of_anton_cli.auth: profile reads fall back to the global
    ...
    if is_truthy_value(params.get("mirror_credentials", True)):
        ...
        launch_home = get_son_of_anton_home()
        try:
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local SON_OF_ANTON_HOME override — the same
        ...
        try:
            ...
            token = set_son_of_anton_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from son_of_anton_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from son_of_anton_cli.config import load_config_readonly, read_user_config_raw
            from son_of_anton_cli.web_routers.profiles import _write_profile_model
            from son_of_anton_constants import (
                reset_son_of_anton_home_override,
                set_son_of_anton_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the SON_OF_ANTON_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from son_of_anton_cli.skills_config import get_disabled_skills
            ...
            # Toolsets: the same filtered universe the `son-of-anton tools`
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from son_of_anton_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            description = ""
            try:
                from son_of_anton_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from son_of_anton_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from son_of_anton_cli.skills_config import save_disabled_skills
