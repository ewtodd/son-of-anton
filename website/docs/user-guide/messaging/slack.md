...
sidebar_position: 4
title: "Slack"
description: "Set up Renco Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Renco Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Renco instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Renco uses the modern
...
The fastest path is to paste a manifest Renco generates for you. It
...
### Option A: From a Renco-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   renco slack manifest --agent-view --write
   ```
   This writes `~/.renco/slack-manifest.json` and prints paste-in
   ...
   ```bash
   renco slack manifest --agent-view \
...
4. Enter an app name (e.g., "Renco Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Renco can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `renco-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Renco configuration issue.
:::
...
Renco uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
renco gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
renco gateway              # Foreground
renco gateway install      # Install as a user service
sudo renco gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Renco now
...
Every Renco command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Renco command with its description.

Under the hood: Renco ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/ewtodd/renco/blob/main/renco_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing Renco
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Renco can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Renco only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When Renco adds new commands (e.g. after `renco update`), regenerate
the manifest and update your Slack app:

```bash
renco slack manifest --write
```
...
3. Paste the new contents of `~/.renco/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/renco <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/renco btw run the tests` — Renco routes it the same way as `/btw
run the tests`. Free-form questions also work: `/renco what's the
...
Slack never delivers them to Renco.

As a workaround, Renco recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Renco treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@Renco !stop`) and with
...
Renco falls back to a text prompt, the prompt instructs you to reply
...
fails, Renco retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@Renco /cmd` — reply as normal visible
messages instead.)
...
```bash
renco slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Renco behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Renco Agent what time is it?`). In channels, Renco replies in a thread attached to that message. |
| **Threads** | If you @mention Renco inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.renco/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # If Slack rejects the native stream, Renco keeps one editable text
      ...
      # that message itself @mentions Renco, and "all" accepts every
      # other bot. Renco always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions Renco, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default Renco sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Renco is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), Renco uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — Renco seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and Renco falls back
...
- If the native stream can't start or update, Renco falls back to a single
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Renco in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Renco will respond.
...
By default Renco ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several Renco instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- Renco always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer Renco agents are filtered consistently across workspaces.
...
manifest — regenerate with `renco slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Renco will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Renco Agent`).
...
Delivery works even when the cron process isn't co-located with the gateway — Renco falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
Renco can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Renco also loads tokens from an **OAuth token file** at:

```
~/.renco/slack_tokens.json
```
...
- When a message arrives, Renco uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Renco Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Renco Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Renco now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Renco Agent`)
...
- Tokens should be stored in `~/.renco/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Renco config directory
