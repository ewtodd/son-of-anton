...
For occasional contributors and PR authors. Full developer docs: https://son-of-anton.nousresearch.com/docs/developer-guide/
...
├── cli.py                # Interactive CLI (SonOfAntonCLI)
├── son_of_anton_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── son_of_anton_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.son-of-anton/config.yaml` (settings), `~/.son-of-anton/.env` (API keys) — both under `$SON_OF_ANTON_HOME` when it is set.
...
`_SON_OF_ANTON_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_son_of_anton_home()` for paths,
never hardcode `~/.son-of-anton`. For custom/local-only tools, write a plugin in
`~/.son-of-anton/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `son_of_anton_cli/commands.py`
...
- Tests auto-redirect `SON_OF_ANTON_HOME` to temp dirs — never touch real `~/.son-of-anton/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/son_of_anton_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_son_of_anton_home()` from `son_of_anton_constants` for all paths (profile-safe)
