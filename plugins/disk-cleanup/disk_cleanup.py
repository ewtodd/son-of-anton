"""disk_cleanup — ephemeral file cleanup for Renco Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under RENCO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly RENCO_HOME and /tmp/renco-*
Never touches: ~/.renco/logs/ or any system directory.
"""
...
try:
    ...
    def get_renco_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("RENCO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".renco").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$RENCO_HOME/logs/``."""
    return get_renco_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$RENCO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under RENCO_HOME or ``/tmp/renco-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    renco_home = get_renco_home()
    try:
        path.resolve().relative_to(renco_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/renco-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("renco-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "renco-agent", "backups", "profiles", ".worktrees",
...
# Paths under $RENCO_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so RENCO_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        renco_home = get_renco_home()
        for parent in ("cron", "cronjobs"):
            base = renco_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside RENCO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under RENCO_HOME, but never recurse into known
    # durable state trees.  Some installs place the Renco checkout, venv,
    # and desktop build under RENCO_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    renco_home = get_renco_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in renco_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    renco_home = get_renco_home()
    try:
        rel = path.resolve().relative_to(renco_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "renco-agent",
        ...
        # Path isn't under RENCO_HOME (e.g. /tmp/renco-*) — fall through.
        pass
