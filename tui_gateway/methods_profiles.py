...
handlers delegate to the same `renco_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Renco profiles (name, path, model, description, skill count).
    ...
    def _latest_profile_session_rows(profile_path):
        ...
        sessions never surface in conversation lists (renco-agent#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's renco-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``renco setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (renco_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local RENCO_HOME override — the same
        ...
        try:
            ...
            token = set_renco_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from renco_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from renco_cli.config import load_config_readonly, read_user_config_raw
            from renco_cli.web_routers.profiles import _write_profile_model
            from renco_constants import (
                reset_renco_home_override,
                set_renco_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the RENCO_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from renco_cli.skills_config import get_disabled_skills
            ...
            # Toolsets: the same filtered universe the `renco tools`
            ...
            # composites (renco-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from renco_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `renco tools` / Settings is where
            ...
            description = ""
            try:
                from renco_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from renco_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from renco_cli.skills_config import save_disabled_skills
