...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: son-of-anton)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: son-of-anton)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $SON_OF_ANTON_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "son-of-anton").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Son of Anton can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``son-of-anton memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'son-of-anton memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SON_OF_ANTON_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_son_of_anton_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "son-of-anton": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "son-of-anton"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Son of Anton config."""
    profile = config.get("profile", "son-of-anton")
    return str(profile or "son-of-anton")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Son of Anton .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``son-of-anton-{user}`` with no user becomes ``son-of-anton``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Son of Anton Agent and the User"
    ...
    def save_config(self, values, son_of_anton_home):
        """Write config to $SON_OF_ANTON_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(son_of_anton_home) / "hindsight"
    ...
    def post_setup(self, son_of_anton_home: str, config: dict) -> None:
        ...
        from son_of_anton_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        # data-volume target instead of writing to /opt/son-of-anton (NS-605).
        ...
        provider_config.setdefault("bank_id", "son-of-anton")
        ...
        self.save_config(provider_config, son_of_anton_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(son_of_anton_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(son_of_anton_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Son of Anton starter template (best-effort)."""
        from son_of_anton_cli.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "son-of-anton")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "son-of-anton"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: son-of-anton-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Son of Anton Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "son-of-anton"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "son-of-anton"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "son-of-anton", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "son-of-anton")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Son of Anton Agent and the User")
        ...
        # Son of Anton emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Son of Anton as a non-root user, or switch "
                    "to cloud / local_external mode via 'son-of-anton memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Son of Anton get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See ewtodd/son-of-anton#5820.
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See son-of-anton#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Son of Anton created/used that client on the shared async loop,
