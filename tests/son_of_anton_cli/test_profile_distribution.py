"""Tests for son_of_anton_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/son_of_anton_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_son_of_anton_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Son of Anton"):
                check_son_of_anton_requires(spec, cur)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from son_of_anton_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_son_of_anton_requires(self, profile_env, monkeypatch):
        # Pin current Son of Anton version to something well below the requirement
        import son_of_anton_cli
        monkeypatch.setattr(son_of_anton_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            son_of_anton_requires=">=99.0.0",
        )
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "son_of_anton_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from son_of_anton_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from son_of_anton_cli.profiles import create_profile, list_profiles, get_profile_dir
