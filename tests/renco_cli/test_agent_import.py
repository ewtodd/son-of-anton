"""Tests for renco_cli.agent_import — ``renco import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real Renco stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/renco_cli/test_profiles.py:
Path.home() and RENCO_HOME are redirected to tmp_path so nothing touches
the real ~/.renco.
"""
...
from renco_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and RENCO_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, RENCO_HOME -> tmp/.renco."""
...
@pytest.fixture()
def renco_home(profile_env):
    return profile_env / ".renco"
...
def run_import(agent, source, renco_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=renco_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, renco_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, renco_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, renco_home):
        before = snapshot_tree(renco_home)
        report = run_import("claude-code", claude_tree, renco_home, execute=False)
        assert snapshot_tree(renco_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, renco_home):
        preview = run_import("claude-code", claude_tree, renco_home, execute=False)
        applied = run_import("claude-code", claude_tree, renco_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, renco_home):
        return run_import("claude-code", claude_tree, renco_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, renco_home):
        config = yaml.safe_load((renco_home / "config.yaml").read_text())
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, renco_home):
        return run_import("codex", codex_tree, renco_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, renco_home):
        config = yaml.safe_load((renco_home / "config.yaml").read_text())
    ...
    def test_skill_copied(self, report, renco_home):
        assert (renco_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, renco_home):
        run_import("claude-code", claude_tree, renco_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in renco_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, renco_home):
        run_import("codex", codex_tree, renco_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in renco_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, renco_home):
        report = run_import("claude-code", claude_tree, renco_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, renco_home):
        run_import("claude-code", claude_tree, renco_home, execute=True)
        config = yaml.safe_load((renco_home / "config.yaml").read_text())
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, renco_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, renco_home, execute=True)
        ...
        assert "still importable" in (
            renco_home / "memories" / "MEMORY.md").read_text()


    def test_empty_tree_all_skipped(self, profile_env, renco_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, renco_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, renco_home):
        dest = renco_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, renco_home):
        run_import("claude-code", claude_tree, renco_home, execute=True)
        first = (renco_home / "memories" / "MEMORY.md").read_text()
        report = run_import("claude-code", claude_tree, renco_home, execute=True)
        assert (renco_home / "memories" / "MEMORY.md").read_text() == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, renco_home):
        memory = renco_home / "memories" / "MEMORY.md"
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, renco_home):
        return renco_home / "config.yaml"
    ...
    def test_malformed_config_is_left_byte_identical(
            self, claude_tree, renco_home, config_path):
    ...
    def test_malformed_config_records_an_error_not_an_import(
        ...
        for kind in CONFIG_WRITING_KINDS:
            ...
            assert "renco config edit" in reason
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, renco_home,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_import_preserves_every_pre_existing_config_key(
        ...
        assert merged["model"] == "renco-4-405b"
    ...
    def test_config_write_is_atomic_and_leaves_no_temp_files(
        ...
        leftovers = [p.name for p in renco_home.glob(".tmp*")]
        assert leftovers == []

    def test_symlinked_config_stays_a_symlink(
            self, claude_tree, renco_home, tmp_path, config_path):
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, renco_home, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from renco_cli import agent_import
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, renco_home, capsys):
        ...
        assert not (renco_home / "memories" / "MEMORY.md").exists()
        assert not (renco_home / "skills" / "claude-code-imports").exists()
        config_text = (renco_home / "config.yaml").read_text(encoding="utf-8") \
            if (renco_home / "config.yaml").exists() else ""
