"""Tests for `son-of-anton chat --safe-mode` isolation."""
...
_VARS = ("SON_OF_ANTON_SAFE_MODE", "SON_OF_ANTON_IGNORE_USER_CONFIG", "SON_OF_ANTON_IGNORE_RULES")
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["SON_OF_ANTON_SAFE_MODE"] == "1"
        assert os.environ["SON_OF_ANTON_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["SON_OF_ANTON_IGNORE_RULES"] == "1"
        return True
...
def test_plugin_discovery_skipped(monkeypatch):
    monkeypatch.setenv("SON_OF_ANTON_SAFE_MODE", "1")
