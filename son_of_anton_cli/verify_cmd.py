"""``son-of-anton verify`` — detect a project's run recipe and smoke-test it.
...
``.son-of-anton/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``son-of-anton verify`` satisfies
...
def _record_evidence(root: Path, recipe, result, *, partial: bool) -> None:
    ...
    try:
        ...
        record_verify_run(
            root=root,
            session_id=os.environ.get("SON_OF_ANTON_SESSION_ID"),
            ok=result.ok,
            command="son-of-anton verify",
