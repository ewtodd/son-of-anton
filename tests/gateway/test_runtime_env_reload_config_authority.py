...
SON_OF_ANTON_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env SON_OF_ANTON_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``son-of-anton config get terminal.backend`` still says local.
    """
    son_of_anton_home = tmp_path / ".son-of-anton"
    son_of_anton_home.mkdir()
    (son_of_anton_home / "config.yaml").write_text(
    ...
    (son_of_anton_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
