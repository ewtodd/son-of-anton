"""Tests for son_of_anton_cli.security_advisories.
...
import son_of_anton_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect SON_OF_ANTON_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full son_of_anton_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "son_of_anton_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
