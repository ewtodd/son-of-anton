"""``son-of-anton doctor --live`` — opt-in bounded real-call tool-backend probes.
...
- **Opt-in only.** These probes make real (cheap, metadata/read-only) network
  ...
  passes ``son-of-anton doctor --live``.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from son_of_anton_cli.doctor import SON_OF_ANTON_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (SON_OF_ANTON_HOME / "node" / "bin",
                          SON_OF_ANTON_HOME / "node",
                          SON_OF_ANTON_HOME / "node_modules" / ".bin"):
    ...
    # son_of_anton_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``son-of-anton mcp test``.
    """
