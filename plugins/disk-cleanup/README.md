...
Auto-tracks and cleans up ephemeral files created during Renco Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$RENCO_HOME` and `/tmp/renco-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `RENCO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `RENCO_HOME` or `/tmp/renco-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$RENCO_HOME/disk-cleanup/` is itself excluded
- `$RENCO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
