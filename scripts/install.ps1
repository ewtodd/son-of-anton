# ============================================================================
# Renco Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$RencoHome = $(if ($env:RENCO_HOME) { $env:RENCO_HOME } else { "$env:LOCALAPPDATA\renco" }),
    [string]$InstallDir = $(if ($env:RENCO_HOME) { "$env:RENCO_HOME\renco-agent" } else { "$env:LOCALAPPDATA\renco\renco-agent" }),
    ...
    # builds apps/desktop into a launchable Renco.exe.
    #
    # Why opt-in:
    #   * Renco-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Renco.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Renco.exe
    ...
    #     `renco desktop` already builds on demand.
...
# RENCO_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[renco] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'RencoInstall.LongPath').Type) {
            Add-Type -Namespace 'RencoInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [RencoInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -RencoHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('RencoHome')) {
    $RencoHome = ConvertTo-LongPath $RencoHome
} else {
    $RencoHome = ConvertTo-LongPath $(
        if ($env:RENCO_HOME) { $env:RENCO_HOME } else { "$env:LOCALAPPDATA\renco" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:RENCO_HOME) { "$env:RENCO_HOME\renco-agent" } else { "$env:LOCALAPPDATA\renco\renco-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: RencoHome=$RencoHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:ewtodd/renco.git"
$RepoUrlHttps = "https://github.com/ewtodd/renco.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Renco Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $RencoHome)) {
        New-Item -ItemType Directory -Force -Path $RencoHome | Out-Null
    }
    $envFile = Join-Path $RencoHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `renco update`'s npx
    ...
    $prefixDir = Join-Path $RencoHome "node"
...
function Install-Uv {
    # Renco owns its own uv at $RencoHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (renco_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `renco update` stay in sync.
    $managedUv = Join-Path $RencoHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $RencoHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $RencoHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $RencoHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $RencoHome "bin"
        ...
        # (renco_cli/managed_uv.py looks only at $RencoHome\bin\uv.exe).
...
# Put the Renco-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone renco-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Renco-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Renco-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("renco-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and RENCO_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Renco-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Renco setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Renco uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\renco\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Renco needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Renco copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\renco\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``RENCO_GIT_BASH_PATH`` (User scope) so Renco can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Renco-managed PortableGit install instead..."
    }

    # Download PortableGit into $RencoHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $RencoHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Renco features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Renco needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``RENCO_GIT_BASH_PATH`` (User env scope) so Renco can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $RencoHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $RencoHome\git\usr\bin\bash.exe
    $candidates += "$RencoHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$RencoHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("RENCO_GIT_BASH_PATH", $candidate, "User")
            $env:RENCO_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set RENCO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Renco may not find Git Bash."
    Write-Info "If needed, set RENCO_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Renco requires Node >=26)"
    }

    # Prefer a Renco-managed Node from a previous run over a too-old system one.
    $managedNode = "$RencoHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$RencoHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$RencoHome\node"
        Write-Success "Node.js $version found (Renco-managed)"
        ...
        Update-ManagedNpm "$RencoHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Renco-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $RencoHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $RencoHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\renco-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$RencoHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$RencoHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$RencoHome\node") {
                    try {
                        Rename-Item "$RencoHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Renco-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$RencoHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $RencoHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\renco-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `renco
                    # update` path (#4735).
                    ...
                    $stashName = "renco-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. renco-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``renco update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Renco behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running renco processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/ewtodd/renco/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/ewtodd/renco/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/ewtodd/renco/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\renco-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\renco-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `renco update` (see the notes at the shared clone-path
    ...
    # next `renco update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Renco-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running renco process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running renco processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Renco_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (renco.exe) plus its child tree.
            & taskkill /F /T /IM renco.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM renco.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m renco_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not renco.exe.
            ...
            # Renco children re-exec through .renco-runtime, so killing only
            ...
            # user (see renco_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Renco " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `renco update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Renco (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `renco.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install renco-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and renco.exe broken with
    ...
    # here, not 30 seconds later when the user runs `renco`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Renco processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Renco processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Renco and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register renco.exe in dist-info/RECORD but fail to
        ...
        # `renco` command while renco-agent.exe / renco-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m renco_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `renco dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\renco_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `renco dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `renco dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: renco_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up renco command..."
    
    if ($NoVenv) {
        $rencoBin = "$InstallDir"
    } else {
        # Expose ONLY the renco launchers on PATH -- never the whole
        ...
        # (#83797): unrelated projects start resolving python to Renco'
        # runtime interpreter. A dedicated bin dir with copies of the
        # launcher exes keeps `renco` globally available without
        # shadowing anything. (Launcher exes embed the venv interpreter
        # path, so they work from any location and survive updates.)
        $rencoBin = "$InstallDir\bin"
        New-Item -ItemType Directory -Force -Path $rencoBin | Out-Null
        foreach ($launcher in @("renco.exe", "renco-acp.exe")) {
            $src = "$InstallDir\venv\Scripts\$launcher"
            if (Test-Path $src) {
                Copy-Item -Force $src "$rencoBin\$launcher"
            }
        }
    }
    ...
    if ((-not $NoVenv) -and $currentPath -like "*$legacyBin*") {
        ...
        $currentPath = $cleaned
        Write-Info "Removed legacy venv\Scripts from user PATH (kept renco via $rencoBin)"
    }
    
    if ($currentPath -notlike "*$rencoBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$rencoBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $rencoBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set RENCO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\renco instead
    # of the Unix default ~/.renco
    $currentRencoHome = [Environment]::GetEnvironmentVariable("RENCO_HOME", "User")
    if (-not $currentRencoHome -or $currentRencoHome -ne $RencoHome) {
        [Environment]::SetEnvironmentVariable("RENCO_HOME", $RencoHome, "User")
        Write-Success "Set RENCO_HOME=$RencoHome"
    }
    $env:RENCO_HOME = $RencoHome
    
    # Update current session
    $env:Path = "$rencoBin;$env:Path"
    
    Write-Success "renco command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.renco-bootstrap-complete which tells the Renco
    ...
    $markerPath = Join-Path $InstallDir ".renco-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the RENCO_HOME directory structure ($RencoHome, default %LOCALAPPDATA%\renco)
    New-Item -ItemType Directory -Force -Path "$RencoHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RencoHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RencoHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RencoHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RencoHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RencoHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RencoHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RencoHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RencoHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$RencoHome\.env"
    ...
    # Create config.yaml
    $configPath = "$RencoHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Renco's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$RencoHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in renco_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $RencoHome"
    
    # Seed bundled skills into $RencoHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $RencoHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $RencoHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$RencoHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $RencoHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Renco-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'renco setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\renco-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\renco-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\renco-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `renco tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $RencoHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Renco' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Renco' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'renco tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `renco
...
# paths still lazy-install via install_cua_driver() (renco_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `renco computer-use install` runs. Bounded
        ...
        # Renco' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'renco tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: renco computer-use install"
            }
        } else {
...
# the final `electron` -> `Renco` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Renco.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_renco_desktop command
    # resolves apps/desktop/release/win-unpacked/Renco.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Renco-managed
    ...
    # The --dir mode produces an unpacked Renco.exe in
    ...
    # are NOT trying to work around). The Renco icon + product name are
    # stamped onto Renco.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\renco-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Renco.exe",
        "$desktopDir\release\win-arm64-unpacked\Renco.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Renco.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Renco icon + identity are stamped onto Renco.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, renco-agent#38216).
    ...
    #    Renco.exe. We deliberately do NOT point them at `renco desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Renco.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Renco.lnk')
        )
        ...
        # with the Renco icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.renco/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'renco setup'."
        return
    }
    ...
    # Run renco setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m renco_cli.main setup
    } else {
        python -m renco_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $rencoCmd = "$InstallDir\venv\Scripts\renco.exe"
    if (-not (Test-Path $rencoCmd)) {
        $rencoCmd = "renco"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$RencoHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'renco whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: renco gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$RencoHome\logs\gateway.log"
            Start-Process -FilePath $rencoCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$RencoHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: renco gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: renco gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$RencoHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$RencoHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$RencoHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$RencoHome\renco-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   renco              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   renco setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   renco config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   renco config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   renco gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   renco update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Renco repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Renco-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Renco to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://renco-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
