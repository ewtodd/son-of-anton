...
it is automatically downloaded from GitHub releases to $RENCO_HOME/bin/tirith.
...
def _get_renco_home() -> str:
    """Return the Renco home directory, respecting RENCO_HOME env var."""
    return str(get_renco_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_renco_home(), ".tirith-install-failed")
...
def _renco_bin_dir() -> str:
    """Return $RENCO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_renco_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $RENCO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_renco_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $RENCO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $RENCO_HOME/bin/tirith
    ...
    renco_bin = os.path.join(_renco_bin_dir(), "tirith")
    if os.path.isfile(renco_bin) and os.access(renco_bin, os.X_OK):
        _resolved_path = renco_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return renco_bin
