...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in son_of_anton_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # Son of Anton uses. Removed here. If OpenAI re-enables them on Codex backend,
...
_FORWARD_COMPAT_TEMPLATE_MODELS: List[tuple[str, tuple[str, ...]]] = [
    ...
    # entitlement; Son of Anton does not.
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while Son of Anton openai-codex talks to the same
