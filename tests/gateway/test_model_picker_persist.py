...
closure the PR changed, against a real temp ``RENCO_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "renco_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "renco_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_renco_home() / config.yaml`` — point it here.
    monkeypatch.setattr("renco_constants.get_renco_home", lambda: renco_home)
    monkeypatch.setattr("renco_cli.config.get_renco_home", lambda: renco_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_renco_home", default_home)
    _stub_picker_dependencies(monkeypatch)
