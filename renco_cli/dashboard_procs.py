"""Dashboard process-hygiene helpers — extracted from ``renco_cli/main.py``.
...
``_detect_concurrent_renco_instances``) are lifted verbatim. References to
helpers that STAY in ``renco_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``renco_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``renco_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``renco_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``renco dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``renco update`` replaces files on disk, the running
    ...
    ``renco-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Renco Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``renco serve`` as
    ...
    environment variable ``RENCO_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`renco serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "renco.*dashboard"`) keeps us consistent
            # with `renco_cli.gateway._scan_gateway_pids` and avoids the
...
def _renco_home_for_pid(pid: int) -> str | None:
    """Best-effort ``RENCO_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("RENCO_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"RENCO_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Renco Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``renco update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], renco_home: str | None = None
) -> str:
    """Stable owner key: ``RENCO_HOME`` when known, else ``--profile`` / ``-p``.

    ``RENCO_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.renco`` roots) keep a resolved ``home:`` key so
    ...
    if renco_home:
        try:
            home_path = Path(renco_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(renco_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(renco_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``renco update``.

    Each candidate is ``(pid, argv, renco_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``RENCO_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``RENCO_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``renco update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, renco_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, renco_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``renco dashboard`` / ``renco serve`` processes.

    Called at the end of ``renco update`` (default ``reason``) and also
    from ``renco dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``renco update`` path), a detected
    ``renco-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``renco-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``renco update``'s systemd
    fleet-restart loop, which restarts ``renco-serve*`` units before this
    ...
    # When the Renco Desktop Electron app spawns this dashboard as a
    # backend child, it sets RENCO_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("RENCO_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `renco update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot RENCO_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _renco_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. renco update's
    ...
    #    remote backend (renco serve) under Restart=on-failure never comes
...
def _detect_concurrent_renco_instances(
    ...
    The Renco Desktop Electron app spawns ``renco.EXE`` as a backend child,
    so during ``renco update`` the user-invoked process and the desktop's
    ...
    shims (``renco.exe`` / ``renco-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``renco update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``renco.exe``) is a separate process from the Python
    ...
    for shim in _m()._renco_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated renco.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``renco update``
    ...
    #      renco.exe sitting *under* a non-Renco parent (e.g. a Renco
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        renco serve --host 127.0.0.1 --port 0
        renco serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "renco" not in cmd and "renco_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (RENCO_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("RENCO_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``renco serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``renco serve`` on a Mac
...
def _renco_home_dir() -> Path:
    """Resolved Renco home (RENCO_HOME override or ~/.renco)."""
    override = os.environ.get("RENCO_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "rencoPath", "rencoHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated RENCO_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{renco_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _renco_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local renco serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    lock_owned_pids_fn=None,
) -> dict[str, list]:
    """Kill leftover Desktop-local ``renco serve`` backends with no parent.
    ...
    running under RENCO_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never RENCO_DESKTOP_CHILD_PID entries
