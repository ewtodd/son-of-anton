"""Tests for son_of_anton_cli.update_inventory — the plan phase (#91277 Phase 2)."""
...
import son_of_anton_cli.update_inventory as ui
...
def fleet(monkeypatch, tmp_path):
    ...
    import re
    monkeypatch.setattr("son_of_anton_cli.profiles._get_default_son_of_anton_home", lambda: default_home)
    monkeypatch.setattr("son_of_anton_cli.profiles._get_profiles_root", lambda: default_home / "profiles")
    monkeypatch.setattr("son_of_anton_cli.profiles._PROFILE_ID_RE", re.compile(r"^[a-z0-9][a-z0-9_-]*$"), raising=False)
    monkeypatch.setattr("gateway.status._pid_exists", lambda pid: pid in (100, 200))
    monkeypatch.setattr("son_of_anton_cli.gateway._get_service_pids", lambda all_profiles=False: {100})
    monkeypatch.setattr("son_of_anton_cli.gateway.supports_systemd_services", lambda: True)
    monkeypatch.setattr("son_of_anton_cli.gateway.find_profile_gateway_processes", lambda exclude_pids=None: [])
    monkeypatch.setattr(
        "son_of_anton_cli.build_info.get_code_identity",
        lambda refresh=False: {"sha": "a" * 40, "short_sha": "a" * 8, "version": "1.0", "source": "git"},
    )
    monkeypatch.setattr("son_of_anton_cli.config.detect_install_method", lambda *a, **k: "git")
    monkeypatch.setattr("son_of_anton_cli.config.get_managed_system", lambda: None)
    return tmp_path
...
class TestCollectInventory:
    def test_two_profile_fleet(self, fleet):
        ...
        assert "son-of-anton -p work gateway restart" in by_profile["work"].restart_via

    def test_docker_install_not_updatable_in_place(self, fleet, monkeypatch):
        monkeypatch.setattr("son_of_anton_cli.config.detect_install_method", lambda *a, **k: "docker")
        monkeypatch.setattr(
            "son_of_anton_cli.config.recommended_update_command_for_method",
            lambda m: "docker pull nousresearch/son-of-anton:latest",
        )
        plan = ui.collect_runtime_inventory()
    ...
    def test_pid_file_fallback_covers_unstamped_profiles(self, fleet, monkeypatch):
        ...
        from son_of_anton_cli.gateway import ProfileGatewayProcess

        monkeypatch.setattr(
            "son_of_anton_cli.gateway.find_profile_gateway_processes",
    ...
    def test_never_raises_when_everything_fails(self, monkeypatch):
        ...
        for target in (
            "son_of_anton_cli.config.detect_install_method",
            "son_of_anton_cli.build_info.get_code_identity",
            "son_of_anton_cli.profiles._get_default_son_of_anton_home",
            "son_of_anton_cli.gateway._get_service_pids",
            "son_of_anton_cli.gateway.find_profile_gateway_processes",
        ):
...
class TestReceiptIntegration:
    def test_plan_recorded_into_active_receipt(self, fleet, monkeypatch, tmp_path):
        import son_of_anton_cli.update_receipt as ur

        home = tmp_path / "receipt_home"
        home.mkdir()
        monkeypatch.setattr("son_of_anton_cli.config.get_son_of_anton_home", lambda: home, raising=False)
        ur._current = None
