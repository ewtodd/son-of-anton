"""Tests for the core Relay-managed Renco tool adapter."""
...
def test_request_rewrite_reaches_authorized_callback_once(relay_turn):
    ...
    relay.intercepts.register_tool_request(
        "renco-test-tool-request", 1, False, rewrite_request
    )
    relay.intercepts.register_tool_execution(
        "renco-test-tool-execution", 1, wrap_execution
    )
    try:
        ...
        relay.intercepts.deregister_tool_execution("renco-test-tool-execution")
        relay.intercepts.deregister_tool_request("renco-test-tool-request")
