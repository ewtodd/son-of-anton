"""``renco gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.renco/auth.json``) — the same path ``renco dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.renco/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `renco auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from renco_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `renco gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.renco/.env (0600 dir) and never logs.
    to_write = {
    ...
    # process is launched under (load_renco_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in renco_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_renco_root()).resolve()
        home = Path(get_renco_home()).resolve()
        try:
