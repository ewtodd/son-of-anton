"""Anthropic Messages API adapter for Renco Agent.

Translates between Renco's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in renco_cli/runtime_provider.py.
    """
...
# Renco effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to renco's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to renco's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + RencoAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://renco-agent.nousresearch.com")
        headers.setdefault("X-Title", "Renco Agent")
        headers.setdefault("User-Agent", f"RencoAgent/{_RENCO_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Renco loads that into the process env from
    # ``~/.renco/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Renco notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Renco historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``renco models``) never mutates ``~/.renco/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Renco)
      ...
      5. Anthropic credential_pool OAuth entry (~/.renco/auth.json)
    ...
    # 1. Renco-managed OAuth/setup token env var
    ...
    # 5. Renco credential_pool OAuth entry.
...
# ── Renco-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.renco/.anthropic_oauth.json (our own file).
...
def _get_renco_oauth_file() -> Path:
    return get_renco_home() / ".anthropic_oauth.json"
...
def run_renco_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Renco-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Renco with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from renco_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_renco_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Renco-managed OAuth credentials from ~/.renco/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Renco OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See renco-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See renco-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See renco-agent#13848 (Kimi) and
    renco-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Renco Agent", "Claude Code")
                text = text.replace("Renco agent", "Claude Code")
                text = text.replace("renco-agent", "claude-code")
        ...
        #      a) bare Renco-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Renco surfaces in its CLI. We
