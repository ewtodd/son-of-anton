...
| | |
|---|---|
| Source | Optional — install with `renco skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${RENCO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${RENCO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${RENCO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client renco
```
...
```bash
PUBLISH="${RENCO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client renco
```
...
```bash
DRIVE="${RENCO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `renco`)    |
