...
The design is **Renco-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
renco skills install official/security/unbroker
```

Then start a new Renco session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Renco home directory (`0600`, optionally
