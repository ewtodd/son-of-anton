...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'renco-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/renco-runner-test',
    sourceRepoRoot: null,
    rencoHome: '/tmp/renco-runner-test',
    logRoot: '/tmp/renco-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'renco-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'renco-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/renco-agent',
      rencoHome: '/tmp/renco'
    }),
    ['--dir', '/tmp/renco-agent', '--renco-home', '/tmp/renco', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/renco-agent', '--renco-home', '/tmp/renco', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/renco',
      rencoHome: '/tmp/home'
    }),
    ['--dir', '/tmp/renco', '--renco-home', '/tmp/home', '--branch', 'main']
  )
})
