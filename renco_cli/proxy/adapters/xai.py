...
# xAI's public API is OpenAI-compatible for the endpoints Renco commonly
# uses. The Responses endpoint is included because Renco' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Renco-managed OAuth credentials."""

    auth_hint = "renco auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`renco auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`renco auth reset xai-oauth` or re-authenticate with "
                    "`renco auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `renco auth add xai-oauth --type oauth`."
            )
