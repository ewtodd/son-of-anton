"""Tests for renco_cli.build_info — baked-in build SHA resolution.

The build SHA is written by the Dockerfile's ``RENCO_GIT_SHA`` build-arg
into ``<project_root>/.renco_build_sha``.  These tests cover the read-side
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    ...
    from renco_cli import build_info

    missing = tmp_path / ".renco_build_sha"  # never created
...
def test_get_build_sha_respects_short_argument(tmp_path):
    ...
    sha_file = tmp_path / ".renco_build_sha"
