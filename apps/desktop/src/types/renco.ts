...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Renco doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in renco_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `renco model` uses — instead of
...
export type RencoConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `renco model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Renco simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (renco.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors renco_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Renco, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `renco tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Renco); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/renco/update/check` — the backend's own update state.
