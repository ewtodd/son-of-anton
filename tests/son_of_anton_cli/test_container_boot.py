"""Tests for son_of_anton_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $SON_OF_ANTON_HOME under tmp_path; no real
...
from son_of_anton_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running son-of-anton containers, those
    ...
    """
    monkeypatch.setattr(
        "son_of_anton_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under son_of_anton_home/profiles/<name>/."""
    p = son_of_anton_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `son-of-anton profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    SON_OF_ANTON_HOME root (the implicit default profile)."""
    if state is not None:
        (son_of_anton_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (son_of_anton_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (son_of_anton_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        son_of_anton_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_main_skips_reconcile_in_dashboard_container_s6v3(
    ...
    """
    from son_of_anton_cli import container_boot
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ...
            "top",
            "/opt/son-of-anton/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "0.0.0.0",
            "--port",
