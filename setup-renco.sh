...
# 5. Symlinks the 'renco' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Renco Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/renco-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink renco into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up renco command..."

RENCO_BIN="$SCRIPT_DIR/venv/bin/renco"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$RENCO_BIN" "$COMMAND_LINK_DIR/renco"
echo -e "${GREEN}✓${NC} Symlinked renco → $COMMAND_LINK_DISPLAY_DIR/renco"
...
# ============================================================================
# Seed bundled skills into ~/.renco/skills/
# ============================================================================

RENCO_SKILLS_DIR="${RENCO_HOME:-$HOME/.renco}/skills"
mkdir -p "$RENCO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.renco/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$RENCO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  renco status        # Check configuration"
if is_termux; then
    echo "  renco gateway       # Run gateway in foreground"
else
    echo "  renco gateway install # Install gateway service (messaging + cron)"
fi
echo "  renco cron list     # View scheduled jobs"
echo "  renco doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m renco_cli.main setup
fi
