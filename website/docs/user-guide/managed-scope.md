...
`~/.renco/config.yaml`, `~/.renco/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/renco`:

```text
/etc/renco/
├── config.yaml     # managed config layer (wins over ~/.renco/config.yaml)
└── .env            # managed env layer (wins over ~/.renco/.env + shell)
```
...
The location can be relocated with the `RENCO_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `RENCO_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Renco.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export RENCO_MANAGED_DIR=/opt/org/renco-policy
```

:::warning
A user who can set `RENCO_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `renco doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/renco/config.yaml` (managed) | `/etc/renco/.env` (managed) |
| 2 | `~/.renco/config.yaml` (user) | `~/.renco/.env` (user) |
...
```bash
renco config        # shows a header naming the managed source + the pinned keys
renco doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Renco refuses and names the source:

```bash
$ renco config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/renco/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `renco config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/renco

# Pin some config values for every user on this machine
sudo tee /etc/renco/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/renco/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/renco
sudo chmod 0644 /etc/renco/config.yaml /etc/renco/.env
```

Changes take effect on the next Renco start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`renco doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Renco as `root`), managed scope is advisory.
