import type { SkinBranding, SkinColors } from '@sonofanton/shared/skin'
...
// Mirrors @sonofanton/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @sonofanton/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Son of Anton navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Son of Anton look was never hand-authored: for years the
 ...
 *  answer when known (cached in SON_OF_ANTON_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.SON_OF_ANTON_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit SON_OF_ANTON_TUI_THEME / COLORFGBG signals above still win,
...
// `SON_OF_ANTON_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `SON_OF_ANTON_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `SON_OF_ANTON_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `SON_OF_ANTON_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Son of Anton palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.SON_OF_ANTON_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.SON_OF_ANTON_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.SON_OF_ANTON_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into SON_OF_ANTON_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into SON_OF_ANTON_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Son of Anton navy
  ...
  // the derived value already carries the identity seeds (e.g. Son of Anton navy).
  const activeRow =
