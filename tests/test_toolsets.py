...
class TestToolsetConsistency:
    ...
    def test_renco_platforms_share_core_tools(self):
        """All renco-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on renco-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["renco-cli", "renco-telegram", "renco-discord", "renco-whatsapp", "renco-slack", "renco-signal", "renco-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_renco_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("renco-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("renco-cli")
        second = resolve_toolset("renco-cli")
        ...
        assert (
            "renco-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("renco-cli", include_registry=False)
        second = resolve_toolset("renco-cli", include_registry=False)
        assert first == second
