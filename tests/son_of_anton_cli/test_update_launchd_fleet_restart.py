"""Regression for #41403 — ``son-of-anton update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.son-of-anton.gateway-<profile>`` services kept running pre-update
...
import son_of_anton_cli.profiles
...
from son_of_anton_cli.update_cmd import (
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed SON_OF_ANTON_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            ...
            "ai.son-of-anton.gateway-merit-ops",
            "ai.son-of-anton.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(son_of_anton_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        gui_loaded = {"ai.son-of-anton.gateway-a"}
        ...
        assert _locate_launchd_gateway_service("ai.son-of-anton.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.son-of-anton.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.son-of-anton.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.son-of-anton.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.son-of-anton.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.son-of-anton.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.son-of-anton.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.son-of-anton.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.son-of-anton.gateway", "ai.son-of-anton.gateway-a", "ai.son-of-anton.gateway-b"],
        )
        located = {
            "ai.son-of-anton.gateway": (f"gui/{UID}", 100),
            "ai.son-of-anton.gateway-a": (f"gui/{UID}", 200),
            "ai.son-of-anton.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through son_of_anton_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.son-of-anton.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.son-of-anton.gateway", current, "ai.son-of-anton.gateway-user-scoped"],
            located={
                "ai.son-of-anton.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.son-of-anton.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.son-of-anton.gateway",
            f"user/{UID}/ai.son-of-anton.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.son-of-anton.gateway",
            f"user/{UID}/ai.son-of-anton.gateway-user-scoped",
        ...
        assert restarted == [
            current,
            "ai.son-of-anton.gateway",
            "ai.son-of-anton.gateway-user-scoped",
        ]
        assert failed == []
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.son-of-anton.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.son-of-anton.gateway-a"],
            located={"ai.son-of-anton.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.son-of-anton.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.son-of-anton.gateway",
            labels=["ai.son-of-anton.gateway", "ai.son-of-anton.gateway-idle"],
            located={
                "ai.son-of-anton.gateway": (f"gui/{UID}", 100),
                "ai.son-of-anton.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.son-of-anton.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.son-of-anton.gateway",
            labels=["ai.son-of-anton.gateway", "ai.son-of-anton.gateway-dormant"],
            located={
                "ai.son-of-anton.gateway": (f"gui/{UID}", 100),
                "ai.son-of-anton.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.son-of-anton.gateway-dormant"]
        assert restarted == ["ai.son-of-anton.gateway", "ai.son-of-anton.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.son-of-anton.gateway",
            labels=["ai.son-of-anton.gateway", "ai.son-of-anton.gateway-a"],
            located={
                "ai.son-of-anton.gateway": (f"gui/{UID}", 100),
                "ai.son-of-anton.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.son-of-anton.gateway-a"]
        assert restarted == ["ai.son-of-anton.gateway", "ai.son-of-anton.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.son-of-anton.gateway",
                "ai.son-of-anton.gateway-bad",
                "ai.son-of-anton.gateway-good",
            ],
            located={
                "ai.son-of-anton.gateway": (f"gui/{UID}", 100),
                "ai.son-of-anton.gateway-bad": (f"gui/{UID}", 200),
                "ai.son-of-anton.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.son-of-anton.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.son-of-anton.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.son-of-anton.gateway-good"]
        assert restarted == ["ai.son-of-anton.gateway", "ai.son-of-anton.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.son-of-anton.gateway",
                "ai.son-of-anton.gateway-wedged",
                "ai.son-of-anton.gateway-after",
            ],
            located={
                "ai.son-of-anton.gateway": (f"gui/{UID}", 100),
                "ai.son-of-anton.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.son-of-anton.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.son-of-anton.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.son-of-anton.gateway-after"]
        assert restarted == ["ai.son-of-anton.gateway", "ai.son-of-anton.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.son-of-anton.gateway": (f"gui/{UID}", 100),
                "ai.son-of-anton.gateway-wedged": (f"gui/{UID}", 200),
                "ai.son-of-anton.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.son-of-anton.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.son-of-anton.gateway",
            labels=["ai.son-of-anton.gateway", "ai.son-of-anton.gateway-zombie"],
            located={
                "ai.son-of-anton.gateway": (f"gui/{UID}", 100),
                "ai.son-of-anton.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.son-of-anton.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.son-of-anton.gateway"]
        assert failed == ["ai.son-of-anton.gateway-zombie"]
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.son-of-anton.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
    ...
    def test_returns_false_when_pid_never_changes(self, monkeypatch):
        ...
        assert not gw._wait_for_launchd_service_pid(
            "ai.son-of-anton.gateway-x", old_pid=200, timeout=3.0, domain=f"gui/{UID}"
        )


class TestIncompleteWarningMentionsLaunchctl:
    def test_launchd_labels_get_launchctl_hint(self, capsys):
        _warn_incomplete_gateway_fleet_restart(["ai.son-of-anton.gateway-merit-ops"])
        out = capsys.readouterr().out
        assert "Update incomplete" in out
        assert "launchctl kickstart -k" in out

    def test_systemd_units_keep_systemctl_hint(self, capsys):
        _warn_incomplete_gateway_fleet_restart(["son-of-anton-gateway-coder"])
        out = capsys.readouterr().out
