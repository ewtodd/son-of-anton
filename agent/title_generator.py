...
# Renco' own machine-authored openers. A compaction handoff or a resumed
...
def _auto_title_enabled() -> bool:
    ...
    try:
        ...
        # from agent code paths where a module-level renco_cli import risks
...
def auto_title_session(
    ...
    post-``renco update`` stale-module window, where this function's lazy
    ...
    try:
        ...
        logger.warning(
            ...
            "restart the running Renco process): %s",
            e,
        )
...
def _is_real_user_turn(message: Any) -> bool:
    ...
    Renco persists a lot of machinery under ``role="user"`` — compaction
