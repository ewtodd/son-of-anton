...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    from son_of_anton_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "get_son_of_anton_home", lambda: tmp_path, raising=False)
    ...
    assert "Start with:   son-of-anton gateway" in output
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in son-of-anton status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.son-of-anton/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.son-of-anton/auth.json" in out
    ...
    def test_status_function_returns_none_does_not_crash(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: son-of-anton auth add xai-oauth)" in out


def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """son-of-anton status should surface freshest gateway last_active (#72016)."""
    from son_of_anton_cli import status as status_mod
    import son_of_anton_cli.auth as auth_mod
    import son_of_anton_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(son_of_anton_state, "SessionDB", _FakeDB)
