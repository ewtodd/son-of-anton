...
the legacy ``RENCO_LOCAL_STT_COMMAND`` escape hatch exists) or this PR's
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "transcription_tools.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "renco-agent-main"
...
assert (PR_DIR / "tools" / "transcription_tools.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a renco-agent checkout"
)
...
# Isolated RENCO_HOME so the config write is hermetic.
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'renco-agent-main' checkout pinned to "
