"""
Session-scoped context variables for the Son of Anton gateway.

Replaces the previous ``os.environ``-based session state
(``SON_OF_ANTON_SESSION_PLATFORM``, ``SON_OF_ANTON_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["SON_OF_ANTON_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("SON_OF_ANTON_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("SON_OF_ANTON_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("SON_OF_ANTON_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("SON_OF_ANTON_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("SON_OF_ANTON_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_TYPE: ContextVar = ContextVar("SON_OF_ANTON_SESSION_CHAT_TYPE", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("SON_OF_ANTON_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("SON_OF_ANTON_SESSION_THREAD_ID", default=_UNSET)
...
_SESSION_SCOPE_ID: ContextVar = ContextVar("SON_OF_ANTON_SESSION_SCOPE_ID", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("SON_OF_ANTON_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("SON_OF_ANTON_SESSION_ID", default=_UNSET)
# In-process UI session/window id for multi-session desktop/TUI hosts. This is
# intentionally separate from SON_OF_ANTON_SESSION_ID: the latter is the durable
...
_CRON_SESSION: ContextVar = ContextVar("SON_OF_ANTON_CRON_SESSION", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("SON_OF_ANTON_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("SON_OF_ANTON_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("SON_OF_ANTON_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("SON_OF_ANTON_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "SON_OF_ANTON_SESSION_PLATFORM": _SESSION_PLATFORM,
    "SON_OF_ANTON_SESSION_SOURCE": _SESSION_SOURCE,
    "SON_OF_ANTON_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "SON_OF_ANTON_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    "SON_OF_ANTON_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "SON_OF_ANTON_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "SON_OF_ANTON_SESSION_USER_ID": _SESSION_USER_ID,
    "SON_OF_ANTON_SESSION_USER_ID_ALT": _SESSION_USER_ID_ALT,
    "SON_OF_ANTON_SESSION_USER_NAME": _SESSION_USER_NAME,
    "SON_OF_ANTON_SESSION_SCOPE_ID": _SESSION_SCOPE_ID,
    "SON_OF_ANTON_SESSION_KEY": _SESSION_KEY,
    ...
    "SON_OF_ANTON_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}
...
def set_current_session_id(session_id: str) -> None:
    ...
    ``SON_OF_ANTON_SESSION_ID`` for the rest of the process — leaking the child id
    ...
    # parent's session identity. See SonOfAntonPRDelegationSessionContext task.
    try:
    ...
    os.environ["SON_OF_ANTON_SESSION_ID"] = session_id
...
def set_session_vars(
    ...
    ``os.environ["SON_OF_ANTON_CRON_SESSION"]`` fallback, ``"1"`` marks a cron job,
...
def reset_session_vars() -> None:
    ...
    *A's* ``SON_OF_ANTON_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``SON_OF_ANTON_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``SON_OF_ANTON_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("SON_OF_ANTON_SESSION_*", default)``.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``SON_OF_ANTON_PLATFORM``, then the session platform, then the session
    source, and reports messaging when any of them names a surface outside
    :data:`NON_MESSAGING_SESSION_SURFACES`.
    """
    import os

    platform = os.getenv("SON_OF_ANTON_PLATFORM") or get_session_env("SON_OF_ANTON_SESSION_PLATFORM", "")
...
def declare_stateless_channel() -> None:
    ...
    See ewtodd/son-of-anton#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    or a one-shot runner that exits after its final response (``son-of-anton -z``,
    cron — see :func:`declare_stateless_channel`) — and dispatcher-spawned
    Kanban workers (identified by ``SON_OF_ANTON_KANBAN_TASK``), which are one-shot
