...
Build the real image and verify: a config.yaml present in $RENCO_HOME
is migrated by docker_config_migrate.py on boot, running as the renco
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $RENCO_HOME must be migrated on boot by
    docker_config_migrate.py, running as the renco user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $RENCO_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/renco/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by renco (migration ran as renco)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "renco", (
        f"config.yaml not owned by renco (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
