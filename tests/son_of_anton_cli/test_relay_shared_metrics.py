"""Focused tests for the Son of Anton shared-metrics durable store."""
...
from son_of_anton_cli.observability import shared_metrics as shared_metrics_module
from son_of_anton_cli.observability.shared_metrics import SharedMetricsStore
from son_of_anton_cli.observability.shared_metrics_contract import (
...
SCHEMA_PATH = (
    ...
    / "observability"
    / "schemas"
    / "son-of-anton.shared_metrics.v2.schema.json"
)
LEGACY_SCHEMA_PATH = SCHEMA_PATH.with_name("son-of-anton.shared_metrics.v1.schema.json")
...
def _resource(
    son_of_anton_version: str = "test-version",
    *,
    os_family: str = "linux",
    ...
    return {
        ...
        "son_of_anton_version": son_of_anton_version,
...
def test_model_call_counter_survives_restart_and_exports_only_new_deltas(tmp_path):
    ...
    assert first_package["schema_version"] == "son-of-anton.shared_metrics.v2"
...
def test_package_schema_matches_the_model_call_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "son-of-anton.shared_metrics.v2"
...
def test_client_active_mark_accepts_only_an_empty_allowlisted_payload():
    event = SimpleNamespace(
        ...
        scope_category=None,
        metadata={
            "son-of-anton.metrics.schema_version": "son-of-anton.metrics.event.v2",
        },
        data={},
    )
    ...
    wrong_schema.metadata["son-of-anton.metrics.schema_version"] = "unknown"
...
def test_v1_package_schema_retains_the_legacy_model_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "son-of-anton.shared_metrics.v1"
...
def test_auxiliary_logical_scope_projects_one_normalized_terminal_route():
    event = SimpleNamespace(
        ...
        metadata={
            ...
            "son-of-anton.call_role": "auxiliary:compression",
        },
    )
    ...
    event.metadata["son-of-anton.call_role"] = "primary"
...
def test_tool_subscriber_contract_accepts_only_bounded_events():
    terminal = SimpleNamespace(
        ...
        category="tool",
        category_profile={},
        name="son-of-anton.tool_call",
        scope_category="end",
        metadata={"son-of-anton.metrics.schema_version": "son-of-anton.metrics.event.v2"},
        data={
    ...
    approval = SimpleNamespace(
        ...
        category=None,
        category_profile=None,
        name="son-of-anton.tool_approval",
        scope_category=None,
        metadata={"son-of-anton.metrics.schema_version": "son-of-anton.metrics.event.v2"},
...
def test_skill_subscriber_contract_accepts_only_bounded_marks():
    ...
    lifecycle = SimpleNamespace(
        ...
        category=None,
        category_profile=None,
        name="son-of-anton.skill.lifecycle",
        scope_category=None,
        metadata=metadata,
    ...
    assert skill_counter(load) == ("son-of-anton.skill.load.count", load.data)
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        connection.execute(
            ...
            CREATE TABLE counter_aggregates (
                ...
                son_of_anton_version TEXT NOT NULL,
...
def test_store_exports_task_started_and_terminal_counters(tmp_path):
    ...
    store.record_counter("son-of-anton.task_run.finished", terminal, _resource())
...
def test_retention_prunes_only_expired_exported_history(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        connection.execute(
            ...
            WHERE son_of_anton_version = 'expired-version'
            """
        )
        connection.execute(
            """
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        aggregate_versions = {
            ...
            for row in connection.execute(
                "SELECT son_of_anton_version FROM counter_aggregates"
            ).fetchall()
        }
