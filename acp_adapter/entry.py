"""CLI entry point for the renco-agent ACP adapter.

Loads environment variables from ``~/.renco/.env``, configures logging
...
try:
    ...
    # shadowing Renco's own modules — ``renco acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from RENCO_HOME (default ``~/.renco``)."""
    from renco_cli.env_loader import load_renco_dotenv

    renco_home = get_renco_home()
    loaded = load_renco_dotenv(renco_home=renco_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", renco_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="renco-acp",
        description="Run Renco Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Renco version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Renco provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.renco/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import RencoACPAgent  # noqa: F401

    print("Renco ACP check OK")


def _run_setup() -> None:
    from renco_cli.main import main as renco_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "renco", "model"]
        renco_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting renco-agent ACP adapter")
    ...
    import acp
    from .server import RencoACPAgent
    ...
    if os.environ.get("RENCO_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
