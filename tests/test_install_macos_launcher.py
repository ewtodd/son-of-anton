"""Regression coverage for the user-facing macOS Son of Anton launcher."""
...
def test_venv_launcher_bypasses_uv_console_script_that_requires_realpath(tmp_path: Path) -> None:
    """Stock macOS must start Son of Anton even when its uv console script needs realpath."""
    ...
    (install_dir / "son-of-anton").write_text("# source entrypoint\n", encoding="utf-8")
    _make_executable(
        venv_bin / "son-of-anton",
        "#!/bin/sh\n"
        f'PATH="{minimal_path}"\n'
    ...
    completed = subprocess.run(
        [command_dir / "son-of-anton", "--version"],
