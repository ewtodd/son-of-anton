...
/// read side does not see EOF until the last of them closes it. `renco update`
...
/// stranded with it.
///
/// Same bound `Invoke-RencoStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `renco_home_override` propagates to the child as $RENCO_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, renco_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = renco_home_override {
        cmd.env("RENCO_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, renco_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = renco_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_renco_home() {
    ...
    /// child. `renco update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
