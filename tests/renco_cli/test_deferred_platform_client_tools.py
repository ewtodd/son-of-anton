...
loader so ``renco chat`` doesn't import ~20 gateway SDKs. The a2a plugin ships
...
``renco tools`` checklist. The same tools worked in gateway/web processes only
...
def _write_platform_plugin(
    ...
    *,
    ...
    """
    from renco_cli.plugins import PluginManifest
...
def clean_registry():
    ...
    ``probeplat`` name, so the imported ``renco_plugins.*`` modules have to go
    ...
    for name in set(sys.modules) - before_modules:
        if name.startswith("renco_plugins."):
...
class TestA2AClientToolsInCliProcess:
    ...
    def test_a2a_appears_in_the_renco_tools_checklist(self):
    ...
    def test_platform_bundle_includes_the_client_tools(self):
        ...
        assert A2A_CLIENT_TOOLS.issubset(set(resolve_toolset("renco-a2a")))
...
class TestDeferredPlatformToolPreregistration:
    ...
    def test_tools_stay_attributed_after_materialization(
        ...
        """`renco plugins list` must still credit the pre-registered tools.
    ...
    def test_partially_registered_tools_are_still_attributed(
        ...
        `renco plugins list` under-report what the process is carrying, and
    ...
    def test_failed_materialization_tears_down_pre_registered_tools(
        ...
        adapter. Attribution and the registry therefore agree at zero: `renco
