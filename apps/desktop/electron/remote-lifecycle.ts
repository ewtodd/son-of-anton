 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Renco dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Renco install on the remote (login-shell probe),
 ...
 *     `RENCO_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForRenco in.
 *
 * The minted RENCO_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^RENCO_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.renco/desktop-ssh'
...
// Resolve the remote renco executable. An EXPLICIT path is honored strictly
...
async function locateRenco(ssh, remoteRencoPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The renco binary or wrapper script
    ...
    //     the Renco version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/ewtodd/renco/issues/74411
    return candidate
  }
  ...
  if (remoteRencoPath) {
    if (await isExecutable(remoteRencoPath)) {
      return resolveLauncher(remoteRencoPath)
    }

    const err: any = new Error(
      `The Renco path you set is not an executable on the remote host: "${remoteRencoPath}". ` +
        'Check the path (it must be the full path to the `renco` binary on the remote, e.g. ' +
        '~/renco-agent/.venv/bin/renco), or clear it to auto-detect.'
    )

    err.kind = 'renco-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v renco')}`)).trim()
  ...
  candidates.push('~/.local/bin/renco')
  candidates.push('/usr/local/bin/renco')
  candidates.push('~/.renco/renco-agent/venv/bin/renco')
  ...
  const err: any = new Error(
    'Renco is not installed on the remote host (could not find a `renco` executable). ' +
      'Install it on the remote with:  curl -fsSL https://renco-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Renco path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<renco> --version`,
// e.g. "Renco Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH renco a
// connection uses, so a stale/unexpected install is visible.
async function probeRencoVersion(ssh, rencoPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(rencoPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Renco Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The RENCO_HOME the remote dashboard will use (explicit env wins, else
// ~/.renco). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteRencoHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${RENCO_HOME:-$HOME/.renco}"')).trim().split('\n').pop()

    return out || '~/.renco'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Renco home.')
...
async function listRemoteRencoProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteRencoHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Renco profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Renco home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'rencoPath', 'rencoHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !rencoPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(rencoPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/renco`, leaving neither
      // launcher nor RENCO_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `renco_home=os.path.expanduser(${shq(rencoHome)}) if ${shq(rencoHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if renco_home:\n' +
      ' expected_entries.add(os.path.join(renco_home,"renco-agent","venv","bin","renco"))\n' +
...
function buildSpawnCommand(rencoPath, profile, opts: any = {}) {
  const renco = expandRemotePath(rencoPath)
  ...
  const dashCmd =
    ...
    `exec env RENCO_DESKTOP=1 ${renco} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, rencoPath) {
  const renco = expandRemotePath(rencoPath)

  const out = await ssh.exec(
    `help="$(${renco} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { rencoPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, rencoPath))) {
    const err: any = new Error(
      'The remote Renco install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Renco on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(rencoPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForRenco, a token-gated
...
async function connect(deps) {
  ...
  const rencoPath = await locateRenco(ssh, remoteRencoPath)
  log(`located renco at ${rencoPath}`)
  const rencoVersion = await probeRencoVersion(ssh, rencoPath)

  if (rencoVersion) {
    log(`remote renco version: ${rencoVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const rencoHome = await probeRemoteRencoHome(ssh)
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.rencoPath === rencoPath &&
      lock.rencoHome === rencoHome
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForRenco(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
