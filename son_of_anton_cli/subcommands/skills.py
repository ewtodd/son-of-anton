"""``son-of-anton skills`` subcommand parser.

Extracted from ``son_of_anton_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_trust = skills_subparsers.add_parser(
        "trust",
        help="Trust a project so its repo-local skills (./.son-of-anton/skills, ./.agents/skills) load",
    )
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.son-of-anton/skills/.bundled_manifest) "
            "so future 'son-of-anton update' runs stop marking it as user-modified. Pass --restore to also "
            "replace the current copy with the bundled version."
        ),
    )
    skills_reset.add_argument(
        "name", help="Skill name to reset (e.g. arxiv)"
    )
    skills_reset.add_argument(
        "--restore",
        action="store_true",
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `son-of-anton update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `son-of-anton update` reports as user-modified and skips. "
            "Use `son-of-anton skills diff <name>` to see changes and `son-of-anton skills reset "
    ...
    skills_diff = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "running `son-of-anton skills reset`."
        ),
    )
    skills_diff.add_argument(
        "name", help="Skill name to diff (e.g. arxiv)"
    )
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`son-of-anton update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `son-of-anton update`. Pass --sync to re-seed now."
        ),
    )
