...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Renco CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Renco CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.renco/skins/mytheme.yaml
```

Or set the default skin in `~/.renco/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Renco — gold and kawaii | `Renco Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Renco Agent` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Renco Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Renco Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Renco Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Renco Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Renco Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Renco ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default RENCO_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.renco/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.renco/skins/mytheme.yaml
...
## Renco Mod — Visual Skin Editor

[Renco Mod](https://github.com/cocktailpeanut/renco-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Renco Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/renco-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Renco skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.renco/skins/`
- Activates a skin by updating `~/.renco/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/renco-mod.git
cd renco-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.renco/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Renco Mod respects the `RENCO_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `renco_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.renco/skins/` take precedence over built-in skins with the same name.
