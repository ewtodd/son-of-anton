...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_renco_home", lambda: renco_home)
    monkeypatch.setattr(uninstall, "_is_default_renco_home", lambda home: False)
    ...
    assert str(renco_home) in output
    assert project_root.exists()
    assert renco_home.exists()
