...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('renco:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('renco:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('renco:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('renco:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('renco:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('renco:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('renco:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('renco:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('renco:pet-overlay:control', payload)
  })
}
