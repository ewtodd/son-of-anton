...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `renco-dev-mock-${Date.now()}`))
  ...
  return { root, rencoHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(rencoHome, mockUrl) {
  fs.writeFileSync(
    path.join(rencoHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(rencoHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.rencoHome, mock.url)
  console.log(`  RENCO_HOME: ${sandbox.rencoHome}`)
  ...
  const env = {
    ...
    RENCO_DESKTOP_APP_NAME: `RencoDevMock-${Date.now()}`,
  }
