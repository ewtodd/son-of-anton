 ...
 *
 * The archive is the CLI's own `renco profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/renco'
...
import type { ProfileDesktopOverlay } from '@/types/renco'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Renco profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.rencoDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.rencoDesktop?.selectPaths?.({
