...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``son-of-anton gateway start/stop/restart``).
    """
    ...
    # `son-of-anton gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged son-of-anton
    ...
    detection always fails. Since every Son of Anton runtime call inside the
    container drops to son-of-anton via ``s6-setuidgid``, that silent failure
...
# in ``son_of_anton_cli.gateway`` (systemd/launchd) and ``son_of_anton_cli.gateway_windows``
...
# active (set via the ``son-of-anton -p <profile>`` flag before the call). This
...
class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``son_of_anton_cli.gateway_windows`` (Scheduled Task /
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-son-of-anton, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources SON_OF_ANTON_HOME (and any extra env) via with-contenv —
             so e.g. ``-e SON_OF_ANTON_HOME=/data/son-of-anton`` is honored at run
          ...
          4. Drops to the son-of-anton user and exec's
             ``son-of-anton -p <profile> gateway run`` (or just ``son-of-anton
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``son-of-anton gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root SON_OF_ANTON_HOME profile" (the implicit profile that exists at
        the top of $SON_OF_ANTON_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``son_of_anton_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$SON_OF_ANTON_HOME/profiles/default/`` — a
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/son-of-anton/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export SON_OF_ANTON_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's SON_OF_ANTON_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``son-of-anton gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-SON_OF_ANTON_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The SON_OF_ANTON_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own SON_OF_ANTON_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "son-of-anton gateway run --replace"
        else:
            gateway_cmd = f"son-of-anton -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid son-of-anton {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${SON_OF_ANTON_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the SON_OF_ANTON_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e SON_OF_ANTON_HOME=/data/son-of-anton``
        gets its logs under /data/son-of-anton/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``son-of-anton logs`` reads and what
        ...
        return (
            ...
            f': "${{SON_OF_ANTON_HOME:=/opt/data}}"\n'
            f'log_dir="$SON_OF_ANTON_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as son-of-anton when
            # this script starts as root. Never chown or unlink son-of-anton-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is son-of-anton-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded son-of-anton-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid son-of-anton mkdir -p "$log_dir"\n'
            f'  s6-setuidgid son-of-anton rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid son-of-anton s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``son-of-anton gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``son-of-anton -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with son-of-anton ownership
            ...
            # the son-of-anton user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is son-of-anton-owned (see ``_seed_supervise_skeleton``), and on
