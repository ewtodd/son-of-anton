...
use the notepad, and the `renco cron notepad` CLI handler.
"""
...
def cron_env(tmp_path, monkeypatch, notepad):
    """Isolated cron environment with temp RENCO_HOME (mirrors test_cron_context_from)."""
...
class TestPromptInjection:
    def test_nonempty_notepad_rendered_into_prompt(self, cron_env, notepad):
        ...
        assert f"renco cron notepad {job['id']} set" in prompt
...
class TestNotepadCli:
    ...
    def test_cli_set_get_list_delete(self, notepad, capsys):
        from renco_cli.cron import cron_notepad
