...
# readiness handshake ("Timed out connecting to Son of Anton backend after
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `son-of-anton auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `son-of-anton auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (son-of-anton auth remove copilot gh_cli) must not pay either on
        ...
        # Son of Anton auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from son_of_anton_cli.auth import resolve_qwen_runtime_credentials
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_son_of_anton_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `son-of-anton auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.son-of-anton/.env.
    ...
    try:
        from son_of_anton_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `son-of-anton auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Son of Anton
        ...
        return (
            ...
            or entry.source == "son_of_anton_pkce"
        )
