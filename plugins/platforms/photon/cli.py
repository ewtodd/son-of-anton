"""
``son-of-anton photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Son of Anton
...
# Writable sidecar runtime dir (mirrors to SON_OF_ANTON_HOME on immutable
...
# wiring imports this module for `son-of-anton --help`).
...
# when called later (gateway start, son-of-anton status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `son-of-anton photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Son of Anton Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``son-of-anton photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Son of Anton Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.son-of-anton/.env,
    ...
    try:
        from son_of_anton_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  son-of-anton gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `son-of-anton photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`son-of-anton photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.son-of-anton/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `son-of-anton photon telemetry on` / `son-of-anton photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.son-of-anton/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  son-of-anton gateway restart")
    return 0
...
#
# `son-of-anton gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``son-of-anton photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `son-of-anton gateway setup` wizard."""
