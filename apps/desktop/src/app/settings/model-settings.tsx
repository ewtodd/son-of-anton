...
import { rencoConfigCacheWriter, invalidateRencoConfig, useRencoConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `renco model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in renco_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile = null }: ModelSettingsProps) {
  ...
  const { data: config } = useRencoConfigRecord(scopeProfile)
  const setConfig = useMemo(() => rencoConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateRencoConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveRencoConfig(next, scopeProfile ?? undefined)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `renco model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Renco runs the flow for you.`}
          </p>
        )}
