"""``son-of-anton sessions`` command — extracted from ``son_of_anton_cli/main.py``.
...
Helpers that stay in ``son_of_anton_cli.main`` (``get_son_of_anton_home``,
...
``son_of_anton_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def _m():
    """Lazy ``son_of_anton_cli.main`` reference (call-time, keeps patches working)."""
    from son_of_anton_cli import main

    return main


def get_son_of_anton_home():
    return _m().get_son_of_anton_home()
...
#: Default age floor for `son-of-anton sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`son-of-anton sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from son_of_anton_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    son-of-anton sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from son_of_anton_cli.session_recovery import (
    ...
    if action == "list":
        ...
        from son_of_anton_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        # son_of_anton_cli.session_export.
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from son_of_anton_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_son_of_anton_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from son_of_anton_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_son_of_anton_home() / "session-exports"
        ...
        # `son-of-anton sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `son-of-anton sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `son-of-anton sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `son-of-anton sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: son-of-anton sessions pin <session_id>"
            )
            return
        ...
        # Launch son-of-anton --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`son-of-anton sessions optimize` later to reclaim freed space.)")
