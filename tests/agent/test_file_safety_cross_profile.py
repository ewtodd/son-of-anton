"""Tests for the cross-Renco-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Renco profile's
...
Reference: May 2026 incident — a renco-security profile session
accidentally edited skills under both ~/.renco/profiles/renco-security/skills/
AND ~/.renco/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Renco root with two profiles, monkeypatch the
...
def _set_active_home(monkeypatch, renco_home: Path):
    """Point file_safety._renco_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_renco, monkeypatch):
        _set_active_home(monkeypatch, fake_renco["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_renco, monkeypatch):
        """If RENCO_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_renco_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_renco, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_renco["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_renco["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "renco-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_renco, monkeypatch):
        """Inverse direction — default-profile session reaching into a named profile."""
        _set_active_home(monkeypatch, fake_renco["default_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_renco["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "default"
        assert result["target_profile"] == "renco-security"


    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_renco, monkeypatch, area):
        _set_active_home(monkeypatch, fake_renco["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_renco["default_home"] / area / "foo.txt"
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_renco, monkeypatch):
        _set_active_home(monkeypatch, fake_renco["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_renco, monkeypatch):
        _set_active_home(monkeypatch, fake_renco["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "renco-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_renco, monkeypatch):
        _set_active_home(monkeypatch, fake_renco["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_renco["default_home"] / "skills" / "foo" / "SKILL.md")
