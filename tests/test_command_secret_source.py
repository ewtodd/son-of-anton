...
RENCO_HOME with a config.yaml routing ``secrets.provider: command`` through
``renco_cli.env_loader._apply_external_secret_sources``.

Security invariants under test (ported from the desktop TS provider):

* the requested key travels ONLY via the ``RENCO_SECRET_KEY`` env var —
...
# ---------------------------------------------------------------------------
# Dispatch E2E through env_loader against a real temp RENCO_HOME
# ---------------------------------------------------------------------------
