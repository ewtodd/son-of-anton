"""Tests for son_of_anton_cli.build_info — baked-in build SHA resolution.

The build SHA is written by the Dockerfile's ``SON_OF_ANTON_GIT_SHA`` build-arg
into ``<project_root>/.son_of_anton_build_sha``.  These tests cover the read-side
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    ...
    from son_of_anton_cli import build_info

    missing = tmp_path / ".son_of_anton_build_sha"  # never created
...
def test_get_build_sha_respects_short_argument(tmp_path):
    ...
    sha_file = tmp_path / ".son_of_anton_build_sha"
