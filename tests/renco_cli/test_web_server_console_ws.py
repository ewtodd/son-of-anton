"""Dashboard Renco Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_renco_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from renco_cli.console_engine import ConsoleResult, RencoConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(RencoConsoleEngine, "execute", slow_execute)
