...
Write plugins for the Son of Anton desktop app: statusbar items, layout panes,
...
system (`~/.son-of-anton/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$SON_OF_ANTON_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$SON_OF_ANTON_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Son of Anton desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SON_OF_ANTON_HOME/desktop-plugins/` (usually
  `~/.son-of-anton/desktop-plugins/`).

## How to Run

1. Create `$SON_OF_ANTON_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.son-of-anton/...` by default, or `~/.son-of-anton/profiles/<profile>/...` under a
...
The ONLY import surface is `@sonofanton/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Son of Anton
  ...
  (`son-of-anton://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.son-of-anton/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@sonofanton/plugin-sdk`, `react`, and
