"""Cross-process mutual exclusion for in-flight Son of Anton updates.

Three different surfaces can start an update of the same install tree:

* ``son-of-anton update`` from a terminal,
* the dashboard's Update button (``POST /api/son-of-anton/update`` →
  ``_spawn_son_of_anton_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``son-of-anton-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``son-of-anton update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <SON_OF_ANTON_HOME>/.son-of-anton-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``son-of-anton update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Son of Anton is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``son-of-anton-setup`` binary under ``~/.son-of-anton`` is only refreshed by a full
  installer run (``copy_self_to_son_of_anton_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Son of Anton is still running") on every
  GUI update forever, with no Son of Anton process actually running.
"""
...
MARKER_NAME = ".son-of-anton-update-in-progress"

# Set by an orchestrating updater (the Tauri `son-of-anton-setup --update` flow) to
# its own pid before spawning `son-of-anton update` as a child stage. The parent
...
HANDOFF_PID_ENV = "SON_OF_ANTON_UPDATE_HANDOFF_PID"
...
# to show "Son of Anton is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Son of Anton home (never the context-local profile override):
    the Rust updater resolves ``$SON_OF_ANTON_HOME`` or the platform default, and the
    ...
    return get_process_son_of_anton_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``son-of-anton update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``son-of-anton-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Son of Anton update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `son-of-anton update` as a stage): we run under ITS claim
