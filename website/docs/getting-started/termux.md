...
description: "Run Renco Agent directly on an Android phone with Termux"
---

# Renco on Android with Termux
...
Renco Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Renco from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Renco installs are marked with install method `apt`. Renco therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade renco-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Renco runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Renco now ships a Termux-aware installer path:
...
- links `renco` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `renco` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/renco" "$PREFIX/bin/renco"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `renco` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.renco/.env`.
...
### `renco doctor` says ripgrep or Node is missing
