...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(RENCO_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Renco-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.renco/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'renco-desktop-ssh')
  }

  return path.join(os.homedir(), '.renco', 'desktop-ssh')
}
