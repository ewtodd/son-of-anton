...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__rencoTourEngine) {
    ${driverIife}
    w.__rencoTourHolder = {};
    w.__rencoTourCollect = (${collectTourTargets.toString()});
    w.__rencoTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__renco-tour-style')) {
    var style = document.createElement('style');
    style.id = '__renco-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__rencoTourEngine(
    w.driver.js.driver,
    w.__rencoTourHolder,
    ${JSON.stringify(action)},
    w.__rencoTourCollect,
    document
  ));
})()`
}
