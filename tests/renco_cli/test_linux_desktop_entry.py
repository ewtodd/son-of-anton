"""Tests for the Linux XDG desktop entry installed by ``renco desktop``."""
...
def test_install_writes_entry_with_absolute_exec_and_icon(tmp_path, xdg_home, monkeypatch):
    ...
    renco_bin = tmp_path / "bin" / "renco"
    renco_bin.parent.mkdir()
    renco_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "renco_cli.relaunch.resolve_renco_bin", lambda: str(renco_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)

    assert entry == xdg_home / "applications" / "renco.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `renco` would not resolve.
    assert values["Exec"] == f"{renco_bin} desktop"
    ...
    assert values["Name"] == "Renco"
    assert values["Terminal"] == "false"


def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("renco_cli.relaunch.resolve_renco_bin", lambda: "/usr/bin/renco")
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("renco_cli.relaunch.resolve_renco_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m renco_cli.main desktop")
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when Renco is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "renco"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "renco"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("renco_cli.relaunch.resolve_renco_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
