...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Renco Agent. Use 'renco debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    renco debug share              Upload debug report (asks for confirmation)
    renco debug share --yes        Skip confirmation (for scripts/CI)
    renco debug share --lines 500  Include more log lines
    renco debug share --expire 30  Keep paste for 30 days
    renco debug share --local      Print report locally (no upload)
    renco debug share --no-redact  Disable upload-time secret redaction
    renco debug share --nous       Upload to Nous-internal storage (private)
    renco debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'renco debug share'",
    )
