"""renco webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.renco/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _renco_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: renco gateway run
"""
...
def webhook_command(args):
    """Entry point for 'renco webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: renco webhook {subscribe|list|remove|test}")
        print("Run 'renco webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (renco gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: renco webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from renco webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (renco gateway run)")
