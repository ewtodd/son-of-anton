...
def renco_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves RENCO_HOME.
...
@pytest.fixture()
def server(renco_home, monkeypatch):
    ...
    # Pin config resolution to the isolated RENCO_HOME. Sibling test
    ...
    # module-level ``_renco_home = get_renco_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_renco_home", renco_home)
...
def test_moa_bare_returns_usage(server, session, renco_home):
    _write_moa_config(renco_home, """
moa:
