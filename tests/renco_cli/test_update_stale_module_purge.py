"""Tests for _purge_stale_renco_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `renco update`.

Field failure (2026-08-20, Teknium's Linux box): `renco update` pulled a
checkout where renco_cli/gateway.py newly imports `line_input` from
renco_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from renco_cli.gateway
...
the Renco package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Renco modules from the cache; later
...
def test_purge_evicts_renco_prefixed_modules():
    victims = [
        "renco_cli.cli_output",
        "renco_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_renco_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_renco_modules()
    assert sys.modules.get("renco_cli.update_cmd") is update_cmd
    assert sys.modules.get("renco_cli.main") is cli_main
    assert "renco_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["renco_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_renco_modules()
    finally:
        sys.modules.pop("renco_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from renco_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
