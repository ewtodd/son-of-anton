...
entries via ``renco mcp catalog`` or the interactive ``renco mcp picker``,
and install them with ``renco mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into renco-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``renco mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.renco/.env`` (the
...
from renco_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.renco/.env.
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Renco understands version {_MANIFEST_VERSION})"
        )
    ...
    if t_type == "http" and a_type == "api_key":
        ...
        # at connect time.
        from renco_cli.mcp_config import _env_key_for_server
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Renco is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Renco
        understands. Update Renco to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_renco_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.renco/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.renco/.env via save_env_value()."""
...
def _build_server_config(
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from renco_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from renco_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``renco mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `renco mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `renco mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `renco mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `renco mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `renco auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`renco auth {entry.auth.provider}` if you have not "
    ...
    from renco_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new Renco session to load its tools.",
        Colors.GREEN,
    ))
