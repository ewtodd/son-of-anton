---
sidebar_position: 7
title: "Use SOUL.md with Renco"
description: "How to use SOUL.md to shape Renco Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Renco

`SOUL.md` is the **primary identity** for your Renco instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Renco to feel like the same assistant every time you talk to it — or if you want to replace the Renco persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Renco should be
- what Renco should avoid stylistically
- how Renco should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Renco is and how Renco speaks
...
Renco now uses only the global SOUL file for the current instance:

```text
~/.renco/SOUL.md
```

If you run Renco with a custom home directory, it becomes:
...
Renco automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Renco does not overwrite it
- if the file exists but is empty, Renco adds nothing from it to the prompt

## How Renco uses it

When Renco starts a session, it reads `SOUL.md` from `RENCO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Renco feels.
...
Renco already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Renco should sound.

# Avoid
What Renco should not do.

# Defaults
How Renco should behave when ambiguity appears.
```
...
Then restart Renco or start a new session.
...
### I edited SOUL.md but Renco still sounds the same

Check:
- you edited `~/.renco/SOUL.md` or `$RENCO_HOME/SOUL.md`
...
### Renco is ignoring parts of my SOUL.md
