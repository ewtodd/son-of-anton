"""`son-of-anton checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.son-of-anton/checkpoints/``.  Actions:

    son-of-anton checkpoints               # same as `status`
    son-of-anton checkpoints status        # total size, project count, breakdown
    son-of-anton checkpoints list          # per-project checkpoint counts + workdir
    son-of-anton checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    son-of-anton checkpoints clear [-f]    # nuke the entire base (asks first)
    son-of-anton checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    son-of-anton checkpoints
    son-of-anton checkpoints prune --retention-days 3 --max-size-mb 200
    son-of-anton checkpoints clear -f
...
from son_of_anton_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: son-of-anton checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``son-of-anton checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `son-of-anton checkpoints` → status
