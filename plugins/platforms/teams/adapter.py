"""
Microsoft Teams platform adapter for Son of Anton Agent.
...
Requires:
    ...
    manually: ``<son-of-anton-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    Son of Anton owns dotenv loading; third-party import side effects must not.
    """
    try:
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Son of Anton"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with create_ssrf_safe_async_client(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; SonOfAntonAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        son_of_anton_action = data.get("son_of_anton_action", "")
        session_key = data.get("session_key", "")

        if not son_of_anton_action or not session_key:
        ...
        choice = choice_map.get(son_of_anton_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="son_of_anton_approve",
            data={**btn_data_base, "son_of_anton_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="son_of_anton_approve",
                data={**btn_data_base, "son_of_anton_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="son_of_anton_approve",
                    data={**btn_data_base, "son_of_anton_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="son_of_anton_approve",
            data={**btn_data_base, "son_of_anton_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Son of Anton\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.son-of-anton/.env")
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual Son of Anton
