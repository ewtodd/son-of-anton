...
@pytest.fixture()
def isolated_son_of_anton_home():
    """Per-test SON_OF_ANTON_HOME dir (already redirected by the autouse conftest
    fixture) as a Path, with the raw-config cache cleared around the test."""
    from pathlib import Path

    import son_of_anton_cli.config as config_mod

    home = Path(os.environ["SON_OF_ANTON_HOME"])
...
def test_freshness_after_config_edit(isolated_son_of_anton_home):
    from son_of_anton_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_son_of_anton_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_son_of_anton_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_son_of_anton_home):
    from son_of_anton_cli.config import read_raw_config_readonly

    cfg = isolated_son_of_anton_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
