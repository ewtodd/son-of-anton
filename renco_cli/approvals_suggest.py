"""``renco approvals suggest`` — mine approval history into allowlist proposals.

Renco has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.renco/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, renco self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/renco home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_renco_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_renco_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  renco approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.renco/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``renco approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``renco approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from renco_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: renco approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `renco approvals <subcommand> -h` for details."
    )
    return 1
