...
from tools.environments.local import _SON_OF_ANTON_PROVIDER_ENV_FORCE_PREFIX
...
class TestOrphanedPipeReconciliation:
    ...
    `son-of-anton update` in Feishu spawned a background subprocess that restarted
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with patch.dict(os.environ, {
            ...
            registry.spawn_local(
                ...
                env_vars={
                    ...
                    f"{_SON_OF_ANTON_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN": "forced-bot-token",
                },
            )
        ...
        assert f"{_SON_OF_ANTON_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN" not in env
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch("tools.process_registry.time.sleep", return_value=None), \
            ...
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/son_of_anton_bg.log",
                "/path with spaces/son_of_anton_bg.pid",
                "/path with spaces/son_of_anton_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/son_of_anton_bg.log' 2>/dev/null"
        assert env.commands[1][0] == "kill -0 \"$(cat '/path with spaces/son_of_anton_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        assert env.commands[2][0] == "cat '/path with spaces/son_of_anton_bg.exit' 2>/dev/null"
...
class TestCheckpoint:
    ...
    def test_recover_dead_wrapper_retries_unreaped_systemd_scope(
        ...
        entry = {
            ...
            "systemd_unit": "son-of-anton-worker-proc_dead_scope.scope",
        }
...
class TestSystemdCgroupIsolation:
    ...
    def test_wraps_in_systemd_scope_when_supervisor_and_available(
        ...
        wrapped in ``systemd-run --user --scope --unit=son-of-anton-worker-<id>``."""
        ...
        assert argv[unit_idx + 1].startswith("son-of-anton-worker-"), argv
        assert argv[unit_idx + 1] == f"son-of-anton-worker-{session.id}", (
            argv
        ...
        assert session.systemd_unit == f"son-of-anton-worker-{session.id}.scope"
    ...
    def test_inherited_gateway_tree_markers_do_not_scope_child_cli(
        ...
        _SON_OF_ANTON_GATEWAY is inherited (and set by importing gateway.run), so
    ...
    def test_systemd_post_spawn_failure_never_kills_gateway_process_group(
        ...
        assert stop_unit.call_args.args[0].startswith("son-of-anton-worker-proc_")
    ...
    def test_pty_spawn_failure_reaps_scope_before_distinct_pipe_fallback(
        ...
        assert stopped_unit == f"son-of-anton-worker-{session.id}.scope"
        unit_idx = fallback_argv.index("--unit")
        assert fallback_argv[unit_idx + 1] == (
            f"son-of-anton-worker-{session.id}-pipe-fallback"
        )
        assert session.systemd_unit == (
            f"son-of-anton-worker-{session.id}-pipe-fallback.scope"
        )
    ...
    def test_kill_recovered_detached_already_exited_stops_persisted_scope(
        ...
        session.systemd_unit = "son-of-anton-worker-proc_recovered_scope.scope"
        registry._running[session.id] = session
        ...
        assert result["status"] == "already_exited"
        assert stopped == ["son-of-anton-worker-proc_recovered_scope.scope"]
        assert terminated == []
        assert session.exited is True
    ...
    def test_stop_systemd_unit_treats_absent_unit_as_clean(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda *args, **kwargs: subprocess.CompletedProcess(
                args=args[0],
                returncode=5,
                stderr=b"Unit son-of-anton-worker-gone.scope not loaded.\n",
            ),
        )

        assert pr._stop_systemd_unit("son-of-anton-worker-gone.scope") is True
