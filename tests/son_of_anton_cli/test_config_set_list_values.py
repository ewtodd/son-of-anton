"""``son-of-anton config set`` must parse list/mapping literals, not store them as strings.

Before this fix, ``son-of-anton config set platform_toolsets.discord '["file","web"]'``
...
def user_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("SON_OF_ANTON_MANAGED_DIR", raising=False)
...
def test_list_literal_is_parsed_to_list(user_home):
    from son_of_anton_cli.config import set_config_value, read_raw_config
...
def test_round_trip_through_load_config(user_home):
    ...
    from son_of_anton_cli.config import set_config_value, load_config
