...
# (%RENCO_HOME%\renco-setup.exe). That binary has no self-update path --
# copy_self_to_renco_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `renco update` refreshes
...
#     -InstallRoot <path>   repo checkout (RENCO_HOME\renco-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Renco.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .renco-update-in-progress in place (tests)
#
...
# .renco-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim RENCO_HOME\.renco-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining RENCO_UPDATE_STARTED_AT from the Desktop hand-off.
# renco_cli/update_lock.py's ancestry rule lets our
# `renco update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Renco.exe pid.
try {
    Add-Type -Namespace RencoHandoff -Name Win32 -MemberDefinition @'
...
$RencoHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $RencoHome ".renco-update-in-progress"
$LogDir = Join-Path $RencoHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $RencoHome ".renco-update-result.json"
$script:Ui = $null
$script:UiStage = "Renco will open once done."   # until the first gate; matches ui.html
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-RencoStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $edge) {
        ...
        if ($server) {
            try {
                ...
                $edgeProfile = Join-Path $TempDir ("renco-update-ui-{0}" -f $PID)
                $edgeArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [RencoHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `renco debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"renco debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [RencoHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Renco.exe before writing the replacement,
    ...
    # DO NOT spawn Renco.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [RencoHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [RencoHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [RencoHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Renco process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [RencoHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `renco update` deliberately runs its build steps with stdout
# inherited (renco_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:RENCO_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:RENCO_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
function Invoke-RencoStep([string]$Exe, [string[]]$RencoArgs, [string]$Tag) {
    ...
    # afterwards, where `renco debug share` picks it up.
    #
    ...
    # .renco-update-result.json, clearing .renco-update-in-progress,
    ...
    # Renco" until the user killed something by hand. Losing the tail of a
    ...
    $psi.Arguments = ($RencoArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    $psi.UseShellExecute = $false
    $psi.RedirectStandardOutput = $true
    $psi.RedirectStandardError = $true
    # renco update prints UTF-8 (checkmarks, arrows, box glyphs). PS 5.1
...
# the page can be QA'd in any browser; RENCO_SELFTEST_FAIL=1 exercises the
# error state, RENCO_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:RENCO_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:RENCO_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:RENCO_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-RencoStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Renco install to
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:RENCO_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:RENCO_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:RENCO_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:RENCO_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "renco-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "renco-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "renco-pipe-drain-$stamp.pid"
...
'@
    ...
    # merely loud. `renco update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-RencoStep $powershell @(
    ...
    $flood = Invoke-RencoStep $powershell @(
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:RENCO_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Renco to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Renco window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Renco fully and try again."
    ...
    Publish-UiProgress "Preparing Renco files"
    $shim = Join-Path $InstallRoot "venv\Scripts\renco.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Renco install open (venv\Scripts\renco.exe locked after 20s). Nothing was changed. Close other Renco windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the renco.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\renco.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_renco_exe must first rename the running renco.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that renco.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Renco window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Renco.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m renco_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Renco installer or `renco doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "renco_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Renco windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m renco_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed renco predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-RencoStep $pythonExe $updateArgs "update"
    Write-HandoffLog "renco update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all Renco windows") is not retryable.
    ...
    # `renco update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "renco update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-RencoStep $pythonExe @("-m", "renco_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `renco desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `renco debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Renco is back",
    ...
    if ($finalCode -ne 0) {
        ...
        Publish-UiProgress "Opening Renco"
        ...
        if (-not $cameBack -and $RelaunchExe) {
            ...
            $finalMsg = "Update complete. Reopen Renco to finish (it could not restart itself)."
