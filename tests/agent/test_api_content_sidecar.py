...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("son_of_anton_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated SON_OF_ANTON_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="son_of_anton_api_content_")
