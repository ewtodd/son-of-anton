...
CHECKPOINT_PATH = get_renco_home() / "processes.json"
...
# ---------------------------------------------------------------------------
# When Renco runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
#
# Wrapping the spawn in ``systemd-run --user --scope --unit=renco-worker-<pid>``
        ...
        if not _IS_WINDOWS:
            try:
                ...
                if binary:
                    ...
                    probe_unit = f"renco-probe-scope-{os.getpid()}-{uuid.uuid4().hex[:8]}"
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised Renco gateway runtime.

    Both supervisor markers and ``_RENCO_GATEWAY`` are inherited by every
...
def _build_systemd_scope_argv(
    ...
    if binary is None:
        ...
        return shell_argv
    unit_name = f"renco-worker-{unit_suffix}"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from renco_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_local(
        ...
        if use_pty:
            ...
            try:
                ...
                if pty_use_systemd_scope:
                    ...
                    session.systemd_unit = f"renco-worker-{session.id}.scope"
        ...
        if use_systemd_scope:
            ...
            session.systemd_unit = f"renco-worker-{unit_suffix}.scope"
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/renco_bg_{session.id}.log"
        pid_path = f"{temp_dir}/renco_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/renco_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``renco_cli.goals``) to support
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `renco update` restarting the gateway) is still
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Renco desktop app.",
            }
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.renco/processes.json with the raw command
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Renco'}"
    elif _reason == "lost":
