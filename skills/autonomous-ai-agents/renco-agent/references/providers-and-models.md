...
Set via `renco model` (picker) or `renco setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://renco-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `renco auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `renco auth add openai-codex` |
| qwen-oauth | OAuth | `renco auth add qwen-oauth` |
| minimax-oauth | OAuth | `renco auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`renco auth`).
Fallback chain when the primary fails: `renco fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`renco_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   renco config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
