...
renco_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `renco config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_renco_home = get_renco_home()`` at import time (module
    singleton), so monkeypatching RENCO_HOME after import doesn't move it.
    ...
    this only matters for tests that swap RENCO_HOME.)
    """
    import cli

    cli._renco_home = home
