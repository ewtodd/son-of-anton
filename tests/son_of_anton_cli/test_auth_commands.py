...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from son_of_anton_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "son-of-anton" / "auth.json").read_text())
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "son_of_anton_cli.auth._nous_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "son-of-anton-cli",
    ...
    # `son-of-anton auth add nous` must also populate providers.nous so the
    ...
    # raises "Son of Anton is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`son-of-anton auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``son-of-anton auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``son-of-anton auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("son_of_anton_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from son_of_anton_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from son_of_anton_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """son-of-anton auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``son-of-anton auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "son-of-anton" / "auth.json").read_text(encoding="utf-8"))
...
def test_clear_provider_auth_removes_provider_pool_entries(tmp_path, monkeypatch):
                ...
                "anthropic": [
                    {
                        ...
                        "auth_type": "oauth",
                        "priority": 0,
                        "source": "manual:son_of_anton_pkce",
    ...
    from son_of_anton_cli.auth import clear_provider_auth
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`son-of-anton logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (son_of_anton_home / "config.yaml").read_text()
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from son_of_anton_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Son of Anton reads from has a
...
def test_seed_from_singletons_respects_son_of_anton_pkce_suppression(tmp_path, monkeypatch):
    """anthropic son_of_anton_pkce must not re-seed from ~/.son-of-anton/.anthropic_oauth.json when suppressed."""
    son_of_anton_home = tmp_path / "son-of-anton"
    son_of_anton_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("SON_OF_ANTON_HOME", str(son_of_anton_home))

    import yaml
    (son_of_anton_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (son_of_anton_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["son_of_anton_pkce"]},
    }))

    # Stub the readers so only son_of_anton_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_son_of_anton_oauth_credentials", lambda: {
    ...
    # son_of_anton_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "son_of_anton_pkce" not in active
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.son-of-anton/.anthropic_oauth.json",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from son_of_anton_cli.auth import is_source_suppressed
    from son_of_anton_cli.auth_commands import auth_remove_command

    with patch(
        "son_of_anton_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "son_of_anton_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_remove_env_seeded_dotenv_with_bom_no_shell_hint(tmp_path, monkeypatch, capsys):
    ...
    (son_of_anton_home / ".env").write_bytes(
