...
``son-of-anton*.exe`` shims aside (``son-of-anton.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``son-of-anton`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("son-of-anton", "son-of-anton", "son-of-anton-acp", "son-of-anton-gateway"):
...
# ---------------------------------------------------------------------------
# son_of_anton_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "son-of-anton.exe" in names, "son-of-anton.exe must be restored after a no-op install"
    assert "son-of-anton-acp.exe" in names
    assert "son-of-anton-gateway.exe" in names
    assert (scripts / "son-of-anton.exe").read_bytes() == b"MZ-old-son-of-anton"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "son-of-anton.exe").read_bytes() == b"MZ-new-son-of-anton"
...
# ---------------------------------------------------------------------------
# son_of_anton_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "son-of-anton.exe" in names, "son-of-anton.exe must be restored after a no-op recovery install"
    assert (scripts / "son-of-anton.exe").read_bytes() == b"MZ-old-son-of-anton"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "son-of-anton.exe").write_bytes(b"MZ-new-son-of-anton")
