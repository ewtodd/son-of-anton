...
import type { SessionInfo } from '@/renco'
...
// zero Renco sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, RencoGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, RencoGitWorktree[]>>({})
