"""``son-of-anton pause`` / ``son-of-anton resume`` — the global emergency stop.

``son-of-anton pause`` writes the ESTOP sentinel at ``$SON_OF_ANTON_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``son-of-anton resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Son of Anton paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `son-of-anton resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Son of Anton resumed — dispatch picks up on the next tick.")
    else:
        print(f"Son of Anton is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`son-of-anton resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `son-of-anton pause`",
