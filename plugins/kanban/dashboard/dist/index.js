/**
 * Renco Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.renco/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__RENCO_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__RENCO_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "renco-kanban-dot-triage",
    todo: "renco-kanban-dot-todo",
    ready: "renco-kanban-dot-ready",
    running: "renco-kanban-dot-running",
    blocked: "renco-kanban-dot-blocked",
    review: "renco-kanban-dot-review",
    done: "renco-kanban-dot-done",
    archived: "renco-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-renco-task";
  ...
  const DOCS_URL = "https://renco-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://renco-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "renco.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="renco-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])renco-kanban-md-code\1/i.test(attrs)) {
      return ' class="renco-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "renco-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "renco-kanban-md",
  ...
  // standard `drop` event and our `renco-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("renco-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("renco-kanban-column--drop");
          if (target) target.classList.add("renco-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("renco-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("renco-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("renco-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__RENCO_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "renco-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "renco-kanban-attention",
        "renco-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "renco-kanban-attention-bar" },
        h("span", { className: "renco-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "renco-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "renco-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "renco-kanban-attention-row",
                  "renco-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "renco-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "renco-kanban-attention-row-id" }, task.id),
                h("span", { className: "renco-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "renco-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "renco-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "renco-kanban-diag-action-btn",
      action.suggested ? "renco-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " renco-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".renco-kanban-drawer-comment-row input, .renco-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "renco-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("renco-kanban-diag", sevClass) },
      h("div", { className: "renco-kanban-diag-header" },
        h("span", { className: "renco-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "renco-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "renco-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "renco-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "renco-kanban-diag-data-row" },
                  h("span", { className: "renco-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "renco-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "renco-kanban-diag-data-row" },
                h("span", { className: "renco-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "renco-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "renco-kanban-diag-reassign-row" },
            h("span", { className: "renco-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "renco-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "renco-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "renco-kanban-diag-msg",
              msg.ok ? "renco-kanban-diag-msg--ok" : "renco-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "renco-kanban-section" },
      h("div", { className: "renco-kanban-section-head-row" },
        h("span", { className: "renco-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "renco-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "renco-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "renco-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "renco-kanban-docs-link",
      title: "Open Renco Kanban docs in a new tab",
      "aria-label": "Renco Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `renco kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "renco-kanban-msg-ok" : "renco-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "renco-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "renco-kanban-boardswitcher" },
      h("div", { className: "renco-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "renco-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "renco-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "renco-kanban-dialog-title" },
        ...
        h("div", { className: "renco-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Renco profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "renco-kanban-bulk" },
      h("span", { className: "renco-kanban-bulk-count" },
      ...
      h("div", { className: "renco-kanban-bulk-priority",
      ...
      h("div", { className: "renco-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Renco profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "renco-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("renco-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("renco-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "renco-kanban-trash",
        dragOver ? "renco-kanban-trash--drop" : "",
        props.draggingTaskId ? "renco-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "renco-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "renco-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".renco-kanban-card")) return true;
      if (target.closest && target.closest(".renco-kanban-column-add")) return true;
      if (target.closest && target.closest(".renco-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("renco-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("renco-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".renco-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "renco-kanban-columns",
        isScrollable ? "renco-kanban-columns--scrollable" : "",
        isPanning ? "renco-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("renco-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("renco-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "renco-kanban-column",
        dragOver ? "renco-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "renco-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "renco-kanban-col-check",
        ...
        h("span", { className: cn("renco-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "renco-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "renco-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "renco-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "renco-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "renco-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "renco-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "renco-kanban-lane" },
                  h("div", { className: "renco-kanban-lane-head" },
                    h("span", { className: "renco-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "renco-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "renco-kanban-card--stale-red";
    if (age >= tier.amber) return "renco-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".renco-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "renco-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "renco-kanban-card",
        props.selected ? "renco-kanban-card--selected" : "",
        props.failed ? "renco-kanban-card--failed" : "",
        props.draggingSource ? "renco-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "renco-kanban-card-content" },
          h("div", { className: "renco-kanban-card-row" },
            h("label", {
              className: "renco-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "renco-kanban-card-check",
            ...
            h("span", { className: "renco-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or renco kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "renco-kanban-warning-badge",
                    "renco-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "renco-kanban-needs-assignee",
          ...
          h("div", { className: "renco-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "renco-kanban-card-row renco-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "renco-kanban-assignee",
                            title: `Assigned to Renco profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "renco-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "renco-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "renco-kanban-ago",
  ...
  // centered modal reusing the renco-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "renco-kanban-dialog renco-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Renco profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Renco profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "renco-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "renco-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "renco-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "renco-kanban-drawer-close",
        ...
        data ? h("div", { className: "renco-kanban-drawer-comment-foot" },
          h("div", {
            className: "renco-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "renco-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "renco-kanban-section" },
      h("div", { className: "renco-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "renco-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "renco-kanban-drawer-body" },
      h("div", { className: "renco-kanban-drawer-title" },
        h("span", { className: cn("renco-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "renco-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "renco-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "renco-kanban-section" },
            h("div", { className: "renco-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "renco-kanban-section" },
            h("div", { className: "renco-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "renco-kanban-done-no-result renco-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "renco-kanban-section" },
            h("div", { className: "renco-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "renco-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "renco-kanban-section" },
        h("div", { className: "renco-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "renco-kanban-comment" },
            h("div", { className: "renco-kanban-comment-head" },
              h("span", { className: "renco-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "renco-kanban-diag-action-btn",
      ...
      h("div", { className: "renco-kanban-section" },
        h("div", { className: "renco-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "renco-kanban-comment" },
                h("div", { className: "renco-kanban-comment-head" },
                  h("span", { className: "renco-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "renco-kanban-comment-ago" },
      ...
      h("div", { className: "renco-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "renco-kanban-event",
              isDiag ? "renco-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "renco-kanban-event-header" },
                  h("span", { className: "renco-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "renco-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "renco-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "renco-kanban-event-header-plain" },
                  h("span", { className: "renco-kanban-event-kind" }, e.kind),
                  h("span", { className: "renco-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "renco-kanban-event-phantom-row" },
                  h("span", { className: "renco-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "renco-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "renco-kanban-event-payload" },
    ...
    return h("div", { className: "renco-kanban-section" },
      h("div", { className: "renco-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "renco-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `renco-kanban-run--${r.outcome || r.status || "ended"}`
          : "renco-kanban-run--active";
        return h("div", { key: r.id, className: cn("renco-kanban-run", outcomeClass) },
          h("div", { className: "renco-kanban-run-head" },
            h("span", { className: "renco-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "renco-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "renco-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "renco-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "renco-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "renco-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "renco-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "renco-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "renco-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "renco-kanban-pre renco-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "renco-kanban-meta-row" },
      h("span", { className: "renco-kanban-meta-label" }, props.label),
      h("span", { className: "renco-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "renco-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "renco-kanban-meta-row" },
        h("span", { className: "renco-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "renco-kanban-meta-value renco-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "renco-kanban-meta-row" },
        h("span", { className: "renco-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "renco-kanban-meta-value renco-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "renco-kanban-meta-row" },
        h("span", { className: "renco-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "renco-kanban-meta-value renco-kanban-editable",
    ...
    return h("div", { className: "renco-kanban-meta-row" },
      h("span", { className: "renco-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "renco-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "renco-kanban-section" },
      h("div", { className: "renco-kanban-section-head-row" },
        h("span", { className: "renco-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "renco-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "renco-kanban-section" },
      h("div", { className: "renco-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "renco-kanban-deps-row" },
        h("span", { className: "renco-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "renco-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "renco-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "renco-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "renco-kanban-dep-chip-x",
      ...
      h("div", { className: "renco-kanban-deps-row" },
        h("span", { className: "renco-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "renco-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "renco-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "renco-kanban-section" },
      ...
      h("div", { className: "renco-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "renco-kanban-home-sub renco-kanban-home-sub--on"
              : "renco-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__RENCO_PLUGINS__ && typeof window.__RENCO_PLUGINS__.register === "function") {
    window.__RENCO_PLUGINS__.register("kanban", KanbanPage);
  }
})();
