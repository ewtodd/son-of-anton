...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Son of Anton process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    return resolve_timeout(
        ...
        env_var="SON_OF_ANTON_CONCURRENT_TOOL_TIMEOUT_S",
    )
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Son of Anton process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Son of Anton policy and dispatch exactly once."""
    from agent import relay_tools
    from son_of_anton_cli.middleware import (
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state["dispatched"]:
                raise RuntimeError(
                    "Son of Anton tool execution callback invoked more than once"
                )
    ...
    def _son_of_anton_pipeline(relay_args: dict[str, Any]) -> Any:
